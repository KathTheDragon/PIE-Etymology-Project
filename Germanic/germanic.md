# Proto-Germanic
[EDPG]: ../bibliography.md#Kroonen-2013 "Etymological Dictionary of Proto-Germanic (Kroonen 2013)"

## A

### <a id="aba">\*aba "(away) from, off" (prep, adv)</a>
E: \
-- Go. _af_ (prep) \
N: \
-- ON _af_ \
-- Far. _av_ \
-- Elfd. _av_ \
W: \
-- OE _of_, E _of_ (prep) \
-- OFri. _of_, _af_ \
-- OS _af_ (prep) \
-- Du. _af_ (adv) \
-- OHG _aba_, _ab_, G _ab_ \
REF: [EDPG]\: 1

### <a id="aban">\*aban- [m.] "man, husband" (n)</a>
E: \
-- Go. _aba_ [m.] "husband, man" \
N: \
-- ON _afi_ [m.] "grandfather; man" \
-- Far. _abbi_ [m.] "grandfather" \
REF: [EDPG]\: 1

### <a id="abnjan">\*abnjan- [wk.] "to execute" (v)</a>
N: \
-- ON _efna_ "to perform, fulfil" \
-- Far. _evna_ "to fulfil" \
W: \
-- OE _efnan_ "to perform, execute, labor" \
REF: [EDPG]\: 1

### <a id="abra">\*abra- "fierce" (adj)</a>
E: \
-- Go. _abrs_ "great, severe" \
REF: [EDPG]\: 1

### <a id="abuha">\*abuha- "turned the wrong way" (adj)</a>
N: \
-- ON _ǫfugr_ "wrong, turned backwards" \
-- Far. _øvigur_, _øvugur_ "backwards" \
-- Elfd. _avun_ "id." (adv) \
W: \
-- OS _abuh_ "inverted, wicked" \
-- OHG _abuh_ "wrong, bad, wicked", MHG _ebich_ "wrong, evil" \
REF: [EDPG]\: 2

### <a id="adesan">\*adesan- [m.] "adze" (n)</a>
W: \
-- OE _adesa_, _eadesa_ [m.], _adese_ [f.], E _adze_ \
REF: [EDPG]\: 2

### <a id="afera">\*afera- "behind, later" (prep, adv)</a>
E: \
-- Go. _afar_ "after, according to" (prep) \
W: \
-- OHG _avar_, _afar_ "but" (adv, conj), MHG _aver_, _aber_ "then again, but" (adv, conj), G _aber_ "but" (conj) \
REF: [EDPG]\: 2

### <a id="afla-1">\*afla- [n.] "strength, power" (n)</a>
N: \
-- ON _afl_ \
-- Far. _alv_ \
W: \
-- OE _afol_ \
REF: [EDPG]\: 2

### <a id="afla-2">\*afla- [m.] "hearth" (n)</a>
N: \
-- ON _afl_ \
-- Far. _alvur_, _alvi_ "fireplace; forge" \
REF: [EDPG]\: 3

### <a id="afteri">\*after(i) "behind" (prep, adv)</a>
N: \
-- ON _eftir_ \
-- Far. _eftir_ \
W: \
-- OE _æfter_ (prep), E _after_ \
-- OFri. _efter_ \
-- Du. _achter_ (prep) \
-- OHG _aftar_ (prep), MHG _after_ (prep) \
REF: [EDPG]\: 3

### <a id="agalaitja">\*agalaitja- "licentious" (adj)</a>
W: \
-- OS _agaleto_ "eagerly, diligently" (adv) \
-- OHG _agaleizi_ "importunate" \
REF: [EDPG]\: 3

### <a id="agan">\*agan- [pp.] "to fear" (v)</a>
E: \
-- Go. _ogan_, 3sg. _og_ \
REF: [EDPG]\: 3

### <a id="agana">\*agana- [n.] "bait" (n)</a>
N: \
-- ON _agn_ \
-- Far. _agn_ \
REF: [EDPG]\: 3

### <a id="agiþō">\*agiþō- [f.] "harrow" (n)</a>
W: \
-- OE _egede_ \
-- OFri. _eide_ \
-- OS _egitha_ \
-- MDu. _egede_, _eegde_, _eg_ \
-- OHG _egida_, MHG _egede_, _egde_ \
REF: [EDPG]\: 4

### <a id="agiz">\*agiz- [n.] "fear" (n)</a>
E: \
-- Go. _agis_ \
W: \
-- OE _ege_ [m.] \
REF: [EDPG]\: 4

### <a id="agjō">\*agjō- [f.] "edge, blade" (n)</a>
N: \
-- ON _egg_ "edge" \
W: \
-- OFri. _edze_ [m.] \
-- OS _eggia_ "edge, sword" \
-- OHG _ecka_, _egga_ "edge, point, corner", G _Ecke_ [f.] "corner, edge" \
REF: [EDPG]\: 4

### <a id="agōn">\*ag/kkōn- [f.] "magpie" (n)</a>
W: \
-- OE _age_, _agu_ \
-- WFri. _akke_ [c.] \
-- OHG _aga_(?), MHG _age_ \
REF: [EDPG]\: 4

### <a id="agla">\*agla- "painful" (adj)</a>
E: \
-- Go. _agls_\* "disgraceful" \
REF: [EDPG]\: 4

### <a id="aglu">\*aglu- "difficult" (adj)</a>
E: \
-- Go. _aglus_\* "difficult, toilsome" \
W: \
-- OE _egle_ "difficult", E obs. _ail_ "loathsome" \
REF: [EDPG]\: 5

### <a id="agra">\*agra- [m.] "flood" (n)</a>
W: \
-- OE _eagor_ "sea, flood", E _eagre_ "tidal bore" \
-- Du. _agger_ [c.] "sea swell during low tide" \
REF: [EDPG]\: 5

### <a id="ahanō">\*ahanō- ~ \*aganō- [f.] "chaff, awns" (n)</a>
E: \
-- Go. _ahana_ "chaff" \
N: \
-- ON _ǫgn_ "id." \
-- Far. _øgn_ "awn, beard of barley" \
-- Elfd. _aungen_ "id." \
W: \
-- OE _ægnan_ [f.pl.] "awns, chaff, refuse", E _awns_ \
-- OHG _agana_ "chaff, awn, straw", G _Ahne_ "fiber from hemp or flax" \
REF: [EDPG]\: 5

### <a id="ahila">\*ahila- ~ \*agila- [m.] "awn" (n)</a>
W: \
-- OE _egl_ [f.] "mote", ME _eile_ "awn, prickle" \
-- OHG _ahil_, G dial. _Achel_ [f.] \
REF: [EDPG]\: 5

### <a id="ahiz">\*ahiz- [n.] "ear" (n)</a>
E: \
-- Go. _ahs_ \
N: \
-- ON _ax_ \
-- Far. _aks_ \
W: \
-- OE _ēar_, _æhher_, _eher_, E _ear_ \
-- OFri. _ār_ pl. "harvest" \
-- Du. _aar_ [c.] \
-- OHG _ahar_, _ehir_, G _Ähre_ [f.] \
REF: [EDPG]\: 5

### <a id="ahjan">\*ahjan- [wk.] "to think" (v)</a>
E: \
-- Go. _ahjan_ "to assume, think" \
REF: [EDPG]\: 6

### <a id="ahslō">\*ahslō- [f.] "shoulder, armpit" (n)</a>
N: \
-- ON _ǫxl_ "shoulder" \
-- Far. _øksl_ "id." \
W: \
-- OE _eaxl_ "armpit" \
-- OS _ahsla_ "shoulder" \
-- OHG _ahsala_, _ahsla_ "id." \
REF: [EDPG]\: 6

### <a id="ahsō">\*ahsō- [f.] "axle" (n)</a>
N: \
-- Nw. dial. _oks_ "tip of the hub" \
W: \
-- OE _eax_ [m.] \
-- MDu. _asse_, Du. _as_ [c.] \
-- OHG _ahsa_, G _Achse_ \
REF: [EDPG]\: 6

### <a id="ahtau">\*ahtau "eight" (num)</a>
E: \
-- Go. _ahtau_ \
N: \
-- ON _atta_ \
-- Far. _atta_ \
-- Elfd. _atta_ \
W: \
-- OE _eahta_, E _eight_ \
-- OFri. _achta_ \
-- OS _ahto_ \
-- Du. _acht_ \
-- OHG _ahto_, G _acht_ \
REF: [EDPG]\: 6

### <a id="ahtō">\*ahtō- [f.] "consideration, attention" (n)</a>
W: \
-- OE _eaht_, _æht_, _eht_ "deliberation" \
-- MDu. _acht_ "attention" \
-- OHG _ahta_ "esteem, reflection, consideration, thought", MHG _ahte_ "evaluation; attention" \
REF: [EDPG]\: 7

### <a id="ahtudan">\*ahtudan- "eighth" (num)</a>
E: \
-- Go. _ahtuda_ \
N: \
-- ON _átti_, _áttandi_, _áttundi_ \
-- Far. _áttandi_ \
-- Elfd. _åttund_ \
W: \
-- OE _eahtoða_, E _eighth_ \
-- OFri. _achtenda_, _achtunda_, _achtanda_ \
-- OS _ahtodo_ \
-- Du. _achtste_ \
-- OHG _ahtodo_, G _achte_ \
REF: [EDPG]\: 7

### <a id="ahurna">\*ahurna- [m.] "maple" (n)</a>
W: \
-- OHG _ahorn_, _ahurn_, _acharn_ , G _Ahorn_ \
REF: [EDPG]\: 7

### <a id="ahwō">\*ahwō- [f.] "river" (n)</a>
E: \
-- Go. _aƕa_ "body of water, river" \
N: \
-- ON _á_ \
-- Far. _á_ "river; stream, brook" \
-- Elfd. _å_ "id." \
W: \
-- OE _ēa_ "stream" \
-- OS _aha_ "water, river" \
-- OHG _aha_, MHG _ahe_ \
REF: [EDPG]\: 7

### <a id="aibra">\*aibra- "harsh" (adj)</a>
W: \
-- OE _āfor_ "vehement, dire, hateful" \
-- OHG _eipar_, _eiber_, _eiver_ "harsh, rough" \
REF: [EDPG]\: 7

### <a id="aida">\*aida- [m.] "pyre" (n)</a>
W: \
-- OE _ād_ "pyre, fire" \
-- OFri. _āde_ \
-- OHG _eit_ [m.] "fireplace, pyre", MHG _eit_ [m.] "fire, oven" \
REF: [EDPG]\: 8

### <a id="aidja">\*aidja- [n.] "isthmus" (n)</a>
N: \
-- ON _eið_, _eiði_ \
-- Far. _eiði_ \
-- Nw. _eid(e)_ "isthmus; passable valley" \
REF: [EDPG]\: 8

### <a id="aigan">\*aigan- [pp.] "to own, possess" (v)</a>
E: \
-- Go. _aigan_, 3sg. _aih_, _aig_, 3pl. _aigun_, _aihun_ \
N: \
-- ON _eiga_, 3sg. _á_, 3pl. _eigu_ "to own, possess; to have" \
-- Far. _eiga_, 3sg. _eigur_, 3pl. _eiga_ "id." \
-- Elfd. _iega_ [wk.] "to possess" \
W: \
-- OE _āgan_, 3sg. _āh_, 3pl. _āgon_ "id.", E _to owe_ \
-- OFri. _āga_, 3sg. _āg_, _ēg_, 3pl. _āga_, _ēga_ "to be obliged, owe, possess" \
-- OS _ēgan_, 3pl. _ēgun_ "to own, possess; to have" \
-- OHG _eigan_, _heigan_, 3pl. _eigun_ "to have, own, keep, receive", MHG _eigen_, _heigen_ "id." \
REF: [EDPG]\: 8

### <a id="aigena">\*aigena- "own" (adj)</a>
N: \
-- ON _eiginn_ \
-- Far. _egin_ \
-- Elfd. _iegen_ \
W: \
-- OE _āgen_, E _own_ \
-- OFri. _ā̆in_, _ē̆in_ \
-- OS _ēgan_ \
-- Du. _eigen_ \
-- OHG _eigan_, G _eigen_ \
REF: [EDPG]\: 8

### <a id="aiginþ">\*aiginþ- [n.] "shoot, barb" (n)</a>
N: \
-- ON _eigin_ "new sprout of corn" \
-- Far. _egi_ "corn, straw; sprout, shoot; barleyfield" \
-- Nw. _eiend(e)_ [f./n.] "grain germ, barb" \
-- Sw. dial. _äjen(ne)_ "grain, insignificant thing" \
REF: [EDPG]\: 9

### <a id="aigla">\*aigla- [m.] "shoot" (n)</a>
N: \
-- Sw. dial. _egel_, _äjel_ "seed sprout" \
REF: [EDPG]\: 9

### <a id="aihti">\*aihti- [f.] "property, belongings; family" (n)</a>
E: \
-- Go. _aihts_ "property" \
N: \
-- ON _ætt_ "family" \
-- Far. _ætt_ "lineage, descent, family" \
W: \
-- OE _ǣht_ "possessions, property, lands" \
-- OHG _ēht_ "possession, goods, wealth" \
REF: [EDPG]\: 9

### <a id="aihtrōjan">\*aihtrōjan- [wk.] "to beg" (v)</a>
E: \
-- Go. _aihtron_ \
REF: [EDPG]\: 9

### <a id="aik">\*aik- [f.] "oak" (n)</a>
N: \
-- ON _eik_ \
-- Far. _eik_ \
-- Elfd. _iek_ \
W: \
-- OE _āc_, pl. _ǣc_, E _oak_ \
-- OS _ek_ \
-- Du. _eik_ [c.] \
-- OHG _eih_, G _Eiche_ \
REF: [EDPG]\: 9

### <a id="aikan">\*aikan- [st.] "to make one's own(?)" (v)</a>
E: \
-- Go. _afaikan_ "to deny, abjure" \
W: \
-- OHG _in-eihhan_ "to sacrifice; dedicate" \
REF: [EDPG]\: 10

### <a id="aikjan">\*aikjan- [wk.] "to irritate" (v)</a>
N: \
-- Nw. _eikja_ "to annoy, pester" \
REF: [EDPG]\: 10

### <a id="aikwernan">\*aikwernan- ~ \*īkurnan- [m.] "squirrel" (n)</a>
N: \
-- ON _íkorni_ \
-- Far. _íkorni_ \
-- Elfd. _aikuonn_ \
W: \
-- OE _ācweorn(a)_ \
-- MDu. _eecoren_, Du. _eekhoren_ [c.] \
-- OHG _eihhurno_, _eihhorno_, G _Eichhörnchen_ [n.] \
REF: [EDPG]\: 10

### <a id="ailida">\*ailida- [m.] "fire" (n)</a>
N: \
-- ON _eldr_ \
-- Far. _eldur_ \
-- Elfd. _jäld_ \
W: \
-- OE _ǣled_ "fire; fire blight" \
REF: [EDPG]\: 11

### <a id="aima">\*aima- [m.] "steam, smoke" (n)</a>
N: \
-- ON _eimr_ "fire, smoke, steam" \
-- Far. _eimur_ "embers" \
-- Nw. _eim_ "vapor, smoke, trace, semblance, inkling" \
REF: [EDPG]\: 11

### <a id="aimauzjōn">\*aima-uzjōn- [f.] "embers" (n)</a>
N: \
-- ON _eimyrja_ \
W: \
-- OE _ǣmyrie_, E _embers_ \
-- MDu. _amere_ \
-- OHG _eimuria_, MHG _eimer(e)_ \
REF: [EDPG]\: 11

### <a id="aina">\*aina- "one" (num)</a>
E: \
-- Go. _ains_ \
N: \
-- ON _einn_ \
-- Far. _ein_ \
-- Elfd. _ien_ \
W: \
-- OE _ān_, E _one_ \
-- OFri. _ān_, _ēn_ \
-- OS _ēn_ \
-- Du. _een_ \
-- OHG _ein_, G _ein_ \
REF: [EDPG]\: 11

### <a id="ainahan">\*ainahan- "single" (adj)</a>
E: \
-- Go. _ainaha_ \
N: \
-- ON _einga_ (indecl) \
W: \
-- OE _ānga_, _ǣnga_, _ēnga_ \
REF: [EDPG]\: 11

### <a id="ainalifa">\*ainalifa- "eleven" (num)</a>
E: \
-- Go. _ainlif_ \
N: \
-- ON _ellifu_ \
-- Far. _ellivu_ \
-- Elfd. _ellåv_ \
W: \
-- OE _endlufon_, E _eleven_ \
-- OFri. _andlova_, _allewa_, _alva_, _elleva_, _elleve_ \
-- OS _ēlleƀan_ \
-- Du. _elf_ \
-- OHG _einlef_, G _elf_ \
REF: [EDPG]\: 11

### <a id="ainja">\*ainja- [m.] "juniper" (n)</a>
N: \
-- ON _einir_ \
-- Far. _eini_ [n.] (also cf. _eini-ber_ "juniper berry") \
-- Nw. _eine_ \
-- Sw. _en(e)_ [c.] \
-- Elfd. _ien_ [m.], _iene_ [n.] \
-- Da. _ene-brer_ \
W: \
-- LG _ēn(e)ke_ \
-- G dial. _Einbeerbaum_ \
REF: [EDPG]\: 12

### <a id="airi">\*airi "early" (adv)</a>
N: \
-- ON _ár_ \
-- Far. _í morgin ári_ "early in the morning" \
W: \
-- OE _ār_ "before" \
-- OFri. _ēr_ (prep, conj) "id." \
-- OS _ēr_ "id." \
-- Du. _eer_ "id." (conj) \
-- OHG _ē(r)_ "before, earlier" (adv, conj), MHG _ē(r)_ "earlier, sooner" \
REF: [EDPG]\: 12

### <a id="airinōn">\*airinōn- [wk.] "to run an errand" (v)</a>
E: \
-- Go. _airinon_ "to be an emissary, to be a messenger" \
N: \
-- ON _árna_ "to intercede, to pray" \
REF: [EDPG]\: 12

### <a id="airiz">\*airiz "earlier" (adv)</a>
E: \
-- Go. _airis_ \
N: \
-- ON _áðr_ "before, earlier" \
REF: [EDPG]\: 12

### <a id="airō">\*airō- [f.] "oar" (n)</a>
N: \
-- ON _ár_ \
-- Far. _ár_ \
-- Elfd. _åra_ \
W: \
-- OE _ār_, E oar \
REF: [EDPG]\: 13

### <a id="airu">\*airu- [m.] "messenger" (n)</a>
E: \
-- Go. _airus_ \
N: \
-- ON _árr_ "messenger, servant" \
-- Icel. _árr_ "id." \
W: \
-- OE _ār_ "id." \
-- OS _ēru_ "id." \
REF: [EDPG]\: 13

### <a id="aiskō">\*aiskō- [f.] "demand" (n)</a>
W: \
-- OFri. _āske_ "claim" \
-- OHG _eisca_ "question" \
REF: [EDPG]\: 13

### <a id="aisōjan">\*aisō(ja)n- [wk.] "to rush" (v)</a>
N: \
-- ON _eisa_ "to rush, dash" \
REF: [EDPG]\: 13

### <a id="aisōn">\*aisōn- [f.] "fireplace" (n)</a>
N: \
-- ON _eisa_ "embers" \
-- Nw. _eise_ "fire, smith's fire" \
W: \
-- MLG _ese_ "forge, fireplace" \
REF: [EDPG]\: 14

### <a id="aista">\*aista/ō- [m./f.] "kiln" (n)</a>
W: \
-- OE _āst_ [m.] "oven", E _oast_ \
-- MLG _eiste_ [f.] "oast house" \
-- MDu. _eest_, _est_ "drying kiln", Du. _eest_ [c.] "oast; kiln" \
REF: [EDPG]\: 14

### <a id="aistēn">\*aistēn- [wk.] "to respect" (v)</a>
E: \
-- Go. _aistan_ \
REF: [EDPG]\: 14

### <a id="aistōn">\*aistōn- [n.] "testicle" (n)</a>
N: \
-- ON _eista_ \
-- Far. _eista_ \
REF: [EDPG]\: 14

### <a id="aita">\*aita- [m.] "ulcer" (n)</a>
W: \
-- Du. coll. _eit_ "ear wax" \
-- OHG _eiz_ "abcess, boil", MHG _eiz_ "ulcer, boil" \
REF: [EDPG]\: 14

### <a id="aitra">\*aitra- [n.] "poison; pus" (n)</a>
N: \
-- ON _eitr_ "venom, poison" \
-- Far. _eitur_ "id." \
-- Elfd. _ietter_ "venom; pus" \
W: \
-- OE _ā̆ttor_ "poison" \
-- WFri. _atter_, _etter_ [c.] "pus" \
-- OS _ē̆ttar_ "poison" \
-- Du. _etter_ "pus" \
-- OHG _eitar_, _eittar_, MHG _eiter_ \
REF: [EDPG]\: 14

### <a id="aiþa">\*aiþa- [m.] "oath" (n)</a>
E: \
-- Go. _aiþs_ \
N: \
-- ON _eiðr_ \
-- Far. _eiður_ \
W: \
-- OE _āþ_, E _oath_ \
-- OFri. _āth_, _ēth_ \
-- OS _ēth_ \
-- Du. _eed_ [c.] \
-- OHG _eid_, G _Eid_ \
REF: [EDPG]\: 15

### <a id="aiþīn">\*aiþīn- ~ \*aiþōn- [f.] "mother" (n)</a>
E: \
-- Go. _aiþei_ \
N: \
-- ON _eiða_ \
W: \
-- OHG _fuotar-eidī_ "nanny", MHG _eide_ \
REF: [EDPG]\: 15

### <a id="aiþja">\*aiþja- "capable of taking an oath" (adj)</a>
E: \
-- Go. _uf-aiþeis_\* "bound by an oath" \
W: \
-- OE _ǣðe_ "qualified to swear an oath" \
REF: [EDPG]\: 15

### <a id="aiþma">\*aiþma- [m.] "son-in-law" (n)</a>
W: \
-- OE _āðum_ "son-in-law; brother-in-law" \
-- OFri. _āthum_, _āthom_, _āthem_ "son-in-law, father-in-law" \
-- OHG _eidum_, _eidam_, MHG _eidem_ [m.] "son-in-law, father-in-law" \
REF: [EDPG]\: 15

### <a id="aiwa-1">\*aiwa/ō- [m./f.] "eternity, age" (n)</a>
E: \
-- Go. _aiws_ [m.] \
W: \
-- OFri. _ēwe_ [f.] \
-- OS _ēwa_ [f.] \
-- Du. _eeuw_ [c.] "century" \
-- OHG _ēwa_ [f.] "eternity" \
REF: [EDPG]\: 16

### <a id="aiwa-2">\*aiwa/ō- [m./f.] "law" (n)</a>
W: \
-- OE _ǣ_, _ǣw_ [f.] "law, religion, marriage" \
-- OFri. _ēwe_, _ē_ [f.] "law" \
-- OS _ēo_, _ēu_ [m.] "law" \
-- OHG _ēwa_, _ēa_, _ēo_ [f.] "law, right, will, contract", MHG _ēwe_, _ē_ [f.] "marriage, alliance" \
REF: [EDPG]\: 16

### <a id="aiwiska">\*aiwiska- "shameful" (adj)</a>
W: \
-- MHG _eisch_ "horrible" \
REF: [EDPG]\: 16

### <a id="aiwjan">\*aiwjan- [wk.] "to graze" (v)</a>
N: \
-- ON _æja_ (pret. _áða_) "to rest, eat" \
W: \
-- Du. _eeuwen_ "to feed, graze" \
REF: [EDPG]\: 16

### <a id="aiza">\*aiza- [n.] "ore, brass" (n)</a>
E: \
-- Go. _ais_, gen. _aizis_ "ore" \
N: \
-- ON _eir_ "brass, copper" \
-- Icel. _eir_ "id." \
W: \
-- OE _ār_, _ǣr_ "ore, brass, copper", E _ore_ \
-- OS _ēr_ "ore" \
-- MDu. _eer_ "copper, metal" \
-- OHG _ēr_ "ore", MHG _ēr_ "ore, iron" \
REF: [EDPG]\: 16

### <a id="aizō">\*aizō- [f.] "respect, benevolence" (n)</a>
N: \
-- ON _eir_ "mercy" \
W: \
-- OE _ār_ "honor, dignity; kindness, mercy" \
-- OFri. _ēre_ "honor; tribute" \
-- Du. _eer_ [c.] "honor" \
-- OHG _era_ "id.", G _Ehre_ "id." \
REF: [EDPG]\: 17

### <a id="ajja">\*ajja- [n.] "egg" (n)</a>
E: \
-- Go. Crim. _ada_ [n.pl.] \
N: \
-- ON _egg_ \
-- Far. _egg_ \
-- Elfd. _egg_ \
W: \
-- OE _ǣg_ \
-- WFri. _aai_ \
-- OS _ei_ \
-- Du. _ei_ \
-- OHG _ei_, G _Ei_ \
REF: [EDPG]\: 17

### <a id="ajuki">\*ajuki- "eternal" (adj)</a>
E: \
-- Go. _ajuk-duþs_ [f.] "eternity" \
W: \
-- OE _ǣce_ "eternal" \
REF: [EDPG]\: 17

### <a id="akan-1">\*akan- [st.] "to drive" (v)</a>
N: \
-- ON _aka_ \
-- Far. _aka_ "to drive; to push, shove" \
-- Elfd. _åka_ \
REF: [EDPG]\: 18

### <a id="akan-2">\*akan- [st.] "to ache" (v)</a>
W: \
-- OE _acan_, E _to ache_ \
REF: [EDPG]\: 18

### <a id="akra-1">\*akra- [m.] "field" (n)</a>
E: \
-- Go. _akrs_ \
N: \
-- ON _akr_ \
-- Far. _akur_ \
-- Elfd. _åker_ "uncultivated field" \
W: \
-- OE _æcer_ \
-- OFri. _ekker_, _ēker_ \
-- OS _akkar_ \
-- Du. _akker_ [c.] \
-- OHG _ackar_, G _Acker_ \
REF: [EDPG]\: 18

### <a id="akra-2">\*akra- [n.(?)] "some kind of metal" (n)</a>
W: \
-- EDu. _aecker_ \
REF: [EDPG]\: 18

### <a id="akrana">\*akrana- [n.] "tree fruit, acorn" (n)</a>
E: \
-- Go. _akran_ "fruit" \
N: \
-- ON _akarn_ "acorn" \
-- OSw. _akarn_ "id." \
W: \
-- OE _æcern_ "id.", E _acorn_ \
-- MDu. _aker_ [m.] "id." \
-- MHG _ackeran_ [m./n.] "acorn, beechnut", G _Eckern_ "acorn" (with intrusive umlaut from the plural) \
REF: [EDPG]\: 18

### <a id="akwesī">\*akwesī- [f.] "axe" (n)</a>
E: \
-- Go. _aqizi_ \
N: \
-- ON _øx_ \
-- Far. _øks_ \
-- Elfd. _ōkse_ \
W: \
-- OE _æx_, E _axe_ \
-- OFri. _axe_ \
-- OS _akus_ \
-- Du. _aks_ [c.] \
-- OHG _ackus_, G _Axt_ \
REF: [EDPG]\: 19

### <a id="alan-1">\*alan- [st.] "to grow up; to rear" (v)</a>
E: \
-- Go. _alan_ "to grow on, feed on" \
N: \
-- ON _ala_ "to give birth to, nourish, support" \
REF: [EDPG]\: 19

### <a id="alan-2">\*ala(n)- [m.] "awl" (n)</a>
N: \
-- ON _alr_ (< \*ala-) \
-- Nw. _ale_ "thin stick to dry meat on" (< \*alan-) \
REF: [EDPG]\: 19

### <a id="alasnō">\*alasnō- [f.] "awl" (n)</a>
W: \
-- MLG _alse_, _else(ne)_ \
-- MDu. _alsene_, _elsen(e)_, _els(e)_, Du. _els_ [c.] \
-- OHG _alansa_, _alunsa_ \
REF: [EDPG]\: 20

### <a id="albi">\*albi- [f.] "river" (n)</a>
N: \
-- ON _elfr_ \
-- Elfd. _ove_ \
W: \
-- MLG _elve_ "riverbed" \
REF: [EDPG]\: 20

### <a id="albut">\*albut- [f.] "swan" (n)</a>
N: \
-- ON _ǫlpt_, gen. _alptar_, _elptr_ \
-- Icel. _álft_ \
W: \
-- OE _ielfetu_, E obs. _elk_(?) "swan, goose" \
-- MHG _albiz_, _elbiz_ \
REF: [EDPG]\: 20

### <a id="alda">\*alda- "old" (adj)</a>
E: \
-- Go. _alþeis_ (< \*alþja-) \
N: \
-- ON _ellri_ comp. "older" \
W: \
-- OE _eald_, E _old_ \
-- OFri. _ald_, _auld_ \
-- OS _ald_ \
-- Du. _oud_ \
-- OHG _alt_, G _alt_ \
REF: [EDPG]\: 20

### <a id="aldi">\*aldi- [f.] "age" (n)</a>
E: \
-- Go. _alþs_ "generation, age" \
N: \
-- ON _ǫld_, pl. "mankind" \
REF: [EDPG]\: 20

### <a id="aldōn">\*aldō(n)- [f.] "hollow (of a tree)" (n)</a>
N: \
-- ON _alda_ "wave, roller" \
-- Icel. _alda_ "wave, small land ridge" \
-- Far. _alda_ "wave" \
-- Nw. _alde_ "wave", dial. _alde_, _olde_ "billow, rivulet, well, (wooden) trough" \
-- Sw. dial. _ålla_ "furrow, vessel" \
W: \
-- OE _aldaht_, _aldot_ "trough(?), channel(?)" \
-- G Bav. _alden_ "furrow" \
REF: [EDPG]\: 21

### <a id="aldra">\*aldra- [n.] "age, life(span)" (n)</a>
E: \
-- Go. _fram-aldrs_\* "aged" \
N: \
-- ON _aldr_ [m.] \
W: \
-- OE _aldor_ \
-- OS _aldar_ \
-- MDu. _ouder_ \
-- OHG _altar_, _aldar_, G _Alter_ \
REF: [EDPG]\: 21

### <a id="algi-1">\*algi- [m.] "elk" (n)</a>
N: \
-- ON _elgr_ \
-- Far. _elgur_ \
REF: [EDPG]\: 21

### <a id="algi-2">\*algi- [m.] "slush" (n)</a>
N: \
-- Icel. _elgur_ [m.] "slush, melting snow" \
REF: [EDPG]\: 21

### <a id="algjan">\*algjan- [wk.] "to be nauseated" (v)</a>
N: \
-- Icel. _elgja_ \
-- Nw. _elgja_ \
REF: [EDPG]\: 21

### <a id="algōjan">\*algōjan- [wk.] "to protect" (v)</a>
W: \
-- OE _ealgian_, _algian_ "to defend" \
REF: [EDPG]\: 21

### <a id="alh">\*alh- [m.] "temple" (n)</a>
E: \
-- Go. _alhs_, dat. _alh_ \
W: \
-- OE _ealh_ \
REF: [EDPG]\: 22

### <a id="alīnō">\*alīnō- [f.] "forearm; cubit" (n)</a>
E: \
-- Go. _aleina_ "cubit, ell" \
N: \
-- ON _ǫln_ \
-- Far. _alin_ "ell" \
-- Elfd. _aln_ "id." \
W: \
-- OE _eln_ "id.", E _ell_ \
-- MDu. _elne_, _ellen_, _elle_, Du. _el_ [c.] "cubit" \
-- OHG _elena_, _elna_ "id.", G _Elle_ "id." \
REF: [EDPG]\: 22

### <a id="aliskōn">\*aliskōn- [wk.] "to tend to(?)" (v)</a>
N: \
-- ON _elska_ "to love" \
-- Far. _alska_, _elska_ "id." \
REF: [EDPG]\: 22

### <a id="alis">\*alis/z- [m.] "alder" (n)</a>
N: \
-- ON _ǫlr_, _alr_ \
-- Nw. _or_, _older_ \
W: \
-- OE _alor_, _aler_, E _alder_ \
-- MDu. _else_, Du. _els_ [c.] \
-- OHG _erila_, _elira_ [f.], G _Erle_ [f.] \
REF: [EDPG]\: 22

### <a id="alja">\*alja- "someone else" (pron)</a>
E: \
-- Go. _aljis_\* \
REF: [EDPG]\: 23

### <a id="aljan">\*aljan- [wk.] "to make grow, rear" (v)</a>
E: \
-- Go. _aljan_\* "to make grow, fatten" \
N: \
-- Nw. dial. _elja_ [st./wk.] "to breed, rear" \
REF: [EDPG]\: 23

### <a id="aljana">\*aljana- [n.] "zeal" (n)</a>
N: \
-- ON _eljan_ [n./f.] "endurance, energy" \
W: \
-- OE _ellen_ "courage" \
-- OS _ellean_ "courage, power" \
-- OHG _ellian_, _ellan_ "courage", MHG _ellen_ "zeal, courage" \
REF: [EDPG]\: 23

### <a id="alkōn">\*alkōn- [f.] "auk" (n)</a>
N: \
-- ON _alka_ "razor-bill" \
-- Far. _álka_ "id." \
W: \
-- OE _ealce_, E auk \
REF: [EDPG]\: 23

### <a id="alla">\*alla- "all" (adj)</a>
E: \
-- Go. _alls_ "all, every" \
N: \
-- ON _allr_ "all, entire, whole" \
W: \
-- OE _eall_ \
-- OFri. _al_, _ol_ \
-- OHG _al(l)_ "all, every, complete" \
REF: [EDPG]\: 23

### <a id="aluþ">\*aluþ- [n.] "ale, beer" (n)</a>
N: \
-- ON _ǫl_ "beer" \
W: \
-- OE _ealop_ "ale" \
-- OS _alo-fat_ "ale-vat" \
-- MDu. _ale_, _ael_ "id." \
-- MHG _al-schaf_ "ale vessel" \
REF: [EDPG]\: 23

### <a id="amaitjō">\*amaitjō- [f.] "ant" (n)</a>
W: \
-- OE _ǣ̆mette_, E _ant_ \
-- MDu. _amete_, _emete_, _eemt_ \
-- OHG _ā̆meiza_, G _Ameise_ \
REF: [EDPG]\: 24

### <a id="amazan">\*amazan- [m.] "bunting" (n)</a>
W: \
-- OE _omer_, _amore_ "_scorellus_ (unidentified bird)", E _yellow-hammer_ \
-- OHG _amoro_ "bunting", MHG _amer_, _amoro_ "bunting" \
REF: [EDPG]\: 24

### <a id="ambahta">\*ambahta- [m.] "servant" (n)</a>
E: \
-- Go. _andbahts_ "servant, minister" \
W: \
-- MDu. _ambacht_ \
-- OHG _ambaht_ "servant, employee, official", MHG _ambet_, _amt_ "servant, caretaker" \
REF: [EDPG]\: 24

### <a id="amban">\*amban- [m.] "belly" (n)</a>
W: \
-- OS _ambon_ pl. \
-- OHG _amban_ \
REF: [EDPG]\: 24

### <a id="ammōn">\*am(m)ōn- [wk.] "to irritate" (v)</a>
N: \
-- ON _ama_ "to vex, molest" \
-- Icel. _ama_ "to trouble, vex" \
-- Far. _ama_ "to spend" \
-- Nw. _ama_, _amma_ "to chafe, grate; incite" \
REF: [EDPG]\: 24

### <a id="ampra">\*ampra- "sharp, sour" (adj)</a>
N: \
-- ON _apr_ "hard, sharp; sad" \
-- Nw. _amper_ "bitter" (<< LG) \
W: \
-- MDu. _amper_ "sour, bitter, harsh", Du. _amper_ "barely" (adv) \
REF: [EDPG]\: 25

### <a id="ampran">\*ampra/ōn- [m./f.] "sorrel, dock" (n)</a>
W: \
-- OE _ampre_, _ompre_ [f.] "sorrel; varicose vein" \
-- OHG _ampfaro_ [m.] "sorrel", G _Ampfer_ [m.] "id." \
REF: [EDPG]\: 25

### <a id="amsa">\*amsa- [m.] "shoulder" (n)</a>
E: \
-- Go. _ams_\* \
N: \
-- Nw. _ås_ "hill, ridge" \
-- Elfd. _ą̊s_ "ridge" \
REF: [EDPG]\: 25

### <a id="amslōn">\*amslōn- [f.] "blackbird" (n)</a>
W: \
-- OE _ōsle_, E _ousle_ \
-- OHG _amsala_, G _Amsel_ \
REF: [EDPG]\: 25

### <a id="ana">\*ana "on(to), to, by" (prep, adv)</a>
E: \
-- Go. _ana_ "on, at, during, in" (prep) \
N: \
-- ON _á_ "on(to)" \
-- Far. _á_ "on, at" \
-- Elfd. _ą̊_ "on" \
W: \
-- OE _on_ "id.", E _on_ \
-- OFri. _on_, _an_, _en_, _in_ "id." \
-- OS _an_ "id." \
-- Du. _aan_ "id." \
-- OHG _ana_, _an_ "id.", G _an_ "id." \
REF: [EDPG]\: 26

### <a id="anad">\*anad- [f.] "duck" (n)</a>
N: \
-- ON _ǫnd_, pl. _endr_ \
-- Far. _ont_, pl. _entur_ \
-- Elfd. _and_ \
W: \
-- OE _ænid_, _æned_, _ened_ \
-- WFri. _ein_ [c.] \
-- MDu. _aent_, _eent_ [m./f.] \
-- OHG _anut_, _enit_, G _Ente_ \
REF: [EDPG]\: 26

### <a id="anadan">\*anadan- [m.] "breath; spirit" (n)</a>
N: \
-- ON _andi_ "breath; mind" \
-- Far. _andi_ "id." \
-- Elfd. _ande_ "spirit" \
W: \
-- OE _anda_, _onda_ "envy, anger, zeal", ME _onde_ "breath, emotion, envy" \
-- OS _ando_ "offense" \
-- OHG _anado_ "emulation, envy", MHG _ande_ "hurt" \
REF: [EDPG]\: 26

### <a id="anan">\*anan- [st.] "to breathe" (v)</a>
E: \
-- Go. _us-anan_ "to breathe out" \
REF: [EDPG]\: 27

### <a id="andiz">\*andiz "earlier, rather" (adv)</a>
N: \
-- ON _enn_ "rather, than; again" \
-- Far. _enn_ \
W: \
-- OE _end_ "formerly, of old" \
-- OHG _enti_ "earlier" \
REF: [EDPG]\: 27

### <a id="andja">\*andja- [m.] "end, extreme" (n)</a>
E: \
-- Go. _andeis_ "end, extremity" \
N: \
-- ON _endir_ "end" \
W: \
-- OE _ende_ "end, back", E _end_ \
-- OFri. _e(i)nda_ "end, verdict" \
-- OS _endi_ "beginning, end, aim" \
-- Du. _eind_ [n.] "end" \
-- OHG _anti_, _enti_ [m./n.] "end, edge, front", MHG _ente_, _ende_ [m./n.] "end, direction", G _Ende_ [n.] "end" \
REF: [EDPG]\: 27

### <a id="andla">\*andla- [m.(?)] "saltmarsh grass" (n)</a>
W: \
-- OFri. _ondul_, _andel_ \
-- LG _andel_ \
REF: [EDPG]\: 27

### <a id="andura">\*andura- [m.] "snow-shoe, skid(?)" (n)</a>
N: \
-- ON _ǫndurr_ \
-- Nw. _onder_ [f.] "the shorter ski of two" \
-- Elfd. _andur_ "id." \
REF: [EDPG]\: 27

### <a id="angan">\*angan- [m.] "curve; neck" (n)</a>
E: \
-- Go. \*_hals-agga_ "neck" \
N: \
-- ON _angi_ "tip, jag" \
-- Far. _angi_ "vertebral process; cog, tooth, prong" \
-- Nw. dial. _ang-boge_ "id." \
W: \
-- OHG _ango_ "hinge, cardinal point; prickle", G Swab. _hals-anke_ [f.] "neck" \
REF: [EDPG]\: 28

### <a id="angaza">\*angaza- [m.] "worry" (n)</a>
N: \
-- ON _angr_ "grief" \
-- Far. _angur_ "repentance, remorse", poet. "anxiety, worry" \
REF: [EDPG]\: 28

### <a id="angra-1">\*angra- [m.] "lowland, meadow" (n)</a>
N: \
-- ON _angr_ "bay" \
W: \
-- MDu. _anger_ "meadow, enclosed piece of land" \
-- OHG _angar_ "uncultivated meadow", MHG _anger_ "grassland" \
REF: [EDPG]\: 28

### <a id="angra-2">\*angra- [m.] "weevil" (n)</a>
W: \
-- OHG _angar_ "corn weevil", G _Engerling_ "cockchafer grub" \
REF: [EDPG]\: 28

### <a id="angula">\*angula- [m.] "hook, tip" (n)</a>
N: \
-- ON _ǫngull_ "angle, hook" \
W: \
-- OE _ongel_ "fishing-rod", E _angle_ \
-- MLG _angel_ "prickle, sting; hinge" \
-- OHG _angul_ "fishing-rod, prick, hook" \
REF: [EDPG]\: 28

### <a id="angwu">\*angwu- "narrow" (adj)</a>
E: \
-- Go. _aggwus_ \
N: \
-- ON _øngr_ \
W: \
-- OE _enge_ \
-- OFri. _enge_, WFri. _eang_ "afraid", EFri. _ong_ "id." \
-- MDu. _enghe_, Du. _eng_ "scary; narrow" \
-- OHG _angi_, _engi_, G _eng_ \
REF: [EDPG]\: 28

### <a id="anhula">\*anhula- [m.] "shoot, tip" (n)</a>
N: \
-- ON _ǫ́ll_, _áll_ "shoot" \
REF: [EDPG]\: 29

### <a id="anhulō">\*anhulō- [f.] "strap" (n)</a>
N: \
-- ON _ǫ́l_, _ál_ "belt, strip" \
-- Far. _ál_ "strap, belt; stretch" \
W: \
-- OE _ōl-þwang_ "halter, snare, strap" \
REF: [EDPG]\: 29

### <a id="ankjōn">\*ankjōn- [f.] "ankle" (n)</a>
N: \
-- ON _ekkja_ "heel" \
W: \
-- OHG _ancha_, _encha_ "ankle" \
REF: [EDPG]\: 29

### <a id="ankōn">\*ankōn- [wk.] "to complain" (v)</a>
W: \
-- MLG _anken_ "to moan, grunt" \
REF: [EDPG]\: 29

### <a id="ankula">\*ankula- [m.] "ankle" (n)</a>
N: \
-- ON _ǫkla_, _ǫkkla_ [n.], _ǫkli_ \
-- Far. _økil_ \
-- Elfd. _okkel_ \
W: \
-- OFri. _ankel_ [m./n.] \
-- MDu. _ankel_, Du. _enkel_ [c.] \
-- OHG _enchil_, G _Enkel_ \
REF: [EDPG]\: 29

### <a id="ankwan">\*ankwan- [m.] "butter" (n)</a>
W: \
-- OHG _ancho_, MHG _anke_ \
REF: [EDPG]\: 29

### <a id="ansa">\*ansa- [m.] "beam" (n)</a>
E: \
-- Go. _ans_ "wooden beam" \
N: \
-- ON _áss_ "thick pole, beam, yard of a sail" \
-- Far. _ásur_ "lath; axle, shaft; post" \
-- Nw. _ås_ "axle, beam, hill, ridge (of a hill)" \
-- Elfd. _guov-ą̊s_ "joist" \
W: \
-- MHG _ans-boum_ "beam" \
REF: [EDPG]\: 30

### <a id="ansjō">\*ansjō- [f.] "handle, ring, eyelet" (n)</a>
N: \
-- ON _æs_ "thread-hole in a shoe" \
-- Far. _æs_ "id." \
W: \
-- WFri. _oes_ [c.] "eyelet" \
-- MLG _ose_ "ring-shaped handle" \
REF: [EDPG]\: 30

### <a id="ansti">\*ansti- [f.] "love, favor" (n)</a>
E: \
-- Go. _ansts_ "beneficence, grace" \
N: \
-- ON _ást_ "love, affection" \
-- Far. _ást_ "id." \
W: \
-- OE _ēst_ [m./f.] "favor, consent, grace" \
-- OFri. _evēst_ "envy" \
-- OS _anst_ "grace" \
-- OHG _anst_ "thanks, favour, grace" \
REF: [EDPG]\: 30

### <a id="ansu">\*ansu- [m.] "god" (n)</a>
N: \
-- ON _áss_, _óss_, pl. _æsir_ \
W: \
-- OE _ōs_ \
-- OS _ās_, _ōs_ \
-- OHG _ans(i)_- prop. \
REF: [EDPG]\: 30

### <a id="anþara">\*anþara- "the other (of two), the second" (adj, num)</a>
E: \
-- Go. _anþar_ (num) \
N: \
-- ON _annarr_ (pron, num) \
-- Far. _annar_ (pron, num) \
W: \
-- OE _ōðer_, E _other_ \
-- OFri. _ōther_ \
-- OS _ōthar_, _āthar_, _andar_ "other" \
-- Du. _ander_ "id." \
-- OHG _andar_ "id.", G _andere_ "id." \
REF: [EDPG]\: 30

### <a id="anþja">\*anþja- "forehead" (n)</a>
N: \
-- ON _enni_ [n.] "forehead; steep crag" \
-- Far. _enni_ [n.] "forehead, brow; sharp projection on a steep mountain side" \
-- Nw. dial. _enne_, _enna_ [n.] "id." \
-- OSw. _ænne_ [n.] "id." \
-- Elfd. _enne_ [n.] "id." \
W: \
-- OHG _andi_, _endi_ [n.] "id.", MHG _e(i)nde_ [n.] "id." \
REF: [EDPG]\: 31

### <a id="anþō">\*anþō- ~ \*andō- [f.] "antechamber" (n)</a>
N: \
-- ON _ǫnn_, _ǫnd_ \
REF: [EDPG]\: 31

### <a id="apaldra">\*apaldra/ō- [m./f.] "apple-tree" (n)</a>
N: \
-- ON _apaldr_ [m.] \
-- Far. _apaldur_ [m.] \
W: \
-- OE _apuldre_ [f.] \
-- OHG _apfoltra_ [f.], G _apfalter_, _affalter_ [f.] \
REF: [EDPG]\: 31

### <a id="apan">\*apan- [m.] "monkey, ape" (n)</a>
N: \
-- ON _api_ \
-- Far. _apa_ [f.] \
-- Sw. _apa_ [c.] \
W: \
-- OE _apa_, E _ape_ \
-- OS _apo_ \
-- Du. _aap_ [c.] \
-- OHG _affo_, G _Affe_ \
REF: [EDPG]\: 31

### <a id="apla">\*apla- [m.] "apple" (n)</a>
E: \
-- Go. Crim. _apel_ \
W: \
-- OE _æpl_, _appel_, E _apple_ \
-- OS _appul_ \
-- Du. _appel_ [c.] \
-- OHG _apful_, _apfel_, G _Apfel_ \
REF: [EDPG]\: 31

### <a id="aran-1">\*aran- [m.] "eagle" (n)</a>
E: \
-- Go. _ara_ \
N: \
-- ON _ari_, _ǫrn_ \
-- Da. _ørn_ [c.] (>> Far. _ørn_ [f.]) \
W: \
-- OE _earn_ \
-- Du. _arend_ [c.] \
-- OHG _aro_, _arn_, MHG _are_, _ar_, G _Aar_ \
REF: [EDPG]\: 32

### <a id="aran-2">\*aran- [m.] "phalanx bone(?)" (n)</a>
N: \
-- OSw. _are_ "part of the thumb, phalanx(?)" \
REF: [EDPG]\: 32

### <a id="arba">\*arba- [m./n.] "working animal, worker(?)" (n)</a>
N: \
-- ON _arfr_ [m.] "ox" \
REF: [EDPG]\: 32

### <a id="arbaidi">\*arbaidi- [f.] "toil, labor" (n)</a>
E: \
-- Go. _arbaiþs_ \
N: \
-- ON _erfiði_ [n.] "trouble, toil, labor" \
W: \
-- OE _earfoð_ "hardship, trouble" \
-- OS _arbēd_, _arabid_ "id." \
-- ODu. _arvit_ "id." \
-- OHG _arabeit_ "id.", G _Arbeit_ "id." \
REF: [EDPG]\: 33

### <a id="arbja">\*arbja- [n.] "inheritance" (n)</a>
N: \
-- ON _arfr_ [m.] "inheritance, patrimony" (< \*arba-) \
W: \
-- OE _ierfe_ \
-- OFri. _erve_ \
-- OS _erbi_ \
-- OHG _arbi_, _erbi_, G _Erbe_ \
REF: [EDPG]\: 33

### <a id="arbjan">\*arbjan- [m.] "heir" (n)</a>
E: \
-- Go. _arbja_ \
W: \
-- OE _ierfe_ \
-- OFri. _erva_ \
-- MDu. _erve_ \
-- OHG _arbeo_, _erbeo_, G _Erbe_ \
REF: [EDPG]\: 33

### <a id="arduga">\*arduga- "steep" (adj)</a>
N: \
-- ON _ǫrðugr_ \
-- Far. _ørdugur_ \
REF: [EDPG]\: 33

### <a id="arfan">\*arfan-(?) [m.] "chickweed" (n)</a>
N: \
-- ON _arfi_ \
-- Far. _arvi_ \
-- Nw. _arve_ \
W: \
-- OE _earfan_ [m.pl.] \
-- LG _arf_ \
-- Du. _erf_ [f.] \
REF: [EDPG]\: 34

### <a id="arga">\*arga- "unmanly" (adj)</a>
N: \
-- ON _argr_ "unmanly, cowardly; lewd" \
-- Far. _argur_ "wicked; greedy; infuriated" \
W: \
-- OE _earg_ "cowardly, timid" \
-- OFri. _erch_ "evil, wrong" \
-- MLG _arch_ "angry, evil" \
-- MDu. _arch_, _erch_ "bad, evil, worthless", Du. _erg_ "bad" \
-- OHG _arg_, _arag_ "greedy, cowardly" \
REF: [EDPG]\: 34

### <a id="arhwō">\*arhwō- [f.] "arrow" (n)</a>
N: \
-- ON _ǫr_ \
-- Far. _ørv_ \
W: \
-- OE _earh_, E _arrow_ \
REF: [EDPG]\: 34

### <a id="arjan">\*arjan- [st.] "to plow" (v)</a>
E: \
-- Go. _arjan_\* [st.?] \
N: \
-- ON _erja_ [wk.] \
W: \
-- OE _erian_ [wk.] \
-- OFri. _era_, _ara_ [wk.] \
-- OHG _erien_, _erren_ pret. _iar_, MHG _ern_ \
REF: [EDPG]\: 34

### <a id="arma-1">\*arma- [m.] "arm" (n)</a>
E: \
-- Go. _arms_ \
N: \
-- ON _armr_ \
W: \
-- OE _earm_, E _arm_ \
-- OFri. _erm_ \
-- OS _arm_ \
-- Du. _arm_ [c.] \
-- OHG _aram_, G _Arm_ \
REF: [EDPG]\: 35

### <a id="arma-2">\*arma- "miserable, pitiful, poor" (adj)</a>
E: \
-- Go. _arms_ \
N: \
-- ON _armr_ \
-- Far. _armur_ \
-- Elfd. _arm_ \
W: \
-- OFri. _erm_ \
-- OS _arm_ \
-- Du. _arm_ \
-- OHG _aram_, G _arm_ \
REF: [EDPG]\: 35

### <a id="arnja">\*arnja- "serious" (adj)</a>
E: \
-- Go. _arniba_ "certain, securely" (adv) \
N: \
-- ON _ern_ "brisk, vigorous" \
REF: [EDPG]\: 35

### <a id="arsa">\*arsa- [m.] "arse" (n)</a>
N: \
-- ON _ars_, _rass_ \
W: \
-- OE _ears_ \
-- OFri. _ers_ \
-- OS _ars_ \
-- Du. _aars_ [c.] \
-- OHG _ars_, G _Arsch_ (with -rs- > -rš- as in _Barsch_ < \*barsa-) \
REF: [EDPG]\: 35

### <a id="artōn">\*artō(n)- [f.] "wagtail?" (n)</a>
N: \
-- ON _arta_ "a species of teal, garganey" (also cf. _ertla_ "wagtail" < \*artilōn-) \
-- Icel. _urt_, _ōrt_ "teal" \
-- Sw. _årta_ "garganey" \
-- Elfd. _arta_ "(Baikal) teal" \
W: \
-- OE _earte_ "pied wagtail" \
REF: [EDPG]\: 36

### <a id="arþōn">\*arþōn- [f.] "crupper?" (n)</a>
N: \
-- Nw. _arde_, _ore_ [m./f.] "steel ring to fasten the saddle with" \
-- OSw. _arþa_ "crupper" \
-- Elfd. _arda_ "ring on a harness" \
REF: [EDPG]\: 36

### <a id="arþra">\*arþra- [m.] "scratch plow" (n)</a>
E: \
-- Go. _ardrs_ "plow" \
N: \
-- ON _arðr_ "id." \
-- OSw. _arþer_ [n.] "id." \
-- Sw. _årder_ [n.] "id." \
W: \
-- MHG _arl_ [f.] "plowshare" \
REF: [EDPG]\: 36

### <a id="arundja">\*arundja- [n.] "errand" (n)</a>
N: \
-- ON _ørendi_, _erendi_, _eyrendi_ "errand, message, business" \
-- Far. _ørindi_ "id." \
W: \
-- OE _ǣrende_ "id.", E _errand_ \
-- WFri. _eernje_ [wk.] "to talk" (< OFri. \*erendia, cf. Brandsma 1936: 68) \
-- OS _ārundi_, WPhal. _êren_, _æren_ [f.] \
-- OHG _ārunti_ \
REF: [EDPG]\: 36

### <a id="arut">\*arut- [m.] "ore" (n)</a>
W: \
-- ODu. _arut_ \
-- OHG _aruz_, _ariz_ [m.], _aruzi_, _arizi_ [n.], MHG _eriz(e)_, _erz(e)_ [n.], G _Erz_ [n.] \
REF: [EDPG]\: 37

### <a id="arwa">\*arwa- "ready, fast" (adj)</a>
E: \
-- Go. _arwjo_ "ready" (adv) \
N: \
-- ON _ǫrr_ "quick, generous" \
W: \
-- OE _earu_ "quick, ready, prepared" \
-- OS _aru_ "ready" \
REF: [EDPG]\: 37

### <a id="arwīt">\*arwīt- [f.] "pea" (n)</a>
N: \
-- ON _ertr_ pl. \
-- Far. _ertur_, pl. _ertrar_ \
-- Elfd. _ert_ \
W: \
-- OS _erit_ \
-- Du. _erwt_ [c.] \
-- OHG _arawīz_, _araweiz_, G _Erbse_ \
REF: [EDPG]\: 37

### <a id="arwiz">\*arwiz- [n.] "scar" (n)</a>
N: \
-- ON _ørr_ \
-- Far. _arr_, poet. _err_ \
-- Elfd. _er_ \
W: \
-- MLG _(n)are_ \
-- MDu. _nerwe_ [f.], Du. _nerf_ [c.] "grain" \
-- OHG _narwo_ [m.], _narwa_ [f.], G _Narbe_, dial. _Arbe_ [f.] \
REF: [EDPG]\: 37

### <a id="asjōn">\*asjōn- [f.] "soapstone" (n)</a>
N: \
-- ON _esja_ \
-- Nw. _esje_ "soapstone; embers" \
-- OSw. _æsia_, _æssia_ "smith's fire", Sw. dial. _ässje_ "embers" \
-- Da. _esse_ "smith's fire" \
W: \
-- OHG _essa_ "forge, melting-oven" \
REF: [EDPG]\: 38

### <a id="aska">\*aska- [m.] "ash" (n)</a>
N: \
-- ON _askr_ \
-- Far. _ask_ [f.] \
-- Elfd. _ask_ \
W: \
-- OE _æsc_, E _ash_ \
-- ODu. _aska_ [f.], MDu. _asch_, _esche_ [f.], Du. _es_ [c.] \
-- OHG _asc_, G _Esche_ [f.] \
REF: [EDPG]\: 38

### <a id="askōn">\*askōn- [f.] "ashes" (n)</a>
E: \
-- Go. _azgo_ "ashes" \
N: \
-- ON _aska_ "ashes" \
W: \
-- OE _æsce_, _asce_, _axe_ "ashes" \
-- WFri. _ask(e)_ "ashes", NFri. _eesk_ "ashes" \
-- Du. _as_ [c.] "ashes" \
-- OHG _aska_ "ashes, dust" \
REF: [EDPG]\: 38

### <a id="aspō">\*aspō- ~ \*apsō- [f.] "aspen" (n)</a>
N: \
-- ON _ǫsp_ \
W: \
-- OE _æsp_, _æps_, E _aspen_ \
-- OHG _aspa_, G _Espe_ \
REF: [EDPG]\: 39

### <a id="at">\*at "at, to" (prep)</a>
E: \
-- Go. _at_ "at" \
N: \
-- ON _at_ "towards, against; at" \
-- Far. _at_ "by, at, to, in" \
-- Elfd. _að_ "to" \
W: \
-- OE _æt_ "at", E _at_ \
-- OFri. _et_, _it_ "in, at, from" \
-- OS _at_ "at, with" \
-- OHG _az_, _ez_, _iz_ "to, at, in" \
REF: [EDPG]\: 39

### <a id="atiska">\*atiska- [n.(?)] "grainfield" (n)</a>
E: \
-- Go. _atisk_ [n.(?)] \
W: \
-- MDu. _esc_ [m.] "collective field" \
-- OHG _ezzisc_ [n.] "seed", G _Esch_ [m./n.] "seeding field" \
REF: [EDPG]\: 39

### <a id="atjan-1">\*atjan- [wk.] "to incite" (v)</a>
N: \
-- ON _etja_ "to incite, make fight; to contend against" \
W: \
-- OFri. _ūt-etta_ "to exact payment" \
-- OHG _an-azzen_ "to incite, provoke" \
REF: [EDPG]\: 39

### <a id="atjan-2">\*atjan- [wk.] "to make eat" (v)</a>
N: \
-- ON _etja_ \
W: \
-- OFri. _etta_ "to graze, pasture" \
-- MDu. _etten_ "id." \
-- OHG _azzen_, _ezzen_ "id." \
REF: [EDPG]\: 39

### <a id="attan">\*attan- [m.] "father" (n)</a>
E: \
-- Go. _atta_ \
N: \
-- ON _atti_ \
W: \
-- OHG _atto_ "forefather", MHG _atte_ "(fore)father", G Wais. _atto_ \
REF: [EDPG]\: 39

### <a id="aþala">\*aþala- [n.] "nature" (n)</a>
N: \
-- ON _aðal_ "nature, disposition" \
-- Nw. _al_ [m.] "pith (of trees), essence" \
W: \
-- OS _athal_ "noble clan" \
-- OHG _adal_ [m./n.] "nobility, race" \
REF: [EDPG]\: 40

### <a id="aþlan">\*aþlan- ~ \*adlan- [m.] "liquid manure" (n)</a>
N: \
-- Nw. _åle_, _ale_ \
W: \
-- OE _adel(a)_ "mud, dirt" \
-- NFri. _ethel_ \
-- MDu. _ad(d)el_ (var. _iddel_ << Fri. \*edel) \
-- G Bav. _adel_ \
REF: [EDPG]\: 40

### <a id="aþna">\*aþna- [n.] "year" (n)</a>
E: \
-- Go. _aþn(s)_\* [m./n.] "period of the year, season" \
REF: [EDPG]\: 40

### <a id="aþulja">\*aþulja- "noble" (adj)</a>
W: \
-- OE _æðele_ \
-- OFri. _ethele_ \
-- OS _athali_, _etheli_ \
-- Du. _edel_ \
-- OHG _edili_, G _edel_ \
REF: [EDPG]\: 40

### <a id="auda">\*auda- [m.] "riches" (n)</a>
E: \
-- Go. _auda-hafts_ "fortunate" \
N: \
-- ON _auðr_ "wealth" \
-- Icel. _auðr_ "wealth" \
W: \
-- OS _ōd_ [n.] "id." \
-- MHG _klein-ōt_ [n.] "jewel, gem", G _Kleinod_ [n.] "id." \
REF: [EDPG]\: 40

### <a id="audana">\*audana- "granted" (adj)</a>
N: \
-- ON _auðinn_ "granted, ordained" \
W: \
-- OE _ēaden_ "id." \
-- OS _ōdan_ "id." \
REF: [EDPG]\: 41

### <a id="augjan">\*augjan- [wk.] "to bring to attention" (v)</a>
E: \
-- Go. _at-augjan_ "to show" \
N: \
-- ON _eygja_ "to furnish with a loop; to look" \
-- Nw. _øygje_ "to see" \
W: \
-- OS _ōgan_ "to show" \
-- OHG _ocken_ "to show" \
REF: [EDPG]\: 41

### <a id="augōn">\*augōn- [n.] "eye" (n)</a>
E: \
-- Go. _augo_ \
N: \
-- ON _auga_ \
-- Far. _eyga_ \
-- Elfd. _oga_ \
W: \
-- OE _ēage_, E _eye_ \
-- OFri. _āge_ \
-- OS _ōga_ \
-- Du. _oog_ \
-- OHG _ouga_, G _Auge_ \
REF: [EDPG]\: 41

### <a id="aujō">\*aujō- [f.] "wetland, island" (n)</a>
N: \
-- ON _ey_ "island" \
-- Far. _oyggj_ "id." \
-- Elfd. _ä_ "id." \
W: \
-- OE _īeg_ "id." \
-- MDu. _ooi_, _ouwe_ "island in a river, wetland" \
-- OHG _ouwa_ "island, meadow", G _Aue_ "meadow" \
REF: [EDPG]\: 41

### <a id="aukan">\*aukan- [st.] "to grow" (v)</a>
E: \
-- Go. _aukan_ "to increase, multiply" \
N: \
-- ON _auka_ "id." \
-- Far. _eyka_ [wk.] "id." \
-- Elfd. _oka_ [wk.] "id." \
W: \
-- OE _ēacian_ [wk.] "id." \
-- OFri. _āka_ "id." \
-- OS _ōkian_ [st./wk.] "to propagate" \
-- OHG _ouhhōn_ [wk.] "to add" \
REF: [EDPG]\: 42

### <a id="auke">\*auke "and; again, but" (conj, adv)</a>
E: \
-- Go. _auk_ "furthermore, but, since" (conj) \
N: \
-- ON _auk_, _ok_ "besides" (adv) \
W: \
-- OE _ēac_ "with, besides" (prep), ME _ēk_ "also" (conj) \
-- OFri. _āk_, _ēk_ "too, even; but, however" \
-- OS _ōk_ "also, but" (adv) \
-- Du. _ook_ "too; also" \
-- OHG _ouh_ "also, even", G _auch_ "id." \
REF: [EDPG]\: 42

### <a id="aula">\*aula- ~ \*eula(n)- [m.] "stalk (of angelica)" (n)</a>
N: \
-- ON _jóli_ \
-- Icel. _hvann-jóli_, _njóli_ \
-- Far. _jólur_, -_jóli_ \
-- Nw. _aul_, _jol_ \
REF: [EDPG]\: 42

### <a id="aura">\*aura- [m.] "mud, sand, sediment" (n)</a>
N: \
-- ON _aurr_ "wet clay, mud" \
-- Far. _eyrur_ "coarse sand, gravel" \
W: \
-- OE _ēar_ "earth; sea" \
REF: [EDPG]\: 42

### <a id="ausan">\*ausan- [st.] "to scoop" (v)</a>
N: \
-- ON _ausa_ "to sprinkle, pour" \
-- Far. _oysa_ [wk.] "id." \
-- Elfd. _äsa_ [wk.] "id." \
W: \
-- WFri. _eazje_ [wk.] "to pour, rain" \
-- ODu. _osen_ [wk.] "to scoop out, make empty", Du. _hozen_ [wk.] "to scoop; to rain" \
-- MHG _ōsen_, _œsen_ [wk.] "to scoop out, make empty" \
REF: [EDPG]\: 43

### <a id="austera">\*austera- "east, eastwards" (adv)</a>
N: \
-- ON _austr_ \
-- Far. _eystur_ \
-- Elfd. _oster_ \
W: \
-- OE _ēast_, E _east_ \
-- OFri. _āster_ \
-- OS _ōstar_ \
-- MDu. _ooster_ \
-- OHG _ōstar_ \
REF: [EDPG]\: 43

### <a id="austrōn">\*austrōn- [f.] "Easter" (n)</a>
W: \
-- OE _ēastre_ "spring goddess", pl. "Easter" \
-- OHG _ōstara_, MHG _oster(n)_, G _Ostern_ [f.pl.] \
REF: [EDPG]\: 43

### <a id="auþja">\*auþja- "empty, void, desolate" (adj)</a>
E: \
-- Go. _auþs_, _auþeis_ \
N: \
-- ON _auðr_ (< \*auþa-) \
-- Far. _eyður_ \
W: \
-- NFri. F-A _ias_, Ha. _iä_ \
-- OHG _ōdi_ "empty, deserted", MHG _œde_ "empty, barren", G _ōde_ "empty" \
REF: [EDPG]\: 43

### <a id="auþu">\*auþu- "easy" (adj)</a>
N: \
-- ON _auð_- (pref) \
-- Far. _eyð_- (pref) \
W: \
-- OE _īeðe_ \
-- OS _ōthi_ \
-- MDu. _ode_ \
-- OHG _ōdi_ \
REF: [EDPG]\: 43

### <a id="auzōn">\*auzōn- [n.] "ear" (n)</a>
E: \
-- Go. _auso_ \
N: \
-- ON _eyra_ \
-- Far. _oyra_ \
-- Elfd. _ära_ \
W: \
-- OE _ēare_, E _ear_ \
-- OFri. _āre_ \
-- OS _ōa_ \
-- Du. _oar_ \
-- OHG _ōra_, G _Ohr_ \
REF: [EDPG]\: 44

### <a id="awadi">\*awadi- [f.] "eiderduck" (n)</a>
N: \
-- ON _æðr_ \
-- Far. _æður_ \
-- Nw. _ær-fugl_, dial. _ær_, _æd_ \
-- Sw. _åda_ \
REF: [EDPG]\: 44

### <a id="awala">\*awala/ō- [m./f.] "awl" (n)</a>
N: \
-- ON _soð-áll_ [m.] "meat awl" \
W: \
-- OE _awul_ [f.] "flesh-hook", ME _a(u)l_ "awl, spike, quill", E _awl_ \
REF: [EDPG]\: 44

### <a id="awan">\*awa/ōn- [m./f.] "grandparent" (n)</a>
E: \
-- Go. _awo_ [f.] "grandmother" \
N: \
-- ON _ái_ [m.] "great-grandfather" \
REF: [EDPG]\: 44

### <a id="awi">\*awi- [f.] "ewe" (n)</a>
N: \
-- ON _ær_ \
-- Far. _ær_ \
W: \
-- OE _ēow_, E _ewe_ \
-- OFri. _ei_ \
-- MDu. _ooi(e)_, _ouw(e)_, Du. _ooi_ \
-- OHG _ouwi_, _ou_ \
REF: [EDPG]\: 45

### <a id="awidja">\*awidja- [n.] "flock of sheep" (n)</a>
E: \
-- Go. _aweþi_ \
W: \
-- OE _eowde_, _eowede_ \
-- OHG _ouwiti_, _ewiti_ \
REF: [EDPG]\: 45

### <a id="awistra">\*awist(r)a- [n.] "sheepfold" (n)</a>
E: \
-- Go. _awistr_ \
W: \
-- OE _ēowestre_ [f.] \
-- OFri. _ēstra_, _ēster_ [m./f.] "fenced place for milking" \
-- OHG _ewist_, _ouwist_ [m.] "sheepfold" \
REF: [EDPG]\: 45

### <a id="awiz">\*awiz "obvious" (adv)</a>
W: \
-- OE _ēawis_ "apparent" (< \*awi-wissa-) \
-- OHG _awi-zoraht_ "evident" (< \*awi-turhta-) \
REF: [EDPG]\: 45

### <a id="azani">\*azani- [f.] "harvest" (n)</a>
E: \
-- Go. _asans_ \
N: \
-- ON _ǫnn_ \
-- OSw. _ann_ \
-- Elfd. _ann-tið_ \
W: \
-- OHG _aran_ [m.], MHG _erne_ \
REF: [EDPG]\: 46

## B

### <a id="ba">\*ba- "both" (num)</a>
E: \
-- Go. _bai_, acc. _bans_, gen. _baddje_, [n.] _ba_ \
N: \
-- ON _báðir_, acc. _báða_, [f.] _báðar_, [n.] _bæði_, gen. _beggja_, dat. _báðum_ \
-- Far. _báðir_, [f.] _báðar_, [n.] _bæði_, gen. _beggja_, dat. _báðum_ \
-- Elfd. _båðer_, [n.] _båðų_ \
W: \
-- OE _bēgen_ [m.] (with secondary ending from "two": _bēgen twēgen_, lit. "both two"), _bā_ (_þā_) [f.], _bū_ [n.], E _both_ \
-- OFri. _bēthe_ \
-- OS _bē-thia_, -_e_ [m./f.], _bē-thiu_ [n.] \
-- Du. _beide_ \
-- OHG _bei dē_, G _beide_ \
REF: [EDPG]\: 46

### <a id="badja">\*badja- [n.] "bed, bedding" (n)</a>
E: \
-- Go. _badi_ "bed, pallet" \
N: \
-- ON _beðr_ [m.] "bolster, bedding" \
-- Far. poet. _beður_ [m.] "bed" \
-- Nw. _bed_ "flower bed" \
W: \
-- OE _bed_ "bed", E _bed_ \
-- ODu. _beddi_ "id.", Du. _bed_ "id." \
-- OHG _betti_ "id.", G _Bett_ "bed", _Beet_ "flower bed" \
REF: [EDPG]\: 46

### <a id="badōjan">\*badōjan- [wk.] "to frighten" (v)</a>
N: \
-- Nw. _bada_ "to weigh down, press; to knead" \
W: \
-- OS _undar-badon_ "to frighten" \
REF: [EDPG]\: 47

### <a id="badwō">\*badwō- [f.] "battle" (n)</a>
N: \
-- ON poet. _bǫð_, gen. -_var_ \
W: \
-- OE _beado_ \
-- OS _badu_ \
-- OHG _batu_- \
REF: [EDPG]\: 47

### <a id="bagma">\*bagma- [m.] "beam, tree" (n)</a>
E: \
-- Go. _bagms_ "tree" \
N: \
-- ON _baðmr_ \
-- OSw. _baghn_ [n.] "tree trunk" \
W: \
-- OE _bēam_ "tree, pillar, beam, post", E _beam_ \
-- OFri. _bām_ "tree; pole" \
-- OS _bōm_ "id." \
-- Du. _boom_ [c.] "id." \
-- OHG _boum_ "id.", G _Baum_ "id." \
REF: [EDPG]\: 47

### <a id="baidjan">\*baidjan- [wk.] "to force, demand" (v)</a>
E: \
-- Go. _baidjan_ "to force" \
N: \
-- ON _beiða_ "to ask, request" \
-- Far. _beiða_ "to request, demand; to trouble" \
W: \
-- OE _bǣdan_ \
-- OHG _beitten_, MHG _beiten_ "to wait" \
REF: [EDPG]\: 47

### <a id="baina">\*baina- [n.] "bone, leg" (n)</a>
N: \
-- ON _bein_ \
W: \
-- OE _bān_ "bone", E _bone_ \
-- OFri. _bēn_ \
-- OS _bēn_ \
-- Du. _been_ \
-- OHG _bein_, G _Bein_ \
REF: [EDPG]\: 48

### <a id="baira">\*baira- [m.] "boar" (n)</a>
W: \
-- OE _bār_, E _boar_ \
-- OS _bēr_ \
-- Du. _beer_ [c.] \
-- OHG _bēr_ \
REF: [EDPG]\: 48

### <a id="baita">\*baita- [n.] "boat" (n)</a>
N: \
-- ON _beit_ \
W: \
-- OE _bāt_ [m.], E _boat_ \
-- MDu. _beitel_ [m.] "small boat" (< \*baitila-) \
REF: [EDPG]\: 48

### <a id="baka">\*baka- [n.] "back" (n)</a>
N: \
-- ON _bak_ \
-- Far. _bak_ \
W: \
-- OE _bæc_, E _back_ \
-- OFri. _bek_ [m.] \
-- OS _bak_ \
REF: [EDPG]\: 48

### <a id="bakja">\*bakja- [m.] "creek" (n)</a>
N: \
-- ON _bekkr_ \
-- Far. _bekkur_ \
W: \
-- OE _bece_ \
-- OS _beki_ \
-- Du. _beek_ [c.] \
-- OHG _bah_, G _Bach_ \
REF: [EDPG]\: 48

### <a id="bakkan">\*bak(k)an- [st.] "to bake, fry" (v)</a>
W: \
-- OE _bacan_ "to bake", E _to bake_ \
-- MDu. _backen_ [st./wk.] "to bake", Du. _bakken_ [st./wk.] "id." \
-- OHG _bahhan_, _backan_ "id.", MHG _bachen_, _backen_ "id.", G _backen_ [wk.] "id." \
REF: [EDPG]\: 49

### <a id="bakkōn">\*bak(k)ōn- [wk.] "to bake, fry" (v)</a>
N: \
-- ON _baka_ "to bake; to warm; to rub" \
-- Far. _baka_ "to bake; to warm the udder of a cow" \
-- Elfd. _båkå_ \
W: \
-- OHG _backōn_ \
REF: [EDPG]\: 49

### <a id="balgi">\*balgi- [m.] "skin bag" (n)</a>
E: \
-- Go. _balgs_ "leather bag, bag made of skin" \
N: \
-- ON _belgr_ "skin; skin bag; bellows" \
-- Elfd. _bōg_ "bag, skin bag" \
W: \
-- OE _belig_ "bag; bellows", E _belly_, _bellows_ \
-- OFri. _blēs-balch_ "bellows", WFri. _bealch_ [c.] "trunk, belly; bulge" \
-- MDu. _balch_ "bellows", Du. _blaas-balg_ [c.] "id." \
-- OHG _balg_ "skin, tube, pod", G _Balg_ "id." \
REF: [EDPG]\: 49

### <a id="balgjan">\*balgjan- [wk.] "to make swell, make angry" (v)</a>
N: \
-- ON _belgja_ "to inflate" \
W: \
-- OE _ā-belgan_ "to anger, to irritate" \
-- OS _belgian_ "id." \
-- OHG _gi-belgen_ "to irritate" \
REF: [EDPG]\: 49

### <a id="balikan">\*balika/ōn- [m./f.] "coot" (n)</a>
W: \
-- OS _beliko_ [m.] \
-- OHG _belihha_ [f.], MHG _belche_ [f.], G _Bōlch_, _Belche_ [f.] \
REF: [EDPG]\: 50

### <a id="ballan">\*ballan- [m.] "ball" (n)</a>
N: \
-- ON _bǫllr_ \
-- Far. _bøllur_ "lump, clump; something rolled up" \
-- Sw. _boll_ \
W: \
-- E _ball_ \
-- MDu. _bal_ "ball (of the foot)", Du. _bal_ [c.] "id." \
-- OHG _ballo_, _bal_, G _Ball_, _Ballen_ \
REF: [EDPG]\: 50

### <a id="baltja">\*baltja- [m./n.] "belt" (n)</a>
N: \
-- ON _belti_ [n.] \
W: \
-- OE _belt_ [m.], E _belt_ \
-- OHG _balz_ [m.] "belt, neck-chain" \
REF: [EDPG]\: 50

### <a id="balþa">\*balþ/da- "brave" (adj)</a>
E: \
-- Go. _balþs_\* "bold, frank" \
N: \
-- ON _ballr_ "dangerous, dire" \
W: \
-- OE _beald_, E _bold_ \
-- OFri. _balde_ "soon" (adv) \
-- MDu. _boude_ "bold, brave", Du. _boud_ "frank, daring" \
-- OHG _bald_ "bold, strong, intense", G _bald_ "soon" (adv) \
REF: [EDPG]\: 50

### <a id="balwa">\*balwa- [n.] "evil" (n)</a>
N: \
-- ON _bǫl_ "misfortune" \
W: \
-- OE _bealo_, E _bale_ \
-- OS _balo_ \
-- OHG _balo_ [m./n.] "evil, harm, disease" \
REF: [EDPG]\: 50

### <a id="banda">\*banda- [n.] "band, bond" (n)</a>
N: \
-- ON _band_ "binding; band, cord; bond" \
-- Far. _band_ "string" \
-- Elfd. _band_ "id." \
W: \
-- OE _beand_ [m.], E _band_ \
-- OFri. _bend_ [m.] \
-- OS _band_ \
-- Du. _band_ [c.] "band; tire" \
REF: [EDPG]\: 51

### <a id="bandī">\*bandī- [f.] "bond, fetter" (n)</a>
E: \
-- Go. _bandi_ \
W: \
-- OE _bend_ \
-- OFri. _be(i)nde_ \
-- MDu. _bende_ \
REF: [EDPG]\: 51

### <a id="bandjan">\*bandjan- [wk.] "to bend (a bow)" (v)</a>
N: \
-- ON _benda_ "to bend" \
W: \
-- OE _bendan_ "id." \
REF: [EDPG]\: 51

### <a id="bandwjan">\*bandwjan- [wk.] "to give a sign, beckon" (v)</a>
E: \
-- Go. _bandwjan_ \
N: \
-- ON _benda_ \
REF: [EDPG]\: 51

### <a id="banjō">\*banjō- [f.] "wound" (n)</a>
E: \
-- Go. _banja_ \
N: \
-- ON _ben_ \
-- Far. _ben_ "wound; cut" \
W: \
-- OE _benu_ \
-- OS _beni-wunda_ \
REF: [EDPG]\: 51

### <a id="bankan">\*bankan- [m.] "elevation" (n)</a>
N: \
-- ON _bakki_ "hill, slope" \
-- Far. _bakki_ "slope, cliff, (river or fog) bank" \
-- Elfd. _bokke_ "slope; hill" \
REF: [EDPG]\: 51

### <a id="banki">\*banki- [m.] "bench" (n)</a>
N: \
-- ON _bekkr_ \
-- Far. _bekkur_ "thwart; bench" \
-- Elfd. _baink_ \
W: \
-- OE _benc_ [f.], E _bench_ \
-- OFri. _bank_, _benk_ [m./f.] \
-- OS _bank_ \
-- Du. _bank_ [c.] \
-- OHG _banc_, G _Bank_ \
REF: [EDPG]\: 51

### <a id="bannan">\*bannan- [st.] "to command, summon" (v)</a>
N: \
-- ON _banna_ [wk.] "to forbid, prohibit; to curse" \
-- Far. _banna_ [wk.] "to curse; to swear (an oath), to vow" \
W: \
-- OE _bannan_, _bonnan_ "to summon" \
-- OFri. _bonna_, _banna_ "id." \
-- MDu. _bannen_ "id." \
-- OHG _bannan_ "to ban, order, summon" \
REF: [EDPG]\: 52

### <a id="bansta">\*bans(t)a- [m.] "cowshed" (n)</a>
E: \
-- Go. _bansts_ "barn" \
N: \
-- ON _báss_ "stall in a cow-barn" \
-- Far. _básur_ "id." \
-- Elfd. _bą̊s_ [n.] "id." \
W: \
-- OE _bōsig_ "cowshed", E _boosy_ \
-- OFri. _bōs_ "cow-barn" \
-- MDu. _banst_, _baenst_ "round basket made of straw or reed", Du. dial. _boes_ [c.] "id." \
-- MHG _banse_ [m./f.] "shelter, lean-to", G _Banse_ [m./f.] "id." \
REF: [EDPG]\: 52

### <a id="bariz">\*bariz- ~ \*barza- [n.] "barley" (n)</a>
N: \
-- ON _barr_ \
W: \
-- OE _bere_ [m.], E _barley_ \
-- NFri. _berre_ \
REF: [EDPG]\: 52

### <a id="barjan">\*barjan- [wk.] "to fight" (v)</a>
N: \
-- ON _berja_ "to beat, strike" \
-- Far. _berja_ "to bear, strike; to knock, hammer; to smash" \
-- Elfd. _berra-s_ "to resist" \
W: \
-- OE _bered_ ptc. "vexed, oppressed, crushed" \
-- OHG _berjan_ "to hit, knock", MHG _berjen_, _bern_ "id." \
REF: [EDPG]\: 53

### <a id="barkjan">\*barkjan- [wk.] "to roar" (v)</a>
N: \
-- Icel. _berkja_ [st.] "to boast" \
REF: [EDPG]\: 53

### <a id="barku">\*barku- [m.] "bark" (n)</a>
N: \
-- ON _bǫrkr_ \
-- Icel. _bōrkur_ "bark, rind" \
-- Far. _børkur_ "bark; skin on milk" \
W: \
-- WPhal. _bark_ "id." \
REF: [EDPG]\: 53

### <a id="barna">\*barna- [n.] "child" (n)</a>
E: \
-- Go. _barn_ \
N: \
-- ON _barn_ \
W: \
-- OE _bearn_ \
-- OFri. _bern_, WFri. _bern_, _barn_ \
-- OS _barn_ \
-- OHG _barn_, MHG _barn_ \
REF: [EDPG]\: 53

### <a id="barsa">\*barsa- [m.] "pike, bass" (n)</a>
W: \
-- OE _bears_, _bærs_, E _bass_ \
-- WFri. _bears_ [c.] \
-- MDu. _barse_, _baerse_, Du. _baars_ [c.] \
-- OHG _bars_, G _Barsch_ \
REF: [EDPG]\: 53

### <a id="baruga">\*baruga- [m.] "barrow" (n)</a>
N: \
-- ON _bǫrgr_ \
W: \
-- OE _bearg_, _bearh_, E _barrow_ \
-- Du. _barg_ [c.] \
-- OHG _barug_, MHG _barc_ \
REF: [EDPG]\: 54

### <a id="barwa">\*barwa- [m.] "grove(?)" (n)</a>
N: \
-- ON _bǫrr_ "unknown dendronym" \
W: \
-- OE _bearu_ "grove" \
-- OHG _bara-wāri_ "forest keeper; priest" \
REF: [EDPG]\: 54

### <a id="barzda">\*barzda- [m.] "edge, brim; beard" (n)</a>
E: \
-- Go. Crim. _bars_ \
N: \
-- ON _barð_ [n.] "brim, prow; beard" \
-- Far. _barð_ [n.] "stern; promontory" \
-- Nw. _bard_ "edge, side" \
W: \
-- OE _beard_ "beard", E _beard_ \
-- OFri. _berd_ "id." \
-- Du. _baard_ [c.] "id." \
-- OHG _bart_ "id.", G _Bart_ "id." \
REF: [EDPG]\: 54

### <a id="basja">\*basja- ~ \*bazja- [n.] "berry" (n)</a>
E: \
-- Go. _basi_ \
N: \
-- ON _ber_ \
-- Far. _ber_ \
-- Elfd. _ber_ \
W: \
-- OE _berige_ [f.] "berry, grape", E _berry_ \
-- OS _beri_ \
-- MDu. _bes_, _besie_ [f.] "berry, grape; bunch of grapes", Du. _bes_ [c.] "berry" \
-- OHG _beri_ "berry, grape", G _Beere_ [f.] "berry" \
REF: [EDPG]\: 54

### <a id="basta">\*basta- [m.] "bast" (n)</a>
N: \
-- ON _bast_ [n.] "bast, inner bark; string" \
W: \
-- OE _bæst_ [m./n.]? "id.", E _bast_ \
-- Du. _bast_ [c.] "id." \
-- OHG _bast_ "bast, rope", G _Bast_ \
REF: [EDPG]\: 55

### <a id="batiz">\*batiz "better" (adv)</a>
E: \
-- Go. _batis_ \
N: \
-- ON _betr_ \
-- Far. _betur_ \
W: \
-- OE _bet_ \
-- OS _bat_ \
-- MDu. _bet_ \
-- OHG _baz_ \
REF: [EDPG]\: 55

### <a id="baþa">\*baþa- [n.] "bath" (n)</a>
N: \
-- ON _bað_ \
-- Far. _bað_ \
-- Elfd. _bað_ \
W: \
-- OE _bæþ_, E _bath_ \
-- OFri. _beth_ \
-- OS _bath_ \
-- Du. _bad_ \
-- OHG _bad_, G _Bad_ \
REF: [EDPG]\: 55

### <a id="baugjan">\*baugjan- [wk.] "to (make) bend" (v)</a>
N: \
-- ON _beygja_ \
-- Far. _boyggja_ \
W: \
-- OE _bīegan_ \
-- OS _bōgian_ \
-- OHG _bougen_, G _beugen_ \
REF: [EDPG]\: 55

### <a id="baunō">\*baunō- [f.] "bean" (n)</a>
N: \
-- ON _baun_ \
W: \
-- OE _bēan_, E _bean_ \
-- OS _bōna_ \
-- Du. _boon_ [c.] \
-- OHG _bōna_, G _Bohne_ \
REF: [EDPG]\: 55

### <a id="bausa">\*bausa- "evil" (adj)</a>
N: \
-- Nw. _baus_ "proud" \
W: \
-- OFri. _bās_\* "obscene" \
-- OS _gi-bōsi_ "vain" \
-- ODu. _bōsi_ "evil, wicked", Du. _boos_ "angry; evil" \
-- OHG _bōsi_ "weak, useless", G _böse_ "wicked, evil, mean" \
REF: [EDPG]\: 56

### <a id="bautan">\*bautan- [st.] "to beat" (v)</a>
N: \
-- ON _bauta_ [wk.] "to beat; to chase" \
W: \
-- OE _bēatan_ "to beat, strike", E _to beat_ \
-- MDu. _boten_ "to hit, beat" \
-- OHG _bōzan_ "id.", MHG _bōzen_ [st./wk.] "id." \
REF: [EDPG]\: 56

### <a id="baza">\*baza- "naked, visible" (adj)</a>
N: \
-- ON _berr_ \
-- Far. _berur_ \
-- Elfd. _ber_ \
W: \
-- OE _bær_, E _bare_ \
-- OFri. _ber_ \
-- OS _bar_ \
-- Du. _barre-voets_ "barefoot" (adv) \
-- OHG _bar_ "naked, bare; pure; public", G _bar_ "id." \
REF: [EDPG]\: 56

### <a id="bebura-1">\*bebura- [m.] "beaver" (n)</a>
N: \
-- ON _bjórr_ \
-- Elfd. _biuor_ \
W: \
-- OE _beofor_, E _beaver_ \
-- OS _biƀar_, _biƀor_ \
-- Du. _bever_ [c.] \
-- OHG _bibar_, _bibur_, G _Biber_ \
REF: [EDPG]\: 56

### <a id="bebura-2">\*bebura- [m.] "piece of skin, cloth(?)" (n)</a>
N: \
-- ON _bjórr_ "triangular cut off piece of skin; land; front wall, party wall" \
-- Far. _bjóri_ "patch, strip, slip" \
-- Nw. _bjor(e)_ "wedge-shaped patch" \
-- Sw. _bjur-ås_ "cross-beams supporting the thatch of a roof" \
-- Gutn. _bjaur_ "stern post" \
REF: [EDPG]\: 57

### <a id="bedjan">\*bedjan- [st.] "to ask, beg, pray" (v)</a>
E: \
-- Go. _bidjan_ \
N: \
-- ON _biðja_ \
-- Far. _biðja_ \
-- Elfd. _biða_ \
W: \
-- OE _biddan_, E _to bid_ \
-- OFri. _bidda_ [wk.] "to ask, bid, order" \
-- OS _biddian_ [wk.] "to ask" \
-- Du. _bidden_ "to pray" \
-- OHG _bitten_ "to ask; to pray", G _bitten_ "id." \
REF: [EDPG]\: 57

### <a id="bēla">\*bēla- [n.] "(bon)fire, (funeral) pyre" (n)</a>
N: \
-- ON _bál_ "fire; pyre" \
-- Far. _bál_ "(bon)fire" \
W: \
-- OE _bǣl_ "id." \
REF: [EDPG]\: 57

### <a id="belgan">\*belgan- [st.] "to swell" (v)</a>
N: \
-- ON _bolginn_ ptc. "swollen" \
W: \
-- OE _belgan_ "to swell with anger" \
-- OFri. _for-bolgen_ ptc. "angry" \
-- OS _ar-belgan_ "to move to anger" \
-- EDu. _ver-bolgen_ "angered" \
-- OHG _belgan_ "to be or grow angry", MHG _belgen_ "to swell (up)", refl. "to become angry" \
REF: [EDPG]\: 58

### <a id="belkan">\*belkan- ~ \*balkan- ~ \*bulkan- [m.] "beam" (n)</a>
N: \
-- ON _bjalki_, _bǫlkr_ \
W: \
-- OE _balca_ "beam, bank, ridge", E _balk_ \
-- OFri. _balka_ "roof beam, rafter" \
-- MLG _balke_ "beam, hayloft, ceiling" \
-- Du. _balk_ [c.] "beam, bar" \
-- OHG _balko_ "plank, winepress, part of a ship", G _Balken_ "beam, bar" \
REF: [EDPG]\: 58

### <a id="bellan">\*bellan- [st.] "to roar, bark" (v)</a>
N: \
-- Icel. _bjalla_ [wk.] "to chatter; to croak" \
W: \
-- OE _bellan_ "to bellow, roar, bark" \
-- MDu. _bellen_ [wk.] "to bark" \
-- OHG _bellan_ "to bark, resound", MHG _bellen_ "to bark", G _bellen_ [wk.] "id." \
REF: [EDPG]\: 58

### <a id="bellōn">\*bellōn- [f.] "knoll(?)" (n)</a>
N: \
-- Icel. _bjalla_ "hill" \
-- Sw. _fota-bjälle_ [n.] "heel" \
REF: [EDPG]\: 58

### <a id="belunōn">\*belunōn- ~ \*bulmōn- [f.] "henbane" (n)</a>
N: \
-- OSw. _bulma_ \
-- Sw. _bolma_, _bolm-ōrt_ \
-- ODa. _bylne_, EDa. _bølme_, _bulme_, Da. _bulme-urt_ \
W: \
-- OE _beolone_ \
-- OS _bilina_ \
-- MDu. _belne_ \
REF: [EDPG]\: 59

### <a id="benuta">\*benuta- [m.] "bentgrass" (n)</a>
W: \
-- OE _beonet_, E _bent_ \
-- OFri. _benet_ "bentgrass(?)", OWFri. _bjint_ [c./n.], EFri. _bjunt_ [n.] \
-- OHG _binuz_, MHG _binez_, _binz_, G _Binse_ [f.] \
REF: [EDPG]\: 59

### <a id="beran-1">\*beran- [st.] "to bear, carry, give birth" (v)</a>
E: \
-- Go. _bairan_ "to bear, bring; to endure, suffer; to give birth" \
N: \
-- ON _bera_ "to bear, carry; to wear; to produce, yield" \
-- Elfd. _bjärå_ "id." \
W: \
-- OE _beran_ "id.", E _to bear_ \
-- OFri. _bera_ "to carry; to give birth" \
-- OS _beran_ "to bear, possess" \
-- Du. _baren_ [wk.] "to give birth" \
-- OHG _beran_ "to bear, produce, bring forth, render", G _ge-bähren_ "to bear, give birth" \
REF: [EDPG]\: 59

### <a id="beran-2">\*beran- [m.] "bear" (n)</a>
N: \
-- OSw. _Biari_ (prop) \
W: \
-- OE _bera_ \
-- MDu. _bere_, Du. _beer_ [c.] \
-- OHG _bero_, G _Bär_ \
REF: [EDPG]\: 59

### <a id="berga">\*berga- [m./n.] "mountain" (n)</a>
N: \
-- ON _bjarg_, _berg_ [n.] \
W: \
-- OE _beorg_ [m.], E _barrow_ \
-- OFri. _berch_ [m.] \
-- OS _berg_ [m.] \
-- Du. _berg_ [c.] \
-- OHG _berg_, _berag_ [m.], G _Berg_ [m.] \
REF: [EDPG]\: 60

### <a id="bergan">\*bergan- [st.] "to keep (safe)" (v)</a>
E: \
-- Go. _bairgan_ "to preserve, protect, keep" \
N: \
-- ON _bjarga_ [wk.] "to save, help" \
W: \
-- OE _beorgan_ "to spare" \
-- Du. _bergen_ "to hide; to salvage" \
-- OHG _bergan_ "to protect, hide", G _bergen_ "to contain, retrieve, salvage" \
REF: [EDPG]\: 60

### <a id="berhta">\*berhta- "bright" (adj)</a>
E: \
-- Go. _bairhts_ "bright, clear, manifest, evident" \
N: \
-- ON _bjartr_ "bright, shining; illustrious" \
W: \
-- OE _beorht_, E _bright_ \
-- OS _berht_ "shining" \
-- OHG _beraht_ "bright, shining", MHG _berht_ "id." \
REF: [EDPG]\: 60

### <a id="bēri">\*bēri- "portable" (adj)</a>
N: \
-- ON _bærr_ "due, entitled to" \
-- Far. -_bærur_ "capable of bearing" \
REF: [EDPG]\: 61

### <a id="berkan">\*berkan- [st.] "to roar" (v)</a>
W: \
-- OE _beorcan_ "to make a sharp explosive sound, bark", E _to bark_ \
REF: [EDPG]\: 61

### <a id="berkō">\*berkō- [f.] "birch-tree" (n)</a>
N: \
-- ON _bjǫrk_ \
W: \
-- OE _beorc_, E _birch_ \
-- Du. _berk_ [c.] \
REF: [EDPG]\: 61

### <a id="bērō">\*bērō- [f.] "bier" (n)</a>
W: \
-- OE _bār_, _bǣr_ , E _bier_ \
-- OFri. _bēre_ \
-- OS _bāra_ \
-- Du. _baar_ [c.] \
-- OHG _bāra_, G _Bahre_ \
REF: [EDPG]\: 61

### <a id="beudan">\*beudan- [st.] "to command; to offer" (v)</a>
E: \
-- Go. _biudan_ \
N: \
-- ON _bjóða_ "to offer; to invite" \
-- Far. _bjóða_ "to offer; to invite; to command" \
-- Elfd. _biuoða_ "to offer; to assist" \
W: \
-- OE _bēodan_ "to bid; to command, order", E _to bid_ \
-- OS _biodan_ "to offer" \
-- Du. _bieden_ "id." \
-- OHG _biotan_ "id.", G _bieten_ "id." \
REF: [EDPG]\: 61

### <a id="beugan">\*beugan- ~ \*biigan- [st.] "to bow, bend" (v)</a>
E: \
-- Go. _biugan_ \
N: \
-- ON _bjúga_ \
-- Far. _bogin_ "bent" \
W: \
-- OE _būgan_ "to bend", E _to bow_ \
-- MDu. _bugen_, Du. _buigen_ \
-- OHG _biogan_ "to bend, swing", G _biegen_ \
REF: [EDPG]\: 61

### <a id="beura">\*beura- [n.] "beer" (n)</a>
N: \
-- ON _bjórr_ [m.] \
-- Far. _bjór_ \
W: \
-- OE _bēor_, E _beer_ \
-- MDu. _bier_, Du. _bier_ \
-- OHG _bior_, G _Bier_ \
REF: [EDPG]\: 62

### <a id="beusta">\*beusta- [m.] "beestings" (n)</a>
W: \
-- OE _bēost_, E _beestings_ \
-- WFri. _bjist_ [c.], NFri. Hall. _biaost_ [n.] \
-- MDu. _biest_ [f.], Du. _biest_ [c.] \
-- OHG _biost_, G _Biest_ \
REF: [EDPG]\: 62

### <a id="bewwa">\*bewwa- [n.] "yield; barley" (n)</a>
N: \
-- ON _bygg_ "barley" \
-- Far. _bygg_ "id." \
-- Elfd. _begg_ "id." \
W: \
-- OE _bēow_ "id." \
-- OS _beu_ [n.(?)] "harvest" \
REF: [EDPG]\: 63

### <a id="bīan">\*bīan-(?) [st?] "to forsee(?), worry(?)" (v)</a>
N: \
-- Far. _bíggja_ [wk.] "to herald, announce" \
-- Nw. _bjå_ [wk.] "to tremble; to warn, predict something bad; to care about; to be of use" \
REF: [EDPG]\: 63

### <a id="bibēn">\*bibēn- [wk.] "to tremble" (v)</a>
N: \
-- ON _bifa_ \
-- Far. _biva_ \
W: \
-- OE _beofian_ \
-- OFri. _beva_ \
-- OS _beƀon_ \
-- Du. _beven_ \
-- OHG _bibēn_, G _beben_ \
REF: [EDPG]\: 63

### <a id="bīdan">\*bīdan- [st.] "to wait" (v)</a>
E: \
-- Go. _beidan_ "to await, look for" \
N: \
-- ON _bíða_ "to wait for; to suffer, undergo" \
-- Far. _bíða_ \
-- Elfd. _baiða_ \
W: \
-- OE _bīdan_ "to wait, remain", E _to bide_ \
-- OFri. _bīdia_ [wk.] "id." \
-- OS _bīdan_ "to wait, stay, expect" \
-- MDu. _biden_ \
-- OHG _bītan_ "to wait, anticipate, hope", MHG _bīten_ "to await" \
REF: [EDPG]\: 63

### <a id="bijēn">\*bi(j)ēn- [wk.] "to hit" (v)</a>
N: \
-- Icel. _bjá_ "to fight, struggle" \
REF: [EDPG]\: 64

### <a id="bila">\*bila- "equal, even" (adj)</a>
W: \
-- OS _bile-wit_ "ambidextrous" \
-- MDu. _bil-lijc_ "reasonable, right" (adv), Du. _bil-lijk_ "id." (adv) \
-- OHG _bil-līh_ "id." (adv), MHG _un-bil_ "unappropriated, injust" \
REF: [EDPG]\: 64

### <a id="bilōn">\*bilōn- [wk.] "to give way" (v)</a>
N: \
-- ON _bila_ "to give way; to fail" \
-- Icel. _bila_ "to fail, break down, give out" \
-- Far. _bila_ "to be missing; to fail, betray, let down; to err" \
-- Nw. _bila_ "to yield, give way, fail, lack" \
-- Sw. _bila_ "to run out, come to an end" \
W: \
-- Du. arch., dial. _belen_ "to pull out (from an agreement), to resile" \
(E)Du. _be(e)len_ (Plantijn 1573: _beel zijn_ "_se repentir d'un marché, marriage, et semblable_"), has not yet received an etymology, but since Nordic _bila_ appears to have had a primary meaning "to give way, to fail", there are no semantic objections against connecting it. No certain extra-Gm. etymology; perhaps to be compared to Lith. _bailùs_, Latv. _baîls_ "afraid" < \*bʰoih₂-l-u-, for which cf. [\*bīan-](#bian). \
REF: [EDPG]\: 64

### <a id="bindan">\*bindan- [st.] "to bind" (v)</a>
E: \
-- Go. _bindan_ \
N: \
-- ON _binda_ \
W: \
-- OE _bindan_, E _to bind_ \
-- OFri. _binda_ \
-- OS _bindan_ \
-- Du. _binden_ \
-- OHG _bintan_, G _binden_ \
REF: [EDPG]\: 64

### <a id="bīōn">\*bīōn- [f.] "bee" (n)</a>
N: \
-- ON _bý_ [n.], _bý-jluga_ \
-- Far. _bý-fluga_ \
-- Nw. _bie_ \
-- Elfd. _bia_ (for expected \*_baja_) \
W: \
-- OE _bēo_, E _bee_ \
-- OHG _bīa_, _bīan_ [m.], MHG _bīe_, _bī̆n_ [m.], G _Biene_ \
REF: [EDPG]\: 64

### <a id="bisōn-1">\*bīsōn- [f.] "wind" (n)</a>
W: \
-- Du. Flem. _bijs_ "rain shower, hail storm" \
-- OHG _bīsa_ "storm", MHG _bise_ "id." \
REF: [EDPG]\: 65

### <a id="bisōn-2">\*bisōn- [wk.] "to run around (of cattle chased by insects)" (v)</a>
N: \
-- OSw. _bisa_ \
-- Sw. _besa_ \
W: \
-- Du. Flem. _biezen_ "to run around (of cattle)" \
-- OHG _bisōn_ "id.", MHG _bisen_ "id.", G _biesen_ "id." \
REF: [EDPG]\: 65

### <a id="bitan-1">\*bītan- [st.] "to bite, be sharp" (v)</a>
E: \
-- Go. _beitan_ \
N: \
-- ON _bíta_ \
-- Far. _bíta_ \
-- Elfd. _baita_ \
W: \
-- OE _bītan_, E _to bite_ \
-- OFri. _bīta_ \
-- OS _bītan_ \
-- Du. _bijten_ \
-- OHG _bīzan_, G _beißen_ \
REF: [EDPG]\: 65

### <a id="bitan-2">\*bitan- [m.] "bite; little bit, fragment" (n)</a>
N: \
-- ON _biti_ \
W: \
-- OE _bita_, E _bit_ \
-- MDu. _bete_, Du. _beet_ [c.] \
-- OHG _bizzo_, G _Bissen_ \
REF: [EDPG]\: 65

### <a id="bitlan">\*bitlan- [m.] "beetle" (n)</a>
N: \
-- Da. _bille_ [c.] \
W: \
-- OE _bitela_ \
REF: [EDPG]\: 65

### <a id="bitra">\*bitra- "bitter, sharp" (adj)</a>
N: \
-- ON _bitr_ "biting, sharp; bitter" \
-- Far. poet. _bitur_ "sharp, keen" \
-- Elfd. _bitter_ "bitter; angry" \
W: \
-- OE _bitor_, _bitter_, _bittor_ "bitter", E _bitter_ \
-- OS _bittar_ "id." \
-- Du. _bitter_ "id." \
-- OHG _bittar_ "bitter, harsh, spicy", G _bitter_ "bitter" \
REF: [EDPG]\: 66

### <a id="bīþla">\*bīþla- [m.] "axe" (n)</a>
N: \
-- ON _bíldr_ \
W: \
-- MDu. _bijl_ [f./n.], Du. _bijl_ [c.] \
-- OHG _bīhal_ [n.], G _Beil_ [n.] \
REF: [EDPG]\: 66

### <a id="blada">\*blada- [n.] "leaf, blade" (n)</a>
N: \
-- ON _blað_ \
-- Far. _blað_ \
-- Elfd. _blað_ \
W: \
-- OE _blæd_ [f.] "cup, bowl" \
-- OFri. -_bleth_ "surface" \
-- OS _blad_ "leaf" \
-- Du. _blad_ "leaf; surface" \
-- OHG _blat_ "leaf; sheet", G _Blatt_ "id." \
REF: [EDPG]\: 66

### <a id="blahjōn">\*blahjōn- [f.] "cloth" (n)</a>
N: \
-- ON _blæja_ "fine colored cloth; cover of a bed; burial sheet" \
W: \
-- OHG _blaha_ "coarse linen", G _Blache_, _Blahe_, _Plane_ "id." \
REF: [EDPG]\: 66

### <a id="blaika">\*blaika- "pale" (adj)</a>
N: \
-- ON _bleikr_ \
W: \
-- OE _blāc_ \
-- OS _blēk_ \
-- Du. _bleek_ \
-- OHG _bleih_, G _bleich_ \
REF: [EDPG]\: 66

### <a id="blaita">\*blaita- "pale" (adj)</a>
W: \
-- OE _blāt_ \
REF: [EDPG]\: 66

### <a id="blandan">\*blandan- [st.] "to mix, mingle" (v)</a>
E: \
-- Go. _blandan_ (_sik_) \
N: \
-- ON _blanda_ "to blend, mix" \
W: \
-- OE _blandan_, _blondan_ [wk.] "id." \
-- OHG _blantan_ "id." \
REF: [EDPG]\: 66

### <a id="blanka">\*blanka- "colorless(?)" (adj)</a>
N: \
-- ON _blakkr_ "black, dun-colored" \
-- Elfd. _blokk_ "pale" \
W: \
-- Du. _blank_ "white, transparent; inundated" \
-- OHG _blanc_ "shining, white", G _blank_ "shiny, sheer" \
REF: [EDPG]\: 67

### <a id="blauta">\*blauta- "soft" (adj)</a>
N: \
-- ON _blautr_ "soft, weak; wet, soaked" \
-- Far. _bleytur_ "soft, tender, sensitive" \
-- Elfd. _blot_ \
W: \
-- OE _blēat_ "miserable" \
-- OFri. _blāt_ "poor; naked" \
-- Du. _bloot_ "naked" \
-- OHG _blōz_ "naked", G _bloß_ "id." \
REF: [EDPG]\: 67

### <a id="blauþu">\*blauþu- "soft, weak, timid" (adj)</a>
N: \
-- ON _blauðr_ \
W: \
-- OE _blēaþ_ \
-- OHG _blōdi_ "lazy, timid", G _blöde_ "dumb, witless" \
REF: [EDPG]\: 67

### <a id="blēan">\*blēan- [st.] "to blow" (v)</a>
W: \
-- OE _blāwan_, E _to blow_ \
-- OFri. _bliā(n)_ \
-- MDu. _blaeyen_ [wk.] \
-- OHG _blāen_ [wk./st.], MHG _blæjen_ [wk.] (ptc. _geblæt_ / _geblān_) "to blow; to smelt", G _blähen_ [wk.] "to swell, distend" \
REF: [EDPG]\: 67

### <a id="blejan">\*blejan- [wk.] "to bleat" (v)</a>
W: \
-- E obs. _to blea_ \
-- MLG _blēen_ \
-- OHG _blāen_(?), MHG _blæjen_, _plēhen_ \
REF: [EDPG]\: 68

### <a id="blēsan">\*blēsan- [st.] "to blow" (v)</a>
E: \
-- Go. _uf-blesan_ "to blow up" \
N: \
-- ON _blása_ \
-- Far. _blása_ [st./wk.] \
-- Elfd. _blåsa_ \
W: \
-- MDu. _blasen_, Du. _blazen_ \
-- OHG _blāsan_, G _blasen_ \
REF: [EDPG]\: 68

### <a id="blēstu">\*blēstu- [m.] "blowing, blast" (n)</a>
N: \
-- ON _blástr_, dat. _blæsti_ "blast, blowing, breathing" \
-- Far. _blástur_, _blóstur_ "breathing; wind; windy weather" \
-- Elfd. _bläst_ "blast, gust" \
W: \
-- OE _blǣst_ "blast", E _blast_ \
-- OHG _blāst_ "breath", MHG _blāst_ "breath, blow" \
REF: [EDPG]\: 68

### <a id="blēwa">\*blēwa- "blue" (adj)</a>
N: \
-- ON _blár_ "blue, livid, black" \
-- Far. _bláur_ "blue; dark" \
-- Elfd. _blå_ \
W: \
-- OE _blāw_, E Scot. _blae_ "blackish; livid, pale" \
-- OFri. _blāw_ "id." \
-- Du. _blauw_ "id." \
-- OHG _blāo_ "blue, dark, grey", G _blau_ "id." \
REF: [EDPG]\: 68

### <a id="blewwan">\*blewwan- [st.] "to beat (up), blow" (v)</a>
E: \
-- Go. _bliggwan_ \
W: \
-- E _to blow_ \
-- MDu. _blouwen_ \
-- OHG _bliuwan_ \
REF: [EDPG]\: 69

### <a id="blikan">\*blikan- [st.] "to shine" (v)</a>
W: \
-- OE _blīcan_ "to shine, glitter" \
-- OFri. _blīka_ "to be visible" \
-- OS _blīkan_ \
-- Du. _blijken_ "to appear" \
-- MHG _blīchen_ \
REF: [EDPG]\: 69

### <a id="blinda">\*blinda- "blind" (adj)</a>
E: \
-- Go. _blinds_ \
N: \
-- ON _blindr_ \
W: \
-- OE _blind_ \
-- OFri. _blind_ \
-- OS _blind_ \
-- Du. _blind_ \
-- OHG _blint_, G _blind_ \
REF: [EDPG]\: 69

### <a id="blīþa">\*blīþa- "mild, kind" (adj)</a>
E: \
-- Go. _bleiþs_\* "merciful" \
N: \
-- ON _blíðr_ "gentle, friendly; pleasant" \
W: \
-- OE _blīðe_ "joyful, glad", E _blithe_ \
-- OS _blīthi_ "joyful, happy" \
-- Du. _blij_ "id." \
-- OHG _blīdi_ "id." \
REF: [EDPG]\: 69

### <a id="bliwa-1">\*blīwa- [n.] "color, hue" (n)</a>
W: \
-- OE _blēo_ "color, hue, complexion", ME _blē_ "color, hue; appearance, guise" \
-- OFri. _blī_ [n.(?)] "complexion" \
-- OS _blī_ "color" \
-- MDu. _blie_ "complexion" \
REF: [EDPG]\: 69

### <a id="bliwa-2">\*blīwa- [n.] "lead" (n)</a>
N: \
-- ON _blý_ \
-- Far. _blýggj_ \
W: \
-- OS _blī_ \
-- OHG _blīo_, G _Blei_ \
REF: [EDPG]\: 69

### <a id="blōan">\*blōan- [st.] "to bloom, flourish" (v)</a>
W: \
-- OE _blōwan_ \
-- OFri. _bloia_ \
-- OS _blōian_ \
-- Du. _bloeien_ [wk.] \
-- OHG _bluoen_, G _blühen_ [wk.] \
REF: [EDPG]\: 69

### <a id="blōda">\*blōda- [n.] "blood" (n)</a>
E: \
-- Go. _bloþ_ \
N: \
-- ON _blóð_ \
-- Elfd. _bluoð_ \
W: \
-- OE _blōd_, E _blood_ \
-- OFri. _blōd_ \
-- MDu. _bloet_ "blood; blushing", Du. _bloed_ \
-- OHG _bluot_, G _Blut_ \
REF: [EDPG]\: 70

### <a id="blōdi">\*blōdi- [f.] "bloom" (n)</a>
W: \
-- OE _blēd_ "shoot, branch, flower, fruit" \
-- OHG _bluot_ "blossom, blossoming", G _Blüte_ "id." \
REF: [EDPG]\: 70

### <a id="blōman">\*blōman- [m.] "flower" (n)</a>
E: \
-- Go. _bloma_ \
N: \
-- ON _blómi_ \
-- Far. _blómi_, _blóma_ [f.] "flower, bloom" \
-- Elfd. _bljomme_ "id." \
W: \
-- OS _blōmo_ "id." \
-- Du. _bloem_ [c.] "id." \
-- OHG _bluomo_, _bluoma_ [f.] "id.", MHG _bluome_ [m./f.] "id.", G _Blume_ [f.] "id." \
REF: [EDPG]\: 70

### <a id="blōtan">\*blōtan- [st.] "to sacrifice" (v)</a>
E: \
-- Go. _blotan_ "to serve, worship" \
N: \
-- ON _blóta_ "to worship; to sacrifice" \
W: \
-- OE _blōtan_ \
-- OHG _blōzan_ [st./wk.] \
REF: [EDPG]\: 70

### <a id="bōan">\*bōan- ~ \*būan- [st.] "to live, dwell" (v)</a>
E: \
-- Go. _bauan_ [st./wk.] \
N: \
-- ON _búa_ "to live; to prepare, fix" \
-- Far. _búgva_ [st./wk.] "id." \
W: \
-- OE _būan_, _būgan_ \
-- OS _būan_ [wk.] \
-- Du. _bouwen_ [wk.] "to build" \
-- OHG _bū(w)an_, _bū(w)en_ [st./wk.] "to live, dwell; to build", G _bauen_ [wk.] "to build" \
REF: [EDPG]\: 71

### <a id="bōgu">\*bōgu- [m.] "shoulder, upper arm" (n)</a>
N: \
-- ON _bógr_ "shoulder" \
-- Elfd. _buog_ "id." \
W: \
-- OE _bōg_ "shoulder, arm, branch", E _bow_ \
-- Du. _boeg_ [c.] "bow" \
-- OHG _buog_ "shoulder, hip, bow", G _Bug_ "id." \
REF: [EDPG]\: 71

### <a id="bōk">\*bōk- [f.] "book" (n)</a>
N: \
-- ON _bók_, pl. _bœkr_ \
-- Far. _bók_, pl. _bøkr_ \
-- Elfd. _buok_, pl. _byōker_ \
W: \
-- OE _bōc_, pl. _bēc_, E _book_ \
-- OFri. _bōk_ \
REF: [EDPG]\: 71

### <a id="bōkjō">\*bōk(j)ō- [f.] "beech" (n)</a>
N: \
-- ON _bók_ \
-- Far. _bók_ \
W: \
-- OE _bōc_, _bēce_, E _beech_ \
-- OS _bōka_ \
-- Du. _beuk_ [c.] \
-- OHG _buohha_, G _Buche_ \
REF: [EDPG]\: 71

### <a id="bōni">\*bōni- [f.] "request, prayer" (n)</a>
N: \
-- ON _bœn_ \
-- Far. _bøn_ \
-- Elfd. _byōn_ \
W: \
-- OE _bōn_ \
REF: [EDPG]\: 72

### <a id="bōnjan">\*bōnjan- [wk.] "to decorate" (v)</a>
W: \
-- OE _bōnian_ "to ornament" \
-- MDu. _boenen_ "to polish", Du. boenen "id." \
-- MHG _büenen_ "id." \
REF: [EDPG]\: 72

### <a id="bōsma">\*bōsma- [m.] "bosom" (n)</a>
W: \
-- OE _bōsm_, E _bosom_ \
-- OFri. _bōsem_ \
-- MDu. _boesem_, _bosem_, Du. _boezem_ [c.] \
-- OHG _buosum_ "bosom, lap", G _Busen_ "id." \
REF: [EDPG]\: 72

### <a id="bōtō">\*bōtō- [f.] "improvement" (n)</a>
E: \
-- Go. _bota_ "advantage, benefit" \
N: \
-- ON _bót_ "improvement; compensation; patch" \
-- Far. _bót_ "id." \
-- Elfd. _buot_ "cure, remedy; fine" \
W: \
-- OE _bōt_ "help; remedy" \
-- OFri. _bōte_ "recovery; compensation, fine" \
-- OS _bōta_ "healing; penitence" \
-- Du. _boete_ [c.] "penitence; fine" \
-- OHG _buoz_ "id.", G _Buße_ "id." \
REF: [EDPG]\: 72

### <a id="bragna">\*bragna- [m.] "brain" (n)</a>
W: \
-- OE _brægen_ [n.] \
-- OFri. _brein_ [n.] \
-- MLG _bragen_, _bregen_ [n.] \
-- MDu. _bragen_, _brein_ [n.], Du. _brein_ [n.] \
REF: [EDPG]\: 73

### <a id="braida">\*braida- "broad" (adj)</a>
E: \
-- Go. _braiþs_ \
N: \
-- ON _breiðr_ \
-- Far. _breiður_ \
-- Elfd. _brieð_ \
W: \
-- OE _brād_, E _broad_ \
-- OFri. _brēd_ \
-- OS _brēd_ \
-- Du. _breed_ \
-- OHG _breit_, G _breit_ \
REF: [EDPG]\: 73

### <a id="brakka">\*brakka- "briny, brackish" (adj)</a>
W: \
-- MLG _brack_ \
-- Du. _brak_ (whence E _brack_, _brackish_) \
REF: [EDPG]\: 73

### <a id="brakkan">\*brakka(n)- [m.] "sleuth dog" (n)</a>
W: \
-- MDu. _bracke_, Du. _brak_ [c.] \
-- OHG _bracko_, G _Bracke_ \
REF: [EDPG]\: 73

### <a id="brakkōn">\*brakkōn-(?) [f.] "pants" (n)</a>
N: \
-- Elfd. _brakka_ \
-- EDa. _brakker_ pl. "lower leg covers" \
W: \
-- OE _braccas_ [m.pl.] "breeches" (< \*brakka-(?)) \
REF: [EDPG]\: 74

### <a id="brannjan">\*brannjan- [wk.] "to make burn" (v)</a>
E: \
-- Go. _brannjan_ \
N: \
-- ON _brenna_ \
W: \
-- OE _bærnan_ \
-- OHG _brennen_, G _brennen_ \
REF: [EDPG]\: 74

### <a id="branta">\*branta- "steep" (adj)</a>
N: \
-- ON _brattr_ "steep; sudden" \
W: \
-- OE _brant_, _bront_ "high, deep, steep, difficult" \
REF: [EDPG]\: 74

### <a id="brauda">\*brauda- [n.] "bread" (n)</a>
N: \
-- ON _brauð_ \
W: \
-- OE _brēad_, E _bread_ \
-- OFri. _brād_ \
-- OS _brōd_ \
-- Du. _brood_ \
-- OHG _brōt_, G _Brot_ \
REF: [EDPG]\: 74

### <a id="brazda">\*brazda- [n.] "edge, brim" (n)</a>
N: \
-- Icel. _bradd_ "edge" \
-- Nw. _bradd_ [m.] "shore, side" \
-- Elfd. _bradd_ [m.] "edge" \
W: \
-- OE _breard_ [m.] "brim, margin" \
-- OHG _brart_ "edge", MHG _brart_ [m.] "edge, board" \
REF: [EDPG]\: 74

### <a id="brēan">\*brēan- [st.] "to fume, smell" (v)</a>
W: \
-- MHG _bræhen_ [wk.] "to smell" \
REF: [EDPG]\: 74

### <a id="brekan">\*brekan- [st.] "to break" (v)</a>
E: \
-- Go. _brikan_ \
W: \
-- OE _brecan_, E _to break_ \
-- OFri. _breka_ \
-- OS _brekan_ \
-- Du. _breken_ \
-- OHG _brehhan_, G _brechen_ \
REF: [EDPG]\: 75

### <a id="breman">\*breman- ~ \*brimman- [st.] "to drone, hum" (v)</a>
W: \
-- OHG _breman_, MHG _bremen_, _brimmen_ \
REF: [EDPG]\: 75

### <a id="bremisan">\*bremisa/ōn- [m./f.] "gadfly" (n)</a>
W: \
-- OE _brēosa_, _brīosa_ [m.], _brimse_ [f.], ME _brēse_, _brimse_ "gadfly; locust" \
-- OS _brimissa_ [f.], MLG _bromese_ [f.] \
-- MDu. _breemse_, _bremse_ [f.] "gadfly; locust; beetle", Du. _brems_ [c.] \
REF: [EDPG]\: 75

### <a id="brestan">\*brestan- [st.] "to break, burst" (v)</a>
N: \
-- ON _bresta_ \
-- Far. _bresta_ "to break, collapse, strike" \
W: \
-- OE _berstan_ "to burst", E _to burst_ \
-- OFri. _bersta_ "to break; to disappear" \
-- OS _brestan_ \
-- Du. _barsten_ [wk.] \
-- OHG _brestan_ "to burst, tear; to lack", G _bersten_ "to crack" \
REF: [EDPG]\: 75

### <a id="breusta">\*breusta- [n.] "breast, chest" (n)</a>
N: \
-- ON _brjóst_ \
W: \
-- OE _brēost_, E _breast_ \
-- OFri. _briast_ \
-- OS _briost_ \
REF: [EDPG]\: 76

### <a id="breutan">\*breutan- [st.] "to break (open), bud" (v)</a>
N: \
-- ON _brjóta_ "to break, break open; to destroy" \
-- Far. _bróta_ "to break; to infringe; to offend" \
-- Elfd. _briuota_ "to break" \
W: \
-- OE _brēotan_ "id." \
-- MHG _briezen_ "to break open, to bud, swell up" \
REF: [EDPG]\: 76

### <a id="brēwō">\*brēwō- [f.] "eyebrow" (n)</a>
N: \
-- ON _brǫ́_, _brá_ "eyelid" \
-- Icel. _brá_ "eyelid; eyelash" \
-- Far. _brá_ "eyelash" \
W: \
-- OS _brāwa_ "eyelid" \
-- MDu. _brauwe_ "eyebrow; edge", Du. _wenk-brauw_ [c.] \
-- OHG _brāwa_, G _Augen-braue_ \
REF: [EDPG]\: 76

### <a id="brewwan">\*brewwan- [st.] "to brew" (v)</a>
N: \
-- ON _brugginn_ ptc. \
-- Far. _bryggja_ [wk.] \
-- OSw. _bryggia_ \
-- Elfd. _bryddja_ [wk.] \
W: \
-- OE _brēowan_, E _to brew_ \
-- OFri. _briouwa_, _brouwa_ \
-- OS _gi-breuwan_ \
-- Du. _brouwen_ [st./wk.] \
-- MHG _briuwen_, _brūwen_, G _brauen_ [wk.] \
REF: [EDPG]\: 76

### <a id="brezda">\*brezda- [n.] "side, edge; board" (n)</a>
N: \
-- Far. _breddi_ [m.] "edge, side" \
-- Nw. _bredd_ [m.] "id." \
-- OSw. _brædder_ [m.] "id." \
W: \
-- OHG _bret_ "board", G _Brett_ "id." \
REF: [EDPG]\: 77

### <a id="brindan">\*brinda(n)- [m.] "elk" (n)</a>
N: \
-- Elfd. _brinde_ \
-- Nw. _bringe_ "male elk" \
REF: [EDPG]\: 77

### <a id="bringan">\*bringan- [st.] "to bring" (v)</a>
E: \
-- Go. _briggan_ [wk.] \
W: \
-- OE _bringan_ [wk.], E _to bring_ \
-- OFri. _bringa_ [wk.] \
-- OS _bringan_ [wk.] \
-- Du. _brengen_ [wk.] \
-- OHG _bringan_ [wk.], G _bringen_ [wk.] \
REF: [EDPG]\: 77

### <a id="brinka">\*brinka- [m.] "edge" (n)</a>
N: \
-- ON _brekka_ [f.] "slope" (< \*brinkōn-) \
W: \
-- ME _brinke_, _brenke_ "seashore; rim, edge; margin, end, boundary", E _brink_ \
-- MLG _brink_ "hill, edge, shore" \
-- MDu. _brinc_ "edge, grass verge; stretch of grass; market", Du. _brink_ [c.] "village square, green" \
REF: [EDPG]\: 77

### <a id="brinnan">\*brinnan- [st.] "to burn" (v)</a>
E: \
-- Go. _brinnan_ \
N: \
-- ON _brinna_, _brenna_ \
-- Far. _brenna_ \
-- Elfd. _brinna_, pres. _brið_ \
W: \
-- OE _beornan_, E _to burn_ \
-- OFri. _berna_ [wk.] \
-- OS _brinnan_ \
-- OHG _brinnan_ \
REF: [EDPG]\: 77

### <a id="brōan">\*brōan- [st.] "to singe" (v)</a>
W: \
-- MDu. _broeyen_ [wk.], Du. _broeien_ [wk.] "to be brewing" \
-- MHG _brüejen_ [wk.] "to singe", G _brühen_ [wk.] "to boil, brew" \
REF: [EDPG]\: 78

### <a id="brōk">\*brōk- [f.] "loin-cloth, breeches; rump(?)" (n)</a>
N: \
-- ON _brōk_ "leg of a pair of breeches", pl. _brœkr_ "breeches" \
-- Far. _brók_ "id.", pl. _brøkur_ "pants" \
-- Elfd. _bruok_, pl. _bryōker_ "shorts; diapers" \
W: \
-- OE _brōc_ "behind, breech", pl. "breeches", E _breech(es)_ \
-- OFri. _brēk_ "pants" \
-- Du. _broek_ [c.] "id." \
-- OHG _bruoh_ "id." \
REF: [EDPG]\: 78

### <a id="brōka-1">\*brōka- [n.] "spot, speckle(?)" (n)</a>
N: \
-- Nw. _brok_ [m.] "young (speckled) salmon" \
-- Sw. dial. _brok_ "spot; spotted animal" \
REF: [EDPG]\: 78

### <a id="brōka-2">\*brōka- [m.] "swamp" (n)</a>
W: \
-- OE _brōc_, E _brook_ \
-- MLG _brōk_ [n.] "wet pasture" \
-- MDu. _broec_ "marsh", Du. _broek_ [n.] \
-- OHG _bruoh_ [n.], MHG _bruoch_ [n.] \
REF: [EDPG]\: 78

### <a id="brōþer">\*brōþer- [m.] "brother" (n)</a>
E: \
-- Go. _broþar_ \
N: \
-- ON _bróðir_, pl. _brœðr_ \
-- Far. _bróðir_, pl. _brøður_ \
-- Elfd. _bruoðer_, pl. _bryōðrer_ \
W: \
-- OE _brōðor_, E _brother_ \
-- OFri. _brōther_ \
-- OS _brōthar_ \
-- Du. _broer_ [c.] \
-- OHG _bruodar_, G _Bruder_ \
REF: [EDPG]\: 79

### <a id="brū">\*brū- [f.] "bridge" (n)</a>
N: \
-- ON _brú_ \
-- Far. _brúgv_ "bridge; quay; heap, pile; eyebrow" \
-- Nw. _bru_ "bridge", dial. "edge; eyebrow" \
W: \
-- OE _brū_ "eyebrow", E _brow_ \
REF: [EDPG]\: 79

### <a id="brūdi">\*brūdi- [f.] "bride" (n)</a>
E: \
-- Go. _bruþs_ \
N: \
-- ON _brúðr_ \
-- Far. _brúður_ \
-- Elfd. _brauðe_ \
W: \
-- OE _brȳd_, E _bride_ \
-- OFri. _breid_ \
-- OS _brūd_ \
-- Du. _bruid_ [c.] \
-- OHG _brūt_, G _Braut_ \
REF: [EDPG]\: 79

### <a id="bruki">\*bruki- [m./f.] "breach" (n)</a>
W: \
-- OE _bryce_ [f.], E _breach_ \
-- OFri. _breke_ [m./f.] \
-- MDu. _broke_ [f.], Du. _breuk_ [c.] \
-- OHG _bruh_ [m.], G _Bruch_ [m.] \
REF: [EDPG]\: 79

### <a id="brukkōn">\*bruk(k)ōn- [wk.] "to break, crumble" (v)</a>
N: \
-- Nw. _broka_ "to break, bite, tear" \
W: \
-- MDu. _brocken_, _broken_ "to bend, break" \
-- MHG _er-brochen_ "to crush, squash" \
REF: [EDPG]\: 79

### <a id="brummōn">\*brummōn- [wk.] "to drone, hum" (v)</a>
W: \
-- Du. _brommen_ \
-- MHG _brummen_, G _brummen_ \
REF: [EDPG]\: 80

### <a id="brunjōn">\*brunjōn- [f.] "breastplate" (n)</a>
E: \
-- Go. _brunjo_ \
N: \
-- ON _brynja_ "coat of mail" \
W: \
-- OE _byrne_ "id." \
-- OS _brunnia_ "id." \
-- OHG _brunja_, _brunna_ "id.", G _Brünne_ "id." \
REF: [EDPG]\: 80

### <a id="brunnan">\*brunna(n)- [m.] "well, spring" (n)</a>
E: \
-- Go. _brunna_ \
N: \
-- ON _brunnr_ \
W: \
-- OE _brunna_, _burna_ \
-- OS _brunno_ \
-- Du. _bron_ [c.] \
-- OHG _brunno_, G _Brunnen_ \
REF: [EDPG]\: 80

### <a id="bruska">\*bruska- [m.] "shrub" (n)</a>
N: \
-- Nw. _brusk_ "shrub, undergrowth" \
REF: [EDPG]\: 80

### <a id="brust">\*brust- [f.] "breast, chest" (n)</a>
E: \
-- Go. _brusts_ \
W: \
-- OFri. _brust_, _burst_ [n.] \
-- OS _brust_, MLG _borst_ \
-- Du. _borst_ [c.] \
-- OHG _brust_, G _Brust_ \
REF: [EDPG]\: 80

### <a id="brustu">\*brustu- [m.] "fissure" (n)</a>
W: \
-- OHG _brust_ "burst, rip", MHG _brust_ "id." \
REF: [EDPG]\: 80

### <a id="bruttōn">\*brut(t)ōn- [wk.] "to bud" (v)</a>
W: \
-- MHG _brozzen_, G Als. _brossen_ \
REF: [EDPG]\: 81

### <a id="bruwī">\*bru(w)ī- ~ \*brujjō- [f.] "bridge" (n)</a>
N: \
-- ON _bryggja_ "pier, quay; bridge" \
-- Far. _bryggja_ "wharf, quay; bridge of a ship" \
-- Elfd. _bryddja_ "dock" \
W: \
-- OE _brycg_, _bricg_, E _bridge_ \
-- OFri. _bregge_ "bridge; arch" \
-- OS _bruggia_ \
-- Du. _brug_ [c.] \
-- OHG _brucka_, G _Brücke_ \
REF: [EDPG]\: 81

### <a id="bruzda">\*bruzda- [m.] "spike" (n)</a>
N: \
-- ON _broddr_ \
-- Far. _broddur_ "thorn, prickle; spike" \
-- Elfd. _brudd_ "shoot, sprout" \
W: \
-- OE _brord_ "point; grass shoot" \
-- OHG _brort_ "spear; edge" \
REF: [EDPG]\: 81

### <a id="buda">\*buda- [n.] "offer" (n)</a>
N: \
-- ON _boð_ \
W: \
-- OE _bod_ "command; message" \
-- OFri. _bod_ "command; announcement" \
-- Du. _bod_ "bid" \
-- OHG _bot_ "authoritative pronouncement" \
REF: [EDPG]\: 81

### <a id="budan">\*budan- [m.] "messenger" (n)</a>
N: \
-- ON _boði_ \
W: \
-- OE _boda_ \
-- OFri. _boda_ \
-- OS _bodo_ \
-- Du. _bode_ [c.] \
-- OHG _boto_, G _Bote_ \
REF: [EDPG]\: 81

### <a id="budman">\*budman- ~ \*buttman- [m.] "bottom" (n)</a>
N: \
-- ON _botn_ \
-- Far. _botnur_ \
-- Elfd. _buottn_ \
W: \
-- OE _botm_, E _bottom_ \
-- OS _bodom_ \
-- Du. _bodem_ [c.] \
-- OHG _bodam_, G _Boden_ \
REF: [EDPG]\: 82

### <a id="bugan">\*bugan- [m.] "bow" (n)</a>
E: \
-- Go. Crim. _boga_ \
N: \
-- ON _bogi_ \
-- Elfd. _bugi_ \
W: \
-- OE _boga_, E _bow_ \
-- OFri. _boga_ \
-- OS _bogo_ \
-- Du. _boog_ [c.] \
-- OHG _bogo_, G _Bogen_ \
REF: [EDPG]\: 82

### <a id="bugjan">\*bugjan- [wk.] "to buy" (v)</a>
E: \
-- Go. _bugjan_ \
N: \
-- ON _byggja_ "to get married; to lend out, let out" \
W: \
-- OE _bycgan_, E _to buy_ \
-- OS _buggian_ "to buy, pay" \
REF: [EDPG]\: 82

### <a id="bugōn">\*bugōn- [wk.] "to brag(?)" (v)</a>
W: \
-- OE _bogian_, _boian_ "to boast" \
-- OFri. _bāgia_ "id." \
-- MDu. _bogen_ "id.", Du. _bogen op_ "id." \
REF: [EDPG]\: 82

### <a id="bukkan">\*bukka(n)- [m.] "billy-goat" (n)</a>
N: \
-- ON _bokkr_, _bukkr_, _bokki_ "fellow" \
-- Far. _bukkur_ \
-- Elfd. _bukk_ \
W: \
-- OE _bucc_ "roebuck", _bucca_, E _buck_ \
-- MDu. _boc_, Du. _bok_ [c.] \
-- OHG _bock_, G _Bock_ \
REF: [EDPG]\: 82

### <a id="bukkōn">\*bukk/gōn- [wk.] "to bend" (v)</a>
N: \
-- Far. _boga_, _buga_, _bukka_ "to bend, curve" \
-- Nw. _boga_ "to bend, swing", _bukka_ "to nod" \
W: \
-- OFri. _bukkia_ "to bend down" \
-- MHG _bocken_ "to alight" \
REF: [EDPG]\: 83

### <a id="bulgjan">\*bulgjan- ~ \*bulkjan- [wk.] "to bellow" (v)</a>
W: \
-- OE _bylgian_, E to _bellow_ \
-- MDu. _bulgen_ "to cough, rattle", Du. _bulken_ "to bellow" \
REF: [EDPG]\: 83

### <a id="bulgjōn">\*bulgjōn- [f.] "wave; bag" (n)</a>
N: \
-- ON _bylgja_ "wave" \
-- Far. _bylgja_ "id." \
W: \
-- E _billow_ \
-- MLG _bulge_ "wave; hide" \
-- MHG _bulge_ "wave; leather bag" \
REF: [EDPG]\: 83

### <a id="bullan-1">\*bullan- "ball" (n)</a>
N: \
-- ON _bolli_ [m.] "cup" \
-- Far. _bolli_ [m.] "cup, bowl; head; ball" \
W: \
-- OE _bolla_ [m.] "bowl" \
-- OFri. _bolla_ [m.] "type of (round?) bread" \
-- MDu. _bolle_ [m.] "sphere, round object", Du. _bol_ [c.] "id." \
-- OHG _bolla_ [f.] "bud; bowl" \
REF: [EDPG]\: 83

### <a id="bullan-2">\*bul(l)an- [m.] "bull" (n)</a>
N: \
-- ON _boli_ \
W: \
-- OE _bula_, E _bull_ \
-- MLG _bulle_ \
-- MDu. _bul(le)_, Du. _bul_ [c.] \
REF: [EDPG]\: 83

### <a id="bullōn">\*bullōn- [wk.] "to roar, howl" (v)</a>
N: \
-- Icel. _bulla_ "to bubble; to talk nonsense" \
-- Far. _bulla_ "to gush, bubble; to chatter, babble" \
W: \
-- MDu. _bullen_ "to bubble up" \
-- OHG _bullōn_ "to howl, bark, roar", MHG _bullen_ "id." \
REF: [EDPG]\: 84

### <a id="bulstran">\*bulstra(n)- [m.] "bolster, pillow" (n)</a>
N: \
-- ON _bolstr_ \
W: \
-- OE _bolster_ [n.] \
-- Du. _bolster_ "husk" \
-- OHG _bolstar_ "cushion", G _Polster_ "id." \
REF: [EDPG]\: 84

### <a id="bultōn">\*bultōn- [wk.] "to roll, turn over" (v)</a>
N: \
-- Far. _bólta_ "to turn over, overturn, tumble" \
W: \
-- G _bolzen_ "to kick around (a ball)" \
REF: [EDPG]\: 84

### <a id="bunda">\*bunda- [m.] "binding" (n)</a>
W: \
-- OS _gi-bund_ [n.] "bundle" \
-- Du. _bond_ [c.] "league" \
-- MHG _bunt_ [m./n.] "fetter; bundle", G _Bund_ "league; bundle" \
REF: [EDPG]\: 84

### <a id="būra">\*būra- [n.] "cabin, hut" (n)</a>
N: \
-- ON _búr_ "women's apartment; pantry; storehouse" \
W: \
-- OE _būr_ [m.] "hut, chamber", E _bower_ \
-- OS _būr_ "chamber, dwelling" \
-- OHG _būr_ [m.] "dwelling; cage", G _Bauer_ [m.] "(bird)cage" \
REF: [EDPG]\: 84

### <a id="burdi">\*burdi- [f.] "birth" (n)</a>
E: \
-- Go. _ga-baurþs_ \
N: \
-- ON _burðr_ [m.] "birth; foetus" \
W: \
-- OE _(ge-)byrd_, E _birth_ \
-- OFri. _berde_ "birth; foetus" \
-- OS _gi-burd_ "birth, descent" \
-- Du. _ge-boorte_ [c.] \
-- OHG _gi-burt_, G _Ge-burt_ \
REF: [EDPG]\: 84

### <a id="burg">\*burg- [f.] "fortified place, town" (n)</a>
E: \
-- Go. _baurgs_ \
N: \
-- ON _borg_ "town; citadel; small hill" \
-- Far. _borg_ "castle; town" \
W: \
-- OE _burg_ "city", E _borough_ \
-- OFri. _burch_, _burich_ [m.] "castle; city" \
-- OS _burg_ "id." \
-- OHG _burg_ "id.", MHG _burc_ "id." \
REF: [EDPG]\: 85

### <a id="burgjan">\*burgjan- [wk.] "to hide(?)" (v)</a>
N: \
-- ON _byrgja_ "to close, shut" \
W: \
-- OE _byrian_ "to bury", E _to bury_ \
REF: [EDPG]\: 85

### <a id="buri">\*buri- [m.] "son" (n)</a>
E: \
-- Go. _baur_ \
N: \
-- ON poet. _burr_ \
W: \
-- OE _byre_ "son, child" \
REF: [EDPG]\: 85

### <a id="burjan">\*burjan- [wk.] "to raise; to come to pass" (v)</a>
N: \
-- ON _byrja_ "to start" \
W: \
-- OE _ge-byrian_ "id." \
-- OFri. _bera_ "to raise, collect" \
-- OS _gi-burian_ "to happen" \
-- Du. _beuren_ "to collect money", _ge-beuren_ "to happen" \
-- OHG _burren_ "to raise", _gi-burren_ "to happen", MHG _bürn_ "id.", G _ge-bühren_ "to collect charges" \
REF: [EDPG]\: 85

### <a id="burkōn">\*burkōn- [wk.] "to roar" (v)</a>
N: \
-- Nw. _borka_ "to boast" \
W: \
-- OE _borcian_ "to bark" \
REF: [EDPG]\: 85

### <a id="burōjan">\*burōjan- [wk.] "to bore" (v)</a>
N: \
-- ON _bora_ \
W: \
-- OE _borian_, E _to bore_ \
-- OS _boron_ \
-- Du. _boren_ \
-- OHG _borōn_, G _bohren_ \
REF: [EDPG]\: 85

### <a id="burzda">\*burzda- [n.] "board" (n)</a>
E: \
-- Go. _fotu-baurd_ "footstool" \
N: \
-- ON _borð_ "board, plank, table" \
-- Far. _borð_ "id." \
W: \
-- OE _bord_ "board, table" \
-- OFri. _bord_ [m./n.]? "id." \
-- Du. _bord_ "plate, plank" \
REF: [EDPG]\: 86

### <a id="buttōn">\*butt/dōn- [wk.] "to shove" (v)</a>
N: \
-- Nw. _butta_ "to hit, shove" \
W: \
-- OFri. _del-boddia_ "to knock down" \
-- MDu. _botten_ "to bump, hit" \
-- G Als. _butzen_ "id." \
REF: [EDPG]\: 86

### <a id="buwwēn">\*buwwēn- [wk.] "to dwell; to form, build" (v)</a>
N: \
-- ON _byggva_, _byggja_ \
-- Far. _byggja_ \
-- Elfd. _byddja_ \
W: \
-- OE _bogian_, _bug(i)an_ "to inhabit" \
-- OFri. _bogian_ "to dwell" \
REF: [EDPG]\: 86

## D

### <a id="daban">\*daban- [st.] "to fit" (v)</a>
E: \
-- Go. _ga-daban_ "to happen, be suitable, be appertinent to" \
REF: [EDPG]\: 86

### <a id="daga">\*daga- [m.] "day" (n)</a>
E: \
-- Go. _dags_ \
N: \
-- ON _dagr_ \
-- Far. _dagur_ \
-- Elfd. _dag_ \
W: \
-- OE _dreg_, E _day_ \
-- OFri. _dei_ \
-- OS _dag_ \
-- Du. _dag_ [c.] \
-- OHG _tac_, G _Tag_ \
REF: [EDPG]\: 86

### <a id="daiga">\*daiga- [m.] "dough" (n)</a>
E: \
-- Go. _daig_ \
N: \
-- ON _deigr_ \
-- Far. _deiggj_ [n.] \
-- Elfd. _dieg_ \
W: \
-- OE _dah_, E _dough_ \
-- MDu. _deech_ [n.], Du. _deeg_ [n.] \
-- OHG _teig_, G _Teig_ \
REF: [EDPG]\: 87

### <a id="daila">\*daila- [n.] "share, part" (n)</a>
W: \
-- OE _dāl_, E _dole_ \
-- OFri. _dēl_ [m./n.] \
-- Du. _deel_ \
-- OHG _teil_ [m./n.], G _Teil_ [m.] \
REF: [EDPG]\: 87

### <a id="dajjan">\*dajjan- [wk.] "to suckle" (v)</a>
E: \
-- Go. _daddjan_ \
N: \
-- OSw. _dæggia_ \
W: \
-- OHG _taen_ \
REF: [EDPG]\: 87

### <a id="dala">\*dala- [m.] "valley" (n)</a>
E: \
-- Go. _dal(s)_ [m./n.] \
N: \
-- ON _dalr_ \
-- Far. _dalur_ \
-- Elfd. _dal_ \
W: \
-- OE _dæl_ [n.], E _dale_ \
-- OS _dal_ [n.] \
-- Du. _dal_ [n.] \
-- OHG _tal_ [n.], G _Tal_ [n.] \
REF: [EDPG]\: 87

### <a id="dampa">\*dampa- [m.] "vapor" (n)</a>
W: \
-- ME _damp_ "fire damp" \
-- MDu. _damp_ "vapor, fog", Du. _damp_ [c.] "id." \
-- MHG _tampf_, _dampf_ "id.", G _Dampf_ "id." \
REF: [EDPG]\: 88

### <a id="dangjan">\*dangjan- [wk.] "to beat" (v)</a>
N: \
-- ON _dengja_ "to beat, to hammer" \
-- Far. _deingja_ "to beat, strike; to fling, throw; to hang about" \
-- Elfd. _daindja_ "to hit, bear" \
W: \
-- OE _dencgan_ "to knock" \
-- MHG _tengen_ "to hit, knock, hammer" \
REF: [EDPG]\: 88

### <a id="dankōn">\*dank/gōn- [wk.] "to beat" (v)</a>
N: \
-- Icel. _danga_ "to beat, hit" \
-- Nw. _dakka_ "to slap" \
-- Sw. dial. _danka_ "to hit" \
REF: [EDPG]\: 88

### <a id="dappōn-1">\*dapp/bōn- [wk.] "to become heavy, sink, deteriorate" (v)</a>
N: \
-- Nw. _dabba_ "to fade, decrease, deteriorate (esp. of sight)", _dapa_ "to fade; become downcast" \
REF: [EDPG]\: 88

### <a id="dappōn-2">\*dapp/bōn- [wk.] "to beat" (v)</a>
N: \
-- Nw. _dabba_ "to beat, kick" \
W: \
-- ME _dabben_ "to strike", E _to dab_ \
-- G Pal. _tapfen_, _tappen_ "to grope" \
REF: [EDPG]\: 88

### <a id="dapra">\*dapra- "heavy" (adj)</a>
N: \
-- ON _dapr_ "downcast, sad" \
-- Icel. _dapur_ "sad", _sjón-dapur_ "blind" \
-- Nw. _daper_ "shy, weak, with young" \
W: \
-- MLG _dapper_ "heavy" \
-- MDu. _dapper_ "stout, strong", Du. _dapper_ "brave" \
-- G _tapfer_ "brave, stout" \
REF: [EDPG]\: 89

### <a id="darbja">\*darbja- "hostile" (adj)</a>
W: \
-- OFri. _derve_ "fierce, severe" \
-- OS _derƀi_ "powerful, hostile, bad", MLG _derve_ "bad" \
REF: [EDPG]\: 89

### <a id="darbjan">\*darbjan- [wk.] "to destroy" (v)</a>
W: \
-- MLG _derven_ "to shrink, wither, spoil" \
-- MHG _ver-derben_ "to ruin, kill" \
REF: [EDPG]\: 89

### <a id="darjan">\*darjan- [wk.] "to hurt" (v)</a>
W: \
-- OE _derian_ \
-- OFri. _dera_ \
-- OS _derian_ \
-- Du. _deren_ \
-- OHG _terjen_, _terren_, MHG _teren_, _tern_ \
REF: [EDPG]\: 89

### <a id="darran">\*darran- [m.] "hinge" (n)</a>
N: \
-- Nw. _darre_ "hinge; vertebra" \
REF: [EDPG]\: 89

### <a id="dauba">\*dauba- "deaf; sluggish; numb" (adj)</a>
E: \
-- Go. _daubs_ \
N: \
-- ON _daufr_ \
-- Far. _deyvur_ \
-- Elfd. _dov_ \
W: \
-- OE _dēaf_, E _deaf_ \
-- OFri. _dāf_ \
-- Du. _doof_ \
-- OHG _toub_, G _taub_ \
REF: [EDPG]\: 89

### <a id="dauda">\*dauda- "dead" (adj)</a>
E: \
-- Go. _dauþs_ \
N: \
-- ON _dauðr_ \
W: \
-- OE _dēad_, E _dead_ \
-- OFri. _dāth_ \
-- OS _dōd_ \
-- Du. _dood_ \
-- OHG _tōt_, G _tot_ \
REF: [EDPG]\: 90

### <a id="daugjan">\*daugjan- [wk.] "to endure" (v)</a>
W: \
-- OE _ge-dīegan_ \
-- OS _ā-dōgian_ "to withstand" \
REF: [EDPG]\: 90

### <a id="daujan">\*daujan- [st.] "to die" (v)</a>
N: \
-- ON _deyja_ \
-- Far. _doyggja_ [wk.] \
-- Elfd. _däa_ \
W: \
-- OE _dīegan_, E _to die_ \
-- OS _dōian_ [wk.] \
-- OHG _touwen_, _tewen_ [wk.], MHG _touwen_, _tōuwen_ [wk.] \
REF: [EDPG]\: 90

### <a id="dauma">\*dauma- [m.] "vapor" (n)</a>
N: \
-- Far. _deymur_ "strong smell" \
W: \
-- MDu. _doom_ "mist, haze" \
-- OHG _tuom_ "vapor, haze, fume", MHG _tuom_ "id." \
REF: [EDPG]\: 90

### <a id="dauna">\*dauna- [m.] "smell" (n)</a>
N: \
-- ON _daunn_ \
-- OSw. _døn_ [m./n.] \
REF: [EDPG]\: 90

### <a id="daupjan">\*daupjan- [wk.] "to dip" (v)</a>
E: \
-- Go. _daupjan_ "immerse, baptize" \
N: \
-- ON _deypa_ "to dip; to baptize" \
-- Far. _doypa_ "to baptize" \
W: \
-- OFri. _depa_ "id." \
-- OS _dopian_ "id." \
-- Du. _dopen_ "id." \
-- OHG _toufen_ "id.", G _taufen_ "id." \
REF: [EDPG]\: 91

### <a id="dauþu">\*dauþu- [m.] "death" (n)</a>
E: \
-- Go. _dauþus_ \
N: \
-- ON _dauðr_, gen. -_ar_ \
W: \
-- OE _dēaþ_, E _death_ \
-- OFri. _dāth_ \
-- OS _dōđ_ \
-- Du. _dood_ [c.] \
-- OHG _tōt_, G _Tod_ \
REF: [EDPG]\: 91

### <a id="dauzan">\*dauzan- [m.] "fool" (n)</a>
W: \
-- MLG _dōre_ \
-- MHG _tōre_ \
REF: [EDPG]\: 91

### <a id="dawēn">\*dawēn- [wk.] "to marvel" (v)</a>
N: \
-- ON _dá_ "to admire" \
REF: [EDPG]\: 91

### <a id="dawwa">\*dawwa/ō- [m./f.] "dew" (n)</a>
N: \
-- ON _dǫgg_ [f.] \
-- Far. _døgg_ [f.] \
-- Elfd. _dagg_ [f.] \
W: \
-- OE _dēaw_ [m.], E _dew_ \
-- OFri. _daw_ [n.] \
-- ODu. _dou_ [m.], Du. _dauw_ [c.] \
-- OHG _tou_ [m./n.], G _Tau_ [m.] \
REF: [EDPG]\: 91

### <a id="dazēn">\*dazēn- [wk.] "to be numbed(?)" (v)</a>
W: \
-- OE _darian_ "to lie motionless, be appalled", ME _daren_ "to stay in one place, remain quiet; to lurk; to be motionless, inactive; to hesitate" \
-- MDu. _be-daren_ "to control oneself", Du. _be-daren_ "to settle, calm down", Flem. _ver-daren_ "to be appalled" \
REF: [EDPG]\: 91

### <a id="dēan">\*dēan- [st.] "to suckle" (v)</a>
W: \
-- OHG _tāan_, _tāen_ [wk.], G Cimb. _taien_ [wk.] \
REF: [EDPG]\: 92

### <a id="dēdi">\*dēdi- [f.] "deed, action" (n)</a>
E: \
-- Go. _ga-deþs_ \
N: \
-- ON _dáð_ \
-- Far. _dáð_ \
-- Elfd. _dåð_ \
W: \
-- OE _dǣd_, E _deed_ \
-- OFri. _dēde_, WFri. _died_ [c.] \
-- OS _dād_ \
-- Du. _daad_ [c.] \
-- MHG _tāt_ "deed, action, thing" \
REF: [EDPG]\: 92

### <a id="delban">\*delban- [st.] "to dig, delve" (v)</a>
W: \
-- OE _delfan_ \
-- OS _bi-delƀan_ "to bury" \
-- Du. _delven_ "to mine" \
-- OHG _bi-telban_ "to bury", MHG _telben_, _delben_ "to dig" \
REF: [EDPG]\: 92

### <a id="dēli">\*dēli- "easily to deal with(?)" (adj)</a>
N: \
-- ON _dæll_ "gentle, easy to deal with" \
-- Nw. dial. _dæl_ "easy-going, good to talk to" \
-- Elfd. _däl_ "trustworthy" \
REF: [EDPG]\: 92

### <a id="deli">\*deli- ~ \*delja- [m.] "dill" (n)</a>
W: \
-- OE _dile_, _dyle_, E _dill_ \
-- OS _dilli_ \
-- Du. _dille_ [c.] \
-- OHG _tilli_, MHG _tille_, _tülle_ \
REF: [EDPG]\: 92

### <a id="delō">\*delō- [f.] "nipple" (n)</a>
W: \
-- OE _delu_ \
-- OHG _tila_ \
REF: [EDPG]\: 93

### <a id="denra">\*denra- [m.] "hollow hand" (n)</a>
W: \
-- OHG _tenar_, _tenara_ [f.], MHG _tener_ \
REF: [EDPG]\: 93

### <a id="derba">\*derba- "brave" (adj)</a>
N: \
-- ON _djarfr_ "bold, daring" \
W: \
-- OE _deorf_ "valiant" \
-- OS _derƀi_ "powerful" \
REF: [EDPG]\: 93

### <a id="derban">\*derban- [st.] "to perish" (v)</a>
W: \
-- OE _deorfan_ "to labor, perish" \
-- MLG _vor-derven_ "to perish" \
-- MDu. _be-derven_ "to be damaged; to perish", Du. _be-derven_ [wk.] "to spoil; to go bad" \
-- MHG _ver-derben_, -_terben_ "to be killed, to die" \
REF: [EDPG]\: 93

### <a id="derka">\*derka- "dark" (adj)</a>
W: \
-- OE _deorc_, E _dark_ \
REF: [EDPG]\: 93

### <a id="deuhtra">\*deuhtra- [n.] "grandchild" (n)</a>
W: \
-- MHG _diehter_, _tiehter_, G Bav. _Tichter_ \
REF: [EDPG]\: 93

### <a id="deupa">\*deupa- "deep" (adj)</a>
E: \
-- Go. _diups_ \
N: \
-- ON _djúpr_ \
-- Far. _djúpur_ \
-- Elfd. _diuop_ \
W: \
-- OE _dēop_, E _deep_ \
-- OFri. _diāp_ \
-- OS _diop_ \
-- Du. _diep_ \
-- OHG _tiuf_, G _tief_ \
REF: [EDPG]\: 94

### <a id="deurja">\*deurja- "precious, valuable, expensive" (adj)</a>
N: \
-- ON _dýrr_ \
-- Far. _dýrur_ \
W: \
-- OE _dīere_, _dēore_, E _dear_ \
-- OFri. _diure_ \
-- OS _diuri_ \
-- Du. _duur_ \
-- OHG _tiuri_, G _teuer_ \
REF: [EDPG]\: 94

### <a id="deuza">\*deuza- [n.] "(wild) animal" (n)</a>
E: \
-- Go. _dius_ \
N: \
-- ON _dýr_ \
-- Far. _djór_ \
-- Elfd. _diuor_ \
W: \
-- OE _dēor_, E _deer_ \
-- OFri. _diār_, _diēr_ \
-- OS _dior_ \
-- ODu. _dier_, Du. _dier_ \
-- OHG _tior_, G _Tier_ \
REF: [EDPG]\: 94

### <a id="dīan">\*dīan- [st.] "to suck" (v)</a>
N: \
-- OSw. _dīa_ \
W: \
-- MHG _tīen_ [wk.] "to suck, suckle" \
REF: [EDPG]\: 95

### <a id="dī̆gan">\*dī̆gan- [st.] "to knead" (v)</a>
E: \
-- Go. _digan_ \
REF: [EDPG]\: 95

### <a id="digra">\*digra- "big" (adj)</a>
N: \
-- ON _digr_ "big, stout; thick; deep" \
-- Far. _digur_ "thick, heavy, stout; rich (of milk)" \
-- Elfd. _diger_ "thick, coarse" \
W: \
-- MDu. _deger_ "completely" (adv) \
-- MHG _tigere_, _tiger_ "id." (adv) \
REF: [EDPG]\: 95

### <a id="dīka">\*dīka- [n.] "dam; ditch" (n)</a>
W: \
-- OE _dīc_ [m.] "dike, ditch", E _dike_, _ditch_ \
-- OFri. _dīk_ [m.] "dike; road" \
-- OS _dīk_ [m.] "pond" \
-- Du. _dijk_ [c.] "dam" \
-- MHG _tīch_ [m.] "pond, swamp, channel", G _Teich_ [m.] "pond, pool" \
REF: [EDPG]\: 95

### <a id="dīkan">\*dīkan- [st.] "to try(?)" (v)</a>
W: \
-- MLG _diken_ [wk.] "to atone" \
-- MDu. _diken_ [st.?] "to try(?)" \
-- MHG _tichen_ [st./wk.] "to get started; to try; to deal with; to undergo; to seep" \
REF: [EDPG]\: 95

### <a id="dimban">\*dimban- ~ \*dimpan- [st.] "to fog" (v)</a>
N: \
-- Nw. _demba_ "to smoke" (with unclear _e_) \
-- Elfd. _dimba_ "id." \
W: \
-- MHG _dimpfen_ "to smoke, steam" \
REF: [EDPG]\: 96

### <a id="dimma">\*dimma- "dark" (adj)</a>
N: \
-- ON _dimmr_ \
W: \
-- OE _dimm_, E _dim_ \
-- OFri. _dim_ \
REF: [EDPG]\: 96

### <a id="dingʷan">\*dingʷan- [st.] "to beat" (v)</a>
N: \
-- OSw. _diunga_ \
W: \
-- ME _dingen_ \
-- MHG _tingelen_ [wk.] "to knock, hammer" \
REF: [EDPG]\: 96

### <a id="dinkʷa">\*dinkʷa- "dark" (adj)</a>
N: \
-- ON _døkkr_ / _dǫkkr_ \
-- Icel. _døkkur_ \
-- Far. _døkkur_ "dull; dim, dark" \
-- Nw. _dokk_, _døkk_ \
-- OSw. _dionker_, _diunker_ \
W: \
-- OFri. _diunk_ "dark" \
REF: [EDPG]\: 96

### <a id="dīsi">\*dīsi- [f.] "lady, fairy, goddess" (n)</a>
N: \
-- ON _dís_ "woman, girl; fairy, nymph; goddess" \
-- Far. _dís_ "goddess, nymph" \
-- Nw. _dis_ "goddess" \
REF: [EDPG]\: 96

### <a id="dōbi">\*dōbi- "suited, fit" (adj)</a>
E: \
-- Go. _ga-dobs_ \
W: \
-- OE _ge-dēfe_ "fit; mild" \
REF: [EDPG]\: 97

### <a id="dōbnan">\*dōbnan- [wk.] "to become dull, numb" (v)</a>
E: \
-- Go. _af-dobnan_ "to become numb, hold one's peace" \
REF: [EDPG]\: 97

### <a id="dōgera">\*dōgera- ~ \*dōgena- [n.] "a full day, twenty-four hours" (n)</a>
N: \
-- ON _dœgr_, _dœgn_ \
-- Far. -_dœgur_ (cf. _javn-dœgur_ "equinox") \
-- Nw. _døger_, _døgn_ \
-- Elfd. _dyōngen_ \
-- Da. _døgn_ \
W: \
-- OE _dōgor_, _dōger_ \
REF: [EDPG]\: 97

### <a id="dōjan">\*dōjan- [wk.] "to exhaust" (v)</a>
E: \
-- Go. _af-dauiþs_ ptc. "exhausted" \
REF: [EDPG]\: 97

### <a id="dōka">\*dōka- [n.] "canvas, cloth" (n)</a>
W: \
-- OFri. _dōk_, _dēk_ [m.] \
-- MDu. _doec_ [m./n.], Du. _doek_ [c./n.] \
-- OHG _tuoh_ [m./n.], G _Tuch_ \
REF: [EDPG]\: 97

### <a id="dōljō">\*dōljō- [f.] "depression" (n)</a>
N: \
-- ON _dœl_ "small valley" \
W: \
-- MDu. _doele_ "ditch" \
-- OHG _tuolla_ "pit, hollow" \
REF: [EDPG]\: 98

### <a id="dōma">\*dōma- [m.] "verdict, evaluation" (n)</a>
E: \
-- Go. _doms_ "insight, esteem" \
N: \
-- ON _dómr_ "opinion, judgment, sentence" \
W: \
-- OE _dōm_ "id.", E _doom_ \
-- OFri. _dōm_ "id." \
-- OS _dōm_ "id." \
-- Du. _doem_ "doom" \
-- OHG _tuom_ [m./n.] "judgement, power, situation" \
REF: [EDPG]\: 98

### <a id="dōn">\*dōn- [irr.] "to do" (v)</a>
W: \
-- OE _dōn_ [irr.], E _to do_ \
-- OFri. _dwā_, _dwān_ [irr.], WFri. _dwaan_ [irr.] \
-- OS _dōn_ [irr.] \
-- Du. _doen_ [irr.] \
-- OHG _tuon_ [irr.], G _tun_ [irr.] \
REF: [EDPG]\: 98

### <a id="draban">\*draban- [st.] "to hew" (v)</a>
E: \
-- Go. _ga-draban_ "to chip, hew out" \
REF: [EDPG]\: 98

### <a id="drabiz">\*drabiz- [n.] "dregs" (n)</a>
N: \
-- ON _draf_ "draff, husks" \
-- Far. _drav_ "draff; thin beer; rain" \
W: \
-- ODu. _draf_ "draff", Du. _draf_ [c./n.] "id." \
-- OHG _trebir_ [n.pl.] "id.", G _Treber_ [n.pl.] "id." \
REF: [EDPG]\: 98

### <a id="dragan">\*dragan- [st.] "to draw, pull, carry" (v)</a>
N: \
-- ON _draga_ \
-- Far. _draga_ [wk.] \
-- Elfd. _drågå_ \
W: \
-- OE _dragan_, E _to draw_ \
-- OFri. _drega_ \
-- OS _dragan_ "to carry, bring" \
-- Du. _dragen_ "id." \
-- OHG _tragan_ "id.", G _tragen_ [wk.] "id." \
REF: [EDPG]\: 99

### <a id="dragjō">\*dragjō- [f.] "dregs" (n)</a>
N: \
-- ON _dregg_ "dregs, lees, yeast" \
-- Far. _drøgg_ "scum, dregs, draff, sediment" \
REF: [EDPG]\: 99

### <a id="drahtu">\*drahtu- [m.] "pull" (n)</a>
N: \
-- ON _dráttr_ "pull; hesitation" \
-- Far. _dráttur_ "pulling" \
W: \
-- ME _draht_ "id.", E _draught_ \
-- MDu. _dracht_ [f.] "carrying; dress", Du. _dracht_ [c.] "id." \
-- MHG _tracht_ [f.] "id.", G _Tracht_ [f.] "id." \
REF: [EDPG]\: 99

### <a id="draibō">\*draibō- [f.] "driving, drift" (n)</a>
N: \
-- ON _dreif_ "scattering" \
W: \
-- OE _drāf_ "driving; flock, herd", E _drove_ \
REF: [EDPG]\: 99

### <a id="dranga">\*dranga- [m.] "post" (n)</a>
N: \
-- ON _drangr_ "(detached) pillar of rock" \
-- Far. _drangi_, -_ur_ "high projecting rock (in the sea)" \
-- Nw. _drange_ "id." \
REF: [EDPG]\: 99

### <a id="drankjan">\*drankjan- [wk.] "to let drink; to drench" (v)</a>
E: \
-- Go. _dragkjan_ \
N: \
-- ON _drekkja_ \
-- Elfd. _drettja_ "to drench; to ret" \
W: \
-- OE _drencan_ "to drench, make drunk", E to drench \
-- OFri. _drenza_, _drinza_ "to drown" \
-- MDu. _drenken_ "to soak; to let drink", Du. _drenken_ "id." \
-- OHG _trenken_ "id.", G _tränken_ "id." \
REF: [EDPG]\: 100

### <a id="drappōn">\*drapp/bōn- [wk.] "to hit" (v)</a>
N: \
-- Nw. _drabba_ "to give a blow" \
-- OSw. _drabba_ "to hit, meet" \
-- ODa. _drabe_ "to kill" \
REF: [EDPG]\: 100

### <a id="drauga">\*drauga- [m.] "ghost" (n)</a>
N: \
-- ON _draugr_ \
-- Far. poet. _dreygur_ "corpse; ghost, apparition" \
REF: [EDPG]\: 100

### <a id="draugja">\*draugja- ~ \*drūgja- ~ \*drukknu- "dry" (adj)</a>
W: \
-- OE _drȳge_, E _dry_ \
-- OFri. _drūch_, WFri. _droech_, EFri. _druuch_, NFri. _druuge_ \
-- OS _drukno_, _drokno_, MLG _drȫge_ "dry, infertile" \
-- MDu. _droeghe_ "id.", Du. _droog_ "id." \
-- OHG _trockan_, _truckan_ "id.", G _trocken_ "id." \
REF: [EDPG]\: 100

### <a id="drauma">\*drauma- [m.] "dream" (n)</a>
N: \
-- ON _draumr_ \
-- Far. _dreymur_ \
W: \
-- OE _drēam_, E _dream_ \
-- OFri. _drām_ \
-- OS _drōm_ \
-- Du. _droom_ [c.] \
-- OHG _troum_, G _Traum_ \
REF: [EDPG]\: 100

### <a id="draupjan">\*draupjan- [wk.] "to make drip" (v)</a>
N: \
-- ON _dreypa_ \
W: \
-- OE _drīepan_ \
-- OHG _troufen_, MHG _troufen_, _trōufen_ \
REF: [EDPG]\: 101

### <a id="drauzjan">\*drauzjan- [wk.] "to make fall" (v)</a>
W: \
-- MHG _trōren_ "to drip, ooze, shed" \
REF: [EDPG]\: 101

### <a id="drēkō">\*drēkō- [f.] "streak" (n)</a>
N: \
-- ON _drák_ \
-- Nw. _dråk_, pl. _dræker_ \
REF: [EDPG]\: 101

### <a id="drenan">\*drenan- [m.] "drone" (n)</a>
W: \
-- OS _dreno_ \
-- Du. Limb. _drene_ [c.] \
-- OHG _treno_, MHG _trene_, _tren_, G Swi. Jaun _trɛnə_ \
REF: [EDPG]\: 101

### <a id="drepan">\*drepan- [st.] "to hit" (v)</a>
N: \
-- ON _drepa_ "to stike, beat, knock; to kill, slay" \
W: \
-- OE _drepan_ "to strike" \
-- MLG _drepen_ \
-- MDu. _drepen_ [st./wk.] "to hit, strike" \
-- OHG _treffan_, G _treffen_ "to hit, touch" \
REF: [EDPG]\: 101

### <a id="dreuga">\*dreuga- "enduring" (adj)</a>
N: \
-- ON _drjúgr_ "substantial, lasting, ample" \
-- Far. _drúgvur_ "long-lasting, adequate" \
-- Elfd. _driuog_ "laborious; disparaging" \
W: \
-- WFri. _dreech_ "strong, enduring, long-lasting", NFri. F-A _drech_ "enduring; stocky, fat" \
REF: [EDPG]\: 102

### <a id="dreugan-1">\*dreugan- [st.] "to mislead" (v)</a>
W: \
-- MHG _triegen_ "to deceive, betray" \
REF: [EDPG]\: 102

### <a id="dreugan-2">\*dreugan- [st.] "to do a duty(?)" (v)</a>
E: \
-- Go. _driugan_ \
W: \
-- OE _drēogan_ "to do, work; to endure, suffer", ME _drīen_ "to perform; to experience; to put up with, endure" \
REF: [EDPG]\: 102

### <a id="dreupan">\*dreupan- ~ \*drūpan- [st.] "to droop; to drip" (v)</a>
N: \
-- ON _drjúpa_ "to drip, drop; to droop with the head" \
W: \
-- OE _drēopan_ "to drip", E _to drip_ \
-- OFri. _driāpa_ "id." \
-- OS _driopan_ "id." \
-- Du. _druipen_ "to drip; to ooze" \
-- OHG _triufan_ "id.", G _triefen_ "id." \
REF: [EDPG]\: 102

### <a id="dreusan">\*dreusan- [wk.] "to fall" (v)</a>
E: \
-- Go. _driusan_ [st.] "to fall down" \
N: \
-- Nw. _drjosa_ [st.] "id." \
W: \
-- OE _drēosan_ [st.] "to rush, fall, perish" \
-- OS _driosan_ [st.] "to fall" \
REF: [EDPG]\: 102

### <a id="drīban">\*drīban- [st.] "to drive" (v)</a>
E: \
-- Go. _dreiban_ \
N: \
-- ON _drífa_ "to drift, drive like spray or snow; to crowd, throng, rush; to perform" \
-- Far. _dríva_ "to chase; to hit, strike; to press, force; to be carried or blow away" \
-- Elfd. _draiva_ "to drive; to fell; to pour, spray, flush" \
W: \
-- OE _drīfan_ "to drive, force, pursue", E to drive \
-- OFri. _drīva_ "id." \
-- OS _drīƀan_ "id." \
-- Du. _drijven_ "to drive; to float" \
-- OHG _trīban_, G _treiben_ "to drive; to operate" \
REF: [EDPG]\: 103

### <a id="dribbōn">\*drib(b)ōn- [wk.] "to push little by little" (v)</a>
N: \
-- Nw. _dribba_ "to shove, push" \
W: \
-- E obs. _to drib_ "to lead one little by little into something", _to dribble_ "to keep (the ball) moving" \
-- Du. _dribbelen_ "to walk quickly (with small steps)" \
-- OHG _tribōn_ "to drive, push" \
REF: [EDPG]\: 103

### <a id="drifti">\*drifti- [f.] "driving" (n)</a>
N: \
-- ON _drift_ "snowdrift" \
W: \
-- ME _drift_ "driving of cattle to pasture; drove; running; falling of rain or snow, snowdrift" \
-- MDu. _drift_ "fervor, urge; herd; district", EDu. _drift_ [c.] "urge, instinct" \
-- MHG _trift_ "pasture, herd", G _Trift_ "id." \
REF: [EDPG]\: 103

### <a id="drinkan">\*drinkan- [st.] "to drink" (v)</a>
E: \
-- Go. _drigkan_ \
N: \
-- ON _drekka_ \
-- Far. _drekka_ \
-- Elfd. _drikka_ \
W: \
-- OE _drincan_, E _to drink_ \
-- OFri. _drinka_ \
-- OS _drinkan_ \
-- Du. _drinken_ \
-- OHG _trinkan_, G _trinken_ \
REF: [EDPG]\: 103

### <a id="drita">\*drita- [n.] "shit, dirt" (n)</a>
N: \
-- ON _drit_ \
W: \
-- OE _drit_ "dirt", E _dirt_ \
REF: [EDPG]\: 103

### <a id="drītan">\*drītan- [st.] "to shit" (v)</a>
N: \
-- ON _dríta_ "to shit (esp. of birds)" \
-- Far. _dríta_ "id." \
-- Elfd. _draita_ "id." \
W: \
-- OE _ge-drītan_ "id." \
-- MDu. _driten_ [st./wk.] "id." \
REF: [EDPG]\: 104

### <a id="drōbu">\*drōbu- "turbid; troubled" (adj)</a>
W: \
-- OE _drōf_ "turbid; troubled" \
-- OS _drōƀi_ "cloudy, sad" \
-- Du. _droef_ \
-- OHG _truobi_ "turbid, dark, confused", MHG _trüebe_ "dim, gloomy" \
REF: [EDPG]\: 104

### <a id="druga">\*druga- [n.] "deceit" (n)</a>
W: \
-- NFri. _drōch_ "dream" \
-- OS _gi-drog_ "illusion" \
-- MDu. _ge-droch_ "deceit; apparition, ghost" \
-- OHG _gi-trog_, MHG _ge-troc_, _troc_ [m.] "id." \
REF: [EDPG]\: 104

### <a id="druggōn">\*druggōn- [wk.] "to pull, drag" (v)</a>
N: \
-- Nw. _drugga_ "to walk bowed down" \
W: \
-- ME _druggen_, _droggen_ "to labor, toil", E dial. _to drug_ "to pull forcibly" \
REF: [EDPG]\: 104

### <a id="druhti">\*druhti- [f.] "host, retinue" (n)</a>
E: \
-- Go. _drauhti-witoþ_ "military service" \
N: \
-- ON _drótt_ "household, people; host of the king's men, bodyguard of a king" \
W: \
-- OE _dryht_ "people, multitude, army" \
-- OFri. _drecht_ [f.?] "wedding entourage" \
-- OS _druht-folk_ "army" \
-- MHG _truht_ "troop, squad, platoon", G _truht_ "troop, squad, platoon" \
REF: [EDPG]\: 104

### <a id="drunki">\*drunki- [m.] "drink" (n)</a>
N: \
-- ON _drykkr_ "drink, drinking; draught" \
-- Far. _drykkur_ \
-- Elfd. _drykk_ \
W: \
-- OE _drync_ "drink; draught", E _drink_ \
-- MDu. _dronc_ "drinking", Du. _dronk_ [c.] "id." \
-- OHG _trunk_ "id.", G _Trunk_ "id." \
REF: [EDPG]\: 105

### <a id="druppōn-1">\*drupp/bōn- [wk.] "to hit" (v)</a>
N: \
-- Nw. _drubba_ "to give a blow" \
W: \
-- E dial. _drub_ "to thresh, beat, bang" \
REF: [EDPG]\: 105

### <a id="druppōn-2">\*drupp/bōn- [wk.] "to drip, droop; to drop" (v)</a>
N: \
-- Nw. _drubba_ "to walk with a stoop, to fall over" \
W: \
-- MDu. _druppen_ "to sag, drip", Du. dial. _drubben_ "to hang one's head, be downcast" \
REF: [EDPG]\: 105

### <a id="drūsēn">\*drūsēn- [wk.] "to fall asleep" (v)</a>
W: \
-- OE _drūsian_ "to become slow", E _to drowse_ \
REF: [EDPG]\: 105

### <a id="duba">\*duba- "dull" (adj)</a>
W: \
-- Du. _dof_ "matt" \
-- MHG _top_ "crazy" \
REF: [EDPG]\: 105

### <a id="dūban">\*dūban- [st.] "to dive" (v)</a>
N: \
-- ON _dúfa_ [wk.] \
-- Nw. _duva_ [wk.] "to squat" \
-- OSw. _dūva_ [wk.] "to squat, bob" \
W: \
-- OE _dūfan_ "to dive" \
-- MLG _be-dūven_ "to become overwhelmed, become covered" \
-- MDu. _be-duven_ "to dip" \
REF: [EDPG]\: 105

### <a id="dubila">\*dubila- [m.] "peg, pin, dowel" (n)</a>
W: \
-- MLG _dȫvel_ \
-- Du. _deuvel_ [c.] \
-- MHG _tübel_, G _Dübel_ \
REF: [EDPG]\: 106

### <a id="dūbōn">\*dūbōn- [f.] "dove" (n)</a>
E: \
-- Go. _hraiwa-dubo_ \
N: \
-- ON _dúfa_ \
-- Far. _dúgva_ \
-- Elfd. _dauva_ \
W: \
-- E _dove_ \
-- OS _dūƀa_ \
-- Du. _duif_ [c.] \
-- OHG _tūba_, G _Taube_ \
REF: [EDPG]\: 106

### <a id="dudiran">\*dud(i)ran- [m.] "egg yolk" (n)</a>
W: \
-- WFri. _djerre_, _djirre_ [c.] \
-- OS _dodro_, MLG _dod(d)er_, _dȫder_, _dōdder_ [n.(?)] \
-- MDu. _doder_, _dodre_ [m./f.], Du. _dooier_ [c.] \
-- OHG _totoro_, G _Dotter_ \
REF: [EDPG]\: 106

### <a id="dudran">\*dudra/ōn- [m./f.] "dodder(?), camelina(?)" (n)</a>
N: \
-- Icel. _hár-doðra_, _akur-doðra_ \
-- Sw. _dådra_ [c.] \
W: \
-- ME _doder_ "flax dodder", E _dodder_ \
-- MLG _doder_, _dodder_ [m.] "id." \
-- Du. _dodder_ [m.] "id." \
-- MHG _toter_ [m.] "wild rocket, field rocket" \
REF: [EDPG]\: 106

### <a id="dugan">\*dugan- [pp.] "to be fit, avail" (v)</a>
E: \
-- Go. _daug_ 3sg.pres. \
N: \
-- ON _duga_ [wk.] \
W: \
-- OE _dugan_ \
-- OFri. _duga_ \
-- OS _dugan_ \
-- Du. _deugen_ [wk.] \
-- OHG _tugan_, G _taugen_ [wk.] \
REF: [EDPG]\: 106

### <a id="dugunþi">\*dugunþi- [f.] "valor" (n)</a>
W: \
-- OE _duguð_ "manhood; glory, advantage; multitude" \
-- OFri. _duged_ "virtue" \
-- ODu. _dugeth_ "id.", Du. _deugd_ [c.] "id." \
-- OHG _tugund_, -_ind_ "id.", G _Tugend_ "id." \
REF: [EDPG]\: 107

### <a id="duhter">\*duhter- [f.] "daughter" (n)</a>
E: \
-- Go. _dauhtar_ \
N: \
-- ON _dóttir_ \
-- Far. _dóttir_ \
-- Elfd. _duotter_ \
W: \
-- OE _dohtor_, _dohter_, E _daughter_ \
-- OFri. _dochter_ \
-- OS _dohtar_ \
-- Du. _dochter_ [c.] \
-- OHG _tohter_, G _Tochter_ \
REF: [EDPG]\: 107

### <a id="dūjan">\*dūjan- [wk.] "to tremble" (v)</a>
N: \
-- ON _dýja_ \
REF: [EDPG]\: 107

### <a id="dūkan">\*dūkan- [st.] "to duck" (v)</a>
W: \
-- ME _douken_ [wk.] "to dive, plunge, immerse", E _to duck_ \
-- MDu. _duken_ "to dive; to duck, cower", Du. _duiken_ "to dive" \
-- OHG _in-tūhhan_ "to immerse", G _tauchen_ [wk.] "to dive, plunge" \
REF: [EDPG]\: 107

### <a id="dukkōn">\*dukkōn- [wk.] "to duck" (v)</a>
W: \
-- EDu. _docken_ "to dive, tumble" \
-- MHG _tocken_ "to immerse" \
REF: [EDPG]\: 107

### <a id="dula">\*dula- "foolish, crazy" (adj)</a>
W: \
-- MHG _tol_ "foolish, nonsensical" \
REF: [EDPG]\: 108

### <a id="dulga">\*dulga- "debt" (adj)</a>
E: \
-- Go. _dulgs_ [m.] \
REF: [EDPG]\: 108

### <a id="dulō-1">\*dulō- [f.] "cluster or bunch of grapes" (n)</a>
W: \
-- OHG _tola_ \
REF: [EDPG]\: 108

### <a id="dulō-2">\*dulō- [f.] "ditch" (n)</a>
W: \
-- WFri. _dôle_ [c.] "puddle, pool" \
-- OHG _tola_, MHG _tole_ \
REF: [EDPG]\: 108

### <a id="dulþi">\*dulþi- [f.] "festival, celebration" (n)</a>
E: \
-- Go. _dulþs_ \
W: \
-- OHG _tuld_, MHG _tult_, _dult_ \
REF: [EDPG]\: 108

### <a id="dumba">\*dumba- "dumb" (adj)</a>
E: \
-- Go. _dumbs_ "dumb, mute" \
N: \
-- ON _dumbr_ "id." \
-- Far. _dumbur_ "id." \
-- Elfd. _dumm_ \
W: \
-- OE _dumb_, E _dumb_ \
-- OFri. _dumb_ \
-- OS _dumb_ \
-- Du. _dom_ \
-- OHG _tumb_, G _dumm_ \
REF: [EDPG]\: 108

### <a id="dumbōn">\*dumbōn- [f.] "fog, haze" (n)</a>
N: \
-- ON _dumba_ "dust, cloud of dust, mist" \
-- Elfd. _dumba_ "fog, haze" \
REF: [EDPG]\: 108

### <a id="dumpōn">\*dumpōn- [wk.] "to suffocate(?)" (v)</a>
W: \
-- MDu. _ver-dompen_ "to suffocate" \
REF: [EDPG]\: 109

### <a id="dūna">\*dūna- [m.] "down (of a bird)" (n)</a>
N: \
-- ON _dúnn_ \
-- Far. _dún_ [n.] \
-- OSw. _dūn_ [n.] \
-- Elfd. _daungen_ \
REF: [EDPG]\: 109

### <a id="dunga">\*dunga- [m.] "dung" (n)</a>
W: \
-- OE _dung_, E _dung_ \
-- OFri. _dung_ \
-- OS _dung_ \
-- G _Dung_ \
REF: [EDPG]\: 109

### <a id="dungōn">\*dungōn- [wk.] "to cover(?), pour over(?)" (v)</a>
N: \
-- Far. _dunga_ "to pile up; to snow heavily" \
-- Sw. _dunga_ "to get soaked", obs. "to rain, pour down" \
W: \
-- MLG _dungen_ "to irrigate" \
REF: [EDPG]\: 109

### <a id="dunkla">\*dunkla- ~ \*dunkra- "dark" (adj)</a>
W: \
-- OS _dunkar_ \
-- Du. _danker_ \
-- OHG _tunkal_, G _dunkel_ \
REF: [EDPG]\: 109

### <a id="dunsta">\*dunsta- [n.(?)] "powder, dust(?)" (n)</a>
W: \
-- OE _dūst_ [n.], E _dust_ \
-- WFri. _dūst_ [n./c.] "dandruff; chaff" \
-- MLG _dust_ [m.] "chaff, husk, dust" \
-- MDu. _donst_, _dunst_, _dust_ [m.?] "fluff, pollen" \
REF: [EDPG]\: 109

### <a id="duppōn">\*dupp/bōn- [wk.] "to dip" (v)</a>
N: \
-- Nw. _duppa_, _dubba_ "to dip; to duck; to nod" \
-- Elfd. _duppa_ "id." \
W: \
-- MDu. _doppen_ "to dip" \
REF: [EDPG]\: 109

### <a id="dura">\*dura- [n.] "gate, (single) door" (n)</a>
E: \
-- Go. _daur_ \
W: \
-- OE _dor_, E _door_ \
-- OS _dor_, _dur_ \
-- ODu. _duri_ [f.] "door", Du. _deur_ "door" \
-- OHG _tor_ "id.", G _Tor_ "id." \
REF: [EDPG]\: 110

### <a id="durgō">\*durgō- [f.] "fishing-line" (n)</a>
N: \
-- ON _dorg_ "trailing-line" \
-- Far. _dorg_, pl. _dergur_ "short fishing-line" \
REF: [EDPG]\: 110

### <a id="durī">\*durī- [f.pl.] "door" (n)</a>
N: \
-- ON _dyrr_ [f./n.pl.] \
-- Far. _dyr_ \
-- Elfd. _dōrer_ \
W: \
-- OS _duri_ \
-- MDu. _dore_ [f.], Du. _deur_ [c.] \
-- OHG _turi_, G _Tür_ [f.] \
REF: [EDPG]\: 110

### <a id="durrōn">\*durrōn- [f.] "stick" (n)</a>
N: \
-- Icel. _dorra_ "piece of wood, stick" \
REF: [EDPG]\: 110

### <a id="dursti">\*dursti- [f.] "courage" (n)</a>
W: \
-- OE _dyrst_, _ge-dyrst_ "tribulation" \
-- OHG _gi-turst_ "courage, audacity", MHG _turst_, _ge-turst_ "id." \
REF: [EDPG]\: 110

### <a id="durzan">\*durzan- [pp.] "to dare, be brave" (v)</a>
E: \
-- Go. _ga-daursan_ \
W: \
-- OE _durran_ [st.] \
-- OS _gi-durran_ \
-- ODu. _durran_, Du. _dorst_ [pret.] \
-- OHG _turran_, _gi-turran_, MHG _turren_, _ge-turren_ \
REF: [EDPG]\: 111

### <a id="dusēn">\*dusēn- [wk.] "to slumber" (v)</a>
W: \
-- E _to doze_ \
-- MHG _dosen_ "to keep oneself quiet, slumber" \
REF: [EDPG]\: 111

### <a id="duska-1">\*duska- [m.] "drizzle(?)" (n)</a>
N: \
-- Nw. _dusk_ [n.] "drizzling rain" \
-- Sw. dial. _dusk_ "light rain" \
REF: [EDPG]\: 111

### <a id="duska-2">\*duska- "dark" (adj)</a>
W: \
-- OE _dox_, _dux_ "sallow", E _dusk_ "twilight" \
REF: [EDPG]\: 111

### <a id="duwēn">\*du(w)ēn- [wk.] "to be misty(?), windy(?)" (v)</a>
N: \
-- Nw. dial. _duva_ "to blow softly" and Elfd. _duvå_ "to drizzle" \
REF: [EDPG]\: 111

### <a id="dwaljan">\*dwaljan- [wk.] "to delay, hinder" (v)</a>
N: \
-- ON _dvelja_ "to delay, tarry" \
-- Far. _dvelja_ "to delay; to hesitate, wait" \
W: \
-- OE _dwelian_ "to lead astray", E _to dwell_ \
-- OS _bi-dwellian_ \
-- MDu. _dwellen_ "to lead astray" \
-- OHG _twaljen_ "to delay; to wait, stay", MHG _twellen_, _twelen_, _dwellen_ "id." \
REF: [EDPG]\: 111

### <a id="dwelan">\*dwelan- [st.] "to err" (v)</a>
W: \
-- OE _ge-dwolen_ ptc. "wrong" \
-- EFri. _dwylje_ [wk.] "to rave; to roam" \
-- OS _for-dwelan_ "to neglect, fail" \
-- ODu. _dwelon_ "to fail, err" \
-- OHG _gi-twelan_ "to be stunned, tarry, fall asleep" \
REF: [EDPG]\: 112

### <a id="dwerga">\*dwerga- [m.] "dwarf" (n)</a>
N: \
-- ON _dvergr_ "dwarf; short pillars which support the beams and rafters in a house" \
-- Far. _dvørgur_ \
-- OSw. _dværgher_ \
-- Elfd. _dyörg_ "spider; deathwatch beetle; wrinkle, crease" \
W: \
-- OE _dweorg_, _dweorh_, E _dwarf_ \
-- MDu. _dwerch_ "monster, dwarf, giant", Du. _dwerg_ [c.] \
-- OHG _twerc_, G _Zwerg_ \
REF: [EDPG]\: 112

### <a id="dwergan">\*dwergan- [st.] "to squeeze, press" (v)</a>
W: \
-- MHG _zwergen_ \
REF: [EDPG]\: 112

### <a id="dwēsa">\*dwēsa- "foolish" (adj)</a>
W: \
-- OE _dwæs_ \
-- OFri. _dwēs_ \
-- MLG _dwās_ \
-- MDu. _dwaes_, Du. _dwaas_ \
-- MHG _twās_ \
REF: [EDPG]\: 112

### <a id="dwīnan">\*dwīnan- [st.] "to diminish" (v)</a>
N: \
-- ON _dvína_ [wk.] \
-- Far. _dvína_ [wk.] "to yield, give way; to diminish" \
W: \
-- OE _dwīnan_ "to dwindle", ME _dwinen_ "id." \
-- MDu. _dwinen_ "to disappear, dwindle, pine away", Du. _ver-dwijnen_ "to disappear" \
REF: [EDPG]\: 112

## E

### <a id="ēbanþ">\*ēbanþ- ~ \*ēbund- [m.] "evening" (n)</a>
N: \
-- ON _aftann_, _eftann_ \
-- Far. _aftan_ \
-- Elfd. _otn_ "early supper" (< ON \*_ǫftunn_?) \
W: \
-- OE _ǣfen_, E _eve_ \
-- OFri. _ēwend_, _iōwen(d)_, _iouwend_, _iōn(d)_, _ioun(d)_, WFri. _jûn_ [c.], EFri. _āiwend_ \
-- OS _āƀand_ \
-- ODu. _āvont_, Du. _avond_ [c.] \
-- OHG _āband_, _ābund_ [c.], G _Abend_ \
REF: [EDPG]\: 113

### <a id="ebna">\*ebna- "even, level" (adj)</a>
E: \
-- Go. _ibns_ "even, level, flat" \
N: \
-- ON _jafn_ "even, equal" \
-- Far. _javnur_ "id." \
-- OSw. _iamn_, _iæmn_ "id." \
-- Elfd. _iemmen_ "id." \
W: \
-- OE _efen_ "even, equal", E _even_ \
-- Du. _effen_, _even_ "level, even" \
-- OHG _eban_ "even, equal, straight", G _eben_ "level" \
REF: [EDPG]\: 113

### <a id="ebura">\*ebura- [m.] "boar" (n)</a>
N: \
-- ON _jǫfurr_ "king" \
W: \
-- OE _eofor_ \
-- MDu. _ever_, Du. _ever-zwijn_ \
-- OHG _ebur_, G _Eber_ \
REF: [EDPG]\: 114

### <a id="edara">\*edara- [m.] "edge; fence" (n)</a>
N: \
-- ON _jaðarr_ "edge" \
-- Far. _jaðari_ "edge, corner" \
-- Nw. _jar(e)_ "edge, side" \
W: \
-- OE _eodor_ "hedge, fence, enclosure, dwelling; limit, end, region" \
-- OS _edar_ "pale, picket, bar" \
-- OHG _etar_ [m./n.] "edge, fence", MHG _eter_ "id." \
REF: [EDPG]\: 114

### <a id="edi">\*edi "again" (conj)</a>
E: \
-- Go. _iþ_ "but, however" \
REF: [EDPG]\: 114

### <a id="edis">\*edis- [f.] "(noble, divine) lady" (n)</a>
W: \
-- OE _ides_ "female, lady" \
-- OS _idis_ "wife, woman" \
-- OHG _itis_ "(divine) woman" \
REF: [EDPG]\: 114

### <a id="ēdra">\*ēdra- "quick" (adj)</a>
W: \
-- OE _ǣdre_ "immediately" (adv) \
-- OFri. _ē̆der_, _edre_, _idder(e)_ "soon, quickly" (adv) \
-- OS _ādro_ "early" (adv) \
-- OHG _ātar_ "quick" \
REF: [EDPG]\: 115

### <a id="egila">\*egila- [m.] "hedgehog, sea urchin" (n)</a>
N: \
-- ON _igull_ "urchin" \
-- Icel. _ígul-ker_ [n.] "sea urchin" \
-- ODa. _jæwæl_ "hedgehog" \
W: \
-- OE _igil_, _īl_ "hedgehog, porcupine; sea urchin" \
-- MDu. _egel_ "hedgehog", Du. _egel_ [c.] "id." \
-- OHG _ī̆gil_ "id.", G _Igel_, obs. _Eigel_ "id." \
REF: [EDPG]\: 115

### <a id="ēgja">\*ēgja- [m.] "sea" (n)</a>
N: \
-- ON _ǣgir_ \
-- Far. poet. _ægir_\* \
REF: [EDPG]\: 115

### <a id="ehwa">\*ehwa- [m.] "horse" (n)</a>
E: \
-- Go. _aiƕa-tundi_ [f.] "thornbush" \
N: \
-- ON poet. _jór_ "stallion" \
W: \
-- OE _eoh_ "war-horse" \
-- OS _ehu-skalk_ "groom" \
REF: [EDPG]\: 115

### <a id="eka">\*ek(a) "I" (pron)</a>
E: \
-- Go. _ik_, dat. _mis_, acc. _mik_ \
N: \
-- ON _ek_, dat. _mér_, acc. _mik_ \
-- Far. _eg_, dat. _mær_, acc. _meg_ \
-- OSw. _iak_, dat. _mær_, acc. _mik_, _mek_ \
-- Elfd. _ig_, obl. _mig_ \
W: \
-- OE _ic_, obl. _mē_, acc. _mec_, E _I_, obl. _me_ \
-- OFri. _ik_, obl. _mī_ \
-- OS _ik_, obl. _mī_ \
-- Du. _ik_, obl. _mij_ \
-- OHG _ih_, dat _mir_, acc. _mih_, G _ich_, dat _mir_, acc. _mich_ \
REF: [EDPG]\: 116

### <a id="ēla">\*ēla- [m.] "eel" (n)</a>
N: \
-- ON _áll_ \
-- Far. _álur_, _álli_, _állur_ \
-- Elfd. _ål_ \
W: \
-- OE _ǣl_, E _eel_ \
-- OFri. _ēl_ \
-- MDu. _ael_, Du. _aal_ [c.] \
-- OHG _āl_, G _Aal_ \
REF: [EDPG]\: 116

### <a id="elgra">\*elgra- "blunt (of teeth)" (adj)</a>
W: \
-- G obs. _elger_ \
REF: [EDPG]\: 116

### <a id="elhan">\*elha(n)- [m.] "elk" (n)</a>
W: \
-- OE _eolh_, _ē̆ola_, ME _elk_, E _elk_ \
-- LG _elk_ "id." \
-- EDu. _elgh_ "bison" \
-- OHG _elaho_, _elho_ "elk" \
REF: [EDPG]\: 116

### <a id="elhja">\*elhja- "evil" (adj)</a>
N: \
-- ON _illr_ "ill, evil, bad, mean" \
-- Far. _illur_ "evil, unfriendly; poor, miserable; angry" \
REF: [EDPG]\: 117

### <a id="ēlō">\*ēlō- [f.] "awl" (n)</a>
W: \
-- OE _ǣl_, _eal_, ME _ēl_, _a(u)l_ \
-- OHG _āla_, G _Ahle_ \
REF: [EDPG]\: 117

### <a id="embja">\*embja- [n.] "swarm of bees" (n)</a>
W: \
-- OE _imbe_ [m.] \
-- MDu. _imme_ [f.] "bee" \
-- OHG _impi_ [m.] "swarm of bees; bee", MHG _imbe_, _imme_ [m.] "id." \
REF: [EDPG]\: 117

### <a id="emōn-1">\*ēmōn- [f.] "erysipelas" (n)</a>
N: \
-- ON _áma_, _ámu-sótt_ \
W: \
-- OE _ōman_ [f.pl.] \
REF: [EDPG]\: 117

### <a id="emōn-2">\*ēmōn- [f.] "larva" (n)</a>
N: \
-- Nw. _åme_ "larva, caterpillar" \
-- Elfd. _oma_ "id." \
W: \
-- OE _ē̆mel_ "caterpillar; weevil" \
-- MLG _emel_, _emelte_ "corn bug; aphid" \
-- Du. _emelt_ [c.] "mosquito larva" \
REF: [EDPG]\: 117

### <a id="ēnu">\*ē̆nu- "without" (prep)</a>
E: \
-- Go. _inu_ \
N: \
-- ON _án_, _ón_ \
-- Far. _ón_ \
W: \
-- OS _āno_ \
-- OHG _ānu_, _āne_, G _ohne_ \
REF: [EDPG]\: 118

### <a id="erban">\*erba(n)- [m.] "wolverine" (n)</a>
N: \
-- Icel. _jerfur_, obs. _jarfi_ \
-- Nw. _jerv_, dial. _jarv_, _erv_ \
REF: [EDPG]\: 118

### <a id="erpa">\*erpa- "light brown" (adj)</a>
N: \
-- ON _jarpr_ "chestnut, reddish-brown" \
-- Far. _jarpur_ "light or yellowish-brown" \
W: \
-- OE _eorp_, _earp_ "dark, dusky, brown, swarthy" \
-- OHG _erpf_ "brown, dark-brown, dark" \
REF: [EDPG]\: 118

### <a id="erþō">\*erþō- [f.] "earth, soil, land" (n)</a>
E: \
-- Go. _airþa_ \
N: \
-- ON _jǫrð_ \
-- Far. _jørð_ \
-- Elfd. _juord_ \
W: \
-- OE _eorþ_, E _earth_ \
-- OFri. _erthe_ \
-- OS _ertha_ \
-- Du. _aarde_ [c.] \
-- OHG _erda_, G _Erde_ \
REF: [EDPG]\: 118

### <a id="erwan">\*erwan- [m.] "sand, soil" (n)</a>
N: \
-- ON _jǫrfi_ "sand (bank)" \
W: \
-- OHG _ero_ "earth" \
REF: [EDPG]\: 119

### <a id="erzja">\*erzja- "stray" (adj)</a>
E: \
-- Go. _airzeis_ "deluded, misled, in error" \
W: \
-- OS _irri_ "furious" \
-- OHG _irri_ "erring, ignorant", MHG _irre_ "astray" \
REF: [EDPG]\: 119

### <a id="ēsa">\*ēsa- [n.] "dead meat; bait" (n)</a>
W: \
-- OE _ǣs_ \
-- Du. _aas_ \
-- OHG _ās_, G _Aas_ \
REF: [EDPG]\: 119

### <a id="etan">\*etan- [st.] "to eat" (v)</a>
E: \
-- Go. _itan_ \
N: \
-- ON _eta_ \
-- Far. _eta_ \
-- Elfd. _jätå_ \
W: \
-- OE _etan_, E _to eat_ \
-- OS _etan_ \
-- Du. _eten_ \
-- OHG _ezzan_, G _essen_ \
REF: [EDPG]\: 119

### <a id="ēþman">\*ēþman- ~ \*ēdman- [m.] "breath" (n)</a>
W: \
-- OE _ǣþm_ "breathing, breath" \
-- OFri. _ēthma_ "breath" \
-- OS _āthum_ "breath, spirit" \
-- Du. _adem_, coll. _asem_ [c.] "breath" \
-- OHG _ātum_ "breath, breeze, spirit", MHG _ātem_, G _Atem_ "breath" \
REF: [EDPG]\: 119

### <a id="ēþrō">\*ēþrō- ~ \*ēdrō- [f.] "vein, rivulet" (n)</a>
N: \
-- ON _æðr_, gen. _æðar_ "vein" (< \*ēdīz, -jōz) \
-- Nw. _år(e)_ "rivulet; vein" \
W: \
-- OE _ǣdre_ "river; vein, artery; nerve, sinew; kidney" \
-- OFri. _eddere_, _ēdere_ "vein" \
-- MDu. _adere_ "vein, artery; sinew, fiber", Du. _ader_ [c.] "vein" \
-- OHG _ād(a)ra_ "vein, sinew, muscle", G _Ader_ "sinew", pl. "bowels" \
REF: [EDPG]\: 120

### <a id="eudra">\*eudra- ~ \*ūdra- [n.] "udder" (n)</a>
N: \
-- ON _júgr_ \
-- Far. _júgur_ \
-- Elfd. _juor_ \
W: \
-- OE _ūder_, E _udder_ \
-- OFri. _ūder_, WFri. _jaer_, NFri. Hall. _jaodər_ \
-- MLG _jeder_, _jüdder_ \
-- Du. _uier_ [c.] (dial. _jadder_ "teat" « OFri. \*_jadder_) \
-- OHG _ūtar(o)_ [m.], G _Euter_ \
REF: [EDPG]\: 120

### <a id="eup">\*eup ~ \*upp "up, upwards" (adv)</a>
E: \
-- Go. _iup_ \
N: \
-- ON _upp_ \
-- Far. _upp_ \
-- Elfd. _upp_ \
W: \
-- OE _up_, E _up_ \
-- OFri. _up_, _op_ \
-- OS _up_ \
-- Du. _op_ (adv/prep) "up; on" \
-- OHG _ūf_, G _auf_ (adv/prep) "id." \
REF: [EDPG]\: 120

### <a id="ewwadjōn">\*ewwadjōn- [f.] "tit" (n)</a>
N: \
-- ON _igða_ "small bird" \
-- Far. poet. _ígða_ "tit(?)" \
-- Nw. _igde_ "id." \
REF: [EDPG]\: 121

## F

### <a id="fader">\*fader- [m.] "father" (n)</a>
E: \
-- Go. _fadar_ \
N: \
-- ON _faðir_ \
-- Far. _faðir_ \
-- Elfd. _faðer_ \
W: \
-- OE _fæder_, E _father_ \
-- OFri. _feder_ \
-- OS _fadar_ \
-- Du. _vader_ [c.] \
-- OHG _fatar_, G _Vater_ \
REF: [EDPG]\: 121

### <a id="fadi">\*fadi- [m.] "lord" (n)</a>
E: \
-- Go. _bruþ-faþs_ "bridegroom", _hunda-faþs_, _þusundi-faþs_ "leader of hundred/thousand" \
REF: [EDPG]\: 121

### <a id="fadōjan">\*fadōjan- [wk.] "to feed" (v)</a>
W: \
-- OHG _fatōn_ \
REF: [EDPG]\: 121

### <a id="fadurwjan">\*fadurwjan- [m.] "paternal uncle, father's brother" (n)</a>
W: \
-- OE _fædera_ \
-- OFri. _federia_, _fedria_, _feria_, _fadria_ \
-- MDu. _vedder_ "father's brother; uncle; brother's son" \
-- OHG _fatureo_, _fetiro_ "father's brother", MHG _vetere_, _veter_ "father's brother, brother's son, male cousin", G _Vetter_ "male cousin" \
REF: [EDPG]\: 122

### <a id="fagjan">\*fagjan- [wk.] "to please" (v)</a>
E: \
-- Go. _fulla-fahjan_ "to satisfy" \
REF: [EDPG]\: 122

### <a id="fagra">\*fagra- "fit (good, beautiful)" (adj)</a>
E: \
-- Go. _fagrs_ "fit, good" \
N: \
-- ON _fagr_ "beautiful, handsome" \
-- Far. _fagur_ "id." \
W: \
-- OE _fæger_ "id.", E _fair_ \
-- OS _fagar_ "id." \
-- OHG _fagar_ "id.", MHG _vager_ "id." \
REF: [EDPG]\: 122

### <a id="fahiz">\*fahiz- [n.] "sheep" (n)</a>
N: \
-- ON _fær_ \
-- OSw. _fār_ \
-- Sw. _får_ \
REF: [EDPG]\: 122

### <a id="fahsa">\*fahsa- [n.] "hair of the head" (n)</a>
N: \
-- ON _fax_ "mane" \
W: \
-- OE _feax_ \
-- OFri. _fax_ \
-- OS _fahs_ \
-- OHG _fahs_, MHG _vahs_ [m./n.] \
REF: [EDPG]\: 122

### <a id="fahti">\*fahti- [m.] "fleece" (n)</a>
N: \
-- OSw. _fæt_ \
W: \
-- OE _feht_ "vellus" \
-- OFri. _fecht_ "wool, fleece" \
-- MDu. _vacht_ [m./f.], Du. _vacht_ [c.] "fleece, fur" \
REF: [EDPG]\: 123

### <a id="faigi">\*faigi- "bound to die" (adj)</a>
N: \
-- ON _feigr_ \
W: \
-- OE _fǣge_, E _fey_ \
-- OFri. _fach_, _fai_ \
-- OS _fēg_, _fēgi_ \
-- Du. _veeg_ \
-- OHG _feigi_, G _feige_ (adj) "coward", Swi. _feig_ "slow, lazy" \
REF: [EDPG]\: 123

### <a id="faiha-1">\*faiha- "hostile" (adj)</a>
W: \
-- OE _fāh_, _fāg_ "hostile, criminal", E _foe_ \
-- OFri. _fāch_ "guilty, outlawed" \
-- MDu. _vee_, _ge-vee_ \
-- OHG _gi-fēh_, MHG _ge-vēch_ \
REF: [EDPG]\: 123

### <a id="faiha-2">\*faiha- "colourful" (adj)</a>
W: \
-- OE _fāh_ "colored", ME _fou_ "particolored, variegated, spotted, streaked" \
-- OS _fēh_ \
-- OHG _fēh_, MHG _vēch_ "variegated" \
REF: [EDPG]\: 123

### <a id="faiman">\*faima(n)- [m.] "foam" (n)</a>
N: \
-- Nw. _feim(e)_ [m./n.] "thin layer of dew, dust, floating fat" \
W: \
-- MHG _veim_ "foam, dross" \
REF: [EDPG]\: 123

### <a id="faita">\*faita- "fat" (adj)</a>
N: \
-- ON _feitr_ \
-- Far. _feitur_ \
-- Elfd. _fiet_ \
W: \
-- MLG _vēt_ \
-- MHG _veiz_, G Cimb. _voaz_ \
REF: [EDPG]\: 124

### <a id="fajja">\*faj(j)a- "fat(?)" (adj)</a>
W: \
-- Du. dial. _vei_ "fat, lush, fertile" \
REF: [EDPG]\: 124

### <a id="faka">\*faka- [n.] "enclosure, section" (n)</a>
W: \
-- OE _fæc_ "space, interval, distance, portion of time" \
-- OFri. _fek_ "part of the house, niche" \
-- MDu. _vac_ "compartment, section", Du. _vak_ "space; profession" \
-- OHG _fah_ "wall, compartment", G _Fach_ "compartment, panel; discipline" \
REF: [EDPG]\: 124

### <a id="fakkōn">\*fakk/gōn- [wk.] "to become sleepy" (v)</a>
W: \
-- E _to fag_ "to tire, weary", obs. "to hang loose, flap", Scot _to faik_ "to fail from weariness; to cease moving" \
REF: [EDPG]\: 124

### <a id="falan">\*falan- [m.] "board" (n)</a>
W: \
-- OE _fala_ \
REF: [EDPG]\: 125

### <a id="falgō">\*falgō- [f.] "fallow land" (n)</a>
W: \
-- ME _falge_, _falwe_ "arable land lying idle; plowing as part of the fallowing process", E _fallow_ \
-- OFri. _fallach_ "arable land" \
-- MDu. _valge_ "id." \
REF: [EDPG]\: 125

### <a id="falkan">\*falkan- [m.] "falcon" (n)</a>
N: \
-- ON _falki_ \
-- Far. _fálkur_ \
-- OSw. _falke_, _falker_ \
W: \
-- MDu. _valce_, Du. _valk_ [c.] \
-- OHG _falco_, G _Falke_ \
REF: [EDPG]\: 125

### <a id="fallan">\*fallan- [st.] "to fall" (v)</a>
N: \
-- ON _falla_ \
-- Far. _falla_ \
-- Elfd. _folla_, OE _fallan_ \
W: \
-- E _to fall_ \
-- OFri. _falla_ \
-- OS _fallan_ \
-- Du. _vallen_ \
-- OHG _fallan_, G _fallen_ \
REF: [EDPG]\: 125

### <a id="falljan">\*falljan- [wk.] "to fell" (v)</a>
N: \
-- ON _fella_ \
-- Far. _fella_ \
-- Elfd. _fella_ [st.] \
W: \
-- OE _fiellan_, E _to fell_ \
-- OFri. _fella_ \
-- OS _fellian_ \
-- Du. _vellen_ \
-- OHG _fellan_, G _fällen_ \
REF: [EDPG]\: 126

### <a id="falōn">\*falōn- [f.] "plain" (n)</a>
N: \
-- OSw. _fala_ \
-- Sw. _fala_ \
REF: [EDPG]\: 126

### <a id="falþan">\*falþan- [st.] "to fold, ply" (v)</a>
E: \
-- Go. _falþan_ \
N: \
-- ON _falda_ "to hood oneself" \
-- Far. _falda_ [wk.] "to put on clothes" \
-- Elfd. _folda_ [wk.] "to hem, seam" \
W: \
-- OE _fealdan_ "to fold", E _to fold_ \
-- WFri. _fâldzje_ [wk.] "id." \
-- MDu. _volden_ [wk.] "id.", Du. _vouwen_ [st./wk.] "id." \
-- OHG _faldan_, _faltan_ "id.", G _falten_ [wk.] "id." \
REF: [EDPG]\: 126

### <a id="faluda">\*faluda- [m.] "pen(?)" (n)</a>
N: \
-- Da. _fold_ "enclosure, pasture" \
W: \
-- OE _fald_, _falod_, _falud_ "fold, stable", E _fold_ \
-- OS _faled_ "id." \
-- MDu. _vaelde_, _vaelt_ "enclosure, yard; dunghill", Du. _mest-vaalt_ [c.] "dunghill" \
REF: [EDPG]\: 126

### <a id="falwa">\*falwa- "pale" (adj)</a>
N: \
-- ON _fǫlr_ \
W: \
-- OE _fealo_, E _fallow_ \
-- OS _falu_ "fallow, yellow" \
-- Du. _vaal_ "faded, drab" \
-- OHG _falo_ "fallow, yellow", G _falb_ "fallow, dun" \
REF: [EDPG]\: 126

### <a id="fanan">\*fanan- [m.] "cloth" (n)</a>
E: \
-- Go. _fana_ \
W: \
-- OE _fana_ "flag, banner", E _vane_ \
-- OFri _fona_ [m.] "id." \
-- OS _fano_ [m.] "id." \
-- Du. _vaan_ [c.] "id." \
-- OHG _fano_ [m.] "id.", G _Fahne_ [m.] "id." \
REF: [EDPG]\: 127

### <a id="fandōn">\*fandōn- [wk.] "to investigate" (v)</a>
W: \
-- OE _fandian_ "to test, investigate, visit" \
-- OFri. _fand(el)ia_ "to visit a sick person", WFri. (early) _fânle_ "to collect fish traps", _fandelje_ "to collect, fetch" \
-- MDu. _vanden_ "to test, investigate, visit" \
-- OHG _fandōn_ "to investigate", MHG _vanden_ "to visit", G _fahnden_ "to search, hunt for" \
REF: [EDPG]\: 127

### <a id="fanē">\*fanē "from" (prep)</a>
W: \
-- OFri. _fan_ \
-- OS _fan_, _fana_ \
-- Du. _van_ \
-- OHG _fana_, _fona_, G _van_ \
REF: [EDPG]\: 127

### <a id="fangi">\*fangi- [m.] "catch" (n)</a>
E: \
-- Go. _ga-fahs_ \
N: \
-- ON _fengr_ \
W: \
-- OE _feng_ "grasp, span, hug" \
-- OFri. _feng_, _fang_, _fong_ "catch, capture" \
-- OHG _fang_ "id.", G _Fang_ "id." \
REF: [EDPG]\: 127

### <a id="fangjan">\*fangjan- ~ \*fankjan- [wk.] "to ignite" (v)</a>
W: \
-- MDu. _ont-fengen_, -_fenken_ \
-- MHG _vengen_, _enpfengen_, _venken_ \
REF: [EDPG]\: 127

### <a id="fanhan">\*fanhan- [st.] "to catch" (v)</a>
E: \
-- Go. _fahan_ \
N: \
-- ON _fá_ \
-- Far. _fáa_ \
-- Elfd. _fą̊_ \
W: \
-- OE _fōn_ \
-- OFri. _fā_ \
-- OS _fāhan_ \
-- Du. _vangen_ \
-- OHG _fāhan_, G _fangen_ \
REF: [EDPG]\: 128

### <a id="fanja">\*fanja- [n.] "fen, swamp" (n)</a>
E: \
-- Go. _fani_ "mud" \
N: \
-- ON _fen_ "fen, bog" \
W: \
-- OE _fen_ [m./n.] "marsh; mud", E _fen_ \
-- OFri. _fenne_ [m.] "pasture" \
-- OS _feni_ "fen" \
-- MDu. _veen_, _vene_- "bog", Du. _veen_ "id.", _ven_ "small lake" \
-- OHG _fenni_ "swamp", MHG _venne_ "id.", G _Fenn_ "id." \
REF: [EDPG]\: 128

### <a id="fanþjan">\*fanþjan- [m.] "foot soldier(?)" (n)</a>
W: \
-- OE _fēða_ "foot soldier; troop" \
-- OHG _fendo_ "foot soldier, pedestrian", MHG _vende_ "pedestrian, boy" \
REF: [EDPG]\: 128

### <a id="faran">\*faran- [st.] "to go" (v)</a>
E: \
-- Go. _faran_ \
N: \
-- ON _fara_ \
-- Elfd. _fårå_ \
W: \
-- OE _faran_, E _to fare_ \
-- OFri. _fara_ \
-- Du. _varen_ [wk.] "to sail" \
-- OHG _faran_ "to go, travel, pass", G _fahren_ "id." \
REF: [EDPG]\: 128

### <a id="fardi">\*fardi- [f.] "journey" (n)</a>
N: \
-- ON _ferð_ \
-- Far. _ferð_ \
-- Elfd. _ferd_ \
W: \
-- OE _fierd_ "expedition" \
-- OFri. _ferd_ "journey, voyage" \
-- OS _fard_ "id." \
-- Du. _vaart_ "canal" \
-- OHG _fart_ "trip, voyage", G _Fart_ "id." \
REF: [EDPG]\: 128

### <a id="fargjan">\*fargjan- [wk.] "to push (down)" (v)</a>
N: \
-- Icel. _fergja_ "to press, put pressure on" \
-- Far. _fergja_ "to press, squeeze, push" \
-- Nw. _fergja_ "to press/weigh down" \
REF: [EDPG]\: 128

### <a id="farha">\*farha- [m.] "farrow, pig" (n)</a>
W: \
-- OE _fearh_, E _farrow_ \
-- OHG _farah_, _farh_ [n.], MHG _varch_ \
REF: [EDPG]\: 129

### <a id="farjōn">\*farjōn- [f.] "ferry" (n)</a>
N: \
-- ON _ferja_ \
W: \
-- MLG _vere_ [f./n.] \
-- MDu. _vere_ [f./n.], Du. _veer_ [n.] \
-- MHG _vere_, G _Fähre_ \
REF: [EDPG]\: 129

### <a id="farma">\*farma- [m.] "moving(?)" (n)</a>
N: \
-- ON _farmr_ "freight, cargo" \
-- Far. _farmur_ "id." \
-- Nw. _farm_ "cargo; flock (of cattle)" \
W: \
-- OE _fearm_ "id." \
-- OS _farm_ "onslaught" \
-- OHG _farm_ "fast ship", MHG _varm_ "id." \
REF: [EDPG]\: 129

### <a id="farna">\*farna- [m.] "fern" (n)</a>
W: \
-- OE _fearn_, _fern_, E _fern_ \
-- OS _farn_ [m./n.] \
-- MDu. _varen_, Du. _varen_ [c.] \
-- OHG _farn_, _farm_ [m./n.], G _Farn_ \
REF: [EDPG]\: 129

### <a id="farwa">\*farwa- "colorful" (adj)</a>
W: \
-- OHG _faro_, MHG _vare_ \
REF: [EDPG]\: 130

### <a id="farzan">\*farza(n)- [m.] "young bull" (n)</a>
N: \
-- ON _farri_ "bull" \
W: \
-- OE _fearr_ "bull, ox" \
-- MDu. _var(r)e_, Du. _var_ [c.] \
-- OHG _farro_ "bull, ox", G _Farre_ "young bull" \
REF: [EDPG]\: 130

### <a id="fasan">\*fasa/ōn- [m./f.] "fringe; fiber" (n)</a>
W: \
-- OE _fæs_ [n.] "fringe" (< \*fasa-) \
-- MDu. _vase_ [f.] "fiber; fringe" \
-- OHG _faso_ [m.], _fasa_ [f.] "fiber, fringe, thread", MHG _vase_ [m./f.] "id.", G _Faser_ [m.] "fiber" \
REF: [EDPG]\: 130

### <a id="fastu">\*fastu- "firm, solid" (adj)</a>
N: \
-- ON _fastr_ \
-- Far. _fastur_ \
-- Elfd. _fast_ \
W: \
-- OE _fæst_, E _fast_ \
-- OFri. _fest_ \
-- OS _fast_, _festi_ \
-- Du. _vast_ \
-- OHG _festi_, G _fest_ \
REF: [EDPG]\: 131

### <a id="fata">\*fata- [n.] "vat, vessel" (n)</a>
N: \
-- ON _fat_ \
W: \
-- OE _fæt_, E _vat_ \
-- OS _fat_ \
-- Du. _vat_ \
-- OHG _faz_, MHG _vaz_, G _Faß_ \
REF: [EDPG]\: 131

### <a id="fatta">\*fatta- "bent" (adj)</a>
N: \
-- ON _fattr_ "bowed backward" \
REF: [EDPG]\: 131

### <a id="fattōn">\*fat(t)ōn- [wk.] "to fetch" (v)</a>
N: \
-- ON _fata_ "to step" \
-- Icel. _fatta_ "to catch on, take in" \
-- Far. _fatta_ (in: _lukki fatti teg!_ "may you prosper") "to catch; grasp" \
W: \
-- OE _fatian_ "to fetch" \
-- OFri. _fatia_ "to load" \
-- MLG _vaten_ "to grasp, pack, load, prepare, dress" \
-- MDu. _vaten_, _vatten_ "to seize, assume, understand", Du. _vatten_, dial. _vaten_ "to seize, understand" \
-- OHG _fazzōn_ "to load, pack, prepare", G _fassen_ "to catch, grasp" \
REF: [EDPG]\: 131

### <a id="faþma">\*faþma- [m.] "fathom" (n)</a>
N: \
-- ON _faðmr_ \
-- Far. _favnur_ \
-- Elfd. _farm_ \
W: \
-- OE _fæðm_ "fathom; bosom", E _fathom_ \
-- OFri. _fethm_ "id." \
-- OS _fathmos_ [m.pl.] "two stretched arms" \
-- Du. _vadem_ [c.] \
-- OHG _fadam_, _fadum_, G _Faden_ "thread" \
REF: [EDPG]\: 132

### <a id="faþō">\*faþō- [f.] "fence" (n)</a>
E: \
-- Go. _faþa_ "hedge, fence, wall" \
W: \
-- MHG _vade_ "fence" \
REF: [EDPG]\: 132

### <a id="faujan-1">\*faujan- [wk.] "to let rot" (v)</a>
N: \
-- ON _feyja_ \
REF: [EDPG]\: 132

### <a id="faujan-2">\*faujan- [wk.] "to sift" (v)</a>
W: \
-- OHG _fowen_, _fewen_, MHG _vewen_ \
REF: [EDPG]\: 132

### <a id="fawa">\*fawa- "few" (adj)</a>
E: \
-- Go. _faus_\* \
N: \
-- ON _fár_ \
-- Far. _fáur_ \
-- Elfd. _fåer_ \
W: \
-- OE _fēa_, E _few_ \
-- OS _fā_ \
-- OHG _fō_ \
REF: [EDPG]\: 132

### <a id="fēan">\*fēan- [st.] "to blame" (v)</a>
E: \
-- Go. _faian\*_ \
REF: [EDPG]\: 132

### <a id="fedwar">\*fedwar- "four" (num)</a>
E: \
-- Go. _fidwor_ \
N: \
-- ON _fjórir_, acc. _fjóra_, [f.] _fjórar_, [n.] _fjǫgur_, gen. _fjǫgurra_, dat. _fjórum_ \
-- Far. _fýra_ \
-- Elfd. _fiuorer_, acc. _fiuora_, [f.] _fiuorer_, [n.] _fiuorų_, dat. _fiuorum_ \
W: \
-- OE _fēower_, E _four_ \
-- OFri. _fiūwer_, _fiōwer_, _fiāwer_ \
-- OS _fiuwar_, _fior_, [n.] _fieri_, dat. _fiwarun_ \
-- Du. _vier_ \
-- OHG _fior_, G _vier_ \
REF: [EDPG]\: 133

### <a id="fehan">\*fehan- [st.] "to rejoice" (v)</a>
W: \
-- OE _gefēon_, -_feohan_ "to be glad, rejoice, exult" \
-- OHG _gi-fehan_ "to rejoice" \
REF: [EDPG]\: 133

### <a id="fēhi">\*fēhi- "satisfactory(?)" (adj)</a>
E: \
-- Go. _ga-fehaba_ "properly" (adv) \
REF: [EDPG]\: 133

### <a id="fehtan">\*fehtan- [st.] "to fight" (v)</a>
W: \
-- OE _feohtan_, E _to fight_ \
-- OFri. _fiuchta_ \
-- Du. _vechten_ \
-- OHG _fehtan_, G _fechten_ \
REF: [EDPG]\: 134

### <a id="fehu">\*fehu- [n.] "cattle" (n)</a>
E: \
-- Go. _faihu_ "movable goods, possessions, cattle" \
N: \
-- ON _fé_, gen. _fjár_ "cattle, property, money" \
-- Far. _fæ_, gen. _fíggjar_ "cattle" \
W: \
-- OE _feoh_, _fioh_ "cattle, property, money", E _fee_ \
-- OS _fehu_ "cattle; property" \
-- Du. _vee_ "cattle" \
-- OHG _fihu_ "cattle, animal, money", G _Vieh_ "cattle" \
REF: [EDPG]\: 134

### <a id="felesa">\*fel(e)sa- [m.] "mountain" (n)</a>
N: \
-- ON _fell_, _fjall_ [n.] \
-- Far. _fjall_ [n.] \
W: \
-- OS _felis_, _filis_, _filis_ "rock, stone" \
-- OHG _felis_ "rock, cliff", MHG _vels(e)_ "id.", G _Felsen_ "id." \
REF: [EDPG]\: 134

### <a id="felgō">\*felgō- [f.] "rim of a wheel" (n)</a>
W: \
-- OE _fealg_, E _felly_ \
-- MDu. _velge_, _vellige_ "felly; patella", Du. _velg_ [c.] "felly" \
-- OHG _felga_ "felly, hoop", G _Felge_ "felly" \
REF: [EDPG]\: 134

### <a id="felhan">\*felhan- [st.] "to hide" (v)</a>
E: \
-- Go. _filhan_ \
N: \
-- ON _fela_ "to hide, conceal; to give in trust" \
W: \
-- OE _felan_, _feolan_, _felgan_ "to stick, adhere; to proceed, go, reach; to undergo" \
-- OS _bi-felhan_ "to entrust, bestow, command; to bury" \
-- Du. _velen_ [wk.] "to suffer" \
-- OHG _felhan_, _fel(a)han_ "to trust, hand over" \
REF: [EDPG]\: 135

### <a id="fella">\*fella- [n.] "membrane, skin, hide" (n)</a>
E: \
-- Go. _þruts-fill_ "leprosy" \
N: \
-- ON _fell_, _fjall_ "skin" \
-- OSw. _fiæl_ "id." \
-- Sw. _fjäll_ "id." \
W: \
-- OE _fell_ "id.", E _fell_ \
-- OS _fell_ "skin, membrane" \
-- Du. _vet_ "id." \
-- OHG _fel_ "skin, pelt", G _Fell_ "id." \
REF: [EDPG]\: 135

### <a id="felman">\*felma(n)- [m.] "membrane" (n)</a>
W: \
-- OE _ǣger-felma_ "skin of an egg", E _film_ \
-- MDu. _velm_, _vilm_ "film" \
REF: [EDPG]\: 135

### <a id="felō">\*felō- [f.] "board" (n)</a>
N: \
-- ON _fjǫl_ \
-- Far. _fjøl_ \
-- Elfd. _fyöl_ \
REF: [EDPG]\: 135

### <a id="fetpa">\*fetpa- [m.] "field" (n)</a>
N: \
-- OSw. _fiälder_ "isolated field" \
W: \
-- OE _feld_, _feald_, E _field_ \
-- OFri. _feld_ [n.] \
-- OS _feld_ \
-- Du. _veld_ [n.] \
-- OHG _feld_ [n.], G _Feld_ [n.] \
REF: [EDPG]\: 135

### <a id="felu">\*felu- ~ \*fulu- "much, many" (adj)</a>
E: \
-- Go. _filu_ \
N: \
-- ON _fjǫl_- (pref) \
-- Far. _fjøl_- (pref) \
W: \
-- OE _fela_, _feala_, _feola_ \
-- OFri. _felo_, _fele_, _fule_, _fole_ \
-- OS _filu_, _filo_ \
-- MDu. _veel_ \
-- OHG _filu_, _filo_, G _viel_ \
REF: [EDPG]\: 136

### <a id="felwō">\*felwō- [f.] "willow" (n)</a>
W: \
-- OHG _felwa_, _felawa_, G _Felbe_ \
REF: [EDPG]\: 136

### <a id="fergunja">\*fergunja- [n.] "mountain" (n)</a>
E: \
-- Go. _fairguni_ "mountain range" \
N: \
-- ON _fjǫrgyn_ [f.] "Mother Earth" \
W: \
-- OE _firgen_ "mountain" \
-- OHG _Firgunnea_ [f.] "Ore Mountains" \
REF: [EDPG]\: 136

### <a id="ferhwa-1">\*ferhwa- [m./n.] "oak(?)" (n)</a>
N: \
-- ON poet. _fjǫrr_ [m.] "tree" \
W: \
-- OHG _fereh-eih_ [f.] "id.", G _Ferch_ [n.] "id." \
REF: [EDPG]\: 136

### <a id="ferhwa-2">\*ferhwa- [n.] "life, essence" (n)</a>
E: \
-- Go. _fairƕus_ [m.] "world" \
N: \
-- ON _fjǫr_ "life, pith" \
W: \
-- OE _feorh_ [m./n.] "life; man" \
-- OFri. _ferech_ [n.(?)] "life" \
-- OS _ferah_, _ferh_ "life, spirit" \
-- OHG _ferah_, _ferh_ "id.", MHG _verch_ "id." \
REF: [EDPG]\: 137

### <a id="ferrai">\*ferrai "far" (adv)</a>
E: \
-- Go. _fairra_ \
N: \
-- ON _fjarri_ \
W: \
-- OE _feorr_, E _far_ \
-- OS _ferro_ \
-- Du. _ver_ \
-- OHG _ferro_, MHG _verre_ \
REF: [EDPG]\: 137

### <a id="ferrjan">\*ferrjan- [wk.] "to be far; to remove" (v)</a>
N: \
-- ON _firra_ "to lose sight of; to deprive of" \
-- OFri. _fīria_ "to be far away" \
W: \
-- OHG _firren_ "to remove; to be far away", MHG _virren_ "id." \
REF: [EDPG]\: 137

### <a id="fersnō">\*fersnō- [f.] "heel" (n)</a>
E: \
-- Go. _fairzna_ \
W: \
-- OE _fiersn_ (< \*fersni-) \
-- MDu. _versene_, _verse_ \
-- OHG _fersana_, G _Fersne_ \
REF: [EDPG]\: 137

### <a id="fertan">\*fertan- ~ \*fretan- [st.] "to fart" (v)</a>
N: \
-- ON _freta_ \
-- Far. _freta_ [wk.] \
-- OSw. _fiærta_ [wk.], Sw. _fjärta_ [wk.] \
-- Da. _fjærte_ [wk.] \
W: \
-- OE _feortan_, E _to fart_ \
-- MDu. _verten_, _vorten_ \
-- OHG _ferzan_, MHG _verzen_ \
REF: [EDPG]\: 137

### <a id="ferudi">\*ferudi "last year" (adv)</a>
N: \
-- ON _í fjǫrð_ \
-- Far. _í fjør_ \
-- Nw. _i fjor_ \
-- Elfd. _i fiuord_ \
W: \
-- MHG _vert_ \
REF: [EDPG]\: 137

### <a id="fesōn">\*fesōn- [f.] "chaff, fiber" (n)</a>
W: \
-- OS _fesa_ "pod" \
-- MDu. _vese_ "fiber, husk; fringe", Du. _vezel_ [c.] "fiber" \
-- OHG _fesa_ "id.", MHG _vese_ "id.", G Swi. Rhntl. _feəsə_ "id." \
REF: [EDPG]\: 138

### <a id="fetan">\*fetan- [st.] "to go; to fall" (v)</a>
N: \
-- ON _feta_ "to step" \
-- Far. _feta_ [wk.] "to stride, pace, step out" \
W: \
-- OE _fetan_ "to fall" \
-- OHG _fezzan_, _gi-fezzan_ "id." \
REF: [EDPG]\: 138

### <a id="fetura">\*fetura/ō- [m./f.] "fetter" (n)</a>
N: \
-- ON _fjǫturr_ [m.] \
-- Far. _fjøtur_ [n.] \
-- OSw. _fiætur_ [m.] \
W: \
-- OE _feter_, _fetor_ [f.], E _fetter_ \
-- OS _feterōs_ [m.pl.] \
-- Du. _veter_ [c.] "shoelace" \
-- OHG _vezzera_ [f.], MHG _vezzer_ [f.] "fetter; manacle" \
REF: [EDPG]\: 138

### <a id="feþan">\*feþan- [m.] "penis(?)" (n)</a>
W: \
-- MDu. _vede_ \
REF: [EDPG]\: 138

### <a id="feþrō">\*feþrō- [f.] "feather" (n)</a>
N: \
-- ON _fjǫðr_ \
-- Far. _fjøður_ "feather; fin" \
-- Elfd. _fjäðer_ \
W: \
-- OE _feðer_, E _feather_ \
-- OS _fethera_ \
-- Du. _veer_ [c.] \
-- OHG _fedara_, _federa_, G _Feder_ \
REF: [EDPG]\: 138

### <a id="feuhtjōn">\*feuhtjōn- [f.] "spruce" (n)</a>
W: \
-- OHG _fiuhta_, MHG _viehte_, G _Fichte_ \
REF: [EDPG]\: 139

### <a id="feukan">\*feukan- ~ \*fūkan- [st.] "to blow" (v)</a>
N: \
-- ON _fjúka_ "to be blown away; to fly off" \
-- Far. _fúka_ "to drift; to dash, dart; to be blown away; to be rumored" \
W: \
-- EDu. _fijcken_, _fuycken_ "to thrust" \
-- G _fauchen_ [wk.] "to hiss, snarl" \
REF: [EDPG]\: 139

### <a id="fewurþan">\*fewurþ/dan- "fourth" (num)</a>
N: \
-- ON _fjórði_ \
-- Far. _fjórði_ \
-- Elfd. _fiuord_ \
W: \
-- OE _fēorða_, _fēowerða_, E _fourth_ \
-- OFri. _fiōrda_, _fiārda_, _fiērda_ \
-- OS _fiortho_ \
-- Du. _vierde_ \
-- OHG _fiordo_, G _vierte_ \
REF: [EDPG]\: 139

### <a id="fīfaldra">\*fīfaldra/ō- [m./f.] "butterfly" (n)</a>
N: \
-- Far. _firvaldur_ [m.] \
-- OSw. _fiædhal_ [m.] \
-- Sw. _fjäril_, dial. _fjärålder_ [c.] \
-- Elfd. _fyörolder_ [m.] \
W: \
-- OE _fīfealde_ [f.] \
-- OS _fīfoldara_ [f.] \
-- MDu. _vivalter_, _viveltere_ [m./f.] \
-- OHG _fīvaltra_ [f.], G _Feifalter_ [m./f.] \
REF: [EDPG]\: 140

### <a id="fijand">\*fi(j)and- [m.] "enemy" (n)</a>
E: \
-- Go. _fijands_ \
N: \
-- ON _fjandi_ \
-- Far. _fíggindi_ \
W: \
-- OE _fēond_, E _fiend_ \
-- OFri. _fiänd_, _fiūnd_, _fīn(d)_ \
-- ODu. _fīunt_, Du. _vijand_ [c.] \
-- OHG _fīant_, G _Feind_ \
REF: [EDPG]\: 140

### <a id="fijēn">\*fi(j)ēn- [wk.] "to hate" (v)</a>
E: \
-- Go. _fijan_, _fian_ \
N: \
-- ON _fjá_ \
-- Far. _fíggja_ \
W: \
-- OE _fēon_ \
-- OHG _fīēn_ \
REF: [EDPG]\: 140

### <a id="fimfe">\*fimfe "five" (num)</a>
E: \
-- Go. _fimf_ \
N: \
-- ON _fimm_ \
W: \
-- OE _fīf_, E _five_ \
-- OFri. _fīf_ \
-- OS _fīf_ \
-- Du. _vijf_ \
-- OHG _fimf_, G _fünf_ \
REF: [EDPG]\: 140

### <a id="fimftan">\*fimftan- "fifth" (num)</a>
E: \
-- Go. _fimfta_ \
N: \
-- ON _fimti_, _femti_ \
-- Far. _fimti_ \
-- Elfd. _femt_ \
W: \
-- OE _fīfta_, E _fifth_ \
-- OFri. _fīfta_ \
-- OS _fīfto_ \
-- Du. _vijfde_ \
-- OHG _fimto_, _finfto_, G _fünfte_ \
REF: [EDPG]\: 140

### <a id="fingra">\*fingra- [m.] "finger" (n)</a>
E: \
-- Go. _figgrs_ \
N: \
-- ON _fingr_ \
W: \
-- OE _finger_, E _finger_ \
-- OFri. _finger_ \
-- OS _fingar_ \
-- Du. _vinger_ [c.] \
-- OHG _fingar_, G _Finger_ \
REF: [EDPG]\: 141

### <a id="finkan">\*finkan- [st.] "to sparkle, burn" (v)</a>
W: \
-- MDu. _vinken_ [st./wk.] "to sparkle; to smolder" \
-- G obs. _finkeln_ [wk.] "to burn, hurt" \
REF: [EDPG]\: 141

### <a id="finkjan">\*finkja(n)- [m.] "finch" (n)</a>
W: \
-- OE _finc_, E _finch_ \
-- MDu. _vink(e)_ [m./f.], Du. _vink_ [c.] \
-- OHG _finko_, G _Fink_ \
REF: [EDPG]\: 141

### <a id="finnōn">\*fin(n)ōn- [f.] "fin" (n)</a>
N: \
-- OSw. _fina_ "fin" \
-- Sw. _fena_ [c.] "fin; chaff, husk" \
W: \
-- MDu. _vinne_ "fin; wing; prickle; awn" \
REF: [EDPG]\: 141

### <a id="finþan">\*finþan- [st.] "to find; to feel" (v)</a>
E: \
-- Go. _finþan_ \
N: \
-- ON _finna_ \
W: \
-- OE _findan_, E _to find_ \
-- OFri. _finda_ \
-- OS _fīđan_, _findan_ \
-- ODu. _findan_, Du. _vinden_ \
-- OHG _findan_, G _finden_ \
REF: [EDPG]\: 142

### <a id="fīsan">\*fīsan- [st.] "to blow" (v)</a>
N: \
-- ON _físa_ \
-- Icel. _físa_ [wk.] "to blow (on a fire); to fart" \
-- Far. _físa_ [wk.] "to blow, draw; to get to fly, stir up; to hiss; to snort" \
-- Elfd. _faisa_ "to fart" \
W: \
-- MHG _vīsen_ "to fart" \
REF: [EDPG]\: 142

### <a id="fiska">\*fiska- [m.] "fish" (n)</a>
E: \
-- Go. _fisks_ \
N: \
-- ON _fiskr_ \
-- Far. _fiskur_ \
-- Elfd. _fisk_ \
W: \
-- OE _fisc_, E _fish_ \
-- OFri. _fisk_ \
-- OS _fisk_ \
-- Du. _vis_ [c.] \
-- OHG _fisc_, G _Fisch_ \
REF: [EDPG]\: 142

### <a id="fisōn">\*fisōn- [wk.] "to blow" (v)</a>
N: \
-- Nw. _fīsa_ "to blow; to fart; to whisper" \
W: \
-- MDu. _vesen_ "to whisper" \
REF: [EDPG]\: 142

### <a id="fitnan">\*fitnan- [wk.] "to become fat" (v)</a>
N: \
-- ON _fitna_ \
-- Far. _fitna_ \
REF: [EDPG]\: 142

### <a id="flahan">\*flahan- [st.] "to flay, strip" (v)</a>
N: \
-- ON _flá_ "to flay; to strip (of clothes)" \
-- Elfd. _flå_ [wk.] "id." \
W: \
-- OE _flēan_ "to flay" \
-- MDu. _vlaen_ "id." \
REF: [EDPG]\: 143

### <a id="flahō">\*flahō- [f.] "strip" (n)</a>
N: \
-- ON _flá_ "strip of land, meadow" \
REF: [EDPG]\: 143

### <a id="flahsa">\*flahsa- [n.] "flax" (n)</a>
W: \
-- OE _fleax_, E _flax_ \
-- OFri. _flax_ \
-- MLG _vlas_ [m.] \
-- Du. _vlas_ \
-- OHG _flahs_ [m.], G _Flachs_ [m.] \
REF: [EDPG]\: 143

### <a id="flaina">\*flaina- "flat(?)" (adj)</a>
N: \
-- Nw. _flein_ "bald, naked; flat" \
REF: [EDPG]\: 143

### <a id="flaizan">\*flaizan- "more" (comp)</a>
E: \
-- Go. _flaiza_ \
N: \
-- ON _fleiri_ \
-- Far. _fleiri_ \
-- Elfd. _flierer_ \
REF: [EDPG]\: 143

### <a id="flaka">\*flaka- "flat" (adj)</a>
W: \
-- MDu. _vlac_, Du. _vlak_ \
-- OHG _flah_, G _flach_ \
REF: [EDPG]\: 143

### <a id="flakkōn-1">\*flakk/gōn- [wk.] "to beat" (v)</a>
W: \
-- E _to flack_ "to beat with a flail" \
-- G _flacken_ "to beat wool" \
REF: [EDPG]\: 144

### <a id="flakkōn-2">\*flakk/gōn- [wk.] "to flutter" (v)</a>
N: \
-- Nw. _flaga_ "to fluctuate" \
W: \
-- ME _flakken_ "to beat feebly, flutter" \
-- MDu. _vlacken_, _vlaken_ "to flame", Du. _flakkeren_ "to flutter" \
-- OHG _flagarōn_ "to flutter", G Swi. _flacken_ "to flare" \
REF: [EDPG]\: 144

### <a id="flakkōn-3">\*flakkōn- [wk.] "to swerve" (v)</a>
N: \
-- ON _flakka_ "to rove about" \
-- Far. _flakka_ "to wander about, roam" \
REF: [EDPG]\: 144

### <a id="flata">\*flata- "flat" (adj)</a>
N: \
-- ON _flatr_ "flat, level" \
W: \
-- OS _flat_ "flat, shallow" \
-- MDu. _vlat_ (infl. _vladde_) "id." \
-- OHG _flaz_ \
REF: [EDPG]\: 144

### <a id="flatja">\*flatja- [n.] "floor" (n)</a>
N: \
-- ON _flet_ "raised flooring" \
W: \
-- OE _flett_ "floor of a house, dwelling", E _flat_ \
-- OFri. _flet_ [n.?] "house; home" \
-- OS _flet_ "hall, house" \
-- OHG _flezzi_ "floor; flat area", MHG _vletz(e)_ "id." \
REF: [EDPG]\: 145

### <a id="flaugjan">\*flaugjan- [wk.] "to let fly" (v)</a>
E: \
-- Go. _us-flaugjan_\* \
N: \
-- ON _fleygja_ \
W: \
-- MHG _er-vlougen_ \
REF: [EDPG]\: 145

### <a id="flauhō">\*flauhō- [f.] "flea" (n)</a>
N: \
-- ON _fló_, pl. _flœr_ \
W: \
-- OE _flēah_, _flēa_, E _flea_ \
-- WFri. _flie_ [c.] \
-- MDu. _vloo_, _vlo(o)y_, Du. _vlo_ [c.] \
-- OHG _flō(h)_ [m.], G _Floh_ [m.] \
REF: [EDPG]\: 145

### <a id="flauja">\*flauja- [n.] "ship" (n)</a>
N: \
-- ON _fley_ \
-- Far. poet. _floy_ "vessel, craft" \
REF: [EDPG]\: 145

### <a id="flauma">\*flauma- [m.] "stream" (n)</a>
N: \
-- ON _flaumr_ "eddy" \
REF: [EDPG]\: 145

### <a id="flauti">\*flauti- [f.] "cream" (n)</a>
N: \
-- ON _flautir_ [f.pl.] "whipped milk" \
W: \
-- OE _flī(e)te_ "cream" \
REF: [EDPG]\: 145

### <a id="flauþī">\*flauþī- [f.] "cross-beam" (n)</a>
N: \
-- ON _fleyðr_ \
REF: [EDPG]\: 145

### <a id="flehtan">\*flehtan- [st.] "to braid, plait" (v)</a>
N: \
-- ON _flétta_ [wk.] \
W: \
-- OE _fleohtan_ \
-- MLG _vlechten_ \
-- MDu. _vlechten_, Du. _vlechten_ \
-- OHG _flehtan_, G _flechten_ \
REF: [EDPG]\: 146

### <a id="fleugan">\*fleugan- [st.] "to fly" (v)</a>
N: \
-- ON _fljúga_ \
-- Far. _flúgva_ \
-- Elfd. _fliuoga_ \
W: \
-- OE _flēogan_, E _to fly_ \
-- OFri. _fliāga_ \
-- MDu. _vliegen_, Du. _vliegen_ \
-- OHG _fliugan_, _fliogan_, G _fliegen_ \
REF: [EDPG]\: 146

### <a id="fleugōn">\*fleugōn- [f.] "fly" (n)</a>
N: \
-- Nw. _fljuge_ \
W: \
-- OE _flēoge_, E _fly_ \
-- MLG _vlēge_ \
-- MDu. _vliege_, Du. _vlieg_ [c.] \
-- MHG _fliuga_, _flioga_, G _Fliege_ \
REF: [EDPG]\: 146

### <a id="fleusiz">\*fleusiz- [n.] "fleece" (n)</a>
W: \
-- OE _flēos_, _flīes_, E _fleece_ \
-- MDu. _vlies_, _vluus_ "fleece; tuft", Du. _vlies_ "id." \
-- MHG _vlies_, _vlius_ "id." \
REF: [EDPG]\: 146

### <a id="fleuta">\*fleuta- [n.] "stream" (n)</a>
N: \
-- ON _fljót_ "stream, river" \
W: \
-- OE _flēot_ "estuary, sea; fleet, ship", E _fleet_ \
-- MLG _flēt_ [m./n.] "river" \
-- MDu. _vliet_ [m.] "body of water" \
REF: [EDPG]\: 146

### <a id="fleutan">\*fleutan- [st.] "to float, stream" (v)</a>
N: \
-- ON _fljóta_ \
W: \
-- OE _flēotan_ "to float" \
-- OFri. _fliāta_ "id." \
-- OS _fliotan_ "id." \
-- MDu. _vlieten_ "id.", Du. _vlieten_ "id." \
-- OHG _fliozan_ "id.", G _fließen_ "id." \
REF: [EDPG]\: 147

### <a id="fleuþra">\*fleuþra- [m.] "elder" (n)</a>
W: \
-- WFri. _flear_ "elder; soggy area" \
-- MLG _vlēder_ \
-- ODu. _fliethar_, MDu. _vlieder_, _vlier_, Du. _vlier_ [c.] \
REF: [EDPG]\: 147

### <a id="flītan">\*flītan- [st.] "to strive, fight" (v)</a>
W: \
-- OE _flītan_ "to strive, contend, dispute" \
-- MDu. _vliten_ [st./wk.] "to be enthusiastic; to strive for" \
-- OHG _flīzan_ "to strive, try, fight" \
REF: [EDPG]\: 147

### <a id="flōan">\*flōan- [st.] "to flow" (v)</a>
N: \
-- ON _flóa_ [wk.] "to flood; be flooded" \
W: \
-- OE _flōwan_ "to flow", E _to flow_ \
-- MLG _vlōien_ [wk.] "to flow; flood" \
-- MDu. _vloeyen_ [wk.] "to flow; swim; flood", Du. _vloeien_ [wk.] "to flow" \
REF: [EDPG]\: 147

### <a id="flōdu">\*flōdu- [m./f.] "flood" (n)</a>
E: \
-- Go. _flódus_ [m.] \
N: \
-- ON _flóð_ [n.] \
-- Far. _flóð_ [f.] "high tide; heavy rain" \
W: \
-- OE _flōd_ [m./n.] "id.", E _flood_ \
-- OS _flōd_ [m./f.] "flood, river" \
-- Du. _vloed_ [c.] "flood; high tide" \
-- OHG _fluot_ [f.], G _Flut_ [f.] "river; tide" \
REF: [EDPG]\: 147

### <a id="flōhō">\*flōhō- [f.] "layer" (n)</a>
N: \
-- ON _fló_ \
W: \
-- OE _flōh_ "chip" \
-- OHG _fluoh_ "rock, cliff", MHG _vluo_ "id.", G dial. _Fluh_ "id." \
REF: [EDPG]\: 148

### <a id="flōkan">\*flōkan- [st.] "to slap, mourn, lament" (v)</a>
E: \
-- Go. _flokan_\* "to lament" \
W: \
-- OE _flōcan_ "id." \
REF: [EDPG]\: 148

### <a id="flōra">\*flōra- [m.] "floor" (n)</a>
N: \
-- ON _flórr_ "floor of a cowshed" \
W: \
-- OE _flōr_ [m./f.] "floor; attic", E _floor_ \
-- Du. _vloer_ [c.] "id." \
-- OHG _fluor_ "field", G _Flur_ "id." \
REF: [EDPG]\: 148

### <a id="flugi">\*flugi- "able to fly" (adj)</a>
W: \
-- E _fledge_ \
-- MDu. _vlugghe_ "being able to fly; fast, quick" \
-- OHG _flucki_, MHG _vlücke_ \
REF: [EDPG]\: 148

### <a id="flukkan">\*flukka(n)- [m.] "group" (n)</a>
N: \
-- ON _flokkr_ "troop, host; flock" \
-- Far. _flokkur_ "multitude, crowd, party; flock (of birds)" \
W: \
-- OE _flocc_ [n.] "flock, band, company", E _flock_ \
-- MLG _vlocke_ "group" \
REF: [EDPG]\: 148

### <a id="flukkan">\*flukkan- [m.] "flake" (n)</a>
W: \
-- MDu. _vlocke_ [f.] "flake, tuft, fluff", Du. _vlok_ [c.] "id." \
-- OHG _flocko_ "id.", G _Flocke_ "id." \
REF: [EDPG]\: 149

### <a id="flukkōn">\*flukk/gōn- [wk.] "to fly, flutter" (v)</a>
W: \
-- MDu. _vlogen_ "to fly" \
-- G _flocken_ "to fly around, flutter" \
REF: [EDPG]\: 149

### <a id="fluta">\*fluta- [n.] "floating" (n)</a>
N: \
-- ON _flot_ "floating; grease" \
W: \
-- OE _flot_ "sailable body of water, sea" \
-- Du. _vlot_ "raft" \
REF: [EDPG]\: 149

### <a id="fluttōn">\*flut(t)ōn- [wk.] "to float, stream" (v)</a>
N: \
-- ON _flota_ \
W: \
-- OE _flotian_ \
-- MDu. _vlot(t)en_ \
REF: [EDPG]\: 149

### <a id="fn">\*fneusan- ~ \*fnūsan- [st.] "to sneeze" (v)</a>
N: \
-- ON _hnjósa_ \
-- Far. _njósa_ \
-- Nw. _njosa_, _nysa_ \
W: \
-- OE _fnēosan_, ME _fnēsen_, _snēsen_, E _to sneeze_ \
-- MDu. _niesen_, _fniesen_, Du. _niezen_, obs. _fniezen_ [wk.] \
-- MHG _pfnūsen_ "to snort, sneeze" \
REF: [EDPG]\: 149

### <a id="fnuzzōn">\*fnuz(z)ōn- [wk.] "to sneeze" (v)</a>
N: \
-- Icel. _hnerra_ \
W: \
-- MHG _pfnurren_ "to swell; to snort; to grumble" \
REF: [EDPG]\: 150

### <a id="fōdjan">\*fōdjan- [wk.] "to feed; to rear" (v)</a>
E: \
-- Go. _fodjan_ \
N: \
-- ON _fœða_ \
W: \
-- OE _fēdan_, E _to feed_ \
-- OFri. _fēda_ \
-- OS _fōdian_ \
-- Du. _voeden_ \
-- OHG _fuottan_, MHG _vüeten_ \
REF: [EDPG]\: 150

### <a id="fōdra">\*fōdra- [n.] "fodder" (n)</a>
E: \
-- Go. _fodr_ "scabbard, sheath, casing" \
N: \
-- ON _fóðr_ "fodder, foddering" \
W: \
-- OE _fōdor_ "f�dder; sheath", E _fodder_ \
-- MLG _vōder_ "fodder; casing" \
-- MDu. _voeder_ "lining, cover; fodder", Du. _voer_, _voeder_ \
-- OHG _fuotar_, MHG _vuoter_, G _Futter_ \
REF: [EDPG]\: 150

### <a id="fōgjan">\*fōgjan- [wk.] "to join together" (v)</a>
W: \
-- OE _fēgan_, E _to fay_ \
-- OS _fōgian_ \
-- Du. _voegen_ \
-- OHG _fuogan_, G _fügen_ \
REF: [EDPG]\: 150

### <a id="fōljan">\*fōljan- [wk.] "to feel" (v)</a>
W: \
-- OE _fēlan_, E _to feel_ \
-- OFri. _fēla_ \
-- OS _gi-fōlian_ \
-- Du. _voelen_ \
-- G _fühlen_ \
REF: [EDPG]\: 150

### <a id="fōra">\*fōra- [m.] "piglet" (n)</a>
W: \
-- OE _fōr_ "pig, hog" \
-- OS _fōr_ [n.] "immature pig", MLG _vōr_ "skinny piglet" \
REF: [EDPG]\: 151

### <a id="fōr">\*fōr ~ \*fun- [n.] "fire" (n)</a>
E: \
-- Go. _for_, gen. _funins_ \
N: \
-- ON _fýrr_, _fúrr_ [m.] _funi_ [m.] "flame" \
-- Far. _fýrur_ [m.] "fire; lighthouse", _funi_ [m.] "dense smoke" \
W: \
-- OE _fȳr_, E _fire_ \
-- OFri. _fiūr_, _fiōr_, WFri. _fjoer_ \
-- OS _fiur_ \
-- ODu. _fuir_, Du. _vuur_ \
-- OHG _fiur_, _fuir_, G _Feuer_ \
REF: [EDPG]\: 151

### <a id="fōri">\*fōri- "passable; able to go" (adj)</a>
N: \
-- ON _fœrr_ "able to go; capable of being passed, safe" \
W: \
-- OE _fēre_ "passable; able to go" \
-- OHG _gi-fuori_ "fit, convenient" \
REF: [EDPG]\: 151

### <a id="fōrjan">\*fōrjan- [wk.] "to lead" (v)</a>
N: \
-- ON _fœra_ \
W: \
-- OE _fēran_ "to go" \
-- OFri. _fēra_ "id." \
-- OS _fōrian_ "id." \
-- Du. _voeren_ "id." \
-- OHG _fuorran_, _fuoren_ "id.", G _führen_ "id." \
REF: [EDPG]\: 151

### <a id="fōt">\*fōt- [m.] "foot" (n)</a>
E: \
-- Go. _fotus_ \
N: \
-- ON _fótr_, pl. _fœtr_ "foot; leg" \
-- Far. _fótur_, pl. _føtur_ \
-- Elfd. _fuot_ "foot; leg" \
W: \
-- OE _fōt_, pl. _fēt_ E _foot_ \
-- OFri. _fōt_ \
-- OS _fōt_ \
-- Du. _voet_ [c.] \
-- OHG _fuoz_, G _Fuß_ \
REF: [EDPG]\: 152

### <a id="fōþra">\*fōþra- [n.] "load; vessel; fodder" (n)</a>
W: \
-- OE _fōðor_ "cartload; fodder" \
-- MLG _vōder_ "cartload" \
-- MDu. _voeder_ "wine vat; lead weight" \
-- OHG _fuodar_ "fodder; cartload", MHG _vuoder_ "id.", G _Fuder_ [m.] "cartload" \
REF: [EDPG]\: 152

### <a id="fraisan">\*fraisan- [st.] "to tempt" (v)</a>
E: \
-- Go. _fraisan_ \
W: \
-- OE _frāsian_ [wk.] "to ask, inquire, tempt" \
REF: [EDPG]\: 152

### <a id="fraiwa">\*fraiwa- [n.] "seed" (n)</a>
E: \
-- Go. _fraiw_ \
N: \
-- ON _fræ_, _frjó_ \
-- Elfd. _frie_ \
REF: [EDPG]\: 152

### <a id="fratōn">\*fratōn- [wk.] "to fart" (v)</a>
N: \
-- ON _frata_ \
-- Far. _frata_ \
-- Elfd. _fråtå_ \
W: \
-- MHG _varzen_, G _farzen_ \
REF: [EDPG]\: 153

### <a id="fraþjan">\*fraþjan- [st.] "to understand" (v)</a>
E: \
-- Go. _fraþjan_ \
REF: [EDPG]\: 153

### <a id="fraujan">\*frauja(n)- [m.] "lord" (n)</a>
E: \
-- Go. _frauja_ \
W: \
-- OE _frēa_ \
-- OS _frō_, _frōio_ \
-- MDu. _vroon_, _vrone_ "lord" (with _n_ generalized from the oblique cases) \
-- OHG _frō_, MHG _vrō_ \
REF: [EDPG]\: 153

### <a id="frawa">\*frawa- "quick, light" (adj)</a>
N: \
-- ON _frár_, _frór_ "swift, lightfooted" \
-- Icel. _frár_ "id." \
-- Far. _fróur_ "cheerful, gay, happy" \
W: \
-- OS _frā_, _frō_ "happy" \
-- OHG _frao_, _frō_ "id.", G _froh_ "id." \
REF: [EDPG]\: 153

### <a id="frēgi">\*frēgi- "renowned" (adj)</a>
N: \
-- ON _frægr_ "famous" \
W: \
-- OE _ge-frǣge_ "renowned, celebrated" \
-- OS _gi-frāgi_ "known, famous" \
REF: [EDPG]\: 153

### <a id="frēgō">\*frēgō- [f.] "question" (n)</a>
W: \
-- Du. _vraag_ [c.] \
-- OHG _frāga_ "question, investigation, doctrine", G _Frage_ "question" \
REF: [EDPG]\: 153

### <a id="frehnan">\*frehnan- [st.] "to announce, rumor" (v)</a>
E: \
-- Go. _fraihnan_ "to ask a question" \
N: \
-- ON _fregna_ "to hear of, be informed of; to ask" \
W: \
-- OE _frignan_ "to ask, inquire", _gi-frignan_ "to find out, hear about, learn something through asking" \
-- OS _fregnan_ "to ask", _gi-fregnan_ "to hear tell of, learn about" \
REF: [EDPG]\: 154

### <a id="freka">\*freka- "avaricious" (adj)</a>
E: \
-- Go. _faihu-friks_ \
N: \
-- ON _frekr_ "greedy; harsh; rigorous" \
-- Far. _frekur_ "avid, covetous; severe; vigorous; audacious" \
-- Elfd. _frek_ "friendly, nice" \
W: \
-- OE _frec_ "eager, greedy, bold" \
-- MDu. _vrec_ "greedy", Du. _vrek_ [c.] "avaricious person" \
-- OHG _freh_ "eager, avid", MHG _vrech_ "brave" \
REF: [EDPG]\: 154

### <a id="freknōn">\*freknōn- [f.] "freckle" (n)</a>
N: \
-- Icel. _frekna_ \
-- Far. _frøkna_ \
-- Nw. _frekne_, _frokne_ \
-- Sw. _fräkne_ \
-- Da. _fregne_ \
W: \
-- E _freckle_ \
REF: [EDPG]\: 154

### <a id="freusan">\*freusan- [st.] "to freeze" (v)</a>
N: \
-- ON _frjósa_ \
-- Elfd. _friuosa_ \
W: \
-- OE _frēosan_, E _to freeze_ \
-- MLG _vrēsen_ \
-- MDu. _vriesen_, Du. _vriezen_ \
-- OHG _friosan_, G _frieren_ \
REF: [EDPG]\: 154

### <a id="freuþan">\*freuþan- [st.] "to froth" (v)</a>
W: \
-- OE _ā-frēoðan_ \
REF: [EDPG]\: 155

### <a id="frija">\*fri(j)a- "free" (adj)</a>
E: \
-- Go. _freis_ \
W: \
-- OE _frēo_, _frī_, E _free_ \
-- OFri. _frī_ \
-- OS _frī_ \
-- Du. _vrij_ \
-- OHG _frī_, G _frei_ \
REF: [EDPG]\: 155

### <a id="frijōn">\*fri(j)ōn- [wk.] "to love" (v)</a>
E: \
-- Go. _frijon_ "to love, like" \
N: \
-- ON _frjá_ \
-- Far. _fríggja_ "to propose, court" \
W: \
-- OE _frēogan_, _frīgan_ "to love, marry" \
-- OS _frion_ "id." \
-- Du. _vrijen_ [st.] "to make love" \
REF: [EDPG]\: 155

### <a id="frijōnd">\*fri(j)ōnd- [m.] "friend" (n)</a>
E: \
-- Go. _frijonds_ \
N: \
-- ON _frændi_ "friend; relative" \
-- Far. _frændi_ "relative" \
W: \
-- OE _frēond_ "friend, loved one, relative", E _friend_ \
-- 0Fri. _friūnd_, _friōnd_ "id." \
-- ODu. _friunt_ "id.", Du. _vriend_ [c.] "id." \
-- OHG _friunt_ "friend, loved one", G _Freund_ "friend" \
REF: [EDPG]\: 155

### <a id="friþu">\*friþu- [m.] "friendship, peace" (n)</a>
N: \
-- ON _friðr_ "peace; security; friendship" \
-- Far. _friður_ "peace; tranquility" \
-- Elfd. _frið_ "id." \
W: \
-- OE _freoðo_ "id." \
-- OFri. _fretho_ "id." \
-- OS _frithu_, _frethu_ "peace; security" \
-- Du. _vrede_ [c.] "peace" \
-- OHG _fridu_ "peace; protection, security", G _Friede_ "peace" \
REF: [EDPG]\: 156

### <a id="frōda">\*frōda- "wise, experienced" (adj)</a>
E: \
-- Go. _froþs_ \
N: \
-- ON _fróðr_ \
W: \
-- OE _frōd_ \
-- OFri. _frōd_ \
-- OS _frōd_ \
-- OHG _fruot_, MHG _vruot_ \
REF: [EDPG]\: 156

### <a id="frōi">\*frōi "early" (adv)</a>
W: \
-- MLG _vrō_ \
-- MDu. _vroech_, Du. _vroeg_ \
-- OHG _fruo_, MHG _vruo_, G _fruh_ \
REF: [EDPG]\: 156

### <a id="frugan">\*frug/kkan- [m.] "frog" (n)</a>
W: \
-- OE _frogga_, ME _frogge_, _frock_, _froke_, E _frog_ \
REF: [EDPG]\: 156

### <a id="fruman">\*fruman- "former, first" (num)</a>
E: \
-- Go. _fruma_ \
N: \
-- ON _frum_- "first, oldest" (pref) \
W: \
-- OE _forma_, E _former_ \
-- OFri. _forma_ \
-- OS _formo_ \
REF: [EDPG]\: 157

### <a id="fruska">\*fruska- [m.] "frog" (n)</a>
N: \
-- ON _froskr_ \
W: \
-- OE _forsc_, _frox_ \
-- OHG _frosc_, G _Frosch_ \
REF: [EDPG]\: 157

### <a id="frusta">\*frusta- [m./n.] "frost" (n)</a>
N: \
-- ON _frost_ [n.] \
W: \
-- OE _forst_ [m.], E _frost_ \
-- OFri. _forst_ [m.] \
-- OS _frost_ [m.] \
-- Du. _vorst_ [c.] \
-- OHG _frost_ [m.], G _Frost_ [m.] \
REF: [EDPG]\: 157

### <a id="fūan">\*fūan- [m.] "mold" (n)</a>
N: \
-- ON _fúi_ "rottenness" \
-- Far. _fúgvi_ "id." \
REF: [EDPG]\: 157

### <a id="fugla">\*fugla- [m.] "bird" (n)</a>
E: \
-- Go. _fugls_ \
N: \
-- ON _fogl_, _fugl_ \
-- Far. _fuglur_ \
-- Elfd. _fugel_ \
W: \
-- OE _fugol_, E _fowl_ \
-- OFri. _fugel_ \
-- OS _fugal_ \
-- Du. _vogel_ [c.] \
-- OHG _fogal_, _fugal_, G _Vogel_ \
REF: [EDPG]\: 157

### <a id="fuhōn">\*fuhōn- [f.] "she-fox" (n)</a>
E: \
-- Go. _fauho_ \
N: \
-- ON _fóa_, _fúa_ \
W: \
-- MLG _vō_ \
-- OHG _voha_, MHG _vohe_ \
REF: [EDPG]\: 157

### <a id="fuhsa">\*fuhsa- [m.] "fox" (n)</a>
N: \
-- ON _fox_ [n.] "deceit" \
-- Icel. _fox_ [n.] \
W: \
-- OE _fox_, E _fox_ \
-- WFri. _foks_, EFri. _foaks_ \
-- MLG _vos(se)_ \
-- MDu. _vos_, _vosse_ [f.], Du. _vos_ [c.] \
-- OHG _fuhs_, G _Fuchs_ \
REF: [EDPG]\: 157

### <a id="fukkōn">\*fuk(k)ōn- [wk.] "to blow" (v)</a>
N: \
-- Icel. _fokka_ "to mess around, rush" \
W: \
-- MLG _vucken_ "to rush, scud" \
-- MDu. _vocken_ "to blow" \
-- MHG _fochen_ "to blow" \
REF: [EDPG]\: 158

### <a id="fūla">\*fūla- "filthy, foul" (adj)</a>
E: \
-- Go. _fuls_ "foul, putrid" \
N: \
-- ON _fúl_ "foul, stinking; mean" \
W: \
-- OE _fūl_ "foul, nasty", E _foul_ \
-- OFri. _fūl_ "foul; vile" \
-- OS _fulitha_ [f.] "filth", MLG _fūl_ "foul, bad" \
-- Du. _vuil_ "dirty" \
-- OHG _fūl_ "rotten, decayed", G _faul_ "putrid; lazy" \
REF: [EDPG]\: 158

### <a id="fulan">\*fulan- [m.] "foal" (n)</a>
E: \
-- Go. _fula_ \
N: \
-- ON _foli_ \
-- Elfd. _fuoli_ \
W: \
-- OE _fola_, E _fole_ \
-- MLG _vole_ \
-- OHG _folo_, G _Fohlen_ \
REF: [EDPG]\: 158

### <a id="fuldō">\*fuldō- [f.] "earth, ground, field, the world" (n)</a>
N: \
-- ON _fold_ \
-- Far. poet. _fold_ \
W: \
-- OE _folde_ \
-- OS _folda_ \
REF: [EDPG]\: 159

### <a id="fulgēn">\*fulgēn- [wk.] "to follow, accompany" (v)</a>
N: \
-- ON _fylgja_ \
-- Far. _fylgja_ \
-- Elfd. _fya_ \
W: \
-- OE _fylgan_, _folgian_, E _to follow_ \
-- OS _folgon_ \
-- Du. _volgen_ \
-- OHG _folgēn_, G _folgen_ \
N: \
-- ON _fylgja_ and OE _fylg(e)an_ continue a formation \*fulgjan- that can be reconciled with OE _fe(o)lan_ [st.] "to proceed, go, reach" < \*felhan- < \*pelk-e-. Extra-Gm. comparanda are unreliable: Bret. _heul_ "to follow" probably continues \*selgʰ- rather than \*pelgʰ- and the connection with CS _plъzati_, Ru. _pólzat'_, SCr. _pȕzati_ "to crawl" must be abandoned in view of the accent pointing to \*plǵ-eh₂- rather than \*plǵʰ-eh₁- \
REF: [EDPG]\: 159

### <a id="fulla-1">\*fulla- "full" (adj)</a>
E: \
-- Go. _fulls_ \
N: \
-- ON _fullr_ \
-- Far. _fullr_ \
-- Elfd. _full_ \
W: \
-- OE _full_, E _full_ \
-- OS _full_ \
-- Du. _vol_ \
-- OHG _fol_, G _voll_ \
REF: [EDPG]\: 159

### <a id="fulla-2">\*fulla- [n.] "cup" (n)</a>
N: \
-- ON _full_ "goblet" \
W: \
-- OE _full_ "cup" \
REF: [EDPG]\: 159

### <a id="fulmō">\*fulmō- [f.] "palm of the hand" (n)</a>
W: \
-- ME _folm_ \
-- OS _folmos_ [f.pl.] "both hands" \
-- OHG _folma_ "palm" \
REF: [EDPG]\: 159

### <a id="fundōn">\*fundōn- [wk.] "to try to find(?)" (v)</a>
W: \
-- OE _fundian_ "to try to find, to strive, intend" \
-- OS _fundon_ "to strive, proceed" \
REF: [EDPG]\: 159

### <a id="funhsti">\*funhsti- [f.] "fist" (n)</a>
W: \
-- OE _fȳst_, E _fist_ \
-- MDu. _vuust_, Du. _vuist_ [c.] \
-- OHG _fūst_, G _Faust_ \
REF: [EDPG]\: 160

### <a id="funhtu">\*funhtu- "humid, moist" (adj)</a>
W: \
-- OE _fūht_ \
-- OS _fūht_ \
-- MDu. _vocht(e)_, _vucht(e)_, Du. _vochtig_ \
-- OHG _fūht_, _fūhti_, G _feucht_ \
REF: [EDPG]\: 160

### <a id="funkōn">\*funkōn- [wk.] "to sparkle" (v)</a>
W: \
-- MDu. _vonken_ "to smolder; to sparkle" \
-- MHG _vunken_ "to shimmer, shine, sparkle", _ent-vunken_ "to ignite" \
REF: [EDPG]\: 160

### <a id="furdu">\*furdu- [m.] "ford" (n)</a>
W: \
-- OE _ford_ \
-- OS _ford_ \
-- MDu. _vort_, _voorde_ \
REF: [EDPG]\: 160

### <a id="furh">\*furh- [f.] "furrow" (n)</a>
N: \
-- ON _for_ \
W: \
-- OE _furh_, E _furrow_ \
-- OFri. _furch_ \
-- MDu. _vore_, Du. _voor_ [c.] \
-- OHG _furh_, _furuh_, G _Furche_ \
REF: [EDPG]\: 160

### <a id="furhnō">\*furhnō- [f.] "trout" (n)</a>
W: \
-- OE _forn(e)_ \
-- OS _forhna_, _furhnia_ \
-- MDu. _voorne_ "?", Du. _voorn_ [c.] "roach" \
-- OHG _forhana_, MHG _forhen_, _forhel_ \
REF: [EDPG]\: 160

### <a id="furhta">\*furhta- "fearful" (adj)</a>
E: \
-- Go. _faurhts_ \
W: \
-- OS _for(a)ht_ \
-- OHG _foraht_ \
REF: [EDPG]\: 161

### <a id="furhtjan">\*furhtjan- [wk.] "to fear" (v)</a>
E: \
-- Go. _faurhtjan_ \
W: \
-- OE _fyrhtan_, _fryhtan_ "to frighten; tremble with fear", ME _frighten_ "to frighten, become frightened" \
-- OFri. _fruchta_ "id." \
-- OHG _furihtan_, G _fürchten_ \
REF: [EDPG]\: 161

### <a id="furhtō">\*furhtō- [f.] "fright" (n)</a>
W: \
-- OFri. _fruchta_ \
-- OS _forhta_ \
-- OHG _forhta_, MHG _vorhte_, _vorht_ \
REF: [EDPG]\: 161

### <a id="furhwōn">\*furhwōn- [f.] "fir" (n)</a>
N: \
-- ON _fura_ \
W: \
-- OE _furh_ \
-- OHG _voraha_, _vorha_, MHG _vorhe_, G _Forhe_ \
REF: [EDPG]\: 161

### <a id="furi">\*furi "in front of, for" (adv/prep)</a>
E: \
-- Go. _faur_ \
N: \
-- ON _fyr_ (prep) \
W: \
-- OE _for_, _fore_ (prep), E _for_ \
-- OS _for_, _fora_ (prep) \
-- Du. _voor_ "for; before" \
-- OHG _furi_, G _für_ (prep) \
REF: [EDPG]\: 161

### <a id="fūrjan">\*fūrjan- [wk.] "to castrate" (v)</a>
W: \
-- OE _(a-)fȳran_ \
-- Du. _vuren_, _vieren_ \
-- OHG _ar-fūrian_ \
REF: [EDPG]\: 161

### <a id="fursa">\*fursa- [m.] "gush(?)" (n)</a>
N: \
-- ON _fors_, _foss_ [n.] "torrent; waterfall" \
-- Far. _fossur_ "id." \
-- OSw. _fors_ "id." \
-- Elfd. _fuoss_ "id." \
REF: [EDPG]\: 161

### <a id="furskō">\*furskō- [f.] "inquiry" (n)</a>
W: \
-- OHG _forsca_, MHG _vorsche_ \
REF: [EDPG]\: 162

### <a id="furtu">\*furtu- [m.] "fart" (n)</a>
W: \
-- MDu. _vort_ \
-- OHG _furz_, G _Furz_ \
REF: [EDPG]\: 162

### <a id="futōn">\*futōn- [wk.] "to go" (v)</a>
N: \
-- Nw. _fòta_ "to step; to go" \
REF: [EDPG]\: 162

### <a id="fuþi">\*fuþi- [f.] "vagina" (n)</a>
N: \
-- ON _fuð_ \
-- Icel. _fuð_ "vagina; holes in a block of wood (for fishing rods)" \
-- Nw. _fud_ "vagina; anus, backside" \
W: \
-- MHG _vut_ \
REF: [EDPG]\: 162

## G

### <a id="gadōjan">\*gadōjan- [wk.] "to fit (together)" (v)</a>
W: \
-- OFri. _gadia_ "to engage in" \
-- MDu. _gaden_ "to fit, please, mate" \
-- OHG _be-gatōn_ "to gather", MHG _(ge-)gaten_ "id." \
REF: [EDPG]\: 162

### <a id="gadurōjan">\*gadurōjan- [wk.] "to gather" (v)</a>
W: \
-- OE _gædrian_ \
-- OFri. _gaduria_ \
-- MDu. _gaderen_, Du. _ver-gaderen_ \
REF: [EDPG]\: 163

### <a id="gaidwa">\*gaidwa- [n.] "lack" (n)</a>
E: \
-- Go. _gaidw_ \
W: \
-- OE _gād_ \
REF: [EDPG]\: 163

### <a id="gaisjan">\*gaisjan- [wk.] "to terrify" (v)</a>
E: \
-- Go. _us-gaisjan_\* \
REF: [EDPG]\: 163

### <a id="gaista">\*gaista- [m.] "spirit, ghost" (n)</a>
W: \
-- OE _gāst_ "breath, spirit, soul, ghost", E _ghost_ \
-- OFri. _gāst_, _gēst_ "spirit, ghost, demon" \
-- OS _gēst_ "soul, vitality, spirit, demon" \
-- Du. _geest_ "spirit" \
-- OHG _geist_ "id.", G _Geist_ "id." \
REF: [EDPG]\: 163

### <a id="gait">\*gait- [f.] "goat" (n)</a>
E: \
-- Go. _gaits_ \
N: \
-- ON _geit_ \
-- Far. _geit_ \
-- Elfd. _djiet_ \
W: \
-- OE _gāt_, E _goat_ \
-- OS _gēt_ \
-- Du. _geit_ [c.] \
-- OHG _geiz_ \
REF: [EDPG]\: 163

### <a id="gaiza">\*gaiza- [m.] "spear, tip" (n)</a>
N: \
-- ON _geirr_ \
-- Nw. _geir_ "leister, eel spear; small tip; ness, headland; small mackerel" \
W: \
-- OE _gār_ "id." \
-- OFri. _gēr_- \
-- OS _gēr_ "id." \
-- MDu. _geer_ "id." \
-- OHG _gēr_ "id.", MHG _gēr_ "id." \
REF: [EDPG]\: 164

### <a id="galan">\*galan- [st.] "to shout, sing, chant" (v)</a>
N: \
-- ON _gala_ "to crow; to sing, chant" \
-- Far. _gala_ "to crow; to call out" \
-- Elfd. _gålå_ "to shout, call" \
W: \
-- OE _galan_ "to sing, enchant, call" \
-- MDu. _galen_ [st./wk.] "to be noisy" \
-- OHG _galan_ "to sing, enchant" \
REF: [EDPG]\: 164

### <a id="galdja">\*gald(j)a- "barren, not (yet) pregnant" (adj)</a>
N: \
-- ON _geldr_ (also "yielding no milk") \
-- OSw. _galder_ \
W: \
-- OE _gielde_ \
-- MDu. _gelde_, _gelt_ \
-- OHG _galt_, MHG _galt_, G _gelt_ \
REF: [EDPG]\: 164

### <a id="galgan">\*galgan- [m.] "stake?" (n)</a>
E: \
-- Go. _galga_ "stake, pole for crucifixion, gallows" \
N: \
-- ON _galgi_ "gallows" \
W: \
-- OE _gealga_ "id.", E _gallows_ \
-- OFri. _galga_ "id." \
-- Du. _galg_ [c.] "id." \
-- OHG _galgo_ "gallows, cross", G _Galgen_ "gallows; boom" \
REF: [EDPG]\: 165

### <a id="gallan">\*galla/ōn- [m./f.] "bile" (n)</a>
N: \
-- ON _gall_ [n.] \
-- Far. _gall_ [n.] (< \*galla-) \
-- Elfd. _golle_ [m.] \
W: \
-- OE _gealla_ [m.], E _gall_ \
-- OS _galla_ [f.] \
-- Du. _gal_ [c.] \
-- OHG _galla_ [f.], MHG _galle_ [f.], G _Galle_ [f.] \
REF: [EDPG]\: 165

### <a id="galra">\*galra- [m.] "swelling(?)" (n)</a>
N: \
-- Nw. _galder_ [m./n.] "swelling in the foot of horses" \
REF: [EDPG]\: 165

### <a id="galtan">\*galtan- [m.] "(castrated) boar" (n)</a>
N: \
-- ON _galti_, _gǫltr_ \
REF: [EDPG]\: 165

### <a id="gamala">\*gamala- "old" (adj)</a>
N: \
-- ON _gamall_ \
W: \
-- OE _gamol_, _gomel_ \
-- MDu. _gamel_, Du. _gammel_ "rickety" \
REF: [EDPG]\: 166

### <a id="gamman">\*gamman- [m.] "stall(?); hut(?)" (n)</a>
N: \
-- ON _gammi_ "Sarni hut; earthen hut" \
-- Elfd. _gamme_ "porch" \
-- Da. _gamme_ [c.] "animal stall; earthen hut" \
REF: [EDPG]\: 166

### <a id="ganazan">\*ganazan- [m.] "gander" (n)</a>
W: \
-- OE _gandra_, E _gander_ \
-- MLG _ganre_ \
REF: [EDPG]\: 166

### <a id="ganēn">\*ganēn- [wk.] "to open" (v)</a>
N: \
-- ON _gana_ "to gape; gaze, stare" \
REF: [EDPG]\: 166

### <a id="gangan">\*gangan- ~ \*gungan- [st.] "to go" (v)</a>
E: \
-- Go. _gaggan_ \
N: \
-- ON _ganga_ \
W: \
-- OE _gangan_, _gongan_ \
-- OFri. _gunga_ \
-- OS _gangan_ \
-- OHG _gangan_, MHG _gangen_ \
REF: [EDPG]\: 166

### <a id="gangi">\*gangi- "passable" (adj)</a>
N: \
-- ON _gengr_ "able to walk" \
W: \
-- OE _genge_ "effective" \
-- OFri. _genze_ "current, valid" \
-- MDu. _genge_ "current, valid" \
-- OHG _gengi_ "current", MHG _genge_ "id.", G _gängig_ "current, usual" \
REF: [EDPG]\: 167

### <a id="ganhēn">\*ganhēn- [wk.] "to inspect; to take into account(?)" (v)</a>
N: \
-- ON _gá_ "to heed" \
-- Icel. _gá_ "to inspect" \
-- Far. _gáa_ "to notice, become aware of" \
-- Elfd. _gą̊_ "to notice; to observe" \
REF: [EDPG]\: 167

### <a id="ganhti">\*ganhti- [f.] "doorway" (n)</a>
E: \
-- Go. _fram-gahts_ "progress" \
N: \
-- ON _gátt_ "rabbet of a doorpost; doorway" \
-- Far. _gátt_ "threshold; doorway" \
-- Elfd. _gą̊tt_ "doorpost" \
REF: [EDPG]\: 167

### <a id="ganhu">\*ganhu- "fast" (adj)</a>
W: \
-- MDu. _gā_, _gauw_ "fast, quick, rash", Du. _gauw_ "soon" (adv) \
-- OHG _gāhi_ "fast, sudden", G _jäh_ "sudden, precipitous" \
REF: [EDPG]\: 167

### <a id="gans">\*gans- [f.] "goose" (n)</a>
N: \
-- ON _gás_, pl. _gæss_ \
-- Far. _gás_, pl. _gæs_ \
-- Elfd. _gą̊s_, pl. _gęser_ \
W: \
-- OE _gōs_, E _goose_ \
-- WFri. _goes_ [c.], EFri. _Gous_ \
-- Du. _gans_ [c.] \
-- OHG _gans_, G _Gans_ \
REF: [EDPG]\: 168

### <a id="ganta">\*ganta- "whole, entire" (adj)</a>
W: \
-- OHG _ganz_, G _ganz_ \
REF: [EDPG]\: 168

### <a id="ganutan">\*ganuta(n)- [m.] "gander" (n)</a>
W: \
-- OE _ganot_ \
-- MDu. _gent_, Du. _jan-van-gent_ "gannet" \
-- OHG _ganzo_, _ganazzo_, MHG _ganze_ \
REF: [EDPG]\: 169

### <a id="gappōn">\*gapp/bōn- [wk.] "to joke" (v)</a>
N: \
-- ON _gabba_ "to mock, make game of one" \
W: \
-- OE _gabbian_ "to mock; to chat" \
-- OFri. _gabbia_ "to denounce, accuse, prosecute" \
-- MLG _gabben_, _gabberen_ "to joke, monkey about" \
-- MDu. _gabben_, Du. _ginne-gappen_ \
REF: [EDPG]\: 169

### <a id="garda">\*garda- [m.] "courtyard" (n)</a>
E: \
-- Go. _gards_ "house, household enclosure, courtyard" \
N: \
-- ON _garðr_ "fence, wall; yard; courtyard, court" \
-- Far. _garður_ "fence; enclosure; farm; ring; gust of wind creating ripples in the water" \
W: \
-- OE _geard_ "id." \
-- OS _gard_ "field, ground, house" \
-- OHG _gart_ "enclosure" \
REF: [EDPG]\: 169

### <a id="garnō">\*garnō- [f.] "intestines" (n)</a>
N: \
-- ON _gǫrn_ \
REF: [EDPG]\: 169

### <a id="garpōn">\*garp/bōn- / \*gurp/bōn- [wk.] "to gobble; to belch" (v)</a>
N: \
-- Far. _gorpa_ "to cough violently; to vomit" \
-- Nw. _garpa_, _garva_, _gurpa_ "to talk loudly; belch; gormandize" \
REF: [EDPG]\: 169

### <a id="garwa">\*garwa- "ready, finished" (adj)</a>
N: \
-- ON _gǫrr_ \
W: \
-- OE _gearo_ \
-- OS _garu_ \
-- Du. _gaar_ \
-- OHG _garo_, G _gar_ \
REF: [EDPG]\: 170

### <a id="garwjan">\*garwjan- [wk.] "to prepare, make ready" (v)</a>
N: \
-- ON _gør(v)a_, _gera_ "to prepare; do" \
-- Far. _gera_ "to do" \
-- Elfd. _djärå_ "id." \
W: \
-- OE _gierwan_ "to prepare, put on, clothe, adorn", E _to gear_ \
-- OS _garwian_, _gerwian_ "to provide, prepare, dress" \
-- OHG _garawen_ "to prepare, equip, decorate", G _gerben_ "to tan" \
REF: [EDPG]\: 170

### <a id="gasti">\*gasti- [m.] "guest" (n)</a>
E: \
-- Go. _gasts_ \
N: \
-- ON _gestr_ \
-- Far. _gestur_ \
-- Elfd. _djäst_ \
W: \
-- OE _giest_ \
-- OFri. _jest_ \
-- OS _gast_ \
-- Du. _gast_ [c.] \
-- OHG _gast_, G _Gast_ \
REF: [EDPG]\: 170

### <a id="gata">\*gata- [n.] "hole" (n)</a>
N: \
-- ON _gat_ \
W: \
-- OE _geat_ "gate", E _gate_ \
-- OFri. _jet_ \
-- OS _gat_ \
-- Du. _gat_ \
REF: [EDPG]\: 170

### <a id="gatwōn">\*gatwōn- [f.] "road, alley" (n)</a>
E: \
-- Go. _gatwo_ "street, main road" \
N: \
-- ON _gata_ "path, way, road" \
W: \
-- MLG _gate_ "alley, street" \
-- OHG _gazza_ "id.", G _Gasse_ "id." \
REF: [EDPG]\: 170

### <a id="gaþma">\*gaþma- [m.] "cabin, room" (n)</a>
W: \
-- MDu. _gadem_ [m./n.] \
-- OHG _gadum_, _gadem_ [n.], MHG _gadem_, _gaden_ [n.] "cabin, barn" \
REF: [EDPG]\: 171

### <a id="gauja">\*gauja- "district, region" (adj)</a>
E: \
-- Go. _gawi_ [n.] \
W: \
-- OE -_gē_ [n.] \
-- OFri. _gā_, _gē_ [n.] "village, parish" \
-- MLG _gō_ [n.] "county" \
-- MDu. _gou(w)_, _gooy_ [n.] "id." \
-- MHG _gou_, gen. _gouwes_ [n.] "id.", G _Gau_ [n.] "id." \
REF: [EDPG]\: 171

### <a id="gaujan">\*gaujan- [wk.] "to throw; to spawn" (v)</a>
W: \
-- MDu. _gōyen_, Du. _gooien_ "to throw" \
REF: [EDPG]\: 171

### <a id="gauka">\*gauka- [m.] "cuckoo" (n)</a>
N: \
-- ON _gaukr_ \
W: \
-- OE _gēac_ \
-- MDu. _gooc_ "cuckoo; fool" \
-- OHG _gouh_ "id.", MHG _gouch_ "id." \
REF: [EDPG]\: 171

### <a id="gauma">\*gauma- [m.] "heed, attention" (n)</a>
N: \
-- ON _gaumr_ \
-- Far. _geymur_, _geymi_ \
W: \
-- MDu. _goom_ \
REF: [EDPG]\: 171

### <a id="gaupnō">\*gaupnō- [f.] "hollow of the hand" (n)</a>
N: \
-- ON _gaupn_ "both hands held together" \
W: \
-- OHG _gaufana_, MHG _goufe_ \
REF: [EDPG]\: 172

### <a id="gaupōn">\*gaupōn- [f.] "lynx" (n)</a>
N: \
-- ON _gaupa_ \
-- Far. _geypa_ \
-- Nw. _gaupe_ \
REF: [EDPG]\: 172

### <a id="gaura">\*gaura- "sad" (adj)</a>
E: \
-- Go. _gaurs_ "grieved, pained, sad" \
W: \
-- OHG _gōrag_ "poor, little, meager" (< \*gauraga-) \
REF: [EDPG]\: 172

### <a id="gazda">\*gazda- [m.] "prickle" (n)</a>
E: \
-- Go. _gazds_ "sting, goad" \
N: \
-- ON _gaddr_ "goad, spike" \
-- Icel. _gaddr_ "goad, spike" \
W: \
-- OS _gard_ "rod" \
-- OHG _gart_, MHG _gart_ \
REF: [EDPG]\: 172

### <a id="geban">\*geban- [st.] "to give" (v)</a>
E: \
-- Go. _giban_ \
N: \
-- ON _gefa_ \
-- Far. _geva_ \
-- Elfd. _djävå_ \
W: \
-- OE _giefan_, E _to give_ \
-- OFri. _jeva_, _jewa_, _jowa_, _jouwa_ \
-- OS _geƀan_ \
-- Du. _geven_ \
-- OHG _geban_, G _geben_ \
REF: [EDPG]\: 172

### <a id="gēbi">\*gēbi- "having been given" (adj)</a>
N: \
-- ON _gæfr_ "quiet, meek" \
-- Far. _gævur_ "doughty, excellent, honest" \
W: \
-- OFri. _jēve_, _gēve_ "usual(?)" \
-- MLG _gēve_ "pleasant" \
-- MHG _gæbe_ "acceptable, enjoyable, convenient" \
REF: [EDPG]\: 173

### <a id="geblan">\*gebla(n)- ~ \*gabla- [m.] "top" (n)</a>
E: \
-- Go. _gibla_ "pinnacle" \
W: \
-- OHG _gebal_ "id.", MHG _gebel_ "id." \
REF: [EDPG]\: 173

### <a id="gebō">\*gebō- [f.] "gift, present" (n)</a>
E: \
-- Go. _giba_ \
N: \
-- ON _gjǫf_ \
W: \
-- OE _giefu_ \
-- OFri. _jeve_, _jowe_, _jouwe_ \
-- OS _geƀa_, _giƀa_ \
-- OHG _geba_, MHG _gebe_ \
REF: [EDPG]\: 173

### <a id="gēbōn">\*gēbōn- [f.] "gift; giving" (n)</a>
N: \
-- ON _gáfa_ \
-- Far. _gáva_ \
-- Elfd. _gåva_ \
W: \
-- MDu. _gave_, Du. _gave_ [c.] \
-- MHG _gābe_ \
REF: [EDPG]\: 173

### <a id="gela">\*gela- [m.] "lust" (n)</a>
N: \
-- OSw. _giæl-skaper_ "unchastity" \
REF: [EDPG]\: 173

### <a id="geldan">\*geldan- [st.] "to pay, be worth something" (v)</a>
E: \
-- Go. _fra-gildan_ "to repay, compensate" \
N: \
-- ON _gjalda_ "to repay, pay for" \
-- Far. _gjalda_ "to pay, suffer" \
W: \
-- OE _gieldan_ "to yield, pay, repay", E _to yield_ \
-- OS _geldan_ "to pay, reward" \
-- Du. _gelden_ "to be true, valid" \
-- OHG _geltan_ "to compensate, pay, repent", G _gelten_ "to apply, count" \
REF: [EDPG]\: 173

### <a id="gellan">\*gellan- [st.] "to sound, yell" (v)</a>
N: \
-- ON _gjalla_ "to yell, scream" \
-- Far. _gjalla_ "to sound" \
W: \
-- OE _giellan_ "to yell", E _to yell_ \
-- MDu. _gellen_, _gillen_ [st./wk.] "to yell", Du. _gillen_ [wk.] "id." \
-- OHG _gellan_ "to sound, yell", MHG _gellen_ "to sound, cry", G _gellen_ [wk.] "to yell" \
REF: [EDPG]\: 174

### <a id="gelwa">\*gelwa- ~ \*gulu- "yellow" (adj)</a>
N: \
-- ON _gulr_ \
-- Far. _gulur_ \
-- Elfd. _guol_ \
W: \
-- OE _geolo_, E _yellow_ \
-- OS _gelo_, _gelu_ \
-- OHG _gelo_, MHG _gel_, G _gelb_ \
REF: [EDPG]\: 174

### <a id="gēn">\*gēn- [suppl.] "to go" (v)</a>
N: \
-- Far. poet. _gá_ [suppl.?] \
-- Nw. _gå_ \
-- OSw. _gā_ \
-- Elfd. _go_ \
-- EDa. _gā_, Da. _gå_ \
W: \
-- OE _gān_, E _to go_ \
-- OFri. _gān_, _gēn_ \
-- OS _gān_ \
-- MDu. _gaen_, Du. _gaan_ \
-- OHG _gān_, _gēn_, G _gehen_ \
REF: [EDPG]\: 174

### <a id="gerdan">\*gerdan- [st.] "to gird" (v)</a>
E: \
-- Go. -_gairdan_\* \
REF: [EDPG]\: 175

### <a id="gerna">\*gerna- "desirous" (adj)</a>
E: \
-- Go. -_gairns_ \
N: \
-- ON _gjarn_ \
-- Far. -_gjarnur_ \
-- Elfd. _djienna_ (adv) \
W: \
-- OE _georn_ \
-- OFri. _gerne_, _jerne_ (adv) \
-- OS _gern_ \
-- Du. _gaarne_ (adv) \
-- OHG _gern_, G _gern(e)_ (adv) \
REF: [EDPG]\: 175

### <a id="gerstō">\*gerstō- [f.] "barley" (n)</a>
W: \
-- OS _gersta_ \
-- Du. _gerst_ [c.] \
-- OHG _gersta_, G _Gerste_ \
REF: [EDPG]\: 175

### <a id="gestra">\*gestra- "the previous or next day" (adv)</a>
E: \
-- Go. _gistra-dagis_ "tomorrow" \
N: \
-- ON _í gær_, _í gjár_ "yesterday; tomorrow" \
-- Icel. _í gær_, _gjár_ "yesterday" \
-- Far. _í gjár_ "id." \
-- OSw. _ī gār_ "id." \
W: \
-- OE _geostran-dæg_, _giestran-dæg_ "id.", E _yesterday_ \
-- MDu. _gist(e)ren_ "id.", Du. _gisteren_ "id." \
-- OHG _gestaron_ "id.", G _gestern_ "id." \
REF: [EDPG]\: 175

### <a id="getan">\*getan- [st.] "to find (a way), to be able" (v)</a>
E: \
-- Go. _bi-gitan_ "to find, discover" \
N: \
-- ON _geta_ "to get; to be able to" \
-- Elfd. _djieta_ [wk.] "to try, guess" \
W: \
-- OE _bi-gietan_ "to beget", E _to get_, _beget_, _forget_ \
-- OFri. _ur-ieta_ [wk.] "to forget" \
-- OS _bi-getan_ "to find" \
-- Du. _ver-geten_ "to forget" \
-- OHG _bi-gezzan_ "to obtain", _fir-gezzan_ "to forget", G _vergessen_ "to forget" \
REF: [EDPG]\: 176

### <a id="geuban">\*geuban- ~ \*gūban- [st.] "to blow, rush" (v)</a>
N: \
-- Nw. _gjuva_, _guva_ \
REF: [EDPG]\: 177

### <a id="geupan">\*geupan- [st.] "to be hollow" (v)</a>
W: \
-- OE _gēopan_ "to take up, swallow" \
REF: [EDPG]\: 177

### <a id="geutan">\*geutan- [st.] "to pour" (v)</a>
E: \
-- Go. _giutan_ \
N: \
-- ON _gjóta_ "to drop, throw, cast young" \
-- Nw. _gyte_, _gjote_ "to pour, run, spawn" \
W: \
-- OE _gēotan_ "to pout (out)" \
-- OS _giotan_ "id." \
-- Du. _gieten_ "id." \
-- OHG _giozan_ "to pour, mix, melt", G _gießen_ "to pour" \
REF: [EDPG]\: 177

### <a id="gīda">\*gīda- [m.] "desire, greed" (n)</a>
W: \
-- MHG _gīt_ "greed, stinginess" \
REF: [EDPG]\: 177

### <a id="gīgēn">\*gīgēn- [wk.] "to desire" (v)</a>
E: \
-- Go. _ga-geigan_ \
N: \
-- Far. _gíggja_ "to praise; to envy" \
REF: [EDPG]\: 177

### <a id="gimban">\*gimban- [st.] "to hop" (v)</a>
W: \
-- MHG _gimpen_ [wk.] \
REF: [EDPG]\: 178

### <a id="gīnan">\*gīnan- [st.] "to gape, yawn" (v)</a>
N: \
-- ON _gína_ \
W: \
-- OE _tō-gīnan_ \
REF: [EDPG]\: 178

### <a id="ginēn">\*ginēn- [st.] "to stand open(?)" (v)</a>
N: \
-- Far. _gina_ [wk.] "to straddle, walk with long strides" \
W: \
-- OHG _ginēn_ [wk.] "to yawn", MHG _ginen_, _genen_ [wk.] "id.", G _gähnen_ [wk.] "id." \
REF: [EDPG]\: 178

### <a id="ginnan">\*ginnan- [st.] "to begin" (v)</a>
E: \
-- Go. _du-ginnan_ \
W: \
-- OE _be-ginnan_, E _to begin_ \
-- OS _be-ginnan_ \
-- Du. _be-ginnen_ \
-- OHG _bi-ginnan_, G _be-ginnen_ \
REF: [EDPG]\: 178

### <a id="gintan">\*gintan- [st?] "to be startled(?)" (v)</a>
W: \
-- G Swi. _er-ginzen_ [wk.] "to cringe, shiver" \
REF: [EDPG]\: 178

### <a id="gisla-1">\*gīsla- [m.] "shaft(?)" (n)</a>
N: \
-- ON _gísl_ "staff, ski stick" \
-- Icel. _gísli_ "beam, ray" \
W: \
-- OHG Lomb. _gīsil_ "arrow" \
REF: [EDPG]\: 178

### <a id="gisla-2">\*gīsla- [m.] "hostage" (n)</a>
N: \
-- ON _gísl_ \
W: \
-- OE _gīsel_ \
-- OFri. _jēsel_- \
-- OS _gīsal_ \
-- Du. _gijzelaar_ \
-- OHG _gīsal_, G _Geisel_ \
REF: [EDPG]\: 179

### <a id="giwēn">\*giwēn- [wk.] "to desire, long for" (v)</a>
W: \
-- OE _giwian_, _giowian_ \
-- OHG _ana-giwēn_ \
REF: [EDPG]\: 179

### <a id="giwōjan">\*giwōjan- [wk.] "to yawn" (v)</a>
W: \
-- MDu. _gewen_, Du. _geeuwen_ \
-- OHG _gewōn_, MHG _giwen_, _gewen_ \
REF: [EDPG]\: 179

### <a id="glada">\*glada- "bright, happy" (adj)</a>
N: \
-- ON _glaðr_ "glad; bright" \
-- Far. _glaður_ "happy, glad" \
-- Elfd. _glað_ "id." \
W: \
-- OE _glæd_ "id.", E _glad_ \
-- OFri. _gled_ "smooth, slippery" \
-- Du. _glad_ "id." \
-- OHG _glat_ "bright, light; smooth", G _glatt_ "smooth, slippery" \
REF: [EDPG]\: 179

### <a id="glantjan">\*glantjan- [wk.] "to shine, be bright" (v)</a>
N: \
-- Icel. _gletta-st_ "to make fun (of)" \
-- Far. _gletta_ "to grin, roar with laughter" \
-- Nw. _gletta_ "to peep" \
-- Sw. dial. _glänta_ "to clear up, shine through; to be ajar" \
REF: [EDPG]\: 180

### <a id="glappōn">\*glapp/bōn- [wk.] "to slip off" (v)</a>
N: \
-- Icel. _af-glapa_ "to disturb, confuse", _glappa-st_ refl. "to fail" \
-- Sw. dial. _glapa_ "to be open" \
REF: [EDPG]\: 180

### <a id="glasa">\*glasa- ~ \*glaza- [n.] "glass" (n)</a>
N: \
-- Icel. _gler_ \
-- Far. _glas_, _gler_ "(glassy) ice" \
-- OSw. _glar_ \
-- Elfd. _glas_ \
W: \
-- OE _glæs_, E _glass_ \
-- OHG _glas_, G _Glas_ \
REF: [EDPG]\: 180

### <a id="glauma">\*glauma- [m.] "jubilation" (n)</a>
N: \
-- ON _glaumr_ \
W: \
-- OE _glēam_ "joyous noise, jubilation" \
REF: [EDPG]\: 180

### <a id="glawwa">\*glawwa- "sharp-sighted" (adj)</a>
E: \
-- Go. _glaggws_\* "cautious" \
N: \
-- ON _gløggr_ "clear-sighted; clear; stingy" \
W: \
-- OE _glēaw_ "clear-sighted, wise" \
-- OS _glau_ "wise" \
-- OHG _glau_ "cautious, wise" \
REF: [EDPG]\: 180

### <a id="gleppan">\*gleppan- [st.] "to slip off" (v)</a>
N: \
-- Far. _gleppa_ "to slip out of position; to fail, let down" \
-- Nw. _gleppa_ "to slip; to turn out differently" \
REF: [EDPG]\: 180

### <a id="gleupan">\*gleupan- ~ \*glūpan- [st.] "to gape" (v)</a>
N: \
-- Nw. _gljupa_, _glupa_ "to gape; to snatch with the mouth; to devour" \
-- Sw. dial. _gliopa_, _glupa_ "to devour" \
-- Elfd. _glaupa_ "id." \
W: \
-- E dial. _gloup_ "to swallow" \
-- OFri. _glūpa_ "to sneak"(?) \
-- MDu. _glupen_ [wk.] "to squint. lurk", Du. _gluipert_ [c.] "creep" \
REF: [EDPG]\: 180

### <a id="glīdan">\*glīdan- [st.] "to glide" (v)</a>
W: \
-- OE _glīdan_, E _to glide_ \
-- OFri. _glīda_ \
-- OS _glīdan_ \
-- Du. _glijden_ \
-- MHG _glīten_, G _gleiten_ \
REF: [EDPG]\: 181

### <a id="glīman">\*glīman- [st.] "to shine" (v)</a>
N: \
-- Nw. _glima_ \
W: \
-- MHG _glīmen_ \
REF: [EDPG]\: 181

### <a id="glintan">\*glintan- [st.] "to shine, look" (v)</a>
W: \
-- MHG _glinzen_ "to shine, gleam" \
REF: [EDPG]\: 181

### <a id="glītan">\*glītan- [st.] "to shine, sparkle" (v)</a>
W: \
-- OS _glītan_ \
-- OHG _glīzan_, G _gleißen_ \
REF: [EDPG]\: 181

### <a id="glittōn">\*glit(t)ōn- [wk.] "to shine, sparkle" (v)</a>
N: \
-- Icel. _glita_ "to gleam, flash" \
W: \
-- MDu. _glitten_ "to shine, sparkle" \
-- G _glitzen_ "to shine" \
REF: [EDPG]\: 181

### <a id="glittōn">\*glittōn- [wk.] "to glide" (v)</a>
W: \
-- G _glitzen_ \
REF: [EDPG]\: 181

### <a id="glīwa">\*glīwa- [n.] "joy" (n)</a>
N: \
-- ON _glý_ \
W: \
-- OE _glīw_, _glēow_ "jesting; fun; game", E _glee_ \
REF: [EDPG]\: 182

### <a id="glōan">\*glōan- [st.] "to glow" (v)</a>
W: \
-- OE _glōwan_, E _to glow_ \
-- OS _glōian_ [wk.] \
-- Du. _gloeien_ [wk.] \
-- OHG _gluoan_ [wk.], G _glühen_ [wk.] \
REF: [EDPG]\: 182

### <a id="glōdi">\*glōdi- [f.] "glow" (n)</a>
N: \
-- ON _glóð_ "red-hot embers" \
-- Far. _glóð_ "glow; ember" \
-- Elfd. _glyōð_ \
W: \
-- OE _glēd_ "burning coal, embers", E _gleed_ \
-- OFri. _glōd_, _glēd_ \
-- MDu. _gloet_ [m./f.], Du. _gloed_ [c.] \
-- OHG _gluot_ "red-heat", G _Glut_ "glow" \
REF: [EDPG]\: 182

### <a id="glūnēn">\*glūnēn- [wk.] "to peer" (v)</a>
N: \
-- Sw. dial. _gluna_ "to stare, peer" \
REF: [EDPG]\: 182

### <a id="gluntēn">\*gluntēn- [wk.] "to peer" (v)</a>
N: \
-- ON _glotta_ "to sneer, grin, smirk" \
-- Far. _glotta_ "to clear up, (of the sun) to peep through the clouds" \
-- Nw. _glutta_ "to peer, peep" \
REF: [EDPG]\: 182

### <a id="gluppōn">\*glupp/bōn- [wk.] "to gape" (v)</a>
N: \
-- Icel. _glopa_ "to drop" \
-- Far. _gloppa_ "to stand a little open; put ajar" \
-- Nw. _glopa_ "to be open; to gulp" \
-- Elfd. _glupå_ "to gobble, devour" \
REF: [EDPG]\: 182

### <a id="gluttōn">\*glut(t)ōn- [wk.] "to peer" (v)</a>
W: \
-- E _to gloat_ \
-- MHG _glotzen_, _glutzen_ "to watch with widespread eyes, stare" \
REF: [EDPG]\: 183

### <a id="gnagan">\*gnagan- [st.] "to gnaw" (v)</a>
N: \
-- ON _gnaga_ \
W: \
-- OE _gnagan_, E _to gnaw_ \
-- OS _knagan_ \
-- Du. _knagen_ [wk.] \
-- OHG _(g)nagan_, MHG _gnagen_, G _nagen_ [wk.] \
REF: [EDPG]\: 183

### <a id="gnaistan">\*gnaista(n)- [m.] "spark" (n)</a>
N: \
-- ON _gneisti_ "spark" \
W: \
-- OE _gnāst_ "spark" \
-- OHG _gneisto_ (also _gneista_ [f.]) "spark" \
REF: [EDPG]\: 183

### <a id="gnīdan">\*gnīdan- [st.] "to rub" (v)</a>
W: \
-- OE _gnīdan_ \
-- MLG _gnīden_ \
-- OHG _gnītan_, MHG _gnīten_ \
REF: [EDPG]\: 183

### <a id="gnuznōjan">\*gnuznōjan- [wk.] "to be sad" (v)</a>
W: \
-- OE _gnornan_, _gnornian_ "to grieve, mourn, be sad, bewail, lament" \
-- OS _gornon_, _gnornon_, _grornon_ "to lament, complain, be worried" \
REF: [EDPG]\: 184

### <a id="gōda">\*gōda- "suitable, good" (adj)</a>
E: \
-- Go. _goþs_ \
N: \
-- ON _góðr_ "good; friendly" \
-- Far. _góður_ "id." \
-- Elfd. _guoð_ "id." \
W: \
-- OE _gōd_ "id.", E _good_ \
-- OFri. _gōd_ "id." \
-- OS _gōd_ "id." \
-- Du. _goed_ "id." \
-- OHG _guot_ "id.", G _gut_ "id." \
REF: [EDPG]\: 184

### <a id="gōīn">\*gōīn- [f.] "late winter" (n)</a>
N: \
-- ON _gói_, _gœ_ \
-- Icel. _góa_ \
-- Far. _gø_ [n.] \
-- Nw. _gjø_, _go_ \
REF: [EDPG]\: 184

### <a id="gōja">\*gōja/ō- [n./f.] "peritendinitis crepitans" (n)</a>
N: \
-- Far. _gø_ [n.] "synovitis (esp. of the wrist)" \
-- Nw. _gjø_ [f.] "pain in the wrist due to strain" \
REF: [EDPG]\: 185

### <a id="gōljan">\*gōljan- [wk.] "to call(?)" (v)</a>
E: \
-- Go. _goljan_ "to greet" \
N: \
-- ON _gœla_ "to comfort, soothe" \
W: \
-- OFri. _gēla_ "to hunt"(?) \
REF: [EDPG]\: 185

### <a id="gōman">\*gōman- ~ \*gauman- [m.] "gum, palate" (n)</a>
N: \
-- ON _gómi_, _gómr_ \
-- Far. _gómi_ \
-- Elfd. _guom_ \
W: \
-- OE _gōma_ "palate", E _gum_ \
-- OHG _guomo_, _gaumo_ "id.", MHG _guome_, _goume_ "id.", G _Gaumen_ "id." \
REF: [EDPG]\: 185

### <a id="graba">\*graba- [n.] "grave" (n)</a>
W: \
-- OE _græf_, _graf_ "grave, trench" \
-- OFri. _gref_ "grave, ditch(?)" \
-- OS _graf_ \
-- Du. _graf_ \
-- OHG _grab_, G _Grab_ \
REF: [EDPG]\: 185

### <a id="graban">\*graban- [st.] "to dig" (v)</a>
E: \
-- Go. _graban_ \
N: \
-- ON _grafa_ \
-- Far. _grava_ [st./wk.] \
-- Elfd. _gråvå_ \
W: \
-- OE _grafan_ \
-- OFri. _greva_ \
-- OS _graƀan_ \
-- Du. _graven_ \
-- OHG _graban_ "dig, carve", G _graben_ "to dig" \
REF: [EDPG]\: 185

### <a id="graipōn">\*graipōn- [wk.] "to grope" (v)</a>
N: \
-- ON _greipa_ "to grasp" \
W: \
-- OE _grāpian_ "to grope, touch with the hands", E _to grope_ \
-- OHG _greifōn_ "to stroke" \
REF: [EDPG]\: 186

### <a id="gramjan">\*gramjan- [wk.] "to provoke, anger" (v)</a>
E: \
-- Go. _gramjan_ \
N: \
-- ON _gremja_ \
W: \
-- OE _gremian_ \
-- MLG _gremmen_ \
-- OHG _grem(m)en_, G _grämen_ \
REF: [EDPG]\: 186

### <a id="granna">\*granna- "sharp" (adj)</a>
N: \
-- ON _grannr_ "thin, slender" \
-- Icel. _grannur_ "thin, slim; accurate" \
-- Far. _grannur_ "thin, fine" \
-- Elfd. _grann_ "thin, delicate, pretty" \
REF: [EDPG]\: 186

### <a id="granō">\*granō- [f.] "hair of the beard" (n)</a>
N: \
-- ON _grǫn_ "hair on the upper lip; moustache" \
-- Far. _gron_ "nose of cattle, snout; perineum", pl. "nostrils, nose" \
W: \
-- OE _granu_ "moustache" \
-- MDu. _grane_, _graen_ "hair of the beard, awn, fish-bone" \
-- OHG _grana_ "awn, beard, whiskers", G _Granne_ "awn" \
REF: [EDPG]\: 186

### <a id="grappōn">\*grapp/bōn- [wk.] "to grapple" (v)</a>
W: \
-- E _to grab_ \
-- MLG _grabben_ "to snatch" \
-- MDu. _grapen_, _grabben_ "to grab, grapple", Du. _grabbelen_ "to grapple" \
REF: [EDPG]\: 187

### <a id="grasa">\*grasa- [n.] "grass" (n)</a>
E: \
-- Go. _gras_ \
N: \
-- ON _gras_ \
W: \
-- OE _gærs_, _græs_, E _grass_ \
-- OFri. _gers_, _gres_ \
-- OS _gras_ \
-- Du. _gras_ \
-- OHG _gras_, G _Gras_ \
REF: [EDPG]\: 187

### <a id="grauta">\*grauta- "coarse" (adj)</a>
W: \
-- OE _grēat_ "course, large, great", E _great_ \
-- OFri. _grāt_ "tall, big, great" \
-- OS _grōt_ "id." \
-- Du. _groat_ "id." \
-- OHG _grōz_ "id.", G _Groß_ "id." \
REF: [EDPG]\: 187

### <a id="gredu">\*gredu- [m.] "hunger, desire" (n)</a>
E: \
-- Go. _gredus_ \
N: \
-- ON _gráðr_ \
W: \
-- OE _grǣd_\*, E _greed_ \
REF: [EDPG]\: 187

### <a id="grētan">\*grētan- [st.] "to wail" (v)</a>
E: \
-- Go. _gretan_ "to cry" \
N: \
-- ON _gráta_ "id." \
-- Elfd. _gråta_ "id." \
W: \
-- OE _grǣtan_ "id.", ME _grēten_ "to salute", E _to greet_ \
-- OFri. _grēta_ [wk.] "to greet; to sue" \
-- OS _grātan_ "to cry" \
-- MHG _grāzen_ [wk.] "id." \
REF: [EDPG]\: 187

### <a id="greujan">\*greujan- [wk.] "to dawn" (v)</a>
N: \
-- ON _grýandi_ [f.] "dawn" \
-- OSw. _grȳ_ "to dawn" \
REF: [EDPG]\: 188

### <a id="greuna">\*greuna- [n.] "gravel" (n)</a>
N: \
-- ON _grjón_ "groats" \
W: \
-- MLG _grēn_ "sea sand" \
-- MHG _grien_ [m./n.] "gravel, sandy riverside" \
REF: [EDPG]\: 188

### <a id="greupan">\*greupan- ~ \*grūpan- [st.] "to cut up, grind" (v)</a>
N: \
-- Nw. _grjupa_, _grupa_ "to grind" \
-- Sw. _grjopa_ "to cut out" \
-- Elfd. _griuopa_ "to grind coarsely" \
REF: [EDPG]\: 188

### <a id="greutan">\*greutan- [st.] "to grind, crush" (v)</a>
W: \
-- OHG _fir-grozzen_ ptc. "ground", MHG _ver-griezen_ "to grind into very small pieces" \
REF: [EDPG]\: 189

### <a id="grēwa">\*grēwa- "grey" (adj)</a>
N: \
-- ON _grár_ \
W: \
-- OE _grǣw_, _grǣg_, E _grey_ \
-- OS _grāu_ \
-- Du. _grauw_ \
-- OHG _grāo_, _grā_, G _grau_ \
REF: [EDPG]\: 189

### <a id="gridi">\*gridi- [f.] "step" (n)</a>
E: \
-- Go. _griþs_\* "standing" \
W: \
-- MHG _grit_ "step" \
REF: [EDPG]\: 189

### <a id="grīman">\*grīma/ōn- [m./f.] "make-up" (n)</a>
N: \
-- ON _gríma_ [f.] "mask, cowl; beak" \
W: \
-- OE _grīma_ [m.] "mask, visor, helmet; spectre", E _grime_ "soot, coal-dust" \
-- OS _grīmo_ [m.] "mask" \
REF: [EDPG]\: 189

### <a id="grimman">\*grimman- [st.] "to rage" (v)</a>
W: \
-- OE _grimman_ "to rage, roar; to rush" \
-- OS _grimman_ "to rage" \
-- MHG _grimmen_ "to rage, yell" \
REF: [EDPG]\: 190

### <a id="grīnan">\*grīnan- [st.] "to flash, snarl" (v)</a>
N: \
-- ON _grína_ "to grimace, grin" \
-- Far. _grína_ "to grin, laugh" \
-- Nw. _grine_ "to grimace, whine" \
-- Da. _grine_ [wk.] "to grimace; to laugh" \
W: \
-- E _to grin_ \
-- MDu. _grinen_ [st./wk.] "to cry; to growl; to grin" \
-- OHG _grīnan_ "to bark, howl, snarl", MHG _grīnen_ "to grin, show teeth", G _greinen_ [wk.] "to whine" \
REF: [EDPG]\: 190

### <a id="grindan">\*grindan- [st.] "to grind" (v)</a>
W: \
-- OE _grindan_, E _to grind_ \
REF: [EDPG]\: 190

### <a id="grindi">\*grindi- [f.] "fence" (n)</a>
N: \
-- ON _grind_ "lattice door, gate, fence" \
-- Icel. _grind_ "trellis, grating, rack, frame" \
-- Far. _grind_ "gate, frame" \
REF: [EDPG]\: 190

### <a id="grīpan">\*grīpan- [st.] "to grasp, seize" (v)</a>
E: \
-- Go. _greipan_ \
N: \
-- ON _grípa_ \
W: \
-- OE _grīpan_ \
-- OFri. _grīpa_ \
-- OS _grīpan_ \
-- Du. _grijpen_ \
-- OHG _grīfan_, G _greifen_ \
REF: [EDPG]\: 190

### <a id="grippōn">\*gripp/bōn- [wk.] "to grasp" (v)</a>
N: \
-- ON _gripla_ "to grope" \
W: \
-- MDu. _grippen_ "to grasp", Du. _gribbelen_ "to pick up" \
-- G Als. _grippen_ "to grab" \
REF: [EDPG]\: 191

### <a id="grīsa">\*grīsa- "grey" (adj)</a>
W: \
-- OE _grīs_ \
-- OS _grīs_ \
-- Du. _grijs_ \
-- OHG _grīs_, G _greis_ "aged" \
REF: [EDPG]\: 191

### <a id="grōan">\*grōan- [st.] "to grow" (v)</a>
N: \
-- ON _gróa_ \
W: \
-- OE _grōwan_, E _to grow_ \
-- OFri. _grōwa_, _grōia_ \
-- MLG _grōien_ [wk.] \
-- MDu. _groeyen_, Du. _groeien_ [wk.] \
-- OHG _gruoan_, MHG _grüen_ \
REF: [EDPG]\: 191

### <a id="grōbō">\*grōbō- [f.] "cavity, pit" (n)</a>
E: \
-- Go. _groba_ "dugout, hole, cave" \
N: \
-- ON _gróf_ "pit" \
-- Far. _grógv_ "depression, hollow" \
W: \
-- ODu. _gruova_ "pit", Du. _groeve_ "pit, ditch, hole" \
-- OHG _gruoba_, G _Grube_ \
REF: [EDPG]\: 191

### <a id="grōni">\*grōni- "green" (adj)</a>
N: \
-- ON _grœnn_ \
-- Far. _grønur_ \
-- Elfd. _gryōn_ \
W: \
-- OE _grēne_ \
-- OFri. _grēne_ \
-- OS _grōni_ \
-- Du. _groen_ \
-- OHG _gruoni_, MHG _grüene_, G _grün_ \
REF: [EDPG]\: 191

### <a id="grōtjan">\*grōtjan- [wk.] "to make shout, greet" (v)</a>
N: \
-- ON _grœta_ "to make cry" \
W: \
-- OE _grētan_ "to greet" \
-- OS _grōtian_ "to address, harass, enrage" \
-- Du. _groeten_ "to greet" \
-- OHG _gruozen_ "to irritate, greet, speak to", MHG _gruozen_, _grüezen_ "to cry; to greet", G _grüßen_ "to greet" \
REF: [EDPG]\: 192

### <a id="grōþi">\*grōþi- [f.] "growing, growth, young vegetation" (n)</a>
N: \
-- ON _gróðr_ [m.] "growth, crop" \
-- Far. _gróður_ [m.] "vegetation" \
W: \
-- OFri. _grēde_ "id." \
-- MDu. _groede_ "id." \
-- MHG _gruot_ "id." \
REF: [EDPG]\: 192

### <a id="gruba">\*gruba- "coarse" (adj)</a>
W: \
-- MDu. _grof_ "large; crude; loud", Du. _grof_ "coarse" \
-- MHG _grop_ "thick, strong, big" \
REF: [EDPG]\: 192

### <a id="grumþu">\*grumþu- ~ \*grundu- [m.] "floor" (n)</a>
E: \
-- Go. _grundu-waddjus_ "ground-wall" \
N: \
-- ON _grunnr_ "bottom" \
-- Far. _gruður_, _grunnur_ "shallow; sea bottom" \
-- Elfd. _grunn_ [n.] "id." \
W: \
-- OE _grund_ "ground, bottom, foundation", E _ground_ \
-- OFri. _grund_ "id." \
-- OS _grund_ "id." \
-- Du. _grond_ [c.] "id." \
-- OHG _grunt_ "id.", G _Grund_, Cimb. _grumf_ "id." \
REF: [EDPG]\: 192

### <a id="gruppōn">\*grupp/bōn- [wk.] "to dig" (v)</a>
N: \
-- Nw. _gropa_ "to dig up; hollow out, groove" \
W: \
-- ME _grubben_ "to dig in earth with a tool; to dig up; to pluck out", _gropen_ "to dig, to make a groove", E _to grub_ \
-- MLG _gropen_ "to hollow out" \
-- MDu. _grobben_ "to scrape" \
-- G Pal. _groppen_ "to scratch" \
REF: [EDPG]\: 192

### <a id="grutjan">\*grutja(n)- [m./n.] "grits" (n)</a>
W: \
-- Du. _grut_ [n.] "small wares; small stuff; young kids" \
-- OHG _gruzi_ [n.] "bran", G Swi. _grütz_ [n.] "seed mixture" \
REF: [EDPG]\: 193

### <a id="gruttōn">\*gruttōn- [wk.] "to cut into pieces" (v)</a>
W: \
-- G Pal. _grotzen_, _grutzen_ "to cut into uneven pieces" \
REF: [EDPG]\: 193

### <a id="gruwwan">\*gruwwa(n)- [n.] "dregs" (n)</a>
N: \
-- Icel. _grugg_ "sediment" \
-- Far. _grugg_ "sediment, dregs; enmity" \
-- Nw. _grugg(e)_ [m./n.] "dregs" \
REF: [EDPG]\: 193

### <a id="gubbōn">\*gub(b)ōn- [wk.] "to steam" (v)</a>
N: \
-- Icel. _gubba_ "to vomit" \
-- Far. _guva_ "to smoke, steam" \
-- Nw. _gova_ "to steam; smell", _gubba_ "to steam, haze" \
REF: [EDPG]\: 193

### <a id="gubēn">\*gubēn- [wk.] "to stoop" (v)</a>
N: \
-- Nw. _guve_ "to cower" \
-- Gutn. _gäua sig_ "to stoop" \
REF: [EDPG]\: 193

### <a id="guda">\*guda- [n.] "god" (n)</a>
E: \
-- Go. _guþ_ [m.] \
N: \
-- ON _guð_ [m./n.] \
-- Elfd. _guð_ [m.] \
W: \
-- OE _god_, E _god_ \
-- OFri. _god_ [m.] \
-- OS _god_ [m.] \
-- Du. _god_ [c.] \
-- OHG _got_ [m.], G _Gott_ [m.] \
REF: [EDPG]\: 193

### <a id="gulba">\*gulba- [n.] "floor" (n)</a>
N: \
-- ON _golf_ \
-- Far. _gólv_ \
-- Elfd. _guov_ \
REF: [EDPG]\: 194

### <a id="guljōn">\*guljōn- [f.] "puddle?" (n)</a>
N: \
-- Sw. dial. _gōlja_ "puddle, hole in the ice" \
W: \
-- MLG _gole_ "marshy place" \
-- MDu. _gole_, _guelle_ "rivulet", Du. _geul_ "rivulet" \
-- MHG _gülie_ w.f. "puddle" \
REF: [EDPG]\: 194

### <a id="gullōn">\*gullōn- [wk.] "to shout, sing" (v)</a>
N: \
-- Nw. _golla_ "to sound; boast" \
REF: [EDPG]\: 194

### <a id="gulþa">\*gulþa- ~ \*gulda- [n.] "gold" (n)</a>
E: \
-- Go. _gulþ_ \
N: \
-- ON _gull_ \
W: \
-- OE _gold_ \
-- OFri. _gold_ \
-- OS _gold_ [m.] \
-- Du. _goud_ \
-- OHG _gold_, MHG _galt_, G _Gold_ \
REF: [EDPG]\: 194

### <a id="guman">\*guman- [m.] "man" (n)</a>
E: \
-- Go. _guma_ \
N: \
-- ON _gumi_ \
W: \
-- OE _guma_ \
-- OS _gumo_ \
-- OHG _gomo_, MHG _gome_ \
REF: [EDPG]\: 195

### <a id="gumban">\*gumban- [m.] "curd(?)" (n)</a>
N: \
-- Nw. _gumbe_, _gumme_ "kind of sweet cheese" \
W: \
-- G Swi. _gümmi_ "curd" (< \*gumbīn-) \
REF: [EDPG]\: 195

### <a id="gumēn">\*gumēn- [wk.] "to pay attention" (v)</a>
N: \
-- Icel. _guma_ \
W: \
-- OE _ofer-gumian_ "to neglect" \
-- OS _far-gumon_ "id." \
REF: [EDPG]\: 195

### <a id="gumpōn">\*gump/bōn- [wk.] "to hop" (v)</a>
N: \
-- ON _goppa_ "to jump" \
-- Sw. _gumpa_ "to jolt" \
-- Elfd. _guppa_ "to bob" \
W: \
-- MHG _gumpen_ "to hop, jump" \
REF: [EDPG]\: 195

### <a id="gunda">\*gunda- [m.] "pus; decaying skin(?)" (n)</a>
E: \
-- Go. _gund_ [n.] "gangrene" \
N: \
-- Nw. _gund_ "skin crust, serum crust" \
W: \
-- OE _gund_ "pus" \
-- OHG _gund_ "id." \
REF: [EDPG]\: 195

### <a id="gunþī">\*gunþī- ~ \*gunþjō- [f.] "fight" (n)</a>
N: \
-- ON _gunnr_, _guðr_ \
W: \
-- OE _gūð_ \
-- OS _gūdea_ \
REF: [EDPG]\: 196

### <a id="gurdjan">\*gurdjan- [wk.] "to gird" (v)</a>
N: \
-- ON _gyrða_ \
W: \
-- OE _gyrdan_ \
-- OS _gurdian_ \
-- OHG _gurten_ \
REF: [EDPG]\: 196

### <a id="guttōn">\*gut(t)ōn- [wk.] "to pour; to spawn" (v)</a>
N: \
-- ON _gota_ "to spawn" \
-- Nw. _gota_, dial. _gåtta_ "id." \
W: \
-- OHG _gozzōn_(?) "to percolate" \
REF: [EDPG]\: 196

## H

### <a id="habanō">\*habanō- [f.] "harbor" (n)</a>
N: \
-- ON _hǫfn_ \
W: \
-- OE _hæfen_, E _haven_ \
-- MLG _havene_ \
-- MDu. _haven_, Du. _haven_ [c.] \
-- G _Hafen_ (= LG) \
REF: [EDPG]\: 196

### <a id="habēn">\*habēn- [wk.] "to have" (v)</a>
E: \
-- Go. _haban_ \
N: \
-- ON _hafa_ \
W: \
-- OE _habban_, E _to have_ \
-- OFri. _hebba_ \
-- OS _hebbian_ \
-- Du. _hebben_ \
-- OHG _haben_, G _haben_ \
REF: [EDPG]\: 197

### <a id="habran">\*habran- [m.] "oats" (n)</a>
N: \
-- ON _hafri_ \
-- Far. _havri_ \
W: \
-- OS _haƀoro_ \
-- Du. _haver_ [c.] \
-- OHG _habaro_, _haparo_, G _Hafer_, _Haber_ \
REF: [EDPG]\: 197

### <a id="habuka">\*habuka- [m.] "hawk" (n)</a>
N: \
-- ON _haukr_ \
-- Far. _heykur_ \
-- Elfd. _ok_ \
W: \
-- OE _hafoc_, _hafuc_, _heafoc_, E _hawk_ \
-- OFri. _hauk_ \
-- OS _haƀuk_ \
-- Du. _havik_ [c.] \
-- OHG _habuh_, G _Habicht_ \
REF: [EDPG]\: 197

### <a id="hafian">\*hafian- ~ \*habjan- [st.] "to raise, lift" (v)</a>
E: \
-- Go. _hafjan_ \
N: \
-- ON _hefja_ \
W: \
-- OE _hebban_, E _to heave_ \
-- OFri. _heva_ \
-- OS _hebbian_, _heffian_ \
-- Du. _heffen_ \
-- OHG _heffen_, _heven_, G _heben_ \
REF: [EDPG]\: 198

### <a id="hafra">\*hafra- [m.] "billy goat, buck" (n)</a>
N: \
-- ON _hafr_ \
-- Far. _havur_ "billy-goat; handful of (unthreshed) corn" \
W: \
-- OE _hæfer_ "id." \
REF: [EDPG]\: 198

### <a id="hagan">\*haga(n)- [m.] "enclosure; pasture" (n)</a>
N: \
-- ON _hagi_ "pasture, field for grazing" \
-- Far. _hagi_ "id." \
-- Elfd. _agi_ "meadow; pen" \
W: \
-- OE _haga_ "hedge, enclosure" \
-- OS _hago_ "hedge" \
-- OHG _hag_ "hedge, enclosure, dam" \
REF: [EDPG]\: 198

### <a id="hagla">\*hagla- [m.?] "hail" (n)</a>
N: \
-- ON _hagl_ [n.] \
-- Far. _hagl_ [n.] "hail(stone); drop; shooting pain, inconvenience" \
W: \
-- OE _hagol_, _haegel_ [m.], E _hail_ \
-- OFri. _heil_ [m.?] \
-- OS _hagal_ [m.] \
-- Du. _hagel_ [c.] \
-- OHG _hagal_ [m.] "hail, thunderstorm", G _Hagel_ [m.] "id." \
REF: [EDPG]\: 199

### <a id="hagra">\*hagra- "slim" (adj)</a>
W: \
-- MHG _hager_, G _hager_ \
REF: [EDPG]\: 199

### <a id="hagran">\*hagran- [m.] "bromus(?)" (n)</a>
N: \
-- Icel. _hellin-hagra_ [f.] "a kind of thyme" (< \*hagrōn-) \
-- Nw. dial. _hagre_ "oats" \
-- OSw. _hagri_ "id." \
-- Elfd. _ager_ "id." \
-- Gutn. _hagre_ "id." \
-- Da. _hejre_ [c.] "brome grass" \
REF: [EDPG]\: 199

### <a id="hahsjō">\*hahsjō- [f.] "ham, back of the knee" (n)</a>
W: \
-- Du. _haas_ [c.] "tenderloin" \
-- OHG _hahsa_ "back of the knee", MHG _hahsa_, _hehse_ "id.", G _Hächse_ "id." \
REF: [EDPG]\: 199

### <a id="haidra">\*haidra- "clear" (adj)</a>
N: \
-- ON _heiðr_ "bright, clear, cloudless" \
W: \
-- OE _hādor_ [m./n.] "clear sky" \
-- OS _hēdar_ "serene" \
-- OHG _heitar_ "clear, bright", G _heiter_ "clear, cheerful" \
REF: [EDPG]\: 200

### <a id="haifsti">\*haifsti- [f.] "conflict" (n)</a>
E: \
-- Go. _haifsts_ "contestation, rivalry, conflict" \
N: \
-- ON _heift_, _heifst_ "hatred, feud, war" \
W: \
-- OE _hǣst_ "ferocity" \
-- OFri. _hāst_ "anger; rush" \
-- MLG _heist_ "vehemence" \
-- MHG _heifte_ "tempestas" \
REF: [EDPG]\: 200

### <a id="haiha">\*haiha- "one-eyed" (adj)</a>
E: \
-- Go. _haihs_ \
REF: [EDPG]\: 200

### <a id="haila">\*haila- "whole, sound" (adj)</a>
E: \
-- Go. _hails_ \
N: \
-- ON _heill_ \
W: \
-- OE _hāl_, E _whole_ \
-- OFri. _hēl_ \
-- OS _hēl_ \
-- Du. _heel_ \
-- OHG _heil_, G _heil_ "sound" \
REF: [EDPG]\: 200

### <a id="hailjan">\*hail(j)a(n)- [m.] "brain" (n)</a>
N: \
-- ON _heili_, _heilir_ \
-- Far. _heili_ \
-- Nw. _heile_ \
W: \
-- OFri. _hēila_, _hāl_ \
REF: [EDPG]\: 201

### <a id="haima">\*haima- [m.] "village, home" (n)</a>
E: \
-- Go. _haims_ [f.] "village" \
N: \
-- ON _heimr_ "home; world" \
-- Far. _heimur_ "world" \
-- Elfd. _iem_ [n.] "home" \
W: \
-- OE _hām_ "id.", E _home_ \
-- OFri. _hēm_ [n.] "id." \
-- OS _hēm_ [n.] "id." \
-- MDu. _heem_ [n.] "id." \
-- OHG _heim_ [m./n.] "id.", G _Heim_ [n.] "id." \
REF: [EDPG]\: 201

### <a id="hainō">\*hainō- [f.] "whetstone" (n)</a>
N: \
-- ON _hein_ \
W: \
-- OE _hān_, E _hone_ \
REF: [EDPG]\: 201

### <a id="haira">\*haira- "hoary, grey-haired" (adj)</a>
N: \
-- ON _hárr_ \
W: \
-- OE _hār_ "hoary, grey, old", E _hoary_ \
-- OS _hēr_ "noble, distinguished, aged" \
-- OHG _hēr_ "old, reverend, grand" \
REF: [EDPG]\: 201

### <a id="hairsa">\*hairsa- ~ \*haisra- "hoarse" (adj)</a>
N: \
-- ON _háss_ \
-- Far. _hásur_ \
-- OSw. _hēs_ \
-- Elfd. _įes_ \
W: \
-- OE _hās_, ME _hōs_, _hōrs_, E _hoarse_ \
-- MDu. _heesc_, Du. _hees_ \
-- OHG _heis(ar)_, MHG _heis(er)_, G _heiser_ \
REF: [EDPG]\: 201

### <a id="haita">\*haita- "hot" (adj)</a>
N: \
-- ON _heitr_ \
-- Far. _heitur_ \
-- Elfd. _iet_ \
W: \
-- OE _hāt_, E _hot_ \
-- OFri. _hēt_ \
-- OS _hēt_ \
-- Du. _heet_ \
-- OHG _heiz_, G _heiß_ \
REF: [EDPG]\: 202

### <a id="haitan">\*haitan- [st.] "to call" (v)</a>
E: \
-- Go. _haitan_ "to call, address; to command" \
N: \
-- ON _heita_ "to call, be called" \
-- Far. _eita_ "to be called" \
-- Elfd. _ietta_ [wk.] "id." \
W: \
-- OE _hātan_ [wk.] "id." \
-- OFri. _hēta_ "to call, to be called; to command; to promise" \
-- OS _hētan_ "to call, be called; to command" \
-- Du. _heten_ [st./wk.] "to be called" \
-- OHG _heizan_ "id.", G _heißen_ "id." \
REF: [EDPG]\: 202

### <a id="haiþi">\*haiþi- [f.] "heath, heather" (n)</a>
E: \
-- Go. _haiþi_ "open field" \
N: \
-- ON _heiðr_ "heath, moor" \
-- Far. _heiði_ "id." \
-- Elfd. _ieð_ "id." \
W: \
-- OE _hǣð_ [m.], [n.] "heather" (< \*haiþja-), E _heath_ \
-- MLG _hēde_ "id." \
-- Du. _hei(de)_ "id." \
-- OHG _heida_ "id.", G _Heide_ "id." \
REF: [EDPG]\: 202

### <a id="hajan">\*hajan- [wk.] "to pound" (v)</a>
W: \
-- MDu. _heien_, Du. _heien_ "to drive, ram" \
REF: [EDPG]\: 203

### <a id="hajja">\*haj(j)a- [n.] "heat" (n)</a>
W: \
-- MDu. _hei_ [m.] "heat, drought" \
-- OHG _hei_, _ge-hei_ "id.", G Swi. _ge-hei_ "warm mist" \
REF: [EDPG]\: 203

### <a id="hakan">\*hakan- [m.] "hook" (n)</a>
N: \
-- Icel. _haki_ "pick; hook" \
-- Far. _haki_ "spade" \
-- Elfd. _atji_ \
W: \
-- OE _haca_ "hook; bolt or bar of a door" \
-- OS _hako_ \
-- Du. _haak_ [c.] \
-- OHG _hāko_, _hācko_, G _Haken_ \
REF: [EDPG]\: 203

### <a id="hakula">\*hakula- [m.] "cloak" (n)</a>
E: \
-- Go. _hakuls_ "cloak, travelling cloak" \
N: \
-- ON _hǫkull_ "covering" \
-- Far. _hakil_ "short cloak" \
W: \
-- OE _hæcla_, _hacele_ [f.] "id." \
-- OFri. _hezil_ "id." (< \*hakila-) \
-- OHG _hahhul_ "id." \
REF: [EDPG]\: 203

### <a id="halan">\*halan- [m.] "tail, tip" (n)</a>
N: \
-- ON _hali_ \
-- Far. _hali_ "tail; top of the shaft of a scythe" \
-- Elfd. _ali_ "tail" \
REF: [EDPG]\: 203

### <a id="halba">\*halba- "half" (adj)</a>
E: \
-- Go. _halbs_ \
N: \
-- ON _halfr_ \
-- Far. _hálvur_ \
-- Elfd. _åv_ \
W: \
-- OE _healf_, E _half_ \
-- OFri. _half_ \
-- OS _half_ \
-- Du. _half_ \
-- OHG _halb_, _halp_, G _halb_ \
REF: [EDPG]\: 204

### <a id="haleþ">\*haleþ- ~ \*haluþ- [m.] "man, hero" (n)</a>
N: \
-- ON _hǫldr_ "yeoman", poet _halr_ "man" \
W: \
-- OE _hæleþ_, _heleþ_ "man, warrior, hero" \
-- OS _helith_ "hero, man" \
-- Du. _held_ [c.] "hero" \
-- OHG _helid_ "man, warrior, hero", G _Held_ "hero" \
REF: [EDPG]\: 204

### <a id="haljō">\*haljō- [f.] "hell" (n)</a>
E: \
-- Go. _halja_ \
N: \
-- ON _Hel_ "goddess of death" \
W: \
-- OE _hel_, _helle_ "id." \
-- OFri. _helle_ "id." \
-- OS _hellia_ "id." (also _hell_ [m.] "id." < \*halli-) \
-- Du. _hel_ [c.] "id." \
-- OHG _hella_ "id.", G _Hölle_ "id." \
REF: [EDPG]\: 204

### <a id="halma">\*halma- [m.] "blade of grass" (n)</a>
N: \
-- ON _halmr_ \
W: \
-- OE _healm_, E _haulm_ \
-- OS _halm_ \
-- Du. _halm_ [c.] \
-- OHG _halm_, G _Halm_ \
REF: [EDPG]\: 204

### <a id="halōn">\*halōn- ~ \*hulōn- [wk.] "to call" (v)</a>
W: \
-- OE _ge-holian_ "to get" \
-- OFri. _halia_ "to fetch" \
-- OS _halōn_ "to fetch, bring" \
-- Du. _halen_ "to fetch, take away" \
-- OHG _halōn_, _holōn_ "to fetch, call, take", G _hohlen_ "to fetch" \
REF: [EDPG]\: 205

### <a id="halsa">\*halsa- [m.] "neck" (n)</a>
E: \
-- Go. _hals_ \
N: \
-- ON _hals_ \
-- Far. _hálsur_ \
-- Elfd. _ås_ \
W: \
-- OE _heals_, _hals_ "neck; prow of a ship" \
-- OFri. _hals_ \
-- MDu. _hals_, Du. _hals_ \
-- OHG _hals_, G _Hals_ \
REF: [EDPG]\: 205

### <a id="halta">\*halta- "lame, limping" (adj)</a>
E: \
-- Go. _halts_ \
N: \
-- ON _haltr_ \
W: \
-- OE _healt_ \
-- OFri. _halt_ \
-- OS _halt_ \
-- OHG _halz_ \
REF: [EDPG]\: 205

### <a id="halþa">\*halþa- "sloping; inclined" (adj)</a>
E: \
-- Go. _wilja-halþei_ [f.] "inclination, bias" \
N: \
-- ON _hallr_ "leaning to one side, lying over, sloping; biased, partial" \
-- Far. _hallur_ "inclined, sloping" \
W: \
-- OE _heald_ "inclined" \
-- OFri. _hald_ "id." \
-- OHG _hald_ "sloping" \
REF: [EDPG]\: 205

### <a id="hamala">\*hamala- "truncated(?)" (adj)</a>
W: \
-- OHG _hamal_ "mutilated" \
REF: [EDPG]\: 206

### <a id="haman">\*hama(n)- [m.] "shape, physical form" (n)</a>
N: \
-- ON _hamr_ "skin, slough; shape, form" \
-- Far. _hamur_ "slough; ghost, apparition" \
W: \
-- OE _hama_, _homa_ "covering" \
-- OS _hamo_ "covering" \
-- OHG _hamo_ "skin, covering, net" \
REF: [EDPG]\: 206

### <a id="haman">\*haman- [m.] "rod, shaft" (n)</a>
W: \
-- E _hame_ "horse collar" \
-- MDu. _hame_, _haem_ "id.", Du. _haam_ "id." \
-- MHG _ham(e)_ "fishing rod" \
REF: [EDPG]\: 206

### <a id="hamara">\*hamara- [m.] "hammer" (n)</a>
N: \
-- ON _hamarr_ "hammer; back of an axe; crag; precipice" \
-- Far. _hamar_ "id." \
-- Elfd. _åmår_ "id." \
W: \
-- OE _hamor_, E _hammer_ \
-- OFri. _homer_, _hamer_ \
-- OS _hamar_, _hamur_ \
-- Du. _hamer_ [c.] \
-- OHG _hamar_, G _Hammer_ \
REF: [EDPG]\: 206

### <a id="hamfa">\*hamfa- "maimed" (adj)</a>
E: \
-- Go. _hamfs_\* \
W: \
-- OS _hāf_ \
-- OHG _hamf_ \
REF: [EDPG]\: 207

### <a id="hammō">\*ham(m)ō- [f.] "shank" (n)</a>
N: \
-- ON _hǫm_ "ham or haunch (of a horse)" \
W: \
-- OE _ham_, _hom_ "back of the knee", E _ham_ \
-- MDu. _hamme_ "shank, ham", Du. _ham_ [c.] "ham" \
-- OHG _hama_, _hamma_ "shank, back of the knee", MHG _hamme_ "id." \
REF: [EDPG]\: 207

### <a id="hanan">\*hanan- [m.] "rooster, singer" (n)</a>
E: \
-- Go. _hana_ "rooster" \
N: \
-- ON _hani_ "id." \
W: \
-- OE _hana_ "id." \
-- OFri. _hona_ "id." \
-- OS _hano_ "id." \
-- Du. _haan_ [c.] "id." \
-- OHG _hano_ "id.", G _Hahne_ "id." \
REF: [EDPG]\: 207

### <a id="handu">\*handu- [f.] "hand" (n)</a>
E: \
-- Go. _handus_ \
N: \
-- ON _hǫnd_, pl. _hendr_ \
-- Far. _hond_, pl. _hendur_ \
W: \
-- OE _hand_, _hond_, E _hand_ \
-- OFri. _hond_ \
-- OS _hand_ \
-- Du. _hand_ [c.] \
-- OHG _hant_, G _Hand_ \
REF: [EDPG]\: 207

### <a id="hangjan">\*hangjan- ~ \*hankjan- [wk.] "to (make) hang" (v)</a>
N: \
-- ON _hengja_ \
W: \
-- OE _hengan_ \
-- MDu. _hengen_ \
-- OHG _henken_, G _hängen_ \
REF: [EDPG]\: 208

### <a id="hanhan">\*hanhan- [st.] "to hang" (v)</a>
E: \
-- Go. _hahan_ [wk.] (tr.) \
N: \
-- ON _hanga_ \
-- Far. _hanga_ \
-- Elfd. _aindja_ (< \*hangjan-) \
W: \
-- OE _hōn_ \
-- OFri. _huā_ \
-- Du. _hangen_ \
-- OHG _hāhan_ [wk.], G _hangen_ \
REF: [EDPG]\: 208

### <a id="hanhilan">\*hanhila(n)- [m.] "heel" (n)</a>
N: \
-- ON _hæll_ \
-- Far. _hælur_ \
-- Elfd. _äl_ \
W: \
-- OE _hēla_, E _heel_ \
-- OFri. _hēla_, _heila_ \
-- Du. _hiel_ [c.] \
REF: [EDPG]\: 209

### <a id="hanhilō">\*hanhilō- [f.] "pothook" (n)</a>
W: \
-- MDu. _hael_ [f./n.] \
-- OHG _hāhala_, G Swi. Visp. _heeli_ "id." \
REF: [EDPG]\: 209

### <a id="hanhista">\*hanhista- ~ \*hangista- [m.] "horse; stallion(?); gelding(?)" (n)</a>
N: \
-- ON _hestr_ "stallion; horse" \
-- Far. _hestur_ "horse; stallion" \
-- Elfd. _est_ "horse" \
W: \
-- OE _hengest_, _hengst_ "gelding; horse" \
-- OFri. _hengst_, _hangst_, _hingst_ "horse" \
-- ODu. _hingest_ "gelding(?)", Du. _hengst_ "stallion" \
-- OHG _hengist_ "gelding", G _Hengst_ "stallion" \
REF: [EDPG]\: 209

### <a id="hanipa">\*hanipa- [m.] "hemp" (n)</a>
N: \
-- ON _hanpr_ \
-- Far. _hampur_ \
-- Elfd. _ampa_ [f.] \
W: \
-- OE _hænep_, _henep_, E _hemp_ \
-- MDu. _hanep_, _hannep_, Du. _hennep_ [c.] \
-- OHG _hanaf_, _hanif_, G _Hanf_ \
REF: [EDPG]\: 209

### <a id="hansō">\*hansō- [f.] "troop" (n)</a>
E: \
-- Go. _hansa_ "troop, cohort" \
W: \
-- OE _hōs_ "company, band" \
-- MLG _hanse_, _hense_ "guild" \
-- OHG _hansa_ "troop", MHG _hanse_ "guild" \
REF: [EDPG]\: 209

### <a id="hanþa">\*hanþa- "sharp" (adj)</a>
N: \
-- ON -_hannr_ \
REF: [EDPG]\: 210

### <a id="happōn-1">\*happ/bōn- [wk.] "to snatch, grab" (v)</a>
W: \
-- EDu. _happen_, _habben_ "to take, pick, steal", Du. _hap-snap_ "haphazardly" (adv) \
REF: [EDPG]\: 210

### <a id="happōn-2">\*happ/bōn- [wk.] "to succeed" (v)</a>
N: \
-- Sw. _happa säg_, dial. _håpa_, habba siig "to happen unexpectedly, fit, succeed" \
REF: [EDPG]\: 210

### <a id="harbista">\*harbista- [m.] "autumn, fall" (n)</a>
N: \
-- ON _haustr_ \
-- Icel. _haust_ [n.] \
-- Far. _heyst_ [n.] \
-- Elfd. _ost_ \
W: \
-- OE _hærfest_ "harvest, autumn", E _harvest_ \
-- MDu. _herfst_, _heerfst_, _herft_ "id.", Du. _herfst_ [c.] "autumn" \
-- OHG _herbist_ "harvest; autumn", G _Herbst_ "autumn" \
REF: [EDPG]\: 210

### <a id="hardi">\*hardi- [f.] "shoulder" (n)</a>
N: \
-- ON _herðr_ \
-- Far. _herðar_ [f.pl.] \
-- Elfd. _erde_ \
W: \
-- OHG _harti_, _herti_, MHG _herte_ \
REF: [EDPG]\: 211

### <a id="hardu">\*hardu- "hard, severe" (adj)</a>
E: \
-- Go. _hardus_ \
N: \
-- ON _harðr_ \
-- Far. _harður_ \
-- Elfd. _ård_ \
W: \
-- OE _heard_, _hard_, E _hard_ \
-- OFri. _herd_ \
-- OS _hard_ \
-- Du. _hard_ \
-- OHG _hart_, _herti_, G _hart_ \
REF: [EDPG]\: 211

### <a id="hargu">\*hargu- [m.] "sacrificial mound?" (n)</a>
N: \
-- ON _hǫrgr_ "pile of rocks; sanctuary" \
W: \
-- OE _hearg_ "pagan tempel, idol" \
-- OHG _harug_ "grove, place of sacrifice" \
REF: [EDPG]\: 211

### <a id="harja">\*harja- [m.] "host, troop, army" (n)</a>
E: \
-- Go. _harjis_ \
N: \
-- ON _herr_ \
-- Far. _herur_ \
W: \
-- OE _here_ \
-- OFri. _here_ [m./f./n.] \
-- OS _heri_ \
-- OHG _hari_, _heri_ [n.], G _Heer_ [n.] \
REF: [EDPG]\: 211

### <a id="harma">\*harma- [m.] "harm, sorrow" (n)</a>
N: \
-- ON _harmr_ "sorrow, grief" \
-- Far. _harmur_ "affliction, distress" \
W: \
-- OE _hearm_ "id.", E _harm_ \
-- OFri. _herm_, _harm_ "id." \
-- OS _harm_ [n.] "id." \
-- MHG _harm_ "id.", G _Harm_ "id." \
REF: [EDPG]\: 212

### <a id="harman">\*harman- [m.] "ermine" (n)</a>
W: \
-- OE _hearma_ "shrew-mouse" \
-- OS _harmo_ \
-- OHG _harmo_ "weasel, ermine", MHG _harm(e)_ "id." \
REF: [EDPG]\: 212

### <a id="harōjan">\*harōjan- [wk.] "to sharpen" (v)</a>
W: \
-- MLG _haren_ "to sharpen" \
-- MDu. _haren_ "to sharpen" \
REF: [EDPG]\: 212

### <a id="harrōn">\*har(r)ōn- [wk.] "to wait out(?)" (v)</a>
N: \
-- ON _hara_ "to stare, watch motionlessly" \
-- Nw. _hara_ "to freeze, watch without motion" \
W: \
-- MHG _harren_ "to await", G _harren_ "id." \
REF: [EDPG]\: 212

### <a id="harta">\*harta- [n.] "resin, pitch" (n)</a>
W: \
-- OS _hart_, MLG _hart_, _hars_ \
-- OHG _harz_, G _Harz_ \
REF: [EDPG]\: 212

### <a id="harunda">\*harunda/ō- [n./f.] "flesh" (n)</a>
N: \
-- ON _hǫrund_ [n./f.] "human flesh; skin, complexion" \
-- Icel. _hōrund_ [n.] "skin" \
-- Nw. dial. _horung_ [m.] "skin, skin tone" \
-- OSw. _harund_ [m./f.] "flesh; skin", EDa. _harend_ "body" \
REF: [EDPG]\: 213

### <a id="harwa">\*harwa- "bitter" (adj)</a>
W: \
-- WFri. _harch_ "dry, barren" \
-- Du. dial. _haar_ "dry, parched" \
-- MHG _har_, _herwe_, G _herb_ \
REF: [EDPG]\: 213

### <a id="harzwa">\*harzwa- [m.] "flax" (n)</a>
N: \
-- ON _hǫrr_ \
-- Far. _hørur_ \
W: \
-- OFri. _her_ \
-- OHG _haro_, -_wes_, MHG _har(e)_, _har_ \
REF: [EDPG]\: 213

### <a id="hasla">\*hasla- [m.] "hazel" (n)</a>
N: \
-- ON _hasl_ \
-- Elfd. _assel-buosk_ \
W: \
-- OE _hæsel_, E _hazel_ \
-- MDu. _hasel_, Du. _hazelaar_ [c.] \
-- OHG _hasal_, _hasala_ [f.] (< \*-ōn-), G _Hasel_ [f.] \
REF: [EDPG]\: 213

### <a id="haswa">\*haswa- "grey" (adj)</a>
N: \
-- ON _hǫss_ \
W: \
-- OE _haso_ \
-- MHG _heswe_ "pale, dull" \
REF: [EDPG]\: 213

### <a id="hatiz">\*hatiz- [n.] "hatred" (n)</a>
E: \
-- Go. _hatis_ "hostility, hate" \
N: \
-- ON _hatr_ "hatred, enmity" \
W: \
-- OE _hete_ [m.] "hatred", ME _hete_ "id." \
-- OS _heti_ [m.] "hate, hostility" \
REF: [EDPG]\: 214

### <a id="hatjan">\*hatjan- [wk.] "to hate" (v)</a>
E: \
-- Go. _hatjan_\* \
REF: [EDPG]\: 214

### <a id="hattu">\*hattu- [m.] "hat" (n)</a>
N: \
-- ON _hǫttr_ \
-- Far. _hattur_ \
-- Elfd. _att_ \
W: \
-- OE _hæt_, E _hat_ \
-- OFri. _hat_ \
REF: [EDPG]\: 214

### <a id="haþarō">\*haþarō- [f.] "fight" (n)</a>
W: \
-- MHG _hader_, G _Hader_ \
REF: [EDPG]\: 214

### <a id="haþu">\*haþu- [f.] "battle" (n)</a>
N: \
-- ON _hǫð_ \
W: \
-- OE _heaðo_- \
-- OS _hathu_- \
-- OHG _hadu_ \
REF: [EDPG]\: 214

### <a id="haubeda">\*ha(u)beda- ~ \*ha(u)buda- [n.] "head" (n)</a>
E: \
-- Go. _haubiþ_ \
N: \
-- ON _hǫfuð_ \
-- Far. _høvd_ \
-- Elfd. _ovuð_ \
W: \
-- OE _hæfud_, _hafud_, _hēafod_, E _head_ \
-- OFri. _hāved_, _hauwed_, _haud_, _hād_ \
-- OS _hōƀid_ \
-- ODu. _hovit_, _houvot_, Du. _hoofd_ \
-- OHG _houbit_, G _Haupt_ \
REF: [EDPG]\: 215

### <a id="hauha">\*hauha- "high" (adj)</a>
E: \
-- Go. _hauhs_ \
N: \
-- ON _hár_, _hór_, _haugr_ \
-- Far. _háur_ \
-- OSw. _høgher_ \
-- Elfd. _og_ \
W: \
-- OE _hēa_, _heah_, E _high_ \
-- OS _hōh_ \
-- Du. _hoog_ \
-- OHG _hōh_, G _hoch_ \
REF: [EDPG]\: 215

### <a id="hauja">\*hauja- [n.] "hay" (n)</a>
E: \
-- Go. _hawi_ \
N: \
-- ON _hey_ \
-- Far. _hoyggj_ \
W: \
-- OE _hīeg_, E _hay_ \
-- OFri. _hā_ \
-- OS _hōi_\* \
-- Du. _hooi_ \
-- OHG _hewi_, _houwi_, G _Heu_ \
REF: [EDPG]\: 215

### <a id="haujan">\*haujan- [wk.] "to see to(?)" (v)</a>
N: \
-- ON _heyja_ "to carry out, hold, conduct" \
W: \
-- OE _hēgan_, _hīegan_ "to execute" \
REF: [EDPG]\: 215

### <a id="haukan">\*haukan- [m.] "fishhook" (n)</a>
W: \
-- G Swi. (App.) _hɔuxə_ \
REF: [EDPG]\: 216

### <a id="haukjan">\*haukjan- [wk.] "to make squat" (v)</a>
N: \
-- ON _heykja-st_ "to bend, cower down" \
-- Far. _hoykja seg_ "to sit down" \
REF: [EDPG]\: 216

### <a id="haulan">\*haula(n)- [m.] "hernia, lump" (n)</a>
N: \
-- ON _haull_ \
W: \
-- OE _hēala_ "hernia, hydrocele" \
-- OHG _hōla_ [f.] "rupture" \
REF: [EDPG]\: 216

### <a id="hauna">\*hauna- [m./n.] "scorn, humilitation" (n)</a>
E: \
-- Go. _hauns_ "base" \
W: \
-- MDu. _hoon_ [n.] "id.", Du. _hoon_ [c.] "id." \
-- G _Hohn_ [m.] "id." \
REF: [EDPG]\: 216

### <a id="haupa">\*haupa- [m.] "pile, heap" (n)</a>
W: \
-- OE _hēap_, E _heap_ \
-- OFri. _hāp_ \
-- OS _hōp_ \
-- Du. _hoop_ [c.] \
-- OHG _houf_ \
REF: [EDPG]\: 216

### <a id="hauþan">\*hauþan- [m.] "testicle" (n)</a>
W: \
-- OFri. _hātha_, _hōtha_ \
-- MDu. _hode_ \
-- OHG _hōdo_, MHG _hōde_, G _Hode_ \
REF: [EDPG]\: 217

### <a id="hauzjan">\*hauzjan- [wk.] "to hear" (v)</a>
E: \
-- Go. _hausjan_ \
N: \
-- ON _heyra_ \
-- Far. _hoyra_ \
-- Elfd. _ära_ \
W: \
-- OE _hīeran_, E _to hear_ \
-- OFri. _hēra_ \
-- OS _hōrian_ \
-- Du. _horen_ \
-- OHG _hōrran_, _hōren_, G _hören_ \
REF: [EDPG]\: 217

### <a id="hawen-1">\*hawēn- [wk.] "to mock" (v)</a>
N: \
-- ON _há_ "to tease" \
-- Icel. _há_ "to disdain" \
-- Nw. _hå_ "to mock, tease" \
REF: [EDPG]\: 217

### <a id="hawen-2">\*hawēn- [wk.] "to observe" (v)</a>
N: \
-- Nw. _hå_ "to notice, sense, be aware of" \
W: \
-- OE _hawian_ "to view, look, observe, regard, survey, inspect" \
REF: [EDPG]\: 217

### <a id="hawō">\*hawō- [f.] "hide" (n)</a>
N: \
-- ON _há_ \
REF: [EDPG]\: 218

### <a id="hawwan">\*hawwan- [st.] "to hew, chop" (v)</a>
N: \
-- ON _hǫggva_ \
-- Far. _høgga_ \
-- Elfd. _ogga_ \
W: \
-- OE _hēawan_, E _to hew_ \
-- OFri. _hāwa_, _hauwa_ \
-- OS _hauwan_ \
-- Du. _houwen_ \
-- OHG _houwan_, G _hauen_ \
REF: [EDPG]\: 218

### <a id="hazjan">\*hazjan- [wk.] "to praise" (v)</a>
E: \
-- Go. _hazjan_ \
W: \
-- OE _herian_, _hærian_, _hergan_, ME _herien_ \
REF: [EDPG]\: 218

### <a id="helan">\*helan- [st.] "to conceal, hide" (v)</a>
W: \
-- OE _helan_ "to conceal, hide; cover" \
-- OFri. _hela_ \
-- OS _helan_ \
-- MDu. _helen_ "to hide, fence", Du. _helen_ [wk.] "to fence" \
-- OHG _helan_ "to conceal, hide", G _hehlen_ "to fence" \
REF: [EDPG]\: 218

### <a id="hēli">\*hēli- "deceitful" (adj)</a>
W: \
-- MLG _hāl_ "hidden" \
-- OHG _hāli_ "smooth, slippery, weak", MHG _hæl(e)_ "concealing; concealed" \
REF: [EDPG]\: 218

### <a id="helma">\*helma- [m.] "helmet" (n)</a>
E: \
-- Go. _hilms_ \
N: \
-- ON _hjalmr_ \
-- Far. _hjálmur_ \
W: \
-- OE _helm_ \
-- OFri. _helm_ \
-- OS _helm_ \
-- MDu. _helm_, Du. _helm_ [c.] \
-- OHG _helm_, G _Helm_ \
REF: [EDPG]\: 219

### <a id="helman">\*helman- [m.] "rudder" (n)</a>
N: \
-- ON _hjalm-vǫlr_ \
-- Far. _hjalmi_ "transverse tiller" \
W: \
-- OE _helma_ "id.", E _helm_ \
-- MDu. _helm_ "handle" \
REF: [EDPG]\: 219

### <a id="helpan">\*helpan- [st.] "to help" (v)</a>
E: \
-- Go. _hilpan_ \
N: \
-- ON _hjalpa_ \
-- Far. _hjálpa_ \
-- Elfd. _jåpa_ \
W: \
-- OE _helpan_, E _to help_ \
-- OFri. _helpa_ \
-- Du. _helpen_ \
-- OHG _helfan_, G _helfen_ \
REF: [EDPG]\: 219

### <a id="hemerō">\*hemerō- [f.] "hellebore" (n)</a>
W: \
-- OHG _hemera_, MHG _hemere_, _hemer_, G dial. _hemern_ pl. \
REF: [EDPG]\: 219

### <a id="hemina">\*hemina- ~ \*hemna- [m.] "heaven" (n)</a>
E: \
-- Go. _himins_ \
N: \
-- ON _himinn_ \
-- Far. _himin_ \
W: \
-- OE _heofon_, E _heaven_ \
-- OFri. _himel_ \
-- OS _himil_, _heƀan_ \
-- ODu. _himil_, Du. _hemel_ [c.] \
-- OHG _himil_, G _Himmel_ \
REF: [EDPG]\: 220

### <a id="hēra">\*hēra- [n.] "hair" (n)</a>
N: \
-- ON _hár_ \
-- Far. _hár_ \
-- Elfd. _år_ \
W: \
-- OE _hǣr_, E _hair_ \
-- OFri. _hēr_ \
-- OS _hār_ \
-- Du. _haar_ \
-- OHG _hār_, G _Haar_ \
REF: [EDPG]\: 220

### <a id="herdō">\*herdō- [f.] "herd" (n)</a>
E: \
-- Go. _hairda_ \
N: \
-- ON _hjǫrð_ \
W: \
-- OE _heord_, E _herd_ \
-- OHG _herta_ "herd; order, queue", G _Herd_ "herd" \
REF: [EDPG]\: 221

### <a id="hermjan">\*hermjan- [wk.] "to rest" (v)</a>
W: \
-- MDu. _hermen_ \
-- OHG _hirmen_, MHG _hirmen_, G Swi. _(ge-)hirmen_ \
REF: [EDPG]\: 221

### <a id="herran">\*her(r)an- [m.] "hinge" (n)</a>
N: \
-- ON _hjarri_ \
-- Icel. _hjari_ ("geographical pole, hinge") \
-- Far. _hjørri_ [f.]  \
W: \
-- OE _heor(ra)_, ME _herre_ \
-- MDu. _harre_, _herre_ [f.], Du. _har_ "hinge, fissure" \
REF: [EDPG]\: 221

### <a id="hersan">\*hersan- ~ \*herzan- [m.] "brain" (n)</a>
N: \
-- ON _hjarni_ \
-- Nw. _hjerne_, dial. _hjarre_ \
-- Elfd. _ienne_ \
W: \
-- ME _hernes_, E obs. _harn(s)_ \
-- MDu. _hersene_, Du. _hersenen_, _hersens_ pl. \
REF: [EDPG]\: 221

### <a id="hersjan">\*hersja(n)- [m.] "millet" (n)</a>
N: \
-- ON _hirsi_ [n.] \
W: \
-- MDu. _herse_, _heerse_ [f.] \
-- OHG _hirsi_, G _Hirse_ \
REF: [EDPG]\: 222

### <a id="hertōn">\*hertōn- [n.] "heart" (n)</a>
E: \
-- Go. _hairto_ \
N: \
-- ON _hjarta_ \
-- Far. _hjarta_ \
-- Elfd. _järta_ \
W: \
-- OE _heorte_, E _heart_ \
-- OFri. _herte_ [f.] \
-- OS _herta_ \
-- Du. _hart_ \
-- OHG _herza_, G _Herz_ \
REF: [EDPG]\: 222

### <a id="herþa">\*herþa- [m.] "hearth" (n)</a>
W: \
-- OE _heorð_, E _hearth_ \
-- OFri. _herth_ \
-- OS _herth_ \
-- Du. _haard_ [c.] \
-- OHG _herd_, G _Herd_ \
REF: [EDPG]\: 222

### <a id="herþra">\*herþra- [n.] "intestines" (n)</a>
E: \
-- Go. _hairþra_ [n.pl.] "intestines, bosom" \
W: \
-- OHG _smala-herdar_ "small intestine, gut" \
REF: [EDPG]\: 222

### <a id="heru">\*heru- [m.] "sword" (n)</a>
E: \
-- Go. _hairus_ \
N: \
-- ON _hjǫrr_ \
W: \
-- OE _heoru_, _heoro_, _hioro_ \
-- OS _heru-drōrag_ "sword-bloody" \
REF: [EDPG]\: 222

### <a id="heruta">\*heruta- [m.] "stag, hart, deer" (n)</a>
N: \
-- ON _hjǫrtr_ "deer" \
-- Far. _hjørtur_ \
-- Elfd. _juort_ \
W: \
-- OE _heorot_, _heort_ "hart, stag, male deer" \
-- OS _hirut_ "stag" \
-- Du. _hert_ [n.] "deer" \
-- OHG _hiruz_, _hirz_, G _Hirsch_ "stag, hart, deer" \
REF: [EDPG]\: 223

### <a id="herzna">\*herzna- [n.] "hard snow" (n)</a>
N: \
-- ON _hjarn_ \
-- Nw. _hjårn_ [f.] \
REF: [EDPG]\: 223

### <a id="hesan">\*hesan- ~ \*hazan- [m.] "hare" (n)</a>
N: \
-- ON _hĕ́ri_ \
-- Icel. _héri_ \
-- Nw. _hare_, dial. _jase_ \
-- OSw. _heri_ \
-- Sw. _hare_ \
-- Elfd. _eri_ \
-- ODa. _harE_, Da. _hare_ (>> Far. _hara_) \
W: \
-- OE _hara_, E _hare_ \
-- OFri. _hasa_ \
-- OS _haso_ \
-- MDu. _hase_, Du. _haas_ [c.] \
-- OHG _haso_, G _Hase_ \
REF: [EDPG]\: 223

### <a id="heuhman">\*heuhman- [m.] "mass, heap, multitude" (n)</a>
E: \
-- Go. _hiuhma_ \
REF: [EDPG]\: 224

### <a id="heuja">\*heuja- [n.] "visible layer, appearance" (n)</a>
E: \
-- Go. _hiwi_ "surface appearance, complexion" \
N: \
-- ON _hý_ "the down of plants, hair, feathers" \
-- Far. _hýggj_ "film of fungus" \
-- Nw. _hy_ "fungus, downy beard, young grass" \
W: \
-- OE _hīw_, _hēow_ "shape, form, hue, color, fashion", ME hue \
REF: [EDPG]\: 224

### <a id="heupan">\*heupa/ōn- [m./f.] "rose hip" (n)</a>
N: \
-- Icel. _hjúpa_ [n.], _hjúpur_ [m.] \
-- Nw. dial. _hjupa_ [f.] \
-- OSw. _hjupon_ [f.] \
W: \
-- OE _hēope_ [f.], E _rose hip_ \
-- OS _hiopo_ [m.] \
-- MDu. _hiepe_, _iepe_, _niepe_ [f.] "cornell" \
-- OHG _hiufo_ [m.], MHG _hiefe_ [f.] \
REF: [EDPG]\: 224

### <a id="hēwja">\*hēwja- "dark blue(?)" (adj)</a>
W: \
-- OE _hāwi_, _hǣwi_, _hēawi_ "blue, purple, grey, discolored", E obs. _haw_ "blue, azure, bluish, dull leaden blue; livid" \
REF: [EDPG]\: 224

### <a id="hezdōn">\*hezdōn- [f.] "flax fibers" (n)</a>
W: \
-- OE _heord_ "hair", _heordan_ pl. "rope", E _hards_, _hurds_ "the coarser parts of flax or hemp separated in hackling" \
-- MDu. _hede_, EDu. _herde_, _heerde_ \
REF: [EDPG]\: 225

### <a id="hi">\*hi- "this, these" (pron)</a>
E: \
-- Go. _his_\* \
REF: [EDPG]\: 225

### <a id="hiar">\*hiar "here" (adv)</a>
E: \
-- Go. _her_ \
N: \
-- ON _hér_ \
-- Elfd. _jär_ \
W: \
-- OE _hēr_, E _here_ \
-- OFri. _hīr_ \
-- OS _hēr_, _hier_ \
-- Du. _hier_ \
-- OHG _hiar_, _hia_, G _hier_ \
REF: [EDPG]\: 225

### <a id="hīgēn">\*hīgēn- ~ \*hīkēn- [wk.] "to pant(?)" (v)</a>
N: \
-- Nw. _hika_ "to long for" \
-- Sw. dial. _hikja_ "to pant" \
-- Da. _hige_ "to long for, wait for" \
W: \
-- OE _hīgian_ "to hie, haste, strive" \
-- MDu. _higen_ "to pant", Du. _hijgen_ "to pant", obs. "to yearn" \
REF: [EDPG]\: 225

### <a id="hihelōn">\*hihelōn- [f.] "hoarfrost" (n)</a>
N: \
-- ON _héla_ \
-- Elfd. _ila_ \
REF: [EDPG]\: 226

### <a id="hindō">\*hindō- [f.] "hind" (n)</a>
N: \
-- ON _hind_ \
-- Far. _hind_ \
W: \
-- OE _hind_, E _hind_ \
-- OFri. _hinde_ \
-- OS _hind-beri_ "raspberry" \
-- Du. _hinde_ [c.] \
-- OHG _hinta_, G _Hinde_ \
REF: [EDPG]\: 226

### <a id="hinkan">\*hinkan- [st.] "to limp, hobble" (v)</a>
W: \
-- OE _hincian_ [wk.] \
-- MDu. _hinken_ [wk.], Du. _hinken_ [wk.] \
-- OHG _hinkan_, G _hinken_ [wk.] \
REF: [EDPG]\: 226

### <a id="hinnō">\*hinnō- [f.] "thin skin, membrane" (n)</a>
N: \
-- ON _hinna_ \
-- Far. _hinna_ \
-- Elfd. _inna_ \
W: \
-- OE _hion_ "meninx(?)" \
-- NFri. _hann_ "lobe of the brain" \
REF: [EDPG]\: 226

### <a id="hinþan">\*hinþan- [st.] "to reach for" (v)</a>
E: \
-- Go. _hinþan_ "to catch" \
N: \
-- OSw. _hinna_ "to obtain" \
REF: [EDPG]\: 227

### <a id="hitjō">\*hitjō- [f.] "heat" (n)</a>
W: \
-- OFri. _hete_ \
-- OS _hittia_ \
-- Du. _hitte_ [c.] \
-- OHG _hizzea_, _hizza_, G _Hitze_ \
REF: [EDPG]\: 227

### <a id="hittjan">\*hittjan- [wk.] "to hit (upon)" (v)</a>
N: \
-- ON _hitta_ "to hit upon; to hit, strike; to meet" \
-- Far. _hitta_ "to meet; to come across" \
-- Elfd. _itta_ "id." \
W: \
-- OE _hittan_ "to hit upon, meet with" \
-- MDu. _hitten_ "to obtain; to succeed" \
REF: [EDPG]\: 227

### <a id="hīwiskja">\*hīwiskja- [n.] "household" (n)</a>
N: \
-- ON _hýski_ \
-- Far. _húski_, _hýski_ \
W: \
-- OE _hīwisc_ \
-- OS _hīwiski_ \
-- OHG _hīwisci_, MHG _hīwisch(e)_ \
REF: [EDPG]\: 227

### <a id="hīwōn">\*hīwōn- [n.] "married couple, household" (n)</a>
N: \
-- ON _hjú(n)_, _hjó(n)_ [n.pl.] \
-- Far. _hjún_ [n.pl.] \
W: \
-- OE _hīwan_ [n.pl.] "married couple, household members" \
-- OFri. _hiōne_, _hiūne_, _hīne_ [n.pl.] "id." \
-- OS _hīwun_, _hīun_ [n.pl.] "married couple" \
-- MDu. _huwe_ [f.] "id." \
-- OHG _hīūn_, _hīwen_, _hīen_ [n.pl.] "id." \
REF: [EDPG]\: 227

### <a id="hlahjan">\*hlah(j)an- [st.] "to laugh" (v)</a>
N: \
-- ON _hlæja_ \
-- Far. _læa_ [wk.] \
-- Elfd. _läa_ \
W: \
-- OS _hlahan_ \
-- Du. _lachen_ [st./wk.] \
-- MHG _lachen_ [wk.], G _lachen_ [wk.] \
REF: [EDPG]\: 228

### <a id="hlahtra">\*hlahtra- [m. (n.?)] "laughter" (n)</a>
N: \
-- ON _hlátr_ [m.] \
-- Far. _látur_ [m.] \
W: \
-- OE _hleahtor_, _hlehter_ [m.], E _laughter_ \
-- OHG _lahtar_, _hlahtar_ [n.], MHG _lahter_ [n.] \
REF: [EDPG]\: 228

### <a id="hlaiba">\*hlaiba- [n.] "bread" (n)</a>
E: \
-- Go. _hlaifs_ [m.] \
N: \
-- ON _hleifr_ [m.] \
W: \
-- OE _hlāf_ [m.], E _loaf_ \
-- OHG _leip_ [m.], G _Laib_ [m.] \
REF: [EDPG]\: 228

### <a id="hlainjan">\*hlainjan- [wk.] "to cause to lean" (v)</a>
W: \
-- OE _hlǣnan_, E _to lean_ \
-- OHG _leinen_ \
REF: [EDPG]\: 228

### <a id="hlaiwa">\*hlaiwa- [n.] "barrow, funeral mound" (n)</a>
E: \
-- Go. _hlaiw_ [m.] "tomb, grave" \
W: \
-- OE _hlāw_ [m.] "id." \
-- OS _hlēo_ [m.] "grave" \
-- OHG _lēo_, _hlēo_ [m.] "id.", MHG _lē_ [m.] "id." \
REF: [EDPG]\: 228

### <a id="hlakkōn">\*hlak(k)ōn- [wk.] "to laugh" (v)</a>
N: \
-- ON _hlakka_ "to cry, scream (of eagles); to rejoice, exult" \
W: \
-- OFri. _hlakkia_, _hlakia_ "id.", WFri. _laitsje_ "id." \
REF: [EDPG]\: 228

### <a id="hlankjan">\*hlankjan- [wk.] "to twist, turn" (v)</a>
W: \
-- OE _ge-hlencan_ "to braid" \
-- MHG _lenken_ "to bend" \
REF: [EDPG]\: 228

### <a id="hlanþōjan">\*hlanþōjan- [wk.] "to rob" (v)</a>
N: \
-- ON _hlanna_ "to rob" \
REF: [EDPG]\: 229

### <a id="hlaþan">\*hlaþan- [st.] "to load" (v)</a>
E: \
-- Go. _af-hlaþan_ "to burden, load down" \
N: \
-- ON _hlaða_ "to pile up; build; load" \
-- Far. _laða_ [st./wk.] "id." \
W: \
-- OE _hladan_ "to heap, pile up, build, load", E _to load_ \
-- OFri. _hleda_ "id." \
-- OS _hladan_ "id." \
-- Du. _laden_ [wk./st.] "id." \
-- OHG _ladan_, _hladan_ "id.", G _laden_ "id." \
REF: [EDPG]\: 229

### <a id="hlauni">\*hlauni- [f.] "loin, buttock" (n)</a>
N: \
-- ON _hlaun_ [n.] \
-- Nw. dial. _laun_ \
REF: [EDPG]\: 229

### <a id="hlaupan">\*hlaupan- [st.] "to jump, leap" (v)</a>
E: \
-- Go. _us-hlaupan_ "to leap forth, jump up" \
N: \
-- ON _hlaupa_ "to leap, jump; to run" \
-- Far. _leypa_ "to jump; to fall, tumble; to curdle" \
-- Elfd. _lopa_ "to leap, run; to creep" \
W: \
-- OE _hlēapan_ "to leap, jump, dance, run", E _to leap_ \
-- OFri. _hlāpa_ "to walk, run; to flow" \
-- OS _hlōpan_ "to run" \
-- Du. _lopen_ "to walk, run" \
-- OHG _loufan_, _hloufan_ "id.", G _laufen_ "id." \
REF: [EDPG]\: 229

### <a id="hlauta">\*hlauta- [m.] "lot" (n)</a>
E: \
-- Go. _hlauts_ \
N: \
-- ON _hlautr_ \
W: \
-- OS _hlōt_ \
-- OHG _lōz_, _hlōz_ \
REF: [EDPG]\: 230

### <a id="hlefan">\*hlefan- [st.] "to steal" (v)</a>
E: \
-- Go. _hlifan_ \
REF: [EDPG]\: 230

### <a id="hleuman">\*hleuman- [m.] "hearing" (n)</a>
E: \
-- Go. _hliuma_ \
N: \
-- ON _hljómr_ "sound, tone" \
-- Far. _ljómur_ "id." \
REF: [EDPG]\: 230

### <a id="hleutan">\*hleutan- [st.] "to obtain by lot" (v)</a>
N: \
-- ON _hljóta_ "to get, obtain; must" \
-- Far. _ljóta_ "to get as one's share; to be obliged, compelled to" \
W: \
-- OE _hlēotan_ "to cast lots; to obtain by lot" \
-- OS _hliotan_ "to acquire, receive" \
-- OHG _liozan_, _hliozan_ "to draw lots", MHG _liezen_ "to draw lots; to soothsay, perform magic" \
REF: [EDPG]\: 230

### <a id="hleuþa">\*hleuþa- [n.] "listening" (n)</a>
E: \
-- Go. _hliuþ_\* "hearing" \
N: \
-- ON _hljóð_ "listening; silence; sound" \
-- Far. _ljoð_ "id." \
-- Elfd. _liuoð_ [f.] "sound" \
REF: [EDPG]\: 230

### <a id="hleuza">\*hleuza- [n.] "cheek" (n)</a>
N: \
-- ON _hlýr_ "cheek; bow of a vessel; flat of an axe" \
-- Icel. _hlýr_ "cheek; bow of a vessel; flat of an axe" \
W: \
-- OE _hlēor_ "cheek, face" \
-- OS _hlior_ "cheek" \
-- MDu. _lier_ "cheek, cheekbone" \
REF: [EDPG]\: 230

### <a id="hlewa">\*hlewa- [n.] "shelter (from wind)" (n)</a>
N: \
-- ON _hlé_ "shelter, lee" \
W: \
-- OE _hlēo(w)_ "id.", ME _lē_ "id." \
-- OFri. _hlī_ [m./n.]? "id." \
-- OS _hlēo_ "concealment" \
-- EDu. _lije_ "lee side", Du. _lij-zijde_ "id." \
REF: [EDPG]\: 231

### <a id="hlēwa">\*hlēwa- "lukewarm" (adj)</a>
N: \
-- Icel. _hlár_ "melted; warm" \
W: \
-- MDu. _laeu_, Du. _lauw_ \
-- OHG _lāo_, MHG _lā_ \
REF: [EDPG]\: 231

### <a id="hlinēn">\*hlinēn- [wk.] "to lean" (v)</a>
W: \
-- OE _hlinian_, _hleonian_ \
-- OS _hlinon_ \
-- Du. _leunen_ \
-- OHG _linēn_, _hlinēn_, G _lehnen_ \
REF: [EDPG]\: 231

### <a id="hlōan">\*hlōan- [st.] "to low, bellow" (v)</a>
W: \
-- OE _hlōwan_, E _to low_ \
-- OS _hlōinga_ [f.] "bellowing" \
-- ODu. _luon_, _luogin_ "id.", Du. _loeien_ [wk.] "id." \
-- OHG _luoen_ "id.", MHG _lüejen_ [wk.] "id." \
REF: [EDPG]\: 231

### <a id="hlōgjan">\*hlōgjan- [wk.] "to make laugh" (v)</a>
E: \
-- Go. _uf-hlohjan_\* \
N: \
-- ON _hlœgja_ \
-- Icel. _hlægja_ \
-- Nw. dial. _løgja_ \
REF: [EDPG]\: 231

### <a id="hlūda">\*hlūda- "loud" (adj)</a>
W: \
-- OE _hlūd_, E _loud_ \
-- OFri. _hlūd_, WFri. _lûd_ \
-- OS _hlūd_ \
-- Du. _luid_ \
-- OHG _lūt_, _hlūt_, G _laut_ \
REF: [EDPG]\: 231

### <a id="hluni">\*hluni- [m.] "maple tree" (n)</a>
N: \
-- ON _hlynr_ \
-- 0Sw. _løn_ [m.(?)], Sw. _lōn_ [c.] \
W: \
-- OE _hlyn_ "id.(?)" \
-- MLG _lonen-holt_ [n.] "maple wood" \
REF: [EDPG]\: 232

### <a id="hluppjan">\*hluppjan- [wk.] "to lift" (v)</a>
W: \
-- MHG _lupfen_, _lüpfen_ "to lift; to rush" \
REF: [EDPG]\: 232

### <a id="hluppōn">\*hluppōn- [wk.] "to run" (v)</a>
W: \
-- MDu. _loppen_ "to run" \
REF: [EDPG]\: 232

### <a id="hlusēn">\*hlusēn- [wk.] "to listen" (v)</a>
W: \
-- OHG _losēn_, _hlosēn_ \
REF: [EDPG]\: 232

### <a id="hlusti">\*hlusti- [f.] "hearing, listening" (n)</a>
N: \
-- ON _hlust_ [n.] "ear" \
-- Far. _lust_ "opening of the auditory meatus" \
W: \
-- OE _hlyst_ [m./f.] "hearing" \
-- OS _hlust_ "ear, hearing, listening" \
REF: [EDPG]\: 233

### <a id="hluti">\*hluti- [m.] "share" (n)</a>
N: \
-- ON _hlutr_ "lot; share, part" \
-- Far. _lutur_ "id." \
W: \
-- E _lot_ \
-- OHG _luz_, _hluz_ "id.", MHG _luz_ "share of land" \
REF: [EDPG]\: 233

### <a id="hluza">\*hluza- [n.] "listening" (n)</a>
N: \
-- ON _hlør_, _hler_ "listening, eavesdropping" \
-- Icel. _hler_ "id." \
-- Far. _ler_ "waiting" \
-- Nw. dial. _standa å lere_ "to stand and listen; to peak" (cf. Icel. _standa á hleri_ "to stand eavesdroping") \
REF: [EDPG]\: 233

### <a id="hnaigjan">\*hnaigjan- ~ \*hnaiwjan- [wk.] "to make bow (down)" (v)</a>
E: \
-- Go. _hnaiwjan_ "to make go down; to humble" \
N: \
-- ON _hneigja_ "to make bow, incline; to lean" \
W: \
-- OE _hnǣgan_ "to humble" \
-- OS _gi-hnēgian_ "to bend down" \
-- Du. _neigen_ "to incline" \
-- OHG _neigan_, _hneiken_ "to bow", G _neigen_ "to tend, to incline, to bend" \
REF: [EDPG]\: 233

### <a id="hnaskwu">\*hnaskwu- "soft" (adj)</a>
E: \
-- Go. _hnasqus_ "soft, delicate" \
W: \
-- OE _hnesce_ "nesh, soft, delicate, tender, effeminate", E _nesh_ "soft, tender" \
-- WFri. _nesk_ "delicate" \
-- MDu. _nesch_, _nisch_ "soft" \
REF: [EDPG]\: 233

### <a id="hnawwu">\*hnawwu- "stingy; tight" (adj)</a>
N: \
-- ON _hnøggr_ "stingy" \
W: \
-- OE _hnēaw_ "stingy; near" \
-- MDu. _nauwe_ "tight; precise", Du. _nauw_ "id." \
-- MHG _nou_ "id.", G _ge-nau_ "precisely" (adv) \
REF: [EDPG]\: 234

### <a id="hnefan">\*hnefan- [m.] "fist" (n)</a>
N: \
-- ON _hnefi_ (>> ME _nēve_, E _nieve_) \
-- Far. _nevi_ \
-- Elfd. _nevi_ "hand" \
W: \
-- MHG _neve-mez_ "a fistful" \
REF: [EDPG]\: 234

### <a id="hnekkan">\*hnekkan- ~ \*hnakka(n)- [m.] "neck" (n)</a>
N: \
-- ON _hnakki_ \
-- Far. _nakki_ "neck; face of a hammer; top of a rock drill" \
-- Elfd. _nakke_ \
W: \
-- OE _hnecca_, E _neck_ \
-- OFri. _hnekka_ \
-- ODu. _nec_, _nac_, Du. _nek_ [c.] \
-- OHG _nack_, _hnack_, G _Nacken_ \
REF: [EDPG]\: 234

### <a id="hneudan">\*hneudan- [st.] "to rivet" (v)</a>
N: \
-- ON _hnjóða_ "to beat; to rivet, clinch" \
W: \
-- OHG _bi-hneotan_ "to fasten, rivet" \
REF: [EDPG]\: 235

### <a id="hneupan">\*hneupan- [st.] "to pluck" (v)</a>
E: \
-- Go. _dis-hniupan_ "to break apart, tear" \
N: \
-- OSw. _niupa_ "to pinch" \
-- Elfd. _niųopa_ "id." \
W: \
-- OE _ā-hnēopan_ "to pluck off", E _to nip_ \
REF: [EDPG]\: 235

### <a id="hnewwan">\*hnewwan- [st.] "to hit" (v)</a>
N: \
-- ON _hnøggva_, _hnyggja_ "to strike, cut off" \
-- Elfd. _negga_ "to butt" \
W: \
-- OHG _niuwan_ "to thrust; to grind" \
REF: [EDPG]\: 235

### <a id="hnīpan">\*hnīpan- [st.] "to pinch, squeeze" (v)</a>
W: \
-- MLG _nīpen_ \
-- MDu. _nipen_ "the squeeze; to tear off (with nails or teeth); to snap", Du. _nijpen_ [wk.] "to press" \
REF: [EDPG]\: 235

### <a id="hnipōn">\*hnipōn- [wk.] "to be downcast" (v)</a>
N: \
-- ON _hnipa_ "to be downcast, droop" \
REF: [EDPG]\: 235

### <a id="hnippōn">\*hnippōn- [wk.] "to pinch" (v)</a>
N: \
-- ON _hnippa_ "to poke" \
W: \
-- ME _nippen_ "to nip; to pinch; to grab", E _to nip_ \
REF: [EDPG]\: 236

### <a id="hnissa">\*hnissa- [m.] "smell" (n)</a>
N: \
-- Icel. _hniss_ [n.] "smell (from cooking)" \
-- Far. _nissur_ "(funny) taste, smell" \
-- Nw. dial. _niss_ "smell" \
REF: [EDPG]\: 236

### <a id="hnit">\*hnit- [f.] "nit" (n)</a>
N: \
-- ON _gnit_ \
-- Icel. _gnit_, _nit_ \
-- Far. _nitrur_ pl. \
-- Elfd. _gnit_ \
W: \
-- OE _hnitu_, E _nit_ \
-- MDu. _nete_, Du. _neet_ [c.] \
-- OHG _niz_, _hniz_, G _Niß_ \
REF: [EDPG]\: 236

### <a id="hnītan">\*hnītan- [st.] "to poke, butt" (v)</a>
N: \
-- ON _hníta_ [wk.] "to poke" \
W: \
-- OE _hnītan_ "to bump, stick" \
-- OS _of-hnītan_ "to tear off" \
-- Du. dial. _nijten_ [wk.] "to butt (of animals)" \
REF: [EDPG]\: 236

### <a id="hnīwan">\*hnīwan- ~ \*hnīgan- [st.] "to bow (down)" (v)</a>
E: \
-- Go. _hneiwan_ \
N: \
-- ON _hníga_ "to bow down, fall gently" \
-- Far. _níga_ "to bend, stoop, drop; to bow" \
W: \
-- OE _hnīgan_ "to bend, bow down, incline" \
-- OFri. _hnīgan_ "to bow down" \
-- OS _hnīgan_ "id." \
-- Du. _nijgen_ "id." \
-- OHG _nīgan_, _hnīgan_ "id.", MHG _nīgen_ "id." \
REF: [EDPG]\: 236

### <a id="hnōjan">\*hnōjan- [wk.] "to groove" (v)</a>
W: \
-- OHG _nōen_, MHG _nūejen_ \
REF: [EDPG]\: 237

### <a id="hnuppōn">\*hnupp/bōn- [wk.] "to tear off" (v)</a>
N: \
-- Elfd. _nuppa_ \
W: \
-- OE _hnoppian_ \
-- G Alem. _noppen_ "to jerk, pluck" \
REF: [EDPG]\: 237

### <a id="hnut">\*hnut- [f.] "nut" (n)</a>
N: \
-- ON _hnot_, pl. _hnøtr_, _hnetr_ \
-- Icel. _hneta_ \
-- Far. _nøta_ \
-- OSw. _nut_, pl. _nyter_, _nøter_ \
W: \
-- OE _hnutu_, pl. _hnyte_, E _nut_ \
-- MDu. _note_, Du. _noot_ [c.] \
-- OHG _nuz_, pl. _nuzzi_, G _Nuss_ \
REF: [EDPG]\: 237

### <a id="hnuttōn-1">\*hnutt/dōn- [wk.] "to beat" (v)</a>
N: \
-- Icel. _hnoða_ "to rivet" \
W: \
-- G Hess. _nussen_ "to beat" \
REF: [EDPG]\: 238

### <a id="hnuttōn-2">\*hnutt/dōn- [wk.] "to shake" (v)</a>
N: \
-- Elfd. _noðå_ "to sprain, wrench" \
W: \
-- ME _nodden_ "to nod", E _to nod_ \
-- MDu. _noten_ "to mate (of animals)" \
-- OHG _hnotōn_ "to shake", MHG _notten_ "to move to and fro", G Lorr. _nottelen_ "to stagger, shudder" \
REF: [EDPG]\: 238

### <a id="hōbō">\*hōbō- [f.] "piece of land" (n)</a>
W: \
-- MLG _hōve_ \
-- MDu. _hoeve_, Du. _hoeve_ [c.] "farm" \
-- OHG _huoba_ "piece of land", G _Hube_ "hide, unit of area" \
REF: [EDPG]\: 238

### <a id="hōda">\*hōda- [m.] "hat, hood" (n)</a>
W: \
-- OE _hōd_ "hood", E _hood_ \
-- MDu. _hoed_ "hat", Du. _hoed_ [c.] "id." \
-- OHG _huot_ "hood, bonnet, hat", G _Hut_ "hat" \
REF: [EDPG]\: 238

### <a id="hōfa">\*hōfa- [m.] "hoof" (n)</a>
N: \
-- ON _hófr_ \
-- Far. _hóvur_ \
-- Elfd. _uov_ [f.] \
W: \
-- OE _hōf_, E _hoof_ \
-- OFri. _hōf_ \
-- OS _hōf_ \
-- MDu. _hoef_, Du. _hoef_ [c.] \
-- OHG _huof_, G _Huf_ \
REF: [EDPG]\: 238

### <a id="hōhan">\*hōhan- [m.] "plow" (n)</a>
E: \
-- Go. _hoha_ \
REF: [EDPG]\: 239

### <a id="hōkīna">\*hōkīna- [n.] "kid, young goat" (n)</a>
W: \
-- OE _hēcen_ \
-- MDu. _hoekijn_ \
REF: [EDPG]\: 239

### <a id="hōla">\*hōla- [n.] "flattery(?)" (n)</a>
N: \
-- ON _hól_ "praise, flattery" \
W: \
-- OE _hōl_ "vain speech, calumny, slander" \
REF: [EDPG]\: 239

### <a id="hōniz">\*hōniz- [n.] "hen, fowl" (n)</a>
W: \
-- OS _hōn_ "hen" \
-- Du. _hoen_ [c.] "id." \
-- OHG _huon_ "id.", G _Huhn_ "id." \
REF: [EDPG]\: 240

### <a id="hōpa">\*hōpa- [n.] "small bay(?)" (n)</a>
N: \
-- ON _hóp_ "land-locked bay, small inlet" \
-- Far. _hópur_ top. "small bay" \
W: \
-- OE _hōp_ "pool" \
REF: [EDPG]\: 240

### <a id="hōra-1">\*hōra- [m.] "adulterer" (n)</a>
E: \
-- Go. _hors_ \
N: \
-- ON _hórr_ \
REF: [EDPG]\: 240

### <a id="hōra-2">\*hōra- [n.] "adultery" (n)</a>
N: \
-- ON _hór_ \
W: \
-- OE _hōr_ \
-- OFri. _hōr_ \
-- OS in: _hōr-hūs_ "brothel" \
-- OHG _huor_, MHG _huor_ \
REF: [EDPG]\: 240

### <a id="hōrōn">\*hōrōn- [f.] "whore" (n)</a>
N: \
-- ON _hóra_ \
W: \
-- OE _hōre_, E _whore_ \
-- OFri. _hōr_ \
-- MLG _hōre_ \
-- MDu. _hoere_, Du. _hoer_ [c.] \
-- OHG _huora_, G _Hure_ \
REF: [EDPG]\: 240

### <a id="hraban">\*hrab/ppan- [m.] "raven" (n)</a>
N: \
-- ON _hrafn_ \
-- Far. _ravnur_ \
W: \
-- OE _hræfn_, _hræm_ \
-- OS _naht-raƀan_, -_ram_ \
-- MDu. _rave_, Du. _raaf_ [c.] \
-- OHG _rabo_, _hrabo_, _rappo_, _hraban_, G _Rabe_ \
REF: [EDPG]\: 240

### <a id="hragjan">\*hragjan- [wk.] "to make stand, wake up, incite" (v)</a>
W: \
-- MDu. _regen_ \
-- MHG _regen_, G _regen_ \
REF: [EDPG]\: 241

### <a id="hragōn">\*hrag/kkōn- [wk.] "to jut out" (v)</a>
W: \
-- OE _ofer-hragan_ "to loom over(?)" \
-- MDu. _ragen_ "id." \
-- OHG _hragēn_ "id.", MHG _ragen_, _racken_ "id.", G _ragen_ "id." \
REF: [EDPG]\: 241

### <a id="hraifan">\*hraifan- [m.] "wrist(?)" (n)</a>
N: \
-- ON _hreifi_ "wrist; the hand and fingers; seal's fin, flipper" \
REF: [EDPG]\: 241

### <a id="hraigran">\*hraigran- ~ \*higran- [m.] "heron" (n)</a>
N: \
-- ON _hegri_ \
-- Far. _hegri_ \
-- Nw. _hegre_, dial. _higre_, _heigre_ \
-- OSw. _hægher_ \
-- Elfd. _āger_ \
-- Da. _hejre_ \
W: \
-- OE _hrāgra_ \
-- OS _hēgero_ \
-- MDu. _heger_, _reiger_, Du. _reiger_ \
-- OHG _reigar_, _hreigir_, _heigar_, _heigiro_, MHG _heiger_, G _Reiher_ \
REF: [EDPG]\: 241

### <a id="hraini">\*hraini- "pure, clean" (adj)</a>
E: \
-- Go. _hrains_ \
N: \
-- ON _hreinn_ \
-- Far. _reinur_ \
-- Elfd. _rien_ \
W: \
-- OS _hrēni_ \
-- Du. _rein_ \
-- OHG _reini_, _hreini_, G _rein_ \
REF: [EDPG]\: 241

### <a id="hrairja">\*hrairja- "rear" (adj)</a>
W: \
-- OE _hrēre-brǣd_ "rear (esp. of eggs)", ME _hrere_, _rere_ "id.", E _rear_ \
REF: [EDPG]\: 241

### <a id="hraiwa">\*hraiwa- [n.] "dead body, death" (n)</a>
E: \
-- Go. _hraiwa-dubo_ [f.] "turtle-dove, wild dark dove" \
N: \
-- ON _hræ_ "corpse, remains" \
W: \
-- OE _hrǣ(w)_, _hrā(w)_ "id." \
-- OFri. _hrē_- "id." \
-- OS _hrēu_, _hrēo_ "id." \
-- Du. _reeuw_ "foam or sweat of a dying person" \
-- OHG _rēo_, _hrēo_ "id.", MHG _rē_ "id." \
REF: [EDPG]\: 242

### <a id="hrakjan">\*hrakjan- [wk.] "to torment" (v)</a>
N: \
-- ON _hrekja_ "to drive away; to worry, vex; to damage, abuse" \
REF: [EDPG]\: 242

### <a id="hraman">\*hraman- [m.] "frame" (n)</a>
W: \
-- MDu. _rame_, _raem_ [m./f.] "window, frame", Du. _raam_ [c./n.] "id." \
-- OHG _rama_ [f.] "prop, support", G _Rahmen_ "frame" \
REF: [EDPG]\: 242

### <a id="hramusan">\*hramusan- / \*hramusjon- [m./f.] "ramsons" (n)</a>
W: \
-- OE _hromsa_ [m.], E _ramsons_ \
-- OS _hramusia_ [f.], MLG _ramese_, _remese_ (>> Nw., Da. _rams_, Sw. _rams-lōk_) \
-- G Swi. (Jaun) _ramsa_ [f.] \
REF: [EDPG]\: 242

### <a id="hrappa">\*hrappa- "fast" (adj)</a>
W: \
-- MDu. _rap_ "swift, fierce", Du. _rap_ "fast" \
REF: [EDPG]\: 243

### <a id="hrattōn">\*hratt/dōn- [wk.] "to rush, tumble" (v)</a>
N: \
-- ON _hrata_ "to stagger, fall, tumble" \
W: \
-- OE _hratian_, _hradian_ "to rush, hasten" \
-- MHG _razzen_ "to rush" \
REF: [EDPG]\: 243

### <a id="hraþa">\*hraþa- ~ \*hrada- "fast" (adj)</a>
N: \
-- ON _hraðr_ \
W: \
-- OE _hræd_, _hræð_, E _rather_ comp. \
-- OHG _rad_, _hrad_ \
REF: [EDPG]\: 243

### <a id="hrauka">\*hrauka- [m.] "pile, rick" (n)</a>
N: \
-- ON _hraukr_ "pile" \
-- Far. _reykur_ "bird's crest" \
W: \
-- OE _hrēac_ "stack, haycock, rick", E _rick_ \
-- Du. _rook_ [c.] "id." \
REF: [EDPG]\: 243

### <a id="hrauza">\*hrauza- [n.] "artificial stone heap" (n)</a>
N: \
-- ON _hreyrr_ [m.] \
-- Icel. _hreyr_ [m./n.] "megalithic grave, cairn, mound" \
-- Nw. _rø(y)r_ [f.] "edge, side" \
-- OSw. _rør_ "cairn", Sw. _rōr_ "pile of stones" \
-- Da. _rør_ "cairn, pile of stones" \
REF: [EDPG]\: 244

### <a id="hrawa">\*hrawa- "raw" (adj)</a>
N: \
-- ON _hrár_ \
-- Far. _ráur_ "moist" \
-- Elfd. _rå(r)_ "raw; moist" \
W: \
-- OE _hræw_, _hreaw_, E _raw_ \
-- Du. _rauw_ \
-- OHG _rao_, _rō_, G _roh_ \
REF: [EDPG]\: 244

### <a id="hrefiz">\*hrefiz- [n.] "stomach" (n)</a>
W: \
-- OE _hrif_ \
-- OS _in-hrif_ "innards, guts" \
-- MDu. _rif_ "body" \
-- OHG _ref_, _href_ "body; womb" \
REF: [EDPG]\: 244

### <a id="hreflinga">\*hreflinga- [m.] "shoe" (n)</a>
N: \
-- ON _hriflingr_ \
W: \
-- OE _hrifeling_ \
REF: [EDPG]\: 244

### <a id="hregan">\*hregan- [st.] "to jut out" (v)</a>
W: \
-- MHG _regen_ "to jut out, stand upright" \
REF: [EDPG]\: 244

### <a id="hrēhi">\*hrēhi- "protruding" (adj)</a>
W: \
-- MHG _ræhe_ \
REF: [EDPG]\: 245

### <a id="hretō">\*hrētō- [f.] "honeycomb" (n)</a>
W: \
-- ODu. _rāta_, MDu. _rate_, _raet_, Du. _honing-raat_ \
-- MHG _rāze_ "honeycomb, stake, stake" \
REF: [EDPG]\: 245

### <a id="hreþan">\*hreþan- [st.] "to sift" (v)</a>
W: \
-- OHG _redan_ \
REF: [EDPG]\: 245

### <a id="hreþra">\*hreþra-(?) [m.] "bat" (n)</a>
N: \
-- Icel. _leður-blaka_ [f.] \
-- Far. _leður-bløka_ \
-- Elfd. _leðer_ \
W: \
-- OE _hrēaðe-mūs_, _hrēre-mūs_ [f.], E obs. _rear-mouse_ \
-- Du. _vleer-muis_ [c.] \
-- OHG _fledar-mūs_, G _Fleder-maus_ [f.] \
REF: [EDPG]\: 245

### <a id="hreþrja">\*hreþrja- [n.] "diaphragm" (n)</a>
W: \
-- OE _mid-hriðere_ \
-- OFri. _mid-rith_, -_rhether_ \
REF: [EDPG]\: 246

### <a id="hreuba">\*hreuba- "scabby" (adj)</a>
N: \
-- ON _hrjúfr_ "rough, scurvy" \
W: \
-- OE _hrēof_ "rough, rugged, scabby, leprous" \
-- OHG _riob_ "leprous" \
REF: [EDPG]\: 246

### <a id="hreuda">\*hreuda- [n.] "reed" (n)</a>
W: \
-- OE _hrēod_, E _reed_ \
-- OS _hriod_ \
-- Du. _riet_ (with _Auslautverhärtung_ lexified because of its occurrence as _singulare tantum_) \
-- OHG _hriot_, G _Riet_ \
REF: [EDPG]\: 246

### <a id="hreutan">\*hreutan- ~ \*hrūtan- [st.] "to snore" (v)</a>
N: \
-- ON _hrjóta_ \
-- Far. _rjóta_, _róta_ \
-- Elfd. _riuota_ "to bellow, low" \
W: \
-- OE _hrūtan_ "to make a noise, to snore", E dial. _to rout_ "to cry out, roar, bellow; to low, moo loudly" \
-- OFri. _hrūta_\* "to rattle (in the throat)" \
-- OS _hrūtan_(?) \
-- OHG _rūzzan_ \
REF: [EDPG]\: 246

### <a id="hrewan">\*hrewan- [st.] "to be sad, make sad" (v)</a>
N: \
-- Icel. _hrjá_ [wk.] "to afflict, vex" \
W: \
-- OE _hrēowan_ "to make sorry, grieve", E to rue \
-- OS _hreuwan_ "to mourn, be painful" \
-- Du. _rouwen_ [wk.] "to mourn" \
-- OHG _riuwan_ "to make/be sad", MHG _riuwen_, _riwen_ "to sadden, to be sad", G _reuen_ [wk.] "to regret" \
REF: [EDPG]\: 246

### <a id="hrīdra">\*hrīdra- [n.] "sieve" (n)</a>
W: \
-- OE _hrīdder_, _hrīddel_, E _riddle_ \
-- OHG _rītera_ [f.], MHG _riter(e)_ [f.], G _Reiter_ [f.] (< \*hridrō-) \
REF: [EDPG]\: 247

### <a id="hrīman">\*hrīman- ~ \*hrīpan- [m.] "hoar frost, rime" (n)</a>
N: \
-- ON _hrím_ [n.] \
-- Far. _rím_ [n.] \
W: \
-- OE _hrīm_, E _rime_ \
-- OS _hrīpo_ \
-- Du. _rijp_, _rijm_ [c.] \
-- OHG _rīfo_, _hrīfo_, G _Reif_ \
REF: [EDPG]\: 247

### <a id="hrīnan">\*hrīnan- [st.] "to touch, affect" (v)</a>
N: \
-- ON _hrína_ [wk.] "to take effect" \
W: \
-- OE _hrīnan_ "to touch, reach, strike" \
-- OS _hrīnan_ "to touch, lay one's hand on" \
-- OHG _rīnan_, _hrīnan_ "to touch, get hold of", MHG _be-rīnen_ "to touch" \
REF: [EDPG]\: 247

### <a id="hringa">\*hringa- [m.] "ring, circle" (n)</a>
N: \
-- ON _hringr_ \
-- Far. _ringur_ \
-- Elfd. _ringg_ \
W: \
-- OE _hring_, E _ring_ \
-- OFri. _hring_ \
-- OS _hring_ \
-- Du. _ring_ [c.] \
-- OHG _ring_, _hring_, MHG _rinc_ "ring", G _Ring_ "id." \
REF: [EDPG]\: 247

### <a id="hrinþiz">\*hrinþiz- ~ \*hrunþiz- [n.] "cow, bovine" (n)</a>
W: \
-- OE _hrīðer_, _hrȳðer_ \
-- OFri. _hrīther_ \
-- Du. _rund_ \
-- OHG _rint_, _hrint_, G _Rind_ \
REF: [EDPG]\: 247

### <a id="hrīsa">\*hrīsa- [n.] "brushwood" (n)</a>
N: \
-- ON _hrís_ \
-- Far. _rís_ \
-- Elfd. _rais_ "brushwood; wood" \
W: \
-- OE _hrīs_ "branch, brush" \
-- MDu. _rijs_ "branch" \
-- OHG _rīs_, _hris_ "twig, branch; shrubs", G Reis "scion, sprig" \
REF: [EDPG]\: 248

### <a id="hrispōn">\*hrispōn- [f.] "curl" (n)</a>
W: \
-- MLG _rispe_ "truss" \
-- MHG _rispe_ "id." \
REF: [EDPG]\: 248

### <a id="hrīþan">\*hrīþan- ~ \*hrittan- [m.] "fever, shivering" (n)</a>
W: \
-- OS _hrido_ \
-- MDu. _ridde_, _rede_, EDu. _rijde_, _ridde_ \
-- OHG _rīdo_, _rit(t)o_, MHG _rite_, _ritte_ \
REF: [EDPG]\: 248

### <a id="hrōfa">\*hrōfa- [n.] "roof" (n)</a>
N: \
-- ON _hróf_ "shed under which ships are built" \
W: \
-- OE _hrōf_ [m.] "id.", E _roof_ \
-- OFri. _hrōf_ [m.(?)] "id." \
-- MLG _rōf_ [m./n.] "cover, lid, roof of the rear deck of a ship, shelter" \
-- MDu. _roef_ [m./n.] "lean-to", Du. _roef_ [c.] "id." \
REF: [EDPG]\: 248

### <a id="hrōkan">\*hrōka(n)- [m.] "rook, jackdaw" (n)</a>
N: \
-- ON _hrókr_, _hraukr_ "rook" \
-- Icel. _hraukur_ "id." \
-- Far. _rókur_ "jackdaw" \
W: \
-- OE _hrōc_ "rook, raven, jackdaw", E _rook_ \
-- OS _hrōk_ "id." (also _hrōka_ [f.] "id." < \*hrōkōn-) \
-- Du. _roek_ [c.] "id." \
-- OHG _ruoh_, _ruoho_ "crow", MHG _ruoch_, _ruoche_ "id." \
REF: [EDPG]\: 248

### <a id="hrōma">\*hrōma- [m.] "fame" (n)</a>
W: \
-- OS _hrōm_ \
-- EDu. _roem_ [c.] \
-- OHG _ruom_, _hruom_, G _Ruhm_ \
REF: [EDPG]\: 249

### <a id="hrōpan">\*hrōpan- [st.] "to call, shout" (v)</a>
W: \
-- OE _hrōpan_ \
-- OFri. _(h)rōpa_ \
-- OS _hrōpan_ \
-- Du. _roepen_ \
-- OHG _ruofan_, _hruofan_, MHG _ruofen_, G _rufen_ [st./wk.] \
REF: [EDPG]\: 249

### <a id="hrōta-1">\*hrōta- [n.] "roof" (n)</a>
E: \
-- Go. _hrot_ \
N: \
-- ON _hrót_ \
REF: [EDPG]\: 249

### <a id="hrōta-2">\*hrōta- [m.] "soot" (n)</a>
W: \
-- OS _hrot_ \
-- MDu. _roet_ [n.] "grease, soot", Du. _roet_ [n.] "soot" \
-- OHG _ruoz_, G _Ruß_, \
REF: [EDPG]\: 249

### <a id="hrōþi">\*hrōþi- [m.] "glory" (n)</a>
E: \
-- Go. in: _hroþeigs_ "glorious" \
W: \
-- OE _hrēð_ \
REF: [EDPG]\: 249

### <a id="hrōþra">\*hrōþra- [m.] "praise" (n)</a>
N: \
-- ON _hróðr_ \
W: \
-- OE _hrōðor_ "solace, benefit, pleasure" \
REF: [EDPG]\: 249

### <a id="hrōza">\*hrōza- "motile" (adj)</a>
W: \
-- OE _hrōr_ "stirring, active, vigorous" \
-- OS _hrōr_ "lively" \
-- MDu. _roer_ "mobile" \
REF: [EDPG]\: 249

### <a id="hrōzjan">\*hrōzjan- [wk.] "to touch, move, stire" (v)</a>
N: \
-- ON _hrœra_ \
-- Far. \*_røra_ \
W: \
-- MHG _ruoren_, _rüeren_ "to move" \
REF: [EDPG]\: 250

### <a id="hrudjan">\*hrudjan- [wk.] "to shake" (v)</a>
W: \
-- MHG _rütten_, G _rütteln_ \
REF: [EDPG]\: 250

### <a id="hrugan">\*hrugan- [m.] "fishroe" (n)</a>
N: \
-- ON _hrogn_ [n.] \
-- Far. _rogn_ [n.] \
-- OSw. _rughn_, _romn_ [n.(?)] \
-- Sw. _rom_ [c.] \
-- Elfd. _rungen_ \
W: \
-- MDu. _roge_, EDu. _roghe_ \
-- OHG _rogo_, _rogan_, MHG _roge_, _rogen_, G _Rogen_ \
REF: [EDPG]\: 250

### <a id="hrugja">\*hrugja- [m.] "ridge; back" (n)</a>
N: \
-- ON _hryggr_ "back" \
-- Far. _ryggur_ "id." \
-- Elfd. _ryg_ "id." \
W: \
-- OE _hrycg_ "back, spine, ridge", E _ridge_ \
-- OFri. _hreg_ "back", WFri. _rêch_, _rich_ [c.] "id." \
-- OS _hruggi_ "id." \
-- ODu. _ruggi_ "id.", Du. _rug_ [c.] "id." \
-- OHG _rucki_, _hrucki_ "id.", G _Rücken_ "id." \
REF: [EDPG]\: 250

### <a id="hrukka">\*hrukka- [m.] "upper garment, robe" (n)</a>
N: \
-- ON _rokkr_ \
W: \
-- OE _roc_ \
-- OS _hrok_ \
-- Du. _rok_ [c.] "skirt" \
-- OHG _rock_ "id.", G _Rock_ "skirt; coat" \
REF: [EDPG]\: 250

### <a id="hrungō">\*hrungō- [f.] "rung" (n)</a>
E: \
-- Go. _hrugga_ "staff, cane" \
W: \
-- OE _hrung_ "staff, rod, beam", E _rung_ \
-- MLG _runge_ "stanchion" \
-- MHG _runge_ "stake", G _Runge_ "id." \
REF: [EDPG]\: 251

### <a id="hrusōn">\*hrusōn- [f.] "crust" (n)</a>
W: \
-- OE _hruse_ "earth, ground" \
-- OHG _rosa_ "crust, ice" \
REF: [EDPG]\: 251

### <a id="hruttōn">\*hrut(t)ōn- [wk.] "to snore, roar" (v)</a>
N: \
-- Elfd. _rotå_ "to babble" \
W: \
-- G _rotzen_ "to blow the nose" \
REF: [EDPG]\: 251

### <a id="hruþjan">\*hruþjan- [m.] "male dog" (n)</a>
W: \
-- OE _hryðða_ \
-- MLG _rude_ \
-- Du. _reu_ [c.] \
-- OHG _rudo_, G _Rüde_ \
REF: [EDPG]\: 251

### <a id="hūdi">\*hūdi- [f.] "skin" (n)</a>
N: \
-- ON _húð_ \
-- Far. _húð_ \
-- Elfd. _auð_ \
W: \
-- OE _hȳd_ \
-- OFri. _hēd(e)_ \
-- OS _hūd_ \
-- ODu. _hūt_, Du. _huid_ [c.] \
-- OHG _hūt_, G _Haut_ \
REF: [EDPG]\: 251

### <a id="hufa">\*hufa- [n.] "hillock" (n)</a>
N: \
-- ON _hof_ "temple" \
-- Far. _hov_ "id." \
W: \
-- OE _hof_ "house, hall" \
-- OFri. _hof_ "court, yard" \
-- OS _hof_ [m.] "id." \
-- Du. _hof_ "id." \
-- OHG _hof_ [m.] "id.", G _Hof_ [m.] "id." \
REF: [EDPG]\: 252

### <a id="hufra">\*hufra- [m.] "bump, hillock" (n)</a>
W: \
-- OE _hofer_ \
-- MLG _hover_ \
-- OHG _hofar_, _hovar_ \
REF: [EDPG]\: 252

### <a id="hugi">\*hugi- [m.] "understanding, mind" (n)</a>
E: \
-- Go. _hugs_ \
N: \
-- ON _hugr_ "mind; mood; desire, wish" \
-- Far. _hugur_ "id." \
-- Elfd. _ug_ "id." \
W: \
-- OE _hyge_ "mind, heart, soul" \
-- OFri. _hei_ "mind" \
-- OS _hugi_ "thought, mind" \
-- MDu. _hoge_ [m./f.] "thought, consideration, memory", Du. _heug_ [c.] "desire" \
-- OHG _hugu_ "mind, spirit, courage", MHG _hüge_ "mind, spirit, memory" \
REF: [EDPG]\: 252

### <a id="hūkan">\*hūkan- [st.] "to squat" (v)</a>
N: \
-- ON _húka_ [wk.] \
-- Far. _húka_ \
-- OSw. _hūka_ [wk.] "to peddle" \
W: \
-- WPhal. _hûken_ "to crouch, cower, sit" \
-- MDu. _huken_ [wk.], Du. _huiken_ [wk.] \
-- MHG _hūchen_ [wk.] "to squat, stoop over" \
REF: [EDPG]\: 252

### <a id="hukkōn">\*huk(k)ōn- [wk.] "to squat, cower" (v)</a>
N: \
-- ON _hoka_ \
W: \
-- G _hocken_ \
REF: [EDPG]\: 252

### <a id="hula">\*hula- "hollow" (adj)</a>
N: \
-- ON _holr_ \
W: \
-- OE _hol_ \
-- OFri. _hol_ \
-- Du. _hol_ \
-- OHG _hol_, G _hohl_ \
REF: [EDPG]\: 253

### <a id="hulandra">\*hulandra- [m.] "elder" (n)</a>
W: \
-- OHG _holuntar_, _holantar_, G _Wasser-holder_ \
REF: [EDPG]\: 253

### <a id="hulisa">\*hulisa- [m.] "holly" (n)</a>
W: \
-- MDu. _huls_, Du. _hulst_ \
-- OHG _hulis_, _huls_, G _Hulse_ [f.] (< \*hulis(j)ōn-) \
REF: [EDPG]\: 253

### <a id="hullōn">\*hul(l)ōn- [wk.] "to move, fetch(?)" (v)</a>
N: \
-- Nw. dial. _hula_ "to bend over and reach forward, to pull oneself" \
W: \
-- MDu. _hollen_ "to run; to go out of control", Du. _hollen_ "to run" \
REF: [EDPG]\: 254

### <a id="hulman">\*hulma(n)- [m.] "small island" (n)</a>
N: \
-- ON _holmr_, _holmi_ \
-- Far. _hólmur_ "small island; battlefield" \
-- Elfd. _uome_ "islet" \
W: \
-- OE _holm_ "mound, hill, rising ground" \
-- OS _holm_ "hill" \
-- MDu. _holm_ "islet in a river; mound" \
REF: [EDPG]\: 254

### <a id="hulta">\*hulta- [n.] "grove" (n)</a>
N: \
-- ON _holt_ \
-- Elfd. _ult_ \
W: \
-- OE _holt_ [m./n.] \
-- OS _holt_ "wood" \
-- Du. _hout_ "id." \
-- OHG _holz_ "id.", G Holz "id." \
REF: [EDPG]\: 254

### <a id="hulþa">\*hulþa- "propitious" (adj)</a>
E: \
-- Go. _hulþs_ "kind, clement" \
N: \
-- ON _hollr_ "faithful, loyal; wholesome" \
-- Far. _hollur_ "faithful; good, favorable" \
W: \
-- OE _hold_ "clement, loyal" \
-- OFri. _hold_ "loyal" \
-- OS _hold_ "devoted, graceful" \
-- MDu. _houd_ "gracious, loyal" \
-- OHG _hold_ "propitious", G _hold_ "id." \
REF: [EDPG]\: 254

### <a id="humara">\*humara- [m.] "lobster" (n)</a>
N: \
-- ON _humarr_ \
-- Sw. _hummer_ [c.] \
REF: [EDPG]\: 254

### <a id="humela">\*humela- [m.] "bumblebee" (n)</a>
N: \
-- Elfd. _myōð-umbel_ [f.] \
W: \
-- ME _humbul-bē_ \
-- MLG _homele_, _homelte_ \
-- MDu. _homel_, Du. _hommel_ [c.] \
-- OHG _humbal_, _hummel_ \
REF: [EDPG]\: 255

### <a id="humelan">\*humelan- [m.] "hops" (n)</a>
N: \
-- ON _humli_ \
-- Far. _humli_ \
-- Elfd. _umbel_ \
W: \
-- OE _hymele_ [f.] (< \*humeljōn-) \
-- MDu. _hommel_ \
REF: [EDPG]\: 255

### <a id="hūnan">\*hūna(n)- [m.] "top of a stick" (n)</a>
N: \
-- ON _húnn_ "knob at the top of the mast-head" \
-- Icel. _húnn_ "door knob, door handle; knob on a flagpole; bear cub" \
-- Far. _húni_ "wooden staple or handle; lug in the middle of a wooden tub" (cf. _hún-band_ "hole high up in the mast through which the halyard passes") \
-- Nw. dial. _hun(e)_ "upper part of the mast; bear cub" \
W: \
-- OE _hūn-þyrel_ "hole in the mast-head for the halyard" \
-- MDu. _hune_ "crow's nest" \
REF: [EDPG]\: 255

### <a id="hunanga">\*hunanga- [m.] "honey" (n)</a>
N: \
-- ON _hunang_ [n.] \
-- Far. _hunang_ [n.], _hunangur_ \
-- OSw. _hunagh_ \
-- Elfd. _onungg_ \
W: \
-- OE _hunig_ [n.], E honey \
-- OS _honig_ \
-- Du. _honing_ [c.] \
-- OHG _hona(n)g_ [n.], G _Honig_ \
REF: [EDPG]\: 255

### <a id="hunda-1">\*hunda- [n.] "hundred" (n)</a>
E: \
-- Go. _hund_ \
W: \
-- OE _hund_ \
-- OS _hund_ [m.] \
-- OHG _hunt_, MHG _hunt_ \
REF: [EDPG]\: 256

### <a id="hunda-2">\*hunda- [m.] "dog" (n)</a>
N: \
-- ON _hundr_ \
-- Far. _hundur_ \
W: \
-- OE _hunt_, E _hound_ \
-- OFri. _hund_ \
-- OS _hund_ \
-- Du. _hand_ [c.] \
-- OHG _hunt_, MHG _hunt_, G _Hund_ \
REF: [EDPG]\: 256

### <a id="hungru">\*hungru- [m.] "hunger" (n)</a>
E: \
-- Go. _huhrus_ \
N: \
-- ON _hungr_ [m./n.] \
W: \
-- OE _hungor_, E _hunger_ \
-- OFri. _hunger_ \
-- OS _hungar_ \
-- Du. _hanger_ [c.] \
-- OHG _hungar_, G _Hunger_ \
REF: [EDPG]\: 256

### <a id="hunsla">\*hunsla- [n.] "sacrifice" (n)</a>
E: \
-- Go. _hunsl_ \
N: \
-- ON _húsl_ \
W: \
-- OE _hūsel_, E _housel_ \
REF: [EDPG]\: 256

### <a id="huntōn">\*huntōn- [wk.] "to chase" (v)</a>
W: \
-- OE _huntian_ "to hunt", E _to hunt_ \
REF: [EDPG]\: 257

### <a id="hūpan">\*hūpan- [m.] "pile, heap" (n)</a>
W: \
-- OHG _hūfo_, G _Haufen_ \
REF: [EDPG]\: 257

### <a id="hupi">\*hupi- [m.] "hip" (n)</a>
E: \
-- Go. _hups_ \
W: \
-- OE _hype_, E _hip_ \
-- MDu. _hope_, _hupe_ [f.], Du. _heup_ [c.] \
-- OHG _huf_ [f.], G _Hüfte_ [f.] (with secondary _t_) \
REF: [EDPG]\: 257

### <a id="huppōn-1">\*hupp/bōn- [wk.] "to move backwards; retreat" (v)</a>
N: \
-- ON _hopa_ "to turn back", Icel. _hopa_ "to fall back, retreat" \
-- Far. _hopa_ "to draw back, recede, retreat" \
-- Nw. dial. _hop(p)a_, _hobba_ "to retreat, drive backwards (esp. of horses)" \
-- Da. dial. _hoppe seg_ "to walk backwards, drive backwards" \
REF: [EDPG]\: 257

### <a id="huppōn-2">\*hupp/bōn- [wk.] "to hop, jump" (v)</a>
N: \
-- ON _hoppa_ "to hop, jump, skip" \
-- Elfd. _uppa_ "id." \
W: \
-- OE _hoppian_ "to hop, leap, dance", E _to hop_ \
-- MDu. _happen_, _hobben_ "to hop", Du. _hobbelen_ "to bounce" \
-- MHG _hopfen_ "to hop", G _hopfen_ "id." \
REF: [EDPG]\: 258

### <a id="hurdi">\*hurdi- [f.] "wickerwork door" (n)</a>
E: \
-- Go. _haurds_ "(lattice) door" \
N: \
-- ON _hurð_ "door" \
-- Far. _hurð_ "id." \
W: \
-- OS _hurth_ "hurdle, wickerwork" \
-- MDu. _horde_ "wattle", Du. _hor_ [c.] "insect screen" \
-- OHG _hurt_, _hurd_ "hurdle, grate, railing", G _Hürde_ "wattle, hurdle" \
REF: [EDPG]\: 258

### <a id="hurha">\*hurha- "lean" (adj)</a>
N: \
-- ON _horr_ \
-- Far. _hor_ pred. "skinny" \
REF: [EDPG]\: 258

### <a id="hurhwa">\*hurhwa- [n.] "dirt, mucus" (n)</a>
N: \
-- ON _horr_ [m.] "mucus" \
-- Icel. _hor_ [m.] "id." \
W: \
-- OE _horh_, _horg_, _horu_ [m./n.] "spit" \
-- OS _horu_, _horo_ "mud" \
-- ODu. _horo_ "mud", MDu. _hore_ "mud, dirt" \
-- OHG _horo_ "dirt, mud, manure" \
REF: [EDPG]\: 258

### <a id="hurja">\*hurja- [m./n.] "burning coal(?)" (n)</a>
E: \
-- Go. _hauri_ [n.] "coal, burning charcoal, ember" \
N: \
-- ON _hyrr_ [m.] "fire" \
REF: [EDPG]\: 258

### <a id="hurjan">\*hurjan- [wk.] "to repeat?" (v)</a>
W: \
-- OE _hyrian_, _on-hyrian_ "to imitate" \
REF: [EDPG]\: 259

### <a id="hurna">\*hurna- [n.] "horn" (n)</a>
E: \
-- Go. _haurn_ \
N: \
-- ON _horn_ \
-- Far. _horn_ "horn; corner" \
-- Elfd. _uonn_ \
W: \
-- OE _horn_ [m.], E _horn_ \
-- OFri. _horn_ [m.] \
-- Du. _horen_ [c.] \
-- OHG _horn_ "horn; point, end", G _Horn_ "horn" \
REF: [EDPG]\: 259

### <a id="hurnuta">\*hurnuta/ō- [m./f.] "hornet" (n)</a>
W: \
-- OS _hornut_ [m.], _horneta_ [f.], MLG _hornte_ \
-- MDu. _hornete_, _hornte_, _hoornte_ [f.] \
-- OHG _hornuz_, _horniz_ [m.], G _Hornisse_ [f.] \
REF: [EDPG]\: 259

### <a id="hursa">\*hursa- [n.] "horse" (n)</a>
N: \
-- ON _hross_ \
-- Far. _ross_, poet _hors_ \
W: \
-- OE _hors_, E _horse_ \
-- OFri. _hors_ \
-- OS _hros_, _hors_ \
-- Du. _ros_ \
-- OHG _ros_, _hros_, G _Ross_ \
REF: [EDPG]\: 259

### <a id="hurtōn">\*hurtōn- ~ \*hrutōn- [wk.] "to rush, tumble" (v)</a>
N: \
-- Nw. _rota_ "to slide down" \
W: \
-- MHG _hurzen_ "to rush" \
REF: [EDPG]\: 260

### <a id="hurzōn">\*hurzōn- [wk.] "to rush" (v)</a>
N: \
-- Far. _hurra_ "to take off, roar" \
-- Nw. _hurre_ "to turn, swirl, rumble" \
W: \
-- E _to hurry_ \
-- MHG _hurren_ "to dash" \
REF: [EDPG]\: 260

### <a id="hūsa">\*hūsa- [n.] "house" (n)</a>
E: \
-- Go. -_hus_ \
N: \
-- ON _hús_ \
-- Far. _hús_ \
-- Elfd. _aus_ \
W: \
-- OE _hūs_, E _house_ \
-- OFri. _hūs_ \
-- OS _hūs_ \
-- Du. _huis_ \
-- OHG _hūs_, G _Haus_ \
REF: [EDPG]\: 260

### <a id="huttōn">\*hut(t)ōn- [wk.] "to run" (v)</a>
N: \
-- Nw. _hotta_, _hutta_ "to quickly move up and down, go fast, run around" \
W: \
-- MHG _hossen_, _hotzen_ "to run", G dial. _hutzen_ "to incite" \
REF: [EDPG]\: 260

### <a id="huzda">\*huzda- [n.] "treasure" (n)</a>
E: \
-- Go. _huzd_ \
N: \
-- ON _hodd_ [m.] \
W: \
-- OE _hard_ [m./n.], E _hoard_ \
-- OS _hard_ \
-- OHG _hart_, MHG _hart_ \
REF: [EDPG]\: 260

### <a id="hūzjan">\*hūzjan- [wk.] "to hire" (v)</a>
W: \
-- OE _hyr(i)an_, E _to hire_ \
-- OFri. _hera_ \
-- MLG _huren_ \
-- MDu. _huren_, Du. _huren_ \
REF: [EDPG]\: 261

## Hw

### <a id="hwa">\*hwa- "who? what?" (pron)</a>
E: \
-- Go. _ƕas_, acc. _ƕana_, [n.] _ƕata_ \
N: \
-- ON _hverr_ (< \*hwaz-ja-), [n.] _hvat_ \
W: \
-- OE _hwā_, [n.] _hwæt_, E _who_, _what_ \
-- OS _hwē_, [n.] _hwat_ \
-- Du. _wie_, _wat_ \
-- OHG _wer_, _hwer_, [n.] _waz_, _hwaz_, G _wer_, [n.] _was_ \
REF: [EDPG]\: 261

### <a id="hwainō">\*hwainō- [f.] "red-top grass" (n)</a>
N: \
-- ON _hvein_ top. "swamp(?)" \
-- Nw. _kvein(e)_ "red-top grass; hair" \
-- Sw. dial. _hven_ "red-top grass; swamp" \
-- Da. _hvene_ [c.] "id." \
REF: [EDPG]\: 261

### <a id="hwaitja">\*hwaitja- [m.] "wheat" (n)</a>
E: \
-- Go. _ƕaiteis_ \
N: \
-- ON _hveiti_ [n.] \
-- Far. _hveiti_ [f./n.] \
W: \
-- OE _hwǣte_, E _wheat_ \
-- OFri. _wēt_ \
-- OS _hwēti_ \
-- MDu. _weit(e)_ [m./f.] \
-- OHG _weizi_, _hweizi_, G _Weizen_ \
REF: [EDPG]\: 261

### <a id="hwaja">\*hwaja- ~ \*huja- [m.] "whey" (n)</a>
W: \
-- OE _hwæg_ [n.], E _whey_ \
-- OFri. _wāi_, WFri. _waai_ [c./n.] \
-- MDu. _wey_, _hoey_ [m./f.], EDu. _wei_, dial. _hui_ [c.] \
REF: [EDPG]\: 261

### <a id="hwalfa">\*hwalfa- [n.] "vault" (n)</a>
N: \
-- ON _hvalf_ "concavity" \
-- Icel. _hválv_ "arch, vault" \
-- Far. _hválv_ "arch, vault" \
W: \
-- OE _hwealf_ [f.] "arched covering" \
REF: [EDPG]\: 262

### <a id="hwali">\*hwali- [m.] "whale" (n)</a>
N: \
-- ON _hvalr_ \
-- Far. _hvalur_ \
-- Elfd. _wal_ \
W: \
-- OE _hwæl_, E _whale_ \
-- MDu. _wal-visch_, Du. _wal-vis_ \
-- OHG _wal_, _wal-fisk_, G _Wal_, _Wal-fisch_ \
REF: [EDPG]\: 262

### <a id="hwannō">\*hwannō- [f.] "(stalk of) angelica" (n)</a>
N: \
-- ON _hvǫnn_ \
-- Far. _hvonn_ \
-- Nw. _kvann_ \
REF: [EDPG]\: 262

### <a id="hwapjan">\*hwapjan- [wk.] "to choke" (v)</a>
E: \
-- Go. _af-ƕapjan_ \
W: \
-- MHG _ver-wepfen_ "to go bad" \
REF: [EDPG]\: 263

### <a id="hwar">\*hwar "where" (adv)</a>
E: \
-- Go. _ƕar_ \
N: \
-- ON _hvar_ \
-- Far. _hvar_ \
-- Elfd. _war_ \
W: \
-- OE _hwār_, _hwǣ̆r_, E _where_ \
-- OFri. _hwār_ \
-- OS _hwar_ \
-- Du. _waar_ \
-- OHG _wā_, _wār_, _hwār_, G _wo_ \
REF: [EDPG]\: 263

### <a id="hwarbjan">\*hwarbjan- [wk.] "to make turn" (v)</a>
N: \
-- ON _hverfa_ "to turn" \
W: \
-- OE _hwierfan_ "to turn, to revolve" \
-- OS _(gi-)hwerƀian_ "to whirl, turn, reverse, change" \
-- OHG _werben_, _hwerben_ "to turn" \
REF: [EDPG]\: 263

### <a id="hwarma">\*hwarma- [m.] "eyelid" (n)</a>
N: \
-- ON _hvarmr_, _augna-hvarmr_ \
-- Far. _hvarmur_, _varmur_\* \
-- Nw. _augne-kvarm_, -_kvarv_ "edge of the eyelids" \
-- Elfd. _ogen-walm_ \
REF: [EDPG]\: 263

### <a id="hwassa">\*hwassa- "sharp" (adj)</a>
E: \
-- Go. _ƕassaba_ (adv) \
N: \
-- ON _hvass_ \
-- Far. _hvassur_ \
-- Elfd. _wass_ \
W: \
-- OE _hwæs_ \
-- OS _hwass_ "rough, shaggy, serrated" \
-- OHG _was_, _hwas_ "sharp, strict", MHG _was_ "sharp, pointy" \
REF: [EDPG]\: 264

### <a id="hwata">\*hwata- "quick" (adj)</a>
N: \
-- ON _hvatr_ "active, brisk, vigorous" \
W: \
-- OE _hwæt_ "fast, bold" \
-- MDu. _wat_ "sharp" \
-- OHG _waz_, _hwaz_ "fierce, sharp" \
REF: [EDPG]\: 264

### <a id="hwaþera">\*hwaþera- "who of two?" (pron)</a>
E: \
-- Go. _ƕaþar_ \
N: \
-- ON _hvárr_ \
-- Icel. _hvor_ \
-- Far. _hvør_ \
W: \
-- OE _hwæðer_, E _whether_ \
-- OFri. _hweder_, _hwether_, _hwoder_, _hoder_ \
-- OS _hwethar_ \
-- OHG _wedar_, _hwedar_, MHG _weder_ \
REF: [EDPG]\: 264

### <a id="hwaþōn">\*hwaþōn- [f.] "foam" (n)</a>
E: \
-- Go. _ƕaþo_ \
N: \
-- Sw. _kva_ \
REF: [EDPG]\: 264

### <a id="hwehla">\*hwehla- ~ \*hweula- [n.] "wheel" (n)</a>
N: \
-- ON _hjól_ \
W: \
-- OE _hweohhol_, _hwēol_, _hweowol_, _hweogol_, E _wheel_ \
-- OFri. _fiāl_, _tiāl_ \
-- MDu. _wiel_, Du. _wiel_ \
-- G _Wiel_ \
REF: [EDPG]\: 264

### <a id="hwekkan">\*hwekkan- [st.] "to be startled" (v)</a>
N: \
-- Far. _hvøkka_ "to be startled; to diminish; to begin slowly" \
REF: [EDPG]\: 265

### <a id="hwelfan">\*hwelfan- [st.] "to vault, revolve" (v)</a>
N: \
-- ON _holfinn_ "vaulted" \
W: \
-- MDu. _welven_ [st./wk.] "to vault; to turn, revolve", Du. _welven_ [wk.] "id." \
-- MHG _walb_ pret "id." \
REF: [EDPG]\: 265

### <a id="hwera">\*hwera- [m.] "kettle" (n)</a>
N: \
-- ON _hverr_ \
W: \
-- OE _hwer_ \
-- OHG _wer_, _hwer_ \
REF: [EDPG]\: 265

### <a id="hwerban">\*hwerban- [st.] "to turn, move around" (v)</a>
E: \
-- Go. _ƕairban_ "to go around, circulate" \
N: \
-- ON _hverfa_ "to turn; to disappear" \
-- Far. _hvørva_ "to disappear, vanish" \
W: \
-- OE _hweorfan_ "to turn, change" \
-- OFri. _hwerva_ "to turn, move around, swap" \
-- OS _hwerƀan_ "to (re)turn" \
-- Du. _werven_ [wk.] "to recruit" \
-- OHG _werban_, _hwerban_ "to return, drive", MHG _werben_, _werven_ "to turn, be concerned with", G _werben_ [wk.] "to recruit" \
REF: [EDPG]\: 265

### <a id="hwerhwetjō">\*hwerhwetjō- [f.] "gourd(?)" (n)</a>
W: \
-- OE _hwerhwette_ "cucumber", ME _hwerwette_, _werwette_ "cucumber, gourd" \
REF: [EDPG]\: 266

### <a id="hwēsan">\*hwēsan- [st.] "to wheeze" (v)</a>
W: \
-- OE _hwǣsan_, E _to wheeze_ \
REF: [EDPG]\: 266

### <a id="hwētan">\*hwētan- [st.] "to stab, pierce" (v)</a>
N: \
-- ON _hváta_ [wk.] \
REF: [EDPG]\: 266

### <a id="hwijōn">\*hwijōn- [wk.] "to whinny" (v)</a>
N: \
-- Icel. _hvía_ \
W: \
-- MHG _wihen_ \
REF: [EDPG]\: 266

### <a id="hwīlō">\*hwīlō- [f.] "while" (n)</a>
E: \
-- Go. _ƕeila_ "period, while" \
N: \
-- ON _hvíla_ "bed" \
-- Far. _hvíla_ "rest, repose; bed" \
-- Elfd. _waila_ \
W: \
-- OE _hwīl_, E _while_ \
-- OFri. _hwīl(e)_ \
-- OS _hwīla_ \
-- MDu. _wile_ \
-- OHG _wīla_, _hwīla_, G _Weile_ \
REF: [EDPG]\: 266

### <a id="hwīnan">\*hwīnan- [st.] "to whistle, whiz" (v)</a>
N: \
-- ON _hvína_ \
W: \
-- OE _hwīnan_, ME _to whine_ \
REF: [EDPG]\: 267

### <a id="hwinda">\*hwinda- [m.] "greyhound" (n)</a>
W: \
-- OFri. _hwynd_ \
-- MDu. _wint_, Du. _wind-hand_ \
-- OHG _wint_, G _Wind-hund_ \
REF: [EDPG]\: 267

### <a id="hwīta">\*hwīta- ~ \*hwitta- "white" (adj)</a>
E: \
-- Go. _ƕeits_ \
N: \
-- ON _hvítr_ \
-- Far. _hvítur_ \
-- Elfd. _wait_ \
W: \
-- OE _hwīt_, E _white_ \
-- OFri. _(h)wī̆t_ \
-- OS _hwīt_ \
-- Du. _wit_ \
-- OHG _wīz_, _hwīz_, G _weiß_ \
REF: [EDPG]\: 267

### <a id="hwōsan">\*hwōsan- [st.] "to cough" (v)</a>
W: \
-- OE _hwōsan_, ME _hwōsen_ \
REF: [EDPG]\: 267

### <a id="hwōstan">\*hwōstan- [m.] "cough" (n)</a>
N: \
-- ON _hósti_ \
W: \
-- OE _hwōsta_ \
-- MDu. _hoeste_ [m./f.], Du. _hoest_ [c.] \
-- OHG _huosto_, MHG _huoste_, G _Husten_ \
REF: [EDPG]\: 268

### <a id="hwōtjan">\*hwōtjan- [wk.] "to bore; to threaten" (v)</a>
E: \
-- Go. _ƕotjan_ "to reprimand, warn sternly, threaten" \
N: \
-- ON _hœta_ "to threaten" \
-- OSw. _høta_ "to threaten; to bore a hole in the ground" \
REF: [EDPG]\: 268

## I

### <a id="i">\*i- "he/she, that one" (pron)</a>
E: \
-- Go. _is_, [n.] _ita_ \
W: \
-- Du. _hij_, [n.] _het_ (with secondary _h_-) \
-- OHG _er_, [n.] _iz_, G _er_, [n.] _es_ \
REF: [EDPG]\: 268

### <a id="īda">\*īda- [m.] "chub, orfe" (n)</a>
N: \
-- Nw. _id_ \
-- Sw. _id_ [c.] \
-- Gutn. _äid_ [f.] \
W: \
-- G dial _Aitel_ "id.". \
REF: [EDPG]\: 268

### <a id="īdala">\*īdala- "void, idle, futile" (adj)</a>
W: \
-- OE _īdel_, E _idle_ \
-- ODu. _īdil_, Du. _ijdel_ \
-- OHG _ītal_, G _eitel_ \
REF: [EDPG]\: 268

### <a id="idōjan">\*idōjan- [wk.] "to move about restlessly" (v)</a>
N: \
-- Icel. _iða_ \
REF: [EDPG]\: 269

### <a id="iliþ">\*iliþ- [m.(?)] "sole of the foot" (n)</a>
N: \
-- ON _il_ [f.] \
-- Far. _il_ [f.] \
W: \
-- OE _ile_ [m.] "sole of the foot; hard skin, callosity" \
-- MLG _ēle_, _ēlde_, _ēlt_ [n.] "callus" \
-- Du. _eelt_ [n.] "id." \
REF: [EDPG]\: 269

### <a id="īljan">\*īljan- [wk.] "to rush" (v)</a>
W: \
-- OS _īlian_ "to hurry" \
-- MDu. _īlen_ "to rush", Du. _ijlen_ "to hurry" \
-- OHG _īllan_, _īlen_ "to strife, rush", G _eilen_ "to rush" \
REF: [EDPG]\: 269

### <a id="ini">\*in(i) "in" (prep)</a>
E: \
-- Go. _in_ \
N: \
-- ON _í_ \
-- Far. _í_ \
-- Elfd. _i_ \
W: \
-- OE _in_, E _in_ \
-- OS _in_ \
-- Du. _in_ \
-- OHG _in_, G _in_ \
REF: [EDPG]\: 269

### <a id="inkan">\*inkan- [m.] "pain, torment" (n)</a>
N: \
-- ON _ekki_ "sorrow, grief" \
-- Far. _ekki_ "pain, suffering; grief; anxiety" \
W: \
-- OE _inca_ "grudge, grievance, suspicion" \
REF: [EDPG]\: 269

### <a id="inkwan">\*inkwa(n)- [m.] "lump" (n)</a>
N: \
-- Icel. _ökkr_, _ökkvi_ "lump; hillock" \
-- Sw. dial. _ink_ "ulcer" \
W: \
-- MDu. _enke_, _inke_ "small wound", EDu. _incke_ "wound, scar" \
REF: [EDPG]\: 270

### <a id="innai">\*innai "in(side)" (adv)</a>
E: \
-- Go. _inna_ \
N: \
-- ON _inni_ \
W: \
-- OE _inne_, E _in_ \
-- OS _inne_ \
-- OHG _inni_, MHG _inne_ \
REF: [EDPG]\: 270

### <a id="instra">\*instra- [n.] "intestinal fat" (n)</a>
N: \
-- ON _ístr_, _ístra_ [f.] "paunch-fat" \
-- Icel. _ístra_ [f.] "paunch, pot-belly" \
-- Far. _ístur_ "id." \
-- Elfd. _įster_ "grease" \
W: \
-- MLG _inster_ "entrails of slaughtered cattle" \
-- MHG _inster_ "entrails" \
REF: [EDPG]\: 270

### <a id="inþera">\*inþera- [n.] "entrails" (n)</a>
N: \
-- ON _iðr_, _innr_, _indr_ \
-- Far. _indur_ "intestines for cooking blood sausage", pl. "entrails" \
REF: [EDPG]\: 270

### <a id="irha">\*irha- [m.] "he-goat" (n)</a>
W: \
-- OHG _irah_, _ireh_, _irh_, MHG _ireh_ \
REF: [EDPG]\: 270

### <a id="īsa">\*īsa- [m./n.] "ice" (n)</a>
N: \
-- ON _íss_ [m.] \
-- Far. _ísur_ [m.] \
-- Elfd. _ais_ [m.] \
W: \
-- OE _īs_ [n.], E _ice_ \
-- OHG _īs_ [n.] \
REF: [EDPG]\: 271

### <a id="īsarna">\*īsarna- ~ \*īzarna- [n.] "iron" (n)</a>
E: \
-- Go. _eisarn_ \
N: \
-- ON _ísarn_ \
W: \
-- OE _īsern_, _īsen_, _īren_ \
-- OS _isarn_ \
-- MDu. _īser_, _īsen_, Du. _ijzer_ \
-- OHG _īsarn_, _īsan_, G _Eisen_ \
REF: [EDPG]\: 271

### <a id="īwa">\*īwa- [m.] "yew" (n)</a>
N: \
-- ON _ýr_ \
-- Elfd. _åyve_ [n.] (< \*īwja-) \
W: \
-- OE _īw_, _ēow_, E _yew_ \
-- OHG _īwa_ [f.], G _Eibe_ [f.] (< \*īwō-) \
REF: [EDPG]\: 271

## J

### <a id="jakkōn">\*jakk/gōn- [wk.] "to rush, chase" (v)</a>
W: \
-- OS _jagon_ "to drive", WPhal. _jacken_ "to rush" \
-- EDu. _jacken_ "id.", Du. _jagen_ "to rush, hunt"; _jakkeren_ "to speed" \
-- OHG _jagōn_ "to rush, chase, hunt", G _jagen_ "id.", Pal. _jacken_ "to rock", _jackern_ "to rush, speed" \
REF: [EDPG]\: 271

### <a id="jaukja">\*jaukja- [m.] "draft animal" (n)</a>
N: \
-- ON _eykr_ "draft animal, horse" \
REF: [EDPG]\: 272

### <a id="jazjan">\*jazjan- [wk.] "to make ferment" (v)</a>
N: \
-- Nw. _erja_ "to dissolve" \
-- OHG _jerien_ "to make ferment" \
REF: [EDPG]\: 272

### <a id="jedan">\*jedan- [st.] "to weed" (v)</a>
W: \
-- OS _ūt-jedan_ "to root out", MLG _geden_ [wk.] "to weed" \
-- OHG _jeten_, G _jäten_ [wk.] \
REF: [EDPG]\: 272

### <a id="jehan">\*jehan- [st.] "to acknowledge" (v)</a>
N: \
-- ON _já_ [wk.] "to say yes; to promise" \
W: \
-- OFri. _jā_, _jān_ "to confess, acknowledge" \
-- OS _jehan_ "to say, confess" \
-- ODu. _gian_, _be-gien_ "to confess" \
-- OHG _jehan_, _gehan_ "to confess, witness" \
REF: [EDPG]\: 272

### <a id="jehti">\*jehti- [f.] "confession" (n)</a>
W: \
-- Du. _biecht_ [c.] \
-- OHG _jiht_ "utterance", _bi-jiht_, _bīht_ "confession", MHG _giht_, _begiht_, _bīht_ "utterance, confession", G _Beichte_ "confession, admission" \
REF: [EDPG]\: 272

### <a id="jekan">\*jeka(n)- [m.] "ice" (n)</a>
N: \
-- ON _jaki_ "broken ice, ice-floe" \
W: \
-- LG _is-jack_ "icicle" \
-- G Swi. _jäch_ "hoar-frost" \
REF: [EDPG]\: 273

### <a id="jēra">\*jēra- [n.] "year" (n)</a>
E: \
-- Go. _jer_ \
N: \
-- ON _ár_ \
W: \
-- OFri. _jēr_ \
-- OS _jār_ \
-- Du. _jaar_ \
-- OHG _jar_, G _Jahr_ \
REF: [EDPG]\: 273

### <a id="jesan">\*jesan- [st.] "to ferment" (v)</a>
N: \
-- Sw. _jäsa_ [wk.] \
-- Elfd. _jäså_ \
-- OHG _jesan_ \
W: \
-- MHG _jesen_ \
REF: [EDPG]\: 273

### <a id="jestu">\*jestu- [m.] "yeast" (n)</a>
N: \
-- ON _jǫstr_ \
-- Nw. _jest_ \
-- OSw. _iæster_ \
-- Elfd. _jäst_ [c.] \
W: \
-- E _yeast_ \
-- MLG _gest_ \
-- MDu. _gest_, _gist_, Du. _gist_ [c.] \
-- MHG _jest_ "foam", G _Gischt_ [f.] "id." \
REF: [EDPG]\: 273

### <a id="jewan">\*jewan- [m.] "creeping insect(?)" (n)</a>
N: \
-- Nw. dial. _jo(e)_ "maggot" \
REF: [EDPG]\: 273

### <a id="juka">\*juka- [n.] "yoke" (n)</a>
E: \
-- Go. _juk_ \
N: \
-- ON _ok_ \
-- Far. _ok_ \
-- Elfd. _uok_ \
W: \
-- OE _geoc_, E _yoke_ \
-- OS _juk_ \
-- Du. _juk_ \
-- OHG _joh_, G _joch_ \
REF: [EDPG]\: 274

### <a id="jukjan">\*jukjan- [wk.] "to itch" (v)</a>
W: \
-- OE _gyccan_, E _to itch_ \
-- OS _juk(k)ian_ \
-- Du. _jeuken_ \
-- OHG _jucken_, G _jucken_ \
REF: [EDPG]\: 274

### <a id="jukkōn">\*jukkōn- [wk.] "to hop; to run" (v)</a>
W: \
-- G dial. _jucken_, Swab. "to jump, hop", Pal. "to jerk, rock", Als. "to jerk, jump up, turn away", Swab. "to jump, run; to hop", Cimb. "to throw; jump", Swi. (Rhntl.) "to run fast", (Schfh.) _jūkxə_ "to hop" \
REF: [EDPG]\: 274

### <a id="jumja">\*jumja- [m.] "divine twin(?)" (n)</a>
N: \
-- ON _Ymir_ "primordial giant in Norse mythology" \
REF: [EDPG]\: 274

### <a id="junga">\*junga- "young" (adj)</a>
E: \
-- Go. _juggs_ \
N: \
-- ON _ungr_ \
-- Far. _ungur_ \
-- Elfd. _ungg_ \
W: \
-- OE _geong_, E _young_ \
-- OFri. _jung_ \
-- OS _jung_ \
-- Du. _jong_ \
-- OHG _jung_, G _jung_ \
REF: [EDPG]\: 274

### <a id="jūz">\*jūz "you (pl.)" (pron)</a>
E: \
-- Go. _jus_, obl. _izwis_ \
N: \
-- ON _ér_, obl. _yðr_ \
W: \
-- OE _gē̆_, obl. _ēow_, _īow_ \
-- OFri. _ī_, _gī_, obl. _iō_, _iū_ \
-- OS _gī̆_, obl. _eu_, _iu_ \
-- Du. _jij_, obl. _jou_ "you (sg.)" \
-- OHG _ir_, dat. _iu_, acc. _iuwih_, G _ihr_, obl. _euch_ \
REF: [EDPG]\: 275

### <a id="jūsta">\*jū̆sta- [m.] "cheese"(n)</a>
N: \
-- ON _ostr_ \
-- Far. _ostur_ \
-- OSw. _oster_ \
-- Elfd. _uost_ \
REF: [EDPG]\: 275

### <a id="jut">\*jut "you (du.)" (pron)</a>
E: \
-- Go. obl. _igqis_ \
N: \
-- ON _it_, obl. _ykkr_ \
-- Far. _tit_, obl. _tykkum_, _tygum_ \
-- Elfd. _ið_ \
W: \
-- OE _git_, obl. _inc_ \
-- OS _git_, obl. _ink_ \
REF: [EDPG]\: 276

### <a id="juwunþi">\*ju(w)unþi- [f.] "youth" (n)</a>
E: \
-- Go. _junds_\* \
W: \
-- OE _geogoð_, E _youth_ \
-- OS _juguth_ \
-- ODu. _iugint_, MDu. _joghet_, Du. _jeugd_ [c.] \
-- OHG _jugund_, G _Jugend_ \
REF: [EDPG]\: 276

## K

### <a id="kablan">\*kabla/ōn- [m./f.] "piece of wood" (n)</a>
N: \
-- ON _kafli_ [m.] "piece cut off" \
W: \
-- MDu. _cavele_ [m./f.] "stick, lot", Du. _kavel_ [c.] "lot, parcel" \
-- MHG _kabel_ [m./f.] "lot" \
REF: [EDPG]\: 276

### <a id="kagan">\*kagan- [m.] "stem of a (small) tree(?)" (n)</a>
N: \
-- ON _kagi_ "shrub, young tree" \
-- Nw. dial. _kage_ "small shrub" \
W: \
-- E dial. _cag_ "trunk" \
REF: [EDPG]\: 277

### <a id="kaiba">\*kaiba- "sharp(?), crooked(?)" (adj)</a>
N: \
-- Nw. _keiv_ "wry, wrong, left" \
W: \
-- OE _cāf_ "quick, sharp, prompt", ME _cōf_ "quick" \
REF: [EDPG]\: 277

### <a id="kaipa">\*kaipa- [m.] "to groove, notch" (n)</a>
N: \
-- ON _keipr_ "rowlock" \
W: \
-- Du. _keep_ [c.] "notch" \
REF: [EDPG]\: 277

### <a id="kaizwjan">\*kaizwjan- [wk.] "to turn, direct" (v)</a>
N: \
-- ON _keyra_ "to whip, lash; to ride, drive; to fling" \
-- Far. _koyra_ "to drive; to push; to pour" \
W: \
-- OE _cierran_ "to turn, change" \
-- OFri. _kēra_, _kerra_ "to turn; to hinder" \
-- ODu. _kēran_ "to turn", Du. _keren_ "to turn" \
-- OHG _kēren_ "to turn, direct" \
REF: [EDPG]\: 277

### <a id="kalan">\*kalan- [wk.] "to be cold" (v)</a>
N: \
-- ON _kala_ [st.] impers. "to freeze" \
W: \
-- OE _calan_ [st.] "to be or become cool or cold" \
REF: [EDPG]\: 277

### <a id="kalban">\*kalba(n)- [m.] "calf (of the leg)" (n)</a>
N: \
-- ON _kalfi_ "calf of the leg" \
-- Far. _kálvi_ "small of the leg; middle of the seine net" \
-- OSw. _kalver_ "extension of a fishing net" \
-- Sw. _kalv_ [c.] "calf of the legs; interior of a rope; extension of a fishing net" \
W: \
-- EDu. _kalf_ "soft part of wood" \
-- G dial. _Kalb_ "muscle" \
REF: [EDPG]\: 278

### <a id="kalbiz">\*kalbiz- [n.] "calf" (n)</a>
N: \
-- ON _kalfr_ [m.] \
-- Far. _kálvur_ [m.] \
-- Elfd. _kåv_ [m.] \
W: \
-- OE _cealf_, _calfur_, E _calf_ \
-- OS _kalf_ \
-- Du. _kalf_ \
-- OHG _kalp_, pl. _kalbir_, G _Kalb_ \
REF: [EDPG]\: 278

### <a id="kalda">\*kalda- "cold" (adj)</a>
E: \
-- Go. _kalds_ \
N: \
-- ON _kaldr_ \
-- Far. _kaldur_ \
-- Elfd. _kold_ \
W: \
-- OE _ceald_, E _cold_ \
-- OS _kald_ \
-- Du. _koud_ \
-- OHG _kalt_, G _kalt_ \
REF: [EDPG]\: 278

### <a id="kallōn">\*kallōn- [wk.] "to freeze" (v)</a>
W: \
-- G Swi. _xallen_ "to freeze, stiffen" \
REF: [EDPG]\: 278

### <a id="kalwa">\*kalwa- "bald" (adj)</a>
W: \
-- OE _calo_, E _callow_ \
-- MDu. _calu_, Du. _kaal_ \
-- OHG _kalo_, G _kahl_ \
REF: [EDPG]\: 278

### <a id="kalzōjan">\*kalzōjan- [wk.] "to call, shout" (v)</a>
N: \
-- ON _kalla_ \
-- Far. _kalla_ \
-- Elfd. _kolla_ \
W: \
-- OE _ceallian_, E _to call_ \
-- Du. dial. _kallen_ "to talk" \
-- OHG _kallōn_ "to chatter, talk", MHG _kallen_ "to talk loudly" \
REF: [EDPG]\: 278

### <a id="kamba">\*kamba- [m.] "comb" (n)</a>
N: \
-- ON _kambr_ "comb; crest" \
-- Far. _kambur_ "comb; pointed mountain" \
-- Elfd. _kamb_ \
W: \
-- OE _camb_, E _comb_ \
-- OS _kamb_ \
-- Du. _kam_ [c.] \
-- OHG _kamb_ "comb, crest", G _Kamm_ "id." \
REF: [EDPG]\: 279

### <a id="kambjan">\*kambjan- [wk.] "to comb" (v)</a>
N: \
-- ON _kemba_ "to comb; to card" \
-- Far. _kemba_ \
-- Elfd. _kemba_ \
W: \
-- OE _cemban_ \
-- OS _kembian_ \
-- OHG _kemben_, G _kämmen_ \
REF: [EDPG]\: 279

### <a id="kanhta">\*kanhta- "glad, happy" (adj)</a>
N: \
-- ON _kátr_ \
-- Far. _kátur_ \
-- Elfd. _ką̊t_ "lively" \
REF: [EDPG]\: 279

### <a id="kanjan">\*kanjan- [wk.] "to bring forth" (v)</a>
W: \
-- OE _cennan_ "to beget, conceive, create, bring forth", ME _kennen_ "id." \
REF: [EDPG]\: 279

### <a id="kannjan">\*kannjan- [wk.] "to make known" (v)</a>
E: \
-- Go. _kannjan_ "to make known, inform of" \
N: \
-- ON _kenna_ "to know, recognize; to assign or attribute" \
-- Far. _kenna_ "to distinguish, recognize, know" \
-- Elfd. _kenna_ "to know" \
W: \
-- OE _cennan_ "to choose, to prove, to declare" \
-- WFri. _kenna_ "to know" \
-- OS _ant-kennian_ "to perceive" \
-- ODu. _kennan_ "to know", Du. _kennen_ "id." \
-- OHG _ar-kennan_ "to recognize", MHG _kennen_ "to know" \
REF: [EDPG]\: 279

### <a id="kannōn-1">\*kannōn- [wk.] "to get to know" (v)</a>
N: \
-- ON _kanna_ "to search, find out" \
-- Far. _kanna_ "to inspect" \
REF: [EDPG]\: 280

### <a id="kannōn-2">\*kannōn- [f.] "jug" (n)</a>
N: \
-- ON _kanna_ \
-- Far. _kanna_ \
-- Elfd. _kanna_ \
W: \
-- OE _canne_ "can, cup", E _can_ \
-- OFri. _kanne_ "id." \
-- MDu. _canne_, Du. _kan_ [c.] \
-- OHG _kanna_, _kanta_, G _Kanne_ \
REF: [EDPG]\: 280

### <a id="kanta">\*kanta- [m.] "side" (n)</a>
W: \
-- E Scot. _cantle_ "crown ofthe head; fleshy part behind the ear in a ram's head" \
-- MDu. _cant_ "edge, region", Du. _kant_ [c.] "side; edge" \
-- G _Kanz_ "mane, upper part of the neck" \
REF: [EDPG]\: 280

### <a id="kappōn">\*kap(p)ōn- [wk.] "to stare" (v)</a>
W: \
-- OE _capian_ "to turn" \
-- MDu. _capen_ "to watch, stare at" \
-- OHG _kapfēn_ "to stare", MHG _kaffen_, _kapfen_ "to stare, be in awe" \
REF: [EDPG]\: 280

### <a id="karō">\*karō- [f.] "worry, care" (n)</a>
E: \
-- Go. _kara_ "care, worry, concern" \
N: \
-- ON _kǫr_ "bed of sickness" \
W: \
-- OE _cearu_ "suffering, sorrow; care, caution", E _care_ \
-- OS _kara_ "sorrow, lament" \
-- OHG _kara_ "sorrow, sadness, repentance" \
REF: [EDPG]\: 281

### <a id="karska">\*karska- "lively" (adj)</a>
N: \
-- ON _karskr_ "brisk, bold" \
W: \
-- MLG _karsch_ "vivid" \
-- Du. _kers-vers_ "fresh" \
-- MHG _karsch_ "vivid" \
REF: [EDPG]\: 281

### <a id="katazan">\*katazan- [m.] "tomcat" (n)</a>
W: \
-- MDu. _catere_, Du. _kater_ [c.] \
-- OHG _katere_, G _Kater_ \
REF: [EDPG]\: 281

### <a id="kattōn">\*kattōn- [f.] "cat" (n)</a>
N: \
-- Nw. _katte_ \
W: \
-- OFri. _katte_ \
-- MDu. _catte_, Du. _kat_ [c.] \
-- OHG _kazza_, G _Katze_ \
REF: [EDPG]\: 281

### <a id="kaujan">\*kaujan- [wk.] "to call, shout" (v)</a>
W: \
-- OE _cīegan_ \
-- OHG _gi-kewen_ \
REF: [EDPG]\: 282

### <a id="kauma">\*kauma-(?) [m.(?)] "folded hands" (n)</a>
N: \
-- Icel. _kaun_ [n.pl.] \
-- Far. _keym(i)_, _kjóm_ \
REF: [EDPG]\: 282

### <a id="kauna">\*kauna- [n.(?)] "blister" (n)</a>
N: \
-- ON _kaun_ [n.] "sore" \
-- Icel. _kaun_ [n.] "boil, carbuncle" \
-- Nw. _kaun_ [m.] "blister" \
REF: [EDPG]\: 282

### <a id="kaunō">\*kaunō- [f.] "jaw" (n)</a>
W: \
-- MDu. _coon_ "jaw (of a fish), gill", Du. _koon_ [c.] "cheekbone, jaw, gill" \
REF: [EDPG]\: 282

### <a id="kautōn">\*kautōn- [f.] "knuckle" (n)</a>
W: \
-- OFri. _kāte_ \
-- MDu. _cote_ "hoof, fetlock", Du. _kootje_ dim. "phalanx" \
-- G Rhnl. _kötche_ dim. "dice made of sheep's knuckles" \
REF: [EDPG]\: 283

### <a id="kawō">\*kawō- [f.] "jackdaw" (n)</a>
W: \
-- OE _cēo_, E _chough_ \
-- OS _kā_ \
-- MDu. _cauwe_, Du. _kauw_ [c.] \
-- OHG _kaha_, _kā_ \
REF: [EDPG]\: 283

### <a id="kaza">\*kaza- [n.] "casket, vat" (n)</a>
E: \
-- Go. _kas_ "jar, vase, vessel" \
N: \
-- ON _ker_ "tub, vessel, goblet" \
-- Far. _kar_ "id." \
-- Elfd. _kar_ "id." \
W: \
-- OS _bī-kar_ "bee-hive" \
-- MDu. _caer_ "creel, bee-hive", Du. _vis-kaar_ "creel" \
-- OHG _kar_ "vessel, vat, trough", MHG _kar_ "vessel" \
REF: [EDPG]\: 283

### <a id="kebran">\*kebra(n)- [m.] "beetle" (n)</a>
W: \
-- OE _ceafor_, _cefer_, E _chafer_ \
-- ODu. _kevera_ [f.] "grasshopper", MDu. _kever(e)_ "beetle", Du. _kever_ [c.] "id." \
-- OHG _kevar(o)_, G _Käfer_ \
REF: [EDPG]\: 283

### <a id="kēgjō">\*kēgjō- [f.] "peg, key" (n)</a>
W: \
-- OE _cǣg_ "key" \
-- OFri. _kāi_, _kēi_ [m.] "id." (< \*kēga-), WFri. _kaai_ [c.] "id." \
REF: [EDPG]\: 284

### <a id="kelkan">\*kelka(n)- [m.] "jawbone" (n)</a>
N: \
-- ON _kjalki_ "jaw; sledge" \
-- Far. _kjálki_ "cheekbone" \
-- Nw. _kjelke_ "sledge", dial. "Adam's apple" \
-- Elfd. _tjåke_ "sledge" \
W: \
-- OHG _kelah_ "goiter", MHG _kelch_ "crop, double chin" \
REF: [EDPG]\: 284

### <a id="kelōn">\*kelōn- [f.] "throat" (n)</a>
W: \
-- OE _ceole_ "throat, jowl" \
-- Du. _keel_ [c.] \
-- OHG _kela_, _kel_, G _Kehle_ "id." \
REF: [EDPG]\: 284

### <a id="kelurō">\*kelurō- [f.] "throat" (n)</a>
W: \
-- OE _ceoler_ "collar, throat" \
-- OHG _kelur_, _kelero_ [m.], _kelera_ "id." \
REF: [EDPG]\: 284

### <a id="kēma">\*kēma- [n.] "mold" (n)</a>
N: \
-- Icel. _kám_ "filth" \
W: \
-- E dial. _coom_ "soot" \
-- EDu. _kaem_, _kaen_ "layer of mold (on fluids)", Du. _kaam_, _kaan_ [c.] "id." \
-- MHG _kām_, _kān(e)_ "id.", G _Kahm_ [m.] "id." \
REF: [EDPG]\: 284

### <a id="kerban">\*kerban- [st.] "to carve" (v)</a>
W: \
-- OE _ceorfan_ "to cut (off); slay" \
-- MDu. _kerven_, _caerven_ "to carve; slice; scrap", Du. _kerven_ [wk.] "to carve" \
REF: [EDPG]\: 285

### <a id="kerla">\*kerla- ~ \*karla- [m.] "man, freeman" (n)</a>
N: \
-- ON _karl_ "man; man ofthe common people" \
-- Far. _kallur_ "man" \
-- Elfd. _kall_ "man; husband" \
W: \
-- OE _ceorl_ "man, low freeman, peasant", E _churl_ \
-- OFri. _tzerl_, _tzirl_, _kerl_ "man; servant" \
-- MDu. _kerle_, _kerel_, _caerl_ "low freeman", Du. _kerel_ [c.] "man, guy" \
-- OHG _karl_, _karal_ "husband, spouse", G _Kerl_ "man, fellow" \
REF: [EDPG]\: 285

### <a id="kernan">\*kernan- [m.] "kernel, core" (n)</a>
N: \
-- ON _kjarni_ "kernel" \
-- Far. _kjarni_ "id." \
W: \
-- OS _kerno_ "kernel, seed" \
-- MDu. _kerne_, _keerne_ "kernel, stone", Du. _kern_ [c.] "kernel" \
-- OHG _kerno_ "kernel, corn", G Kern "kernel" \
REF: [EDPG]\: 285

### <a id="kerzan">\*kerzan- [st.] "to creak, to cry (of birds)" (v)</a>
W: \
-- OE _ceorran_ "to creak" \
-- MDu. _kerren_, _carren_ [wk.] "to creak, squeak" \
REF: [EDPG]\: 285

### <a id="kespa">\*kespa- [m.(?)] "beam(?)" (n)</a>
W: \
-- Du. _kesp_ [c.] "beam lying on top of the foundation" \
REF: [EDPG]\: 286

### <a id="ketwa">\*ketwa- [n.] "meat" (n)</a>
N: \
-- ON _kjǫt_ \
-- Far. _kjøt_ \
-- Elfd. _tjyōt_ \
REF: [EDPG]\: 286

### <a id="keusan">\*keusan- [st.] "to trial, select" (v)</a>
E: \
-- Go. _kiusan_ "to put to a test, prove by trial" \
N: \
-- ON _kjósa_ "to choose, select, elect" \
-- Far. _kjósa_ "id." \
W: \
-- OE _cēosan_ "to choose, to elect" \
-- OS _kiosan_, _keosan_, _keosan_ "to choose, recognize" \
-- Du. _kiezen_ "to choose" \
-- OHG _kiosan_ "to choose, examine, judge", MHG _kiesen_ "to choose" \
REF: [EDPG]\: 286

### <a id="kewan">\*kewan- [st?] "to cry" (v)</a>
N: \
-- Icel. _kjá_ [wk.] "to cry (of birds)" \
REF: [EDPG]\: 286

### <a id="kewwan">\*kewwan- [st.] "to chew" (v)</a>
N: \
-- ON _tyggva_, _tyggja_ \
-- Far. _tyggja_ [wk.] \
-- Elfd. _tegga_ \
W: \
-- OE _cēowan_, E _to chew_ \
-- MDu. _couwen_, _cuwen_ [wk.], Du. _kauwen_ [wk.] \
-- OHG _kiuwan_, G _kauen_ [wk.] \
REF: [EDPG]\: 286

### <a id="kīban">\*kīban- [st.] "to quarrel" (v)</a>
N: \
-- ON _kífa_ [wk.] \
W: \
-- MDu. _kiven_ [st./wk.] \
-- MHG _kīben_ "to quarrel, fight" \
REF: [EDPG]\: 287

### <a id="kila">\*kila- [m.] "bulb(?)" (n)</a>
W: \
-- MHG _kil_ "onion", G _Kiel_ "bulb" \
REF: [EDPG]\: 287

### <a id="kīman">\*kīman- [m.] "germ, sprout" (n)</a>
W: \
-- OS _kīmo_ \
-- Du. _kiem_ [c.] \
-- OHG _kīmo_, G _Keim_ \
REF: [EDPG]\: 287

### <a id="kimbōn">\*kimbōn- [f.] "crest, ridge" (n)</a>
W: \
-- OE _cimb-stān_ [m.] "pedestal, socket", E _chimbe_, _chime_ "edge or rim of a cask or drum" \
-- MDu. _kimme_ "edge, chime", Du. _kim_ [c.] "horizon", dial. "edge of a cask, chime" \
REF: [EDPG]\: 287

### <a id="kīnan">\*kīnan- [st.] "to burst open, germinate" (v)</a>
E: \
-- Go. _us-keinan_ [st./wk.] "to sprout up, bud forth, grow from" \
W: \
-- OE _cīnan_ "to gape, break into chinks" \
-- OHG _kīnan_ "to sprout, blossom, germinate" \
REF: [EDPG]\: 287

### <a id="kindi">\*kindi- [f.] "kind" (n)</a>
N: \
-- ON _kind_ "kind, race" \
REF: [EDPG]\: 288

### <a id="kinnu">\*kinnu- [f.] "cheek" (n)</a>
E: \
-- Go. _kinnus_ "cheek, jawbone" \
N: \
-- ON _kinn_ "cheek; slope" \
W: \
-- OE _cinn_ "chin" \
-- OS _kinni_, _kin_, _kin_ [n.] "jaw, chin" \
-- Du. _kin_ [c.] "chin" \
-- OHG _kinni_ [n.] "chin, jaw-bone" \
REF: [EDPG]\: 288

### <a id="kinþa">\*kinþa- ~ \*kinda- [n.] "child" (n)</a>
N: \
-- Nw. _kind_ \
W: \
-- OFri. _kind_ \
-- OS _kind_ \
-- Du. _kind_ \
-- OHG _kind_, G _Kind_ \
REF: [EDPG]\: 288

### <a id="kippōn-1">\*kipp/bōn- [wk.] "to quarrel, fight" (v)</a>
W: \
-- OFri. _kebbia_, WFri. _kibje_ \
-- MLG _kibben_ \
-- Du. _kibbelen_ \
REF: [EDPG]\: 288

### <a id="kippōn-2">\*kippōn- [wk.] "to stumble" (v)</a>
N: \
-- Icel. _kippa_ "to feel tipsy; to fall into place" \
-- Far. _kippa_ "to go under water (of stones)" \
-- Elfd. _tjippa-s_ "to jump" \
W: \
-- E _chip_ "to trip up, cause to stumble" \
-- MLG _kippen_ "to stumble, stagger, knock over" \
-- Du. dial. _kippen_ "to tip" \
REF: [EDPG]\: 288

### <a id="kippōn-3">\*kippōn- [wk.] "to chop(?)" (v)</a>
N: \
-- ON _kippa_ "to pull, snatch" \
-- Icel. _kippa_ "to jerk" \
-- Far. _kippa_ "to cut off" \
W: \
-- OE _cippian_ "to chop, cut", E _chip_ "to cut away" \
-- MDu. _kippen_ "to catch" \
REF: [EDPG]\: 289

### <a id="kisila">\*kisila- [m.] "gravel" (n)</a>
W: \
-- OE _ciosol_, _ceosol_, _cisel_ "pebble" \
-- MDu. _kesel_ "id.", Du. _kiezel_ [c.] "gravel; pebble" \
-- OHG _kisel_ "pebble" \
REF: [EDPG]\: 289

### <a id="kittōn">\*kit(t)ōn- [wk.] "to tickle" (v)</a>
N: \
-- ON _kitla_ \
-- Nw. _kita_, _kitla_, _kisla_ \
-- Elfd. _tjissla_ \
W: \
-- OE _citelian_, E _tickle_ (with metathesis) \
-- ODu. _kitilon_, Du. _kietelen_ \
-- OHG _kizzilōn_, _kuzzilōn_, MHG _kitzen_, _kitzeln_, G _kitzeln_ \
REF: [EDPG]\: 289

### <a id="kiþa">\*kiþa- [m.] "sprout, bud" (n)</a>
W: \
-- OE _cīð_ "young shoot, sprig" \
-- OS _kīth_ "germ, sprout" \
-- OHG _kīdi_ [n.] "shoot", MHG _kīde_ [n.] "id." (< \*kīþja-) \
REF: [EDPG]\: 289

### <a id="kizna">\*kizna- [m.] "pine tree" (n)</a>
W: \
-- OE _cēn_ "pine tree, spruce" \
-- MLG _kēn_ "pine cone, pinewood" \
-- OHG _kien_ [m./n.] "pine tree, pinewood torch", G _Kien_ "resinous pinewood" \
REF: [EDPG]\: 289

### <a id="klagō">\*klagō- [f.] "complaint, lament" (n)</a>
W: \
-- OS _klaga_ \
-- MDu. _clage_, _claech_ \
-- OHG _klaga_, G _Klage_ \
REF: [EDPG]\: 290

### <a id="klainja">\*klainja- "fine" (adj)</a>
W: \
-- OE _clǣne_ "clean", E _clean_ \
-- MDu. _cleine_ "small, fine, neat", Du. _klein_ "small" \
-- OHG _kleini_ "small, delicate, fine, pure, clean", G _klein_ "small" \
REF: [EDPG]\: 290

### <a id="klaiþa">\*klaiþa- [n.] "cloth" (n)</a>
W: \
-- OE _clōð_, E _cloth_ \
-- OFri. _klāth_, _klēth_, WFri. _klaad_ \
-- Du. _kleed_ \
-- OHG _kleid_, G _Kleid_ \
REF: [EDPG]\: 290

### <a id="klaiwiz">\*klaiwiz- [n.] "clover" (n)</a>
W: \
-- OE _clāfre_, _clǣfre_ [f.], E _clover_ \
-- OFri. _klā_(?) [m.] (?), WFri. _klaver_ [c.], EFri. _kleeuwer_ [m.] \
-- OS _klē_ \
-- MDu. _clavere_ [f.], Du. _klaver_ [c.] \
-- OHG _klē(o)_ [m./n.], G _Klee_ [m.] \
REF: [EDPG]\: 290

### <a id="klajja">\*klajja- [m.] "clay" (n)</a>
W: \
-- OE _clǣg_, E _clay_ \
-- OFri. _klai_ [m.(?)], WFri. _klaai_ [c.] \
-- MDu. _clei_ [n.], Du. _klei_ [c.] \
REF: [EDPG]\: 291

### <a id="klammjan">\*klammjan- [wk.] "to clamp; to jam" (v)</a>
W: \
-- OE _clemman_ "to fetter, bind, enclose" \
-- Du. _klemmen_ "id." \
-- OHG _klemmen_ "id.", G _klemmen_ "id." \
REF: [EDPG]\: 291

### <a id="klangjan">\*klangjan- ~ \*klankjan- [wk.] "to make stick" (v)</a>
N: \
-- ON _klengja-st_ "to interfere (< "to make oneself cling")" \
-- Elfd. _klaindja_ "to clamber" \
W: \
-- OE _clengen_ "to adhere, remain", _be-clencan_ "to make cling", E _to clench_, _clinch_ \
-- OHG _klenken_ "to tie, bind" \
REF: [EDPG]\: 291

### <a id="klankwjan">\*klankwjan- [wk.] "to make shrink" (v)</a>
N: \
-- ON _kløkkva_ "to make weak, weaken" \
-- Far. _kløkka_ "to frighten" \
REF: [EDPG]\: 291

### <a id="klaubjan">\*klaubjan- [wk.] "to split" (v)</a>
N: \
-- ON _kleyfa_ \
-- Far. _kloyva_ \
REF: [EDPG]\: 291

### <a id="klawō">\*klawō- [f.] "claw" (n)</a>
W: \
-- OE _clāwu_, _clā_, E _claw_ \
-- Du. _klauw_ [c.] \
-- OHG _klāwa_, _klā_, _klō(a)_, G _Klaue_ \
REF: [EDPG]\: 292

### <a id="kleuban">\*kleuban- [st.] "to cleave, split" (v)</a>
N: \
-- ON _kljúfa_ \
-- Far. _klúgva_ \
-- Elfd. _kliuova_ \
W: \
-- OE _clēofan_, E _to cleave_ \
-- OS _klioƀan_ \
-- Du. _klieven_ [wk.] \
-- OHG _klioban_, G _klieben_ [st./wk.] \
REF: [EDPG]\: 292

### <a id="klewan">\*klewan- [m.] "lump, ball" (n)</a>
N: \
-- ON _klé_, obl. _kljá_ "stone to keep the warp straight in the upright loom" \
-- Far. _klíggja-steinur_ "id." \
-- Nw. _kljå_ "id." \
REF: [EDPG]\: 292

### <a id="kliba">\*kliba- [n.] "cliff" (n)</a>
N: \
-- ON _klif_ "cliff, protruding isolated rock" \
-- Far. _kliv_ [f.] "steep place, cliff, rock" \
-- Elfd. _kliv_ "id." \
W: \
-- MDu. _clif_ "cliff, steep, slope" \
-- OHG _klep_ "id." \
REF: [EDPG]\: 292

### <a id="klīban">\*klīban- [st.] "to stick" (v)</a>
N: \
-- ON _klífa_ "to climb" \
-- Far. _klúgva_ (_klíva_) "id." \
-- Elfd. _klaiva_ "to climb" \
W: \
-- OS _bi-klíban_ \
-- Du. _be-klijven_ \
-- OHG _klīban_ \
REF: [EDPG]\: 292

### <a id="klibēn">\*klibēn- [wk.] "to stick" (v)</a>
W: \
-- OE _clifian_, _cleofian_, E _to cleave_ \
-- OS _kliƀon_ \
-- EDu. _kleven_ \
-- OHG _klebēn_, G _kleben_ \
REF: [EDPG]\: 293

### <a id="klimman">\*klimman- ~ \*klimb/pan- [st.] "to clamp; to climb" (v)</a>
W: \
-- OE _climban_, _climman_ "to climb", E _to climb_ \
-- MDu. _climmen_, _clemmen_ "id.", Du. _klimmen_ "id." \
-- OHG _klimban_ "id.", MHG _klimmen_, _klimpfen_ "to squeeze; to climb", G _klimmen_ "to climb" \
REF: [EDPG]\: 293

### <a id="klingan">\*klingan- [st.] "to stick, cling" (v)</a>
W: \
-- OE _clingan_, E _to cling_ \
-- MDu. _clingen_ \
REF: [EDPG]\: 293

### <a id="klingan">\*klingan- ~ \*klinkan- [st.] "to sound, ring" (v)</a>
N: \
-- Elfd. _klingga_ \
W: \
-- E _to clink_ \
-- OFri. _klinga_, _klinka_ \
-- MLG _klingen_ \
-- MDu. _clingen_, _clinken_, Du. _klinken_ \
-- OHG _klingan_, G _klingen_ \
REF: [EDPG]\: 293

### <a id="klingwan">\*kling(w)an- ~ \*klinkwan- [st.] "to shrink" (v)</a>
N: \
-- Icel. _klökkva_ "to be moved, cringe, become afraid" \
-- Nw. _kløkka_ "to be shocked, appalled" \
-- ODa. _klykke_ "to be moved" \
W: \
-- OE _clingan_ "to shrink" \
-- Du. _in-klinken_ "to settle (of sediment, clay)" \
REF: [EDPG]\: 293

### <a id="klinnan">\*klinnan- [st.] "to sound, resound" (v)</a>
W: \
-- OFri. _klinnan_ \
REF: [EDPG]\: 294

### <a id="kluban">\*kluban- [m.] "cleft, rift" (n)</a>
N: \
-- ON _klofi_ \
W: \
-- MDu. _clove_ [m./f.], Du. _kloof_ [c.] \
REF: [EDPG]\: 294

### <a id="klumpan">\*klumpan- [m.] "lump" (n)</a>
W: \
-- E _clump_ \
-- Du. _klomp_ [c.] \
-- G _Klumpen_ \
REF: [EDPG]\: 294

### <a id="klunjan">\*klunjan- [wk.] "to sound?" (v)</a>
W: \
-- OE _clynnan_, _clynian_ "to sound, resound; to cause to sound, knock" \
-- MDu. _clonen_ "to hit", Du. _kleunen_ "id." \
REF: [EDPG]\: 294

### <a id="klunkōn">\*klunk/gōn- [wk.] "to sound, ring" (v)</a>
W: \
-- E _to clunk_ "to sound; to hit" \
-- MDu. _clongen_, _clonken_ "to sound" \
REF: [EDPG]\: 294

### <a id="knabila">\*knabila- [m.] "(horizontal) bar" (n)</a>
N: \
-- ON _knefill_ "post, pole" \
W: \
-- MLG _knevel_ "short thick bail, toggle" \
-- EDu. _knevel_ "sinew, strap; fetter", Du. _knevel_ [c.] "gag" \
-- OHG _knebil_ "cudgel", G _Knebel_ "handle, toggle, gag" \
REF: [EDPG]\: 294

### <a id="knaban">\*knab/ppan- [m.] "knob; boy" (n)</a>
N: \
-- Nw. _knape_ "guy; pin" \
W: \
-- OE _cnafa_, _cnapa_ "boy", E _knave_ \
-- OFri. _knap(p)a_ "id." \
-- OS _knapo_ "id." \
-- ODu. _knapo_ "id.", Du. _knaap_ [c.] "boy; coat hanger" \
-- OHG _knabo_, _knappo_ "boy", G _Knabe_, _Knappe_ "boy", dial. "peg" \
REF: [EDPG]\: 294

### <a id="knawa">\*knawa- "strong" (adj)</a>
N: \
-- ON _knár_ "hardy, vigorous; having strength" \
-- Far. _knáur_ "skilful, able; strong; persevering" \
REF: [EDPG]\: 295

### <a id="knean">\*knean- [st.] "to know" (v)</a>
N: \
-- ON _kná_, pl. _knegum_ pret.-pres. "to be able" \
W: \
-- OE _ge-cnāwan_ "to know, perceive, understand, recognise" \
-- OS _and-knāan_ "to understand", _bi-knāan_, _bi-knegan_ "to attain" \
-- OHG _bi-knāen_ [wk.] "to recognize" \
REF: [EDPG]\: 295

### <a id="knedan">\*knedan- ~ \*knudan- [st.] "to knead" (v)</a>
N: \
-- ON _knoða_ \
-- Far. _knoða_ [wk.] \
-- Elfd. _knoða_ [wk.] \
W: \
-- OE _cnedan_, E _knead_ \
-- OS _knedan_ [wk.], MLG _kneden_ \
-- MDu. _cneden_, Du. _kneden_ [wk.] \
-- OHG _knetan_, _knedan_, G _kneten_ \
REF: [EDPG]\: 295

### <a id="knēi">\*knēi- "acknowledging" (adj)</a>
W: \
-- OE _ge-cnǣwe_ "in agreement with, acknowledging" \
REF: [EDPG]\: 296

### <a id="knewa">\*knewa- [n.] "knee" (n)</a>
E: \
-- Go. _kniu_ \
N: \
-- ON _kné_ [m.] "knee; node" \
-- Far. _knæ_ "id." \
-- Elfd. _kni_ "id." \
W: \
-- OE _cnēo(w)_ \
-- OS _knio_ \
-- Du. _knie_ [c.] \
-- OHG _kneo_, G _Knie_ \
REF: [EDPG]\: 296

### <a id="knewan">\*knewan- [m.] "relative" (n)</a>
N: \
-- ON poet. _kníar_ [m.pl.] "men" \
W: \
-- OFri. _kniā_ "relative" \
-- MDu. _even-cnie_ [c.] "of equal birth", Du. _even-knie_ [c.] "match" \
REF: [EDPG]\: 296

### <a id="knīban">\*knība(n)- [m.] "knife" (n)</a>
N: \
-- ON _knífr_ "knife, dirk" \
-- Far. _knívur_ "id." \
-- Elfd. _knaiv_ "id." \
W: \
-- OE _cnīf_ \
-- G Swab. _Kneip(eⁿ)_ "large knife", Swi. (App.) _gniippə_ "id." (< \*knībban-) \
REF: [EDPG]\: 296

### <a id="knīēn">\*knīēn- [wk.] "to investigate" (v)</a>
N: \
-- Icel. _knjá_ \
REF: [EDPG]\: 296

### <a id="knikkōn">\*knikkōn- [wk.] "to snap, fold" (v)</a>
N: \
-- Nw. _knikka_ "to kneel" \
W: \
-- MDu. _cnicken_ "to snap", Du. _knikken_ "to nod, kink" \
-- G _knicken_ "to snap, bend" \
REF: [EDPG]\: 297

### <a id="knīpan">\*knīpan- [st.] "to pinch" (v)</a>
W: \
-- Du. _knijpen_ "to pinch" \
-- G dial. _kneipen_ "to pinch" (< \*knībban-) \
REF: [EDPG]\: 297

### <a id="knippōn">\*knipp/bōn- [wk.] "to cut, snap" (v)</a>
W: \
-- MLG _cnippen_ \
-- Du. _knippen_ "to cut", dial. "to pinch" \
-- G Pal. _knippen_ "to castrate; to pluck" \
REF: [EDPG]\: 297

### <a id="knōdi">\*knōdi- [f.] "lineage" (n)</a>
E: \
-- Go. _knoþs_ "family, clan" \
W: \
-- OHG _knuot_ "nature, descent" \
REF: [EDPG]\: 297

### <a id="knūjan">\*knūjan- [wk.] "to press(?)" (v)</a>
N: \
-- ON _knýja_ "to knock; to press, to push, urge on" \
REF: [EDPG]\: 297

### <a id="knukkan">\*knuk(k)an- [m.] "knuckle" (n)</a>
N: \
-- Nw. _knoke_ [f.] "bone, joint" (< \*knukōn-) \
W: \
-- MDu. _cnoke_, _cnoc(ke)_ "knuckle, joint, vertebra" \
-- MHG _knoche_ "bone", G _Knochen_ "id." \
REF: [EDPG]\: 298

### <a id="knukkōn">\*knukkōn- [f.] "bundle" (n)</a>
W: \
-- MLG _knocke_, _knucke_ "bundle of flax" \
-- G _Knocke_ [m./f.] "id." \
REF: [EDPG]\: 298

### <a id="knussjan">\*knussjan- [wk.] "to kneel" (v)</a>
E: \
-- Go. _knussjan_ \
REF: [EDPG]\: 298

### <a id="knuttōn">\*knuttōn- [wk.] "to press, crush" (v)</a>
W: \
-- G dial. _knotzen_ "to press, knead, squeeze" \
REF: [EDPG]\: 298

### <a id="knūþan">\*knūþan- ~ \*knuttan- [m.] "knot" (n)</a>
N: \
-- ON _knítr_ "knot; hump, protuberance", Icel. _hnúði_ "knob, hump" \
-- Far. _knútur_ \
W: \
-- OE _cnotta_, E _knot_ \
-- Du. _knot_ [c.] \
-- OHG _knodo_, _knoto_, G _Knoten_ \
REF: [EDPG]\: 298

### <a id="kō">\*kō- ~ \*kū- [f.] "cow" (n)</a>
N: \
-- ON _kýr_ \
-- Far. _kúgv_ \
-- Elfd. _tjyr_ \
W: \
-- OE _cū_, pl. _cȳ_, E _cow_ \
-- OFri. _kū_, WFri. _ko_ [c.], EFri. _kuu_ \
-- OS _kō_ \
-- Du. _koe_ [c.] \
-- OHG _kuo_, G _Kuh_ \
REF: [EDPG]\: 299

### <a id="kōkan">\*kōkan- [m.] "cookie" (n)</a>
W: \
-- MDu. _coeke_ [m./f.], Du. _koek_ [c.] \
-- OHG _kuohho_, G _Kuchen_ \
REF: [EDPG]\: 299

### <a id="kōkōn">\*kōkōn- [m.] "skid" (n)</a>
W: \
-- MLG _kōke_ [f.] "sledge runner" \
-- OHG _slito-kōhha_ [f.] "skid", G _Kufe_ [f.] "runner, blade, skid" (with -_f_- < -_ch_-?) \
REF: [EDPG]\: 299

### <a id="kōlu">\*kōlu- "cool" (adj)</a>
W: \
-- OE _cōl_, E _cool_ \
-- MDu. _coele_, Du. _koel_ \
-- OHG _kuoli_, G _kühl_ \
REF: [EDPG]\: 299

### <a id="kōni">\*kōni- "able" (adj)</a>
N: \
-- ON _kœnn_ "wise, skilful, expert" \
-- Far. -_kønur_ "id." \
-- Elfd. _tjyōn_ "articulate" \
W: \
-- OE _cēne_ "keen, fierce, bold", E _keen_ \
-- MLG _kōne_ "bold" \
-- MDu. _coene_ "id.", Du. _koen_ "id." \
-- OHG _kuoni_ "bold, strong, grim", G _kühn_ "bold, audacious" \
REF: [EDPG]\: 299

### <a id="krabban">\*krabban- [m.] "crab, crayfish" (n)</a>
N: \
-- ON _krabbi_ \
-- Far. _krabbi_ \
W: \
-- OE _crabba_, E _crab_ \
-- MDu. _crabbe_, Du. _krab_ [c.] \
REF: [EDPG]\: 299

### <a id="kraben">\*kraben- [wk.] "to urge, demand" (v)</a>
N: \
-- ON _krefja_ "to demand" \
-- Far. _krevja_ "to demand", poet. "to urge" \
-- Elfd. _kreva_ "id." \
W: \
-- OE _crafian_ "to ask, crave, implore, demand", E _to crave_ \
REF: [EDPG]\: 300

### <a id="krabita">\*krabita- [m.] "crayfish" (n)</a>
W: \
-- OS _kreƀit_ \
-- MDu. _crevet_, _creeft_ [m./n.], Du. _kreeft_ [c.] \
-- OHG _krebazzo_, _krebiz_, MHG _krebez_, _krebz_, G _Krebs_ [f.] \
REF: [EDPG]\: 300

### <a id="kraftu">\*kraftu- [m.] "strength, force" (n)</a>
N: \
-- ON _krǫftr_ \
-- Far. _kraft_ [f.] \
-- Elfd. _krapt_ [f.] \
W: \
-- OE _cræft_, E _craft_ \
-- OS _kraft_ [f.] \
-- Du. _kracht_ [c.] \
-- OHG _kraft_ [f.], G _Kraft_ [f.] \
REF: [EDPG]\: 300

### <a id="kragan">\*kragan- [m.] "throat; collar" (n)</a>
W: \
-- ME _craue_ "neck, throat; crop", E _craw_ \
-- MDu. _crage_ "neck, throat", Du. _kraag_ [c.] "collar" \
-- MHG _krage_, G _Kragen_ \
REF: [EDPG]\: 300

### <a id="kramjan">\*kramjan- [wk.] "to squeeze" (v)</a>
N: \
-- ON _kremja_ "to squeeze, bruise" \
REF: [EDPG]\: 300

### <a id="kramma">\*kramma- "moist" (adj)</a>
N: \
-- ON _krammr_ "half thawed" \
-- Icel. _kramur_ "id." \
-- Nw. _krammr_ "id." \
REF: [EDPG]\: 300

### <a id="krammōn">\*krammōn- [wk.] "to squeeze" (v)</a>
W: \
-- OE _crammian_ "to stuff", E _to cram_ \
-- MHG _krammen_ "to grab with one's claws" \
REF: [EDPG]\: 301

### <a id="krampa">\*krampa- "crooked" (adj)</a>
N: \
-- ON _krappr_ "sharp, crafty" \
-- Far. _krappur_ "acute-angled; bent, curved" \
-- Elfd. _kropp_ "bent, crooked; sad" \
W: \
-- MDu. _cramp_ "convulsive" \
-- OHG _krampf_ "id." \
REF: [EDPG]\: 301

### <a id="kranan">\*krana/ōn- [m./f.] "crane" (n)</a>
N: \
-- ON _trani_ [m.], _trana_ [f.] \
-- Far. _trani_ [m.] \
-- Elfd. _trånå_ [f.] \
W: \
-- OE _cran_ [m.], E _crane_ \
-- OS _krano_ [m.] \
-- Du. _kraan-vogel_ [c.] \
-- MHG _krane_ [m.], G _Krahn_ [m.] \
REF: [EDPG]\: 301

### <a id="kranga">\*kranga- ~ \*kranka- "bent, crooked, weak" (adj)</a>
N: \
-- ON _krangr_ "weak, frail" \
W: \
-- MDu. _cranc_ ad. "weak, ill, insignificant", Du. _krang_ "inside-out" \
-- MHG _krank_ "weak, thin", G _krank_ "ill" \
REF: [EDPG]\: 301

### <a id="krangjan">\*krangjan- [wk.] "to make bend" (v)</a>
W: \
-- E _to cringe_ \
-- MDu. _crenghen_ "to make turn" \
REF: [EDPG]\: 302

### <a id="kranhō">\*kranhō- [f.] "corner" (n)</a>
N: \
-- ON _krá_, _kró_ \
-- Nw. _krå_, pl. _krær_ \
REF: [EDPG]\: 302

### <a id="krappōn">\*krapp/bōn- [wk.] "to scratch" (v)</a>
W: \
-- MLG _krabben_ \
-- MDu. _crabben_, Du. _krabben_, _krappen_ dial. _kraven_ \
-- G _krappen_, _kraben_ \
REF: [EDPG]\: 302

### <a id="krattan">\*kratt/dan- [m.] "basket, wickerwork container" (n)</a>
N: \
-- ON _kartr_ "cart" (< OE \*_cart_?) \
W: \
-- OE _cræt_ [n.] "chariot" \
-- MDu. _cratte_ "wickerwork, hurdle", Du. _krat_ [c.] "crate" \
-- OHG _kratto_, _kretto_, _kretzo_ "basket", MHG _kratte_, _kretze_ "id.", G _Kratte_ "id." \
REF: [EDPG]\: 302

### <a id="krausja">\*krausja- [n.] "bowels" (n)</a>
W: \
-- MDu. _kroos_ "bowels (esp. of geese); gristle, cartilage", Du. obs. _kroos_ "bowels of a calf" \
-- MHG _krœse_, _ge-krœse_ "guts", G _Ge-kröse_ "id." \
REF: [EDPG]\: 303

### <a id="kreban">\*kreban- ~ \*kruppan- [m.] "basket" (n)</a>
W: \
-- EFri. _krääf_, _krääwe_ "trough, crib" \
-- MLG _karve_ [f.] "creel" \
-- OHG _korb_ "basket", MHG _krebe_, _kreppe_ [f.], _korb(e)_ "id.", G _Korb_ "id." \
REF: [EDPG]\: 303

### <a id="krēbi">\*krēbi- "to be claimed" (adj)</a>
N: \
-- ON _kræfr_ "strong" \
-- Icel. _kræfur_ "tough; due for payment" \
-- Nw. dial. _kræv_ "strong, tough" \
REF: [EDPG]\: 303

### <a id="kreudan">\*kreudan- ~ \*krūdan- [st.] "to push forward" (v)</a>
N: \
-- Nw. dial. _kryda_ "to gather, flock; to crawl slowly" \
W: \
-- OE _crēodan_, _crūdan_ "to crowd, press, drive" \
-- MDu. _cruden_ [wk.] "to push forth/aside", Du. _kruien_ [wk.] "to push a wheelbarrow; to pile up (of ice floes)" \
REF: [EDPG]\: 303

### <a id="kreukan">\*kreukan- ~ \*krūkan- [st.] "to fold(?)" (v)</a>
N: \
-- Nw. _krjuka_ "to cringe; to crawl" \
W: \
-- WFri. _krûke_ [wk.] "to cringe" \
REF: [EDPG]\: 304

### <a id="kreupan">\*kreupan- ~ \*krūpan- [st.] "to crawl" (v)</a>
N: \
-- ON _krjúpa_ "to creep, crouch; to kneel" \
-- Far. _krúpa_ "to creep, crawl; to drag oneself forth" \
-- Elfd. _kriuopa_ "to kneel" \
W: \
-- OE _crēopan_, E _to creep_ \
-- OFri. _kriāpa_ [wk.] \
-- MDu. _crupen_, Du. _kruipen_ \
-- OHG _kriofan_, MHG _krūfen_, G Rhnl. _krufen_ \
REF: [EDPG]\: 304

### <a id="kriagi">\*kriagi- "capable of fighting" (adj)</a>
W: \
-- MHG _kriege_ "reluctant, militant" \
REF: [EDPG]\: 304

### <a id="krīgan">\*krīgan- [st.] "to be stubborn" (v)</a>
W: \
-- MDu. _crigen_ "to strive, fight; to obtain", Du. _krijgen_ "to get" \
REF: [EDPG]\: 304

### <a id="krimman">\*krimman- [st.] "to crumble(?)" (v)</a>
W: \
-- OE _crimman_ "to crumb, crumble, mingle" \
-- OHG _krimman_ "to bite, hurt; to tear up; to squeeze, stuff", MHG _krimmen_, _grimmen_ "to grab, squeeze", G Tyr. _krimmen_ "to push into" \
REF: [EDPG]\: 305

### <a id="krimpan">\*krimpan- [st.] "to shrink" (v)</a>
N: \
-- ON _kreppa_ "to cringe" \
W: \
-- MDu. _crempen_, Du. _krimpen_ \
-- OHG _krimfan_, MHG _krimpfen_ \
REF: [EDPG]\: 305

### <a id="kringa">\*kringa- [m.] "circle" (n)</a>
N: \
-- ON _kringr_ \
-- Far. _kringur_ \
W: \
-- Du. _kring_ [c.] \
-- MHG _kring_ \
REF: [EDPG]\: 305

### <a id="kringan">\*kringan- ~ \*krinkan- [st.] "to fall" (v)</a>
W: \
-- OE _cringan_, _crincan_ "to yield, fall" \
REF: [EDPG]\: 305

### <a id="kriwa">\*kriwa- "lively, healthy" (adj)</a>
N: \
-- Nw. _kry_ \
-- Sw. _kry_ \
REF: [EDPG]\: 306

### <a id="krubjan">\*krubjan- [wk.] "to disembowel" (v)</a>
N: \
-- ON _kryfja_ \
REF: [EDPG]\: 306

### <a id="krūda">\*krūda- [m.] "herb" (n)</a>
W: \
-- OS _krūd_ [n.] \
-- Du. _kruid_ [n.] \
-- OHG _krūt_ [n.], G _Kraut_ [n.] \
REF: [EDPG]\: 306

### <a id="kruddan">\*krudda/ōn- [m./f.] "curds(?)" (n)</a>
N: \
-- Nw. _krodde_ [m.] "dregs (of curdled milk, coffee, beer, grease etc.)", [f.] "boiled cheese" \
W: \
-- ME _crudde_, _curde_ "curds", E _curds_ \
REF: [EDPG]\: 306

### <a id="kruddōn">\*krud(d)ōn- [f.] "toad" (n)</a>
W: \
-- MDu. _crode_, EDu. _krodde_ \
-- OHG _krota_, _krotta_, G _Kröte_ \
REF: [EDPG]\: 306

### <a id="krumba">\*krumba- ~ \*krumpa- "crooked, bent" (adj)</a>
W: \
-- OE _crump_ \
-- OS _krumb_ \
-- Du. _krom_ \
-- OHG _krumpf_, _krumb_, MHG _krumpf_, _krump_, G _krumm_ \
REF: [EDPG]\: 307

### <a id="krunkōn">\*krunkōn- [wk.] "to wind" (v)</a>
W: \
-- EDu. _kronken_, Du. _kronkelen_ "to wind, squirm" \
REF: [EDPG]\: 307

### <a id="kruppa">\*kruppa- [m.] "compact object" (n)</a>
N: \
-- ON _kroppr_ "body, trunk" \
-- Far. _kroppur_ "body, build; corpse" \
-- Sw. _krupp_ "body" \
W: \
-- OE _crop_ "sprout, bunch, rock", E _crop_ \
-- MDu. _crop(pe)_ "gnarl, bud, rosette; body; goiter", Du. _krop_ "goiter; rosette (of lettuce)" \
-- OHG _kropf_ "goiter", G _Kropf_ "id." (oRv?) \
REF: [EDPG]\: 307

### <a id="kruppōn">\*krupp/bōn- [wk.] "to crawl" (v)</a>
W: \
-- MDu. _cropen_ \
-- G Rhnl. _kruppen_ "to crouch, curl up", Cimb. _kruppen_ "to crawl" \
REF: [EDPG]\: 307

### <a id="kruppila">\*krup(p)ila- "cripple" (adj)</a>
N: \
-- ON _kryppill_ \
-- Far. _krypil_ \
W: \
-- OE _crypel_ \
-- MDu. _cropel_, Du. _kreupel_ \
REF: [EDPG]\: 307

### <a id="kruppjan">\*kruppjan- [wk.] "to bend, stoop" (v)</a>
W: \
-- OE _cryppan_ "to bend" \
-- G _krüpfen_, _krupfen_ "to bend, refl. to cringe" \
REF: [EDPG]\: 308

### <a id="kruttōn">\*krutt/dōn- [wk.] "to pack(?)" (v)</a>
N: \
-- Nw. _kroda_ "to huddle (around)", _krota_ "to coagulate, curdle" \
W: \
-- ME _crudden_ "to coagulate, congeal, solidify, curdle" \
-- G obs. _krotten_ "to swell", Swi. _xrotten_ "to pack, stuff" \
REF: [EDPG]\: 308

### <a id="kuban">\*kuban- [m.] "shed" (n)</a>
N: \
-- ON _kofi_ "convent cell; hut, shed" \
-- Far. _kovi_ "outbuilding" \
-- Elfd. _kuvi_ "chamber" \
W: \
-- OE _cofa_ "chamber, cave, den" \
-- MDu. _cove_ "hut, shed" \
-- MHG _kobe_ "stable" (also kobel "shed, box"), G _Koben_ "shed" \
REF: [EDPG]\: 308

### <a id="kuddan">\*kuddan- [m.] "bag" (n)</a>
N: \
-- ON _koddi_ "pillow" \
-- Far. _koddi_ "id." \
-- Nw. _kodd(e)_ "pillow; scrotum, testicle" \
-- OSw. _kodde_ "testicle" \
W: \
-- OE _codd_ "bag, scrotum; husk" \
-- EDu. _kodde_ "testicle" \
REF: [EDPG]\: 308

### <a id="kudja">\*kudja- [n.] "herd" (n)</a>
W: \
-- OFri. _kedde_ [m.] \
-- MDu. _cudde_ [n./f.], Du. _kudde_ [c.] \
-- OHG _kutti_, G Bav. _kütt_ [n./f.] "flock of birds" \
REF: [EDPG]\: 308

### <a id="kukjan">\*kukjan- [wk.] "to kiss" (v)</a>
E: \
-- Go. _kukjan_ \
W: \
-- LG _kükken_ \
-- Du. dial. _kukken_ \
REF: [EDPG]\: 309

### <a id="kula-1">\*kula- [n.] "cool wind" (n)</a>
N: \
-- ON _kul_, _kol_ \
REF: [EDPG]\: 309

### <a id="kula-2">\*kula- [n.] "coal, charcoal" (n)</a>
N: \
-- ON _kol_ [n.pl.] \
-- Far. _kol_ \
-- Elfd. _kuol_ \
W: \
-- OE _col_, E _coal_ \
-- OFri. _kole_ [f.] \
-- MDu. _cole_ [m./f.], Du. _kool_ [n./c.] \
-- OHG _kol_, _kolo_ [m.], MHG _kol_, G _Kohle_ [f.] \
REF: [EDPG]\: 309

### <a id="kulban">\*kulba(n)- [m.] "round object" (n)</a>
N: \
-- ON _kolfr_ "bulb; bolt, arrow" \
-- Far. poet. _benjar-kolvur_ "sword" \
-- Elfd. _kuov_ "arrow" \
W: \
-- OS _kolƀo_ "club, cudgel, thong" \
-- Du. _kolf_ [c.] "butt; flask, ear of corn" \
-- OHG _kolbo_ "cob, cudgel", G _Kolben_ "spadix; flask" \
REF: [EDPG]\: 309

### <a id="kulda">\*kulda- [m.] "litter (of progeny)" (n)</a>
N: \
-- Nw. dial. _kull_, _kuld_ [m./f.] \
-- OSw. _koller_, _kolder_ \
-- Da. _kuld_ [c.] \
REF: [EDPG]\: 309

### <a id="kūma">\*kūma- "miserable(?)" (adj)</a>
W: \
-- MHG _kūm_ "thin, weak" \
REF: [EDPG]\: 310

### <a id="kumban">\*kumb/pan- [m.] "basin, bowl" (n)</a>
W: \
-- OE _cumb_ "hollow among hills, narrow valley; liquid measure", E _comb_ \
-- MLG _kump_ "basin, tub" \
-- MDu. _com(me)_ [m./f.] "bowl, box; valley", Du. _kom_ [c.] "bowl; valley between dunes" \
-- MHG _kumpf_ "cup, measure for corn and fruit", G _Kumpf_ "id." \
REF: [EDPG]\: 310

### <a id="kumbra">\*kumbra- [m.] "sorrow; burden" (n)</a>
W: \
-- OFri. _kummer_ [m./n.] "sorrow, impediment, arrest, seizure" \
-- MDu. _commer_, _comber_ "trouble, hindrance, arrest, seizure", Du. _kommer_ [c.] "sorrow, misery" \
-- MHG _kumber_ "burden, sorrow" \
REF: [EDPG]\: 310

### <a id="kunda">\*kunda- "born" (adj)</a>
E: \
-- Go. -_kunds_ \
N: \
-- ON -_kundr_ \
W: \
-- OE -_cund_ \
REF: [EDPG]\: 310

### <a id="kundjan">\*kundjan- [wk.] "to kindle" (v)</a>
N: \
-- ON _kynda_ \
-- Far. _kynda_ \
W: \
-- MHG _kunten_ \
REF: [EDPG]\: 310

### <a id="kuninga">\*kuninga- ~ \*kununga- [m.] "king" (n)</a>
N: \
-- ON _konungr_ \
-- Far. _kongur_ \
-- Elfd. _kunungg_ \
W: \
-- OE _cyning_, _cyng_, E _king_ \
-- OFri. _kening_, _keneg_, _kining_, _kinig_ \
-- OS _kuning_ \
-- Du. _koning_ [c.] \
-- OHG _kuning_, _kunig_, G _König_ \
REF: [EDPG]\: 311

### <a id="kunja-1">\*kunja- [n.] "clan" (n)</a>
E: \
-- Go. _kuni_ "clan, tribe, generation" \
N: \
-- ON _kyn_ "kin; kind, type; gender" \
-- Far. _kyn_ "family, stock; gender; nature" \
-- Elfd. _tjyōn_ "gender" \
W: \
-- OE _cynn_ "sort, kind" \
-- OS _kunni_ "lineage; generation; kind" \
-- ODu. _cunni_ "id." \
-- OHG _kunni_ [m.] "id.", MHG _künne_ "id." \
REF: [EDPG]\: 311

### <a id="kunja-2">\*kunja- [n.] "omen" (n)</a>
N: \
-- ON _kyn_ "miracle" \
REF: [EDPG]\: 311

### <a id="kunnan">\*kunnan- [pp.] "to know (how), be able" (v)</a>
E: \
-- Go. _kunnan_, 3sg. _kann_, 3pl. _kunnun_ "know, understand" \
N: \
-- ON _kunna_, 3sg. _kann_, 3pl. _kunnu_ "id." \
-- Far. _kunna_, 3sg. _kann_, 3pl. _kunnu_, _kunna_ "id." \
-- Elfd. _kunna_ "id." \
W: \
-- OE _cunnan_, 3sg. _can_, 3pl. _cunnon_ "id.", E _can_ \
-- OS _kunnan_, 3sg. _kan_, 3pl. _kunnun_ "id." \
-- Du. _kunnen_, 3sg. _kan_, 3pl. _kunnen_ "id." \
-- OHG _kunnan_ "id.", G _können_, 3sg. _kann_, 3pl. _können_ "id." \
REF: [EDPG]\: 311

### <a id="kunþa">\*kunþa- "known" (adj)</a>
E: \
-- Go. _kunþs_ "known, recognized" \
N: \
-- ON _kunnr_ "known, familiar" \
-- Far. _kunnur_ "id." \
W: \
-- OE _cūþ_ "known, clear" \
-- OHG _kund_, MHG _kunt_ \
REF: [EDPG]\: 312

### <a id="kurna">\*kurna- [n.] "corn, wheat" (n)</a>
E: \
-- Go. _kaurn_ "grain, wheat" \
N: \
-- ON _korn_ "corn, grain" \
-- Far. _korn_ "corn; grain; seed; lump" \
-- Elfd. _kuonn_ "grain" \
W: \
-- OE _corn_ "grain, corn", E _corn_ \
-- OS _korn_ "grain, rye" \
-- Du. _koren_ "grain" \
-- OHG _korn_ "grain; kernel", G _Koren_ "id." \
REF: [EDPG]\: 312

### <a id="kurþra">\*kurþra- ~ \*kurdra- [n.] "flock" (n)</a>
W: \
-- OE _corðor_ "troop" \
-- OHG _kortar_ "herd" \
REF: [EDPG]\: 312

### <a id="kuru">\*kuru- "heavy" (adj)</a>
E: \
-- Go. _kaurus_\* "heavy" \
REF: [EDPG]\: 312

### <a id="kursan">\*kursan- [m.] "bull calf" (n)</a>
N: \
-- ON _kursi_, _kussi_ \
REF: [EDPG]\: 312

### <a id="kurzōn">\*kurzōn- [wk.] "to creak" (v)</a>
N: \
-- ON _kurra_ "to murmur, grumble" \
-- Elfd. _kurra_ "to coo; to purr" \
W: \
-- MDu. _corren_ "to creak" \
REF: [EDPG]\: 313

### <a id="kuspa">\*kuspa- [m.] "fetter" (n)</a>
W: \
-- OE _cosp_, _cops_ "rope, cord, fetter", E _cosp_, _cops(e)_ "hasp for fastening a gate, head of a plough, cross piece at the top of a spade handle" \
-- OS _lithu-kospos_\* [m.pl.] "chains" \
REF: [EDPG]\: 313

### <a id="kussjan">\*kussjan- [wk.] "to kiss" (v)</a>
N: \
-- ON _kyssa_ \
W: \
-- OE _cyssan_, E _to kiss_ \
-- MDu. _cussen_, Du. _kussen_ \
-- OHG _kussen_, G _küssen_ \
REF: [EDPG]\: 313

### <a id="kusti">\*kusti- [f.] "trial" (n)</a>
E: \
-- Go. _ga-kusts_ "test" \
W: \
-- OE _cyst_ "choice" \
-- OFri. _kest_ "choice, statute" \
-- OS _kust_ "choice, preference, glory" \
-- OHG _kust_ "choice, evaluation" \
REF: [EDPG]\: 313

### <a id="kustu">\*kustu- [m.] "trial" (n)</a>
E: \
-- Go. _kustus_ "test, trial" \
N: \
-- ON _kostr_ "choice, alternative; opportunity" \
-- Far. _kostur_ "condition; alternative" \
W: \
-- OE _cyst_ [f.] "choice, election; excellence, virtue" \
-- MDu. _cust_ "choice" \
-- OHG _kust_ "evaluation, trial, choice", MHG _kust_ "id." \
REF: [EDPG]\: 313

### <a id="kuta">\*kuta- [n.] "shed" (n)</a>
N: \
-- ON _kot_ "cottage, hut" \
W: \
-- OE _cot_ "bed-chamber" \
-- MDu. _cot_ "hut, cabin", Du. _kot_ "id." \
REF: [EDPG]\: 313

### <a id="kūta">\*kūta- [n.] "soft tissue" (n)</a>
W: \
-- E Scot _kite_ "stomach" \
-- MDu. _cute_ [n./f.] "calf of the leg; fish roe", Du. _kuit_ [c.] "id." \
REF: [EDPG]\: 314

## Kw

### <a id="kwabban">\*kwabban- [m.] "swamp, bog" (n)</a>
W: \
-- E dial. _quab_ "marshy place" \
-- MLG _quabbe_ "swamp" \
-- Du. _kwab_ [c.] "soft fleshy lobe, outgrowth" \
REF: [EDPG]\: 314

### <a id="kwadjan">\*kwadjan- [wk.] "to greet" (v)</a>
N: \
-- ON _kveðja_ "to address; to greet" \
-- Far. _kvøðja_ "id." \
W: \
-- OS _queddian_ "id." \
-- OHG _quetten_, _chwetten_, _cheten_ \
REF: [EDPG]\: 314

### <a id="kwaikwjan">\*kwaikwjan- [wk.] "to make lively" (v)</a>
N: \
-- ON _kveykva_, _kveikja_ "to light; quicken" \
-- Far. _kveikja_ "to light; to cause, bring about; to refresh" \
W: \
-- MDu. _queken_, _queiken_ "to feed, cherish, cheer up" \
REF: [EDPG]\: 314

### <a id="kwainōjan">\*kwainōjan- [wk.] "to lament, mourn, wail" (v)</a>
E: \
-- Go. _qainon_ \
N: \
-- ON _kveina_ \
-- Far. _kveina-st_ "to be afraid of everything" \
W: \
-- OE _cwānian_ "to lament, mourn" \
REF: [EDPG]\: 314

### <a id="kwaljan">\*kwaljan- [wk.] "to make suffer, pain" (v)</a>
N: \
-- ON _kvelja_ "to torment, torture" \
W: \
-- OE _cwellan_ "to kill" \
-- OS _quellian_ "to torment, kill" \
-- MDu. _quellen_ "to torture, abuse; to suffer", Du. _kwellen_ "to torment, trouble" \
REF: [EDPG]\: 315

### <a id="kwalō">\*kwalō- [f.] "torment" (n)</a>
N: \
-- ON _kvǫl_ "torment" \
W: \
-- OE _cwalu_ "torment, violent death, destruction" \
-- MDu. _quale_, _quael_ "misery, pain; deficiency", Du. _kwaal_ "ailment" \
REF: [EDPG]\: 315

### <a id="kwēbjan">\*kwēbjan- [wk.] "to suffocate, choke" (v)</a>
N: \
-- ON _kvæfa_, _kœfa_ \
-- Icel. _kæfa_ \
-- Far. _køva_ "to choke, put out, extinguish; to darken the sky" \
-- Nw. _kjøva_ "to suffocate; become cloudy" \
W: \
-- MHG _er-queben_ "to suffocate" \
REF: [EDPG]\: 315

### <a id="kwedu-1">\*kwedu- [m.] "utterance" (n)</a>
N: \
-- ON _kviðr_ "verdict; inquest; saying, word" \
W: \
-- OE _cwide_, _cwyde_ "utterance, word, speech, discourse" \
-- OHG _quiti_ [m./n.] "statement, proof" \
REF: [EDPG]\: 315

### <a id="kwedu-2">\*kwedu- [m.] "resin" (n)</a>
W: \
-- OE _cwidu_, _cweodu_, _c(w)udu_ "cud, mastix", E _cud_, _quid_ "ruminated substance; wad of tobacco" \
-- OHG _quiti_, _kuti_, MHG _küte_, _küt_, G _Kitt_ "putty, cement" \
REF: [EDPG]\: 315

### <a id="kwelan">\*kwelan- [st.] "to suffer(?)" (v)</a>
W: \
-- OE _cwelan_ "to die" \
-- OS _quelan_ "to suffer" \
-- MDu. _quelen_ [wk.] "to languish, suffer; to pester" \
-- OHG _quelan_ "to suffer, be angry" \
REF: [EDPG]\: 316

### <a id="kwēlō">\*kwēlō- [f.] "agony" (n)</a>
W: \
-- OS _quāla_ "pangs of death" \
-- OHG _quāla_, _chuāla_ "torment, misery", G _Qual_ "pain, agony" \
REF: [EDPG]\: 316

### <a id="kweman">\*kweman- ~ \*kuman- [st.] "to come" (v)</a>
E: \
-- Go. _qiman_ \
N: \
-- ON _koma_ \
-- Far. _koma_ \
-- Elfd. _kuma_ \
W: \
-- OE _cuman_, E _to come_ \
-- OFri. _kuma_, _koma_ \
-- OS _kuman_ \
-- Du. _komen_ \
-- OHG _queman_, _cuman_, G _kommen_ \
REF: [EDPG]\: 316

### <a id="kwēni">\*kwēni- [f.] "wife" (n)</a>
E: \
-- Go. _qens_ "wife" \
N: \
-- ON _kván_, _kvæn_ "woman, wife" \
W: \
-- OS _quān_ "noblewoman" \
REF: [EDPG]\: 316

### <a id="kwenōn">\*kwenōn- [f.] "woman, wife" (n)</a>
E: \
-- Go. _qino_ "woman" \
N: \
-- ON _kona_, gpl. _kvinna_ "woman, wife" \
-- Far. _kona_ "wife", _kvinna_ "woman" \
-- Elfd. _kunå_ "wife" \
W: \
-- OE _cwene_ "woman", E _queen_ \
-- OS _quena_ "woman, wife, consort" \
-- OHG _quena_ "woman, wife" \
REF: [EDPG]\: 317

### <a id="kweran">\*kweran- [st.] "to sigh" (v)</a>
W: \
-- OHG _queran_ "to lament, moan, sigh" \
REF: [EDPG]\: 317

### <a id="kwerkō">\*kwerkō- [f.] "throat" (n)</a>
N: \
-- ON _kverk_ "the angle below the chin; throat" \
-- Far. _kvørkrar_ [f.pl.] \
-- Nw. _kverk_ [m.] "throat; angle below the chin" \
W: \
-- MLG _querke_, _quarke_ \
-- OHG _querka_, _querkela_ \
REF: [EDPG]\: 317

### <a id="kwernu">\*kwernu- [f.] "mill; millstone" (n)</a>
E: \
-- Go. _asilu-qairnus_ [f.(?)] "donkey-mill" \
N: \
-- ON _kvern_ "millstone, quern" \
-- Far. _kvørn_ "id." \
-- Elfd. _kwenn_ "id." \
W: \
-- OE _cweorn_ "id.", E _quern_ \
-- OS _quern_, _quirn_ "id." \
-- MDu. _querne_, _queern_ "id." \
-- OHG _quirn_ "id.", MHG _kürn(e)_ "id." \
REF: [EDPG]\: 318

### <a id="kwerru">\*kwerru- "quiet, still" (adj)</a>
E: \
-- Go. _qairrus_ "gentle, mild" \
N: \
-- ON _kvirr_, _kyrr_ "calm" \
-- Icel. _kyr_ "id." \
-- Far. _kvirrur_, _kyrrur_ "calm" \
-- Elfd. _kwer(e)_ "id." \
W: \
-- MLG _querre_ "tame" \
-- MHG _kürre_ "mild", G _kirre_ "tame, compliant" \
REF: [EDPG]\: 318

### <a id="kwerthra-1">\*kwerþra- [m./n.] "bait" (n)</a>
W: \
-- OHG _querdar_ [m./n.], MHG _querder_ [m./n.], G _Köder_ [m.] \
REF: [EDPG]\: 318

### <a id="kwerthra-2">\*kwerþra- [m./n.] "cut off piece, edge(?)" (n)</a>
W: \
-- MLG _querder_, _querdel_, _korder_ [m./n.] "patch (of leather, wick)" \
-- MDu. _querdel_, _quardel_ [m./n.] "strap of shoe-leather" \
-- OHG _querdar_ [m./n.] "wick", MHG _querder_ [m./n.] "patch" \
REF: [EDPG]\: 318

### <a id="kweþan">\*kweþan- [st.] "to say" (v)</a>
E: \
-- Go. _qiþan_ "to say, tell, express" \
N: \
-- ON _kveða_ "to say; recite" \
-- Far. _kvøða_ "to chant" \
-- Elfd. _kweðå_ "to sing" \
W: \
-- OE _cweþan_ \
-- OS _quethan_ \
-- OHG _quedan_, MHG _queden_, _koden_, _kōden_ \
REF: [EDPG]\: 319

### <a id="kweþu">\*kweþu- [m.] "stomach; uterus" (n)</a>
E: \
-- Go. _qiþus_ \
N: \
-- ON _kviðr_ \
-- Far. _kviður_ \
-- Elfd. _kwið_ \
W: \
-- OE _cwið_, _cwiða_ \
-- OHG _quiti_ "vagina" \
REF: [EDPG]\: 319

### <a id="kwīnan">\*kwīnan- [st.] "to wither, dwindle" (v)</a>
W: \
-- OE _ā-cwīnan_ "to waste, dwindle away" \
-- MDu. _quinen_ [wk.] "to wither", Du. _kwijnen_ [wk.] (also strong _kween_?) "to wither, languish" \
-- MHG _ver-quīnen_ "to dwindle" \
REF: [EDPG]\: 319

### <a id="kwisti">\*kwisti- [f.] "damage, destruction" (n)</a>
E: \
-- Go. _qists_ \
W: \
-- MDu. _quist_ "destruction, demise" \
-- OHG _quist_ "destruction" \
REF: [EDPG]\: 320

### <a id="kwiwa">\*kwiwa- ~ \*kwikwa- "alive" (adj)</a>
E: \
-- Go. _qius_ \
N: \
-- ON _kvikr_, _kykr_ \
-- Far. _kykur_ \
-- Elfd. _kwikk_ "lively, brisk; swift" \
W: \
-- OE _cwic(u)_, _cucu_, E _quick_ \
-- OFri. _quik_ \
-- OS _quik_ \
-- MDu. _quic_ "vivid", Du. _kwiek_ "id." \
-- OHG _queh_, _quec_, _chech_ "alive, vivid", G _keck_ "cheeky" \
REF: [EDPG]\: 320

### <a id="kwōba">\*kwōba- [n.] "thickening (of the air)" (n)</a>
N: \
-- Icel. _kóf_ "blizzard" \
-- Far. _kó(g)v_ "hazy sky" \
-- Nw. _kov_ "thickening sky; shortness of breath" \
REF: [EDPG]\: 321

### <a id="kwumþi">\*kwumþi- [f.] "arrival" (n)</a>
E: \
-- Go. _ga-qumþs_ "convention" \
W: \
-- OHG _qhumft_, _kumft_ "id.", G -_kunft_ "id." \
REF: [EDPG]\: 321

### <a id="kwurran">\*kwurran- [m.] "dog" (n)</a>
W: \
-- ME _curre_ "watchdog; small hunting dog; mongrel", E _cur_ "inferior dog" \
-- EDu. _korre_ "dog" \
REF: [EDPG]\: 321

## L

### <a id="labēn">\*labēn- [wk.] "to hang loosely, dangle" (v)</a>
N: \
-- ON _lafa_ \
-- Far. _lava_ \
-- Elfd. _lavi_, obl. _låvå_ \
REF: [EDPG]\: 321

### <a id="lagja">\*lagja- [m.] "leg" (n)</a>
N: \
-- ON _leggr_ "leg; hollow bone; stalk" \
-- Far. _leggur_ "leg, bone, shaft" \
-- Elfd. _legg_ "trunk; leg" \
REF: [EDPG]\: 321

### <a id="lagjan">\*lagjan- [wk.] "to put" (v)</a>
E: \
-- Go. _lagjan_ \
N: \
-- ON _leggja_ \
-- Far. _leggja_ \
-- Elfd. _legga_ \
W: \
-- OE _lecgan_, E _to lay_ \
-- OS _leggian_ \
-- Du. _leggen_ \
-- OHG _lecken_, _legen_, G _legen_ \
REF: [EDPG]\: 322

### <a id="lagu">\*lagu- [m.] "lake" (n)</a>
N: \
-- ON _lǫgr_ "sea; water, liquid" \
-- Far. _løgur_ "liquid; (hot) water; rainfall" \
-- Elfd. _lag_ "decoction" \
W: \
-- OE _lagu_ \
-- OS _lagu_ \
REF: [EDPG]\: 322

### <a id="lahan">\*lahan- [st.] "to blame, reproach" (v)</a>
N: \
-- Icel. _lá_ [wk.] \
W: \
-- OE _lēan_ \
-- OS _lahan_ "to blame, prohibit" \
-- OHG _lahan_ "id." \
REF: [EDPG]\: 322

### <a id="lahsa">\*lahsa- [m.] "salmon" (n)</a>
N: \
-- ON _lax_ \
-- Far. _laksur_ \
-- Elfd. _laks_ \
REF: [EDPG]\: 322

### <a id="lahtra">\*lahtra- [n./m.] "vice" (n)</a>
W: \
-- OE _leahtor_ [n.] "vice, fault, blemish, sin" \
-- MDu. _lachter_ [m.] "blemish, shame, abomination" \
REF: [EDPG]\: 322

### <a id="laibjan">\*laibjan- [wk.] "to leave over, leave behind" (v)</a>
E: \
-- Go. _bi-laibjan_ \
N: \
-- ON _leifa_ \
-- Far. _leiva_ \
-- Elfd. _lieva_ \
W: \
-- OE _lǣfan_, E _to leave_ \
-- OFri. _lēva_, _lēwa_, _liōwa_, _liouwa_ \
-- OS _far-lēƀian_ \
-- OHG _leiben_ \
REF: [EDPG]\: 323

### <a id="laibō">\*laibō- [f.] "remains, remnant" (n)</a>
E: \
-- Go. _laiba_ \
N: \
-- ON _leif_ "inheritance" \
-- Elfd. _lievur_ [f.pl.] "id." \
W: \
-- OE _lāf_ "id." \
-- OS _lēƀa_ "remainder; inheritance" \
-- OHG _leiba_ "remains", MHG _leibe_ "id." \
REF: [EDPG]\: 323

### <a id="laidjan">\*laidjan- [wk.] "to lead" (v)</a>
N: \
-- ON _leiða_ \
-- Far. _leiða_ \
-- Elfd. _lieða_ \
W: \
-- OE _lǣdan_, E _to lead_ \
-- OFri. _lēda_ \
-- OS _lēdian_ \
-- Du. _leiden_ \
-- OHG _leiten_, _leitten_, G _leiten_ \
REF: [EDPG]\: 323

### <a id="laihna">\*laihna- [n.] "loan, loaned property" (n)</a>
N: \
-- ON _lán_ \
-- Far. _lán_ \
W: \
-- OFri. _lēn_ \
-- OS _lēhan_ \
-- Du. _leen_ \
-- OHG _lēhan_, _lēehin_, G _Lehn_ \
REF: [EDPG]\: 323

### <a id="laikan">\*laikan- [st.] "to jump" (v)</a>
E: \
-- Go. _laikan_ "to frolic, jump for joy, hop" \
N: \
-- ON _leika_ [wk.] "to play; to move, swing" \
-- Far. _leika_ [wk.] "to play; to jump about" \
-- Elfd. _lieka_ [wk.] "to play" \
W: \
-- OE _lācan_ "to swing, flicker; to play", ME leiken "to play; fight; rush" \
-- MLG _lēken_ [wk.] "to jump" \
-- MHG _leichen_ [wk./st.] "to jump up; play" \
REF: [EDPG]\: 323

### <a id="laiman">\*laiman- [m.] "clay" (n)</a>
W: \
-- OE _lām_ [n.], E _loam_ \
-- OS _lēmo_ \
-- Du. _leem_ [n.] \
-- OHG _leimo_, G _Lehm_ \
REF: [EDPG]\: 323

### <a id="laisizan">\*laisizan- "less" (comp)</a>
W: \
-- OE _lǣssa_, E _less_ \
-- OFri. _lessa_, _lessera_ \
-- OS _les_ (adv) \
REF: [EDPG]\: 324

### <a id="laisō">\*laisō- [f.] "track, trail" (n)</a>
W: \
-- MDu. _lees_ \
-- OHG _leisa_, MHG _leise_ \
REF: [EDPG]\: 324

### <a id="laisti">\*laisti- [m.] "track, trace" (n)</a>
E: \
-- Go. _laists_ \
N: \
-- ON _leistr_ "stocking-foot, sock" \
W: \
-- OE _lǣst_ "footprint, bottom of the foot" \
-- MDu. _leest_ "model", Du. _leest_ [c.] "boot-tree; waist" \
-- OHG _leist_ "track", MHG _leist_ "id." \
REF: [EDPG]\: 324

### <a id="laiwizakōn">\*laiwizakōn- [m.] "lark" (n)</a>
W: \
-- OE _lāwrice_, _lāwerce_, _lǣwerce_ [f.], E _lark_ \
-- WFri. _ljurk_ [c.], NFri. _lāsk_ \
-- Du. _leeuwerik_ [c.] \
-- OHG _lērahha_, _lēricha_ [f.], MHG _lerche_, _lewercke_ [f.], G _Lerche_ [f.] \
REF: [EDPG]\: 324

### <a id="laiza">\*laiza- [n.] "clay" (n)</a>
N: \
-- ON _leir_ \
-- Far. _leir_ \
-- Elfd. _liera_ [f.] \
REF: [EDPG]\: 324

### <a id="laizjan">\*laizjan- [wk.] "to teach, instruct" (v)</a>
E: \
-- Go. _laisjan_ \
W: \
-- OE _lǣran_ \
-- OFri. _lēra_ \
-- OS _lērian_ \
-- Du. _leren_ \
-- OHG _lēren_, _lērran_, G _lehren_ \
REF: [EDPG]\: 325

### <a id="lajō">\*lajō- [f.] "rock(?)" (n)</a>
W: \
-- OFri. _laie_ "slate" \
-- OS _leia_ "rock" \
-- MDu. _leye_ "schist, slate", Du. _lei_ [c.] "id." \
-- MHG _leie_ "rock" \
REF: [EDPG]\: 325

### <a id="lakjan-1">\*lakjan- [wk.] "to grab" (v)</a>
W: \
-- OE _læccan_, _læccean_ "to take, grasp, seize, catch", E _to latch_ \
REF: [EDPG]\: 325

### <a id="lakjan-2">\*lakjan- [wk.] "to cause to leak, moisten" (v)</a>
N: \
-- Elfd. _lekka_ \
W: \
-- OE _leccan_ \
-- MDu. _lecken_, Du. _lekken_ \
-- OHG _lecken_, G _lecken_ \
REF: [EDPG]\: 325

### <a id="lakkōn">\*lak(k)ōn- [wk.] "to reproach, blame" (v)</a>
W: \
-- MDu. _lacken_, _laken_, Du. _laken_ \
REF: [EDPG]\: 325

### <a id="lambiz">\*lambiz- [n.] "lamb" (n)</a>
E: \
-- Go. _lamb_ "lamb, sheep" \
N: \
-- ON _lamb_ \
-- Far. _lamb_ \
W: \
-- OE _lamb_ \
-- OFri. _lam_ \
-- OS _lamb_ \
-- Du. _lam_ \
-- OHG _lamb_, G _Lamm_ \
REF: [EDPG]\: 325

### <a id="lamjan">\*lamjan- [wk.] "to beat (up), cripple" (v)</a>
N: \
-- ON _lemja_ "to flog, beat (down), suppress" \
-- Far. _lemja_ "to paralyze; to beat, hit" \
W: \
-- OE _lemian_ "to lame, cripple; to strike" \
-- OFri. _lemma_, _lamma_ "to cripple" \
-- OS _lemmian_ "id." \
-- MDu. _lemmen_ "to damage, mutilate" \
-- OHG _lemjan_ "to cripple, lame", G _lähmen_ "id." \
REF: [EDPG]\: 326

### <a id="lamsa">\*lamsa- [n.] "lock" (n)</a>
N: \
-- ON _láss_ [m.] \
-- Far. _lás_ \
-- Elfd. _lą̊s_ \
W: \
-- MLG _lōs_ "padlock" \
REF: [EDPG]\: 326

### <a id="landa">\*landa- [n.] "land" (n)</a>
E: \
-- Go. _land_ \
N: \
-- ON _land_ \
-- Far. _land_ \
-- Elfd. _land_ \
W: \
-- OE _land_, _lond_, E _land_ \
-- OFri. _lond_ \
-- OS _land_ \
-- Du. _land_ \
-- OHG _lant_, G _Land_ \
REF: [EDPG]\: 326

### <a id="landjan">\*landjan- [wk.] "to go slowly(?)" (v)</a>
W: \
-- MHG _lendern_ "to stroll, amble" \
REF: [EDPG]\: 326

### <a id="landjō">\*landjō- [f.] "lower part of the back, kidney, loin" (n)</a>
N: \
-- ON _lend_ \
-- Far. _lend_ \
W: \
-- OE _lendenu_ [f.pl.] \
-- ODu. _lendin_ [f.pl.], Du. _lenden_ [pl.] \
-- OHG _lentīn_ [f.pl.], G _Lende_ \
REF: [EDPG]\: 327

### <a id="langa">\*langa- "long" (adj)</a>
E: \
-- Go. _laggs_ \
N: \
-- ON _langr_ \
-- Far. _langur_ \
-- Elfd. _laungg_ \
W: \
-- OE _lang_, E _long_ \
-- OFri. _long_ \
-- OS _lang_ \
-- Du. _lang_ \
-- OHG _lang_, G _lang_ \
REF: [EDPG]\: 327

### <a id="lapan">\*lapan- [st.] "to lick up, lap" (v)</a>
W: \
-- OHG _laffan_, MHG _laffen_, G Swi. _laffen_ [st./wk.] \
REF: [EDPG]\: 327

### <a id="lappōn">\*lapp/bōn- [wk.] "to lick up, lap" (v)</a>
N: \
-- OSw. _lapa_ \
-- Elfd. _låpå_ \
W: \
-- OE _lapian_, E _to lap_ \
-- EDu. _labben_, _lappen_ \
-- G Swi. _labben_ \
REF: [EDPG]\: 327

### <a id="lasiwa">\*lasiwa- "weak, feeble" (adj)</a>
E: \
-- Go. _lasiws_ \
W: \
-- OE _lysu_, _lesu_ "depraved, corrupt, evil" \
REF: [EDPG]\: 327

### <a id="lata">\*lata- "lax, sluggish" (adj)</a>
E: \
-- Go. _lats_ \
N: \
-- ON _latr_ "slow, lazy" \
-- Far. _latur_ "lazy, idle" \
-- Elfd. _lat_ "id." \
W: \
-- OE _læt_ "slow, late", E _late_ \
-- OFri. _let_ "late" \
-- OS _lat_ "lazy" \
-- Du. _laat_ "late" \
-- OHG _laz_ "lazy" \
REF: [EDPG]\: 328

### <a id="laþōjan">\*laþōjan- [wk.] "to invite" (v)</a>
E: \
-- Go. _laþon_ \
W: \
-- OE _laðian_ \
-- OFri. _lathia_ \
-- OS _lathian_ \
-- OHG _ladōn_, G _laden_ \
REF: [EDPG]\: 328

### <a id="lauba">\*lauba- [n.] "leaf, foliage" (n)</a>
E: \
-- Go. _lauf_ \
N: \
-- ON _lauf_ \
-- Far. _leyv_ \
-- Elfd. _lov_ \
W: \
-- OE _leaf_, E _leaf_ \
-- OFri. _lāf_ \
-- OS _lōf_ \
-- Du. _loof_ \
-- OHG _loub_, G _Laub_ \
REF: [EDPG]\: 328

### <a id="lauda-1">\*lauda- [n.] "lead" (n)</a>
W: \
-- OE _lēad_, E _lead_ \
-- OFri. _lād_ \
-- Du. _lood_ \
REF: [EDPG]\: 328

### <a id="lauda-2">\*lauda- [m.] "man" (n)</a>
E: \
-- Go. _jugga-lauþs_ "young man" \
REF: [EDPG]\: 328

### <a id="laugnjan">\*laugnjan- [wk.] "to keep hidden, deny" (v)</a>
E: \
-- Go. _laugnjan_ "to reject, deny, cover up" \
N: \
-- ON _leyna_ "to hide, conceal" \
-- Far. _loyna_ "id." \
W: \
-- OE _līegnian_ "to deny" \
-- OFri. _leina_, _laina_ "id." \
-- OS _lōgnian_ "id." \
-- Du. _loochenen_ "id." \
-- OHG _loug(a)nen_ "id.", G _läugnen_ "id." \
REF: [EDPG]\: 328

### <a id="lauka">\*lauka- [m./n.] "onion, leek" (n)</a>
N: \
-- ON _laukr_ [m.] "leek, onion, garlic" \
-- Far. _leykur_ [m.] "onion" \
-- Elfd. _lok_ [m.] "id." \
W: \
-- OE _lēac_ [m.] "leek, onion, garlic", E _leek_ \
-- MDu. _looc_ [n.] "garlic", Du. Flem. _look_ "id." \
-- OHG _louh_ [n.] "leek", G _Lauch_ [m.] "id." \
REF: [EDPG]\: 329

### <a id="launa">\*launa- [n.] "reward, recompense" (n)</a>
E: \
-- Go. _laun_ \
N: \
-- ON _laun_ [n.pl.] \
-- OSw. _lø̄n_ [n.pl./f.] \
-- Da. _løn_ [c.] \
W: \
-- OE _lēan_ \
-- OFri. _lān_ \
-- OS _lōn_ \
-- Du. _loon_ \
-- OHG _lōn_ [m./n.], G _Lohn_ [m.] \
REF: [EDPG]\: 329

### <a id="lausa">\*lausa- "empty, idle" (adj)</a>
E: \
-- Go. _laus_ "empty, devoid" \
N: \
-- ON _lauss_ "loose, free" \
-- Far. _leysur_ "loose, free; idle; uncertain, vague" \
-- Elfd. _los_ "free, loose" \
W: \
-- OE _lēas_ "loose, free; void of; vain, false, deceitful" \
-- OFri. _lās_ "free; lost; deceitful" \
-- OS _lōs_ "loose, free" \
-- Du. _loos_ "idle, stray" \
-- OHG _los_ "lax, light-hearted, loose", G _los_ "loose" \
REF: [EDPG]\: 329

### <a id="lauþra">\*lauþra- [n.] "lather" (n)</a>
N: \
-- ON _lauðr_ "lather; froth, foam of the sea" \
W: \
-- OE _lēaðor_ "lather", E _lather_ \
-- OS _lōthar_ "detergent" \
REF: [EDPG]\: 329

### <a id="lawiz">\*lawiz- [n.] "ham, thigh" (n)</a>
N: \
-- ON _lær_ \
-- Far. _lær_ \
-- OSw. _lār_ \
-- Elfd. _lår_ \
W: \
-- OE _lēow_, pl. _lēower(a)_ \
REF: [EDPG]\: 329

### <a id="lawwa">\*law(w)a- [n.] "oak bark, tan" (n)</a>
W: \
-- MDu. _loo_, Du. _looi_ \
-- OHG _lō_, G _Lohe_ [f.] \
REF: [EDPG]\: 330

### <a id="lawwō">\*lawwō- [f.] "groove" (n)</a>
N: \
-- ON _lǫgg_ "groove in the staves of a cask" \
-- Far. _løgg_ "id." \
W: \
-- OE _lim-lǣw_ "injury to the limb, mutilation" \
-- OHG _līh-lōa_ "scar" \
REF: [EDPG]\: 330

### <a id="legjan">\*leg(j)an- [st.] "to lie" (v)</a>
E: \
-- Go. _ligan_ \
N: \
-- ON _liggja_ \
-- Far. _liggja_ \
-- Elfd. _ligga_ \
W: \
-- OE _licgan_, E _to lie_ \
-- OFri. _lidza_ \
-- OS _liggian_ \
-- Du. _liggen_ \
-- OHG _lickan_, _ligan_, G _liegen_ \
REF: [EDPG]\: 330

### <a id="lēgu">\*lēgu- "low" (adj)</a>
N: \
-- ON _lágr_ \
-- Far. _lágur_ \
-- Elfd. _låg_ \
W: \
-- OFri. _lēch_, _leich_, WFri. _leech_ \
-- MDu. _lage_, Du. _laag_ \
-- MHG _læge_ \
REF: [EDPG]\: 330

### <a id="lehizan">\*lehizan- [m.] "fleshy part of the body, calf" (n)</a>
W: \
-- OE _līra_ "muscle, soft parts of the body", E _lire_ "flesh, muscle, brawn" \
-- MDu. _liere_ [f.] "fleshy part of the leg, calf" \
REF: [EDPG]\: 330

### <a id="lekan">\*lekan- [st.] "to leak, drip" (v)</a>
N: \
-- ON _leka_ \
-- Far. _leka_ \
W: \
-- E _to leak_ \
-- MDu. _leken_ [st./wk.] \
-- OHG _lehhan_, MHG _lechen_ [st./wk.] \
REF: [EDPG]\: 331

### <a id="lēkinōn">\*lēkinōn- [wk.] "to heal" (v)</a>
E: \
-- Go. _lekinon_ \
N: \
-- ON _lækna_ \
W: \
-- OE _lǣcnian_ \
-- ODu. _lāknon_ \
-- OHG _lāchinōn_, MHG _lāchenen_ \
REF: [EDPG]\: 331

### <a id="lēkja">\*lēkja- [m.] "doctor; leech" (n)</a>
E: \
-- Go. _lekeis_ "doctor" \
N: \
-- ON _lækir_ "id." \
-- Da. _læge_ [c.] "id." \
W: \
-- OE _lǣce_ "doctor; leech", E _leech_ \
-- OFri. _lētza_ "doctor" \
-- ODu. _lake_ "leech", MDu. _laecke_ "id." \
-- OHG _lāhhi_, _lāchi_ "id." \
REF: [EDPG]\: 331

### <a id="lēkjōn">\*lēkjōn- [f.] "rivulet(?)" (n)</a>
N: \
-- Far. _lækja_ "well, waterhole, waterspout" \
-- Nw. _lækje_ "rivulet, wooden water-pipe" \
REF: [EDPG]\: 331

### <a id="lepjan">\*lepjan- [m.] "lip" (n)</a>
W: \
-- OE _lippa_, E _lip_ \
-- OFri. _lippa_ \
-- MDu. _lippe_ [f.], Du. _lip_ [c.] \
REF: [EDPG]\: 331

### <a id="lesan">\*lesan- [st.] "to pick, select" (v)</a>
E: \
-- Go. _lisan_ "to collect" \
N: \
-- ON _lesa_ "to grasp, pick up, to gather; to knit" \
W: \
-- OE _lesan_ "to gather, collect", E _to lease_ ("to glean") \
-- OS _lesan_ "to gather" \
-- Du. _lezen_ "to read; to select" \
-- OHG _lesan_ "to read, examine; to collect", G _lesen_ "id." \
REF: [EDPG]\: 331

### <a id="lētan">\*lētan- [st.] "to let, allow" (v)</a>
E: \
-- Go. _letan_ \
N: \
-- ON _láta_ \
-- Far. _láta_ \
-- Elfd. _låta_ [wk.] \
W: \
-- OE _lǣtan_, E _to let_ \
-- OFri. _lēta_ \
-- OS _lātan_ \
-- Du. _laten_ \
-- OHG _lazan_, G _lassen_ \
REF: [EDPG]\: 332

### <a id="leþra">\*leþra- [n.] "leather" (n)</a>
N: \
-- ON _leðr_ \
-- Far. _leður_ \
-- Elfd. _leðer_ \
W: \
-- OE _leðer_, E _leather_ \
-- OFri. _lether_ \
-- MDu. _leder_, Du. _leer_ \
-- OHG _ledar_, G _Leder_ \
REF: [EDPG]\: 332

### <a id="leuba">\*leuba- "dear, beloved" (adj)</a>
E: \
-- Go. _liufs_ \
N: \
-- ON _ljúfr_ \
W: \
-- OE _lēof_ \
-- OS _liof_ \
-- Du. _lief_ \
-- OHG _liub_, _liob_, G _lieb_ \
REF: [EDPG]\: 332

### <a id="leudan">\*leudan- [st.] "to grow" (v)</a>
E: \
-- Go. _liudan_ \
W: \
-- OE _lēodan_ \
-- OS _liodan_ \
-- OHG _liotan_ \
REF: [EDPG]\: 332

### <a id="leudi">\*leudi- [m.] "man; people" (n)</a>
N: \
-- ON _lýðr_ "people, men" \
-- Far. _undir-lýður_ "weak-willed person" \
W: \
-- OFri. _liūd_ [n.] "people" \
-- OS _liud_ "people" \
-- OHG _liut_ [m./n.] "people" \
REF: [EDPG]\: 332

### <a id="leugan">\*leugan- [st.] "to lie" (v)</a>
E: \
-- Go. _liugan_ \
N: \
-- ON _ljúga_ \
-- Far. _lúgva_ \
-- Elfd. _liuoga_ \
W: \
-- OE _lēogan_, E _to lie_ \
-- OFri. _liāga_ \
-- OS _liogan_ \
-- Du. _liegen_ \
-- OHG _liugan_, _liogan_, G _lügen_ \
REF: [EDPG]\: 333

### <a id="leugō">\*leugō- [f.] "marriage" (n)</a>
E: \
-- Go. _liuga_ \
REF: [EDPG]\: 333

### <a id="leuhada">\*leuhada- [n.] "light" (n)</a>
E: \
-- Go. _liuhaþ_ [f.] \
W: \
-- OE _lēoht_, E _light_ \
-- OFri. _liācht_, WFri. _ljocht_ \
-- OS _lioht_ \
-- Du. _licht_ \
-- OHG _lioht_, G _Licht_ \
REF: [EDPG]\: 333

### <a id="leuhman">\*leuhman- [m.] "beam of light" (n)</a>
N: \
-- ON _ljómi_ "flash of light, radiance" \
-- Far. _ljómi_ "light, glare, brilliance" \
W: \
-- OE _lēoma_ "(beam of) light, radiance" \
-- OS _liomo_ "shine" \
REF: [EDPG]\: 333

### <a id="leuhna">\*leuhna- [n.] "lightning" (n)</a>
N: \
-- Nw. _ljon_, _lyn_ \
-- Sw. _ljung-eld_ \
-- Gutn. _liaugn_ \
-- Da. _lyn_ \
REF: [EDPG]\: 333

### <a id="leuhsa">\*leuhsa- "light, bright" (adj)</a>
N: \
-- ON _ljóss_ \
-- Far. _ljósur_ \
-- Elfd. _liuos_ \
REF: [EDPG]\: 334

### <a id="leukan">\*leukan- ~ \*lūkan- [st.] "to close; to pull" (v)</a>
E: \
-- Go. _ga-lukan_ "to close", _us-lukan_ "to pull (a sword)" \
N: \
-- ON _lúka_, _ljúka_ "to close, finish" \
-- Far. _lúka_ "to close; to weed, clean out" \
W: \
-- OE _lūcan_ "to plait" \
-- OFri. _lūka_ "to close; to join; to pull" \
-- MDu. _luken_ "to weed; to pull; to draw a sword or knife; to stab(?)", Du. luiken "to close" \
-- OHG _liohhan_ "to pull, pluck", _lūhhan_ "to close", MHG _liechen_, _lūchen_ "to close; to pull; to shirk, crouch; to slip" \
REF: [EDPG]\: 334

### <a id="leusan">\*leusan- [st.] "to lose" (v)</a>
E: \
-- Go. _fra-liusan_ \
W: \
-- OE _for-lēosan_ \
-- OFri. _for-liāsa_, _for-liēsa_ \
-- OS _far-liosan_ \
-- Du. _ver-liezen_ \
-- G _ver-lieren_ \
REF: [EDPG]\: 334

### <a id="leuskan">\*leuskan- [m.] "groin" (n)</a>
N: \
-- Icel. _ljóski_ \
-- OSw. _liuske_ \
-- Sw. _ljumske_ [c.] \
-- Elfd. _liuoska_ [f.] \
W: \
-- E _leske_ "flank" \
-- NFri. Hall. _jaoskə_ pl. "id." \
-- MLG _lēsche_, _leische_ "genitalia" \
-- MDu. _liesche_ [f.] "groin; membrane", Du. _lies_ [c.] "groin" \
REF: [EDPG]\: 334

### <a id="leuta">\*leuta- "ugly, bad" (adj)</a>
E: \
-- Go. _liuts_ "deceitful, fraudulent" \
N: \
-- ON _ljótr_ "ugly" \
-- Far. _ljótur_ "ugly; bad" \
-- Elfd. _liuot_ "bad, mean" \
REF: [EDPG]\: 335

### <a id="lewan">\*lewan- [m.] "scythe" (n)</a>
N: \
-- ON _lé_, obl. _ljá_ \
-- Far. _líggi_ \
-- Nw. _ljå_ \
-- Sw. _lie_ [c.] \
-- Elfd. _ljå_ \
REF: [EDPG]\: 335

### <a id="lēwjan">\*lēwjan- [wk.] "to deliver, betray" (v)</a>
E: \
-- Go. _lewjan_ \
W: \
-- OE _lǣwan_, ME _be-lēwen_ \
-- OHG _gi-lāen_, _fir-lāen_ \
REF: [EDPG]\: 335

### <a id="lība">\*lība- [n.] "life; body" (n)</a>
N: \
-- ON _líf_ \
-- Far. _lív_ \
-- Elfd. _laiv_ "life" \
W: \
-- OE _līf_ "id.", E _life_ \
-- OFri. _līf_ "body; womb" \
-- OS _līf_ "life" \
-- Du. _lijf_ "body" \
-- OHG _līb_ [m./n.], G _Leib_ [m.] "body" \
REF: [EDPG]\: 335

### <a id="līban">\*līban- [wk.] "to stay" (v)</a>
E: \
-- Go. _bi-leiban_ [st.] \
W: \
-- OE _be-līfan_ [st.] "to stay, remain" \
-- OFri. _bi-līva_ [st.] "id." \
-- OS _bi-līƀan_ [st.] "id." \
-- Du. _blijven_ [st.] "id." \
-- OHG _bi-līban_ [st.] "to stay, stop", G _bleiben_ [st.] "id." \
REF: [EDPG]\: 335

### <a id="libēn">\*libēn- [wk.] "to be left; to live" (v)</a>
E: \
-- Go. _liban_ "to live" \
N: \
-- ON _lifa_ \
-- Far. _liva_ "to live" \
-- Elfd. _livå_ "id." \
W: \
-- OE _libban_ "id.", E _to live_ \
-- OFri. _libba_ "id." \
-- OS _libbian_ "id." \
-- Du. _leven_ "id." \
-- OHG _lebēn_ "id.", G _leben_ "id." \
REF: [EDPG]\: 336

### <a id="librō">\*librō- [f.] "liver" (n)</a>
N: \
-- ON _lifr_ \
-- Far. _livur_ \
-- OSw. _liver_, _lyr_ \
-- Elfd. _lyr_ \
W: \
-- OE _lifer_, E _liver_ \
-- OFri. _livere_ \
-- Du. _lever_ [c.] \
-- OHG _libara_, _lebara_, _lebera_, G _Leber_ \
REF: [EDPG]\: 336

### <a id="līhwan">\*līhwan- [st.] "to loan" (v)</a>
E: \
-- Go. _leiƕan_ \
N: \
-- ON _ljá_ [wk.] "to lend; to give, grant" \
-- Far. _líggja_ "to borrow, take out" \
W: \
-- OE _lēon_ "to lend" \
-- OS _far-līhan_ "id." \
-- MDu. _lien_ "id." \
-- OHG _līhan_ "id.", G _leihen_ "id." \
REF: [EDPG]\: 336

### <a id="līka-1">\*līka- [n.] "(dead) body" (n)</a>
E: \
-- Go. _leik_ \
N: \
-- ON _lík_ \
-- Far. _lík_ \
-- Elfd. _laik_ \
W: \
-- OE _līc_ \
-- OFri. _līk_ \
-- OS _līk_ \
-- Du. _lijk_ \
-- OHG _līh_ [n./f.], G _Leiche_ [f.] \
REF: [EDPG]\: 336

### <a id="lika-2">\*lika- "similar, equal" (adj)</a>
E: \
-- Go. _ga-leiks_ \
N: \
-- ON _glíkr_, _líkr_ \
-- Far. _líkur_ \
-- Elfd. _laik_ \
W: \
-- OE _ge-līc_, E _like_ \
-- OFri. _ge-līk_ \
-- OS _gi-līk_ \
-- Du. _ge-lijk_ \
-- OHG _ga-līh_, G _gleich_ \
REF: [EDPG]\: 336

### <a id="līka-3">\*līka- [n.] "leech-line, bolt-rope" (n)</a>
N: \
-- ON _lík_ \
W: \
-- E _leech_ \
-- MLG _līk_ \
-- Du. _lijk_ \
-- MHG _ge-leich_ "joint, link" \
REF: [EDPG]\: 337

### <a id="likkōn-1">\*likkōn- [wk.] "to lick" (v)</a>
W: \
-- OE _liccian_, E _to lick_ \
-- OS _likkon_ \
-- Du. _likken_ \
-- OHG _leckōn_, G _lecken_ \
REF: [EDPG]\: 337

### <a id="likkōn-2">\*likkōn- [wk.] "to jump" (v)</a>
W: \
-- MHG _lecken_ "to kick, jump" \
REF: [EDPG]\: 337

### <a id="likkra">\*likkra- [wk.] "delicious" (v)</a>
W: \
-- MDu. _lecker_, Du. _lekker_ \
-- MHG _lecker_, G _lecker_ \
REF: [EDPG]\: 338

### <a id="līma">\*līma- [m.]j [n.] "clay, mud, lime" (n)</a>
N: \
-- ON _lím_ [n.] "lime" \
-- Far. _lím_ [n.] "glue" \
W: \
-- OE _līm_ [m.] "lime", E _lime_ \
-- OS _līm_ [m.] "glue" \
-- Du. _lijm_ [c.] "id." \
-- OHG _līm_ [m.] "clay; lime", G _Leim_ [m.] "glue; paste" \
REF: [EDPG]\: 338

### <a id="limu">\*limu- [m.] "limb" (n)</a>
N: \
-- ON _limr_ "limb; joint" \
-- Far. _limur_ "id." \
W: \
-- E _limb_ \
REF: [EDPG]\: 338

### <a id="lindō">\*lindō- [f.] "lime tree" (n)</a>
N: \
-- ON _lind_ \
-- Far. _lind_ \
-- Elfd. _lind_ \
W: \
-- OE _lind_, E _lime_ \
-- OS _linda_ \
-- Du. _linde_ [c.] \
-- OHG _linta_, G _Linde_ \
REF: [EDPG]\: 338

### <a id="lingʷa">\*lingʷa- [n.] "heather" (n)</a>
N: \
-- ON _lyng_ \
-- Far. _lyngur_ [m.] \
-- OSw. _liung_ \
-- Sw. _ljung_ \
-- Elfd. _lingg_ \
REF: [EDPG]\: 338

### <a id="lingʷan">\*lingʷan- [st.] "to succeed, make progress" (v)</a>
W: \
-- OHG _gi-lingan_, MHG _lingen_, _ge-lingen_, G _ge-lingen_ \
REF: [EDPG]\: 338

### <a id="linhta">\*linhta- "light" (adj)</a>
E: \
-- Go. _leihts_ \
N: \
-- ON _léttr_ "light, easy; nimble, active" \
-- Far. _lættur_ "light, easy" \
-- Elfd. _litt_ "id." \
W: \
-- OE _līht_, _lēoht_ "id.", E _light_ \
-- OFri. _licht_ "id." \
-- Du. _licht_ "id." \
-- OHG _līht_ "easy", G _leicht_ "id." \
REF: [EDPG]\: 339

### <a id="linþa">\*linþa- "flexible, soft" (adj)</a>
N: \
-- ON _linnr_ "mild, soft" \
-- Far. _linur_ "soft, slack, limp" \
-- Elfd. _lin_ "soft" \
W: \
-- OE _līð_, _līðe_ "id.", E _lithe_ \
-- OS _līthi_ "id." \
-- OHG _lind_, _lindi_ "mild, tender", G _ge-linde_ "delicate, soft" \
REF: [EDPG]\: 339

### <a id="lisan">\*lisan- [pp.] "to know" (v)</a>
E: \
-- Go. _lisan\*_, 3sg. _lais_ \
REF: [EDPG]\: 339

### <a id="listi">\*listi- [f.] "skill" (n)</a>
E: \
-- Go. _list_ "cunning" \
N: \
-- ON _list_ "art, craft" \
-- Far. _list_ "art; cunning" \
W: \
-- OE _list_ "art, skill; trick" \
-- OFri. _list_ "knowledge; deceit" \
-- OS _list_ "craftiness, cleverness" \
-- Du. _list_ [c.] "trick" \
-- OHG _list_ [m./f.] "knowledge", G _List_ "cunning" \
REF: [EDPG]\: 339

### <a id="lītila">\*lītila- "small" (adj)</a>
E: \
-- Go. _leitils_ \
N: \
-- ON _lítill_ \
-- Far. _lítil_ \
-- Elfd. _litn_, _lisl_- \
W: \
-- OE _lȳtel_, _lītel_, Nrth. _lyttil_, E _little_ \
-- OS _luttil_ \
-- Du. _luttel_ "few" \
-- OHG _lūzil_, _luzzil_ "id.", MHG _lūtzel_ "id." \
REF: [EDPG]\: 339

### <a id="līþan">\*līþan- [st.] "to go (by), pass" (v)</a>
E: \
-- Go. _ga-leiþan_ "to go, travel" \
N: \
-- ON _líða_ "to go, pass by; to pass away, to elapse" \
-- Far. _líða_ "to move smoothly, stride; to pass (away), elapse, proceed" \
-- Elfd. _laiða_ "to go by, pass" \
W: \
-- OE _līðan_ "to go, sail" \
-- OFri. _lītha_ "to go" \
-- OS _līthan_ "to go, leave" \
-- Du. _lijden_ "to suffer" \
-- OHG _līdan_ "to go, pass; to tolerate, suffer", G _leiden_ "to suffer" \
REF: [EDPG]\: 340

### <a id="liþu">\*liþu- [m.] "member, body part" (n)</a>
E: \
-- Go. _liþus_ \
N: \
-- ON _liðr_ "joint" \
-- Far. _liður_ "joint; knot" \
-- Elfd. _lið_ "id." \
W: \
-- OE _lið_ [m./n.] "joint" \
-- OFri. _lith_ [n.] "joint, articulation" \
-- OS _lith_ "id." \
-- Du. _lid_ [n.] "id." \
-- OHG _lid_ [m./n.] "id.", G _Glied_ [n.] "id." \
REF: [EDPG]\: 340

### <a id="līþu">\*līþu- [n.] "strong drink, cider" (n)</a>
E: \
-- Go. _leiþu_ \
N: \
-- ON _líð_ \
W: \
-- OE _līð_ \
-- OFri. _līth_ \
-- OS _līth_ \
-- OHG _līd_ [m./n.] \
REF: [EDPG]\: 340

### <a id="liznōn">\*liznōn- [wk.] "to learn" (v)</a>
W: \
-- OE _leornian_, E _to learn_ \
-- OFri. _lirna_, _lerna_ \
-- OS _līnon_ \
-- OHG _lernōn_, _lernēn_, _lirnēn_, G _lernen_ \
REF: [EDPG]\: 340

### <a id="lōfan">\*lōfan- ~ \*lappan- [m.] "palm of the hand" (n)</a>
E: \
-- Go. _lofa_ \
N: \
-- ON _lófi_ \
-- Icel. _lófi_ \
-- Far. _lógvi_ "palm of the hand; small glove", _labbi_ "paw" \
-- Nw. _labb_ "paw, big hand" \
W: \
-- ME _lōve_ \
-- OHG _lappo_, _laffa_ [f.] "palm of the hand; blade of an oar", MHG _laffe_ [f.] "id." \
REF: [EDPG]\: 340

### <a id="lōki">\*lōki- [m.] "creek" (n)</a>
N: \
-- ON _lœkr_ "brook, rivulet" \
-- Far. _løkur_ "brooklet" \
REF: [EDPG]\: 341

### <a id="lōma">\*lōma- [m.] "betrayal" (n)</a>
N: \
-- Icel. _lōmur_ \
REF: [EDPG]\: 341

### <a id="lōmi">\*lōmi- "crippled(?)" (adj)</a>
W: \
-- MDu. _loem_ "dull", EDu. _loom_ "sluggish" \
-- MHG _lüeme_ "dull, pliant, mild" \
REF: [EDPG]\: 341

### <a id="luba">\*luba- [n.] "praise" (n)</a>
N: \
-- ON _lof_ "praise; permission" \
-- Far. _lov_ "id." \
-- Elfd. _luv_ "id." \
W: \
-- OE _lof_ \
-- OFri. _lof_ \
-- Du. _lof_ \
-- OHG _lob_, G _Lob_ \
REF: [EDPG]\: 341

### <a id="lubja">\*lubja- [n.] "herb, potion" (n)</a>
E: \
-- Go. _lubja-leisei_ [f.] "witchcraft" \
N: \
-- ON _lyf_ "medicine, healing herb" \
-- Elfd. _lyv_ "remedy" \
W: \
-- OE _lyb_ "medicine, drug, potion" \
-- MDu. _lubbe_ "rennet" \
-- OHG _luppi_ "poison, witchcraft", G Alem. _lupp_ "rennet" \
REF: [EDPG]\: 341

### <a id="ludēn">\*ludēn- [wk.] "to dangle" (v)</a>
N: \
-- Icel. _loða_ "to hang down, stick to", _lodda_ "to dangle" \
-- Far. _loða_ "to hang down" \
-- Nw. _loda_ "to hang down, stick to" \
REF: [EDPG]\: 342

### <a id="ludja">\*ludja/ō- [n./f.] "face" (n)</a>
E: \
-- Go. _ludja_ [f.] \
W: \
-- OHG _ant-lutti_ [n.], G _Antlitz_ [n.] \
REF: [EDPG]\: 342

### <a id="luftu">\*luftu- [m.] "roof, air" (n)</a>
E: \
-- Go. _luftus_ "air" \
N: \
-- ON _loft_ [n.] "air, sky; loft" \
-- Far. _loft_ [n.] "air; ceiling, loft" \
-- Elfd. _lopt_ [n.] "id." (< \*lufta-) \
W: \
-- OE _lyft_ [m./f./n.] "air, sky" \
-- OFri. _lufte_, _luft_ [f.] "id." \
-- OS _luft_ [f.] "air" \
-- Du. _lucht_ [c.] "air, sky" \
-- OHG _luft_ [m./f.] "id.", G _Luft_ [f.] "id." \
REF: [EDPG]\: 342

### <a id="lugi">\*lugi- "deceitful, false" (adj)</a>
W: \
-- OE _lygge_ \
-- OS _luggi_ "deceitful" \
-- OHG _lucki_ "id.", MHG _lüge_, _lücke_ "id." \
REF: [EDPG]\: 342

### <a id="luginō">\*luginō- [f.] "lie, falsehood" (n)</a>
N: \
-- Far. _lygn_ \
-- Elfd. _lyngen_ \
W: \
-- OE _lygen_ \
-- OFri. _leine_ \
-- OS _lugina_ \
-- Du. _leugen_ [c.] \
-- OHG _lugina_, G _Lüge_ \
REF: [EDPG]\: 342

### <a id="luhō">\*luhō- [f.] "lynx" (n)</a>
N: \
-- OSw. _lō_ \
-- Elfd. _luo_ \
REF: [EDPG]\: 342

### <a id="luhsa">\*luhsa- [m.] "lynx" (n)</a>
W: \
-- OE _lox_ \
-- Du. _los_ [c.] \
-- OHG _luhs_, G _Luchs_ \
REF: [EDPG]\: 342

### <a id="luka">\*luka- [n.] "weed(s)" (n)</a>
N: \
-- ON _lok_ \
-- Far. _lok_ \
-- Sw. dial. _luk_, EDa. _lug_ \
REF: [EDPG]\: 343

### <a id="lukka">\*lukka- [m.] "tuft" (n)</a>
N: \
-- ON _lokkr_ "lock of hair" \
-- Far. _lokkur_ "id." \
W: \
-- OE _loc_ "id.", E _lock_ \
-- Du. _lok_ [c.] "id." \
-- OHG _loc_ "id.", G _Locke_ [f.] "id." \
REF: [EDPG]\: 343

### <a id="lukkōn">\*luk(k)ōn- [wk.] "to caress, lure, entice" (v)</a>
N: \
-- ON _lokka_ "to allure, entice" \
-- Elfd. _lukka_ "id." \
W: \
-- OE _a-loccian_ "to entice", _ge-loccian_ "to caress" \
-- MDu. _locken_ "to entice", Du. _lokken_ "id." \
-- OHG _lockōn_, _lohhōn_ "id.", G _locken_ "id." \
REF: [EDPG]\: 343

### <a id="lumēn">\*lumēn- [wk.] "to beat" (v)</a>
N: \
-- Icel. _luma_ "to beat, thrash" \
REF: [EDPG]\: 343

### <a id="lunda">\*lunda/ō- [m./f.] "grove" (n)</a>
N: \
-- ON _lundr_ [m.] \
-- Icel. _lundur_ [m.] \
-- Far. _lund_ [f.] \
-- Sw. _lund_ [c.] \
REF: [EDPG]\: 344

### <a id="lundjō">\*lund(j)ō- [f.] "fat" (n)</a>
N: \
-- ON _lund_ "mind, temper" \
-- Far. _lund_ "disposition", pl. "internal back muscles, tenderloin" \
W: \
-- OE _lynd_ "fat" \
-- OHG _lunda_ "tallow" \
REF: [EDPG]\: 344

### <a id="lungōn">\*lungōn- [f.] "lung" (n)</a>
N: \
-- ON _lungu_ [n.pl.] \
-- Far. _lunga_ [n.] \
-- Elfd. _lungga_ \
W: \
-- E _lung_ \
-- OS _lunga_ \
-- Du. _long_ [c.] \
-- OHG _lungun_ [f.pl.], G _Lunge_ \
REF: [EDPG]\: 344

### <a id="lungra">\*lungra- "quick, light" (adj)</a>
W: \
-- OE _lungre (adv)_ \
-- OHG _lungar_ "easy, quick", MHG lunger "quick, happy" \
REF: [EDPG]\: 344

### <a id="luniz">\*luniz- [n.] "linchpin" (n)</a>
W: \
-- OE _lynis_ [m.], E _linch_ \
-- OS _lunis_ [m.] \
-- MDu. _lunse_, _lense_ [f.], Du. _funs_ [c.] \
-- OHG _lun_ [f.], G _Lünse_ [f.] \
REF: [EDPG]\: 344

### <a id="luppōn">\*luppōn- [f.] "flea" (n)</a>
N: \
-- Far. _loppa_ \
-- OSw. _loppa_ \
-- Elfd. _luppa_ \
W: \
-- OE _loppe_ \
REF: [EDPG]\: 345

### <a id="lusti">\*lusti- [f.] "loss" (n)</a>
E: \
-- Go. _fra-lusts_ \
W: \
-- OS _far-lust_ \
-- OHG _for-lust_, G _Ver-lust_ [m.] \
REF: [EDPG]\: 345

### <a id="lustu">\*lustu- [m.] "desire, lust" (n)</a>
E: \
-- Go. _lustus_ \
W: \
-- OE _lust_, E _lust_ \
-- OFri. _lust_ [m./f.] \
-- OS _lust_ [f.] \
-- Du. _lust_ [c.] \
-- OHG _lust_ [m./f.], G _Lust_ [f.] \
REF: [EDPG]\: 345

### <a id="lūtan">\*lūtan- [st.] "to stoop, bend forward" (v)</a>
N: \
-- ON _lúta_ \
-- Far. _lúta_ [wk.] \
-- Elfd. _lauta_ \
W: \
-- OE _lūtan_, E _to lout_ \
REF: [EDPG]\: 345

### <a id="lutta">\*lutta- [n.] "puddle(?)" (n)</a>
W: \
-- G Rhnl. _Lotz_ "deep spot in a creek (to scoop water)" \
REF: [EDPG]\: 345

### <a id="luttōn">\*luttōn- [wk.] "to stoop, droop(?)" (v)</a>
N: \
-- Nw. _lota_ "to be askew, slanted" \
W: \
-- G Rhnl. _lotzen_ "to feel drowsy, sleep" \
REF: [EDPG]\: 345

## M

### <a id="madrōn">\*madrōn- [f.] "madder" (n)</a>
N: \
-- ON _maðra_ \
-- Nw. _modre_ \
W: \
-- OE _mædere_, _mæddre_, E _madder_ \
-- MHG _matare_, _metere_ \
REF: [EDPG]\: 346

### <a id="magan">\*magan- [m.] "stomach" (n)</a>
N: \
-- ON _magi_ \
-- Far. _magi_ \
-- Elfd. _magi_ \
W: \
-- OE _maga_, E _maw_ \
-- OFri. _maga_ \
-- Du. _maag_ [c.] \
-- OHG _mago_, G _Mage_ \
REF: [EDPG]\: 346

### <a id="magaþi">\*magaþi- [f.] "girl, maiden" (n)</a>
E: \
-- Go. _magaþs\*_ \
W: \
-- OE _mæg(e)ð_, E _maid_ \
-- OFri. _megith_ \
-- OS _magath_ \
-- Du. _maagd_ \
-- OHG _magad_, G _Magd_ \
REF: [EDPG]\: 346

### <a id="magra">\*magra- "lean" (adj)</a>
N: \
-- ON _magr_ \
-- Elfd. _mager_ \
W: \
-- OE _mæger_, E _meager_ (= Anglo-Norman _megre_, _meigre_) \
-- Du. _mager_ \
-- OHG _magar_, G _mager_ \
REF: [EDPG]\: 347

### <a id="magu">\*magu- [m.] "boy, relative" (n)</a>
E: \
-- Go. _magus_ "boy" \
N: \
-- ON _mǫgr_ "id." \
W: \
-- OE _magu_ "child, young man, son" \
-- OFri. _mach_ [n.] "child" \
-- OS _magu_ "son" \
-- OHG _maga-zoho_ "nurturer" \
REF: [EDPG]\: 347

### <a id="mahti">\*mahti- [f.] "strength" (n)</a>
E: \
-- Go. _mahts_ \
W: \
-- OE _meaht_, _miht_, E _might_ \
-- OFri. _macht_ \
-- OS _maht_ \
-- Du. _macht_ [c.] \
-- OHG _maht_, G _Macht_ \
REF: [EDPG]\: 347

### <a id="maida">\*maida- [m.] "post" (n)</a>
N: \
-- ON _meior_ "beam; log, tree" \
-- Icel. _meiður_ "id." \
-- Nw. _mei_ "skid" \
-- Sw. _med_ [c.] "beam; skid" \
REF: [EDPG]\: 347

### <a id="maidjan">\*maidjan- [wk.] "to damage, hurt" (v)</a>
N: \
-- ON _meiða_ "to hurt; to damage, destroy" \
-- Far. _meiða_ "to injure a limb" \
REF: [EDPG]\: 347

### <a id="maina">\*maina- "common" (adj)</a>
E: \
-- Go. _ga-mains_ "shared, communal, common, vulgar" \
W: \
-- OE _ge-mǣne_ "common", E _mean_ \
-- OS _gi-mēni_ "joined, common, ordinary" \
-- Du. _ge-meen_ "mean; common" \
-- OHG _gi-mein_, _gi-meini_ "common, public", G _ge-mein_ "mean; common" \
REF: [EDPG]\: 348

### <a id="mainjan">\*mainjan- [wk.] "to think" (v)</a>
W: \
-- OE _mǣnian_ "to mean, intend", E _to mean_ \
-- OFri. _mēna_ "to mean, believe; to suspect; to intend" \
-- OS _mēnian_ "to mean, mention" \
-- Du. _menen_ \
-- OHG _meinen_ "to mean; to believe; to say", G _meinen_ "id." \
REF: [EDPG]\: 348

### <a id="mairja">\*mairja- [n.] "mooring-post" (n)</a>
N: \
-- ON _landa-mæri_ "border post" \
W: \
-- OE _land-gemǣre_ "id." \
-- MDu. _mere_, _meer_ [f.] "post, boundary, mooring-post; pillory", Du. _meer-paal_ [c.] "mooring-post" \
REF: [EDPG]\: 348

### <a id="maisa">\*maisa- [m.] "backpack" (n)</a>
N: \
-- ON _meiss_ "wooden box, basket" \
-- Far. _meis_ [f.] "net basket; net stretched on frame on which corn is dried; certain amount of corn; tousled hair" \
-- Nw. _meis_ [m./f.] "wicker basket, wicker backpack, haynet" (< \*maisa/ō-) \
W: \
-- MDu. _mese_ [f.] "basket, hamper" \
-- OHG _meis(s)a_ [f.] "carriage" (< \*mais-jō-), MHG _meise_ [f.] "a frame to carry on the back" (< \*maisjō-) \
REF: [EDPG]\: 348

### <a id="maisōn">\*maisōn- [f.] "titmouse" (n)</a>
N: \
-- Icel. _-meisa_ \
-- Nw. _meis (<_ \*maisō-) \
W: \
-- OE _māse_, E _tit-mouse_ (with -_mouse_ supplanting regular \*-_mose_) \
-- MDu. _mese_, Du. _mees_ [c.] \
-- OHG _meisa_, G _Meise_ \
REF: [EDPG]\: 349

### <a id="maitan">\*maitan- [st.] "to hew, cut" (v)</a>
E: \
-- Go. _maitan_ \
N: \
-- ON _meita_ [wk.] \
W: \
-- OHG _meizan_, MHG _meizen_ \
REF: [EDPG]\: 349

### <a id="maiwa-1">\*maiwa- "slim, narrow" (adj)</a>
N: \
-- ON _mjórr_ (adv) \
-- Far. _mjáur_ \
-- Elfd. _mją̊_ \
REF: [EDPG]\: 349

### <a id="maiwa-2">\*maiwa- [m.] "sea-gull" (n)</a>
N: \
-- ON _már_, _mór_ \
-- Icel. _máfur_, _mávur_ \
-- Far. _má(v)ur_ \
W: \
-- OE _māw_ \
-- Du. _meeuw_ [c.] \
-- OHG _mēh_, G _Möwe_ (= LG) \
REF: [EDPG]\: 349

### <a id="maizan">\*maizan- "more" (comp)</a>
E: \
-- Go. _maiza_ \
N: \
-- ON _meiri_ \
W: \
-- OE _māra_, E _more_ \
-- OFri. _māra_, _mēra_ \
-- OS _mēro_ \
-- Du. _meer_ \
-- OHG _mēro_, G _mehr_ \
REF: [EDPG]\: 350

### <a id="maka">\*maka- "fit, comfortable" (adj)</a>
N: \
-- ON _makr_ "easy to deal with; suitable" \
-- Far. _makur_ "quite, still, calm" \
W: \
-- MDu. _ge-mak_ "settled, calm, pleasant", Du. _mak_ "tame" \
-- OHG _gi-mah_ "modest, suitable", MHG _ge-mach_ "appropriate, convenient, pleasant" \
REF: [EDPG]\: 350

### <a id="makan">\*makan- [m.] "match" (n)</a>
N: \
-- ON _maki_ \
-- Far. _maki_ "equal, peer; spouse" \
-- Elfd. _matji_ "spouse; match" \
W: \
-- OE _ge-maca_ "mate, equal, companion", ME _make_ "spouse" \
-- OS _gi-mako_ "comrade" \
-- OHG _ga-mahho_ "id." \
REF: [EDPG]\: 350

### <a id="malan">\*malan- [st.] "to grind" (v)</a>
E: \
-- Go. _malan_ \
N: \
-- ON _mala_ \
-- Far. _mala_ \
-- Elfd. _målå_ \
W: \
-- OS _malan_ \
-- Du. _malen_ [st./wk.] \
-- OHG _malan_, G _mahlen_ [st./wk.] \
REF: [EDPG]\: 351

### <a id="maldjō">\*maldjō- [f.] "saltbush, orache" (n)</a>
N: \
-- Nw. _svine-melde_ \
-- OSw. _mæld_ \
W: \
-- OE _melde_ \
-- OS _maldia_ \
-- Du. _melde_ [c.] \
-- OHG _melta_, _melda_, G _Melde_ \
REF: [EDPG]\: 351

### <a id="malha">\*malha/ō- [m./f.] "bag" (n)</a>
N: \
-- ON _malr_ [m.] "knapsack" \
W: \
-- MDu. _male_ [f.] "pouch, purse, bag" \
-- OHG _malaha_ [f.] "bag, knapsack", MHG _malhe_ [f.] "leather bag" \
REF: [EDPG]\: 351

### <a id="malta">\*malta- "soft; gone bad(?)" (adj)</a>
N: \
-- ON _maltr_ "bitter" \
W: \
-- OHG _malz_ "mild" \
REF: [EDPG]\: 351

### <a id="maltjan">\*maltjan- [wk.] "to make dissolve" (v)</a>
E: \
-- Go. _ga-maltjan\*_ "to dissolve" \
N: \
-- ON _melta_ "to malt for brewing; to digest" \
-- Far. _melta_ "to break, smash, crush; to digest, dissolve in the stomach" \
W: \
-- OE _mieltan_ "to (make) melt" \
-- ODu. _gi-meltian_ "to malt" \
REF: [EDPG]\: 351

### <a id="malwjan">\*malwjan- [wk.] "to crush, pound" (v)</a>
E: \
-- Go. _ga-malwjan_ \
N: \
-- ON _mølva_ \
REF: [EDPG]\: 352

### <a id="mampjan">\*mampjan- [wk.] "to mock, scorn" (v)</a>
E: \
-- Go. _bi-mampjan_ \
REF: [EDPG]\: 352

### <a id="managa">\*managa- "many" (adj)</a>
E: \
-- Go. _manags_ \
N: \
-- ON _mangr_, _margr_ (with assimilation) \
-- Far. _mangur_ \
-- Elfd. _maung_ \
W: \
-- OE _manig_, _monig_, _menig_, E _many_ \
-- OFri. _manich_, _monich_, _menich_, WFri. _mennich_ \
-- OS _manag_ \
-- Du. _menig_ \
-- OHG _manag_, _manig_, G _manch_ \
REF: [EDPG]\: 352

### <a id="mandula">\*mandula- [m.] "handle on a grinding mill" (n)</a>
N: \
-- ON _mǫndull_ \
-- Far. _mondul_ \
REF: [EDPG]\: 352

### <a id="mangjan">\*mangjan- [wk.] "to mix" (v)</a>
W: \
-- OE _mengan_ \
-- OFri. _mengia_ \
-- OS _mengian_ \
-- Du. _mengen_ \
-- OHG _mengen_, G _mengen_ \
REF: [EDPG]\: 353

### <a id="mangō">\*mangō- [f.] "lack" (n)</a>
W: \
-- MHG _manc_ \
REF: [EDPG]\: 353

### <a id="manja">\*manja- [n.] "necklace" (n)</a>
N: \
-- ON _men_ \
W: \
-- OE _mene_ \
-- OHG _menni_ \
REF: [EDPG]\: 353

### <a id="mankan">\*mankan- [m.] "mane; upper part of a horse's neck" (n)</a>
N: \
-- ON _makki_ \
-- Nw. _manke_ \
-- Elfd. _maunke_ \
-- Da. _manke_ [c.] \
REF: [EDPG]\: 353

### <a id="mannan">\*mannan- [m.] "man" (n)</a>
E: \
-- Go. _manna_, pl. _manns_ \
N: \
-- ON _maðr_, pl. _meðr_, _menn_ \
-- Far. _maður_, pl. _menn_ \
W: \
-- OE _mann(a)_, pl. _menn_, E _man_ \
-- OFri. _mon_ \
-- Du. _man_ [c.] \
-- OHG _man_ [c.], G _Mann_ [c.] \
REF: [EDPG]\: 353

### <a id="manniska">\*manniska- "human" (adj)</a>
E: \
-- Go. _mannisks_ \
N: \
-- ON _men(n)skr_ \
W: \
-- OE _mennisc_ \
-- OHG _mennisk_ \
REF: [EDPG]\: 354

### <a id="manō">\*manō- [f.] "mane, neck" (n)</a>
N: \
-- ON _mǫn_ \
-- Far. _mon_ "mane hair" \
W: \
-- OE _manu_ "mane", E _mane_ \
-- OFri. _mona_ [m.] "id.", WFri. _moannen_ pl. "id." \
-- MDu. _mane_ "id.", Du. _manen_ pl. "id." \
-- OHG _mana_, G _Mähne_ "mane, crest" \
REF: [EDPG]\: 354

### <a id="marha">\*marha- [m.] "horse, steed" (n)</a>
N: \
-- ON _marr_ \
W: \
-- OE _mearh_ \
-- OFri. _mar_ \
-- OHG _marh_, _marah_, MHG _march_ \
REF: [EDPG]\: 354

### <a id="mari">\*mari- [m./n.] "lake, sea" (n)</a>
E: \
-- Go. _mari-saiws_ [m.] \
N: \
-- ON _marr_ [m.] \
-- Far. _mar_ \
W: \
-- OE _mere_ [m.] \
-- OFri. _mar_ [m.] "ditch" \
-- Du. _meer_ [n.] "lake" \
-- OHG _meri_ [m./n.], G _Meer_ [m./n.] \
REF: [EDPG]\: 354

### <a id="marikan">\*marikan- [m.] "wild celery, smallage" (n)</a>
N: \
-- ON _merki_ [n.] \
-- OSw. _mærke_ [f.] \
-- EDa. _mærkæ_ \
W: \
-- OE _mer(e)ce_, E obs. _march_ \
-- MLG _merk_ \
REF: [EDPG]\: 355

### <a id="marka">\*marka- [n.] "sign" (n)</a>
N: \
-- ON _mark_ "mark, sign; matter of importance" \
-- Far. _mark_ "mark, characteristics" \
-- Elfd. _mark_ [m.] "house mark" \
W: \
-- OE _mearc_ [f.] "sign, mark", E _mark_ \
-- MDu. _marc_ "mark, trademark", Du. _merk_ "id." \
REF: [EDPG]\: 355

### <a id="markō">\*markō- [f.] "boundary, region" (n)</a>
E: \
-- Go. _marka_ \
N: \
-- ON _mǫrk_ "forest" \
-- Far. _mørk_ "wood, wilderness" \
W: \
-- OE _mearc_ "boundary, district" \
-- OFri. _merke_ "id." \
-- OS _marka_ "border, region" \
-- MDu. _marke_ "id." \
-- OHG _marka_ "border, region, end", G _Marke_ "id." \
REF: [EDPG]\: 355

### <a id="marþru">\*marþru- [m.] "marten" (n)</a>
N: \
-- ON _mǫrðr_ \
-- Nw. _mord_ \
-- Sw. _mård_ [c.] \
-- Elfd. _mard_, _marder_ \
W: \
-- OE _mearð_ \
-- WFri. _mird_ [c.] \
-- OHG _mardar_, MHG _mart_, _marder_, G _Marder_ \
REF: [EDPG]\: 355

### <a id="marzjan">\*marzjan- [wk.] "to impede" (v)</a>
E: \
-- Go. _marzjan_ "to hamper, impede, offend" \
W: \
-- OE _mierran_ "to impede; to get lost" \
-- OS _merrian_ "to disturb, annoy" \
-- ODu. _merren_ "to wait, linger" \
-- OHG _merren_ "to impede, tie up" \
REF: [EDPG]\: 356

### <a id="masjinga">\*masjinga-(?) [m.] "brass" (n)</a>
W: \
-- OE _mæs(t)ling_, _mæslen(n)_ [n.] \
-- OFri. _mesken_ "made of brass" \
-- MDu. _messinc_, _missinc_ [n.] "id.", Du. _messing_ [n.] "id." \
-- MHG _messinc_, _missinc_, _mōschinc_ "id." \
REF: [EDPG]\: 356

### <a id="maska">\*maska- [m.] "mesh; stain(?)" (n)</a>
W: \
-- OE _masc_, _max_ [n.] "mesh, net, toil", E _mesh_ \
-- WFri. _mesk_ [c.] "id." \
REF: [EDPG]\: 356

### <a id="masta-1">\*masta- [m.] "mast, fruit of forest trees" (n)</a>
W: \
-- OE _mæst_, E _mast_ \
-- OHG _mast_ [n.], G _Mast_ [f.] "fattening" \
REF: [EDPG]\: 357

### <a id="masta-2">\*masta- [m.] "post, mast" (n)</a>
W: \
-- OE _mæst_ "mast", E _mast_ \
-- OS _mast_ "mast, pole" \
-- Du. _mast_ [c.] "id." \
-- OHG _mast_ "stick, pole, mast", G _Mast_ "mast" \
REF: [EDPG]\: 357

### <a id="masura">\*masura- [m.] "burl, growth" (n)</a>
N: \
-- ON _mǫsurr_ "burl wood; maple" \
-- Icel. _mösur_ "maple" \
-- Sw. _masur_ [c.] "burl wood" \
W: \
-- OS _masur_ "swelling" \
-- MDu. _maser_ "burl" \
-- OHG _masar_ "speckle; burl, vein (of wood)", G _Maser_ [f.] "id." \
REF: [EDPG]\: 357

### <a id="mati">\*mati- [m.] "food" (n)</a>
E: \
-- Go. _mats_ \
N: \
-- ON _matr_ \
-- Far. _matur_ \
-- Elfd. _mat_ \
W: \
-- OE _mete_, E _meat_ \
-- OFri. _mete_ \
-- OS _meti_ [n.], MLG _met_ [n.] "pork" \
-- MDu. _met-worst_ "air-dryed pork sausage" \
-- OHG _maz_ [n.] "food" \
REF: [EDPG]\: 358

### <a id="matjan">\*matjan- [wk.] "to lap" (v)</a>
N: \
-- ON _metja_ \
REF: [EDPG]\: 358

### <a id="maþan">\*maþan- [m.] "maggot, moth" (n)</a>
E: \
-- Go. _maþa_ "worm" \
W: \
-- OE _maða_ "grub, worm, maggot" \
-- OS _matho_ "maggot, worm" \
-- Du. _made_ [c.] "maggot" \
-- OHG _mado_ "maggot, worm", MHG _matte_ [f.] "moth", G _Made_ "maggot" \
REF: [EDPG]\: 358

### <a id="maþla">\*maþla- [n.] "meeting-place" (n)</a>
E: \
-- Go. _maþl_ "forum, marketplace, assembly" \
N: \
-- ON _mál_ "speech; case" \
W: \
-- OE _mæðel_ "assembly, council; speech, address" \
-- OS _mahal_ "court, meeting; speech" \
-- OHG _mahal_ "court, assembly" \
REF: [EDPG]\: 358

### <a id="mauja">\*mauja- "fine, pretty" (adj)</a>
W: \
-- LG _moie_ "pretty" \
-- MDu. _mooy_ "pretty, elegant, proud, vain", Du. _mooi_ "pretty, fine, beautiful" \
REF: [EDPG]\: 359

### <a id="maura">\*maura- [m.] "ant" (n)</a>
N: \
-- ON _maurr_ \
-- Nw. _maur_ \
-- OSw. _mø̄r_ \
REF: [EDPG]\: 359

### <a id="mawēn">\*mawēn- [wk.] "to scrape" (v)</a>
N: \
-- ON _má_ "to blot or rub out, efface; to wear, make blunt" \
-- Icel. _má_ "to blur, efface" \
-- Nw. _må_ "to scrape, grate; wear" \
REF: [EDPG]\: 359

### <a id="mawī">\*mawī- [f.] "girl" (n)</a>
E: \
-- Go. _mawi_, gen. _maujos_ "girl, maid" \
N: \
-- ON _mær_, gen. _meyjar_ "girl; daughter" \
-- Far. _moyggj_ "maid(en), virgin" \
REF: [EDPG]\: 359

### <a id="mazga">\*mazga- [m.] "marrow" (n)</a>
N: \
-- ON _mergr_, gen. _mergjar_ \
-- Far. _mergur_ \
-- Elfd. _mjärg (<_ \*mazgi-) \
W: \
-- OE _mearh_, _mærh_ [m./n.] \
-- OFri. _merch_ [m./n.] \
-- OS _marg_ [n.] \
-- Du. _merg_ [n.] \
-- OHG _marg_ [n.], G _Mark_ [n.] \
REF: [EDPG]\: 359

### <a id="mēan">\*mēan- [st.] "to mow" (v)</a>
W: \
-- OE _mawan_, E _to mow_ \
-- OFri. _miā_, _miān_ \
-- MDu. _maeyen_ [st./wk.], Du. _maaien_ [wk.] \
-- OHG _māen_ [wk.], G _mähen_ [wk.] \
REF: [EDPG]\: 360

### <a id="medi">\*medi "with" (prep)</a>
E: \
-- Go. _miþ_ \
N: \
-- ON _með_ \
W: \
-- OE _mid_ \
-- OFri. _mith_ \
-- OS _mid_, _midi_, _mith_ \
-- Du. _met_ \
-- OHG _mit_, _miti_, G _mit_ \
REF: [EDPG]\: 360

### <a id="medja-1">\*medja- "middle" (adj)</a>
E: \
-- Go. _midjis_ \
N: \
-- ON _miðr_ \
-- Far. _miður_ \
W: \
-- OE _midde_ \
-- OFri. _midde_ \
-- OS _middi_ \
-- OHG _mitti_ \
REF: [EDPG]\: 360

### <a id="medja-2">\*medja- [n.] "aim" (n)</a>
N: \
-- Icel. _mið_ \
REF: [EDPG]\: 360

### <a id="medu">\*medu- [m.] "mead, alcoholic liquor made of honey and water" (n)</a>
N: \
-- ON _mjóðr_ \
-- Far. _mjøður_ "mead" \
-- Elfd. _myöð_ "honey" \
W: \
-- OE _meodo_ [m./n.] "mead", E _mead_ \
-- OFri. _mede_ "id." \
-- MDu. _mede_ "id.", Du. _mede_ [c.] "id." \
-- OHG _metu_, _meta_, _mito_ "id.", G _Meth_ "id." \
REF: [EDPG]\: 361

### <a id="meduma">\*meduma- "middle, midmost" (comp)</a>
E: \
-- Go. _miduma_ [f.] "middle" \
W: \
-- OE _medume_, _medeme_, _meodume_ "moderate, common" (adj) \
-- OHG _metemo_ "middle(most)" \
REF: [EDPG]\: 361

### <a id="mēga">\*mēga- [m.] "son-in-law" (n)</a>
E: \
-- Go. _megs_ \
N: \
-- ON _mágr_ "son-in-law; brother-in-law" \
-- Far. _mágur_ "id." \
-- Elfd. _mą̊g_ "id." \
W: \
-- OE _mǣg_ "relative" \
-- OFri. _mēch_ "id." \
-- OS _māg_ "id." \
-- Du. obs. _maag_ [c.] "id." \
-- OHG _māg_ "id." \
REF: [EDPG]\: 361

### <a id="mekan">\*mekan- [st.] "to hinder" (v)</a>
W: \
-- MHG _ver-mechen_ "to delay, hinder, weaken" \
REF: [EDPG]\: 361

### <a id="mekila">\*mekila- "large, great; much" (adj)</a>
E: \
-- Go. _mikils_ \
N: \
-- ON _mikill_ \
-- Far. _mikil_ \
-- Elfd. _mikkel_ \
W: \
-- OE _micel_ \
-- OS _mikil_ \
-- MDu. _mekel_ \
-- OHG _mihhil_, MHG _michel_ \
REF: [EDPG]\: 361

### <a id="mēla-1">\*mēla- [n.] "point in time" (n)</a>
E: \
-- Go. _mel_ "season, point in time, period of time" \
N: \
-- ON _mál_ "time; meal" \
W: \
-- OE _mǣl_ "moment; meal", E _meal_ \
-- MDu. _mael_ "point in time", Du. _maal_ "meal; time" \
-- OHG _māl_ "point (in time), spot; meal", G _Mal_ "time", _Mahl_ "meal" (with unetymological orthographical disambiguation) \
REF: [EDPG]\: 362

### <a id="mēla-2">\*mēla- [n.] "spot, mark" (n)</a>
N: \
-- ON _mál_ "goal" \
W: \
-- OE _mǣl_ "mark", E _mole_ \
-- MDu. _mael_ "sign, mark, stigma" \
-- OHG _mal_ "spot", G _Mal_ "mark" \
REF: [EDPG]\: 362

### <a id="mēla-3">\*mēla- [n.] "measure, amount" (n)</a>
E: \
-- Go. _mela_ [m.] "bushel" (< \*mēlan-) \
N: \
-- ON _mál_ "measure" \
W: \
-- OE _mǣl_ "measure" \
-- ODu. _māl_ "jar, vase" \
REF: [EDPG]\: 362

### <a id="melda">\*melda- "pleasant, mild" (adj)</a>
E: \
-- Go. _un-milds\*_ "unloving" \
N: \
-- ON _mildr_ "merciful, generous" \
-- Far. _mildur_ "mild, gentle, pleasant" \
W: \
-- OE _milde_ "friendly, merciful", E _mild_ \
-- OFri. _milde_ "merciful, gracious" \
-- OS _mildi_ "kind" \
-- Du. _mild_ "mild, benign" \
-- OHG _milti_ "merciful, meek", G _milt_ "lenient, mild" \
REF: [EDPG]\: 362

### <a id="melha">\*melha- ~ \*melga- [m.] "sand bank" (n)</a>
N: \
-- ON _melr_ "sand bank" \
-- Elfd. _mjåg_ "steep sandy river-bank" \
REF: [EDPG]\: 363

### <a id="melhman">\*melhman- [m.] "cloud" (n)</a>
E: \
-- Go. _milhma_ \
REF: [EDPG]\: 363

### <a id="meliþ">\*meliþ- [n.] "honey" (n)</a>
E: \
-- Go. _miliþ_ \
REF: [EDPG]\: 363

### <a id="meltan">\*meltan- [st.] "to dissolve, be digested" (v)</a>
N: \
-- Icel. _moltinn_ "soft, half-dissolved" \
-- Far. _moltin_ "not quite fresh, slightly decomposed" \
W: \
-- OE _meltan_ "to melt, dissolve", E _to melt_ \
-- MDu. _melten_ "to malt" \
REF: [EDPG]\: 363

### <a id="meltja">\*meltja- [n.] "milt, spleen" (n)</a>
N: \
-- ON _milti_ \
-- Far. _milt(i)_ \
-- Nw. _milte_ \
-- OSw. _milter_ [m.] \
W: \
-- OE _milte_ [f.], E _milt_ \
-- OFri. _milte_ [f.] \
-- MLG _milte_ [f.] \
-- MDu. _milt(e)_, _melte_ [f.], Du. _milt_ [c.] \
-- OHG _milzi_, MHG _milze_, G _Milz_ [f.] \
REF: [EDPG]\: 364

### <a id="meluk">\*meluk- [f.] "milk" (n)</a>
E: \
-- Go. _miluks_ \
N: \
-- ON _mjǫlk_, gen. _mjǫlkr_ \
-- Icel. _mjólk_ \
-- Far. _mjólk_ \
-- Elfd. _mjok_ \
W: \
-- OE _meoloc_, _meolc_, E _milk_ \
-- OFri. _melok_ \
-- OS _miluk_ \
-- MDu. _melc_, _mellic_, _meelc_, Du. _melk_ [m.] \
-- OHG _miluh_, G _Milch_ \
REF: [EDPG]\: 364

### <a id="meluka">\*meluka- "giving milk" (adj)</a>
N: \
-- ON _mjǫlkr_ \
W: \
-- OE _meolc_ \
-- OHG _melch_ \
REF: [EDPG]\: 364

### <a id="melukan">\*mel(u)kan- [st.] "to milk" (v)</a>
W: \
-- OE _melcan_ \
-- OFri. _melka_ \
-- Du. _melken_ \
-- OHG _melkan_, G _melken_ \
REF: [EDPG]\: 364

### <a id="melwa">\*melwa- [n.] "meal, flour" (n)</a>
N: \
-- ON _mjǫl_ \
-- Far. _mjøl_ \
-- Elfd. _myöl_ \
W: \
-- OE _melo_, _melu_, E _meal_ \
-- OFri. _mele_ \
-- OS _melo_ \
-- Du. _meel_ \
-- OHG _melo_, G _Mehl_ \
REF: [EDPG]\: 365

### <a id="mēnan">\*mēnan- [m.] "moon" (n)</a>
E: \
-- Go. _mena_ \
N: \
-- ON _máni_ \
-- Far. _máni_ "moon; month" \
W: \
-- OE _mōna_, E _moon_ \
-- OFri. _mōna_ "moon; month" \
-- OS _māno_ \
-- Du. _maan_ [c.] \
-- OHG _māno_, G _Mond_ \
REF: [EDPG]\: 365

### <a id="mēnōþ">\*mēnōþ- [m.] "month" (n)</a>
E: \
-- Go. _menoþs_ \
N: \
-- ON _mánaðr_ \
-- Icel. _mánaður_ \
-- Far. _mánaður_ \
W: \
-- OE _mōnað_, E _month_ \
-- OFri. _mōnath_ \
-- Du. _maand_ [c.] \
-- OHG _mānōd_, G _Monat_ \
REF: [EDPG]\: 365

### <a id="mēri">\*mēri- "famous" (adj)</a>
E: \
-- Go. _waila-mers_ "with a good reputation" \
N: \
-- ON _mærr_ "famous, excellent; bright, shining" \
W: \
-- OE _mǣre_ "excellent" \
-- OS _māri_ "bright, known, famous, excellent" \
-- OHG _māri_ "id.", MHG _mære_ "shining; famous" \
REF: [EDPG]\: 366

### <a id="mērjan">\*mērjan- [wk.] "to make illustrious" (v)</a>
E: \
-- Go. _merjan_ "to announce" \
N: \
-- ON _mæra_ "to praise" \
W: \
-- OE _mǣran_ "to make known, celebrate, declare" \
-- OHG _mārren_ "to make known", MHG _mæren_ "id." \
REF: [EDPG]\: 366

### <a id="merkwu">\*merkwu- "dark" (adj)</a>
N: \
-- ON _myrkr_ \
-- Far. _myrkur_ \
-- Elfd. _mörk_ \
W: \
-- OE _mierce_ \
-- OS _mirki_ \
REF: [EDPG]\: 366

### <a id="mēslō">\*mēslō- [f.] "growth" (n)</a>
W: \
-- OE _masele_ "rash", E _measles_ \
-- MLG _masele_, _massele_ "rash, blister" \
-- MDu. _masel_ "stain", EDu. _maselen_, _maseren_ pl. "measles", Du. _mazelen_ pl. "id." \
-- OHG _masala_ "inflammation", MHG _masel(e)_ "swelling, growth", G _Masern_ \
REF: [EDPG]\: 366

### <a id="mēsōn">\*mēsōn- [f.] "spot, scar" (n)</a>
W: \
-- MLG _māse_ "stain, scar" \
-- MDu. _mase_ "mud, dirt", EDu. _maese_ "stain" \
-- OHG _māsa_ "id.", MHG _māse_ "spot; scar", G Swi. (App.) _mɔsə_ [m.] "spot" (< \*mēsan-) \
REF: [EDPG]\: 366

### <a id="metan">\*metan- [st.] "to measure, evaluate" (v)</a>
E: \
-- Go. _mitan_ "to measure" \
N: \
-- ON _meta_ "to evaluate" \
-- Far. _meta_ [wk.] "to estimate; to appreciate" \
W: \
-- OE _metan_ "to measure" \
-- OFri. _meta_ "id.", WFri. mjitte "id." \
-- ODu. _metan_ "id.", Du. meten "id." \
-- OHG _mezzan_ "to measure, estimate, compare", G _messen_ "id." \
REF: [EDPG]\: 367

### <a id="mēti">\*mēti- "estimable" (adj)</a>
N: \
-- ON _mætr_ "valuable, excellent" \
W: \
-- OE _ge-mǣte_ "moderate; low" \
-- MDu. _ge-māte_ "moderate; mediocre" \
-- OHG _māzi_ "moderate", MHG _ge-mæze_ "id." \
REF: [EDPG]\: 367

### <a id="metōjan">\*metōjan- [wk.] "to evaluate" (v)</a>
E: \
-- Go. _miton_ "to evaluate, think over, argue" \
W: \
-- OE _metian_ "to assign due measure; to moderate" \
-- OHG _mezzōn_ "to moderate" \
REF: [EDPG]\: 367

### <a id="mēþa">\*mēþa- [n.] "mowing, mown grass" (n)</a>
W: \
-- OE _mǣð_ \
-- OFri. _-mē̆th_ \
-- OHG _ā-mād_ "aftermath", G _Mahd_ [f.] "mown grass" \
REF: [EDPG]\: 367

### <a id="mēþi">\*mēþi- [f.] "measure" (n)</a>
W: \
-- OE _mǣð_ \
REF: [EDPG]\: 367

### <a id="meuka">\*meuka- ~ \*mūka- "weak" (adj)</a>
E: \
-- Go. _muka-modei_ [f.] "meekness" \
N: \
-- ON _mjúkr_ "weak, soft" \
-- Far. _mjúkur_ "soft; supple" \
W: \
-- MDu. _muyck_ "soft, mellow" \
-- G Swi. _mūch_ "timid" \
REF: [EDPG]\: 368

### <a id="meurja">\*meurja- [n.] "chickweed" (n)</a>
W: \
-- WFri. _mier(e)_ [c.], WPhal. _mîr_ [m.], _mîre_ [f.] \
-- MDu. _miere_, _mure_ [f.], EDu. _muer(-kruyd)_, Du. _muur_ [c./n.] \
-- G _Stern-miere_ [f.] \
REF: [EDPG]\: 368

### <a id="meurjōn">\*meurjōn- [f.] "ant" (n)</a>
E: \
-- Go. Crim. _meura_ \
N: \
-- OSw. _mȳra_ \
-- Sw. _myra_ [c.] \
-- Da. _myre_ [c.] \
W: \
-- ME _mīre_, E _pis-mire_ \
-- MDu. _miere_, Du. _mier_ [c.] \
REF: [EDPG]\: 368

### <a id="meusa">\*meusa- [n.] "moss" (n)</a>
W: \
-- OE _mēos_ [m./n.] \
-- MDu. _mies_ [n.(?)] \
-- OHG _mios_ [m./n.], G obs. _Mies_ [m./n.] \
REF: [EDPG]\: 368

### <a id="mīgan">\*mīgan- [st.] "to pass water (usually of animals)" (v)</a>
N: \
-- ON _míga_ \
-- Far. _míga_ \
-- Elfd. _maiga_ \
W: \
-- OE _mīgan_ \
-- MDu. _migen_, Du. dial. _miegen_ \
REF: [EDPG]\: 368

### <a id="miggōn">\*miggōn- [wk.] "to drizzle" (v)</a>
W: \
-- Du. _miggelen_ \
REF: [EDPG]\: 369

### <a id="mihstu">\*mihstu- [m.] "dung, manure" (n)</a>
E: \
-- Go. _maihstus_ \
W: \
-- MDu. _mest_ [m./n.], Du. _mest_ [c.] \
-- OHG _mist_, G _Mist_ \
REF: [EDPG]\: 369

### <a id="mimza">\*mimza- [n.] "meat" (n)</a>
E: \
-- Go. _mimz_, _mims_ "meat" \
REF: [EDPG]\: 369

### <a id="mīna">\*mīna- "my" (pron)</a>
E: \
-- Go. _meins_ \
N: \
-- ON _minn_ \
-- Far. _mín_ \
-- Elfd. _maį_ \
W: \
-- OE _mīn_, E _my_ \
-- OFri. _mīn_ \
-- OS _mīn_ \
-- Du. _mijn_ \
-- OHG _mīn_, G _mein_ \
REF: [EDPG]\: 369

### <a id="minna">\*minna- "small" (adj)</a>
W: \
-- OE _minn_ "small" \
-- Du. _min_ "small, slight" \
REF: [EDPG]\: 369

### <a id="minniz">\*minniz "less" (comp)</a>
E: \
-- Go. _mins_ \
N: \
-- ON _minnr_, _miðr_ \
-- Far. _miður_ "worse" (adv) \
W: \
-- OFri. _minn_ "id." \
-- OS _min_ \
-- OHG _min_ \
REF: [EDPG]\: 369

### <a id="minþla">\*minþla- [m.] "bit of a bridle" (n)</a>
N: \
-- ON _mél_, _míl_ [n.pl.] \
-- Icel. _mél_ [n.] \
-- Far. _meil_ [f.] (with unexplained _ei_ instead of _æ_) \
-- Nw. _mel_ [n.] \
-- OSw. _mīl_ [f.] \
W: \
-- OE _mīðl_, _mīdl_ [n.], ME _mīld_ \
-- OHG _mindil_ [n.] \
REF: [EDPG]\: 370

### <a id="miskan">\*miskan- [st.] "to mix" (v)</a>
W: \
-- OE _miscian_ [wk.], E _to mix_ \
-- MDu. _mischen_ [wk.] \
-- OHG _miscan_, _miscen_ [wk.], G _mischen_ [wk.] \
REF: [EDPG]\: 370

### <a id="mītōn">\*mītōn- [f.] "mite" (n)</a>
N: \
-- Nw. _mit_ [m.] \
W: \
-- OE _mīte_, E _mite_ \
-- MDu. _mite_, Du. _mijt_ [c.] \
-- OHG _mīza_ "mosquito" \
REF: [EDPG]\: 370

### <a id="mittōn">\*mittōn- [wk.] "to cut" (v)</a>
N: \
-- Nw. _mita_ "to perforate, prick, sting" \
W: \
-- MHG _metzen_ "to cut", G Pal. _metzen_ "to slaughter, butcher" \
REF: [EDPG]\: 370

### <a id="mizdōn">\*mizdō(n)- [f.] "reward" (n)</a>
E: \
-- Go. _mizdo_ \
W: \
-- OE _mēd_, _meord_, E _meed_ \
-- OFri. _mēde_ \
-- OS _mēda_ \
-- MDu. _miede_ \
-- OHG _miata_, G _Miete_ "rent" \
REF: [EDPG]\: 370

### <a id="mōder">\*mōder- [f.] "mother" (n)</a>
N: \
-- ON _móðir_ \
-- Far. _móðir_ \
W: \
-- OE _mōdor_, E _mother_ \
-- OFri. _mōder_ \
-- OS _mōdar_ \
-- Du. _moeder_ [c.] \
-- OHG _muoter_, G _Mutter_ \
REF: [EDPG]\: 371

### <a id="mōhan">\*mōhan- ~ magan- [m.] "poppy" (n)</a>
N: \
-- Nw. _vall-mo(g)_ ("lily of the valley") \
-- OSw. _val-moge_ "id." \
-- Sw. _vall-mo_ "id." \
-- ODa. _val-mughe_ "id.", Da. _valmue_ [c.] "id." \
W: \
-- E _maw-seed_ "poppy seed" \
-- OS _magon-hōƀud_ "poppy head" \
-- Du. _maan-zaad_ "poppy seed" \
-- OHG _maho_, _mago_, MHG _mage_, _mahen_, _mān_, G _Mohn_, Swab. _mage_, Cimb. _mago_ \
REF: [EDPG]\: 371

### <a id="mōjan">\*mōjan- [wk.] "to tire" (v)</a>
E: \
-- Go. _mojan\*_ \
W: \
-- MDu. _moyen_ "to bother, annoy" \
-- OHG _muojan_, _muon_ "to encumber, anger", MHG _müejen_, _müen_ "to encumber, to anger, to annoy", G _sich mühen_ "to struggle, strive" \
REF: [EDPG]\: 371

### <a id="mōnjan">\*mōnjan- [wk.] "to tower" (v)</a>
N: \
-- ON _mœna_ \
REF: [EDPG]\: 372

### <a id="mōra">\*mōra- [m./n.] "marsh, moor" (n)</a>
W: \
-- OE _mōr_ [n.], E _moor_ \
-- MDu. _moer_ [m./n.] "bog, peat, mud" \
-- OHG _muor_ [m./n.] "id.", G _Moor_ [n.] "id." \
REF: [EDPG]\: 372

### <a id="mōsa">\*mōsa- [n.] "food" (n)</a>
W: \
-- OE _mōs_ \
-- OFri. _mōs_ \
-- MDu. _moes_ \
-- OHG _muos_ \
REF: [EDPG]\: 372

### <a id="mōtan">\*mōtan- [pp.] "to approach, concede to(?)" (v)</a>
E: \
-- Go. _ga-motan_ "to find opportunity, be permitted" \
W: \
-- OFri. _mōta_ "to be allowed, to be able, to have to" \
-- OS _mōtan_, 3sg. _mōt_ "to be allowed, be permitted, have opportunity to" \
-- Du. _moeten_, 3sg. _moet_ "to be obliged to" \
-- OHG _muozan_, 3sg. _muot_ "to find room for, may, be able", G _müssen_, 3sg. _muß_ "to be obliged to" \
REF: [EDPG]\: 372

### <a id="mōtjan">\*mōtjan- [wk.] "to meet" (v)</a>
E: \
-- Go. _ga-motjan_ \
N: \
-- ON _mœta_ \
-- Far. _møta_ \
-- Elfd. _myöta_ \
W: \
-- OE _mētan_, E _to meet_ \
-- OFri. _mēta_ \
-- OS _mōtian_ \
-- Du. _ont-moeten_ \
REF: [EDPG]\: 372

### <a id="mōþra">\*mōþra- [n.] "womb, belly, bodice" (n)</a>
W: \
-- OFri. _mōther_ "bodice(?)" \
-- MLG _mōder_ [f.] "womb" \
-- Du. _baar-moeder_ [c.] "placenta" \
-- OHG _muodar_ "belly, waistcoat", MHG _muoder_, _müeder_ "body, bodice, girdle", G _Mieder_ "id." \
REF: [EDPG]\: 372

### <a id="mōþu">\*mōþu- "tired, weary" (adj)</a>
N: \
-- ON _móðr_ \
-- Far. _móður_ \
W: \
-- OE _mēðe_ \
-- OS _mōthi_ \
-- Du. _moe_ \
-- OHG _muodi_, G _müde_ \
REF: [EDPG]\: 373

### <a id="mudena">\*mudena- "moldy" (adj)</a>
N: \
-- Far. _moðin_ "moldy, musty" \
-- Nw. _moden_ "mature, ripe, gone bad" \
-- OSw. _moghin_, _moin_ "mature; ripe" \
-- Elfd. _muogen_ "id." \
-- Da. _moden_ "id." \
REF: [EDPG]\: 373

### <a id="mugan">\*mugan- [pp.] "to be able" (v)</a>
E: \
-- Go. _magan_ \
N: \
-- ON _mega_ \
-- Far. _mega_, _muga_ \
W: \
-- OE _magan_ "to be able, may", E _to may_ \
-- OFri. _muga_, _moga_ "id." \
-- OS _mugan_ "id." \
-- Du. _mogen_ "id." \
-- OHG _magan_, _mugan_ "id.", G _mögen_ "to like" \
REF: [EDPG]\: 373

### <a id="mūhan">\*mūhan- ~ \*mukkan- [m.] "pile" (n)</a>
N: \
-- ON _múgi_, _múgr_ "pile, crowd" \
-- Icel. _múgi_ "id." \
-- Far. _múgvi_ "crowd" (also _múgva_ [f.] "id." < \*mūhōn-) \
-- Nw. _muge_ "heap", _mukke_ [f.] "id." \
-- Gutn. _måckå_ [f.] "id." \
W: \
-- OE _mūha_, _mūga_, _mūwa_ "heap", E _mow_ \
REF: [EDPG]\: 373

### <a id="mukkōn">\*mukkōn-(?) [f.] "sow" (n)</a>
W: \
-- MDu. _mocke_ "sow; prostitute" \
-- MHG _mocke_ "breeding sow" \
REF: [EDPG]\: 374

### <a id="mūlan">\*mūlan- [m.] "muzzle" (n)</a>
N: \
-- ON _múli_ "muzzle, snout; promontory" \
-- Far. _múli_ "muzzle, nose; promontory" \
W: \
-- OFri. _mūla_ "muzzle, mouth" \
-- MDu. _mule_ [m./f.] "mouth (of animal)", Du. _muil_ [c.] "id." \
-- OHG _mūla_ [f.] "id.", MHG _mūl_ [n.] "id." \
REF: [EDPG]\: 374

### <a id="mulki">\*mulki- "milkable" (adj)</a>
N: \
-- Icel. _mylkur_ "giving milk" \
W: \
-- G Swi. _lind-mulch_ "easy to milk" \
REF: [EDPG]\: 374

### <a id="mulkna">\*mulkna- [n.] "dairy" (n)</a>
W: \
-- OE _molcen_ "curdled milk" \
-- MDu. _molcken_ \
-- MHG _molken_, _molchen_, G _Molken_ [f.pl.] "whey" \
REF: [EDPG]\: 374

### <a id="mulkōn">\*mulkōn- [wk.] "to milk" (v)</a>
N: \
-- ON _molka_ \
-- OSw. _molka_ \
-- OGutn. _mulka_ \
REF: [EDPG]\: 374

### <a id="multōjan">\*multōjan- [wk.] "to become soft" (v)</a>
N: \
-- Nw. _molta_ "to ripen (of berries)" \
W: \
-- G dial. _molzet_ "soaked" \
REF: [EDPG]\: 374


### <a id="mumpōn">\*mump/bōn- [wk.] "to stain" (v)</a>
W: \
-- E obs. _to mump_ "to cheat, get the better of" \
-- Du. obs. _mompen_ "to cheat, deceive, hide" \
REF: [EDPG]\: 375

### <a id="munan">\*munan- [pp.] "to think, remember, intend" (v)</a>
E: \
-- Go. _munan_ "to reckon, believe, consider" \
N: \
-- ON _muna_, _munu_ "to remember" \
-- Far. _munna_ "to will, be about to" \
W: \
-- OE _munan_ "to remember; to consider, think" \
-- OS _munan_ "to judge"; _far-munan_ "to scorn, disregard, renounce" \
REF: [EDPG]\: 375

### <a id="mundi">\*mundi- [f.] "memory, mind" (n)</a>
E: \
-- Go. _ga-munds_ "remembrance, memory" \
N: \
-- ON _mynd_ "form, image" \
-- Far. _mynd_ "picture" \
W: \
-- OE _ge-mynd_ "mind, memory, memorial", E _mind_ \
-- OHG _gi-munt_ "memory" \
REF: [EDPG]\: 375

### <a id="mundō">\*mundō- [f.] "hand" (n)</a>
N: \
-- ON _mund_ \
W: \
-- OE _mund_ "hand; protection" \
-- OFri. _mund_, _mond_ "protection" \
-- OHG _munt_ "protection" \
REF: [EDPG]\: 375

### <a id="mundra">\*mundra- "lively" (adj)</a>
W: \
-- OHG _muntar_ "brisk, cheerful", G _munter_ "id." \
REF: [EDPG]\: 376

### <a id="muni">\*muni- [m.] "thought, mind" (n)</a>
E: \
-- Go. _muns_ "intention" \
N: \
-- ON _munr_ "mind; difference" \
-- Far. _munur_ "effect, amount, degree; change, difference; balance" \
-- Elfd. _mun_ "effect" \
W: \
-- OE _myne_ "mind" \
REF: [EDPG]\: 376

### <a id="muniwōn">\*muniwōn- [f.] "minnow" (n)</a>
W: \
-- OE _myne_ [m.] "small seafish, eelpout" \
-- MDu. _mone_ "rockling(?)", Du. _meun_ [c.] "rockling" \
-- OHG _mun(e)wa_ "eelpout", G _Möhn_, Pal. _Münne_, _Mimf_ "dace, chub" \
REF: [EDPG]\: 376

### <a id="munþa">\*munþa- [m.] "mouth" (n)</a>
E: \
-- Go. _munþs_ \
N: \
-- ON _munnr_ \
-- Far. _munnur_, _muður_ \
-- Elfd. _munn_ \
W: \
-- OE _mūþ_, E _mouth_ \
-- OFri. _mūth_ \
-- OS _mūth_ \
-- Du. _mond_ [c.] \
-- OHG _mund_, G _Mund_ \
REF: [EDPG]\: 376

### <a id="murgena">\*murge/ana- [m.] "morning" (n)</a>
E: \
-- Go. _maurgins_ \
N: \
-- ON _myrginn_, _morgunn_, _morginn_ \
-- Icel. _morgunn_ \
-- Far. _morgun_ \
-- OSw. _morghon_ \
-- Elfd. _morgun_ \
W: \
-- OE _mergen_, _morgen_ \
-- OFri. _morn_, WFri. _moarn_ [c.], EFri. _māiden_ \
-- OS _morgan_, _moragan_ \
-- ODu. _morgan_, Du. _morgen_ [c.] \
-- OHG _morgan_, G _Morgen_ \
REF: [EDPG]\: 377

### <a id="murgu">\*murgu- "short" (adj)</a>
W: \
-- OHG _murg_, _murgi_ \
REF: [EDPG]\: 377

### <a id="murhōn">\*murhōn- [f.] "wild carrot" (n)</a>
W: \
-- OE _moru_, _more_ \
-- OS _morha_ "parsnip" \
-- MDu. _more_ "carrot; rutabaga" \
-- OHG _moraha_, G _Mohr-rübe_, _Möhre_ (< \*murhjōn-) \
REF: [EDPG]\: 378

### <a id="murþa">\*murþa- [n.] "murder" (n)</a>
N: \
-- ON _morð_ \
-- Far. _morð_ \
-- Elfd. _mųord_ \
W: \
-- OE _morð_ [m.] \
-- OFri. _morth_ \
-- OS _morth_ \
-- Du. _moord_ [c.] \
-- OHG _mord_, G _Mord_ [m.] \
REF: [EDPG]\: 378

### <a id="mūs">\*mūs- [f.] "mouse, muscle" (n)</a>
N: \
-- ON _mús_ "mouse; biceps" \
-- Far. _mús_ "mouse" \
-- Elfd. _maus_ "id." \
W: \
-- OE _mūs_ "mouse", E _mouse_ \
-- OFri. _mūs_ "id.", WFri. _mûs_ [c.] "id." \
-- OS _mūs_ "id." \
-- Du. _muis_ [c.] "id." \
-- OHG _mūs_ "id.", G _Maus_ "id." \
REF: [EDPG]\: 379

### <a id="musan">\*musa(n)- [m./n.] "moss" (n)</a>
N: \
-- ON _mosi_ [m.] "moss; bog" \
-- OSw. _mos_ [n.], _mosi_ [m.] "id." \
-- Elfd. _muosi_ [m.] "id." \
W: \
-- OE _mos_ [n.] "id.", E _moss_ \
-- Du. _mos_ [n.] "moss" \
-- OHG _mos_ [n.] "moss; swamp", G _Moos_ [n.] "moss" \
REF: [EDPG]\: 379

### <a id="musī">\*musī- [f.] "mosquito, fly" (n)</a>
W: \
-- MDu. _meusie_ "mosquito; fly", EDu. _meusie_, _mosie_ "mosquito", Du. Flem. _meuz(i)e_ "id." \
REF: [EDPG]\: 379

### <a id="muska">\*muska- [m.] "haze(?)" (n)</a>
N: \
-- Icel. _musk_ [n.] "dust, fluff; haze" \
-- Far. _æl-musk_ [n.] "light shower of rain" \
-- Nw. _musk_ [n.] "fog; drizzle from fog; dark hue" \
REF: [EDPG]\: 379

### <a id="muþþan">\*muþþa/ōn- [m./f.] "moth" (n)</a>
N: \
-- ON _motti_ [m.] \
-- Far. _motti_ [m.], _motta_ [f.] "mite" \
-- Elfd. _måtta_ [f.] \
W: \
-- OE _moþþe_ [f.], E _moth_ \
-- MDu. _motte_ [f.], Du. _mot_ [c.] \
-- MHG _motte_, _mutte_ [f.], G _Motte_ [f.] \
REF: [EDPG]\: 379

### <a id="muwī">\*muwī- [f.] "mosquito" (n)</a>
N: \
-- OSw. _mygga_ \
W: \
-- OE _mycge_, E _midge_ \
-- WFri. _mich_ [c.], EFri. _määge_ \
-- Du. _mug_ [c.] \
-- OHG _mucka_, G _Mücke_ \
REF: [EDPG]\: 380

## N

### <a id="nabja">\*nabja- [n.] "beak" (n)</a>
N: \
-- ON _nef_ "nose; bill" \
-- Far. _nev_ "bill, beak" \
-- Elfd. _nev_ "nose; mouth, muzzle" \
W: \
-- OE _nebb_ "nose; neb; face" \
-- MLG _nebbe_ [m./f.] "beak" \
-- MDu. _nebbe_ [m./f.] "beak" \
REF: [EDPG]\: 380

### <a id="nablan">\*nablan- [m.] "navel" (n)</a>
N: \
-- ON _nafli_ \
-- Far. _nalvi_ \
-- Elfd. _navel_ \
W: \
-- OE _nafela_, E _navel_ \
-- OFri. _naula_ \
-- Du. _navel_ [c.] \
-- OHG _nabalo_, _nabulo_, G _Nabel_ \
REF: [EDPG]\: 380

### <a id="nabō">\*nabō- [f.] "nave" (n)</a>
N: \
-- ON _nǫf_ \
W: \
-- OE _nafu_, E _nave_ \
-- WFri. _nauwe_ [c.] \
-- MDu. _nave_, Du. _naaf_ [c.] \
-- OHG _naba_, G _Nabe_ \
REF: [EDPG]\: 380

### <a id="nadra">\*nadra- [m.] "adder, snake" (n)</a>
E: \
-- Go. _nadrs\*_ \
N: \
-- ON _naðr_ \
REF: [EDPG]\: 381

### <a id="nagl">\*nagl- [m.] "nail" (n)</a>
N: \
-- ON _nagl_, pl. _negl_ \
-- Icel. _nogl_, pl. _neglur_ [f.] \
-- Far. _naglur_, [f.pl.] _negl_ \
-- Elfd. _nagel_ \
W: \
-- OE _nægel_, _nægl_, E _nail_ \
-- OFri. _neil_ \
-- OS _nagal_ \
-- Du. _nagel_ [c.] \
-- OHG _nagal_, G _Nagel_ \
REF: [EDPG]\: 381

### <a id="naht">\*naht- [f.] "night" (n)</a>
E: \
-- Go. _nahts_ \
N: \
-- ON _nǫtt_, gen. _nætr_, _náttar_, pl. _nætr_ \
-- Icel. _nótt_, pl. _nætur_ \
-- Far. _nátt_, pl. _nætur_ \
-- Elfd. _ną̊t_, pl. _nę́ter_ \
W: \
-- OE _næht_, _niht_, E _night_ \
-- OFri. _nacht_ \
-- OS _naht_ \
-- Du. _nacht_ [c.] \
-- OHG _naht_, G _Nacht_ \
REF: [EDPG]\: 381

### <a id="nakwan">\*nakwan- [m.] "dugout boat" (n)</a>
N: \
-- ON _nǫkkvi_ "ship, dugout" \
-- Icel. _nökkvi_ "id." \
-- Far. poet. _nokki_ "small boat" \
W: \
-- OE _naca_ "id." \
-- MDu. _ake_, _aec_ "boat, barge", EDu. _aecke_, _naecke_ "id.", Du. _aak_ [c.] "id." \
-- OHG _nahho_ "dugout boat", G _Nachen_ "barque" \
REF: [EDPG]\: 382

### <a id="nakweda">\*nakwe/ada- "naked" (adj)</a>
E: \
-- Go. _naqaþs_ \
N: \
-- ON _nøkkviðr_ \
-- OSw. _nakuþer_ \
W: \
-- OE _nacod_, _næcad_, E _naked_ \
-- OFri. _naked_, _nakend_, _naken_ \
-- MDu. _naket_, _nakent_ "id", Du. _naakt_ \
-- OHG _nackot_, MHG _nacket_, _nackent_, G _nackt_ \
REF: [EDPG]\: 382

### <a id="namōn">\*namōn- [n.] "name" (n)</a>
E: \
-- Go. _namo_, pl. _namna_ \
N: \
-- ON _nafn_ \
-- Far. _navn_ \
-- Elfd. _nammen_ \
W: \
-- OE _nama_ [m.], E _name_ \
-- OFri. _noma_ [m.] \
-- OS _namo_ [m.] \
-- Du. _naam_ [c.] \
-- OHG _namo_ [m.], G _Name_ [m.] \
REF: [EDPG]\: 382

### <a id="nanþjan">\*nanþjan- [wk.] "to dare" (v)</a>
E: \
-- Go. _ana-nanþjan_ "to dare, take courage" \
N: \
-- ON _nenna_ "to be inclined, willing" \
-- Far. _nenna_ "to have the heart to" \
W: \
-- OE _nēðan_ "to have courage, dare" \
-- OFri. _nētha_ \
-- OS _nāthian_ \
-- OHG _nenden_, MHG _nenden_, _ge-nenden_ \
REF: [EDPG]\: 383

### <a id="narwa">\*narwa- "narrow" (adj)</a>
W: \
-- OE _nearu_, E _narrow_ \
-- WFri. _near_ \
-- OS _naro_, _naru_ \
-- MDu. _nare_, _naer_, Du. _naar_ "disagreeable, dreary" \
REF: [EDPG]\: 383

### <a id="nasō">\*nasō- ~ \*nusō- [f.] "nose" (n)</a>
N: \
-- ON _nǫs_ "nostril" \
-- Far. _nøs_, pl. _nasar_ "nose" \
-- OSw. _nasær_ [f.pl.] \
W: \
-- OE _nosu_, E _nose_ \
-- OFri. _nose_ \
-- OS _nasa-druppo_ [m.] "cold" \
-- Du. _neus_ [c.] \
-- OHG _nasa_, G _Nase_ \
REF: [EDPG]\: 383

### <a id="nata">\*nata- "wet" (adj)</a>
W: \
-- ODu. _nat_, Du. _nat_ \
-- OHG _naz_, G _naß_ \
REF: [EDPG]\: 384

### <a id="natja">\*natja- [n.] "net" (n)</a>
E: \
-- Go. _nati_ \
N: \
-- ON _net_ \
-- Far. _net_ \
-- Elfd. _nęt_ \
W: \
-- OE _net_, E _net_ \
-- OS _netti_ \
-- Du. _net_ \
-- OHG _nezzi_, G _Netz_ \
REF: [EDPG]\: 384

### <a id="natjō">\*natjō- [f.] "caul, omentum" (n)</a>
N: \
-- ON _netja_ \
-- Far. _netja_ \
W: \
-- OFri. _nette_, _nitte_ "retina" \
-- EDu. _net_ "id." \
-- G _Netzen_ "id." \
REF: [EDPG]\: 384

### <a id="natōn">\*natōn- [f.] "nettle" (n)</a>
N: \
-- ON _nata_ "spear" \
-- Far. _nota_ \
-- Nw. _nata_ \
-- Gutn. _nata_ \
REF: [EDPG]\: 384

### <a id="naustra">\*naust(r)a- [n.] "boathouse, boatshed" (n)</a>
N: \
-- ON _naust_ \
-- Far. _neyst_ \
-- Nw. _naust(r)_ \
REF: [EDPG]\: 384

### <a id="nauta">\*nauta- [n.] "cattle" (n)</a>
N: \
-- ON _naut_ "cattle, ox" \
W: \
-- OE _nēat_ "ox, cow, cattle" \
-- OFri. _nāt_ [m.] "animal" \
-- OS _nōtil_ "domestic animal" (= dim. \*nautila-) \
-- MDu. _noot_ "cattle, cow" \
-- OHG _nōz_, G dial. _Noß_ \
REF: [EDPG]\: 385

### <a id="nauþi">\*nauþi- [f.] "compulsion, distress" (n)</a>
E: \
-- Go. _nauþs_ \
N: \
-- ON _nauð_ "need, difficulty, distress" \
-- Far. _neyð_ "id." \
W: \
-- OE _nīed_ "id.", E _need_ \
-- OFri. _nēd_ "id." \
-- OS _nōd_ "id." \
-- Du. _nood_ [c.] "id." \
-- OHG _not_ "id.", G _Not_ "id." \
REF: [EDPG]\: 385

### <a id="nawi">\*nawi- [m.] "corpse" (n)</a>
E: \
-- Go. _naus_ \
N: \
-- ON _nár_ \
REF: [EDPG]\: 385

### <a id="nazjan">\*nazjan- [wk.] "to save, heal" (v)</a>
E: \
-- Go. _nasjan_, _ga-nasjan_ \
W: \
-- OE _nerian_ "to save" \
-- OFri. _nera_ "to save; to nourish" \
-- OS _nerian_ "id." \
-- MDu. _neren_ "to nourish" \
-- OHG _nerren_, _neren_ "to heal, nourish", G _nähren_ "to nourish" \
REF: [EDPG]\: 385

### <a id="ne">\*ne "not" (adv)</a>
E: \
-- Go. _ni_ \
N: \
-- ON _ne_ \
W: \
-- OE _ne_ \
-- OFri. _ne_, _ni_, _en_ \
-- MDu. _ne_, _en_ \
-- OHG _ne_, _ni_, MHG _ne_, _en_ \
REF: [EDPG]\: 385

### <a id="nēan">\*nēan- [st.] "to sew" (v)</a>
W: \
-- MDu. _naeyen_ [wk.], Du. _naaien_ [wk.] \
-- OHG _nāan_, _nājan_, _nāwan_, G _nähen_ [wk.] \
REF: [EDPG]\: 385

### <a id="nebala">\*nebala- [m.] "fog" (n)</a>
W: \
-- OFri. _nevil_, _newil_ \
-- OS _neƀal_ "fog, darkness" \
-- Du. _nevel_ [c.] "mist" \
-- OHG _nebul_ "fog, darkness", G _Nebel_ "id." \
REF: [EDPG]\: 386

### <a id="nēdrōn">\*nēdrōn- [f.] "viper" (n)</a>
W: \
-- OE _nǣ̆dre_, _næddre_ (also Nrth. _neder_ [m.] < \*nēdra-), E _adder_ \
-- WFri. _njirre_ [c.], EFri. _needer_ \
-- OS _nādra_, _nādara_ \
-- MDu. _nadre_, _addre_, _adder_, Du. _adder_ [c.] \
-- OHG _nā̆t(a)ra_, G _Natter_ \
REF: [EDPG]\: 386

### <a id="nefan">\*nefan- [m.] "nephew, cousin" (n)</a>
N: \
-- ON _nefi_ "relative" \
W: \
-- OE _nefa_ "nephew, grandson, stepson" \
-- OFri. _neva_, _newa_, _nia(u)wa_, _nōwa_ "nephew" \
-- MDu. _neve_ "relative, nephew, cousin", Du. _neef_ [c.] "nephew, cousin" \
-- OHG _nefo_, _nevo_ "grandson, descendant, cousin, nephew", G _Neffe_ "nephew" \
REF: [EDPG]\: 386

### <a id="neftī">\*neftī- [f.] "niece, cousin" (n)</a>
N: \
-- ON _nift_ "female relative, sister" \
W: \
-- OE _nift_ "niece; grand-daughter; step-daughter" \
-- OFri. _nift(e)_, neft "niece, cousin" \
-- MDu. _nichte_ "relative, niece, cousin", Du. _nicht_ [c.] "niece, cousin" \
-- OHG _nift_ "niece", G _Nichte_ "id." \
REF: [EDPG]\: 387

### <a id="nēhwa">\*nēhwa- "near" (adj)</a>
E: \
-- Go. _neƕ_ (adv) \
N: \
-- ON _ná-_ (pref) \
W: \
-- OE _nēah_ (adv/prep), E _nigh_ \
-- OFri. _nei_ "to" (prep) \
-- OS _nāh_ "id." (adv) \
-- Du. _na_ "id." (adv) \
-- OHG _nāh_ "id.", G _nahe_ (adv/prep) "id." \
REF: [EDPG]\: 387

### <a id="neman">\*neman- [st.] "to take" (v)</a>
E: \
-- Go. _niman_ \
N: \
-- ON _nema_ "to take (possession of); to touch; to learn" \
-- Far. _nema_ "to touch; to learn" \
W: \
-- OE _niman_ \
-- OFri. _niman_ \
-- OS _neman_, _niman_ \
-- Du. _nemen_ \
-- OHG _neman_, G _nehmen_ \
REF: [EDPG]\: 387

### <a id="nēmi">\*nēmi- "takeable, acceptable" (adj)</a>
E: \
-- Go. _anda-nems_ "pleasant" \
N: \
-- ON _næmr_ "quick at learning; deprived of" \
-- Far. _næmur_ "apt, quick" \
W: \
-- MDu. _ge-name_ "pleasant" \
-- OHG _nāmi_ "acceptable", G _ge-nehm_ "acceptable, convenient" \
REF: [EDPG]\: 387

### <a id="nesan">\*nesan- [st.] "to save, heal; to be saved" (v)</a>
E: \
-- Go. _ga-nisan_ \
W: \
-- OE _nesan_, _ge-nesan_ "to be saved, survive" \
-- OS _gi-nesan_ "to be saved" \
-- Du. _ge-nezen_ "to heal; to recuperate" \
-- OHG _gi-nesan_ "id.", G _ge-nesen_ "id." \
REF: [EDPG]\: 387

### <a id="nesta">\*nesta- [n.] "food" (n)</a>
N: \
-- ON _nest_ "provisions" \
W: \
-- OE _nest_ "nourishment" \
-- OHG _wega-nest_ "traveling provisions" \
REF: [EDPG]\: 388

### <a id="neþan">\*neþan- [st.] "to grant safety(?)" (v)</a>
E: \
-- Go. _niþan_ "to unite (in consanguinity), help" \
REF: [EDPG]\: 388

### <a id="neþja">\*neþja- [m.] "relative" (n)</a>
E: \
-- Go. _niþjis_ "kinsman" \
N: \
-- ON _niðr_ "son, kinsman, relative" \
W: \
-- OE _niþþas_, _niþas_ [m.pl.] "men" \
REF: [EDPG]\: 388

### <a id="nēþlō">\*nēþlō- [f.] "needle" (n)</a>
E: \
-- Go. _neþla_ \
N: \
-- ON _nál_ \
-- Far. _nál_ \
-- Elfd. _ną̊l_ \
W: \
-- OE _nǣþl_, _nǣdl_, E _needle_ \
-- OFri. _nēdle_, _nēlde_ \
-- OS _nāthla_, _nādla_ \
-- MDu. _naelde_, Du. _naald_ [c.] \
-- OHG _nādala_, _nālda_, G _Nādel_ \
REF: [EDPG]\: 388

### <a id="nēþō">\*nēþō- [f.] "mercy, safety" (n)</a>
N: \
-- ON _náð_ "grace, mercy; rest, peace, quietness" \
W: \
-- OFri. _nēthe_, _nāthe_ "mercy, protection, peace, privilege" \
-- OS _(ge-)nātha_ "id." \
-- Du. _ge-nade_ [c.] "id." \
-- OHG _gi-nāda_ "id.", G _Gnade_ "id." \
REF: [EDPG]\: 388

### <a id="neuhsjan">\*neuhsjan- [wk.] "to search out" (v)</a>
E: \
-- Go. _bi-niuhsjan_ "to scout out, spy out" \
N: \
-- ON _nýsa_ "id." \
W: \
-- OE _nēosan_ \
-- OS _niusian_ "to try" \
-- OHG _niusen_ "id." \
REF: [EDPG]\: 389

### <a id="neuja">\*neuja- "new" (adj)</a>
E: \
-- Go. _niujis_ \
N: \
-- ON _nýr_ \
-- Far. _nýggjur_ \
-- Elfd. _ny(r)_ \
W: \
-- OE _nīwe_, _nēowe_, E _new_ \
-- OS _niuwi_ \
-- Du. _nieuw_ \
-- OHG _niuwi_, G _neu_ \
REF: [EDPG]\: 389

### <a id="neuran">\*neura/ōn- [n./m.] "kidney" (n)</a>
N: \
-- ON _nýra_ [n.] \
-- Far. _nýra_ [n.] \
-- OSw. _niura_ [f.] \
-- Elfd. _niųora_ [n.] \
W: \
-- ME _nēre_ \
-- MDu. _niere_ [f.], Du. _nier_ [c.] \
-- OHG _nioro_ [m.], G _Niere_ [f.] \
REF: [EDPG]\: 389

### <a id="neutan">\*neutan- [st.] "to make use of, enjoy" (v)</a>
E: \
-- Go. _niutan_ "to acquire use of, attain, enjoy" \
N: \
-- ON _njóta_ "to use; to enjoy" \
-- Far. _njóta_ "to enjoy; to keep, retain" \
-- Elfd. _niųota_ "to enjoy" \
W: \
-- OE _nēotan_ "to enjoy; to make use of" \
-- OS _niotan_ "id." \
-- Du. _ge-nieten_ "to enjoy" \
-- OHG _niozan_ "to use; to enjoy", G _ge-nießen_ "to enjoy" \
REF: [EDPG]\: 389

### <a id="newun">\*newun- "nine" (num)</a>
E: \
-- Go. _niun_ \
N: \
-- ON _níu_ \
-- Far. _níggju_ \
-- Elfd. _niu_ \
W: \
-- OE _nigon_, E _nine_ \
-- OFri. _niugun_, _niugen_, _niogen_, _nigun_ \
-- OS _nigun_ \
-- Du. _negen_ \
-- OHG _niun_, G _neun_ \
REF: [EDPG]\: 389

### <a id="newunþan">\*newunþ/dan- "ninth" (num)</a>
E: \
-- Go. _niunda_ \
N: \
-- ON _níundi_ \
-- Far. _níggjundi_ \
-- Elfd. _niund_ \
W: \
-- OE _nigoða_ \
-- OFri. _niugunda_, _niugende_, _niogenda_ \
-- OS _nigundo_, _nigutho_ \
-- Du. _negende_ \
-- OHG _niunto_, G _neunte_ \
REF: [EDPG]\: 390

### <a id="nidwō">\*nidwō- [f.] "downfall(?)" (n)</a>
E: \
-- Go. _nidwa_ "corrosion, decay" \
N: \
-- ON _nið_ [f.pl.] "new moon; darkness" \
-- Far. _niða_ "id." \
REF: [EDPG]\: 390

### <a id="nikwasa">\*nikwas/za- [m.] "water monster" (n)</a>
N: \
-- ON _nykr_ "hippopotamus, water being" \
-- Far. _nykur_ "water elf" \
W: \
-- OE _nicor_ "hippopotamus; water-monster" \
-- MDu. _nicker_ "water-sprite" \
-- OHG _nihhus_, _nichus_ [m./n.] "water-sprite, crocodile", MHG _niches_, _nickes_ [m./n.] "water-sprite", G _Nix_ "river merman" \
REF: [EDPG]\: 390

### <a id="ninþan">\*ninþan- [st.] "to dare" (v)</a>
W: \
-- OHG _gi-nindan_ \
REF: [EDPG]\: 391

### <a id="nista">\*nista- [n.] "nest" (n)</a>
W: \
-- OE _nest_, E _nest_ \
-- Du. _nest_ \
-- OHG _nest_, G _Nest_ \
REF: [EDPG]\: 391

### <a id="niþera">\*niþera- "down" (adv)</a>
N: \
-- ON _niðr_ \
-- Far. _niður_ \
-- Elfd. _niðer_ \
W: \
-- OE _niþer_ \
-- OFri. _nither_ \
-- OS _niđar_ \
-- Du. _neer_ \
-- OHG _nidar_, G _nieder_ \
REF: [EDPG]\: 391

### <a id="nō">\*nō- [m.] "ship" (n)</a>
N: \
-- ON _nór_ \
-- Icel. _nór_ "tempering trough" \
-- Nw. _no_, _nu_ "wooden bowl" \
REF: [EDPG]\: 391

### <a id="nōga">\*nōga- "enough" (adv)</a>
E: \
-- Go. _ga-nohs_ \
N: \
-- ON _gnógr_, _nógr_ \
-- Far. _nógvur_ "much" \
W: \
-- OE _ge-nōh_, E _enough_ \
-- OFri. _enōch_ \
-- OS _gi-nōh_, _gi-nōg_ \
-- Du. _ge-noeg_ \
-- OHG _gi-nuog_ (adj/adv), G _ge-nug_ \
REF: [EDPG]\: 391

### <a id="nōmjan">\*nōmjan- [wk.] "to call, give a name" (v)</a>
W: \
-- MDu. _noemen_, Du. _noemen_ \
-- MHG _be-nüemen_ \
REF: [EDPG]\: 391

### <a id="nōtan">\*nōtan- [m.] "rear (of a ship)" (n)</a>
E: \
-- Go. _nota\*_ "afterdeck, stem, rear of a ship" \
REF: [EDPG]\: 392

### <a id="nōzjan">\*nōzjan- [wk.] "to refresh, nourish" (v)</a>
N: \
-- ON _nœra_ \
-- Far. _nøra_ "to bring up, rear; to increase" \
REF: [EDPG]\: 392

### <a id="nū">\*nū "now" (adv)</a>
E: \
-- Go. _nu_ \
N: \
-- ON _nú_ \
-- Far. _nú_, obs. _núgv_ \
-- Elfd. _nų_ \
W: \
-- OE _nū_, E _now_ \
-- OFri. _nū_ \
-- MDu. _nu_, Du. _nu_, _nou_ \
-- OHG _nū̆_, _nun_, G _nun_ \
REF: [EDPG]\: 392

### <a id="nugan">\*nugan- [pp.] "to suffice" (v)</a>
E: \
-- Go. _ga-nauhan_, 3sg. _ga-nah_ \
W: \
-- OE _ge-nugan_, 3sg. _ge-neah_ \
-- OHG _gi-nugan\*_, 3sg. _ge-nah_ \
REF: [EDPG]\: 392

### <a id="nurþera">\*nurþera- "north, northwards" (adv)</a>
N: \
-- ON _norðr_ \
-- Far. _norður_ \
-- Elfd. _norter_ \
W: \
-- OFri. _norther_ \
-- MDu. _norder_ \
-- MHG _norder_ \
REF: [EDPG]\: 393

### <a id="nuti">\*nuti- "useful" (adj)</a>
E: \
-- Go. _un-nuts_ "useless, unavailing" \
W: \
-- OE _nyt_ \
-- OFri. _nette_ \
-- OS _nutti_ \
-- MDu. _nutte_ \
-- OHG _nuzzi_, MHG _nütze_ \
REF: [EDPG]\: 393

### <a id="nuttōn">\*nut(t)ōn- [wk.] "to drive" (v)</a>
N: \
-- Nw. _not(t)a_ "to drive (a horse)" \
REF: [EDPG]\: 393

## O

### <a id="ōbjan">\*ōbjan- [wk.] "to perform" (v)</a>
W: \
-- OS _ōƀian_ "to celebrate" \
-- MDu. _oeven_ "to practice" \
-- OHG _uoben_, _uoppen_ "to practice, exercise; to celebrate", G _üben_ "to practice" \
REF: [EDPG]\: 393

### <a id="ōfera">\*ōf(e)ra- [m./n.] "edge; shore" (n)</a>
W: \
-- OE _ōfer_, _ōfor_ [m.] "edge, border, margin" \
-- MDu. _oever_ [m./n./f.] "shore", Du. _oever_ [c.] "shore" \
-- MHG _uover_ [n.] "shore", G _Ufer_ [n.] "bank, shore" \
REF: [EDPG]\: 394

### <a id="ōgjan">\*ōgjan- [wk.] "to scare" (v)</a>
E: \
-- Go. _ogjan_ \
N: \
-- ON _œgja_ \
W: \
-- OE _on-œgan_ "to fear" \
REF: [EDPG]\: 394

### <a id="ōhstnō">\*ōhs(t)nō- [f.] "armpit" (n)</a>
W: \
-- OE _ōxn_ \
-- OHG _uohsana_ \
REF: [EDPG]\: 394

### <a id="ōsan">\*ōsa(n)- [m.(?)] "mouth of a river, estuary" (n)</a>
N: \
-- ON _óss_ [m.] \
-- Far. _ósi_ [m.] \
-- Nw. _os_ [m./n.] "estuary, hole in the ice" (cf. dial. _øse_ [n.] "id." < \*ōsja-) \
REF: [EDPG]\: 394

### <a id="ōþala">\*ōþala- [n.] "patrimony" (n)</a>
N: \
-- ON _óðal_ "ancestral property, patrimony, inheritance" \
W: \
-- OE _ōðel_ [m.] "home, native country" \
-- OS _ōthil_ "home" \
-- OHG _uodal_ "ancestral property" \
REF: [EDPG]\: 395

## P

### <a id="paddōn">\*pad(d)ōn- [f.] "toad" (n)</a>
N: \
-- ON _padda_ \
-- Far. _padda_ \
-- Elfd. _padda_ \
W: \
-- ME _pade_, _padde_ "toad; toadstool, mushroom", E _paddock_ \
-- MDu. _padde_, Du. _pad_ [c.] \
REF: [EDPG]\: 395

### <a id="pagila">\*pagila- [m.] "measuring stick(?)" (n)</a>
W: \
-- OE _pægel_ "wine-vessel", E _pail_ \
-- MDu. _pegel_, _peil_ [m./f.] "knob in vessel for liquid goods to measure their contents", Du. _pegel_ [c.] "peg, pin, icicle" \
REF: [EDPG]\: 395

### <a id="paidō">\*paidō- [f.] "coat, shirt" (n)</a>
E: \
-- Go. _paida_ "tunic, vest, shirt" \
W: \
-- OE _pād_ "coat, cloak" \
-- OS _pēda_ "garment" \
-- OHG _pfeit_ "coat", MHG _pfeit_ "shirt" \
REF: [EDPG]\: 395

### <a id="pakka">\*pakka- [n.] "bundle, pack" (n)</a>
W: \
-- ME _packe_, E _pack_ \
-- MLG _pack_ \
-- MDu. _pac_, Du. _pak_ \
REF: [EDPG]\: 396

### <a id="paþa">\*paþa- [m./n.] "path" (n)</a>
W: \
-- OE _pæð_, _pað_ [m.], E _path_ \
-- OFri. _path_, _pad_ [n.] \
-- MDu. _pat_ [m./n.], Du. _pad_ [n.] \
-- OHG _pfad_ [m./n.], G _Pfad_ [m.] \
REF: [EDPG]\: 396

### <a id="piþman">\*piþman- ~ \*pittan- [m.] "root, pith" (n)</a>
W: \
-- OE _piþa_ "pith" \
-- WFri. _piid_ "pith, stone (of a fruit)" (< \*pidan-?) \
-- EDu. _pee_ "skirwort, skirret, (edible) root", _peën_ "bent grass, tall oat grass", Du. _peen_ [c.] "carrot" \
REF: [EDPG]\: 396

### <a id="plagōn">\*plag/kkōn- [f.] "rag, sod" (n)</a>
N: \
-- ON _plagg_ [n.] "cloth" \
W: \
-- MDu. _placke_, _plagge_ "rag; sod, turf", Du. _plag_ [c.] "sod", _plak_ [c.] "slice, patch" \
-- MHG _pflacke_ [m.] "rag, patch, stain" \
REF: [EDPG]\: 397

### <a id="plehan">\*plehan- ~ \*plegan- [st.] "to take responsibility" (v)</a>
W: \
-- OS _plegan_ \
-- MDu. _plien_, _plegen_ "to guarantee, take care of, be accustomed to" \
-- OHG _pflegan_ "to take care of, maintain, nurse", G _pflegen_ [wk.] "id." \
REF: [EDPG]\: 397

### <a id="plehti">\*plehti- [m./f.] "responsibility" (n)</a>
W: \
-- OE _pliht_ [m./f.] "danger, damage", E _plight_ \
-- OFri. _plicht_ [f.] "caution, responsibility" \
-- MDu. _plicht_ [f.] "obligation", Du. _plicht_ [c.] "id." \
-- OHG _pfliht_ [f.] "fervor, caution, practice, manner", G _Pflicht_ [f.] "obligation, responsibility" \
REF: [EDPG]\: 397

### <a id="plōga">\*plōga- [m.] "plow" (n)</a>
N: \
-- ON _plógr_ \
-- Icel. _plógur_ \
-- Far. _plógv_ [n.] "jointer plane; groove in a board; plow"; _plógv_ [f.] "plow" \
W: \
-- OE _plōh_, E _plow_ \
-- OFri. _plōch_ \
-- MDu. _ploech_ [m./f.] "plow; team, group", Du. _ploeg_ [c.] "id." \
-- OHG _pfluog_ "plow; livelihood", G _Pflug_ "plow" \
REF: [EDPG]\: 398

### <a id="plukkjan">\*plukkjan- [wk.] "to pluck" (v)</a>
W: \
-- OE _plyccean_, ME _plicchen_ "to pull, snatch, pluck", E obs. _to plitch_ "id." \
-- MDu. _plocken_, _plucken_, Du. _plukken_ "to pick, pluck" \
REF: [EDPG]\: 398

### <a id="plukkōn">\*pluk(k)ōn- [wk.] "to pluck" (v)</a>
W: \
-- OE _ploccian_ "to pull off, cull", E _to pluck_ \
-- OFri. _hēr-plokkia_ "to pluck out hair", WFri. _ploats(j)e_, _plôk(j)e_ "to pluck", LG _ploken_ "to pluck" \
REF: [EDPG]\: 398

### <a id="pōla">\*pōla- [m.] "pool" (n)</a>
W: \
-- OE _pōl_, E _pool_ \
-- OFri. _pōl_ \
-- MDu. _poel_, Du. _poel_ [c.] \
-- OHG _pfuol_, G _Pfuhl_ \
REF: [EDPG]\: 398

### <a id="prakkōn">\*prakkōn- [wk.] "to press(?)" (v)</a>
W: \
-- LG _prakken_ "to squeeze" \
-- Du. _prakken_ "to mash" \
REF: [EDPG]\: 399

### <a id="prangan">\*prangan- [st.] "to press" (v)</a>
E: \
-- Go. _ana-praggan_ "to press upon, oppress, trouble" \
W: \
-- MLG _prangen_ [wk.] "to press, clamp" \
-- MDu. _prangen_ [wk.] "to press, squeeze; to wrestle, fight", Du. _prangend_ [ptc.] "urgent" \
REF: [EDPG]\: 399

### <a id="preunan">\*preuna(n)- [m.] "needle, pin" (n)</a>
N: \
-- Icel. _prjónn_ "knitting needle" \
-- Far. _prónur_ "large pin, fixing pin" \
-- Nw. _prjóna_ [f.] "fixing pin" \
-- Da. _pryne_ [c.] "needle for stringing fish; ring for pigs; eel prong" \
W: \
-- OE _prēon_ "pin, brooch, fastening", E _preen_ "decorative pin, brooch, clasp" \
-- MLG _prēn(e)_, _prēme_ "awl" \
-- MDu. _priem(e)_ "needle, awl, pin", Du. _priem_ [c.] "awl" \
-- MHG _pfrieme_ "id.", G _Pfriem_ "id." \
REF: [EDPG]\: 399

### <a id="prikkōn">\*prik(k)ōn- [wk.] "to prick" (v)</a>
W: \
-- OE _prician_, E _to prick_ \
-- MLG _pricken_ \
-- Du. _prikken_ \
-- MHG _pfrecken_ \
REF: [EDPG]\: 399

### <a id="puhan">\*puh/kkan- [m.] "bag" (n)</a>
N: \
-- ON _poki_ \
W: \
-- OE _pocca_, _pohha_ "purse", E _pough_ \
-- MDu. _poke_ "bag (for wool)", EDu. _poke_ "hairshirt, crop", Du. obs. _pook_ "bag" \
-- G _Pfoch_ "id." \
REF: [EDPG]\: 400

### <a id="pūka">\*pūka- ~ \*pūga- [n.(?)] "sheeting(?)" (n)</a>
W: \
-- MLG _pūche_, _pūghe_ [f.] "blanket, covering", LG _pūch_ "bed" \
-- MDu. _puuc_ [n.] "(high quality) sheeting" \
REF: [EDPG]\: 400

### <a id="punga">\*punga- [m.] "purse, pouch" (n)</a>
E: \
-- Go. _puggs_ \
N: \
-- ON _pungr_ \
W: \
-- OE _pung_ \
-- ODu. _pung_, _pong_ \
-- OHG _scaz-pfung_ \
REF: [EDPG]\: 400

### <a id="pursa">\*pursa- [m.] "bog myrtle" (n)</a>
W: \
-- MLG _pors_ \
-- MDu. _pors(e)_ \
REF: [EDPG]\: 400

### <a id="puttōn">\*put(t)ōn- [wk.] "to poke, prod" (v)</a>
N: \
-- Icel. _pota_ "to prod/tuck into" \
-- Far. _pota_ "to thrust, stick; to prod, poke" \
-- Elfd. _puotå_ "to poke" \
W: \
-- OE _potian_ "to push, strike, butt" \
-- MLG _poten_, _potten_ "to plant in the ground, implant" \
-- MDu. _poten_ "id.", Du. _poten_ "id." \
REF: [EDPG]\: 401

## R

### <a id="rada">\*rada- "fast(?)" (adj)</a>
E: \
-- Go. _raþs_ "effortless, facile, easy" \
W: \
-- OE _ræd_ "fast" \
-- Du. _rad_ "fast" \
REF: [EDPG]\: 401

### <a id="radō">\*radō- [f.] "row" (n)</a>
N: \
-- ON _rǫð_ \
-- Elfd. _rað_ "row, line" \
W: \
-- MLG _rat_ "line, order(?)" \
REF: [EDPG]\: 401

### <a id="ragina">\*ragina- [n.] "decision" (n)</a>
E: \
-- Go. _ragin_ "opinion, law, decree, task, responsibility" \
N: \
-- ON _regin_ [n.pl.] "the ruling powers, the gods" \
W: \
-- OS _regina-giskapu_ "fate" \
REF: [EDPG]\: 401

### <a id="rahna">\*rahna- [n.] "booty" (n)</a>
N: \
-- ON _rán_ "robbery, plundering; booty" \
W: \
-- OHG _rahan_ "booty" \
REF: [EDPG]\: 402

### <a id="rahō">\*rahō- [f.] "pole, ray" (n)</a>
N: \
-- ON _rá_ "berth in a ship" \
-- Far. _rá_ "yard; stick, cudgel" \
W: \
-- MDu. _ra_ "stick; yard", Du. _ra_ [c.] "yard" \
-- MHG _rahe_ "stick; yard" \
REF: [EDPG]\: 402

### <a id="raibō">\*raibō- [f.] "swaddling cloth, wrap" (n)</a>
N: \
-- ON _reifar_ [pl.] "swaddling bands" \
REF: [EDPG]\: 402

### <a id="raidja">\*raid(j)a- "arranged, prepared" (adj)</a>
E: \
-- Go. _ga-raiþs_ "determined, arranged, commanded" \
N: \
-- ON _greiðr_ "cleared, ready, settled" \
-- Far. _greiður_ "straightforward; gentle" \
W: \
-- OE _rǣde_, _ge-rǣde_ "ready, easy", E _ready_ \
-- OFri. _rede_, red, reid "ready; available; payable" \
-- MDu. _ge-rede_ "ready; considerable", Du. _ge-reed_ "id." \
-- OHG _gi-reiti_ "ready", MHG _reite_, _ge-reite_ "id." \
REF: [EDPG]\: 402

### <a id="raihan">\*raiha(n)- [m.] "roe deer" (n)</a>
N: \
-- ON _rá_ [f.] \
-- Far. _rá-djór_ [n.] \
-- Elfd. _rå-diuor_ [n.] \
W: \
-- OE _rā_, _rāha_, E _roe_ \
-- ODu. _rēho_, Du. _ree_ [c.] \
-- OHG _rēh_ [n.], _rēho_, G _Reh_ [n.] \
REF: [EDPG]\: 402

### <a id="raihō">\*raihō- [f.] "row" (n)</a>
N: \
-- Nw. _rå_ "border between fields" \
W: \
-- OE _rāwe_ "row" \
REF: [EDPG]\: 403

### <a id="raina">\*raina/ō- [m./n./f.] "hill, border(?)" (n)</a>
N: \
-- ON _rein_ [f./n.] "marge, strip of land" \
-- Far. _rein_ [f.] "edge of a field, piece of land" \
-- Elfd. _rien_ [f.] "id." \
W: \
-- MHG _rein_ [m.] "marge, elevation", G _Rain_ [m.] "id." \
REF: [EDPG]\: 403

### <a id="raipa">\*raipa- [m./n.] "strap" (n)</a>
E: \
-- Go. _skauda-raip_ [n.] "shoelace" \
N: \
-- ON _reip_ [n.] "rope" \
W: \
-- OE _rāp_ [m.] "id.", E _rope_ \
-- OFri. _rāp_ [n.] "id." \
-- OHG _reif_ [m.], G _Reif_ [m.] \
REF: [EDPG]\: 403

### <a id="raizjan">\*raizjan- [wk.] "to cause to rise, raise" (v)</a>
E: \
-- Go. _ur-raisjan_ \
N: \
-- ON _reisa_ \
-- Far. _reisa_ \
W: \
-- OE _rǣran_, E _to rear_ \
-- MDu. _reren_ "to make fall, make shed" \
-- OHG _rēren_ "to cause to fall", MHG _rēren_ "to cause to fall; to make shed" \
REF: [EDPG]\: 403

### <a id="raka">\*raka- "straight" (adj)</a>
N: \
-- ON _rakr_ \
W: \
-- LG _rak_ "straight, correct" \
REF: [EDPG]\: 403

### <a id="rakjan">\*rakjan- [wk.] "to stretch" (v)</a>
E: \
-- Go. _uf-rakjan_ "to stretch out, extend, reach out" \
N: \
-- ON _rekja_ "to wrap" \
W: \
-- OE _reccan_ "to stretch, extend" \
-- OFri. _rekka_, retsa "to reach", EFri. _räkke_ "id." \
-- OS _rekkian_ "to tell, explain" \
-- Du. _rekke_ \
-- OHG _recken_ "to stretch, reach, explain", G _recken_ "id." \
REF: [EDPG]\: 403

### <a id="rakkan">\*rak(k)an- [m.] "(sleuth) dog" (n)</a>
N: \
-- ON _rakki_ \
-- Far. _rakki_ "(young) dog" \
-- Elfd. _rakke_ "dog" \
W: \
-- OE _ræc_, _racca_ "sleuth dog", E dial. _rake_ "sheepdog" \
-- G Alem. _rache_ "sleuth dog (Canis Brittanicus)" \
REF: [EDPG]\: 404

### <a id="rakkan">\*rakkan- [m.] "parrel" (n)</a>
N: \
-- ON _rakki_ \
-- Far. _rakki_ "parrel; jaw rope, gaffe parrel" \
W: \
-- OE _racca_ \
-- Du. _rak_ [n.] (< Scand.?) \
REF: [EDPG]\: 404

### <a id="rakkōn">\*rakk/gōn- [wk.] "to move to and fro" (v)</a>
N: \
-- Icel. _raga_ "to stroll" \
-- Far. _ragga_ "to walk slowly" \
-- Nw. _rakka_ "to swerve, roam" \
W: \
-- MDu. _raggen_ "to run, be wild" \
-- G Pal. _racken_ "to rock, swing" \
REF: [EDPG]\: 404

### <a id="rakkōn">\*rak(k)ōn- [wk.] "to incline" (v)</a>
W: \
-- E _to rake_ \
-- EDu. _racken_ "to turn, be bent, incline" \
REF: [EDPG]\: 404

### <a id="ramma">\*ramma- [m.] "ram" (n)</a>
W: \
-- OE _ram_, E _ram_ \
-- MDu. _ram_, Du. _ram_ [c.] \
-- OHG _ram_, _rammo_, G _Ramm_ \
REF: [EDPG]\: 404

### <a id="ranka">\*ranka- "erect" (adj)</a>
N: \
-- ON _rakkr_ (also _hug-rakkr_ "bold") \
W: \
-- OE _ranc_ "proud, arrogant", E _rank_ \
-- MDu. _ranc_ "slim", Du. _rank_ "tall" \
REF: [EDPG]\: 404

### <a id="rannjan">\*rannjan- [wk.] "to cause to run" (v)</a>
E: \
-- Go. _ur-rannjan_ "to cause to rise" \
N: \
-- ON _renna_ "to make run" \
W: \
-- OS _rennian_ "id." \
-- Du. _rennen_ "to run" \
-- OHG _rennen_ "id.", G _rennen_ "id." \
REF: [EDPG]\: 405

### <a id="rastō">\*rastō- [f.] "interval" (n)</a>
E: \
-- Go. _rasta_ "mile" \
N: \
-- ON _rǫst_ "id." \
W: \
-- OE _ræst_ "rest, peace, grave", E _rest_ \
-- OS _rasta_ "rest, resting place, grave" \
-- OHG _rasta_ "rest, length (of three hours)" \
REF: [EDPG]\: 405

### <a id="raþa">\*raþa- [n.] "wheel" (n)</a>
W: \
-- OFri. _reth_ \
-- ODu. _rath_, Du. _rad_ \
-- OHG _rad_, _hrad_, G _Rad_ \
REF: [EDPG]\: 405

### <a id="raþan">\*raþan- ~ \*ratta(n)- [m.] "rat" (n)</a>
W: \
-- OE _ræt_, E _rat_ \
-- WFri. _rôt_ [c.] \
-- OS _ratta_ [f.] \
-- MDu. _ratte_, _rotte_ [f.], Du. _rat_ [c.] \
-- OHG _rato_, _ratto_, MHG _ratze_, G _Ratte_ [f.] \
REF: [EDPG]\: 405

### <a id="raþjōn">\*raþjōn- [f.] "account(?)" (n)</a>
E: \
-- Go. _raþjo_ "counting, account, explanation, number" \
W: \
-- OFri. _rethia_ "talk, acount" \
-- OS _rethia_ "id." \
-- OHG _red(e)a_ "account, reasoning, speech", G _Rede_ "speech" \
REF: [EDPG]\: 405

### <a id="rauba">\*rauba- [m./n.] "loot" (n)</a>
W: \
-- OE _rēaf_ [n.] "spoil, booty; garment, robe" \
-- OFri. _rāf_ [n.] "booty" \
-- OS _nōd-rōf_ [m.] "violent robbery" \
-- Du. _roof_ [c.] "theft, robbery" \
-- OHG _roub_ [m.] "robbery, booty", G _Raub_ [m.] "id." \
REF: [EDPG]\: 406

### <a id="raubjan">\*raubjan- [wk.] "to break (off), tear off, rob" (v)</a>
N: \
-- ON _reyfa_ "to break, tear; to rob" \
W: \
-- OE _be-riefan_ "to rob", E _to bereave_ \
REF: [EDPG]\: 406

### <a id="raubō">\*raubō- [f.] "rift" (n)</a>
N: \
-- ON _rauf_ "rift, hole" \
-- Far. _reyv_ "seat; behind, rump" \
-- Elfd. _rov_ "tail; behind" \
-- Da. _røv_ [c.] "behind" \
REF: [EDPG]\: 406

### <a id="rauda">\*rauda- "red" (adj)</a>
E: \
-- Go. _rauþs_ \
N: \
-- ON _rauðr_ \
-- Far. _reyður_ \
-- Elfd. _roð_ \
W: \
-- OE _rēad_, E _red_ \
-- OFri. _rād_ "red, yellow" \
-- OS _rōd_ \
-- Du. _rood_ \
-- OHG _rōt_, G _rot_ \
REF: [EDPG]\: 406

### <a id="rauki">\*rauki- [m.] "smoke" (n)</a>
N: \
-- ON _reykr_ "smoke, steam" \
-- Far. _roykur_ "smoke; dust; smell" \
-- Elfd. _räk_ \
W: \
-- OE _rēc_ \
-- OFri. _rēk_ \
-- OS _rōk_ \
-- Du. _rook_ [c.] \
-- OHG _rouh_, G _Rauch_ \
REF: [EDPG]\: 406

### <a id="raupjan">\*raupjan- [wk.] "to tear (Gff)" (v)</a>
E: \
-- Go. _raupjan_ "to rip" \
W: \
-- OE _rīepan_ "to loot", E _to reap_ \
-- EDu. _rōpen_ "to pluck, tear off" \
-- OHG _roufen_ "to rip, tear" \
REF: [EDPG]\: 406

### <a id="rautōjan">\*rautōjan- [wk.] "to roar(?)" (v)</a>
N: \
-- ON _rauta_ \
REF: [EDPG]\: 407

### <a id="rauza">\*rauza- [n.] "reed" (n)</a>
E: \
-- Go. _raus_ \
N: \
-- ON _reyrr_ [m.], _reyr_ [f.] \
-- Icel. _reyr_ \
-- Far. _royggj_ "reed canary grass" (with analogical removal of the -_r_) \
W: \
-- OFri. _rēr_ [m.] "reed (stalk)" \
-- MDu. _roer_ "id." \
-- OHG _rōr_ "id.", G _Rohr_ "reed; pipe, tube" \
REF: [EDPG]\: 407

### <a id="rawwō">\*rawwō- [f.] "(plucked) wool" (n)</a>
N: \
-- ON _rǫggr_ [m.], _rǫgg_ "shaggy hair or wool" \
-- Icel. _rōgg_ "id." \
-- Nw. _ragg_ [n.] "coarse hair of animals (esp. goats)" \
REF: [EDPG]\: 407

### <a id="razdō">\*razdō- [f.] "voice" (n)</a>
E: \
-- Go. _razda_ "language, speech" \
N: \
-- ON _rǫdd_ \
-- Far. _rødd_ \
W: \
-- OE _reord_ "speech, language, voice" \
-- OHG _rarta_ "sound, voice" \
REF: [EDPG]\: 407

### <a id="rebja">\*rebja- [n.] "rib" (n)</a>
N: \
-- ON _rif_ \
-- Far. _riv_ "rib; reef" \
W: \
-- OE _rib_, E _rib_ \
-- OFri. _rib_ [m.] \
-- MDu. _ribbe_ [n./f.], Du. _rib_ [c.] \
-- OHG _rippi_, G _Rippe_ [f.] \
REF: [EDPG]\: 407

### <a id="rēdan">\*rēdan- [st.] "to decide" (v)</a>
E: \
-- Go. _ga-redan_ "to plan, intend" \
N: \
-- ON _ráða_ "to advise, counsel; to consult about; to devise, plan" \
-- Far. _ráða_ "id." \
-- Elfd. _råða_ "id." \
W: \
-- OE _rǣdan_ "to counsel, decide, interpret", E _to read_ \
-- OFri. _rēda_ "to advice, protect, help, cause, decide" \
-- OS _rādan_ "to counsel, plan, realize, help" \
-- Du. _raden_ [st./wk.] "to advise, guess" \
-- OHG _rātan_ "to counsel, decide, help", G _raten_ [wk.] "to advise" \
REF: [EDPG]\: 408

### <a id="rēfiz">\*rēfiz- [n.] "roof beam" (n)</a>
N: \
-- ON _ráfr_, _ræfr_ "roof" \
-- Far. _ráva_ [f.] "roof ridge of a house" \
W: \
-- OS _rāƀo_ [m.] \
-- MDu. _rave_ [m.] \
-- OHG _rāfo_, _rāvo_ [m.] "board" \
REF: [EDPG]\: 408

### <a id="regna">\*regna- [n.] "rain" (n)</a>
E: \
-- Go. _rign_ \
N: \
-- ON _regn_ \
-- Far. _regn_ \
-- Elfd. _raingen_ \
W: \
-- E _rain_ \
-- OFri. _rein_ [m.], WFri. _rein_ [c.], EFri. _rien_ [m.] \
-- OS _regan_, _regin_ [m.] \
-- ODu. _regan_ [m.], Du. _regen_ [c.] \
-- OHG _regan_ [m.], G _Regen_ [m.] \
REF: [EDPG]\: 408

### <a id="rehta">\*rehta- "straight" (adj)</a>
E: \
-- Go. _raihts_ \
N: \
-- ON _réttr_ "straight; correct" \
-- Far. _rættur_ "id." \
-- Elfd. _rett_ "id." \
W: \
-- OE _riht_ "id.", E _right_ \
-- OFri. _riucht_ "id.", WFri. _rjocht_ "id.", EFri. _gjucht_ "id." \
-- OS _reht_ "id." \
-- Du. _recht_ \
-- OHG _reht_ "straight, good, right", G _recht_ "id." \
REF: [EDPG]\: 408

### <a id="rekan">\*rekan- [st.] "to rake up, pile up" (v)</a>
E: \
-- Go. _rikan_ \
W: \
-- MHG _rechen_ \
REF: [EDPG]\: 409

### <a id="rekwiz">\*rekwiz- [n.] "darkness" (n)</a>
E: \
-- Go. _riqis_ \
N: \
-- ON _røkkr_ \
REF: [EDPG]\: 409

### <a id="rēma">\*rēma- "hoarse" (adj)</a>
N: \
-- ON _rámr_ \
-- Nw. _råm_ \
REF: [EDPG]\: 409

### <a id="remiz">\*remiz- [n.] "quiet, tranquility" (n)</a>
E: \
-- Go. _rimis_ \
REF: [EDPG]\: 409

### <a id="rēti">\*rēti- "fierce" (adj)</a>
W: \
-- OHG _rāzi_, MHG _ræze_ \
REF: [EDPG]\: 409

### <a id="reuda">\*reuda- "red" (adj)</a>
E: \
-- Go. _ga-riuds_ "honorable" \
N: \
-- ON _rjóðr_ "red (in the face), ruddy" \
W: \
-- OE _rēod_ "red" \
REF: [EDPG]\: 409

### <a id="reudan-1">\*reudan- [st.] "to make red" (v)</a>
N: \
-- ON _rjóða_ \
W: \
-- OE _rēodan_ "to redden, stain with blood" \
REF: [EDPG]\: 410

### <a id="reudan-2">\*reudan- [st.] "to clear, remove, eradicate" (v)</a>
N: \
-- ON _rjóða_ "to clear" \
W: \
-- OE _rēodan_ "to kill" \
-- MHG _rieten_ "to exterminate, wipe out" \
REF: [EDPG]\: 410

### <a id="reufan">\*reufan- [st.] "to break (off)" (v)</a>
N: \
-- ON _rjúfa_ "to break (open)" \
-- Far. poet. _rjúva_ "to tear apart" \
W: \
-- OE _rēofan_ "to break, tear" \
REF: [EDPG]\: 410

### <a id="reukan">\*reukan- ~ \*rūkan- [st.] "to smoke; to drift, fly" (v)</a>
N: \
-- ON _rjúka_ "to smoke, steam; to be driven, be flung, fly" \
-- Far. _rúka_ "to smoke; to fly off; to run, dash" \
W: \
-- OE _rēocan_ "to reek, send forth smoke or steam" \
-- OFri. _riāka_ "to smoke" \
-- MDu. _rieken_, _ruken_ "to smell", Du. _ruiken_ "id." \
-- OHG _riohhan_ "to smoke, steam", G _riechen_ "to smoke" \
REF: [EDPG]\: 410

### <a id="reuman">\*reuman- [m.] "strap, belt" (n)</a>
W: \
-- OE _rēoma_ "belt; ligament" \
-- OS _riomo_ "thong" \
-- Du. _riem_ [c.] "belt" \
-- OHG _riomo_ "id.", G _Riemen_ "id." \
REF: [EDPG]\: 410

### <a id="reuman">\*reuman- ~ \*rauma(n)- [m.] "cream" (n)</a>
N: \
-- ON _rjómi_ \
-- Far. _rómi_ \
W: \
-- OE _rēoma_, _rēama_ "membrane" \
-- WFri. _rjemme_ [c.] "id." \
-- MDu. _rome_, _room_ [m./f.] "id.", Du. _room_ [c.] "id." \
-- OHG _roum_ "id.", G _Rahm_ "id." \
REF: [EDPG]\: 411

### <a id="reupōn">\*reupōn- [f.] "grouse" (n)</a>
N: \
-- ON _rjúpa_ \
-- Far. _rýpa_ (« Da. _rype_?) \
-- Nw. _rjupe_, _rype_ \
-- Elfd. _rjoppa_ \
REF: [EDPG]\: 411

### <a id="reutan">\*reutan- [st.] "to cry" (v)</a>
W: \
-- OE _rēotan_ "to make a noise; to cry" \
REF: [EDPG]\: 411

### <a id="rewan">\*rewan- [st?] "to tear, jerk, fight" (v)</a>
N: \
-- Icel. _rjá_ [wk.] "to wrestle, fight; to move, tear" \
REF: [EDPG]\: 411

### <a id="rīdan">\*rīdan- [st.] "to ride; to move, swing, rock" (v)</a>
N: \
-- ON _ríða_ "to ride" \
W: \
-- OE _rīdan_ "to ride; to move, rock", E _to ride_ \
-- OFri. _rīdan_ "to ride, drive" \
-- OS _rīdan_ "id." \
-- Du. _rijden_ "id." \
-- OHG _ritan_ "id.", G _reiten_ "id." \
REF: [EDPG]\: 412

### <a id="ridra">\*ridra- [n.] "penis" (n)</a>
N: \
-- ON _reðr_, _erðr_ \
-- Icel. _reður_ [m.] \
-- Far. _reður_ \
REF: [EDPG]\: 412

### <a id="rīfan">\*rīfan- ~ \*rīpan- [st.] "to tear" (v)</a>
N: \
-- ON _rífa_ "to tear; to be rent, give way" \
W: \
-- OE _rīpan_ "to reap; to harvest", ME _rīven_ "to tear" \
-- OFri. _rīva_ "id." \
REF: [EDPG]\: 412

### <a id="rīhō">\*rīhō- [f.] "bar, line" (n)</a>
N: \
-- OGutn. _ri_ "pole", Gutn. _räj_ "bar" \
W: \
-- MDu. _rie_ "slat, line", Du. _rij_ [c.] "row" \
-- MHG _rīhe_ "line", G _Reihe_ "line, row" \
REF: [EDPG]\: 412

### <a id="rīk">\*rīk- [m.] "ruler, king" (n)</a>
E: \
-- Go. _reiks_ \
REF: [EDPG]\: 412

### <a id="rikkī">\*rikkī- [f.] "female roe deer" (n)</a>
W: \
-- Du. _rekke_ [c.] \
-- G _Ricke_ \
REF: [EDPG]\: 413

### <a id="rīma">\*rīma- [n.] "number, calculation" (n)</a>
N: \
-- ON _rím_ "computation" \
W: \
-- OE _rīm_ "number" \
-- OHG _rīm_ [m.] "number, calculation" \
REF: [EDPG]\: 413

### <a id="rimi">\*rimi- [f.] "beam" (n)</a>
N: \
-- ON _rim_ "rail; rung" \
-- Far. _rim_ "slat, narrow, board" \
W: \
-- E _rim_ "rung; edge" \
-- Du. dial. _rim_ "edge, rim" \
REF: [EDPG]\: 413

### <a id="rinnan">\*rinnan- [st.] "to run" (v)</a>
E: \
-- Go. _rinnan_ "to run, rush" \
N: \
-- ON _renna_ "to run, flow" \
-- Far. _renna_ "id." \
-- Elfd. _rinna_ (pres. _rið_ < ON \*_riðr_ < \*_rinnr_) "id." \
W: \
-- OE _rinnan_ \
-- OS _rinnan_ "to run, flow" \
-- OHG _rinnan_ "id.", G _rinnen_ "id." \
REF: [EDPG]\: 413

### <a id="rīpi">\*rīpi- "ripe" (adj)</a>
W: \
-- OE _rīpe_, E _ripe_ \
-- OFri. _rīp_ \
-- OS _rīpi_ \
-- Du. _rijp_ \
-- OHG _rīf_, G _reif_ \
REF: [EDPG]\: 414

### <a id="rippōn">\*ripp/bōn- [wk.] "to tear" (v)</a>
N: \
-- Far. _ripa_ "to tear off; to hurl, fling" \
-- Nw. _ripa_ "to tear off" \
W: \
-- MDu. _repen_ "to tear", EDu. _reppen_ "to pluck", Du. _rippen_ "to tear off (violently)" \
-- G Pal. _reffen_ "to reap" \
REF: [EDPG]\: 414

### <a id="rīrēn">\*rīrēn- [wk.] "to tremble" (v)</a>
E: \
-- Go. _reiran_ "id." \
REF: [EDPG]\: 414

### <a id="rīsan">\*rīsan- [st.] "to rise, get up" (v)</a>
E: \
-- Go. _ur-reisan_ \
N: \
-- ON _rísa_ \
-- Far. _rísa_ \
W: \
-- OE _rīsan_, E _to rise_ \
-- OFri. _rīsa_ \
-- Du. _rijzen_ \
-- OHG _rīsan_ "to fall", MHG _rīsen_ "id." \
REF: [EDPG]\: 414

### <a id="rōan">\*rōan- [st.] "to row" (v)</a>
N: \
-- ON _róa_ \
-- Far. _rógva_ [irr.] \
-- Elfd. _ruo_ [wk.] \
W: \
-- OE _rowan_, E _to row_ \
-- MDu. _roen_, _roeyen_ [wk.], Du. _roeien_ [wk.] \
-- MHG _rüejen_ [wk.] \
REF: [EDPG]\: 414

### <a id="rōbjōn">\*rōbjōn- [f.] "turnip" (n)</a>
W: \
-- MDu. _rove_, Du. dial. _reuf_ \
-- OHG _ruoba_, _ruoppa_, G _Rube_ \
REF: [EDPG]\: 415

### <a id="rōbōn">\*rōbōn- [f.] "tail" (n)</a>
N: \
-- ON _rófa_ \
-- Far. top. _rógva_ "narrow projection" \
-- Nw. _rove_ "tail; corner of a field" \
W: \
-- G Alem. _rueb(en)_ "(base of the) tail" \
REF: [EDPG]\: 415

### <a id="rōdjan">\*rōdjan- [wk.] "to speak" (v)</a>
E: \
-- Go. _rodjan_ \
N: \
-- ON _rœða_ "to speak; to converse" \
REF: [EDPG]\: 415

### <a id="rōkjan">\*rōkjan- [wk.] "to heed" (v)</a>
N: \
-- ON _rœkja_ "to heed, take care of" \
W: \
-- OE _rēcan_, _reccan_ (pret. _rōhte_) "id." \
-- OS _rōkian_ "to be careful" \
-- OHG _ruochen_ \
REF: [EDPG]\: 415

### <a id="rōō">\*rōō- [f.] "calm, rest" (n)</a>
N: \
-- ON _ró_ \
-- Far. _rógv_ \
-- Elfd. _ruo_ \
W: \
-- OE _rōw_ \
-- OHG _ruowa_, G _Ruhe_ \
REF: [EDPG]\: 415

### <a id="rōta">\*rōta- "glad" (adj)</a>
W: \
-- OE _rōt_ "glad, cheerful; excellent" \
REF: [EDPG]\: 416

### <a id="rōþra">\*rōþra- [n.] "rudder" (n)</a>
N: \
-- Far. _róður_ \
-- Elfd. _ruoðer_ \
W: \
-- OE _rodor_, E _rudder_ \
-- OFri. _roder_ [m.] \
-- Du. _roer_ \
-- OHG _ruodar_ "oar, rudder", G _Ruder_ "id." \
REF: [EDPG]\: 416

### <a id="ruddōn">\*rud(d)ōn- [wk.] "to clear" (v)</a>
W: \
-- MDu. _roden_ "to clear", Du. _rooien_ "to pull out, clear (of land)" \
-- MHG _roten_ "to clear", G _aus-rotten_ "to exterminate" \
REF: [EDPG]\: 416

### <a id="rudēn">\*rudēn- [wk.] "to redden" (v)</a>
N: \
-- ON _roða_ "to emit a red gleam" \
-- Far. _roða_ "to redden" \
-- Elfd. _roðå_ "to turn red (e.g. of the sky)" \
W: \
-- OHG _rotēn_ "to redden, shine red" \
REF: [EDPG]\: 416

### <a id="rugi">\*rugi- [m.] "rye" (n)</a>
N: \
-- ON _rugr_ \
-- Far. _rugur_ \
-- Elfd. _rug_ \
W: \
-- OE _ryge_, E _rye_ \
-- WPhal. _rů-strô_ "rye straw" \
REF: [EDPG]\: 416

### <a id="rūjan">\*rūjan- [wk.] "to tear" (v)</a>
N: \
-- ON _rýja_ "to pluck the wool off sheep" \
-- Icel. _rýja_ "to shear" \
REF: [EDPG]\: 416

### <a id="rukjan">\*rukjan- [wk.] "to chew the cud, ruminate" (v)</a>
W: \
-- MDu. _ed-ricken_ \
-- MHG _ita-rücken_ \
REF: [EDPG]\: 417

### <a id="rukkan">\*rukk/gan- [m.] "rye" (n)</a>
W: \
-- OFri. _rogga_ \
-- OS _roggo_, _rokko_ \
-- Du. _rogge_ [c.] \
-- OHG _rocko_, G _Roggen_ \
REF: [EDPG]\: 417

### <a id="rukkōn-1">\*rukk/gōn- [wk.] "to move to and fro, to rock" (v)</a>
N: \
-- ON _rugga_ \
-- Icel. _rugga_, _rogga_, _rokka_; _roga_ "to slave, work hard; to jerk" \
-- Far. _rugga_ "to rock" \
W: \
-- OE _roccian_ "id.", E _to rock_ \
-- WFri. _rokke_, _rukke_ "to rock, jerk", EFri. _rukje_ "id." \
-- EDu. _rocken_ "to incite" \
REF: [EDPG]\: 417

### <a id="rukkōn-2">\*rukk/gōn- [wk.] "to smoke; to rush" (v)</a>
N: \
-- Nw. _rogga_ "to speed up, make move" \
W: \
-- G Cimb. _rucken_ "to smoke" \
REF: [EDPG]\: 417

### <a id="rūma">\*rūma- "roomy, spacious" (adj)</a>
E: \
-- Go. _rums_ \
N: \
-- ON _rúmr_ \
-- Far. _rúmur_ \
W: \
-- OE _rūm_ \
-- OFri. _rūm_ \
-- Du. _ruim_ \
-- OHG _rūmi_, MHG _rūm_, _rūme_ \
REF: [EDPG]\: 418

### <a id="runi">\*runi- [m.] "run, stream" (n)</a>
E: \
-- Go. _runs_ "run, course", ur-runs "rising (of the sun), exit, latrine" \
W: \
-- OE _ryne_ "course, running" \
-- OHG _runs_ [m./f.] "flood, current" \
REF: [EDPG]\: 418

### <a id="runnōn">\*runnōn- [wk.] "to run, flow" (v)</a>
W: \
-- MDu. _ronnen_ \
REF: [EDPG]\: 418

### <a id="ruppōn-1">\*rupp/bōn- [wk.] "to tear off" (v)</a>
N: \
-- ON _ruppa_ "to tear loose" \
W: \
-- LG _rubben_ "to scratch, rip" \
-- MDu. _roppen_ "to jerk, tear off" \
-- MHG _ropfen_, _rupfen_ "to rip", G _rupfen_ "to pluck" \
REF: [EDPG]\: 418

### <a id="ruppōn-2">\*rup(p)ōn- [wk.] "to belch" (v)</a>
N: \
-- ON _ropa_ "to belch; to shout (esp. of grouse)" \
-- Far. _ropa_ \
W: \
-- MDu. _roppen_, Du. _roppen_ "to devour" \
REF: [EDPG]\: 418

### <a id="rusila">\*rusila- [m.] "fat" (n)</a>
W: \
-- OE _rysel_ "fat" \
-- OS _ruslos_ [m.pl.] "piece of bacon" \
-- Du. _reuzel_ [c.] "tallow" \
REF: [EDPG]\: 418

### <a id="ruttōn">\*rut(t)ōn- [wk.] "to lament" (v)</a>
N: \
-- Far. _ruta_ "to grumble, complain" \
W: \
-- G Pal. _rotzen_ "to cry" \
REF: [EDPG]\: 418

### <a id="ruzlja">\*ruzlja- "loose (of sand)" (adj)</a>
W: \
-- Du. _rul_ \
REF: [EDPG]\: 419

## S

### <a id="sada">\*sada- "satiated, sated, full" (adj)</a>
E: \
-- Go. _saþs_, _sads_ \
N: \
-- ON _saðr_ \
W: \
-- OE _sæd_, E _sad_ \
-- OS _sad_ \
-- Du. _zat_ \
-- OHG _sat_, G _satt_ \
REF: [EDPG]\: 419

### <a id="sadula">\*sadula- [m.] "saddle" (n)</a>
N: \
-- ON _sǫðull_ \
-- Far. _saðil_ \
-- Elfd. _saðel_ \
W: \
-- OE _sadel_, _sadol_, E _saddle_ \
-- Du. _zadel_ [n.] \
-- OHG _satal_, _satil_, _satul_, G _Sattel_ \
REF: [EDPG]\: 419

### <a id="safjan">\*safjan- [st.] "to perceive" (v)</a>
W: \
-- OS _an-seffian_, _bi-seffian_ "to notice" \
-- MDu. _be-seffen_ "to perceive, notice. taste", _ont-seffen_ "to become aware of", Du. _be-seffen_ [wk.] "to realize" \
-- OHG _int-seffen_ "to realize, notice", MHG _ent-seben_ "to perceive, taste" \
REF: [EDPG]\: 419

### <a id="safan">\*saf/ppan- [m.] "sap; j uice" (n)</a>
N: \
-- ON _safi_ \
-- Far. _sjó-savi_ "smell of the sea" \
-- OSw. _sava_ [f.] "id." \
W: \
-- OE _sæp_ "sap", E _sap_ \
-- MDu. _sap_, Du. _sap_ [n.] (with secondary neuter gender due to collective semantics) \
-- OHG _saf_, _sapf_, G _Saft_ \
REF: [EDPG]\: 420

### <a id="sagja-1">\*sagja- [m.] "man, hero" (n)</a>
N: \
-- ON _seggr_ \
-- Far. poet. _seggur_ \
W: \
-- OE _secg_ \
REF: [EDPG]\: 420

### <a id="sagja-2">\*sagja- [m.] "sedge" (n)</a>
W: \
-- OE _secg_, E _sedge_ \
-- EDu. _segghe_, Du. _zegge_ [c.] \
REF: [EDPG]\: 420

### <a id="sagjan">\*sagjan- [wk.] "to say, recount" (v)</a>
N: \
-- ON _segja_ \
-- Far. _siga_ \
-- Elfd. _saja_ \
W: \
-- OE _secgan_, E _to say_ \
-- OFri. _sedza_, _sidza_, WFri. _sizze_ \
-- OS _seggian_ \
-- Du. _zeggen_ \
-- OHG _sagēn_, G _sagen_ (< \*sagēn-) \
REF: [EDPG]\: 420

### <a id="sagō">\*sagō- [f.] "saw" (n)</a>
N: \
-- ON _sǫg_ \
-- Far. _sag_ \
-- Elfd. _sågå (<_ \*sagōn-) \
W: \
-- OE _sagu_, E _saw_ \
-- OS _saga_ \
-- MDu. _sage_, Du. _zaag_ [c.] \
-- OHG _saga_, _sega_, G _Säge_ (< \*segō-) \
REF: [EDPG]\: 421

### <a id="sagōn">\*sagō(n)- [f.] "recount" (n)</a>
N: \
-- ON _saga_ "story, account, history" \
-- Far. _søga_ "id." \
W: \
-- OE _sagu_ "statement, story" \
-- OFri. _sege_ "statement, testimony" \
-- OHG _saga_ "story, statement" \
REF: [EDPG]\: 421

### <a id="sahaza">\*sahaza- [m.] "sedge" (n)</a>
W: \
-- OS _saher-ahi_ [n.] "sedgy place" \
-- OHG _sahar_ "id.", MHG _saher_ "id." \
REF: [EDPG]\: 421

### <a id="sahsa">\*sahsa- [n.] "knife" (n)</a>
N: \
-- ON _sax_ "short sword; scissors" \
-- Elfd. _saks_ [m.] "scissors" \
W: \
-- OE _seax_ \
-- OFri. _sax_ "knife; short sword, dagger" \
-- OS _sahs_ \
-- OHG _sahs_ \
REF: [EDPG]\: 421

### <a id="saida">\*saida- [m.] "magic, charm" (n)</a>
N: \
-- ON _seiðr_ "charm, spell" \
REF: [EDPG]\: 421

### <a id="saikjan">\*saikjan- [wk.] "to make drip" (v)</a>
W: \
-- MDu. _seiken_ "to piss; to filter", Du. _zeiken_ "to piss" \
-- OHG _seihhen_ "id.", G _seichen_ "id." \
REF: [EDPG]\: 421

### <a id="saila">\*saila/ō- [n./f.] "rope" (n)</a>
N: \
-- ON _seil_ [f.] \
-- Far. _seil_ [f.] "band, cow harness, scarf" \
W: \
-- OE _sāl_ [m./f.] "rope, tether" \
-- OFri. _wind-sēl_ [n.] \
-- OS _sēl_ [n.] \
-- Du. _zeel_ [n.] \
-- G _Seil_ [n.] "rope, noose" \
REF: [EDPG]\: 422

### <a id="saima">\*saima- [m.] "virgin honey" (n)</a>
N: \
-- ON _seimr_ "honeycomb" \
W: \
-- OS _sēm_ "(fresh) honey" \
-- ODu. _seym_ "id.", Du. _zeem_ [c.] "id." \
-- OHG _seim_ "id.", G _Honig-seim_ "id." \
REF: [EDPG]\: 422

### <a id="saipwōn">\*saipwōn- [f.] "soap" (n)</a>
W: \
-- OE _sāpe_, E _soap_ \
-- OFri. _sēpe_ \
-- Du. _zeep_ [c.] \
-- OHG _seifaf_, _seipfa_, G _Seife_, Swab., Alem. _seipfe_ \
REF: [EDPG]\: 422

### <a id="saiwalō">\*saiwalō- [f.] "soul" (n)</a>
E: \
-- Go. _saiwala_ \
W: \
-- OE _sāwol_, _sāwl_ [m.], E _soul_ \
-- OFri. _sēle_ \
-- OS _seola_, _siola_ \
-- OHG _sēula_, _sēla_, G _Seele_ \
REF: [EDPG]\: 423

### <a id="saiwi">\*saiwi- [m.] "sea" (n)</a>
E: \
-- Go. _saiws_ "lake, swamp" \
N: \
-- ON _sær_, gen. _sjóar_ \
-- Icel. _sjór_ \
-- Far. _sjógvur_ \
-- Elfd. _sju_ \
W: \
-- OE _sǣ_, E _sea_ \
-- OFri. _sē_ \
-- OS _sēu_, _sēo_ \
-- Du. _zee_ [c.] \
-- OHG _sē(o/u)_, gen. _sēwes_, G _See_ [m.] "lake", [f.] "sea" \
REF: [EDPG]\: 423

### <a id="sakan">\*sakan- [st.] "to charge" (v)</a>
E: \
-- Go. _sakan_ "to dispute, reprimand" \
W: \
-- OS _sakan_ "to rebuke" \
-- MDu. _mis-saken_ [wk./st.] "to deny, reject" \
-- OHG _sahhan_ "to argue, reprimand" \
REF: [EDPG]\: 423

### <a id="sakkōn">\*sakk/gōn- [wk.] "to drop, sag" (v)</a>
W: \
-- ME _saggen_, E _to sag_ \
-- Du. _zakken_ \
-- G _sacken_ \
REF: [EDPG]\: 423

### <a id="sakō">\*sakō- [f.] "case, lawsuit" (n)</a>
N: \
-- ON _sǫk_ "lawsuit, accusation, trial" \
W: \
-- OE _sacu_ "strife, contention, dispute", E _sake_ \
-- OFri. _seke_ "case" \
-- OS _saka_ "affair, lawsuit" \
-- Du. _zaak_ [c.] "affair, lawsuit" \
-- OHG _sahha_ "case, guilt", G _Sache_ "affair, matter" \
REF: [EDPG]\: 424

### <a id="salbō">\*salbō- [f.] "ointment" (n)</a>
E: \
-- Go. _salba_ \
W: \
-- OE _sealf(e)_, E _salve_ \
-- OS _salba_ \
-- Du. _zalf_ [c.] \
-- OHG _salba_, G _Salbe_ \
REF: [EDPG]\: 424

### <a id="salihōn">\*salihōn- [f.] "sallow, willow" (n)</a>
N: \
-- ON _selja_ \
-- Sw. _sälg_ [c.] \
W: \
-- OE _sealh_, E _sallow_ \
-- OHG _salaha_, G _Sal-weide_ \
REF: [EDPG]\: 424

### <a id="saliz">\*saliz- [n.] "hall, house" (n)</a>
N: \
-- ON _salr_ [m.] \
W: \
-- OE _sæl_, _salor_ "hall" \
-- OS _seli_ [m.] "hall, building" \
-- Du. _zaal_ [c.] "hall" \
-- OHG _sal_ "hall, room", G _Saal_ [m.] "id." \
REF: [EDPG]\: 424

### <a id="saljan">\*saljan- [wk.] "to offer, sell" (v)</a>
E: \
-- Go. _saljan_ "to lodge" \
N: \
-- ON _selja_ "to hand over; to sell" \
-- Far. _selja_ "to sell; to let go of; to surrender" \
-- Elfd. _sela_ "to sell" \
W: \
-- OE _sellan_ [st.] "to give", E _to sell_ \
-- OFri. _sella_ "to hand over; to pay; to sell" \
-- OS _gi-sellian_ "give, sell" \
-- OHG _sellan_ "id." \
REF: [EDPG]\: 424

### <a id="salta">\*salta- [n.] "salt" (n)</a>
E: \
-- Go. _salt_ \
N: \
-- ON _salt_ \
-- Far. _salt_ \
-- Elfd. _salt_ \
W: \
-- OE _sealt_, E _salt_ \
-- OFri. _salt_ \
-- OS _salt_ \
-- Du. _zout_ \
-- OHG _salz_, G _Salz_ \
REF: [EDPG]\: 425

### <a id="saltan">\*saltan- [st.] "to salt (down), pickle" (v)</a>
E: \
-- Go. _saltan_ \
N: \
-- ON _salta_ [wk.] \
-- Elfd. _solta_ [wk.] \
W: \
-- OE _sealten_ [ptc.] \
-- MDu. _souten_ [st./wk.], Du. _ge-zouten_ "pickled" \
-- OHG _salzan_ "to pickle", G _salzen_ [st./wk.] "id." \
REF: [EDPG]\: 425

### <a id="salwa">\*salwa- "yellowish brown" (adj)</a>
N: \
-- ON _sǫlr_ "yellow, pale" \
W: \
-- OE _salo_ "dusky, dark", E _sallow_ \
-- MDu. _salu_, _saluwe_ "pale yellow, dirty" \
-- OHG _salo_ "dark, black, dirty" \
REF: [EDPG]\: 425

### <a id="saman">\*sama(n)- "the same" (adj)</a>
E: \
-- Go. _sama_ \
N: \
-- ON _samr_, _sami_ \
-- Far. _samur_, _sami_ \
W: \
-- OHG _samo_ \
REF: [EDPG]\: 425

### <a id="samda">\*samda- [m.] "sand" (n)</a>
N: \
-- ON _sandr_ \
-- Far. _sandur_ \
-- Elfd. _sand_ \
W: \
-- OE _sand_, E _sand_ \
-- OFri. _san(d)_, _saun(d)_ \
-- OS _sand_ \
-- Du. _zand_ [n.] \
-- OHG _sant_ [m./n.], G _Sand_ \
REF: [EDPG]\: 425

### <a id="samþu">\*samþu- "soft" (adj)</a>
W: \
-- OE _sēfte_, _sōfte_ (adv), E _soft_ \
-- OS _sāfto_ (adv) \
-- Du. _zacht_ \
-- OHG _samfti_, _semfti_, _samfto_ (adv), G _sanft_ \
REF: [EDPG]\: 426

### <a id="sandjan">\*sandjan- [wk.] "to cause to go, send" (v)</a>
E: \
-- Go. _sandjan_ \
N: \
-- ON _senda_ \
-- Far. _senda_ \
W: \
-- OE _sendan_, E _to send_ \
-- OFri. _senda_ \
-- OS _sendian_ \
-- Du. _zenden_ \
-- OHG _sentan_, G _senden_ \
REF: [EDPG]\: 426

### <a id="sangʷa">\*sangʷa- [m.] "singing; song" (n)</a>
E: \
-- Go. _saggws_ \
N: \
-- ON _sǫngr_ \
-- Far. _songur_ \
-- Elfd. _saungg_ \
W: \
-- OE _sang_, E _song_ \
-- OFri. _song_ \
-- OS _sang_ \
-- Du. _zang_ [c.] \
-- OHG _sang_ [m./n.], G _Sang_ \
REF: [EDPG]\: 426

### <a id="sankwjan">\*sankwjan- [wk.] "to cause to sink" (v)</a>
E: \
-- Go. _sagqjan_ \
N: \
-- ON _søkkva_ \
-- Far. _søkkja_ \
-- Elfd. _saintja_ \
W: \
-- OE _sencan_ "to sink, plunge, immerse", E _to sink_ \
-- OS _bi-senkian_ \
-- OHG _senkan_, G _senken_ \
REF: [EDPG]\: 426

### <a id="sanþa">\*sanþa- "true" (adj)</a>
N: \
-- ON _saðr_, _sannr_ "true; meet and proper; guilty" \
-- Far. _sannur_ "true; right" \
-- Elfd. _sann_ "id." \
W: \
-- OE _sōð_ "id." \
-- OS _sōth_ "id." \
-- OHG _sand_ "id." \
REF: [EDPG]\: 427

### <a id="sarwa">\*sarwa- [m.] "armor, equipment" (n)</a>
E: \
-- Go. _sarwa_ [n.] \
W: \
-- OE _searu_, _searo_ [n.] "device, design, art" \
-- OHG _saro_ [n.] "armor" \
REF: [EDPG]\: 427

### <a id="satjan">\*satjan- [wk.] "to put" (v)</a>
E: \
-- Go. _satjan_ \
N: \
-- ON _setja_ \
-- Far. _seta_ \
-- Elfd. _settja_ [st.] \
W: \
-- OE _settan_, E _to set_ \
-- OFri. _setta_ \
-- OS _settian_ \
-- Du. _zetten_ \
-- OHG _sezzen_, G _setzen_ \
REF: [EDPG]\: 427

### <a id="sauma">\*sauma- [m.] "seam" (n)</a>
N: \
-- ON _saumr_ \
-- Far. _seymur_ \
-- Elfd. _sōm_ \
W: \
-- OE _sēam_, E _seam_ \
-- OFri. _sām_ \
-- MDu. _soom_, Du. _zoom_ [c.] \
-- OHG _soum_, G _Saum_ \
REF: [EDPG]\: 427

### <a id="sauþa">\*sauþa- [m.] "broth" (n)</a>
N: \
-- Nw. _saud_ "cooking" \
W: \
-- OE _sēað_ "well" \
-- OFri. _sāth_ "id." \
-- MDu. _soot_ "well; boiling water" \
-- MHG _sōt_ "boiling, seething; well; heartburn" \
REF: [EDPG]\: 428

### <a id="sauza">\*sauza- "dry" (adj)</a>
N: \
-- Nw. _søyr_ \
W: \
-- OE _sēar_, E _sear_ \
-- MDu. _soor_, Du. _zoor_ \
REF: [EDPG]\: 428

### <a id="sawwa">\*sawwa- [n.] "juice" (n)</a>
N: \
-- Icel. _söggur_ [m.] "moisture" \
W: \
-- OE _sēaw_ [m.] "juice, moisture", ME _seu_ "sap, juice, broth" \
-- OHG _sou_ "juice" \
REF: [EDPG]\: 428

### <a id="sēan">\*sēan- [st.] "to sow" (v)</a>
E: \
-- Go. _saian_ \
N: \
-- ON _sá_, pret. _sera_ \
-- Far. _sáa_ [wk.] \
W: \
-- OE _sāwan_, E _to sow_ \
-- OFri. _e-sen_ [ptc.] \
-- OS _sāian_ [wk.] \
-- Du. _zaaien_ [wk.] \
-- OHG _sāen_ [wk.], G _säen_ [wk.] \
REF: [EDPG]\: 428

### <a id="sebjō">\*sebjō- [f.] "kinship" (n)</a>
E: \
-- Go. _sibja_ \
N: \
-- ON _sif_ \
W: \
-- OE _sib_ \
-- OFri. _sibbe_ \
-- OS _sibbia_ \
-- OHG _sippa_, G _Sippe_ \
REF: [EDPG]\: 429

### <a id="sebun">\*sebun- "seven" (num)</a>
E: \
-- Go. _sibun_ \
N: \
-- ON _sjau_ \
-- Far. _sjey_ \
-- Elfd. _sju_ \
W: \
-- OE _seofon_, _syfon_, E _seven_ \
-- OFri. _sigun_ \
-- OS _siƀun_, _siƀon_ \
-- Du. _zeven_ \
-- OHG _sibun_, G _sieben_ \
REF: [EDPG]\: 429

### <a id="sebunþan">\*sebunþ/dan- "seventh" (num)</a>
E: \
-- Go. _sibunda_ \
N: \
-- ON _sj(a)undi_ \
-- Far. _sjeyndi_ \
-- Elfd. _sjuund_ \
W: \
-- OE _seofoða_, E _seventh_ \
-- OFri. _sigunda_ (with _g_ after _nigunda_ "ninth") \
-- OS _siƀondo_, _siƀotho_ \
-- Du. _zevende_ \
-- OHG _sibunto_, _sibanto_, G _siebente_ \
REF: [EDPG]\: 429

### <a id="sēda">\*sēda- [n.] "seed" (n)</a>
N: \
-- ON _sáð_ \
-- Far. _sáð_ \
-- Elfd. _såðer_ [m.pl.] "bran" \
W: \
-- OE _sǣd_ "id.", E _seed_ \
-- OFri. _sēd_ "id." \
-- OS _sād_ "id." \
-- Du. _zaad_ "id." \
REF: [EDPG]\: 429

### <a id="sedi">\*sedi- [f.] "seed" (n)</a>
E: \
-- Go. _mana-seþs_ "mankind" \
W: \
-- OHG _sāt_ "seed; field", G _Saat_ "seed; sowing" \
REF: [EDPG]\: 430

### <a id="sēdla">\*sēdla- [n.] "sieve, riddle" (n)</a>
N: \
-- ON _sáld_ \
-- Elfd. _såld(er)_ [m.] \
REF: [EDPG]\: 430

### <a id="sefan">\*sefan- [m.] "mind" (n)</a>
N: \
-- ON _sefi_ "mind, affection" \
W: \
-- OE _sefa_ "understanding, mind, heart" \
-- OS _mōd-seƀa_ "spirit, soul, heart" \
REF: [EDPG]\: 430

### <a id="segiz">\*segiz- [n.] "victory" (n)</a>
E: \
-- Go. _sigis_ \
N: \
-- ON _sigr_ [m.] \
-- Far. _sigur_ [m.] \
-- OSw. _sigher_ [m.] \
-- Sw. _seger_ [c.] \
W: \
-- OE _sigor_ [m.] \
-- OFri. _sige_, _sīge_ [m.] \
-- OS _sigi-drohtin_ "God" \
-- Du. _zege_ [c.] \
-- OHG _sigi_, _sigu_ [m.], G _Sieg_ [m.] \
REF: [EDPG]\: 430

### <a id="segja">\*segja- [n.] "hard skin" (n)</a>
N: \
-- ON _sigg_ "hard skin" \
-- Far. _sigg_ "hard, cartilaginous whale blubber" \
REF: [EDPG]\: 430

### <a id="segla">\*segla- [n.] "sail, canvas" (n)</a>
N: \
-- ON _segl_ \
-- Far. _segl_ \
-- Elfd. _sigel_ \
W: \
-- OE _seg(e)l_ [m.], E _sail_ \
-- OS _segal_ (also cf. _segela_ [f.] "curtain" < \*seglō-) \
-- Du. _zeil_ \
-- OHG _segal_, _segil_ [m.], G _Segel_ \
REF: [EDPG]\: 430

### <a id="sehs">\*sehs "six" (num)</a>
E: \
-- Go. _saihs_ \
N: \
-- ON _sex_ \
-- Far. _seks_ \
-- Elfd. _sjäks_ \
W: \
-- OE _siex_, _six_, E _six_ \
-- OFri. _sex_ \
-- OS _sehs_ \
-- Du. _zes_ \
-- OHG _sehs_, G _sechs_ \
REF: [EDPG]\: 431

### <a id="sehtan">\*sehtan- "sixth" (num)</a>
E: \
-- Go. _saihsta_ \
N: \
-- ON _sétti_ \
-- Far. _sætti_ \
-- Elfd. _sjäkst_ \
W: \
-- OE _siexta_, E _sixth_ \
-- OFri. _sexta_ \
-- OS _sehsto_ \
-- Du. _zesde_ \
-- OHG _sehto_, _sehsto_, G _sechste_ \
REF: [EDPG]\: 431

### <a id="sehwan">\*sehwan- [st.] "to see" (v)</a>
E: \
-- Go. _saiƕan_ \
N: \
-- ON _sjá_ \
-- Far. _síggja_ \
-- Elfd. _sją̊_ \
W: \
-- OE _sēon_, E _to see_ \
-- OFri. _siā_, _siān_, _siēn_, WFri. _sjen_ \
-- OS _sehan_ \
-- Du. _zien_ \
-- OHG _sehan_, G _sehen_ \
REF: [EDPG]\: 431

### <a id="sēhwi">\*sēhwi- "visible" (adj)</a>
N: \
-- ON _-sær_ \
REF: [EDPG]\: 432

### <a id="selban">\*selba(n)- "self" (pron)</a>
E: \
-- Go. _silba_ \
N: \
-- ON _sjalfr_ \
-- Far. _sjálvur_ \
-- Elfd. _siuov_ \
W: \
-- OE _self_, _seolf_, E _self_ \
-- OFri. _self_ \
-- OS _self_ \
-- Du. _zelf_ \
-- OHG _selb_, G _selber_, _selbst_ \
REF: [EDPG]\: 432

### <a id="selha">\*selha- [m.] "seal" (n)</a>
N: \
-- ON _selr_ \
-- Far. _selur_ \
-- OSw. _siæl_ \
-- Sw. _själ_ [c.] \
W: \
-- OE _seolh_, E _seal_ \
-- OS _selah_ \
-- EDu. _seel_, _seel-hont_, Du. _zee-hond_ [c.] \
-- OHG _selah_, MHG _sele_ \
REF: [EDPG]\: 432

### <a id="sēman">\*sēman- [m.] "seed" (n)</a>
W: \
-- OHG _samo_ "seed, offspring", G _Samen_ "seed" \
REF: [EDPG]\: 432

### <a id="semeþa">\*semeþa/ō- [n./f.] "rush" (n)</a>
W: \
-- OS _semith_ [n.], MLG _sem(e)de_ [f.] \
-- OHG _semida_ [f.], MHG _semede_, _sende_, _semt_, _sebede_ [n./f.] \
REF: [EDPG]\: 432

### <a id="senīga">\*seniga- "senior" (adj)</a>
E: \
-- Go. _sineigs_, superl. _sinista_ \
REF: [EDPG]\: 433

### <a id="senuwō">\*senuwō- [f.] "sinew" (n)</a>
N: \
-- ON _sin_ \
-- Far. _sina_ "sinew, tendon; penis" \
-- Elfd. _sina_ "tendon" \
W: \
-- OE _seonu_, _sionu_, _sinu_ "id.", E _sinew_ \
-- OFri. _sine_, _sini_ "id." \
-- OS _senewa_, _sinewa_ "id." \
-- Du. _zenuw_ [c.] "nerve" \
-- OHG _senawa_, _sena_ "tendon", G _Sehne_ "id." \
REF: [EDPG]\: 433

### <a id="serþan">\*serþan- ~ \*serdan- [st.] "to hit; to violate sexually" (v)</a>
N: \
-- ON _serða_, _streða_ \
W: \
-- OE _seorðan_ \
-- MLG _serden_ \
-- OHG _sertan_, MHG _serten_ "to have sex; to bother, vex; to hit; to entice" \
REF: [EDPG]\: 433

### <a id="sessa">\*sessa- [m.] "seat, bench" (n)</a>
N: \
-- ON _sess_ "seat (in a row boat)" \
-- Far. _sessur_ "id." \
W: \
-- OE _sess_ \
REF: [EDPG]\: 433

### <a id="sēti">\*sēti- "possible to sit on" (adj)</a>
E: \
-- Go. _anda-sets_ "odious" \
N: \
-- ON _sætr_ "suitable for sitting on" \
W: \
-- OE _and-sǣte_ "odious" \
REF: [EDPG]\: 433

### <a id="setjan">\*set(j)an- [st.] "to sit" (v)</a>
E: \
-- Go. _sitan_ \
N: \
-- ON _sitja_ \
-- Far. _sita_ \
-- Elfd. _sittja_ \
W: \
-- OE _sittan_, E _to sit_ \
-- OFri. _sitta_ \
-- OS _sittian_ \
-- Du. _zitten_ \
-- OHG _sizzen_, G _sitzen_ \
REF: [EDPG]\: 434

### <a id="setla">\*setla- [m.] "seat" (n)</a>
E: \
-- Go. _sitls_ \
W: \
-- OE _setl_ [m./n.], ME _setel_ \
-- Du. _zetel_ [c.] \
-- OHG _sezzal_, G _Sessel_ \
REF: [EDPG]\: 434

### <a id="seuda">\*seuda- [m.] "purse, money-bag" (n)</a>
N: \
-- ON _sjóðr_ \
W: \
-- OE _sēod_ \
REF: [EDPG]\: 434

### <a id="seukan">\*seukan- [st.] "to be ill" (v)</a>
E: \
-- Go. _siukan_ \
REF: [EDPG]\: 434

### <a id="seuni">\*seuni- [f.] "appearance, face" (n)</a>
E: \
-- Go. _siuns_ "sight, appearance, face" \
N: \
-- ON _sjón_ "sight, eyesight; look" \
-- Far. _sjón_ "id." \
W: \
-- OE _sīen_ "sight, vision" \
-- OFri. _siune_ "face, sight, appearance", NFri. F sjiin "apparition" \
-- OS _siun_ "sight" \
-- MDu. _sune_ "face, vision" \
REF: [EDPG]\: 434

### <a id="seuþan">\*seuþan- [st.] "to boil, seethe" (v)</a>
N: \
-- ON _sjóða_ \
-- Far. _sjóða_ [st./wk.] \
-- Elfd. _siuoða_ \
W: \
-- OE _sēoðan_, E _to seethe_ \
-- OFri. _siātha_ \
-- MDu. _sieden_, Du. _zieden_ [wk.?] \
-- OHG _siodan_, G _sieden_ \
REF: [EDPG]\: 435

### <a id="sīda">\*sīda- "long" (adj)</a>
N: \
-- ON _síðr_ "long (e.g. of hair), hanging down" \
W: \
-- OE _sīd_ "wide, broad, ample" \
-- OFri. _sīde_ "low" \
-- MDu. _side_, _sijt_ "broad, wide", Du. _zijd_ "id." \
-- OHG _sīto_ "loose" (adv) \
REF: [EDPG]\: 435

### <a id="sidu">\*sidu- [m.] "custom" (n)</a>
E: \
-- Go. _sidus_ \
N: \
-- ON _siðr_ \
-- Far. _siður_ \
-- Elfd. _sið_ \
W: \
-- OE _sidu_, _seodu_ \
-- OFri. _side_ [m./f.] \
-- OS _sidu_ \
-- Du. _zede_ [c.] \
-- OHG _situ_, G _Sitte_ \
REF: [EDPG]\: 435

### <a id="sīhwan">\*sīhwan- [st.] "to percolate" (v)</a>
N: \
-- ON _sía_ [wk.] "to filter, strain; to sip through the beard" \
W: \
-- OE _sēon_ "to strain, filter; to ooze, trickle" \
-- MDu. _sien_ "to filter; to pour" \
-- OHG _sīhan_ "to filter, strain", G _seihen_ [wk.] "id." \
REF: [EDPG]\: 435

### <a id="sikkōn">\*sikkōn- [wk.] "to trickle" (v)</a>
N: \
-- Nw. _sikka_ "to lower oneself" \
W: \
-- OE _sicerian_ "id." \
-- G Pal. _sickern_ "id.", Rhnl. _sicken_ "to piss" \
REF: [EDPG]\: 436

### <a id="sīled">\*sī̆led- [f.] "herring" (n)</a>
N: \
-- ON _síld_, pl. síldr \
-- Far. _síld_ \
-- Elfd. _sild_ \
W: \
-- Du. _zeelt_ [c.] "tench, doctor fish" \
REF: [EDPG]\: 436

### <a id="silubra">\*silubra- [n.] "silver" (n)</a>
E: \
-- Go. _silubr_ \
N: \
-- ON _silfr_ \
-- Far. _silvur_ \
W: \
-- OE _seolfor_, _siolufr_, _sylfor_, E _silver_ \
-- OFri. _sel(o)ver_, _silver_, _sulver_ \
-- OS _siluƀar_ \
-- Du. _zilver_ \
-- OHG _silabar_, G _Silber_ \
REF: [EDPG]\: 436

### <a id="sīna">\*sīna- "own" (pron)</a>
E: \
-- Go. _seins_ \
N: \
-- ON _sinn_ \
-- Far. _sín_ \
-- Elfd. _saį_ \
W: \
-- OE _sīn_ \
-- OFri. _sīn_ \
-- OS _sīn_ \
-- Du. _zijn_ \
-- OHG _sīn_, G _sein_ \
REF: [EDPG]\: 436

### <a id="singʷan">\*singʷan- [st.] "to sing" (v)</a>
E: \
-- Go. _siggwan_ \
N: \
-- ON _syngva_, _syngja_ \
-- Far. _syngja_ \
-- OSw. _siunga_ \
-- Elfd. _singga_ \
W: \
-- OE _singan_, E _to sing_ \
-- OFri. _siunga_, _sionga_, WFri. _sjonge_ \
-- OS _singan_ \
-- ODu. _singon_, Du. _zingen_ \
-- OHG _singan_, G _singen_ \
REF: [EDPG]\: 437

### <a id="sinkwan">\*sinkwan- [st.] "to sink" (v)</a>
E: \
-- Go. _sigqan_ \
N: \
-- ON _søkkva_ \
-- Far. _søkka_ \
-- OSw. _siunka_ \
-- Elfd. _sikka_ \
W: \
-- OE _sincan_, E _to sink_ \
-- OS _sinkan_ \
-- Du. _zinken_ \
-- OHG _sincan_, G _sinken_ \
REF: [EDPG]\: 437

### <a id="sinnan">\*sinnan- [st.] "to head for; to long for" (v)</a>
W: \
-- MDu. _sinnen_ [wk.] "to think, contemplate", Du. _zinnen_ "to consider" \
-- OHG _sinnan_ "id.", G _sinnen_ "to contemplate" \
REF: [EDPG]\: 437

### <a id="sīþu">\*sīþu- "late" (adj)</a>
E: \
-- Go. _seiþus_ \
N: \
-- ON _síðr_ (comp) \
-- Far. _síður_ (comp) \
W: \
-- MDu. _side_ \
-- OHG _sīd_ (comp) (< \*siþ-iz) \
REF: [EDPG]\: 437

### <a id="siujan">\*siujan- [wk.] "to sew" (v)</a>
E: \
-- Go. _siujan_ \
N: \
-- ON _sýja_ \
W: \
-- OE _seowian_, _siwian_, E _to sew_ \
-- OFri. _siā_ \
-- MLG _sūwen_ \
-- OHG _siuwen_ \
REF: [EDPG]\: 437

### <a id="skaban">\*skaban- [st.] "to shave, scrape" (v)</a>
E: \
-- Go. _skaban_ \
N: \
-- ON _skafa_ \
-- Far. _skava_ [wk.] \
-- Elfd. _skåvå_ \
W: \
-- OE _scafan_ "to shave", E _to shave_ \
-- MDu. _schaven_ [st./wk.] "id.", Du. _schaven_ [wk.] "to scrape" \
-- OHG _scaban_ "id.", G _schaben_ [wk.] "id." \
REF: [EDPG]\: 438

### <a id="skadu">\*skadu- [m.] "shadow" (n)</a>
E: \
-- Go. _skadus_ \
W: \
-- OE _sceadu_ [f.], E _shadow_, _shade_ \
-- OS _skado_ \
-- Du. _schaduw_ [c.] \
-- OHG _scato_, G _Schatten_ \
REF: [EDPG]\: 438

### <a id="skaiwarō">\*skaiwarō- [f.] "magpie" (n)</a>
N: \
-- ON _skjór_ \
-- Icel. _skjór_ [m.] \
-- Far. _skjóra_ \
-- Nw. _skjor_, _skjære_ \
-- Elfd. _stjier_ \
REF: [EDPG]\: 438

### <a id="skakan">\*skakan- [st.] "to shake; to leave" (v)</a>
N: \
-- ON _skaka_ "to shake" \
-- Nw. _skaka_ "id." \
W: \
-- OE _scacan_ "to shake, shiver; to run, leave, flee", E _to shake_ \
-- OFri. _skeka_ "to run away from home" \
-- OS _skakan_ "to leave" \
-- MDu. _schaken_ [st./wk.] "to leave, escape, run away with someone (a woman)" \
-- OHG _unt-scahhōn_ [wk.] "to be driven" \
REF: [EDPG]\: 438

### <a id="skakkōn">\*skakk/gōn- [wk.] "to shake" (v)</a>
N: \
-- Far. _skaka_ \
-- Elfd. _skåkå_ "to shake, shiver" \
W: \
-- ME _shaggen_ "to shake, cast off, tremble", E _to shag_ "to toss, shake, waggle" \
-- LG _skaken_ "to bump, shake" \
-- MHG _schacken_ "id." \
REF: [EDPG]\: 439

### <a id="skalda">\*skalda- [n.] "skald, poet" (n)</a>
N: \
-- ON _skáld_ \
REF: [EDPG]\: 439

### <a id="skalka">\*skalka- [m.] "servant" (n)</a>
E: \
-- Go. _skalks_ \
W: \
-- OE _scealc_ \
-- OFri. _skalk_ "servant; villain" \
-- OS _skalk_ "servant; disciple" \
-- Du. _schalk_ [c.] "mischief" \
-- OHG _scale_ "servant, slave", G _Schalk_ "rogue, mischief" \
REF: [EDPG]\: 439

### <a id="skamō">\*skamō- [f.] "shame" (n)</a>
W: \
-- OE _scamu_, E _shame_ \
-- OFri. _skome_ "shame; genitals" \
-- OS _skama_ "id." \
-- MDu. _schame_ "id." \
-- OHG _scama_ "id.", G _Scham_ "id." \
REF: [EDPG]\: 439

### <a id="skapjan">\*skap(j)an- [st.] "to form, create" (v)</a>
E: \
-- Go. _ga-skapjan\*_ \
N: \
-- ON _skapa_, skepja [wk.] "to shape, form, mould" \
-- Far. _skapa_ [wk./st.] "to shape, mould; to cause, bring about" \
W: \
-- OE _scieppan_, _sceppan_ \
-- OFri. _skeppa_ "to form, make" \
-- OS _skeppian\*_ "id." \
-- Du. _scheppen_ "id." \
-- OHG _scepfen_ [st./wk.] "id.", G _schöpfen_ [wk.] "id." \
REF: [EDPG]\: 440

### <a id="skappōn">\*skapp/bōn- [wk.] "to shave, scrape" (v)</a>
N: \
-- Nw. _skabba_ \
W: \
-- MDu. _schabben_ "to scratch" \
-- G Rhnl. _schappen_ "id." \
REF: [EDPG]\: 440

### <a id="skarda">\*skarda- "cut up" (adj)</a>
N: \
-- ON _skarðr_ "damaged" \
W: \
-- OS _skard_ "cut; hurt" \
-- OHG _scart_ "id.", MHG _schart_ "id." \
REF: [EDPG]\: 440

### <a id="skarna">\*skarna- [n.] "dung, muck" (n)</a>
N: \
-- ON _skarn_ \
-- Far. _skarn_ \
W: \
-- OE _scearn_, E _sharn_ \
-- OFri. _skern_ [m./n.], WFri. _skern_ [c.] \
-- MDu. _scharn_ \
REF: [EDPG]\: 440

### <a id="skarpa">\*skarpa- "sharp" (adj)</a>
N: \
-- ON _skarpr_ "sharp, bitter, desiccated" \
-- Far. _skarpur_ "id." \
-- Elfd. _skarp_ "hard" \
W: \
-- OE _scearp_, E _sharp_ \
-- OFri. _skerp_ \
-- OS _skarp_ \
-- Du. _scherp_ \
-- OHG _scarp_, G _scharf_ \
REF: [EDPG]\: 440

### <a id="skatta">\*skatta- [m.] "wealth, property, cattle" (n)</a>
E: \
-- Go. _skatts_ "coin" \
N: \
-- ON _skattr_ "tribute, tax; treasure" \
W: \
-- OE _sceat_ "property, goods, wealth" \
-- OFri. _sket_ [m./n.] "goods, property, cattle" \
-- OS _skat_ "treasure, money, possession" \
-- Du. _schat_ [c.] "treasure, wealth" \
-- OHG _skaz_ "treasure, money, wealth, tax", G _Schatz_ "treasure" \
REF: [EDPG]\: 441

### <a id="skaþjan">\*skaþjan- [st.] "to harm, do wrong" (v)</a>
E: \
-- Go. _skaþjan_ \
N: \
-- ON _skeðja_ [wk.] \
W: \
-- OE _sceþþan_ [st./wk.] \
REF: [EDPG]\: 441

### <a id="skauni">\*skauni- "beautiful" (adj)</a>
E: \
-- Go. _skauns_, _skauneis_ \
W: \
-- OE _scīene_, E _sheen_ \
-- OFri. _skēne_ \
-- OS _skōni_ \
-- Du. _schoon_ "beautiful; clean" \
-- OHG _scōni_, G _schön_ \
REF: [EDPG]\: 441

### <a id="skauta">\*skauta- [m.] "seam" (n)</a>
E: \
-- Go. _skaut(s)_ [m./n.] "projection, edge, fringe" \
N: \
-- ON _skaut_ [n.] "corner; flap; sheet" \
-- Far. _skeyt_ [n.] "sheet; lap, bosom" \
W: \
-- OE _scēat_ "corner, angle; projection, promontory" \
-- OFri. _skāt_ "lap; coat-tail" \
-- Du. _schoot_ [c.] "lap" \
-- OHG _scōz_ "tip, corner, shirt-tail", G _Schoss_ "lap; shirt-tail" \
REF: [EDPG]\: 441

### <a id="skawa">\*skawa- "observant(?)" (adj)</a>
E: \
-- Go. _us-skaws_ "sober" \
REF: [EDPG]\: 442

### <a id="skehan">\*skehan- [st.] "to move quickly; to happen" (v)</a>
W: \
-- OE _scēon_ [wk.] "to go quickly, fly" \
-- OFri. _skiā(n)_ "to happen" \
-- MDu. _scien_, _ge-scien_ [st./wk.] "to happen", Du. _ge-schieden_ [wk.] "id." \
-- OHG _gi-scehan_, _scehan_ "id.", MHG _schehen_ "to move quickly; to happen", G _ge-schehen_ "to happen" \
REF: [EDPG]\: 442

### <a id="skekkjan">\*skekkjan- [wk.] "to make move" (v)</a>
W: \
-- MDu. _schicken_ "to cause to happen, order", Du. _schikken_ "to order" \
-- MHG _schicken_ "to start, prepare, order, send", G _schicken_ "to send" \
REF: [EDPG]\: 442

### <a id="skeldan">\*skeldan- [st.] "to announce; to reproach" (v)</a>
W: \
-- ME _scolden_ [wk.], E _to scold_ \
-- OFri. _skelda_ \
-- ODu. _sceldan_, Du. _schelden_ \
-- OHG _sceltan_, G _schelten_ \
REF: [EDPG]\: 442

### <a id="skeldu">\*skeldu- [m.] "shield" (n)</a>
E: \
-- Go. _skildus_ \
N: \
-- ON _skjǫld_ \
-- Far. _skjøldur_ \
-- Elfd. _stjöld_ \
W: \
-- OE _scield_, E _shield_ \
-- OFri. _skeld_, _skiold_, _skiuld_ \
-- OS _skeld_, _skild_ \
-- Du. _schild_ [n.] \
-- OHG _scilt_, G _Schild_ \
REF: [EDPG]\: 442

### <a id="skelha">\*skelha- ~ \*skelga- "wry, oblique" (adj)</a>
N: \
-- ON _skjálgr_ "wry, oblique; squinting" \
-- Icel. _skjálgur_ "cross-eyed" \
-- Far. _skjálgur_ "wry, askewed, warped" \
-- Elfd. _stjål_ "wry, warped; exhausted" \
W: \
-- OE _sceolh_, _sceol_ "oblique, wry" \
-- OFri. _skilich_ "cross-eyed" \
-- MDu. _scheel_ "looking askance", Du. _scheel_ "cross-eyed" \
-- OHG _scelah_ "wry, bent", G _scheel_ "askew" \
REF: [EDPG]\: 442

### <a id="skeljan">\*skeljan- [wk.] "to separate, distinguish" (v)</a>
N: \
-- ON _skilja_ "to divide, separate; to distinguish; to understand" \
-- Far. _skilja_ "to separate, remove; to distinguish; to grasp, understand" \
-- Elfd. _stjila_ "to separate" \
REF: [EDPG]\: 443

### <a id="skēlō">\*skēlō- [f.] "bowl" (n)</a>
N: \
-- ON _skál_ "bowl, cup" \
-- Far. _skál_ "id." \
-- Elfd. _skål_ "id." \
W: \
-- OS _skāla_ "bowl; shell" \
-- Du. _schaal_ [c.] "id." \
-- OHG _scāla_, M.HG _schāle_ \
REF: [EDPG]\: 443

### <a id="skēpa">\*skēpa- [n.] "sheep" (n)</a>
W: \
-- OE _scēp_, _scīp_, E _sheep_ \
-- OFri. _skēp_ \
-- OS _skāp_ \
-- Du. _schaap_ \
-- OHG _scāf_, G _Schaf_ \
REF: [EDPG]\: 443

### <a id="skēra">\*skēra- [n.] "pair of scissors" (n)</a>
N: \
-- ON _skæri_ [n.pl.] \
-- Far. poet. _skærur_ [m.] \
W: \
-- OE _scēar(r)a_ [f.pl.] \
-- OFri. _skēre_ [f.] \
-- Du. _schaar_ [c.] \
-- OHG _scāra_, pl. _scāri_ [f.], G _Schere_ [f.] \
REF: [EDPG]\: 443

### <a id="skeran-1">\*skeran- [st.] "to cut" (v)</a>
N: \
-- ON _skera_ "to cut; to slaughter" \
-- Far. _skera_ "id." \
-- Elfd. _stjärå_ \
W: \
-- OE _scieran_ "to cut, shave", E _to shear_ \
-- OFri. _skera_ "id." \
-- Du. _scheren_ [wk.] "to shave; to skim" \
-- OHG _sceran_ "to shave, shear, cut", G _scheren_ "id." \
REF: [EDPG]\: 443

### <a id="skeran-2">\*skeran- [m.] "shrew(?), vole(?)" (n)</a>
N: \
-- Nw. _vand-skjer_ "Norway lemming; shrew" \
-- Elfd. _stjäri_ "shrew" \
W: \
-- OHG _scero_ "mouse, mole", G _Scher-maus_ "water vole", Swi. (Jaun) _šære_ "mole" \
REF: [EDPG]\: 444

### <a id="skerban">\*skerban- ~ \*skerpan- ~ \*skrepan- [st.] "to scrape, scratch" (v)</a>
W: \
-- OE _sceorfan_, _sceorpan_, _screpan_ "to scratch, bite, vex" \
-- MHG _schreffen_, _schreven_ "to scratch" \
REF: [EDPG]\: 444

### <a id="skertan">\*skertan- [st.] "to cut off" (v)</a>
W: \
-- Du. dial. _scharten_ [wk.] "to scratch" \
-- MHG _scherzen_ "to cut off, shorten(?)" \
REF: [EDPG]\: 444

### <a id="skeuban">\*skeuban- ~ \*skūban- [st.] "to shove" (v)</a>
E: \
-- Go. _af-skiuban_ "to push away, reject" \
W: \
-- OE _scēofan_, _scūfan_ \
-- OFri. _skūva_ "to push, shove" \
-- Du. _schuiven_ "to shove, slide" \
-- OHG _scioban_ "id.", G _schieben_ "id." \
REF: [EDPG]\: 444

### <a id="skeutan">\*skeutan- [st.] "to shoot, dash" (v)</a>
N: \
-- ON _skjóta_ \
-- Far. _skjóta_ \
-- Elfd. _stjuota_ "to shoot; to jump" \
W: \
-- OE _scēotan_ "id.", E _to shoot_ \
-- OFri. _skiāta_ "id." \
-- OS _sciotan_ "id." \
-- Du. _schieten_ "id." \
-- OHG _skiozan_ "id.", G _schießen_ "id." \
REF: [EDPG]\: 445

### <a id="skēwjan">\*skēwjan- [wk.] "to go, walk" (v)</a>
E: \
-- Go. _skewjan_ "to proceed, walk on" \
N: \
-- ON _skæva_ "to stride, stride on" \
REF: [EDPG]\: 445

### <a id="skīman">\*skīman- [m.] "shine" (n)</a>
E: \
-- Go. _skeima_ "lantern, torch" \
W: \
-- OE _scīma_ "splendor, brightness" \
-- OS _skīmo_ "shine" \
-- OHG _scīmo_ "shine, beam" \
REF: [EDPG]\: 445

### <a id="skīnan">\*skīnan- [st.] "to shine" (v)</a>
E: \
-- Go. _skeinan_ \
N: \
-- ON _skína_ \
-- Far. _skína_ \
-- Elfd. _skaina_ \
W: \
-- OE _scīnan_, E _to shine_ \
-- OFri. _skīna_ "to shine; to appear" \
-- OS _skīnan_ \
-- Du. _schijnen_ \
-- OHG _scīnan_, G _scheinen_ \
REF: [EDPG]\: 445

### <a id="skinkan">\*skinkan- ~ \*skankan- ~ \*skunkan- [m.] "shank, ham" (n)</a>
W: \
-- OE _sceanca_, _sconca_, E _shank_ \
-- OFri. _skunka_, WFri. _skonk_ [c.] \
-- OHG _scinco_, G _Schinken_, Car., Swab. _Schunke_ \
REF: [EDPG]\: 446

### <a id="skipa">\*skipa- [n.] "ship" (n)</a>
E: \
-- Go. _skip_ \
N: \
-- ON _skip_ \
W: \
-- OE _scip_, E _ship_ \
-- OFri. _skip_ \
-- OS _skip_ \
-- Du. _schip_ \
-- OHG _scif_, _scef_, G _Schif_ \
REF: [EDPG]\: 446

### <a id="skītan">\*skītan- [st.] "to shit" (v)</a>
N: \
-- ON _skíta_ \
W: \
-- OE _scītan_, E _to shit_ \
-- MDu. _schiten_, Du. _schijten_ \
-- OHG _scīzan_, G _scheißen_ \
REF: [EDPG]\: 446

### <a id="skittōn">\*skittōn- [wk.] "to shit" (v)</a>
N: \
-- Far. _skitta_ \
REF: [EDPG]\: 446

### <a id="skōha">\*skōha- [m.] "shoe" (n)</a>
E: \
-- Go. _skohs_ \
N: \
-- ON _skór_ \
W: \
-- OE _scōh_, _scō_ [m./n.], E _shoe_ \
-- OFri. _skō_ \
-- OS _skōh_ \
-- Du. _schoen_ [c.] \
-- OHG _scuoh_, G _Schuh_ \
REF: [EDPG]\: 446

### <a id="skōþi">\*skōþi- "harmful" (adj)</a>
N: \
-- ON _skœðr_ \
REF: [EDPG]\: 446

### <a id="skradan">\*skrad/ttan- [m.] "demon" (n)</a>
N: \
-- ON _skratti_, skrati "wizard, troll" \
-- Icel. _skratti_ "devil" \
W: \
-- OE _scrætte_ [f.] "adulteress", ME _skratte_ "hermaphrodite; goblin" (< \*skrattōn-) \
-- OHG _scrato_, _scratto_, _scraz_ "demon, faun", MHG _schrat(e)_ "goblin", G _Schrat_ "id." \
REF: [EDPG]\: 447

### <a id="skrankwjan">\*skrankwjan- [wk.] "to make fall(?)" (v)</a>
W: \
-- OE _ge-screncan_ "to cause to shrink, destroy, supplant", _screncan_ "to lay a stumbling-block in a person's way, trip up, ensnare" \
-- MDu. _schrencken_ "to ensnare, hinder, deceive" \
-- OHG _screncken_ "to lay crosswise, make fall, deceive", MHG _schrenken_ "to place crosswise" \
REF: [EDPG]\: 447

### <a id="skrappōn">\*skrapp/bōn- ~ \*skarbōn- [wk.] "to scrape, scratch" (v)</a>
N: \
-- ON _skrapa_ "to scrape, clatter" \
W: \
-- MDu. _schraven_, _schrabben_, _schrappen_, _schrapen_ "id.", Du. _schrappen_, _schrapen_ "to scratch" \
-- MHG _schraffen_ "id." \
REF: [EDPG]\: 447

### <a id="skraudan">\*skraudan- [st.] "to cut, shred" (v)</a>
W: \
-- OE _scrēadian_ [wk.] "to shred, cut off, prune", E _to shred_ \
-- Du. obs. _schroden_, _schrooien_ [wk.] "to peel" \
-- OHG _scrōtan_ "to cut, shred", G _schroten_ [wk.] "to crack, crush" \
REF: [EDPG]\: 447

### <a id="skrikkōn">\*skrikkōn- [wk.] "to jump, skip" (v)</a>
W: \
-- MLG _schrecken_, schricken "to dance" \
-- MDu. _schricken_ "to stride; to be startled", EDu. _schricken_ "to jump", Du. _schrikken_ [st.] "to be startled" \
-- OHG _screckōn_ "to jump", MHG _schricken_ "id." \
REF: [EDPG]\: 447

### <a id="skrimman">\*skrimman- ~ \*skrimpan- [st.] "to contract" (v)</a>
W: \
-- OE _scrimman_ "to shrink, contract" \
-- MDu. _schrimpen_ "id." \
-- MHG _schrimpfen_ "to shrivel" \
REF: [EDPG]\: 448

### <a id="skrinkwan">\*skrinkwan- [st.] "to shrivel" (v)</a>
N: \
-- Nw. _skrøkka_ "to shrivel" \
W: \
-- OE _scrincan_ "to shrink up, dry up, dwindle", E _to shrink_ \
-- EDu. _schrincken_ "to cringe, retreat" \
REF: [EDPG]\: 448

### <a id="skrīþan">\*skrīþan- ~ \*skrīdan- [st.] "to stride" (v)</a>
N: \
-- ON _skríða_ "to creep, crawl" \
-- Far. _skríða_ "id." \
-- Elfd. _skraiða_ "to pace" \
W: \
-- OE _scrīðan_ "to go, take one's way" \
-- OFri. _skrīda_ "to pass by" \
-- OS _skrīthan_, skridan "to pace" \
-- Du. _schrijden_ "id." \
-- OHG _scrītan_ "id.", G _schreiten_ "id." \
REF: [EDPG]\: 448

### <a id="skriþi">\*skriþi- [m.] "pace" (n)</a>
N: \
-- ON _skriðr_ "creeping" \
W: \
-- OE _scriðe_ "course" \
-- Du. _schrede_ [c.] "footstep" \
-- OHG _scrit_ "id.", G _Schritt_ "id." \
REF: [EDPG]\: 449

### <a id="skrumpōn">\*skrumpōn- [wk.] "to shrink, shrivel" (v)</a>
W: \
-- MDu. _schrompen_, Du. _ver-schrompelen_ \
REF: [EDPG]\: 449

### <a id="skudjan">\*skudjan- [wk.] "to shiver, shake" (v)</a>
W: \
-- OS _skuddian_ \
-- Du. _schudden_ \
-- OHG _scutten_ "to shake, pour, break", G _schütten_ "to pour", _schütteln_ "to shake" \
REF: [EDPG]\: 449

### <a id="skufta-1">\*skufta- [n.] "hair of the head" (n)</a>
E: \
-- Go. _skuft_ \
N: \
-- ON _skopt_ \
-- Nw. _skoft_ "tail; stump" \
W: \
-- OHG _skuft_ [f.] "id." \
REF: [EDPG]\: 449

### <a id="skufta-2">\*skufta- [m.] "shoulder blade of cows or horses" (n)</a>
W: \
-- MLG _schuft_ \
-- EDu. _schoft_, _schocht_, Du. _schoft_ [c.] \
REF: [EDPG]\: 449

### <a id="skukkōn">\*skukk/gōn- [wk.] "to shake" (v)</a>
W: \
-- ME _shoggen_ "to shake, cast off, tremble", E _to shog_ "to shake, roll", obs. _shock_ "to go swiftly, dash" \
-- MDu. _schocken_ "to shake, clash" \
-- MHG _schocken_ "id." \
REF: [EDPG]\: 450

### <a id="skulan">\*skulan- [pp.] "shall, to be obliged to" (v)</a>
E: \
-- Go. _skulan_ "to owe, be indebted, be obliged to" \
N: \
-- ON _skulu_ \
-- Far. _skula_ \
-- Elfd. _ula_ \
W: \
-- OE _sculon_, E _to shall_ \
-- OFri. _skela_ \
-- OS _sculan_ \
-- Du. _zullen_ \
-- OHG _scolan_, G _sollen_ \
REF: [EDPG]\: 450

### <a id="skuldi">\*skuldi- [f.] "debt, guilt" (n)</a>
N: \
-- ON _skuld_ "debt" \
-- Far. _skuld_ "debt; guilt; cause" \
-- Elfd. _skuld_ "id." \
W: \
-- OE _scyld_ "guilt, fault" \
-- OFri. _skielde_, _skiolde_ "guilt; tax" \
-- OS _skuld_ "id." \
-- Du. _schuld_ [c.] \
-- OHG _skuld_, G _Schuld_ \
REF: [EDPG]\: 450

### <a id="skuldra">\*skuldra- [m.] "shoulder" (n)</a>
W: \
-- OE _sculdor_, E _shoulder_ \
-- OFri. _skolder_, _skulder_ \
-- Du. _schouder_ [c.] \
-- OHG _scultirra_, _scultera_ [f.], G _Schulter_ [f.] (< \*skuldrjōn-) \
REF: [EDPG]\: 450

### <a id="skuppōn-1">\*skupp/bōn- [wk.] "to shove" (v)</a>
N: \
-- Icel. _skopa_ "to swing, rock, roll" \
-- Far. _skopa_ "to shoot (of a star); to hurry up; to push, shove" \
-- Nw. _skubba_, _skuppa_, _skopa_ "to bump, shove", EDu. _schobben_, _schoppen_ "to stuff; to swing" \
W: \
-- Du. _schoppen_ "to kick" \
-- MHG _schopfen_, _schoppen_ "to stuff" \
REF: [EDPG]\: 450

### <a id="skuppōn-2">\*skupp/bōn- [wk.] "to mock" (v)</a>
N: \
-- ON _skopa_ \
W: \
-- MLG _schoven_ \
-- EDu. _schobben_, _schoppen_ \
-- MHG _schopfen_ "to versify" \
REF: [EDPG]\: 450

### <a id="skūra">\*skūra/ō- [m./f.] "short shower (of rain/hail)" (n)</a>
E: \
-- Go. _skura_ [f.] \
N: \
-- ON _skúr_ [f.] \
-- Far. _skúrur_ [m.] \
-- Elfd. _skaur_ [f.] \
W: \
-- OE _scūr_ [m.], E _shower_ \
-- MDu. _schuur_ [m./f.] \
-- OHG _skūr_ [m.], G _Schauer_ [m.] \
REF: [EDPG]\: 451

### <a id="skurdi">\*skurdi- [m.] "cutting" (n)</a>
N: \
-- ON _skurðr_ \
W: \
-- OFri. _skerd_ "cut, piece" \
REF: [EDPG]\: 451

### <a id="skuti">\*skuti- [m.] "shooting" (n)</a>
W: \
-- OE _scyte_ \
-- MDu. _scote_ "shoot; shot", Du. _scheut_ [c.] "shoot; gulp; pang" \
-- OHG _scuz_ "shot, speed", G _Schuss_ "shot" \
REF: [EDPG]\: 451

### <a id="skuttōn">\*skut(t)ōn- [wk.] "to shoot" (v)</a>
N: \
-- ON _skotra_ "to shove, push" (r-frequentative) \
-- Icel. _skota_ "id." \
-- Nw. _skota_, _skotta_ "to shoot; to shove, push, drive" \
REF: [EDPG]\: 451

### <a id="skuwwan">\*skuwwan- [m.] "mirror" (n)</a>
E: \
-- Go. _skuggwa_ \
N: \
-- ON _skuggi_ "shadow, shade" \
W: \
-- OE _scuwa_ "id." \
-- OHG _scuwo_, _scū_ "id." \
REF: [EDPG]\: 452

### <a id="slagi">\*slagi- [m.] "blow" (n)</a>
E: \
-- Go. _slahs_ \
W: \
-- OE _slege_ \
-- OFri. _slei_ \
-- OS _slegi_ \
-- Du. _slag_ [c.] \
-- OHG _slag_, G _Schlag_ \
REF: [EDPG]\: 452

### <a id="slahan">\*slahan- [st.] "to beat, strike, slay" (v)</a>
E: \
-- Go. _slahan_ \
N: \
-- ON _slá_ \
-- Far. _sláa_ \
W: \
-- OE _slēan_, E _to slay_ \
-- OFri. _slā_, _slān_ \
-- OS _slahan_ \
-- Du. _slaan_ \
-- OHG _slahan_, G _schlagen_ \
REF: [EDPG]\: 452

### <a id="slaiwa">\*slaiwa- "lethargic" (adj)</a>
N: \
-- ON _slær_, _sljór_ "blunt" \
-- Elfd. _sliuog_ "id." \
W: \
-- OE _slāw_ "sluggish, inert", E _slow_ \
-- OS _slēu_ "slack" \
-- Du. _sleeuw_ "blunt" \
-- OHG _slēo_ "dull, faded" \
REF: [EDPG]\: 452

### <a id="slaka">\*slaka- "slack" (adj)</a>
N: \
-- ON _slakr_ "id." \
-- Far. _slakur_ "miserable, wretched; short" \
W: \
-- OE _slæc_ "id.", E _slack_ \
-- OS _slak_ "slack, cowardly" \
-- MDu. _slac_ "slack, loose, slow" \
-- OHG _slach_ "id." \
REF: [EDPG]\: 452

### <a id="slangan">\*slangan- [m.] "snake" (n)</a>
W: \
-- OS _slango_ \
-- Du. _slang_ \
-- OHG _slango_, MHG _slange_, G _Schlange_ [f.] \
REF: [EDPG]\: 453

### <a id="slanka">\*slanka- "flexible, thin" (adj)</a>
W: \
-- MLG _slank_ "flexible, limp" \
-- MDu. _slanc_ "weak; loose; skinny", Du. _slank_ "slim" \
-- MHG _slanc_ "slim, skinny" \
REF: [EDPG]\: 453

### <a id="slappōn">\*slapp/bōn- [wk.] "to be slack" (v)</a>
N: \
-- ON _slafa-st_ [refl.wk.] "to slacken" \
-- Icel. _slapa_ "to be slack", _slabba_ "to dangle" \
-- Far. _slabba_ "to toil" \
-- Nw. _slapa_ "to dangle" \
REF: [EDPG]\: 453

### <a id="slaubjan">\*slaubjan- ~ \*slaupjan- [wk.] "to roll up" (v)</a>
E: \
-- Go. _af-slaupjan_ "to put off, lay aside" \
W: \
-- OE _slīepan_ "id." \
-- OFri. _slēpa_ "to fasten, drape around" \
-- OS _slōpian_ "to loosen" \
-- MDu. _sloven_ "to roll up, slip" \
-- OHG _sloufen_ "to slip" \
REF: [EDPG]\: 453

### <a id="slawēn">\*slawēn- [wk.] "to be silent" (v)</a>
E: \
-- Go. _slawan_ \
REF: [EDPG]\: 453

### <a id="slēpan">\*slēpan- [st.] "to sleep" (v)</a>
E: \
-- Go. _slepan_ \
W: \
-- OE _slǣpan_, E _to sleep_ \
-- OFri. _slēpa_ \
-- OS _slāpan_ \
-- Du. _slapen_ \
-- OHG _slāfan_, G _schlafen_ \
REF: [EDPG]\: 453

### <a id="sleupan">\*sleupan- ~ \*slūpan- [st.] "to sneak" (v)</a>
E: \
-- Go. _sliupan_ "to slip, slide" \
W: \
-- OE _slūpan_ "id." \
-- OFri. _sliapa_ "id." \
-- MDu. _slupen_ \
-- OHG _sliofan_ "to slip, sneak", MHG _sliefen_ "to slip" \
REF: [EDPG]\: 454

### <a id="sleutan">\*sleutan- ~ \*slūtan- [st.] "to close" (v)</a>
W: \
-- OFri. _slūta_ \
-- OS _ūt-bi-slūtan_ "to exclude" \
-- MDu. _sluten_, Limb. _slieten_ "id.", Du. _sluiten_ "to close" \
-- OHG _sliozzan_, G _schließen_ \
REF: [EDPG]\: 454

### <a id="slidan">\*slidan- [m.] "sled" (n)</a>
N: \
-- ON _sleði_ \
W: \
-- Du. _slee_ [c.] \
-- OHG _slito_, G _Schlitten_ \
REF: [EDPG]\: 454

### <a id="slīdan">\*slīdan- [st.] "to slide, slip" (v)</a>
W: \
-- OE _slīdan_, E _to slide_ \
-- MDu. _sliden_ \
-- MHG _slītan_ \
REF: [EDPG]\: 454

### <a id="slīma">\*slīma- [n.] "slime" (n)</a>
N: \
-- ON _slím_ \
-- Far. _slím_ "smoke, haze; slime; mucus" \
W: \
-- OE _slīm_ [m./n.] "slime, mud", E _slime_ \
-- Du. _slijm_ \
-- MHG _slīm_ [m.], G _Schleim_ [m.] \
REF: [EDPG]\: 455

### <a id="slindan">\*slindan- [st.] "to devour" (v)</a>
E: \
-- Go. _fra-slindan_ \
W: \
-- OS _far-slindan_ \
-- Du. _ver-slinden_ \
-- OHG _slintan_, _slindan_, MHG _slinden_, G _ver-schlingen_ (with dialectal -_ng_- < -_nd_-) \
REF: [EDPG]\: 455

### <a id="slingan">\*slingan- ~ \*slinkan- [st.] "to slink" (v)</a>
N: \
-- OSw. _slinka_ "to sneak, crawl, slip" \
-- Elfd. _slinka_ "id." \
W: \
-- OE _slingan_, _slincan_ "to creep", E _to slink_ \
-- MDu. _slinken_ [wk.] "to diminish; to slump, collapse"; _slingen_ [wk.] "to creep, crawl", Du. _slinken_ "to decrease" \
-- OHG _slingan_ "to swing, wind", G _schlingen_ "to loop" \
REF: [EDPG]\: 455

### <a id="slīwan">\*slīwa/ōn- [m./f.] "tench" (n)</a>
W: \
-- OE _slīw_, _slēow_ [m.] "tench/mullet" \
-- MDu. _slie_ [f.] "id." \
-- OHG _slīo_ [m.] "id.", G _Schleie_ [f.] "id." \
REF: [EDPG]\: 455

### <a id="slunkōn">\*slunkōn- [wk.] "to slink(?)" (v)</a>
N: \
-- Nw. _slunka_ "to stumble" \
W: \
-- G _schlunken_ "ro slouch" \
REF: [EDPG]\: 455

### <a id="slutila">\*slutila- [m.] "key" (n)</a>
W: \
-- OFri. _sletel_ \
-- OS _slutil_ \
-- Du. _sleutel_ [c.] \
-- OHG _sluzzil_, G _Schlüssel_ \
REF: [EDPG]\: 455

### <a id="slutōn">\*slutōn- [wk.] "to come to an end" (v)</a>
N: \
-- Nw. _slota_ "to diminish, subside, stop" \
REF: [EDPG]\: 456

### <a id="smahrja">\*smahrja- [m.] "lip" (n)</a>
W: \
-- OE _smǣras_ [m.pl.] "lips" \
REF: [EDPG]\: 456

### <a id="smala">\*smala- "small" (adj)</a>
E: \
-- Go. _smals_ \
N: \
-- ON _smalr_ \
-- Far. _smalur_ "narrow" \
W: \
-- OE _smæl_ "small, narrow", E _small_ \
-- OFri. _smel_ "id." \
-- OS _smal_ "id." \
-- Du. _smal_ "id." \
-- OHG _smal_ "id." \
REF: [EDPG]\: 456

### <a id="smalta">\*smalta- ~ \*smulta- [n.] "(molten) fat, lard" (n)</a>
N: \
-- Icel. _smolt_ \
-- Nw. _smolt_ \
W: \
-- OE _smolt_ \
-- MDu. _smout_ \
-- OHG _smalz_, G _Schmalz_ \
REF: [EDPG]\: 456

### <a id="smēha">\*smēha- "small" (adj)</a>
N: \
-- ON _smár_ \
-- Far. _smáur_ \
-- Elfd. _smą̊er_, _smą̊_- \
W: \
-- OHG _smahi_, MHG _smæhe_ \
REF: [EDPG]\: 456

### <a id="smelhwō">\*smelhwō- ~ \*smelwō- [f.] "hair-grass" (n)</a>
N: \
-- Nw. _smyle_, dial. _smile_, _smele_ [n.] \
W: \
-- OHG _smelha_, MHG _smelehe_, _smilehe_, _smelwe_, G _Schmiele_ \
REF: [EDPG]\: 456

### <a id="smeltan">\*smeltan- [st.] "to melt, smelt" (v)</a>
N: \
-- Nw. _smelta_ \
-- Elfd. _smelta_ \
W: \
-- OS _smeltan_ \
-- Du. _smelten_ \
-- OHG _smelzan_, G _schmelzen_ \
REF: [EDPG]\: 457

### <a id="smērjōn">\*smērjōn- [f.] "clover" (n)</a>
N: \
-- Icel. _smæra_ \
-- Far. _smæra_ \
-- Nw. _smære_ [m./f.] \
-- Sw. dial. _smäre_ [m.] \
-- Da. _smære_ [c.] \
REF: [EDPG]\: 457

### <a id="smertan">\*smertan- [st.] "to hurt" (v)</a>
W: \
-- OE _smeortan_, E _to smart_ \
-- MDu. _smarten_ [st./wk.] \
-- OHG _smerzen_, G _schmerzen_ [wk.] \
REF: [EDPG]\: 457

### <a id="smerwa">\*smerwa- [n.] "butter, grease" (n)</a>
N: \
-- ON _smjǫr_, _smør_ \
-- Far. _smør_ "butter" \
-- Elfd. _smyör_ "id." \
W: \
-- OE _smeoru_ "fat, grease, tallow" \
-- OFri. _smere_ [m.] "pus" \
-- OS _smeru_ "fat" \
-- Du. _smeer_ "grease" \
-- OHG _smero_ [m.] "fat", G _Schmer_ [m./n.] "kidney fat" \
REF: [EDPG]\: 458

### <a id="smeugan">\*smeugan- ~ \*smāgan- [st.] "to sneak, creep" (v)</a>
N: \
-- ON _smjúga_ "to creep through" \
-- Far. _smúgva_ "to slip through; to gnaw at, nibble away" \
W: \
-- OE _smūgan_ "to creep, crawl" \
-- MDu. _smugen_, _smuken_ [wk.] "to crawl" \
-- MHG _smiegen_ "to snuggle up; to crouch" \
REF: [EDPG]\: 458

### <a id="smeuhō">\*smeuhō- [f.] "widgeon, pochard, smew(?)" (n)</a>
W: \
-- WFri. _smjunt_, NFri. _smün_ \
-- EDu. _smiente_, Du. _smient_ \
-- MHG _smiehe_, G _Schmei(ch)-ente_, _Schmü-ente_, Als. _Schmiche_ \
REF: [EDPG]\: 458

### <a id="smeukan">\*smeukan- ~ \*smūkan- [st.] "to smoke" (v)</a>
W: \
-- OE _smēocan_ \
-- MDu. _smieken_, _smuken_ [wk.] \
REF: [EDPG]\: 458

### <a id="smītan">\*smītan- [st.] "to stroke; to strike" (v)</a>
E: \
-- Go. _ga-smeitan_ "to daub" \
N: \
-- Nw. _smita_ "to smear over; to sneak away" \
W: \
-- OE _smītan_ "to daub, smear, smudge", E _to smite_ \
-- OFri. _smīta_ "to throw, strike, destroy" \
-- MDu. _smiten_ "to throw, hit, smack", Du. _smijten_ "to throw" \
-- OHG _smīzan_ "to smear", G _schmeißen_ "id." \
REF: [EDPG]\: 459

### <a id="smittōn">\*smit(t)ōn- [wk.] "to strike; to smudge" (v)</a>
N: \
-- Nw. _smita_ "to stroke, smear; to spur; to flatter" \
-- OSw. _smita_ "to daub, smear; to hit" \
-- Elfd. _smitå_ "to smack; to smear, daub" \
W: \
-- OE _smittian_ "to befile" \
-- MDu. _smetten_, _smitten_ "to stain, befoul" \
-- MHG _smitzen_ "to jerk; to hit, beat; to smear, daub", G Pal. _schmitzen_ "to insult" \
REF: [EDPG]\: 459

### <a id="smīþō">\*smīþō- [f.] "craftsmanship" (n)</a>
N: \
-- ON _smíð_ "(handi)craft, smith's work" \
-- Far. _smíð_ "forging, making; (handi)craft; trade" \
W: \
-- OHG _smīda_ "jewelry, metal" \
REF: [EDPG]\: 459

### <a id="smiþu">\*smiþu- [m.] "smith" (n)</a>
E: \
-- Go. _aiza-smiþa (<_ \*smipan-) \
N: \
-- ON _smiðr_ \
-- Far. _smiður_ \
-- Elfd. _smið_ \
W: \
-- OE _smiþ_, E _smith_ \
-- OFri. _smeth_ \
-- Du. _smid_ [c.] \
-- OHG _smid_, G _Schmied_ \
REF: [EDPG]\: 460

### <a id="smukkōn-1">\*smukk/gōn- [wk.] "to creep" (v)</a>
N: \
-- Far. _smokka_ "to sink, slide down" \
-- Nw. _smugga_ "to do something secretly" \
W: \
-- Du. _smokkelen_ "to steal, sneak, smuggle" \
REF: [EDPG]\: 460

### <a id="smukkōn-2">\*smuk(k)ōn- [wk.] "to smoke" (v)</a>
W: \
-- OE _smocian_ \
-- MDu. _smoken_ \
-- G Lux. _schmocken_ \
REF: [EDPG]\: 460

### <a id="snaidō">\*snaidō- [f.] "(cut off) chunk" (n)</a>
N: \
-- ON _sneið_ \
-- Far. _sneið_ \
-- Elfd. _snįeð_ [n.] \
W: \
-- OFri. _snēthe_ \
-- Du. _snee_, _snede_ [c.] \
-- MHG _sneite_ "road through a forest" \
REF: [EDPG]\: 460

### <a id="snaiwa">\*snaiwa- [m.] "snow" (n)</a>
E: \
-- Go. _snaiws_ \
N: \
-- ON _snær_, gen. _snjóar_ \
-- Far. _snjógvur_ \
-- Elfd. _sniųo_ \
W: \
-- OE _snāw_, E _snow_ \
-- OS _snēo_ \
-- Du. _sneeuw_ [c.] \
-- OHG _snē(o)_, G _Schnee_ \
REF: [EDPG]\: 460

### <a id="snakan">\*snakan- [st.] "to crouch(?)" (v)</a>
N: \
-- ON _snaka_ [wk.] "to rummage, snuff about" \
-- Nw. _snaka_ [st./wk.] "to snuff, look for food; to snatch" \
W: \
-- OHG _snahhan_ "to crouch" \
REF: [EDPG]\: 460

### <a id="snawwa">\*snawwa- "bald" (adj)</a>
N: \
-- ON _snøggr_ "short; short-haired; sudden" \
-- Far. _snøggur_ "with short hair, smooth; bald" \
REF: [EDPG]\: 461

### <a id="snerkan">\*snerkan- ~ \*snerpan- [st.] "to shrivel" (v)</a>
N: \
-- Icel. _snorkinn_ "shrivelled, shrunken, wizened" \
-- Far. _snorkin_ "lean (esp. of meat); scanty, scarce" \
-- Nw. _snerka_, _snerpa_ "to shrivel; to form a layer of ice" \
W: \
-- OE _ge-sneorcan_ "id." \
-- MDu. _snerpen_ [wk.] "to hurt, singe" \
-- OHG _snerfan_, MHG _snerfen_ \
REF: [EDPG]\: 461

### <a id="snewan">\*snewan- ~ \*snūan- [st.] "to rush" (v)</a>
E: \
-- Go. _sniwan_ \
N: \
-- ON _snúa_ "to turn" \
-- Far. _snúgva_ "to twist; to turn; to change" \
W: \
-- OE _snēowan_ \
REF: [EDPG]\: 461

### <a id="sniggan">\*sniggan- [m.] "snail" (n)</a>
W: \
-- Du. dial. _snigge_ [c.] \
-- OHG _snecko_, G _Schnecke_ \
REF: [EDPG]\: 462

### <a id="snīpōn">\*snīpōn- ~ \*snippōn- [f.] "snipe" (n)</a>
N: \
-- ON _snípa_ \
-- Elfd. _snaipa_ \
W: \
-- OE _snīpe_, E _snipe_ \
-- OS _snippa_ \
-- MDu. _snippe_, _sneppe_, Du. _snip_ [c.] \
-- OHG _snepfo_ [m.], _snepfa_, G _Schnepfe_ \
REF: [EDPG]\: 462

### <a id="snittōn">\*snittōn- [wk.] "to cut" (v)</a>
W: \
-- MHG _snitzen_, G _schnitzen_ \
REF: [EDPG]\: 462

### <a id="snīþan">\*snīþan- [st.] "to cut" (v)</a>
E: \
-- Go. _sneiþan_ \
N: \
-- ON _sníða_ \
-- Far. poet. _sníða_ [st.?] \
W: \
-- OE _snīðan_ \
-- OFri. _snītha_ \
-- OS _snīthan_ \
-- Du. _snijden_ \
-- OHG _snīdan_, G _schneiden_ \
REF: [EDPG]\: 462

### <a id="snīwan">\*snīwan- [st.] "to snow" (v)</a>
N: \
-- ON poet. _snýr_ 3sg.pres. \
-- Elfd. _snaja_ [st./wk.] \
W: \
-- OE _snīwan_ \
-- MDu. _snuwen_ [wk.] \
-- OHG _snīwan_, _snīan_, G _schneien_ [wk.] "id." \
REF: [EDPG]\: 462

### <a id="snōbri">\*snōbri- "narrow" (adj)</a>
N: \
-- ON _snœfr_ "quick" \
-- Nw. dial. _snøvar_ "short" \
REF: [EDPG]\: 462

### <a id="snōka">\*snōka- [m.] "animal(?)" (n)</a>
N: \
-- Icel. _snókur_ "trunk, snout, snake" \
-- Nw. _snok_ "snout, snail" \
-- Sw. _snok_ [c.] "viper" \
W: \
-- MDu. _snoek_ "pike", Du. _snoek_ [c.] "id." \
REF: [EDPG]\: 463

### <a id="snōtō">\*snōtō- [f.] "bride(?)" (n)</a>
N: \
-- ON _snót_ "woman" \
REF: [EDPG]\: 463

### <a id="snutra">\*snutra- "clever, wise" (adj)</a>
E: \
-- Go. _snutrs\*_ \
N: \
-- ON _snotr_ \
W: \
-- OE _snotor_, _snottor_ \
-- OHG _snottar_ \
REF: [EDPG]\: 463

### <a id="snuzō">\*snuzō- [f.] "daughter-in-law" (n)</a>
E: \
-- Go. Crim. _schnos_ \
N: \
-- ON _snør_ \
-- Icel. _snör_ \
W: \
-- OE _snoru_ \
-- OFri. _snore_ \
-- MDu. _snore_ \
-- OHG _snur_, _snura_, G _Schnur_ \
REF: [EDPG]\: 463

### <a id="sōda">\*sōda- [n.] "satiation" (n)</a>
E: \
-- Go. _soþ_ [m./n.] \
REF: [EDPG]\: 463

### <a id="sōel">\*sōel- ~ \*sunnōn- [n.] "sun" (n)</a>
E: \
-- Go. _sauil_, dat. _sunnin_, _sunno_ [f.] \
N: \
-- ON _sól_ [f.], _sunna_ [f.] \
-- Far. _sól_ [f.] "sun; moon" \
-- Elfd. _suol_ [f.] \
W: \
-- OE _sunne_ [f.], E _sun_ \
-- OFri. _sunne_ [f.] \
-- OS _sunna_ [f.] \
-- Du. _zon_ [c.] \
-- OHG _sunna_ [f.], G _Sonne_ [f.] \
REF: [EDPG]\: 463

### <a id="sōkjan">\*sōkjan- [wk.] "to seek, find, demand" (v)</a>
E: \
-- Go. _sokjan_ "to quest, search, request" \
N: \
-- ON _sœkja_ "to fetch; to seek" \
-- Far. _søkja_ "to look, search for, fetch; to visit, ravage; to seek, obtain; poet. to accuse, charge; to sue, summon" \
-- Elfd. _syötja_ "to look" \
W: \
-- OE _sēcan_ "to try to find, look for" \
-- OFri. _sēka_ "to try to find, visit; to charge, attack" \
-- OS _sōkian_ "to try to find; to claim" \
-- Du. _zoeken_ "to try to find" \
-- OHG _suohhen_ "to try to find, request", G _suchen_ "id." \
REF: [EDPG]\: 464

### <a id="spadan">\*spadan- [m.] "spade" (n)</a>
N: \
-- ON _spaði_ \
-- Far. _spaði_, _spaki_ \
-- Elfd. _spaði_ \
W: \
-- OE _spadu_ [f.] (< \*spadō-), E _spade_ \
-- OFri. _spada_ \
-- OS _spado_ \
-- Du. _spade_ [c.] \
-- G _Spaten_ \
REF: [EDPG]\: 464

### <a id="spaikuldra">\*spaikuldra/ō- [n./f.] "saliva" (n)</a>
E: \
-- Go. _spaiskuldr_ [n.] (miswritten for \*_spaikuldr_?) \
W: \
-- OS _spēkaldra_ [f.] \
-- MDu. _speikeltre_ [f.], _spekelter_ [m.] \
-- OHG _speihhaltra_ [f.], MHG _speicholter_ [f.] \
REF: [EDPG]\: 464

### <a id="spaitan">\*spaitan- [st.] "to spit" (v)</a>
W: \
-- OE Nrth. _spātan_ (pret. _speoft_) \
REF: [EDPG]\: 465

### <a id="spaldan">\*spaldan- [st.] "to split" (v)</a>
W: \
-- MDu. _spouden_ [st./wk.] \
-- OHG _spaltan_, G _spalten_ [st./wk.] \
REF: [EDPG]\: 465

### <a id="spannan">\*spannan- [st.] "to stretch" (v)</a>
W: \
-- OE _spannan_ "to attach, fasten, clasp", E _to span_ \
-- OFri. _spanna_ [wk.] "to fasten" \
-- OS _und-spannan_ ptc. "unstrung" \
-- Du. _spannen_ [st./wk.] "to stretch, strain" \
-- OHG _spannan_ "to tighten", G _spannen_ "to strain, stretch" \
REF: [EDPG]\: 465

### <a id="spara">\*spara- "sparing, frugal" (adj)</a>
N: \
-- ON _sparr_ \
W: \
-- OE _spær_, E _spare_ \
-- MDu. _spaer_ \
-- OHG _spar_ "sparse" \
REF: [EDPG]\: 465

### <a id="sparran">\*spar(r)an- [m.] "bar, beam, rafter" (n)</a>
N: \
-- ON _spari_, _sparri_ "rafter, bar" \
W: \
-- OS _sparro_ "beam" \
-- Du. _dak-spar_ [c.] "rafter" \
-- OHG _sparro_ "bar", G _Sparren_ "rafter" \
REF: [EDPG]\: 466

### <a id="sparwan">\*sparwa(n)- [m.] "sparrow" (n)</a>
E: \
-- Go. _sparwa_ \
N: \
-- ON _spǫrr_, pl. _spǫrvar_ \
-- Icel. _spör_ \
-- OSw. _sparver_ \
-- Da. _spurv_ \
W: \
-- OE _spearwa_, E _sparrow_ \
-- OHG _sparo_, MHG _spar(e)_, _sparwe_ \
REF: [EDPG]\: 466

### <a id="spēdi">\*spēdi- "late" (adj)</a>
E: \
-- Go. _spediza_ comp. \
W: \
-- MDu. _spade_ \
-- OHG _spāti_, G _spät_ \
REF: [EDPG]\: 466

### <a id="spelda">\*spelda/ō- [n./f.] "piece of wood" (n)</a>
E: \
-- Go. _spilda_ [f.] "tablet" \
N: \
-- ON _spjald_ [n.] "tablet, square" \
W: \
-- OE _speld_ [n.] "splinter, piece of wood" \
-- MHG _spelte_ [f.] "id." \
REF: [EDPG]\: 466

### <a id="spella">\*spella- [n.] "story, tale, legend" (n)</a>
E: \
-- Go. _spill_ \
N: \
-- ON _spjall_ \
W: \
-- OE _spell_ \
-- OS _spel_ \
-- ODu. _spel_ \
-- OHG _spel_ \
REF: [EDPG]\: 466

### <a id="spenan">\*spenan- [m.] "nipple" (n)</a>
N: \
-- ON _speni_ \
-- Far. _spini_ "teat; penis" \
REF: [EDPG]\: 466

### <a id="speru">\*speru- [m.] "spear" (n)</a>
N: \
-- ON _spjǫr_ [n.] \
W: \
-- OE _spere_ [n.], E _spear_ \
-- OFri. _spere_, _spiri_ \
-- OS _sper_ [n.] \
-- Du. _speer_ [c.] \
-- OHG _sper_ [n.], G _Speer_ \
REF: [EDPG]\: 467

### <a id="speuta">\*speuta- [n.] "spear" (n)</a>
N: \
-- ON _spjót_ \
-- Far. _spjót_ \
-- Elfd. _spiuot_ \
W: \
-- OS _spiot_ \
-- MDu. _spiet_ \
-- OHG _spioz_ [m.], G _Spieß_ [m.] \
REF: [EDPG]\: 467

### <a id="spika">\*spika- [m.] "bacon" (n)</a>
N: \
-- ON _spik_ [n.] "blubber" \
W: \
-- OE _spic_ [n.] "bacon, lard" \
-- OS _spek_ [n.] "id." \
-- Du. _spek_ [n.] "id." \
-- OHG _spec_ "id.", G _Speck_ "id." \
REF: [EDPG]\: 467

### <a id="spila">\*spila- [n.] "play; game" (n)</a>
W: \
-- OFri. _spil_, _spel_ \
-- OS _spil_ "play; movement" \
-- Du. _spel_ "id." \
-- OHG _spil_ "id.", G _Spiel_ "id." \
REF: [EDPG]\: 467

### <a id="spinkan">\*spinka(n)- [m.] "finch" (n)</a>
N: \
-- ON _spiki_ "tit" \
-- Nw. _spikke_ "sparrow, small bird" \
-- Sw. _spink(e)_ "small bird, sparrow, finch" (> E dial. _spink_ "finch") \
REF: [EDPG]\: 467

### <a id="spinnan">\*spinnan- [st.] "to spin" (v)</a>
E: \
-- Go. _spinnan_ \
N: \
-- ON _spinna_ \
W: \
-- OE _spinnan_, E _to spin_ \
-- Du. _spinnen_ \
-- OHG _spinnan_, G _spinnen_ \
REF: [EDPG]\: 467

### <a id="spīwan">\*spīwan- [st.] "to spit" (v)</a>
E: \
-- Go. _speiwan_ \
N: \
-- ON _spýja_ "to spew, vomit" \
-- Far. _spýggja_ [wk.] "to belch forth, vomit" \
-- Elfd. _spaja_ [wk.] "id." (without w-mutation) \
W: \
-- OE _spīwan_ "to spew, vomit", E _to spew_ \
-- OFri. _spīa_ \
-- OS _spīwan_ \
-- MDu. _spien_, _spuwen_, Du. _spuwen_, _spugen_ \
-- OHG _spīwan_, _spīan_ "to spit, vomit", G _speien_ "id." \
REF: [EDPG]\: 468

### <a id="splītan">\*splītan- [st.] "to split" (v)</a>
W: \
-- MDu. _spliten_, Du. _splijten_ \
-- MHG _splīzen_, G Bav., Pal. _spleißen_ [wk.] \
REF: [EDPG]\: 468

### <a id="splittōn">\*splittōn- [wk.] "to split" (v)</a>
W: \
-- E _to split_, _splet_, Scot. _spleet_ \
-- WFri. _splitte_ \
-- MDu. _spletten_, _splitten_ \
REF: [EDPG]\: 468

### <a id="spōan">\*spōan- [st.] "to succeed" (v)</a>
W: \
-- OE _spōwan_ \
-- MDu. _spoen_ [wk.] "to strive for, look after" \
-- OHG _spuon_ [wk.] \
REF: [EDPG]\: 468

### <a id="spōdi">\*spōdi- [f.] "success; speed" (n)</a>
W: \
-- OE _spēd_, E _speed_ \
-- MDu. _spoed_ "progress; success", Du. _spoed_ "rush" \
-- OHG _spuot_ "quality, velocity" \
REF: [EDPG]\: 469

### <a id="spraidjan">\*spraidjan- ~ \*spraitjan- [wk.] "to spread, disperse" (v)</a>
N: \
-- Far. _spreiða_ \
W: \
-- OE _sprǣdan_, E _to spread_ \
-- MDu. _spreden_, _spreiden_, Du. _spreiden_ \
-- OHG _spreiten_, G _spreiten_, _spreizen_ \
REF: [EDPG]\: 469

### <a id="sprangjan">\*sprangjan- [wk.] "to make jump, to make burst" (v)</a>
N: \
-- ON _sprengja_ "to make burst" \
-- Far. _spreingja_ "to blow up; to overwork someone" \
-- Elfd. _spraindja_ "to blast; to crack, break open" \
W: \
-- OE _sprengan_ "to spring" \
-- OFri. _sprenza_ "to sprinkle" \
-- OHG _sprengan_ "to make jump; to sprinkle", G _sprengen_ "to blast; to break; to gallop" \
REF: [EDPG]\: 469

### <a id="sprekan">\*sprekan- [st.] "to speak" (v)</a>
W: \
-- OE _sprecan_, _specan_, E _to speak_ \
-- OFri. _spreka_ \
-- OS _sprekan_ \
-- Du. _spreken_ \
-- OHG _sprehhan_, G _sprechen_ \
REF: [EDPG]\: 469

### <a id="sprekō">\*sprekō- [f.] "speech" (n)</a>
W: \
-- OE _sprǣc_, _spǣc_, E _speech_ \
-- OFri. _sprēka_, _sprētze_ \
-- OS _sprāka_ \
-- Du. _spraak_ [c.] \
-- OHG _spāhha_, G _Sprache_ \
REF: [EDPG]\: 469

### <a id="spreutan">\*spreutan- ~ \*sprūtan- [st.] "to bud, sprout" (v)</a>
W: \
-- OE _ā-sprūtan_, E _to sprout_ \
-- OFri. _sprūta_ \
-- MLG _sprēten_, _sprūten_ \
-- MDu. _spruten_, Du. _spruiten_ \
-- MHG _spriezen_, G _sprießen_ \
REF: [EDPG]\: 470

### <a id="springan">\*spring/kan- [st.] "to break out, leap" (v)</a>
N: \
-- ON _springa_ "to spring; to spurt out; to burst, break" \
-- Icel. _springa_, _sprinka_ "id." \
-- Far. _springa_ "to leap, jump; to burst, explode" \
-- Elfd. _springga_ "to grow" \
W: \
-- OE _springan_, _sprincan_ [wk.] "to spring, burst out; to grow", E _to spring_ \
-- OFri. _springa_ "to spring; to jump, leap, run" \
-- OS _springan_ "id." \
-- Du. _springen_ "id." \
-- OHG _springan_ "to jump, gush, flow", G _springen_ "id." \
REF: [EDPG]\: 470

### <a id="sprittōn">\*sprit(t)ōn- [wk.] ".to disperse" (v)</a>
N: \
-- Nw. _sprita_ \
W: \
-- G _spritzen_ "to squirt; to become dispersed" \
REF: [EDPG]\: 470

### <a id="sprīþan">\*sprīþan- ~ \*sprītan- [st.] "to break up, become scattered" (v)</a>
W: \
-- OHG _sprīzan_, MHG _sprīden_, _sprīten_ "to spread" \
REF: [EDPG]\: 470

### <a id="spruttōn">\*sprut(t)ōn- [wk.] "to sprout" (v)</a>
W: \
-- EDu. _sprotten_ \
-- G _sprossen_ "to germinate" \
REF: [EDPG]\: 470

### <a id="spurdi">\*spurdi- [f.] "track, course" (n)</a>
E: \
-- Go. _spaurds_ \
W: \
-- OE _spyrd_ [m.] \
-- OHG _spurt_ [m.] \
REF: [EDPG]\: 470

### <a id="spurjan">\*spurjan- [wk.] "to follow a track" (v)</a>
N: \
-- ON _spyrja_ "to track, trace; to investigate; to ask" \
W: \
-- OE _spyrian_ "to track; to enquire, investigate" \
-- OFri. _spera_ "to investigate; to probe" \
-- Du. _speuren_ "to investigate" \
-- OHG _spurren_ "to trace; to sense", G _spüren_ "id." \
REF: [EDPG]\: 471

### <a id="spurkōn">\*spurkōn- [wk.] "to crackle, frizzle, roast(?)" (v)</a>
W: \
-- Du. Flem. _spokken_ ~ _sporken_ "to roast" \
REF: [EDPG]\: 471

### <a id="spurnan">\*spurnan- [st.] "to kick, spurn" (v)</a>
N: \
-- ON _sperna_, _sporna_ \
-- Elfd. _spienna_ \
W: \
-- OE _spornan_, E _to spurn_ \
-- OFri. _spurna_ [wk.] \
-- OS _spurnan(?)_ \
-- OHG _spurnan_ [wk.] \
REF: [EDPG]\: 471

### <a id="staba">\*staba- [m.] "staff; letter" (n)</a>
E: \
-- Go. _stabos_ [m.pl.] "letters" \
N: \
-- ON _stafr_ "staff; stave" \
-- Far. _stavur_ "post, stick; letter" \
-- Elfd. _stav_ "staff" \
W: \
-- OE _stæf_ "staff, stick; letter", E _staf_, _stave_ \
-- OFri. _stef_ "staff" \
-- Du. _staf_ [c.] "id." \
-- OHG _stap_ "id.", G _Stab_ "id." \
REF: [EDPG]\: 471

### <a id="stadi">\*stadi- [m./f.] "place, town" (n)</a>
E: \
-- Go. _staþs_ [m.] \
N: \
-- ON _staðr_ [m.] \
-- Far. _staður_ [m.] \
-- Elfd. _stað_, pl. _stäðer_ [m.] \
W: \
-- OE _stede_ [f.], E _stead_ \
-- OFri. _sted_ [f.] \
-- OS _stad_ [f.] \
-- Du. _stad_ [f.] "city" \
-- OHG _stat_ [f.] "place", G _Stadt_ [f.] "city" \
REF: [EDPG]\: 472

### <a id="stahla">\*stahla- [n.] "steel" (n)</a>
N: \
-- ON _stál_ \
-- Far. _stál_ \
-- Elfd. _stą̊l_ \
W: \
-- ODu. _stāl_, Du. _staal_ \
-- OHG _stahal_ [m.], G _Stahl_ [m.] \
REF: [EDPG]\: 472

### <a id="staina">\*staina- [m.] "stone" (n)</a>
E: \
-- Go. _stains_ \
N: \
-- ON _steinn_ \
-- Far. _steinur_ \
-- Elfd. _stie_ \
W: \
-- OE _stān_, E _stone_ \
-- OFri. _stēn_ \
-- OS _stēn_ \
-- Du. _steen_ [c.] \
-- OHG _stein_, G _Stein_ \
REF: [EDPG]\: 472

### <a id="stakan">\*stakan- [m.] "pole, post" (n)</a>
W: \
-- OE _staca_, E _stake_ \
-- MDu. _stake_, Du. _staak_ [c.] \
REF: [EDPG]\: 472

### <a id="stakka">\*stakka- [m.] "haystack" (n)</a>
N: \
-- ON _stakkr_ "stack of hay; cape" \
-- Far. _stakkur_ "high rock with flat top; pile (of dried cod)" \
-- Elfd. _stakk_ "pile, bunch" \
REF: [EDPG]\: 472

### <a id="stalla">\*stalla- [m.] "standing, stand, stall, stable" (n)</a>
N: \
-- ON _stallr_ "pedestal, support; crib, manger" \
-- Far. _stallur_ "rock; foundation; manger" \
-- Elfd. _stall_ [n.] "stable" \
W: \
-- OE _steall_ "position, place", E _stall_ \
-- OFri. _stal_ "stall in a stable; standing" \
-- Du. _stal_ "stable; stand; stall" \
-- OHG _stal_ "id.", G Stall "id." \
REF: [EDPG]\: 472

### <a id="stamma">\*stamma- "stammering" (adj)</a>
E: \
-- Go. _stamms_ \
N: \
-- ON _stammr_ \
W: \
-- OE _stamm_ \
-- OHG _stam_ \
REF: [EDPG]\: 473

### <a id="stampōn">\*stamp/bōn- [wk.] "to compress" (v)</a>
N: \
-- ON _stappa_ "to mash, stamp" \
-- Far. _stappa_ "to stuff, compress, squeeze" \
-- Elfd. _stampa_ "to stamp" \
W: \
-- MDu. _stampen_ "to stamp, trample", Du. _stampen_ "to trample" \
-- OHG _stampfōn_ "to stamp, press, beat", G _stampfen_ "to stamp, stump, ram" \
REF: [EDPG]\: 473

### <a id="standan">\*standan- [st.] "to stand" (v)</a>
E: \
-- Go. _standan_ \
N: \
-- ON _standa_ \
-- Far. _standa_ \
-- Elfd. _standa_ \
W: \
-- OE _standan_, E _to stand_ \
-- OFri. _stonda_ \
-- OS _standan_ \
-- OHG _stantan_ \
REF: [EDPG]\: 473

### <a id="stangō">\*stangō- [f.] "rod, bar" (n)</a>
N: \
-- ON _stǫng_ \
-- Far. _stong_ \
-- Elfd. _staungg_ \
W: \
-- MDu. _stange_, Du. _stang_ \
-- OHG _stanga_, G _Stange_ \
REF: [EDPG]\: 474

### <a id="stappan">\*stap(p)an- ~ \*stapjan- [st.] "to step, pace" (v)</a>
W: \
-- OE _stæppan_, _steppan_, E _to step_ \
-- OFri. _stapa_, _steppa_ \
-- OS pret. _stōp_ \
-- MDu. _stappen_, _steppen_ [st./wk.], Du. _stappen_ [wk.] \
-- OHG _stepfen_ [wk.] \
REF: [EDPG]\: 474

### <a id="stappōn">\*stapp/bōn- [wk.] "to walk" (v)</a>
N: \
-- Nw. _stabba_ "to stumble" \
W: \
-- Du. _stappen_ "to step" \
-- OHG _stapfōn_ "id.", MHG _stapfen_, _stappen_, _staben_ "id.", G _stapfen_, Rhnl. _staben_ "id." \
REF: [EDPG]\: 474

### <a id="stara">\*stara- "stiff" (adj)</a>
W: \
-- MDu. _star_, Du. _star_ \
-- MHG _star_, _sterre_ \
REF: [EDPG]\: 474

### <a id="starfa">\*starfa- [n.] "labor" (n)</a>
N: \
-- ON _starf_ \
-- Far. _starv_ "work, employment; pains, trouble, effor" \
REF: [EDPG]\: 474

### <a id="starka">\*starka- "strong" (adj)</a>
N: \
-- Icel. obs. _starkur_ "healthy" \
-- OSw. _starker_ "stiff, firm, solid, strong" \
-- Elfd. _stark_ "strong" \
W: \
-- OE _stearc_ "stiff, rigid, strong", E _stark_ \
-- OFri. _sterk_ "strong" \
-- OS _stark_ "strong, hostile" \
-- Du. _sterk_ "strong" \
-- OHG _starah_ "strong, big", G _stark_ "strong, vigorous, severe" \
REF: [EDPG]\: 474

### <a id="starran">\*star(r)a(n)- [m.] "starling" (n)</a>
N: \
-- ON _stari_ \
-- Icel. _star(r)i_ \
W: \
-- OE _stær_, E _starling_ \
-- OS _stara_ [f.] (< \*starōn-) \
-- MDu. _sterre_ [f.] \
-- OHG _star(o)_, G _Star_ "id." \
REF: [EDPG]\: 475

### <a id="stapla">\*stapla- [m.] "standing, shed" (n)</a>
W: \
-- OE _staðol_ "place, foundation" \
-- MDu. _stadel_, _stael_ "foundation of a dam" \
-- OHG _stadal_ "standing, shed" \
REF: [EDPG]\: 475

### <a id="staubjan">\*staubjan- [wk.] "to make steam(?)" (v)</a>
W: \
-- MDu. _stoven_ "to steam, stew" \
REF: [EDPG]\: 475

### <a id="stauma">\*stauma- [m.] "steam" (n)</a>
W: \
-- OE _stēam_, E _steam_ \
-- Du. _stoom_ [c.] \
REF: [EDPG]\: 475

### <a id="staupa">\*staupa- "steep" (adj)</a>
W: \
-- OE _stēap_, E _steep_ \
-- OFri. _stāp_ \
REF: [EDPG]\: 475

### <a id="staurjan">\*staurjan- [wk.] "to disturb, destroy" (v)</a>
W: \
-- OFri. _to-stēra_ \
-- ODu. _storen_, EDu. _storen_ "to disturb" \
-- OHG _stōren_, _stōrren_ "to destroy", G _stören_ "to disturb, destroy" \
REF: [EDPG]\: 476

### <a id="stautan">\*stautan- [st.] "to hit, jab, thrust" (v)</a>
E: \
-- Go. _stautan_ \
N: \
-- ON _stauta_ [wk.] \
-- Far. _steyta_ [wk.] \
W: \
-- OFri. _stēta_ \
-- MDu. _stoten_, Du. _stoten_ [st./wk.] \
-- OHG _stōzan_, G _stoßen_ \
REF: [EDPG]\: 476

### <a id="stekan">\*stekan- [st.] "to stick" (v)</a>
W: \
-- OFri. _steka_ \
-- OS _stekan\*_ \
-- Du. _steken_ \
-- OHG _stechan_, G _stechen_ \
REF: [EDPG]\: 476

### <a id="steki">\*steki- "stinking" (adj)</a>
N: \
-- Icel. _stækur_ \
-- Nw. _stæk_ "rancid" \
REF: [EDPG]\: 476

### <a id="stelan">\*stelan- [st.] "to steal; to go stealthily" (v)</a>
E: \
-- Go. _stilan_ "to steal" \
N: \
-- ON _stela_ "to steal; steal upon" \
-- Far. _stjala_ "to steal" \
-- Elfd. _stjälå_ "to steal; to sneak" \
W: \
-- OE _stelan_ "to steal, go stealthily", E _to steal_ \
-- OFri. _stela_ "to steal" \
-- OS _stelan_ "id." \
-- Du. _stelen_ "id." \
-- OHG _stelan_ "id.", G _stehlen_ "id." \
REF: [EDPG]\: 476

### <a id="sten">\*sten- [suppl.] "to stand" (v)</a>
N: \
-- Far. poet. _stá_ \
-- Nw. _stå_ \
-- OSw. _stā_, Sw. _stå_ \
-- Da. _stå_ \
W: \
-- OFri. _stān_ \
-- ODu. _stān_, Du. _staan_ \
-- MHG _stān_, _stēn_, G _stehen_ \
REF: [EDPG]\: 477

### <a id="sterban">\*sterban- [st.] "to become stiff, die" (v)</a>
W: \
-- OE _steorfan_ "to die", E _to starve_ \
-- OFri. _sterva_ "id." \
-- OS _sterƀan_ "id." \
-- Du. _sterven_ "id." \
-- OHG _sterban_ "id.", G _sterben_ "id." \
REF: [EDPG]\: 477

### <a id="sterōn">\*sterōn- [f.] "sterile woman" (n)</a>
E: \
-- Go. _stairo_ \
REF: [EDPG]\: 478

### <a id="sterran">\*ster(r)a/ōn- [m./f.] "star" (n)</a>
E: \
-- Go. _stairno_ [f.] \
N: \
-- ON _stjarna_ [f.] \
-- Far. _stjørna_ [f.] \
-- Elfd. _stienna_ [f.] \
W: \
-- OE _steorra_ [m.], E _star_ \
-- OFri. _stera_ [m.], WFri. _stjer_ [c.] \
-- OS _sterro_ [m.] \
-- Du. _ster_ [c.] \
-- OHG _sterro_, _sterno_ [m.], G _Stern_ [m.] \
REF: [EDPG]\: 478

### <a id="steuban">\*steuban- ~ \*stūban- [st.] "to blow; to fly about" (v)</a>
W: \
-- MDu. _stuven_, _stieven_, Du. _stuiven_ \
-- OHG _stioban_, G _stieben_ \
REF: [EDPG]\: 478

### <a id="steura">\*steura- [m.] "bull" (n)</a>
E: \
-- Go. _stiur_ \
W: \
-- OE _stēor_, E _steer_ \
-- ODu. _stier_, Du. _stier_ [c.] \
-- OHG _stior_, G _Stier_ \
REF: [EDPG]\: 478

### <a id="steurjan">\*steurjan- [wk.] "to direct" (v)</a>
E: \
-- Go. _stiurjan_ "to establish" \
N: \
-- ON _stýra_ "to steer; to manage, rule" \
-- Far. _stýra_ "id." \
W: \
-- OE _stīeran_ "id." \
-- OFri. _stiāra_ "to steer; to detain" \
-- OS _gi-stiurian_ "to show the way" \
-- Du. _sturen_ "to steer; to direct, send" \
-- OHG _stiurren_ "to support; to steer, direct; to regulate", G _steuern_ "to steer; to govern" \
REF: [EDPG]\: 479

### <a id="stīgan">\*stīgan- [st.] "to ascend" (v)</a>
E: \
-- Go. _steigan_ \
N: \
-- ON _stíga_ "to step, tread" \
-- Far. _stíga_ "id." \
-- Sw. _staiga_ "id." \
W: \
-- OE _stīgan_ "to go; to ascend, mount" \
-- OFri. _stīga_ \
-- OS _stīgan_ \
-- Du. _stijgen_ \
-- OHG _stīgan_, G _steigen_ \
REF: [EDPG]\: 479

### <a id="stihti">\*stihti- [f.] "track" (n)</a>
N: \
-- ON _stétt_ "pavement; degree, rank" \
-- Nw. _stætt_ "position, standing" \
W: \
-- Du. dial. _sticht_ "road; stile" \
REF: [EDPG]\: 479

### <a id="stija">\*stija- [n.] "sty" (n)</a>
N: \
-- ON _stí_; _stía_ [f.] "sty, kennel" \
-- Icel. _stía_ [f.] "id." \
-- Far. _stíggj_ "sty, paddock" \
W: \
-- OE _stig_, _stī_, E _sty_ \
-- MDu. _stije_ [f.] \
-- OHG _stīga_ [f.], MHG _stīje_, _stīge_ [f.] \
REF: [EDPG]\: 479

### <a id="stikkōn">\*stik(k)ōn- [wk.] "to stick" (v)</a>
W: \
-- OE _stician_ "to stick", E _to stick_ \
-- MDu. _sticken_ "to stick, fix" \
REF: [EDPG]\: 479

### <a id="stimbiza">\*stimbiza- [n.] "firmness" (n)</a>
N: \
-- Far. _stimbur_ "strength, firmness, solidity" \
REF: [EDPG]\: 480

### <a id="stimnō">\*stimnō- [f.] "voice" (n)</a>
E: \
-- Go. _stibna_ \
W: \
-- OFri. _stemme_ \
-- OS _stemna_ \
-- Du. _stem_ [c.] \
-- OHG _stimma_, _stimna_, G _Stimme_ \
REF: [EDPG]\: 480

### <a id="stingan">\*stingan- [st.] "to stick" (v)</a>
E: \
-- Go. _us-stiggan_ "to pluck out" \
N: \
-- ON _stinga_ "to sting, stick, stab" \
-- Far. _stinga_ "to stick, stab, thrust" \
-- Elfd. _stingga_ "id." \
W: \
-- OE _stingan_ "id.", E _to sting_ \
-- OHG _stingan_ "to let bleed, venesect" \
REF: [EDPG]\: 480

### <a id="stinkwan">\*stinkwan- [st.] "to thrust, clash; to stink" (v)</a>
E: \
-- Go. _stigqan_ [wk.] "to hit" \
N: \
-- ON _støkkva_ "to leap, jump, flee" \
-- Far. _støkka_ [wk.] "to rush, run; to burst, snap; to drop, splash" \
-- Elfd. _stekka_ "to sprinkle, splash" \
W: \
-- OE _stincan_ "to smell, stink; to leap, spring", E _to stink_ \
-- OS _stinkan_ "id." \
-- Du. _stinken_ "id." \
-- OHG _stinkan_ "id.", G _stinken_ "id." \
REF: [EDPG]\: 480

### <a id="stōda">\*stōda- [n.] "flock" (n)</a>
N: \
-- ON _stóð_ "stud" \
-- Far. _stóð_ "lazy horse" \
-- Elfd. _stuoð_ [f.] "litter; flock" \
W: \
-- OE _stōd_ "stud, herd of horses", E _stud_ \
-- Du. _stoet_ [c.] "guidance, procession" \
-- OHG _stuot_ [f.] "herd of horses", G _Stute_ [f.] "mare" \
REF: [EDPG]\: 481

### <a id="stōla">\*stōla- [m.] "chair, seat" (n)</a>
E: \
-- Go. _stols_ \
N: \
-- ON _stóll_ \
-- Far. _stólur_ \
-- Elfd. _stuol_ \
W: \
-- OE _stōl_, E _stool_ \
-- OFri. _stōl_ \
-- OS _stōl_ \
-- Du. _stoel_ [c.] \
-- OHG _stuol_, G _Stuhl_ \
REF: [EDPG]\: 481

### <a id="stōō">\*stōō- [f.] "place; judgement" (n)</a>
E: \
-- Go. _staua_ "judgement, verdict" \
N: \
-- ON _eld-stó_ "fire-place" \
W: \
-- OE _stōw_ "place" \
-- OFri. _stō_ "site; place" \
-- OHG _stuo-tago_ [m.] "Judgement Day" \
REF: [EDPG]\: 481

### <a id="stōpan">\*stōpan- [m.] "footstep" (n)</a>
W: \
-- OE _stōpel_ "footstep, trail" \
-- OS _stōpo_ "track" \
REF: [EDPG]\: 482

### <a id="stōra">\*stōra- ~ \*stura- "big" (adj)</a>
N: \
-- ON _stórr_ \
-- Far. _stórur_ \
-- OSw. _stōr_, _stur_, Sw. _star_ \
-- Elfd. _stur_ \
W: \
-- OE _stōr_ "giant" \
-- OFri. _stōr_ \
-- OS _stōri_ "renowned" \
-- MDu. _stuur_ "strong, bold, tough, harsh", Du. _stoer_ "tough" \
-- OHG _stūri_ "id.", MHG _stiure_ "big; excellent" \
REF: [EDPG]\: 482

### <a id="strandō">\*strandō- [f.] "edge; shore, beach" (n)</a>
N: \
-- ON _strǫnd_ "border, edge; coast, shore" \
-- Far. _strond_ "beach, shore" \
-- Elfd. _strand_ "id." \
W: \
-- OE _strand_ [n.] "id.", E _strand_ \
-- MDu. _strande_ "beach, shore", _strand_ [m.] "inlet", Du. _strand_ [n.] "beach" \
-- G _Strand_ [m.] "id." (= LG) \
REF: [EDPG]\: 482

### <a id="strangi">\*strangi- [m.] "string, cord, rope" (n)</a>
N: \
-- ON _strengr_ \
-- Far. _strongur_, gen. _streingjar_ \
-- Elfd. _straingg_ \
W: \
-- OE _streng_, E _string_ \
-- Du. _streng_ [c.] \
-- OHG _stranc_, G _Strang_ \
REF: [EDPG]\: 483

### <a id="strangja">\*strang(j)a- "strong" (adj)</a>
N: \
-- ON _strangr_ "strong, severe, hard" \
-- Far. _strangur_ "severe, harsh" \
W: \
-- OE _strong_ "powerful, hard", E _strong_ \
-- WFri. _strang_ "strict" \
-- OS _strang_ \
-- Du. _streng_ "severe, strict" \
-- OHG _strengi_ "strong, rigorous", G _streng_ "id." \
REF: [EDPG]\: 483

### <a id="straujan">\*straujan- [wk.] "to strew" (v)</a>
E: \
-- Go. _straujan_ \
N: \
-- ON _strá_ \
-- Far. _stroya_ \
-- Nw. _strøya_ \
-- Elfd. _sträa_ \
W: \
-- OE _strēowian_, E _to strew_ \
-- WFri. _streauwe_ \
-- OS _strōian_ \
-- Du. _strooien_ \
-- OHG _strewen_, G _streuen_ \
REF: [EDPG]\: 483

### <a id="strauma">\*strauma- [m.] "stream" (n)</a>
N: \
-- ON _straumr_ \
-- Far. _streymur_ \
W: \
-- OE _strēam_, E _stream_ \
-- OFri. _strām_ \
-- OS _strōm_ \
-- Du. _stroom_ [c.] \
-- OHG _stroum_, G _Strom_ \
REF: [EDPG]\: 483

### <a id="straupjan">\*straupjan- [wk.] "to strip" (v)</a>
N: \
-- Nw. _strøypa_ "to squeeze, strangle" \
W: \
-- OE _be-strīepan_ "to strip", E _to strip_ \
-- MDu. _stropen_ "to strip, streak, brush" \
-- OHG _stroufen_ "to pluck", MHG _stroufen_ "to strip, wipe, pull" \
REF: [EDPG]\: 483

### <a id="strawa">\*strawa- [n.] "straw" (n)</a>
N: \
-- ON _strá_ \
W: \
-- OE _strēa(w)_, E _straw_ \
-- OFri. _strē_, WFri. _strie_ \
-- ODu. _strō_, Du. _stro_ \
-- OHG _strō_, G _Stroh_ \
REF: [EDPG]\: 484

### <a id="strēlō">\*strēlō- [f.] "ray, beam, stick, arrow" (n)</a>
N: \
-- Nw. _stråle_ "long stick, slender tree" (< \*strēlōn-) \
W: \
-- OE _strǣl_ [m./f.] "id." \
-- OS _strāla_ "id." \
-- MDu. _strael_ [m./n.], _strale_ "id.", Du. _straal_ [c.] "beam; water jet" \
-- OHG _strāl_ [m.], _strala_ "arrow", G _Strahl_ [m.] "beam" \
REF: [EDPG]\: 484

### <a id="streþan">\*streþan- [st.] "to seethe, swirl" (v)</a>
W: \
-- OHG _stredan_, MHG _streden_ \
REF: [EDPG]\: 484

### <a id="streukan">\*streukan- [st.] "to stroke" (v)</a>
N: \
-- ON _strjúka_ "to stroke, rub, wipe" \
-- Far. _strúka_ "to stroke; to erase; to run, rush, dash" \
-- Elfd. _striuoka_ "to stroke, wipe (out)" \
REF: [EDPG]\: 484

### <a id="streupan">\*streupan- ~ \*strūpan- [st.] "to squeeze, strip" (v)</a>
N: \
-- Nw. _str(j)úpa_ "to squeeze, compress; to strangle" \
W: \
-- MHG _striefen_ "to graze, brush" \
REF: [EDPG]\: 484

### <a id="strīdan">\*strīdan- [st.] "to stand upright, resist" (v)</a>
W: \
-- OE _strīdan_ "to stride; to pillage", E _to stride_ \
-- OFri. _strīda_ "to fight" \
-- MLG _strīden_ "to straddle the legs apart, stride" \
-- MDu. _striden_ "to resist, struggle", Du. _strijden_ "to fight" \
-- OHG _strītan_ "to fight", MHG _strīten_ "to battle, endeavour; to stride(?)", G _streiten_ "id." \
REF: [EDPG]\: 485

### <a id="strīkan">\*strīkan- [st.] "to stroke" (v)</a>
W: \
-- OE _strīcan_, E _to strike_ \
-- OFri. _strīka_ \
-- Du. _strijken_ \
-- OHG _strihhan_, G _streichen_ \
REF: [EDPG]\: 485

### <a id="strīpan">\*strīpa/ōn- [f.] "stripe" (n)</a>
N: \
-- Far. _strípa_ \
-- Nw. _stripe_ \
W: \
-- MDu. _stripe_ \
-- MHG _strīfe_ [m.], G _Streifen_ [m.] \
REF: [EDPG]\: 485

### <a id="strittōn">\*strit(t)ōn- [wk.] "to stand upright, resist" (v)</a>
N: \
-- ON _strita_ "to drag with difficulty; to strive hard" \
-- Far. _strita_ "id." \
-- Nw. _stritta_ "to resist, walk slowly" \
-- Da. _stritte_ "to resist; to stick out; to straddle the legs" \
REF: [EDPG]\: 485

### <a id="strudu">\*strudu- [f.] "river" (n)</a>
W: \
-- G obs. _Strut_ \
REF: [EDPG]\: 485

### <a id="strukkōn">\*struk(k)ōn- [wk.] "to stroke" (v)</a>
N: \
-- Icel. _stroka_ "to rage, sweep, dash, blow" \
-- Nw. _strokka_ "to beat, thresh" \
-- Elfd. _strukå_ "to wander" \
W: \
-- OE _stroccian_, E _to stroke_ \
-- MLG _straken_ \
-- EDu. _stroocken_ "to stroke, pat" \
REF: [EDPG]\: 486

### <a id="struppōn">\*struppōn- [wk.] "to writhe" (v)</a>
W: \
-- MDu. _strop(p)en_ "to strip, skin" \
-- MHG _strupfen_ "to writhe; to strip" \
REF: [EDPG]\: 486

### <a id="struttōn-1">\*strut(t)ōn- [f.] "throat" (n)</a>
W: \
-- OFri. _strot-bolla_ [m.] "Adam's apple" \
-- OS _strota_ "id." \
-- MDu. _strote_, _strotte_ "throat, windpipe, gullet", Du. _strot_ [c.] "id." \
-- MHG _strozze_ "windpipe, throat" \
REF: [EDPG]\: 486

### <a id="struttōn-2">\*struttōn- [wk.] "to well (up)" (v)</a>
W: \
-- MHG _strotzen_ \
REF: [EDPG]\: 486

### <a id="struttōn-3">\*struttōn- [wk.] "to jut out" (v)</a>
W: \
-- MHG _strotzen_ "to bristle up" \
REF: [EDPG]\: 486

### <a id="stubju">\*stubju- [m.] "dust" (n)</a>
E: \
-- Go. _stubjus_ \
W: \
-- MDu. _stubbe_ [n.] \
-- OHG _stuppi_ [n.], G _Ge-stüpp_ [n.] \
REF: [EDPG]\: 486

### <a id="stukka">\*stukka- [m.] "stick" (n)</a>
N: \
-- ON _stokkr_ "stock, trunk, block, log of wood" \
-- Far. _stokkur_ "log, beam, cane" \
-- Elfd. _stukk_ \
W: \
-- OE _stocc_ "stick, log, trunk" \
-- OFri. _stok_ \
-- Du. _stok_ [c.] \
-- OHG _stoc_ "stick, trunk", G _Stock_ "id." \
REF: [EDPG]\: 487

### <a id="stukkja">\*stukkja- [n.] "piece" (n)</a>
N: \
-- ON _stykki_ \
-- Far. _stykki_ \
-- Elfd. _styttje_ \
W: \
-- OE _stycce_ \
-- OS _stukki_ \
-- MDu. _stuk_ \
-- OHG _stucki_, G _Stück_ \
REF: [EDPG]\: 487

### <a id="stukkōn">\*stuk(k)ōn- [wk.] "to stick" (v)</a>
W: \
-- Du. _stokken_ "to come to a halt" \
-- G _stocken_ "to coagulate; to stop" \
REF: [EDPG]\: 487

### <a id="stullōn">\*stullōn- [wk.] "to steal" (v)</a>
W: \
-- G Rhnl. _stollen_ \
REF: [EDPG]\: 487

### <a id="stumma">\*stumma- "mute" (adj)</a>
W: \
-- OFri. _stum_ \
-- OS _stum_ \
-- Du. _stom_ "mute, dull, stupid" \
-- OHG _stum(b)_ "silent", G _stumm_ "id." \
REF: [EDPG]\: 487

### <a id="stunkwi">\*stunkwi- [m.] "hit; smell" (n)</a>
E: \
-- Go. _bi-stugqs_ "thrust" \
W: \
-- OE _stenc_ "smell", E _stench_ \
-- OS _stunc_ "id." \
REF: [EDPG]\: 487

### <a id="stunta">\*stunta- "short" (adj)</a>
N: \
-- ON _stuttr_ \
-- Far. _stuttur_ \
-- Elfd. _stutt_ \
W: \
-- OE _stunt_ "foolish" \
-- MHG _stunz_ "dull; cropped, short" \
REF: [EDPG]\: 487

### <a id="stūpēn">\*stūpēn- [wk.] "to stoop" (v)</a>
N: \
-- ON _stúpa_ \
-- Nw. _stupa_ [st./wk.] "to drop, fall down" \
W: \
-- OE _stūpian_ "to stoop, bend the back", E _to stoop_ \
-- MDu. _stupen_ "to bend" \
REF: [EDPG]\: 488

### <a id="sturjan">\*sturja/ōn- [m./f.] "sturgeon" (n)</a>
N: \
-- ON _styrja_ [f.] \
W: \
-- OE _styria_, _styriga_ [m.] \
-- ODu. _sturio_ [m.], Du. _steur_ [c.] \
-- OHG _sturio_, _sturo_ [m.], MHG _stūre_ [m.], G _Stör_ [m.] (= LG) \
REF: [EDPG]\: 488

### <a id="sturki">\*sturki- [m.] "strength" (n)</a>
N: \
-- ON _styrkr_ \
-- Far. _styrkur_ "help, aid, assistance" \
REF: [EDPG]\: 488

### <a id="sturma">\*sturma- [m.] "storm" (n)</a>
N: \
-- ON _stormr_ \
W: \
-- OE _storm_, E _storm_ \
-- OS _storm_ \
-- Du. _storm_ [c.] \
-- OHG _sturm_, G _Sturm_ \
REF: [EDPG]\: 488

### <a id="sturnēn">\*sturnēn- [wk.] "to stand out" (v)</a>
E: \
-- Go. _and-staurran_ "to stare at" \
W: \
-- OHG _storren_ "to stand out" \
REF: [EDPG]\: 488

### <a id="stuttōn">\*stut(t)ōn- [wk.] "to shove, bump" (v)</a>
W: \
-- ME _stoten_ "to stop, halt; to stagger; to falter" \
-- G obs. _stotzen_ "to stick", Swi. stotzen "to stuff", Rhnl. _stotzen_ "to stumble, stagger" \
REF: [EDPG]\: 489

### <a id="stuþ">\*stuþ/d- [f.] "prop, support, post" (n)</a>
N: \
-- ON _stoð_, _stuð_, pl. _støðr_, _steðr_ \
-- Elfd. _stuoð_ \
W: \
-- OE _studu_, _stuðu_, pl. _styde_, _styðe_ \
-- MHG _stud_ \
REF: [EDPG]\: 489

### <a id="sufan">\*sufan- [st.] "to sleep" (v)</a>
N: \
-- ON _sofa_ \
-- Far. _sova_ \
-- Elfd. _såvå_ \
-- OGutn. _sufa_ \
W: \
-- OE _swefan_ \
REF: [EDPG]\: 489

### <a id="sufja">\*sufja- "sleepy" (adj)</a>
W: \
-- Du. _suf_ \
REF: [EDPG]\: 489

### <a id="sūgan">\*sūgan- ~ \*sūkan- [st.] "to suck" (v)</a>
N: \
-- ON _súga_ \
-- Icel. _sjúga_ \
-- Far. _súva_ \
-- Elfd. _sauga_ \
W: \
-- OE _sūgan_, _sūcan_, E _to suck_ \
-- MDu. _sugen_, _sucen_, Du. _zuigen_ \
-- OHG _sūgan_, G _saugen_ \
REF: [EDPG]\: 489

### <a id="sū">\*sū- ~ \*suw- [f.] "sow" (n)</a>
N: \
-- ON _sýr_ \
-- Far. _súgv_ \
-- Elfd. _sugga_ \
W: \
-- OE _sū_, _sugu_, E _sow_ \
-- OFri. _sūg(e)-lubber_ [m.] "sow-gelder", WFri. _sûch_ [c.] "sow" \
-- OS _sū_ \
-- MDu. _soge_, Du. _zeug_ [c.] \
-- OHG _sū_, G _Sau_ \
REF: [EDPG]\: 490

### <a id="suhti">\*suhti- [f.] "sickness, disease" (n)</a>
E: \
-- Go. _sauhts_ \
N: \
-- ON _sótt_ \
-- Far. _sótt_ \
-- Elfd. _suott_ \
W: \
-- OFri. _secht_ \
-- OS _suht_ \
-- OHG _suht_, G _Sucht_ "addiction" \
REF: [EDPG]\: 490

### <a id="suhturwjan">\*suhturwjan- [m.] "brother's son" (n)</a>
W: \
-- OE _suhtri(g)a_ "brother's son, nephew, cousin" \
REF: [EDPG]\: 490

### <a id="sukkōn">\*sukk/gōn- [wk.] "to suck" (v)</a>
W: \
-- OE _socian_ \
-- G Rhnl. _sucken_, Swi. (App.) _sugə_ \
REF: [EDPG]\: 490

### <a id="sukkōn">\*suk(k)ōn- [wk.] "to be ill" (v)</a>
W: \
-- MHG _sochen_, G Swab. _sochen_, _socken_ \
REF: [EDPG]\: 491

### <a id="sulha">\*sulha- [f.] "plow" (n)</a>
W: \
-- OE _sulh_, pl. _sylh_ \
REF: [EDPG]\: 491

### <a id="sūli">\*sūli- [f.] "pillar, column" (n)</a>
N: \
-- ON _súl_ \
W: \
-- OE _sȳl_ \
-- OFri. _sēle_ \
-- ODu. _sūl_, Du. _zuil_ [c.] \
-- OHG _sūl_, G _Säule_ \
REF: [EDPG]\: 491

### <a id="sultjō">\*sultjō- [f.] "brine(?)" (n)</a>
N: \
-- Nw. _sylt_ "brackish swamp" \
W: \
-- ODu. _sulta_ "brackish terrain", MDu. _sult_ "head cheese", Du. _zult_ [n.] "id." \
-- OHG _sulza_ "brine", G _Sülze_ "aspic" \
REF: [EDPG]\: 491

### <a id="suma">\*suma- "some(one)" (pron)</a>
E: \
-- Go. _sums_ \
N: \
-- ON _sumr_ \
-- Far. _summur_ \
W: \
-- OE _sum_, E _some_ \
-- OFri. _sum_ "partly" (adv) \
-- OS _sum_ \
-- OHG _sum_ \
REF: [EDPG]\: 491

### <a id="sumara">\*sumara- [n.] "summer" (n)</a>
N: \
-- ON _sumar_ \
-- Far. _summar_ \
-- Elfd. _såmår_ [m.] \
W: \
-- OE _sumer_, _sumor_ [m.], E _summer_ \
-- OFri. _sumer_, _sumur_, _simer_ [m.], WFri. _simmer_ [c.] \
-- OS _sumar_ [m.] \
-- Du. _zomer_ [c.] \
-- OHG _sumar_ [m.], G _Sommer_ [m.] \
REF: [EDPG]\: 491

### <a id="sunda">\*sunda- [n.] "sound" (n)</a>
N: \
-- ON _sund_ "swimming; sound, strait" \
-- Far. _sund_ "id." \
W: \
-- OE _sund_ "swimming", E _sound_ \
-- MLG _sund_ "strait" \
-- G _Sund_ [m.] "id." \
REF: [EDPG]\: 492

### <a id="sundjō">\*sundjō- [f.] "transgression, sin" (n)</a>
W: \
-- OS _sundea_ \
-- Du. _zonde_ [c.] \
-- OHG _suntea_, _sunta_, G _Sünde_ \
REF: [EDPG]\: 492

### <a id="sunja">\*sunja- "true" (adj)</a>
E: \
-- Go. _sunjis_ \
REF: [EDPG]\: 492

### <a id="sunþera">\*sunþera- "south, southwards" (adv)</a>
N: \
-- ON _suðr_ \
-- Far. _suður_ \
-- Elfd. _sutter_ \
W: \
-- OE _sūþ_, E _south_ \
-- OFri. _sūth_, _sūther_ [n.] \
-- OS _sūthar_ \
-- MDu. _suder_ \
-- OHG _sundar_ \
REF: [EDPG]\: 492

### <a id="sunu">\*sunu- [m.] "son" (n)</a>
E: \
-- Go. _sunus_ \
N: \
-- ON _sunr_, _sonr_ \
-- Far. _sonur_ \
-- Elfd. _sun_ \
W: \
-- OE _sunu_, E _son_ \
-- OFri. _suno_ \
-- OS _sunu_ \
-- Du. _zoon_ [c.] \
-- OHG _sun(u)_, G _Sohn_ \
REF: [EDPG]\: 492

### <a id="sūpan">\*sūpan- [st.] "to guzzle, soak" (v)</a>
N: \
-- ON _súpa_ "to sip, drink" \
-- Far. _súpa_ "to eat with a spoon" \
-- Elfd. _saupa_ "to drink; to sniff" \
W: \
-- OE _sūpan_ "to sup, to drink" \
-- MDu. _supen_ "to guzzle", Du. _zuipen_ "id." \
-- OHG _sūfan_ "id.", G _saufen_ [wk.] "to drink", intr. "to sink" \
REF: [EDPG]\: 493

### <a id="suppōn">\*supp/bōn- [wk.] "to soak" (v)</a>
E: \
-- Go. _supon_ "to season" (< \*"to dip"?) \
N: \
-- Icel. _subba_ "to soil" \
-- Nw. _subba_ "to dabble" \
W: \
-- OE _soppian_ "to dip, dunk", E _to sop_ ~ _sob_ "to soak, dip" \
-- MDu. _sopp(er)en_ "to soak", Du. _soppen_ "to soak; be wet" \
-- OHG _sofōn_, _soffōn_ "to dunk" \
REF: [EDPG]\: 493

### <a id="sūra">\*sūra- "sour" (adj)</a>
N: \
-- ON _súrr_ "sour, bitter" \
-- Far. _súrur_ "acid, sourish" \
-- Elfd. _saur_ "id." \
W: \
-- OE _sūr_ "id.", E _sour_ \
-- OFri. _sūre_ "id." \
-- Du. _zuur_ "id." \
-- OHG _sūr_ "id.", G _sauer_ "id." \
REF: [EDPG]\: 493

### <a id="surgō">\*surgō- [f.] "grief, sorrow, worry" (n)</a>
E: \
-- Go. _saurga_ \
N: \
-- ON _sorg_ \
-- Far. _sorg_ \
-- Elfd. _sorg_ \
W: \
-- OE _sorg_, _sorh_, E _sorrow_ \
-- OS _sorga_ \
-- Du. _zorg_ [c.] \
-- OHG _sorga_, _sworga_, G _Sorg_ \
REF: [EDPG]\: 493

### <a id="swabjan">\*swabjan- [wk.] "to cause to sleep" (v)</a>
N: \
-- ON _svefja_ "to lull to sleep, soothe" \
W: \
-- OE _swebban_ "to put to sleep", poet. "to kill" \
-- OHG _swebben_ "to put to sleep" \
REF: [EDPG]\: 494

### <a id="swaipan">\*swaipan- [st.] "to sweep" (v)</a>
N: \
-- ON _sveipa_ "to cast; to wrap, swaddle" \
W: \
-- OE _swāpan_ "to sweep", E _to swope_ \
-- OHG _sweifan_ "to sway, swing, sling" \
REF: [EDPG]\: 494

### <a id="swaipjan">\*swaipjan- [wk.] "to sweep, cast" (v)</a>
N: \
-- ON _sveipa_ "to sweep, drive; to wrap, swaddle" \
W: \
-- ME _swēpen_ "to sweep; to destroy; to rush", E _to sweep_ \
-- MHG _sweifen_ "to roam, wander", G _schweifen_ "id." \
REF: [EDPG]\: 494

### <a id="swaita">\*swaita- [m./n.] "sweat" (n)</a>
N: \
-- ON _sveiti_ [m.] "sweat; blood" \
-- Far. _sveiti_ [m.] "blood" (< \*swaitan-) \
W: \
-- OE _swāt_ [n.] \
-- OFri. _swēt_ [m.], WFri. _swit_ [n.] \
-- Du. _zweet_ [n.] \
-- OHG _sweiz_ [m.], G _Schweiß_ [m.] "sweat; blood (in hunters' jargon)" \
REF: [EDPG]\: 494

### <a id="swajuta">\*swajuta- [n.] "host" (n)</a>
N: \
-- ON _sveit_, _sjót_ "company, host" \
W: \
-- OE _swēot_ "troop, band" \
REF: [EDPG]\: 494

### <a id="swaljan">\*swaljan- [wk.] "to burn" (v)</a>
W: \
-- OHG _swellen_ \
REF: [EDPG]\: 494

### <a id="swalljan">\*swalljan- [wk.] "to cause to swell" (v)</a>
W: \
-- OHG _swellen_, MHG _swellen_ \
REF: [EDPG]\: 494

### <a id="swalwōn">\*swalwōn- [f.] "swallow" (n)</a>
N: \
-- ON _svala_ \
-- Far. _svala_ \
-- Elfd. _sålå_, obl. _salu_ \
W: \
-- OE _swealwe_, _swealewe_, E _swallow_ \
-- WFri. _swel_ [c.], EFri. _swoalke_ \
-- OS _swala_ \
-- Du. _zwaluw_ \
-- OHG _swalawa_, G _Schwalbe_ \
REF: [EDPG]\: 495

### <a id="swamban">\*swamb/ppan- [m.] "sponge, mushroom" (n)</a>
E: \
-- Go. _swamms_ "sponge" \
N: \
-- ON _svǫppr_ "mushroom" (< \*swampu-) \
-- Far. _soppur_ "wisp of straw; mushroom; fungus" \
-- Elfd. _sopp_ "mushroom; fungus" \
W: \
-- OE _swam_, _swom_ "id." \
-- MDu. _swam(me)_ "id.", Du. _zwam_ [c.] "mushroom" \
-- OHG _swamp/swam_ "id.", G _Schwamm_ "sponge; mushroom" \
REF: [EDPG]\: 495

### <a id="swammjan">\*swammjan- [wk.] "to make swim" (v)</a>
W: \
-- OE _be-swemman_ \
-- MHG _swemmen_, G _schwemmen_ "to float, wash" \
REF: [EDPG]\: 495

### <a id="swamōn">\*swamōn- [wk.] "to swim" (v)</a>
N: \
-- Far. _svam(b)la_ "to splash around water, bathe" \
-- Nw. _svamla_ "id." \
W: \
-- MHG _swamen_ \
REF: [EDPG]\: 495

### <a id="swana">\*swana- [m.] "swan" (n)</a>
N: \
-- ON _svanr_ \
-- Far. _svanur_ \
-- Elfd. _swan_ \
W: \
-- OE _swan_, _swon_, E _swan_ \
-- OS _swan_ \
-- Du. _zwaan_ [c.] \
-- OHG _swan_, G _Schwan_ \
REF: [EDPG]\: 496

### <a id="swanhan">\*swanhan- "to swing" (v)</a>
W: \
-- E _to sway_ \
-- OFri. _swaaie_ [wk.] \
-- Du. _zwaaien_ [wk.] \
REF: [EDPG]\: 496

### <a id="swarjan">\*swar(j)an- [st.] "to swear" (v)</a>
E: \
-- Go. _swaran_ \
N: \
-- ON _sverja_ \
-- Far. _svørja_ \
-- Elfd. _swera_ \
W: \
-- OE _swerian_, E _to swear_ \
-- OFri. _swera_, _swara_, _swora_ \
-- OS _swerian_ \
-- Du. _zweren_ \
-- OHG _swerren_, G _schwören_ \
REF: [EDPG]\: 496

### <a id="swe">\*swe- "self-" (pref)</a>
E: \
-- Go. _swi-kunps_, _swe-kunps_ "manifest, evident" \
N: \
-- ON _své-dauðr_ "suddenly-dead" \
REF: [EDPG]\: 496

### <a id="swē">\*swē "as, like; so" (adv)</a>
E: \
-- Go. _swe_ \
N: \
-- ON _svá_ \
-- Far. _so_ \
-- Elfd. _så_ \
W: \
-- OE _swā_, _swǣ_, _swē_, E _so_ \
-- OFri. _sā_ \
-- OS _sō_ \
-- Du. _zo_ \
-- OHG _sō_, G _so_ \
REF: [EDPG]\: 496

### <a id="swēbjan">\*swēbjan- [wk.] "to cause to sleep" (v)</a>
N: \
-- ON _svæfa_, _sœfa_ "to lull to sleep; to calm down" \
REF: [EDPG]\: 497

### <a id="swebla">\*swebla- [m.] "sulfur" (n)</a>
E: \
-- Go. _swibls_ \
W: \
-- OE _swefl_ \
-- MDu. _swavel_, _swevel_, Limb. _swegel_, Du. _zwavel_ [c.] \
-- OHG _swebal_, _swefal_, G _Schwefel_ \
REF: [EDPG]\: 497

### <a id="swefna">\*swefna- [m.] "sleep" (n)</a>
N: \
-- ON _svefn_ "sleep; dream" \
-- Far. _svøvnur_ "sleep; numbness in limbs" \
-- Elfd. _swemmen_ "id." \
W: \
-- OE _swefn_ [n.] "sleep; dream" \
-- OS _sweƀan_ \
REF: [EDPG]\: 497

### <a id="swēgura">\*swēgura- [m.] "brother-in-law" (n)</a>
W: \
-- MDu. _swager_, Du. _zwager_ [c.] \
-- MHG _swāger_, G _Schwager_ \
REF: [EDPG]\: 498

### <a id="swegru">\*swegru- [f.] "mother-in-law" (n)</a>
W: \
-- OE _sweger_ \
-- OHG _swigur_, G _Schwieger_ \
REF: [EDPG]\: 498

### <a id="swehran">\*swehra(n)- [m.] "father-in-law" (n)</a>
E: \
-- Go. _swaihra_ \
N: \
-- OSw. _svǣr_ \
-- Elfd. _swäre_ \
W: \
-- OE _swehor_, _swēor_ \
-- MDu. _swere_, _sweer_, _swaer_ \
-- OHG _swehur_, _sweher_, G _Schwäher_ \
REF: [EDPG]\: 498

### <a id="swelan">\*swelan- [st.] "to burn" (v)</a>
W: \
-- OE _swelan_ "to burn, perish with heat" \
REF: [EDPG]\: 498

### <a id="sweljan">\*sweljan-(?) [m.] "sister's husband" (n)</a>
N: \
-- ON _svili_ \
REF: [EDPG]\: 498

### <a id="swellan">\*swellan- [st.] "to swell" (v)</a>
N: \
-- ON _svella_ \
-- Far. _svølla_ \
-- Elfd. _swella_ \
W: \
-- OE _swellan_, E _to swell_ \
-- OFri. _swella_ \
-- OS _swellan_ \
-- Du. _zwellen_ \
-- OHG _swellan_, G _schwellen_ \
REF: [EDPG]\: 499

### <a id="sweltan">\*sweltan- [st.] "to dwindle, starve, perish" (v)</a>
E: \
-- Go. _swiltan_ "to die, lie dying" \
N: \
-- ON _svelta_ "to die; to starve, suffer hunger" \
-- Far. _svølta_ "to starve" \
-- Elfd. _swelta_ "id." \
W: \
-- OE _sweltan_, _swiltan_, _swyltan_ "to perish" \
-- OS _sweltan_ "to die" \
-- MDu. _swelten_ "to languish, pine, crave" \
-- OHG _swelzan_ "to burn", MHG _swelzen_ "id." \
REF: [EDPG]\: 499

### <a id="swēra">\*swēra- "heavy" (adj)</a>
E: \
-- Go. _swers_ "dignified" \
N: \
-- ON _svárr_ "heavy, grave" \
-- Far. _svárur_ "hard, distressing, painful" \
W: \
-- OE _swǣr_ "difficult" \
-- OFri. _swēr_ "hard, difficult; pregnant" \
-- OS _swār_ "heavy, glorious" \
-- Du. _zwaar_ \
-- OHG _swār_, _swari_, G _schwer_ \
REF: [EDPG]\: 499

### <a id="sweran">\*sweran- [st.] "to ulcerate, gather" (v)</a>
W: \
-- MDu. _sweren_ [wk.], Du. _zweren_ \
-- OHG _sweran_ [wk.], G _schwären_ \
REF: [EDPG]\: 499

### <a id="swerda">\*swerda- [n.] "sword" (n)</a>
N: \
-- ON _sverð_ \
W: \
-- OE _sweord_, _swurd_, E _sword_ \
-- OFri. _swerd_ \
-- ODu. _swert_, Du. _zwaard_ \
-- OHG _swert_, G _Schwert_ \
REF: [EDPG]\: 499

### <a id="swerkan">\*swerkan- [st.] "to darken, become gloomy/sad" (v)</a>
W: \
-- OE _sweorcan_ \
-- OS _swerkan_ \
-- MDu. _swerken_ [st./wk.] \
REF: [EDPG]\: 500

### <a id="swester">\*swester- [f.] "sister" (n)</a>
E: \
-- Go. _swistar_ \
N: \
-- ON _systir_ \
-- Far. _systir_ \
-- Elfd. _syster_ \
W: \
-- OE _sweostor_, _suster_, E _sister_ \
-- OFri. _swester_ \
-- OS _swestar_, _sustar_ \
-- Du. _zuster_ \
-- OHG _swestar_, _soster_, G _Schwester_ \
REF: [EDPG]\: 500

### <a id="sweþan">\*sweþan- [st.] "to burn (with smoke)" (v)</a>
W: \
-- OHG _swedan_ \
REF: [EDPG]\: 500

### <a id="swīban">\*swīban- [st.] "to sway, swerve" (v)</a>
N: \
-- ON _svífa_ "to ramble; to swerve, drift; to bustle" \
-- Far. _svíva_ "to float, hang, swing" \
W: \
-- OE _swīfan_ "to move, wend, sweep" \
-- OFri. _swīva_ "to deviate" \
-- MDu. _swiven_ [wk.] "to move, remove" \
-- MHG _swīfen_ "to move, swing" \
REF: [EDPG]\: 500

### <a id="swīgēn">\*swīgēn- [wk.] "to be silent" (v)</a>
W: \
-- OE _swīgian_, _sugian_ \
-- OFri. _swīgia_ \
-- OS _swīgon_ \
-- Du. _zwijgen_ [st.] \
-- OHG _swīgēn_, G _schweigen_ [st.] \
REF: [EDPG]\: 501

### <a id="swiglōjan">\*swiglōjan- [wk.] "to play the flute" (v)</a>
E: \
-- Go. _swiglon_ \
W: \
-- EDu. _sweghelen_ \
-- OHG _swegalōn_, MHG _swegeln_ \
REF: [EDPG]\: 501

### <a id="swimman">\*swimman- [st.] "to swim, float" (v)</a>
N: \
-- ON _swimma_, _svima_, _symja_ \
-- Far. _svimja_ \
-- Elfd. _simma_ \
W: \
-- OE _swimman_, E _to swim_ \
-- MDu. _swemmen_ "to swim, float", Du. _zwemmen_ "to swim" \
-- OHG _swimman_, G _schwimmen_ \
REF: [EDPG]\: 501

### <a id="swīna">\*swīna- [n.] "pig" (n)</a>
E: \
-- Go. _swein_ \
N: \
-- ON _svín_ \
-- Far. _svín_ \
-- Elfd. _swain_ \
W: \
-- OE _swīn_, E _swine_ \
-- OFri. _swīn_ \
-- OS _swīn_ \
-- Du. _zwijn_ \
-- OHG _swīn_, G _Schwein_ \
REF: [EDPG]\: 502

### <a id="swīnan">\*swīnan- [st.] "to dwindle, decrease" (v)</a>
N: \
-- ON _svína_ "to go down, subside" \
W: \
-- OHG _swīnan_ "id.", MHG _swīnen_ "id." \
REF: [EDPG]\: 502

### <a id="swippōn">\*swipp/bōn- [wk.] "to sway, swerve" (v)</a>
N: \
-- ON _svipa_ "to move swiftly, shift; to whip" \
W: \
-- OE _swipian_ "to strike, scourge, lash" \
-- G _schwippen_ "to sway" \
REF: [EDPG]\: 502

### <a id="switjan">\*switjan- [wk.] "to sweat" (v)</a>
W: \
-- OHG _swizzen_, G _schwitzen_ \
REF: [EDPG]\: 502

### <a id="swīþan">\*swīþan- [st.] "to burn, ache" (v)</a>
N: \
-- ON _svíða_ "to singe, burn" \
-- Far. _svíða_ "id." \
-- Elfd. _swaiða_ "id." \
REF: [EDPG]\: 502

### <a id="swōra">\*swōra- [m.] "oath" (n)</a>
W: \
-- MHG _swuor_, G _Schwur_ \
REF: [EDPG]\: 503

### <a id="swōtu">\*swōtu- "sweet" (adj)</a>
N: \
-- ON _sœtr_ \
-- Far. _søtur_ \
-- Elfd. _syöt_ \
W: \
-- OE _swōt_, _swēte_, E _sweet_ \
-- OFri. _swēte_ \
-- OS _swōti_ \
-- Du. _zoet_ \
-- OHG _swuozi_, _suozi_, G _süß_ \
REF: [EDPG]\: 503

### <a id="swulli">\*swul(l)i- [m.] "boil" (n)</a>
N: \
-- ON _sullr_ \
-- Far. _sullur_, _svullur_ \
W: \
-- OE _swyle_ \
REF: [EDPG]\: 503

### <a id="swulti">\*swulti- [m.] "starvation" (n)</a>
N: \
-- ON _sultr_ "hunger" \
-- Far. _svultur_ "id." \
W: \
-- OE _swylt_ "death" \
REF: [EDPG]\: 503

### <a id="swurrōn">\*swurrōn- [wk.] "to buzz, crackle(?)" (v)</a>
N: \
-- Icel. _surra_ "to creak" \
W: \
-- MLG _surren_ "to buzz" \
REF: [EDPG]\: 503

## T

### <a id="tabjan">\*tabjan- [wk.] "to delay, hinder" (v)</a>
N: \
-- ON _tefja_ "to hinder, delay" \
REF: [EDPG]\: 504

### <a id="tafna">\*tafna- [n.] "sacrificial meat" (n)</a>
N: \
-- ON _tafn_ \
REF: [EDPG]\: 504

### <a id="tagla">\*tagla- [n.] "hair" (n)</a>
E: \
-- Go. _tagl_ \
N: \
-- ON _tagl_ "horse's hair, tail" \
W: \
-- OE _tægl_ [m.] "id.", E _tail_ \
-- MLG _tagel_ "end of a rope" \
-- OHG _zagal_ [m.] "tail, sting, penis", MHG _zagel_ [m.] "id." \
REF: [EDPG]\: 504

### <a id="tagra">\*tagra- ~ \*trahna- [n.] "tear" (n)</a>
E: \
-- Go. _tagr_ \
N: \
-- ON _tár_ \
-- Far. _tár_ \
-- Elfd. _tår_ \
W: \
-- OE _tæhher_, _tēar_, _teagor_ [m.], E _tear_ \
-- OFri. _tār_ [m./n.] \
-- OS _trahan_ [m.] \
-- Du. _traan_ [c.] \
-- OHG _trahan_, _zahar_ [m.], MHG _traher_, _zaher_ [m.], G _Trähne_, obs. _Zähre_ [f.] \
REF: [EDPG]\: 504

### <a id="tahjan">\*tahjan- [wk.] "to tear" (v)</a>
E: \
-- Go. _tahjan_ "lacerate, tear" \
N: \
-- Icel. _tæja_ "to card" \
-- Far. _tæ(g)a_ "to pluck loose; to unravel; to tear into bits and pieces" \
-- Nw. _tæja_ "to unravel, pluck" \
REF: [EDPG]\: 505

### <a id="taihwōn">\*taihwō(n)- ~ \*taiwō(n)- [f.] "toe" (n)</a>
N: \
-- ON _tá_, pl. _tær_ \
-- Far. _tá_, pl. _tær_, dat. _tóm_ \
-- Elfd. _ton_, pl. _töner_ \
W: \
-- OE _tā(he)_, E _toe_ \
-- MLG _tē(n)_, _tēwe_ \
-- Du. _teen_ [c.] \
-- OHG _zēha_, G _Zehe_ \
REF: [EDPG]\: 505

### <a id="taikōn">\*taik/gōn- [wk.] "to show" (v)</a>
W: \
-- OHG _zeigōn_, G _zeigen_, Swi. Visp. _zeixxu_ \
REF: [EDPG]\: 505

### <a id="taikjan">\*taikjan- [wk.] "to show" (v)</a>
W: \
-- OE _tǣcan_ "to show", E _to teach_ \
REF: [EDPG]\: 505

### <a id="taikna">\*taikna- [n.] "sign" (n)</a>
N: \
-- ON _teikn_ \
W: \
-- OE _tācn_ \
-- OS _tēkan_ \
-- Du. _teken_ \
-- MHG _zeichan_, G _Zeichen_ \
REF: [EDPG]\: 506

### <a id="taikwer">\*taikwer- [n.] "brother-in-law" (n)</a>
W: \
-- OE _tācor_, _tācur_ [m.] "husband's brother, brother-in-law" \
-- OFri. _tāker_ [m.] "id." \
-- OHG _zeihhur_, _zeichur_, _zeihhor_ [m.], MHG _zeicher_ [m.] \
REF: [EDPG]\: 506

### <a id="tailja">\*tailja- [m.] "teal" (n)</a>
W: \
-- ME _tēl_, E _teal_ \
-- MLG _tēlink_ \
-- MDu. _teling_, _teiling_, EDu. _teelingh_, Du. _taling_ [c.] \
REF: [EDPG]\: 506

### <a id="taina">\*taina- [m.] "sprig" (n)</a>
E: \
-- Go. _tains_ "branch, shoot, twig" \
N: \
-- ON _teinn_ "twig; spit; stake" \
W: \
-- OE _tān_ "twig, sprout, shoot" \
-- MLG _tēn_ "thin metal rod; shaft of an arrow" \
-- MDu. _teen_ "willow twig", Du. _teen_ [c.] "id." \
-- OHG _zein_ "twig, stick, ruler, shaft, pipe, bar (of metal)", MHG _zein_ "stick, twig, staff" \
REF: [EDPG]\: 506

### <a id="tairi">\*tairi- "clear" (adj)</a>
N: \
-- ON _tærr_ "clear (of water)" \
-- Nw. _tær_ "clean" \
REF: [EDPG]\: 506

### <a id="taisan">\*taisan- [st.] "to tousle" (v)</a>
W: \
-- ME _tōsen_ [wk.] "to tease (wool, flax, etc.), card; to pull apart, to pluck" \
-- OHG _zeisan_ "to pull, tousle, pluck", MHG _zeisen_ "id." \
REF: [EDPG]\: 507

### <a id="taisjan">\*taisjan- [wk.] "to pluck, card, prick" (v)</a>
N: \
-- Sw. _tesa_ "pull, to pluck (of wool)" \
W: \
-- OE _tǣsan_ "to pluck, card", E _to tease_ \
-- MLG _tēsen_ "to pluck, scratch" \
-- MDu. _tesen_ "to pluck, jerk; to point something sharp at someone" \
REF: [EDPG]\: 507

### <a id="takan">\*takan- ~ \*tēkan- [st.] "to touch" (v)</a>
E: \
-- Go. _tekan_ \
N: \
-- ON _taka_ "to take; to reach, touch" \
-- Far. _taka_ "id." \
-- Elfd. _tågå_ "to take; to hit" \
W: \
-- MDu. _taken_ [wk.] "to take, seize" \
REF: [EDPG]\: 507

### <a id="tala">\*tala- [n.] "number; speech" (n)</a>
N: \
-- ON _tal_ "talk; speech" \
-- Far. _tal_ "number; speech_, conversation" \
-- Elfd. _tal_ "number; talk(ing)" \
W: \
-- OE _tæl_, _ge-tæl_ "number; reckoning" \
-- OS _gi-tal_ "number, row" \
-- Du. _ge-tal_ "number" \
REF: [EDPG]\: 508

### <a id="talga">\*talga/ō- [m./n./f.] "tallow" (n)</a>
N: \
-- Icel. _tólg_ [f.] \
-- Far. _talg_ [f.] \
-- Nw. _talg_, _tolg_ [f.] \
-- Elfd. _tåg_ [m.] \
W: \
-- OE _tælg_ [m.] "dye", E _tallow_ \
-- MLG _talch_ [n.] \
-- MDu. _talch_ [n.], Du. _talg_ [n.] \
REF: [EDPG]\: 508

### <a id="talōn">\*talō(n)- [f.] "speech, recount" (n)</a>
N: \
-- ON _tala_ "speech; number" \
W: \
-- OE _talu_ "recount, story", E _tale_ \
-- OFri. _tele_ "number; story" \
-- MDu. _tale_ "conversation; speech", Du. _taal_ [c.] "language" \
-- OHG _zala_ "number; calculation", G _Zahl_ "number, amount" \
REF: [EDPG]\: 508

### <a id="tamjan">\*tamjan- [wk.] "to tame" (v)</a>
E: \
-- Go. _ga-tamjan_ "tame, subdue" \
N: \
-- ON _temja_ "to tame; train" \
W: \
-- OE _temian_ \
-- MLG _temen_, _temmen_ \
-- Du. _temmen_ \
-- OHG _zemman_, MHG _zemen_, G _zähmen_ \
REF: [EDPG]\: 508

### <a id="tandjan">\*tandjan- [wk.] "to kindle" (v)</a>
E: \
-- Go. _tandjan_ \
N: \
-- ON _tenda_ \
W: \
-- OE _on-tendan_, ME _on-tenden_ \
REF: [EDPG]\: 508

### <a id="tandran">\*tandra(n)- [m.] "ember" (n)</a>
N: \
-- ON _tandri_ "fire, spark" \
-- Far. _tandur_ [n.] "weakly burning light" \
W: \
-- OHG _zantaro_, MHG _zanter_, _zander_ \
REF: [EDPG]\: 509

### <a id="tangan">\*tangan- [m.] "tip, blade(?)" (n)</a>
N: \
-- ON _tangi_ "point projecting into the sea, spit of land; pointed end by which the blade is driven into the handle" \
-- Far. _tangi_ \
-- Nw. _tange_ \
REF: [EDPG]\: 509

### <a id="tangja">\*tangja- "approximate" (adj)</a>
W: \
-- OE _ge-tenge_ "close to, pressing upon" \
-- OS _bi-tengi_ "sticking, linked" \
-- OHG _gi-zengi_ "near, close" \
REF: [EDPG]\: 509

### <a id="tangō">\*tangō- [f.] "tongs" (n)</a>
N: \
-- ON _tǫng_, pl. _tangir_, _tengr_ \
-- Far. _tong_, pl. _tengur_ \
W: \
-- OE _tang(e)_, E _tongs_ \
-- OFri. _tange_ \
-- MDu. _tange_, Du. _tang_ [c.] \
-- OHG _zanga_, G _Zange_ \
REF: [EDPG]\: 509

### <a id="tangra">\*tangra- "sharp, acute" (adj)</a>
W: \
-- MDu. _tanger_ \
-- OHG _zangar_, MHG _zanger_ \
REF: [EDPG]\: 509

### <a id="tanhu">\*tanhu- "tough" (adj)</a>
W: \
-- OE _tōh_, E _tough_ \
-- MLG _tā_ \
-- MDu. _taey_, Du. _taai_ \
-- OHG _zāhi_, MHG _zæhe_, _zæh_, _zæch_, G _zähe_ \
REF: [EDPG]\: 509

### <a id="tanþ">\*tanþ- ~ \*tunþ- [m./f.] "tooth" (n)</a>
E: \
-- Go. _tunþus\*_ [m.] \
N: \
-- ON _tǫnn_, pl. _teðr_ [f.] \
-- Far. _tonn_, pl. _tenn_ [f.] \
-- Elfd. _tann_, pl. _tenner_ [f.] \
W: \
-- OE _tōð_ [m.], E _tooth_ \
-- OFri. _tōth_ [m.] \
-- OS _tand_ [m.] \
-- MDu. _tan_, _tant_ [m.], Du. _tand_ [c.] \
-- OHG _zan_, _zant_ [m.], G _Zahn_ [m.] \
REF: [EDPG]\: 509

### <a id="tappōn">\*tapp/bōn- [wk.] "to lose" (v)</a>
N: \
-- ON _tapa_ \
-- OSw. _tappa_, _tapa_ \
-- Elfd. _tappa_ \
REF: [EDPG]\: 510

### <a id="tarbjan">\*tarbjan- [wk.] "to roll" (v)</a>
W: \
-- OHG _zerben_ "to turn, roll", MHG _zirben_ "to circle, whirl" \
REF: [EDPG]\: 510

### <a id="targjan">\*targjan- [wk.] "to vex" (v)</a>
N: \
-- Elfd. _tjärga_ "to nag, carp" \
W: \
-- OE _tiergan_ "to irritate" \
-- MDu. _tergen_, _torgen_ "to tease, provoke", Du. _tergen_ "id." \
-- G _zergen_ "to pester, provoke" \
REF: [EDPG]\: 510

### <a id="targōn">\*targōn- [f.] "edge" (n)</a>
N: \
-- ON _targa_ "target, small round shield" \
W: \
-- OE _targe_, _targa_ [m.] "small shield" \
-- OHG _zarga_ "edge, rim", MHG _zarge_ "id." \
REF: [EDPG]\: 510

### <a id="tarhjan">\*tarhjan- [wk.] "to mark" (v)</a>
E: \
-- Go. _ga-tarhjan_ "to expose, make public, reveal" \
REF: [EDPG]\: 510

### <a id="tauhōn">\*tauhōn- [f.] "bitch" (n)</a>
N: \
-- Icel. _tófa_ "vixen" \
W: \
-- MLG _tō_ \
-- OHG _zō(h)a_, MHG _zohe_ \
REF: [EDPG]\: 511

### <a id="taujan">\*taujan- [wk.] "to do, make" (v)</a>
E: \
-- Go. _taujan_ \
W: \
-- OE _tawian_ "to prepare, make ready", E _to taw_ \
-- MLG _touwen_ "to dress (furniture etc.), tan" \
-- MDu. _touwen_ "to make, prepare (esp. leather)", Du. _vol-tooien_ "to complete" \
-- OHG _zawen_, _zowen_ "to succeed", MHG _zouwen_, _zöuwen_ "to make, treat, produce" \
REF: [EDPG]\: 511

### <a id="tauma">\*tauma- [m.] "rein, bridle, rope" (n)</a>
N: \
-- ON _taumr_ \
-- Far. _teymur_ \
-- Elfd. _tom_ \
W: \
-- OE _tēam_ \
-- OFri. _tām_ \
-- ODu. _tōm_, Du. _toom_ [c.] \
-- OHG _zoum_, G _Zaum_ \
REF: [EDPG]\: 511

### <a id="taunōn">\*taunōn- [wk.] "to show" (v)</a>
W: \
-- MDu. _tonen_, Du. _tonen_ \
-- MHG _zounen_ \
REF: [EDPG]\: 511

### <a id="tawō">\*tawō- [f.] "pair" (n)</a>
N: \
-- ON _tá_ "two people together, pair, couple" \
REF: [EDPG]\: 511

### <a id="tawwa">\*tawwa- [n.] "rope" (n)</a>
W: \
-- OS _tou_ \
-- MDu. _tou_, Du. _touw_ \
REF: [EDPG]\: 511

### <a id="tegu">\*tegu- [m.] "a number of ten" (n)</a>
E: \
-- Go. _tigus_ \
N: \
-- ON _tigr_ \
-- Elfd. _tjäg_ \
REF: [EDPG]\: 512

### <a id="tehswōn">\*tehswō(n)- [f.] "right hand" (n)</a>
E: \
-- Go. _taihswa_, -o \
W: \
-- OS _tesewa_ \
-- OHG _zesawa_, MHG _zesewe_, _zeswe_ \
REF: [EDPG]\: 512

### <a id="tehun">\*tehun- "ten" (num)</a>
E: \
-- Go. _taihun_ \
N: \
-- ON _tíu_ \
-- Far. _tíggju_ \
-- Elfd. _tiu_ \
W: \
-- OE _tīen_, E _ten_ \
-- OFri. _tiān_, _tiēn_ \
-- OS _tehun_ \
-- Du. _tien_ \
-- OHG _zehan_, G _zehn_ \
REF: [EDPG]\: 512

### <a id="tehunþan">\*tehunþ/dan- "tenth" (num)</a>
E: \
-- Go. _taihunda_ \
N: \
-- ON _tíundi_ \
-- Far. _tíggjundi_ \
-- Elfd. _tiund_ \
W: \
-- OE _tēoða_, _teogeða_, E _tenth_ \
-- OFri. _tiānda_, _tiēnda_ \
-- OS _tehando_ \
-- Du. _tiende_ \
-- OHG _zehanto_, G _zehnte_ \
REF: [EDPG]\: 512

### <a id="telda">\*telda- [n.] "drape, tent" (n)</a>
N: \
-- ON _tjald_ "tent; hangings, tapestry" \
W: \
-- OE _teld_, _ge-teld_ "tent, pavilion" \
-- MDu. _ge-telt_, _telt_ "id." \
-- OHG _gi-zelt_, _zelt_ "id.", G _Zelt_ "id." \
REF: [EDPG]\: 512

### <a id="teman">\*teman- [st.] "to suit, befit" (v)</a>
E: \
-- Go. _ga-timan_ "to be congruent with, match" \
W: \
-- OFri. _timia_ [wk.] "to befit" \
-- OS _gi-teman_ "id." \
-- OHG _zeman_ "id.", MHG _zemen_ "id.", G _ziemen_ [wk.] "id." \
REF: [EDPG]\: 513

### <a id="tēmi">\*tēmi- "suitable, agreeable" (adj)</a>
E: \
-- Go. _ga-temiba_ "congruently, appositely" (adv) \
W: \
-- MLG _be-tāme_ "appropriate" \
-- MDu. _be-tame_, _be-taem_ "appropriate, agreeable" \
-- OHG _gi-zāmi_ "decent, pleasant", MHG _ge-zæme_ "befitting, appropriate" \
REF: [EDPG]\: 513

### <a id="teran">\*teran- [st.] "to tear" (v)</a>
E: \
-- Go. _tairan_ \
W: \
-- OE _teran_, E _to tear_ \
-- OHG _zeran_, _fir-zeran_ "to cut off, finish, fight", MHG _zer-zeren_ "to destroy" \
REF: [EDPG]\: 513

### <a id="ternō">\*ternō- [f.] "pond" (n)</a>
N: \
-- ON _tjǫrn_ \
-- Far. _tjørn_ \
REF: [EDPG]\: 513

### <a id="tersa">\*tersa- [m.] "peg; penis" (n)</a>
W: \
-- OE _teors_, E _tarse_ \
-- MLG _ters_ \
-- MDu. _teers_, Du. _taas_ [c.] \
-- OHG _zers_ "twig; penis", MHG _zers_ "penis" \
REF: [EDPG]\: 513

### <a id="terwan">\*terwa/ōn- [n./f.] "tar" (n)</a>
N: \
-- ON _tjara_ [f.] \
W: \
-- OE _teoro_, _teru_ [n.], E _tar_ \
-- MDu. _ter(re)_, _tar_ [n.], Du. _teer_ [n./c.] \
REF: [EDPG]\: 514

### <a id="terwōn">\*terwōn- [f.] "tare(s)" (n)</a>
W: \
-- ME _tare_ "wild or cultivated vetch, tare; vetch seed", pl. "weeds growing in grain fields, darnel", E _tare_ \
-- NFri. (Wiedingharde) _tāre_ [m./n.] "buckwheat" \
-- MDu. _tarwe_, _terwe_ "wheat", Du. _tarwe_ [c.] "id." \
REF: [EDPG]\: 514

### <a id="tetru">\*tetru- [m.] "tetter" (n)</a>
W: \
-- OE _teter_, _tetr_, E _tetter_ \
-- OHG _zitaroh_, MHG _ziteroch_ (< \*tetruka-), G _Zitter-mal_ \
REF: [EDPG]\: 514

### <a id="teudra">\*teudra- ~ \*tūdra- [n.] "tether" (n)</a>
N: \
-- ON _tjóðr_ \
-- Far. _tjóður_ \
W: \
-- ME _teder_, _tether_ (« ON?), E _tether_ \
-- OFri. _tiāder_, _tzāder_, _tzār_ \
-- MDu. _tuder_, _tudder_ [m.], EDu. _tuyer_ \
-- OHG _zeotar_ [m./n.], MHG _zieter_ [m./n.] "front shaft of a carriage" \
REF: [EDPG]\: 514

### <a id="teuhan">\*teuhan- [st.] "to pull" (v)</a>
E: \
-- Go. _tiuhan_ "to lead, guide" \
N: \
-- ON _toginn_ ptc. "drawn" \
W: \
-- OE _tēon_ \
-- OS _tiohan_ "to pull, educate" \
-- MDu. _tien_ "to pull, draw; to proceed", Du. _tijgen_ "to proceed" \
-- OHG _ziohan_ "to lead, bring (up), educate", MHG _ziehen_ "lead, bring, retreat", G _ziehen_ "id." \
REF: [EDPG]\: 515

### <a id="teuna">\*teuna- [n.] "damage" (n)</a>
N: \
-- ON _tjón_ [f./n.] "damage, loss" \
W: \
-- OE _tēon_ "hurt, damage; insult, abuse" \
REF: [EDPG]\: 515

### <a id="tēwō">\*tēwō- [f.] "order" (n)</a>
E: \
-- Go. _tewa_ "order, arrangement" \
W: \
-- OHG _zāwa_ "coloration" \
REF: [EDPG]\: 515

### <a id="tibbōn">\*tib(b)ōn- [f.] "bitch" (n)</a>
N: \
-- Icel. _tebba_ "vixen" \
W: \
-- OE _tife_ "id." \
-- MDu. _teve_ "id.", Du. _teef_ [c.] "id." \
-- G dial. _Zippe_ "bitch; ewe" \
REF: [EDPG]\: 515

### <a id="tibra">\*tibra- [n.] "sacrifice" (n)</a>
E: \
-- Go. _aibr_ "sacrificial animal" \
W: \
-- OE _tiber_, _tifer_ "sacrifice" \
-- OHG _zebar_ "offering" \
REF: [EDPG]\: 516

### <a id="tīdi">\*tīdi- [f.] "time" (n)</a>
N: \
-- ON _tíð_ "time; hour; divine service" \
-- Far. _tíð_ "time; moment; period" \
W: \
-- OE _tīd_ "time, season, hour", E _tide_ \
-- OFri. _tīd_ [m./f.] "time, hour" \
-- OS _tīd_ \
-- Du. _tijd_ [c.] \
-- OHG _zīt_ [f./n.] "age, time; time of prayer", G _Zeit_ "time" \
REF: [EDPG]\: 516

### <a id="tīgan">\*tīgan- ~ \*tikkan- [m.] "tick" (n)</a>
N: \
-- Nw. _tikk_, _tikke_ [f.] \
W: \
-- OE _ticia_ "ricinus", E _tick_ \
-- MLG _teke_ "id." \
-- MDu. _teke_ [f.] "id.", Du. _teek_, dial. _tijk_, _tijg_ "id." \
-- MHG _zecke_ [m./f.] "id.", G _Zecke_, dial. _Zeche_ [f.] "id." \
REF: [EDPG]\: 516

### <a id="tigōn">\*tigōn- [f.] "goat" (n)</a>
W: \
-- OHG _ziga_, G _Ziege_ \
REF: [EDPG]\: 516

### <a id="tīhan">\*tīhan- [st.] "to point out" (v)</a>
E: \
-- Go. _ga-teihan_ "to announce, tell, report" \
N: \
-- ON _téa_, _tjá_ pret. _téða_, _tjáða_, ptc. _tiginn_ "to show, proclaim" \
W: \
-- OE _tēon_ "to accuse" \
-- OS _af-tīhan_ "to fail" \
-- MDu. _tien_ "to accuse" \
-- OHG _zīhan_ "id.", MHG _zīhen_ "id." \
REF: [EDPG]\: 517

### <a id="tila">\*tila- [n.] "planned point in time" (n)</a>
E: \
-- Go. _til_ [m./n.] "opportunity" \
N: \
-- ON _til_ "to" (prep) \
-- Far. _til_ "id." (prep) \
W: \
-- OE _til_ "fitness, station, standing-place" \
-- OFri. _til_ "to" (prep) \
-- MLG _til_ "border, goal" \
-- MDu. _til_ [m./n.] "circumstance, case" \
-- OHG _zil_ "border, aim", G _Ziel_ "id." \
REF: [EDPG]\: 517

### <a id="tīlō">\*tīlō- [f.] "row" (n)</a>
W: \
-- WPhal. _tîle_, Du. dial. _tijl_ [c.] \
-- OHG _zīla_, G _Zeile_ \
REF: [EDPG]\: 517

### <a id="tīman">\*tīman- [m.] "time" (n)</a>
N: \
-- ON _tími_ "time; fit time; good luck" \
-- Far. _tími_ "hour" \
-- Elfd. _taime_ "id." \
W: \
-- OE _tīma_ "time, hour", E _time_ \
-- G Swi. _zīmmen_ "occasion" \
REF: [EDPG]\: 517

### <a id="timbra">\*timbra- [n.] "timber, lumber" (n)</a>
N: \
-- ON _timbr_ \
-- Elfd. _timber_ \
W: \
-- OE _timber_ "timber; building, structure" \
-- OS _timbar_ "carpenter's work" \
-- OHG _zimbar_ "timber", MHG _zimber_, _zimmer_ "timber, building", G _Zimmer_ "room" \
REF: [EDPG]\: 517

### <a id="tina">\*tina- [n.] "tin" (n)</a>
W: \
-- OE _tin_, E _tin_ \
-- OS _tin_, MLG _tin_, gen. _tenes_, _tinnes_ \
-- MDu. _tin_, _ten_, _teen_, Du. _tin_ \
-- OHG _zin_, G _Zinn_ \
REF: [EDPG]\: 517

### <a id="tinda">\*tinda- [m.] "jag, tip" (n)</a>
N: \
-- ON _tindr_ "spike, tooth; mountain-peak" \
-- Far. _tindur_ "id." \
W: \
-- OE _tind_ "prong, tooth" \
-- MHG _zint_ "prong; a wind instrument" \
REF: [EDPG]\: 518

### <a id="tingan">\*tingan- [st.] "to press upon, throng" (v)</a>
W: \
-- OE _ge-tingan_ "to press upon, throng" \
REF: [EDPG]\: 518

### <a id="tinnan">\*tinnan- [st.] "to burn" (v)</a>
W: \
-- MHG _zinnen_ "to burn, glow" \
REF: [EDPG]\: 518

### <a id="tīrēn">\*tīrēn- [wk.] "to shimmer, shine" (v)</a>
N: \
-- Nw. _tira_ \
-- Sw. dial. _tira_ \
REF: [EDPG]\: 518

### <a id="titrōn">\*titrōn- [wk.] "to tremble" (v)</a>
N: \
-- ON _titra_ [wk.2] "to twinkle" \
-- Icel. _titra_ "to quake, shiver, vibrate" \
W: \
-- OHG _zittarōn_ "to stagger", G _zittern_ "to tremble" \
REF: [EDPG]\: 518

### <a id="tittōn">\*tittōn- [f.] "tit, breast" (n)</a>
W: \
-- OE _tit_ [m.], E _tit_ \
-- WFri. _tit_, _titte_ [c.] \
-- MLG _titte_ [m./f.] \
-- MHG _zitze_, G _Zitze_ \
REF: [EDPG]\: 519

### <a id="tīwa">\*tīwa- [m.] "Tyr" (n)</a>
N: \
-- ON _Týr_ \
W: \
-- OE _Tīw_, _Tīg_ \
REF: [EDPG]\: 519

### <a id="tōba">\*tōba- [m./n.] "waiting(?)" (n)</a>
N: \
-- Icel. _tóf_ [n.] "hesitation" \
W: \
-- MDu. _toef_ [n.] "reception, treat, nursing" \
REF: [EDPG]\: 519

### <a id="tōgan">\*tōgan- ~ \*takkan- [m.] "branch" (n)</a>
N: \
-- Nw. _tagg(e)_ "tip, jag" \
-- OSw. _tagger_ "spike" \
-- Sw. _tagg_ [c.] "spine, spike" \
-- ODa. _tagge_ "prickle", Da. _tak_, _tag_ "jag, tip" \
W: \
-- ME _tak(ke)_ "clasp, nail", E _tack_, _tag_ \
-- OS _tōgo_, MLG _takke_, _tagge_, LG _take_ "prickle" \
-- MDu. _tac(ke)_, _tooch_, Du. _tak_, dial. _toeg_, _toek_ [c.] \
-- OHG _zuogo_, MHG _zacken_ pl. "jags", G _Zacke(n)_ "jag, prong" \
REF: [EDPG]\: 519

### <a id="tōja">\*tōja- [n.] "deed, act" (n)</a>
E: \
-- Go. _taui_, gen. _tojis_ \
REF: [EDPG]\: 520

### <a id="tōla">\*tōla- [n.] "tool" (n)</a>
N: \
-- ON _tól_ \
-- Far. _tól_ \
W: \
-- OE _tōl_, E _tool_ \
REF: [EDPG]\: 520

### <a id="tōma">\*tōma- "empty" (adj)</a>
N: \
-- ON _tómr_ \
-- Far. _tómur_ \
-- Elfd. _tuom_ \
REF: [EDPG]\: 520

### <a id="trabō">\*trabō- [f.] "fringe" (n)</a>
N: \
-- ON _trǫf_ \
W: \
-- MHG _trabe_ \
REF: [EDPG]\: 520

### <a id="trannjan">\*trannjan- [wk.] "to split" (v)</a>
W: \
-- Du. _tornen_ "to pull, jerk" \
-- MHG _trennen_ [st.] "to separate, to cut", G _trennen_ "id." \
REF: [EDPG]\: 520

### <a id="trappōn">\*trappōn- [wk.] "to trample" (v)</a>
W: \
-- MLG _trappen_ "to stamp" \
REF: [EDPG]\: 520

### <a id="trausta">\*trausta- "reliable" (adj)</a>
N: \
-- ON _traustr_ \
-- Far. _treystur_ "strong, solid, lasting" \
REF: [EDPG]\: 521

### <a id="tredan">\*tredan- ~ \*trudan- [st.] "to tread, trample" (v)</a>
E: \
-- Go. _trudan_ \
N: \
-- ON _troða_ "to tread; to cram, pack, stuff" \
-- Far. _troða_ "to tread, step" \
W: \
-- OE _tredan_, E _to tread_ \
-- OFri. _treda_ \
-- MDu. _treden_, _terden_, _tarden_, _torden_, Du. _treden_ \
-- OHG _tretan_, G _treten_ \
REF: [EDPG]\: 521

### <a id="tregan">\*tregan- [st.] "to be downcast" (v)</a>
W: \
-- OS _tregan_ "to grieve" \
REF: [EDPG]\: 521

### <a id="trēgi">\*trēgi- "slow, down" (adj)</a>
W: \
-- OS _trāg(i)_ "lazy" \
-- MDu. _traghe_, _traech_ "lazy, slow", Du. _traag_ "slow" \
-- OHG _trāgi_ "lazy, slow", G _träge_ "id." \
REF: [EDPG]\: 522

### <a id="trekkan">\*trek(k)an- [st.] "to pull" (v)</a>
W: \
-- MLG _treken_ [wk.] "to shudder" \
-- MDu. _tre(c)ken_ \
-- OHG _trehhan_, MHG _trechen_ "to shove, thrust, jerk" \
REF: [EDPG]\: 522

### <a id="treman">\*treman- [st.] "to waver" (v)</a>
W: \
-- MHG _tremen_ \
REF: [EDPG]\: 522

### <a id="trettōn">\*trett/dōn- ~ \*trutt/dōn- [wk.] "to trample" (v)</a>
N: \
-- Elfd. _troðå_ \
W: \
-- MDu. _tredden_, _tretten_ "to tread, trample" \
-- MHG _trot(t)en_ "to trot" \
REF: [EDPG]\: 522

### <a id="trewa">\*trewa- [n.] "tree" (n)</a>
E: \
-- Go. _triu_ "stick; vine" \
N: \
-- ON _tré_ "tree; mast; rafter, beam" \
-- Far. _træ_ "tree; piece of wood" \
-- Elfd. _trai_ "id." \
W: \
-- OE _trēo_, _trēow_, E _tree_ \
-- OFri. _trē_ \
-- OS _trio_, _treo_ \
REF: [EDPG]\: 522

### <a id="trewwu">\*trewwu- "loyal, trustworthy" (adj)</a>
E: \
-- Go. _triggws_ \
N: \
-- ON _tryggr_ \
-- Far. _tryggur_ "safe, calm" \
W: \
-- OE _trīewe_, _ge-trīewe_ "id." \
-- OFri. _triūwe_, _triōwe_ "id." \
-- OS _triuwi_ "id." \
-- MDu. _trouwe_, _ge-trouwe_ "id.", Du. _trouw_, _ge-trouw_ "loyal" \
-- OHG _triuwi_, _gi-triuwi_ "id.", G _treu_, _ge-treu_ "id." \
REF: [EDPG]\: 523

### <a id="trinnan">\*trinnan- [st.] "to separate oneself" (v)</a>
W: \
-- OHG _intrinnan_ "to separate oneself, to elope", MHG _trinnen_ "id." \
REF: [EDPG]\: 523

### <a id="trūēn">\*trūēn- [wk.] "to trust" (v)</a>
E: \
-- Go. _trauan_ \
N: \
-- ON _trúa_ "to believe; to believe in, trust" \
-- Far. _trúgva_ "id." \
W: \
-- OE _trūwian_ "to trust, confide" \
-- OFri. _triūwa_, _trōwa_ "to trust; to marry" \
-- OS _trauwian_ \
-- Du. _trouwen_ "to marry" \
-- OHG _trū(w)ēn_ "to trust, believe", G _trauen_ "id." \
REF: [EDPG]\: 523

### <a id="truga">\*truga- [n.] "trough" (n)</a>
N: \
-- ON _trog_ \
W: \
-- OE _trog_, _troh_ [m.], E _trough_ \
-- MDu. _troch_ [m.] "trough; boat", Du. _trog_ [c.] "trough" \
-- OHG _troc_ [m.], G _Trog_ [m.] \
REF: [EDPG]\: 523

### <a id="trukkōn">\*trukkōn- [wk.] "to pull" (v)</a>
W: \
-- MDu. _trocken_ "to pull, jerk" \
REF: [EDPG]\: 524

### <a id="truma-1">\*truma- [m.] "root(?)" (n)</a>
W: \
-- OE _wyrt-trum_ "root-stock" \
REF: [EDPG]\: 524

### <a id="truma-2">\*truma- "reliable" (adj)</a>
W: \
-- OE _trum_ "firm, strong" \
REF: [EDPG]\: 524

### <a id="trumpōn">\*trumpōn- [wk.] "to step(?)" (v)</a>
W: \
-- MDu. _trompen_ "to step, trample" \
-- MHG _trumpfen_ "to walk, toddle off" \
REF: [EDPG]\: 524

### <a id="trussōn">\*trussōn- [wk.] "to roll, turn" (v)</a>
W: \
-- WPhal. _trossen_ "to roll" \
REF: [EDPG]\: 524

### <a id="trūþa">\*trūþa- [m.] "joker" (n)</a>
N: \
-- ON _trúðr_ "juggler, fool" \
W: \
-- OE _trūð_ "trumpeter, actor, buffoon" \
REF: [EDPG]\: 524

### <a id="tuhti">\*tuhti- [f.] "pulling" (n)</a>
E: \
-- Go. _us-tauhts_ "completion" \
W: \
-- OE _tyht_ [m.] "conduct, practice" \
-- MDu. _tucht_ "discipline", Du. _tucht_ [c.] "id." \
-- OHG _zuht_ "pulling, tug, correction", G _Zucht_ "breeding, rearing; discipline" \
REF: [EDPG]\: 525

### <a id="tukkōn">\*tukk/gōn- [wk.] "to pull, jerk" (v)</a>
N: \
-- ON _toga_ "to drag" \
-- Elfd. _tugå_ "id." \
W: \
-- OE _togian_ "to tug", ME _toggen_ "id.", E _to tug_ \
-- MDu. _tocken_, _token_ "to push, jerk, strike", _togen_ "to pull, drag" \
-- OHG _zogōn_, _zockōn_, _zohhōn_ "to pull, jerk", MHG _zogen_ "to acquire, tear" \
REF: [EDPG]\: 525

### <a id="tulga">\*tulga- [m.] "tongue(?)" (n)</a>
W: \
-- G dial. _Zungen-zolch_ \
REF: [EDPG]\: 525

### <a id="tulgu">\*tulgu- "firm" (adj)</a>
E: \
-- Go. _tulgus_ "firm" \
W: \
-- OE _tylg_ "rather" (comp) (< \*tulg-iz) \
-- OS _tulgo_ "very" (adv) \
REF: [EDPG]\: 525

### <a id="tumfeti">\*tumfeti- [f.] "foundation" (n)</a>
N: \
-- ON _toft_, _tuft_ \
-- Icel. _tóft_ \
-- Far. _toft_ \
-- OSw. _tomt_, _tompt_ \
REF: [EDPG]\: 525

### <a id="tumþi">\*tumþi- [f.] "guild" (n)</a>
W: \
-- OHG _zumft_ "agreement, community", MHG _zumft_, _zunft_ "appropriateness, rule, association, guild", G _Zunft_ "craft, guild" \
REF: [EDPG]\: 526

### <a id="tūna">\*tūna- [n.] "fenced area" (n)</a>
N: \
-- ON _tún_ "enclosure, courtyard, homestead; home field; town" \
W: \
-- OE _tūn_ [m.] "enclosed piece of ground, yard; town", E _town_ \
-- OFri. _tūn_ [m.] "fence, enclosure, yard" \
-- MLG _tūn_ [m.] "fence" \
-- Du. _tuin_ [c.] "garden" \
-- OHG _zūn_ [m.] "fence, enclosure", G _Zaun_ [m.] "id." \
REF: [EDPG]\: 526

### <a id="tundra">\*tundra- [n.] "tinder" (n)</a>
N: \
-- ON _tundr_ \
W: \
-- OE _tynder_, _tyndre_ [f.] "tinder, fuel" (< \*tundrjō-) \
-- MLG _tunder_ "id." \
-- MHG _zunder_ [m./n.] "id.", G _Zunder_ [m.] "id." \
REF: [EDPG]\: 526

### <a id="tungla">\*tungla- [n.] "star, celestial body(?)" (n)</a>
E: \
-- Go. _tuggl_ "celestial body, star" \
N: \
-- ON _tungl_ "moon" (also _himin-tungl_ "heavenly body") \
-- Elfd. _tunggel_ "moon" \
W: \
-- OE _tungol_ [m./n.] "celestial body" \
-- OS _tungal_ "star" \
-- OHG _himil-zungal_ "celestial body" \
REF: [EDPG]\: 526

### <a id="tungōn">\*tungōn- [f.] "tongue" (n)</a>
E: \
-- Go. _tuggo_ \
N: \
-- ON _tunga_ \
-- Far. _tunga_ \
-- Elfd. _tungga_ \
W: \
-- OE _tunge_, E _tongue_ \
-- OFri. _tunge_ \
-- OS _tunga_ \
-- Du. _tong_ [c.] \
-- OHG _zunga_, G _Zunge_ \
REF: [EDPG]\: 526

### <a id="turba">\*turba- [m./n.] "turf, peat" (n)</a>
N: \
-- ON _torf_ [n.] \
W: \
-- MLG _torf_ [m.] \
-- MDu. _torf_, _turf_ [m.], Du. _turf_ [c.] \
REF: [EDPG]\: 527

### <a id="turhta">\*turhta- "bright" (adj)</a>
W: \
-- OE _torht_ "bright, splendid" \
-- OS _toroht_ "bright" \
-- OHG _zoraht_ "bright, clear" \
REF: [EDPG]\: 527

### <a id="turna">\*turna- [n./m.] "anger, rage" (n)</a>
W: \
-- OE _torn_ [n.] \
-- OS _torn_ [n.] \
-- Du. _toorn_ [c.] \
-- OHG _zorn_ [n.], G _Zorn_ [m.] \
REF: [EDPG]\: 527

### <a id="turnan">\*turnan- [st.] "to separate oneself, tear oneself away" (v)</a>
E: \
-- Go. _af-taurnan_ "to tear off", _dis-taurnan_ "to tear apart" \
W: \
-- Du. obs. _tornen__ [wk.] "to run off" \
REF: [EDPG]\: 527

### <a id="turþi">\*turþi- [f.] "destruction" (n)</a>
E: \
-- Go. _ga-taurþs_ \
REF: [EDPG]\: 527

### <a id="turzda">\*turzda- [m.] "excrement" (n)</a>
W: \
-- OE _tord_ [n.], E _turd_ \
-- MDu. _to(o)rt_ [m./n.] "dung" \
PIE:;> \*drs-dʰh₁-o- \
REF: [EDPG]\: 527

### <a id="tūsan">\*tūsan- [st.] "to pull" (v)</a>
W: \
-- ME -_tousen_ [wk.] "to pull, tear at; to dishevel", E _to touse_ \
-- LG _tūsen_ [wk.] "to pull apart" \
-- OHG _zir-zūsōn_ [wk.] "to tear to pieces, clear of undergrowth", MHG _zer-zūsen_ [wk.] "to tousle", G _zer-zausen_ [wk.] "id." \
REF: [EDPG]\: 527

### <a id="tuska">\*tuska- [m.] "tusk, fang" (n)</a>
W: \
-- OE _tusc_, _tux(a)_, E _tusk_ \
-- OFri. _tusk_, WFri. _tosk_ [c.] "tooth" \
REF: [EDPG]\: 528

### <a id="tuskan">\*tuska(n)- [m.] "toad(?), frog(?)" (n)</a>
N: \
-- Elfd. _tuosk_ "toad" \
W: \
-- OE _tosca_ "frog" \
REF: [EDPG]\: 528

### <a id="tuskjan">\*tuskjan- [wk.] "to be burning" (v)</a>
W: \
-- OHG _zuscen_ "to burn" \
REF: [EDPG]\: 528

### <a id="tussōn">\*tus(s)ōn- [wk.] "to fray" (v)</a>
N: \
-- Icel. _tos(s)a_ "to pull, tug" \
-- Nw. _tos(s)a_ "to jerk, pull, pluck" \
W: \
-- ME _tosen_, E obs. _to toze_ "to pull asunder" \
-- MLG _tōsen_ "to tear, tug" \
-- MDu. _tosen_ "to pick out" \
REF: [EDPG]\: 528

### <a id="tuttōn">\*tutt/dōn- [wk.] "to pull, drag" (v)</a>
N: \
-- Nw. _tot(t)a_ "to jerk, drag; to toil; to squeeze" \
-- Sw. dial. _tåtta_ "to squeeze" \
-- ODa. _tuttæ_, _tyttæ_ "to pull" \
W: \
-- Du. dial. _todden_, _tooien_ "to drag" \
-- MHG _zoten_ "to go slowly" \
REF: [EDPG]\: 528

### <a id="tuz">\*tuz- "apart, asunder" (pref)</a>
E: \
-- Go. _tuz-_ \
N: \
-- ON _tor-_ \
W: \
-- OE _tor_ \
-- OHG _zur-_, G _zer_- \
REF: [EDPG]\: 529

### <a id="twa">\*twa- "two" (num)</a>
E: \
-- Go. _twai_ [m.], _twos_ [f.] _twa_ [n.] \
N: \
-- ON _tveir_, acc. _tvá_, [f.] _tvær_, [n.] _tvau_, gen. _tveggja_, dat. _tveim(r)_ \
-- Far. _tveir_, [f.] _tvær_, [n.] _tvey_, gen. _tveggja_, dat. _tveimum_ \
-- Elfd. _twer_, acc. _tųo_, [f.] _twär_, [n.] _tau_, dat. _twem_ \
W: \
-- OE _twēgen_, [f.] _twā_, [n.] _tū_, E _two_ \
-- OFri. _twēne_, _twē_, _twēr(e)_, [f./n.] _twā_ \
-- OS _twēne_, _twēna_, [f.] _twā_, _twō_, [n.] _twē_ \
-- Du. _twee_ \
-- OHG _zwēne_, [f.] _zwā_, _zwō_, [n.] _zwei_, G _zwei_ \
REF: [EDPG]\: 529

### <a id="twalifa">\*twalifa- "twelve" (num)</a>
E: \
-- Go. _twalif_ \
N: \
-- ON _tolf_ \
-- Far. _tólv_ \
-- Elfd. _tolv_ \
W: \
-- OE _twelf_, E _twelve_ \
-- OFri. _twelif_, _toulif_, _toleve_, _tolewe_, _tolve_ \
-- OS _twelif_ \
-- Du. _twaalf_ \
-- OHG _zwelif_, G _zwölf_ \
REF: [EDPG]\: 529

### <a id="twīfla">\*twifla- [m.] "doubt" (n)</a>
E: \
-- Go. _tweifls\*_ \
W: \
-- OFri. _twīfel_, _twīwil_ [m./n.] \
-- MDu. _twivel_, Du. _twijfel_ [c.] \
-- OHG _zwīval_, G _Zweifel_ \
REF: [EDPG]\: 530

### <a id="twigan">\*twig/kkan- [m.] "twig" (n)</a>
N: \
-- EDa. _tvege_, Da. _tveje_, _tvege_, _tvegge_ \
W: \
-- OE Nhb. _twigge_ [f.], E _twig_ \
-- LG _twig_, WPhal. _twick_, _twiək_ [f.] \
-- OHG _zweck(o)_ "nail, peg", MHG _zwec_ "nail, bolt, twig", G _Zweck_ "id.", _Zwick_ "peg, sprout" \
REF: [EDPG]\: 530

### <a id="twis">\*twis "twice" (adv)</a>
W: \
-- MLG _twis_ \
REF: [EDPG]\: 530

### <a id="twiskja">\*twisk(j)a- "twofold" (adj)</a>
W: \
-- OS _twisk_ \
-- OHG _zwisk_, _zwiski_, MHG _zwisc(h)_ \
REF: [EDPG]\: 530

## Þ

### <a id="þa">\*þa- "that, those" (pron)</a>
E: \
-- Go. _sa_, acc. _þana_, [f.] _so_, acc. _þo_, [n.] _þata_ \
N: \
-- ON _sá_, acc. _þann_, [f.] _sú_, acc. _þá_, [n.] _þat_ \
-- Far. _tann_, [n.] _tað_ \
W: \
-- OE _se_, acc. _þone_, [f.] _sēo_, _sīo_, acc. _þā_, [n.] _þæt_, E _that_ \
-- OS _se_, _thē_, _thie_, acc. _thena_, _thana_, [f.] _thiu_, _thia_, acc. _thia_, _thea_, [n.] _that_ \
-- Du. _die_, [n.] _dat_ \
-- OHG _der_, acc. _den_, [f.] _diu_, acc. _dea_, _dia_, [n.] _daz_, G _der_, acc. _den_, [f.] _die_, [n.] _das_ \
REF: [EDPG]\: 530

### <a id="þagēn">\*þagēn- [wk.] "to be silent" (v)</a>
E: \
-- Go. _þahan_ \
N: \
-- ON _þegja_ \
-- Far. _tiga_ \
-- Elfd. _taja_ \
W: \
-- OS _thagian_, _thagon_ \
-- OHG _dagēn_, MHG _dagen_ \
REF: [EDPG]\: 531

### <a id="þaggōn">\*þaggōn- [wk.] "to silence" (v)</a>
N: \
-- ON _þagga_ \
-- Nw. _tagga_ \
REF: [EDPG]\: 531

### <a id="þahsu">\*þahsu- [m.] "badger" (n)</a>
W: \
-- MDu. _das_, Du. _das_ [c.] \
-- MHG _dahs_, G _Dachs_ \
REF: [EDPG]\: 531

### <a id="þaismjan">\*þaismjan- [m.] "sourdough" (n)</a>
W: \
-- OE _þǣsma_ \
-- MLG _dēsem_ \
-- MDu. _desem_, Du. _zuur-desem_ [c.] \
-- OHG _deismo_, MHG _deisme_ \
REF: [EDPG]\: 531

### <a id="þaka">\*þaka- [n.] "roof" (n)</a>
N: \
-- ON _þak_ \
-- Far. _tak_ \
-- Elfd. _tak_ \
W: \
-- OE _þæc_ \
-- Du. _dak_ \
-- OHG _dach_, G _Dach_ \
REF: [EDPG]\: 531

### <a id="þakjan">\*þakjan- [wk.] "to burn" (v)</a>
W: \
-- OE _þeccan_ "to burn" \
REF: [EDPG]\: 532

### <a id="þakkja">\*þakkja- / \*þekka- "mild(?), sweet(?)" (adj)</a>
N: \
-- ON _þekkr_ "pleasant" \
W: \
-- OHG _decki_ "dear" \
REF: [EDPG]\: 532

### <a id="þakkōn">\*þak(k)ōn- [wk.] "to touch" (v)</a>
W: \
-- OE _þaccian_ "to pat, clap, strike gently" \
-- MDu. _tacken_ "to touch" \
REF: [EDPG]\: 532

### <a id="þambō">\*þambō- [f.] "string" (n)</a>
N: \
-- ON _þǫmb_ "guts, womb; bow-string" \
REF: [EDPG]\: 532

### <a id="þanga">\*þanga- [n.] "seaweed" (n)</a>
N: \
-- ON _þang_ \
W: \
-- MLG _dank_ \
REF: [EDPG]\: 532

### <a id="þanhōn">\*þanhōn- [f.] "clay" (n)</a>
E: \
-- Go. _þaho_ \
W: \
-- OE _þōhe_, _þō_ \
-- OHG _dāha_, MHG _dāhe_, _tāhe_, G _Ton_ \
REF: [EDPG]\: 533

### <a id="þanhtu">\*þanhtu- [m.] "thread" (n)</a>
N: \
-- ON _þáttr_ "strand, thread; section, part; story" \
-- Far. _táttur_ "strand of a rope; part; song" \
-- Elfd. _tą̊t_ "id." \
W: \
-- OHG _tāht_ [m./n.] "wick", G _Docht_ "id." \
REF: [EDPG]\: 533

### <a id="þanjan">\*þanjan- [wk.] "to stretch, extend" (v)</a>
E: \
-- Go. _uf-þanjan_ "to stretch, extend; to strive for" \
N: \
-- ON _þenja_ \
-- Far. _tenja_ \
W: \
-- OE _þennan_, _þenian_ "to stretch, spread out, extend, bend (a bow)" \
-- OS _thenian_ "to stretch" \
-- MDu. _denen_ "to stretch; to torture", Du. _deinen_ "to wave" \
-- OHG _dennan_, MHG _den(n)en_ [st./wk.], G _dehnen_ "to extend" \
REF: [EDPG]\: 533

### <a id="þanka">\*þanka- [m.] "thanks" (n)</a>
E: \
-- Go. _þagks_ \
N: \
-- ON _þǫkk_ [f.] \
-- Far. _tøkk_ [f.] \
-- Elfd. _tokk (<_ \*þankō-) \
W: \
-- OE _þanc_ "thanks, satisfaction", E _thanks_ \
-- OFri. _thonk_ "thanks; satisfaction; intention" \
-- OS _thank_ "thanks; grace" \
-- Du. _dank_ [c.] "thanks" \
-- OHG _dank_ "thanks; mercy, gratification", G _Dank_ "thanks" \
REF: [EDPG]\: 533

### <a id="þankjan">\*þankjan- [wk.] "to think" (v)</a>
E: \
-- Go. _þagkjan_ "to think, plan" \
N: \
-- ON _þekkja_ "to perceive, notice; to comprehend; to know, recognize" \
W: \
-- OE _þencan_, E _to think_ \
-- OS _thenkian_ "to think, consider, watch" \
-- Du. _denken_ "id." \
-- OHG _denkan_ "id.", G _denken_ "id." \
REF: [EDPG]\: 533

### <a id="þansjan">\*þansjan- [wk.] "to move back" (v)</a>
W: \
-- MLG _deisen_ "to sneak away" \
-- MDu. _dei(n)sen_ "to draw back, stand back", Du. deinzen "id." \
REF: [EDPG]\: 534

### <a id="þansōn">\*þansōn- [wk.] "to stretch" (v)</a>
W: \
-- OHG _dansōn_ "to pull, stretch", MHG dansen "id." \
REF: [EDPG]\: 534

### <a id="þar">\*þar "there" (adv)</a>
E: \
-- Go. _þar_ \
N: \
-- ON _þar_ \
-- Far. _har_, poet. _tar_ \
-- Elfd. _dar_ \
W: \
-- OE _þār_, _þǣ̆r_, E _there_ \
-- OFri. _ther_ \
-- OS _thār_ \
-- Du. _daar_ \
-- OHG _dā̆r(a)_, MHG _dār_, _dā_, G _da_ \
REF: [EDPG]\: 534

### <a id="þarbō">\*þarbō- [f.] "need, want, lack" (n)</a>
E: \
-- Go. _þarba_ "want, need, necessity" \
N: \
-- ON _þǫrf_ "id." \
W: \
-- OFri. _therve_, _thēre_ "need, want" \
-- OS _tharf_ "need, lack" \
REF: [EDPG]\: 534

### <a id="þarhan">\*þarha(n)- [m.] "seaweed, driftwood" (n)</a>
N: \
-- ON _þari_ \
-- Far. _tari_ \
-- Nw. _tar(r)e_ "seaweed; washed-up wood" \
W: \
-- MDu. _dary_, _darich_ "peat sod; muck", Du. _derrie_ [c.] "muck" \
REF: [EDPG]\: 535

### <a id="þarma">\*þarma- [m.] "bowel, intestine" (n)</a>
N: \
-- ON _þarmr_ \
W: \
-- OE _þearm_ \
-- OFri. _therm_ \
-- OS _tharm_, MLG _darm_ "intestine" \
-- MDu. _dar(e)m_ "id.", Du. _darm_ [c.] "id." \
-- OHG _daram_, _darm_ "id.", G _Darm_ "id." \
REF: [EDPG]\: 535

### <a id="þarzjan">\*þarzjan- [wk.] "to make dry" (v)</a>
N: \
-- ON _þerra_ "to dry, make dry; to dry, wipe" \
W: \
-- MLG _derren_ "to dry" \
-- MHG _derren_ "to wither, dry up" \
REF: [EDPG]\: 535

### <a id="þaulan">\*þaula(n)- [m.] "reciter, mumbler(?)" (n)</a>
N: \
-- Icel. _þauli_ "speaker, orator" \
-- Nw. dial. _taul_ "fool, half-wit" \
REF: [EDPG]\: 535

### <a id="þawa">\*þawa- [m.] "custom" (n)</a>
W: \
-- OE _þēaw_ "custom, usage", ME _thew_ "habit, practice, virtue" \
-- OFri. _thāw_ \
-- OS _thau_, MLG _dūw_ "conduct" \
-- OHG _dou_ "instruction" \
REF: [EDPG]\: 535

### <a id="þedura">\*þedura- [m.] "wood grouse, capercaillie" (n)</a>
N: \
-- ON _þiðurr_ \
-- OSw. _þiædur_ \
-- Elfd. _työðer_ \
-- Da. _tjur_ [c.] \
REF: [EDPG]\: 535

### <a id="þegjan">\*þegjan- [st.] "to request" (v)</a>
N: \
-- ON _þiggja_ "to obtain, accept" \
W: \
-- OE _þicgan_ "id." \
-- OS _thiggian_ [wk.] "to ask, request; to endure" \
-- OHG _dicken_, _digen_ [wk.] "to beg for, request", MHG _digen_ [wk.] "id." \
REF: [EDPG]\: 536

### <a id="þegna">\*þegna- [m.] "retainer" (n)</a>
N: \
-- ON _þegn_ "thane, franklin, freeman, man, Hegeman, subject" \
W: \
-- OE _þegn_ "thane, servant, retainer" \
-- OS _thegan_ "youth, man, warrior, disciple" \
-- MDu. _degen_ "boy, prince, hero" \
-- OHG _degan_ "warrior, valet, companion, hero, soldier", MHG _degen_ "lad, servant, warrior" \
REF: [EDPG]\: 536

### <a id="þēka">\*þēka- [m.] "drifting seaweed" (n)</a>
W: \
-- WFri. obs. _teek_, NFri. (F) _tiək_, (Hlg., Mo., Wi., Ka.) _teek_, (Go., Ha.) _täik_ [n.] \
-- LG _deek_, _teek_, Brem. _feek_ \
-- Du. dial., arch. _daak_, _deek_, _vaak_, _veek_ [c.] \
REF: [EDPG]\: 536

### <a id="þeku">\*þeku- "fat" (adj)</a>
N: \
-- ON _þykkr_, _þjokkr_, _þjukkr_ \
-- Far. _tjúkkur_, _tjukkur_ \
-- Elfd. _tjokk_ \
W: \
-- OE _þicce_, E _thick_ \
-- OFri. _thikke_ "manifold, multiple" \
-- OS _thikki_ "id." \
-- Du. _dik_ "fat; thick" \
-- OHG _dicki_ "dense, thick; frequent", G _dick_ "fat, thick" \
REF: [EDPG]\: 537

### <a id="þemestra">\*þemestra- "dark, dusky" (adj)</a>
W: \
-- OE _þīestre_ \
-- OFri. _thiūstere_ \
-- OS _thiustri (<_ \*þiustrja-), MLG _demster_ \
-- MDu. _dee(m)ster_, _duuster_, _vinster_, Du. _duister_, obs. _deemster_ \
-- OHG _dinstar_, _finstar_, MHG _dinster_, _dimster_, _finster_, G _finster_ \
REF: [EDPG]\: 537

### <a id="þemra">\*þemra- [m.] "darkness" (n)</a>
W: \
-- OHG _demar_ [n.] "dusk" \
REF: [EDPG]\: 537

### <a id="þenura">\*þenura- [m.] "reaction wood" (n)</a>
N: \
-- ON _þinurr_ "resinous fir-tree (of which bows and hoops were made); bow" \
-- Nw. _tinur_, _tinar_ "hard (piece of) wood" \
-- Elfd. _tiųor_ "reaction wood" \
REF: [EDPG]\: 538

### <a id="þerba">\*þerba- "tough(?)" (adj)</a>
N: \
-- ON _þjarfr_ "unleavened (of bread); fresh (of water); insipid, flat" \
W: \
-- OE _þeorf_ "unleavened" \
-- OFri. _therve_ "rough, fierce" \
-- OHG _derb_ "unleavened", MHG _derp_ "id.", G dial. _derb_ "unleavened", Bav. "dry" \
REF: [EDPG]\: 538

### <a id="þerhwe">\*þerhwe ~ \*þurhwe "through" (prep, adv)</a>
E: \
-- Go. _þairh_ \
W: \
-- OE _þer(i)h_, _þur(u)h_, _þorh_, E _through_ \
-- OFri. _thriuch_, _thruch_, NFri. _troch_ \
-- OS _thurh_, _thoro_, _thuru_ \
-- ODu. _thuro_, MDu. _dore_, _dure_, _do(o)rch_, Du. _door_ \
-- OHG _durh_, _durah_, _duruh_, G _durch_ \
REF: [EDPG]\: 538

### <a id="þerkōn">\*þerkōn- [f.] "hole" (n)</a>
E: \
-- Go. _þairko_ "hole" \
REF: [EDPG]\: 539

### <a id="þersan">\*þersan- [st.] "to be dry" (v)</a>
E: \
-- Go. _ga-þairsan_ "to dry up, wither" \
REF: [EDPG]\: 539

### <a id="þeuba">\*þeuba- [m.] "thief" (n)</a>
E: \
-- Go. _þiubs_ \
N: \
-- ON _þjófr_ \
W: \
-- OE _þēof_, E _thief_ \
-- OFri. _thiāf_ \
-- OS _thiof_ \
-- Du. _dief_ [c.] \
-- OHG _diob_, _diub_, G _Dieb_ \
REF: [EDPG]\: 539

### <a id="þeudja">\*þeudja- "favorable" (adj)</a>
N: \
-- ON _þýðr_ "kind, affectionate, affable" \
W: \
-- OE _ge-þīede_ "good, decent" \
REF: [EDPG]\: 539

### <a id="þeudō">\*þeudō- [f.] "nation, people" (n)</a>
E: \
-- Go. _þiuda_ \
N: \
-- ON _þjóð_ \
-- Far. _tjóð_ \
W: \
-- OE _þēod_ \
-- OFri. _thiāde_, _thiēde_ \
-- OS _thiod_, _thioda_ \
-- OHG _diota_, MHG _diet_ \
REF: [EDPG]\: 540

### <a id="þeuha">\*þeuha- [n.] "thigh" (n)</a>
N: \
-- ON _þjó_ \
-- Far. _tjógv_ \
W: \
-- OE _þēoh_, E _thigh_ \
-- OFri. _thiāch_ \
-- MDu. _die_ [n./f.], Du. _dij_ [c.] \
-- OHG _dioh_ \
REF: [EDPG]\: 540

### <a id="þeura">\*þeura- [m.] "bull" (n)</a>
N: \
-- ON _þjórr_ \
-- Far. _tjórur_ "castrated bull" \
-- OSw. _þiur_ \
W: \
-- Du. dial. _duur_ \
REF: [EDPG]\: 540

### <a id="þeutan">\*þeutan- ~ \*þūtan- [st.] "to howl, roar" (v)</a>
E: \
-- Go. _þut-haurn_ [n.] "trumpet" \
N: \
-- ON _þjóta_ "to howl, whistle, roar" \
-- Nw. _tjota_, _tyta_ "id." \
W: \
-- OE _þēotan_, _þūtan_ "to howl" \
-- OHG _diozan_ "to roar", MHG _diezen_ "to roar, woosh" \
REF: [EDPG]\: 540

### <a id="þewa">\*þewa- [m.] "servant" (n)</a>
E: \
-- Go. _þius_ "thrall, bondsman, servant" \
N: \
-- ON _-þer_ (pn) "id." \
W: \
-- OE _þēow_, _þēowa_, ME _theow_ \
-- OS _theo-lico_ "humble" (adv) \
-- Du. _dee-moedig_ "id." \
-- MHG _deo_ \
REF: [EDPG]\: 541

### <a id="þewen">\*þewen- [wk.] "to subdue" (v)</a>
E: \
-- Go. _ga-þiwan_ "to subdue, subjugate" \
N: \
-- ON _þjá_ "to constrain; to enslave" \
REF: [EDPG]\: 541

### <a id="þīna">\*þīna- "your" (pron)</a>
E: \
-- Go. _þeins_ \
N: \
-- ON _þinn_ \
-- Far. _tín_ \
-- Elfd. _denn_ \
W: \
-- OE _þīn_ \
-- OFri. _thīn_ \
-- OS _thīn_ \
-- MDu. _dijn_ \
-- OHG _dīn_, G _dein_ \
REF: [EDPG]\: 541

### <a id="þīnan">\*þīnan- [st.] "to become moist" (v)</a>
W: \
-- OE _þīnan_ \
REF: [EDPG]\: 541

### <a id="þinga">\*þinga- [n.] "meeting, case, thing" (n)</a>
N: \
-- ON _þing_ \
W: \
-- OE _þing_, E _thing_ \
-- OFri. _thing_ \
-- OS _thing_ "thing, court, meeting" \
-- Du. _ding_ "thing" \
-- OHG _ding_ "meeting, court, thing, affair", G _Ding_ "thing" \
REF: [EDPG]\: 542

### <a id="þingjan">\*þingjan- [wk.] "to believe" (v)</a>
W: \
-- OHG _dingan_, MHG _dingen_ \
REF: [EDPG]\: 542

### <a id="þinhan">\*þinhan- [st.] "to thrive, prosper" (v)</a>
E: \
-- Go. _þeihan_ \
W: \
-- OE _þēon_, _(ge-)þingan_ \
-- OS _thīhan_ \
-- Du. _gedijen_ [wk.] \
-- OHG _dīhan_, G _gedeihen_ \
REF: [EDPG]\: 542

### <a id="þinhla">\*þinhla- [n.] "curds(?)" (n)</a>
N: \
-- ON _þél_ "fresh-curded milk" \
REF: [EDPG]\: 542

### <a id="þinhslō">\*þinhslō- [f.] "drawbar, cart-pole" (n)</a>
N: \
-- ON _þísl_ "cart-pole" \
W: \
-- OE _þīxl_, _þīsl_ "beam or pole of a wagon" \
-- MLG _dīsel_, _dissel_ "shaft, drawbar" \
-- Du. _dissel_ [c.] "id." \
-- OHG _dīhsila_ "id.", MHG _dihsel_ "id.", G _Deichsel_ "id." \
REF: [EDPG]\: 542

### <a id="þinhtu">\*þinhtu- "dense, tight, close" (adj)</a>
N: \
-- ON _þéttr_ \
-- Far. _tettur_ \
-- Elfd. _ti'tt_ \
W: \
-- OE _-þiht_ "strong", ME _thiht_, _tiht_ (influenced by MScand. _tēttr_?) "dense, tight", E _tight_ \
-- WFri. _ticht_ "id." \
-- MDu. _dichte_ "id.", Du. _dicht_ "id." \
-- G _dicht_, dial. _deicht_ "tight, close, dense" \
REF: [EDPG]\: 543

### <a id="þinhwōn">\*þinhwōn- [f.] "thunder" (n)</a>
E: \
-- Go. _þeiƕo_ \
REF: [EDPG]\: 543

### <a id="þinkila">\*þinkila- [m.] "spelt" (n)</a>
W: \
-- OS _thinkil_ \
-- OHG _dinkil_ "spelt, wheat", G _Dinkel_ "spelt" \
REF: [EDPG]\: 543

### <a id="þinnō">\*þinnō- ~ \*þunnō- [f.] "forehead, temple" (n)</a>
W: \
-- MHG _tinne_, _tunne_ "forehead, temple" \
REF: [EDPG]\: 543

### <a id="þinsan">\*þinsan- [st.] "to stretch" (v)</a>
E: \
-- Go. _at-þinsan_ "to draw towards, pull up to" \
W: \
-- OS _thinsan_ "to pull, tear" \
-- MDu. (E) _dinsen_ [wk.] "to drag" \
-- OHG _dinsan_ "to pull, trail", MHG _dinsen_ "to pull, yank, trail", G _gedunsen_ "bloated" \
REF: [EDPG]\: 543

### <a id="þleuhan">\*þleuhan- [st.] "to flee" (v)</a>
E: \
-- Go. _þliuhan_ \
W: \
-- OE _flēon_, E _to flee_ \
-- OFri. _flīa_ "to reconcile" \
-- OS _fliohan_ \
-- MDu. _vlien_ [st./wk.] \
-- OHG _fliohan_, G _fliehen_ \
REF: [EDPG]\: 544

### <a id="þragjan">\*þragjan- [wk.] "to run" (v)</a>
E: \
-- Go. _þragjan_ \
W: \
-- OE _þrægan_ \
REF: [EDPG]\: 544

### <a id="þraibjan">\*þraibjan- [wk.] "to seize" (v)</a>
N: \
-- ON _þreifa_ "to feel with the hand, touch" \
-- Icel. _þreifa_ "to touch, feel, finger" \
REF: [EDPG]\: 544

### <a id="þrangʷjan">\*þrangʷjan- [wk.] "to press" (v)</a>
N: \
-- ON _þrøngva_ "to press, force" \
-- Far. _treingja_ "to push, press, urge; to need; to make narrow(er)" \
-- Elfd. _traindja_ "to push" \
W: \
-- MDu. _drengen_ "to make penetrate" \
-- MHG _drengen_ "to urge" \
REF: [EDPG]\: 544

### <a id="þranha">\*þranha- "dirty" (adj)</a>
N: \
-- Icel. _þrár_ "rancid" \
-- Far. _tráur_ "id." \
-- Nw. _trå_ "id." \
W: \
-- OE _þrōh_ "id." \
REF: [EDPG]\: 544

### <a id="þrappōn">\*þrapp/bōn- [wk.] "to trot" (v)</a>
N: \
-- Elfd. _tråvå_ \
W: \
-- OE _þrafian_ "to urge, press" \
-- OS _thraƀon_ \
-- EDu. _draven_, _drabben_, Du. _draven_, dial. _drabben_ \
-- MHG _draben_ \
REF: [EDPG]\: 544

### <a id="þrastu">\*þrastu- [m.] "thrush" (n)</a>
N: \
-- ON _þrǫstr_ \
-- Far. _trøstur_ \
-- Elfd. _trast_ \
W: \
-- OE _þrysce_, _þræsce_ [f.], E _thrush_ \
-- OHG _drōsca_ [f.], G _Droschel_ [f.] \
REF: [EDPG]\: 545

### <a id="þrautjan">\*þrautjan- [wk.] "to exhaust" (v)</a>
N: \
-- ON _þreyta_ "to strive, struggle" \
-- Far. _troyta_ "to complete; to drag on, exert one's strength; to exhaust" \
REF: [EDPG]\: 545

### <a id="þrawjan">\*þrawjan- [wk.] "to grow" (v)</a>
W: \
-- OHG _trowwen_ [st./wk.] "to grow (up)", MHG ge-drouwen past ptc. "grown" \
REF: [EDPG]\: 545

### <a id="þrawō">\*þrawō- [f.] "longing, suffering" (n)</a>
N: \
-- ON _þrá_ "longing, yearning" \
W: \
-- OE _þrēa_ "rebuke, threat; punishment; infliction" \
-- OS _thrā-werk_ [n.] "pain" \
-- OHG _drawa_, _drowa_, _drōa_ "threat, blame, burden", MHG _dro(u)we_, _drō_ "threat" \
REF: [EDPG]\: 545

### <a id="þrēan">\*þrēan- [st.] "to turn" (v)</a>
W: \
-- OE _þrāwan_ "to twist, turn; to throw", E _to throw_ \
-- OFri. _drāia_ [wk.] \
-- Du. _draaien_ [wk.] \
-- OHG _drāen_, G _drehen_ [wk.] \
REF: [EDPG]\: 546

### <a id="þrēdu">\*þrēdu- [m.] "thread" (n)</a>
N: \
-- ON _þráðr_ \
W: \
-- OE _þrǣd_, E _thread_ \
-- OFri. _thrēd_ \
-- MDu. _draet_, Du. _draad_ [c.] \
-- OHG _drāt_, G _Draht_ \
REF: [EDPG]\: 546

### <a id="þreutan">\*þreutan- [st.] "to bother" (v)</a>
E: \
-- Go. _us-þriutan_ "to harass, bother, persecute" \
N: \
-- ON _þrjóta_ (imp.v) "to fail, come to an end, become exhausted" \
-- Far. _tróta_ "id." \
W: \
-- OE _þrēotan_ "to weary" \
-- MLG _drēten_, _vor-drēten_ "to annoy" \
-- Du. _ver-drieten_ "to make sad" \
-- OHG _bi-driozan_ (imp.v) "to weary, trouble", G _ver-drießen_ "to annoy" \
REF: [EDPG]\: 546

### <a id="þrewan">\*þrewan- [st.] "to grow, thrive" (v)</a>
W: \
-- OHG _driuwan_ "to thrive" \
REF: [EDPG]\: 546

### <a id="þrī">\*þrī- "three" (num)</a>
E: \
-- Go. _þreis\*_, acc. _þrins_, [n.] _þrija_, gen. _þrije_, dat. _þrim_ \
N: \
-- ON _þrír_, acc. _þrjá_, [f.] _þrjár_, [n.] _þrjú_, gen. _þriggja_, dat. _þrim(r)_, _þrem(r)_ \
-- Far. _triggir_, acc. _tríggjar_, [f.] _tríggjar_, [n.] _trý_, gen. _tríggja_, dat. _trimum_ \
-- Elfd. _trair_, acc. _triųo_, [f.] _trjär_, [n.] _tråy_, dat. _trim_ \
W: \
-- OE _þrī_, [f./n.] _þrīo_, _þrēo_, E _three_ \
-- OFri. _thrē_ \
-- OS _thria_, _threa_, [n.] _thriu_ \
-- Du. _drie_ \
-- OHG _drī_, _drīe_ [f.] _drīo_, [n.] _driu_, G _drei_ \
REF: [EDPG]\: 546

### <a id="þridjan">\*þridjan- "third" (num)</a>
E: \
-- Go. _þridja_ \
N: \
-- ON _þriði_ \
-- Far. _triði_ \
-- Elfd. _trið_ \
W: \
-- OE _þridda_, E _third_ \
-- OFri. _thredda_ \
-- OS _thriddio_ \
-- Du. _derde_ \
-- OHG _dritto_, G _dritte_ \
REF: [EDPG]\: 547

### <a id="þrīfan">\*þrīfan- [st.] "to seize" (v)</a>
N: \
-- ON _þrífa_ "to catch, grasp", refl. "to thrive, grow" \
-- Far. _tríva_ "to seize, grip, grasp" \
-- Nw. _triva_ "to snatch" \
REF: [EDPG]\: 547

### <a id="þrinhan">\*þrinhan- [st.] "to press" (v)</a>
E: \
-- Go. _þreihan_ "to press, crowd" \
N: \
-- ON _þryngva_ "to press, thrust; to rush" \
-- Nw. _trenga_, _trønga_ "to need" \
W: \
-- OE _þringan_ "to press, crowd throng" \
-- OS _thringan_ "to press, urge" \
-- Du. _dringen_ "id." \
-- OHG _dringan_ "id.", G _dringen_ "id." \
REF: [EDPG]\: 547

### <a id="þrippjbōn">\*þrippjbōn- [wk.] "to seize" (v)</a>
N: \
-- ON _þrifa_ "to seize" \
-- Icel. _þrifla_ "to touch, feel, finger" \
-- Nw. _tripla_ "to finger, touch upon", _trivla_ "to grope, feel tentatively" \
-- ODa. _threve_ "to seize", Da. _treve_, _træve_, _trave_ "to fumble, grope" \
REF: [EDPG]\: 547

### <a id="þrōdjan">\*þrōdjan- [wk.] "to train(?)" (v)</a>
E: \
-- Go. _þroþjan_ "to exercise, train, drill" \
REF: [EDPG]\: 547

### <a id="þrōēn">\*þrōēn- [wk.] "to endure" (v)</a>
W: \
-- OE _þrōwian_ "to endure" \
-- OHG _druoēn_ "to suffer" \
REF: [EDPG]\: 548

### <a id="þruban">\*þruban- [m.] "cluster" (n)</a>
W: \
-- MDu. _drubbel_ "cluster (of plants)" \
REF: [EDPG]\: 548

### <a id="þrunhta">\*þrunhta- [m.] "hardship(?)" (n)</a>
N: \
-- ON _þróttr_ "stamina, strength" \
W: \
-- OE _þroht_ "oppression, hardship" \
REF: [EDPG]\: 548

### <a id="þruttōn">\*þruttōn- [wk.] "to exhaust" (v)</a>
W: \
-- G dial. _trotzen_ "to over-exploit" (DWB: 22, 1115) \
REF: [EDPG]\: 548

### <a id="þrūþi">\*þrūþi- [f.] "strength" (n)</a>
N: \
-- ON _þrúð-_ \
W: \
-- OE _þrȳð_ \
REF: [EDPG]\: 548

### <a id="þruwēn">\*þruwēn- [wk.] "to thrive, mature" (v)</a>
N: \
-- ON _þróa-st_ "to increase, grow" \
REF: [EDPG]\: 548

### <a id="þū">\*þū "you" (pron)</a>
E: \
-- Go. _þu_, dat _þus_, acc. _þuk_ \
N: \
-- ON _þú_, dat _þér_, acc. _þik_ \
-- Far. _tú_, obl. _teg_ \
-- Elfd. _du_, obl. _dig_ \
W: \
-- OE _þū_, obl. _þē_, acc. _þec_, _þē_ \
-- OFri. _thu_, obl. _thi_ \
-- OS _thu_, obl. _thī̆_, acc. _thik_ \
-- MDu. _du_, obl. _dij_ \
-- OHG _dū̆_, dat. _dir_, acc. _dih_, G _du_, dat _dir_, acc. _dich_ \
REF: [EDPG]\: 549

### <a id="þuftōn">\*þuftōn- [f.] "thwart, rowing bench" (n)</a>
N: \
-- ON _þofta_ \
W: \
-- OE _þoft_, _þofte_ \
-- MLG _ducht_ "thwart" \
-- MDu. _dochte_ "id.", Du. _doft_ [c.] "id." \
-- OHG _dofta_ "id." \
REF: [EDPG]\: 549

### <a id="þukan">\*þuka/ōn- [m./f.] "fog, mist" (n)</a>
N: \
-- ON _þoka_ [f.] \
-- Far. _toka_ [f.] \
W: \
-- MLG _dake_ [m.] \
-- Du. dial. _dook_ [m.] \
REF: [EDPG]\: 549

### <a id="þukkōn">\*þuk(k)ōn- [wk.] "to shove" (v)</a>
N: \
-- ON _þokka_ "to move" \
-- Icel. _þok(k)a_ "to move slowly/slightly; to fall back, make room for" \
-- Far. _toka_ "to force, press, push, advance" \
-- Nw. _tok(k)a_ "to move, shove" \
W: \
-- OE _þocerian_ "to run to and fro, run about" \
REF: [EDPG]\: 549

### <a id="þuli">\*þuli- [m.] "reciter" (n)</a>
N: \
-- ON _þulr_ "sage" \
W: \
-- OE _þyle_ "orator, spokesman" \
REF: [EDPG]\: 550

### <a id="þulla">\*þulla- [m.] "thole, peg in the side of a boat to keep the oar in place" (n)</a>
N: \
-- Icel. _þollur_ "peg; thole" \
-- Far. _tollur_ "tholepin" \
-- Nw. _toll(e)_ "thole, tholepin" \
-- Da. _tol_ [c.] "id." \
W: \
-- OE _þoll_ "id." \
-- MLG _dolle_, _dulle_ [f.] "id." \
-- MDu. _dolle_ [f.] "id.", Du. _dol_ [c.] "id." \
REF: [EDPG]\: 550

### <a id="þūman">\*þū̆man- [m.] "thumb" (n)</a>
N: \
-- Icel. _þumi_ \
-- Far. _tummi_ "inch" \
-- Nw. _tomme_ "id." \
-- OSw. _þumi_ "thumb; inch" \
-- Sw. _tumme_ [c.] "id." \
-- ODa. _thumæ_ "id.", Da. _tomme_ [c.] "id." \
W: \
-- OE _þūma_ "id.", E _thumb_ \
-- OFri. _thūma_ "id." \
-- MLG _dūm(e)_ "id." \
-- MDu. _dume_ "id.", Du. _duim_ [c.] "id." \
-- OHG _dūmo_ "id.", G _Daumen_ "id." \
REF: [EDPG]\: 550

### <a id="þuna">\*þuna- "stretched" (adj)</a>
W: \
-- MDu. _done_, _duene_ "tight, close" \
-- MHG _don_ "stretched out" \
REF: [EDPG]\: 551

### <a id="þungu">\*þungu- "heavy" (adj)</a>
N: \
-- ON _þungr_ \
-- Far. _tungur_ \
-- Elfd. _tungg_ \
REF: [EDPG]\: 551

### <a id="þunhjan">\*þunhjan- [wk.] "to press" (v)</a>
N: \
-- Far. _týggja_ "to be hard on" \
W: \
-- OE _ge-þȳwan_ "to press, impel, urge" \
-- MDu. _duwen_, _douwen_ "to push, press", Du. _duwen_, reg. _douwen_ "to push" \
-- MHG _diuhen_, _dūhen_ [st.] "to push, press down" \
REF: [EDPG]\: 551

### <a id="þunkjan">\*þunkjan- [wk.] "to think" (v)</a>
E: \
-- Go. _þugkjan_ "to have the impression, suppose, appear" \
N: \
-- ON _þykkja_ "to deem, think" \
-- Far. _tykja_ "to seem, appear; to think, believe" \
-- Elfd. _tyttja_ "to think; to like" \
W: \
-- OE _þyncan_ "to seem, appear", E _to think_ \
-- OFri. _thinka_, thinza "to seem, think" \
-- OS _thunkian_ "consider" \
-- Du. _dunken_ "to think, deem" \
-- OHG _dunken_ "to deem, think, mean", G _dünken_ "id." \
REF: [EDPG]\: 551

### <a id="þunkōn">\*þunkōn- [wk.] "to make wet" (v)</a>
W: \
-- OHG _dunkōn_ "to dip", MHG _dunken_, _tunken_ "id.", G _tunken_ "id." \
REF: [EDPG]\: 551

### <a id="þunnu">\*þunnu- "thin" (adj)</a>
N: \
-- ON _þunnr_ \
-- Far. _tunnur_ \
-- Elfd. _tunn_ \
W: \
-- OE _þynne_, E _thin_ \
-- MDu. _dunne_, Du. _dun_ \
-- OHG _dunni_, G _dünn_ \
REF: [EDPG]\: 551

### <a id="þurfan">\*þurfan- [pp.] "to need" (v)</a>
E: \
-- Go. _þaurban_ \
N: \
-- ON _þurfa_ \
-- Far. poet. _turva_ \
W: \
-- OE _þurfan_ "to be wanton" \
-- OFri. _thurva_ \
-- OS _thurƀan_ \
-- MDu. _dorven_ "to need; to dare", Du. _durven_ [wk.] "to dare" \
-- OHG _durfan_, G _dürfen_ "to be allowed to" \
REF: [EDPG]\: 552

### <a id="þurfti">\*þurfti- [f.] "need" (n)</a>
E: \
-- Go. _þaurfts_ \
N: \
-- ON _þurft_ \
W: \
-- OS _thurft_, _thuruft_ \
-- OHG _durft_, MHG _durft_ "need, want, necessity" \
REF: [EDPG]\: 552

### <a id="þurisa">\*þurisa- [m.] "giant" (n)</a>
N: \
-- ON _þurs_ \
-- Far. poet. _tussur_ \
-- Nw. _tuss_ \
W: \
-- OE _þyrs_ \
-- OHG _durs_, _duris_, MHG _dürse_, _turse_, _türse_, _türsch_ \
REF: [EDPG]\: 552

### <a id="þurjan">\*þurjan- [wk.] "to sweep" (v)</a>
N: \
-- ON _þyrja_ "to sweep, rush" \
REF: [EDPG]\: 552

### <a id="þurna">\*þurna- [m.] "thorn, briar" (n)</a>
E: \
-- Go. _þaurnus (<_ \*þurnu-) \
N: \
-- ON _þorn_ \
W: \
-- OE _þorn_, E _thorn_ \
-- OFri. _thorn_ \
-- ODu. _thorn_, Du. _doorn_ [c.] \
-- OHG _dorn_, G _Dorn_ \
REF: [EDPG]\: 552

### <a id="þurpa">\*þurpa- [n.] "crowd(?)" (n)</a>
E: \
-- Go. _þaurp_ "farmland" \
N: \
-- ON _þorp_ "isolated settlement" \
-- Nw. _torp_ "group; small settlement" \
W: \
-- OE _þorp_, _þrop_ [m.] "crowd" \
-- OFri. _therp_, _thorp_ "village; (fallow) land" \
-- OS _thorp_ "id." \
-- Du. _dorp_ "id." \
-- OHG _dorf_ "id.", G _Dorf_ "id." \
REF: [EDPG]\: 553

### <a id="þurska">\*þurska- [m.] "cod" (n)</a>
N: \
-- ON _þorskr_ \
-- Far. _toskur_ \
-- Elfd. _tuosk_ \
W: \
-- MDu. _dorsch_ \
-- G _Dorsch_ \
REF: [EDPG]\: 553

### <a id="þurstu">\*þurstu- [m.] "thirst" (n)</a>
N: \
-- Elfd. _tuost_ [f.] \
W: \
-- OE _þurst_, _þyrst_, E _thirst_ \
-- OS _thurst_ \
-- Du. _dorst_ [c.] \
-- OHG _durst_, G _Durst_ \
REF: [EDPG]\: 553

### <a id="þurzēn">\*þurzēn- [wk.] "to be dry" (v)</a>
N: \
-- Sw. dial. _torra_ \
W: \
-- OS _thorron_ "to waste away" \
-- Du. _ver-dorren_ "to wither" \
-- OHG _dorrēn_, _dorren_ "to dry, wither, stiffen", G _ver-dorren_ "to wither" \
REF: [EDPG]\: 553

### <a id="þurzjan">\*þurzjan- [wk.] "to be thirsty" (v)</a>
E: \
-- Go. _þaursjan_ \
W: \
-- OE _þyrreð_ p.ptc. "dried out" \
REF: [EDPG]\: 554

### <a id="þurzu">\*þurzu- "dry" (adj)</a>
E: \
-- Go. _þaursus_ \
N: \
-- ON _þurr_, _þyrr_ \
-- Far. _turrur_ \
-- Nw. _tørr_, _turr_, dial. _tjurr_ \
-- Elfd. _tuorr_ \
W: \
-- OE _þyrre_ \
-- OFri. _thurre_ "dry(?)" \
-- OS _thurri_ \
-- Du. _dor_ \
-- OHG _durri_, G _dürr_ \
REF: [EDPG]\: 554

### <a id="þūshundī">\*þūshundī- [f.] "thousand" (num)</a>
E: \
-- Go. _þusundi_ \
N: \
-- ON _þúsund_ \
-- Far. _túsundi_ \
-- Elfd. _tusn_ \
W: \
-- OE _þūsend_, E _thousand_ \
-- OFri. _thūsend_ \
-- OS _thūsundig_ \
-- Du. _duizend_ \
-- OHG _dūsunt_, G _Tausend_ \
REF: [EDPG]\: 554

### <a id="þusiþa">\*þusiþa- "silent" (adj)</a>
N: \
-- Nw. _tyst_ \
-- OSw. _thyster_, _tøster_ \
-- Elfd. _tyst_ \
-- ODa. _t(h)yst(ær)_, Da. _tyst_ \
REF: [EDPG]\: 554

### <a id="þuti">\*þuti- [m.] "roaring" (n)</a>
N: \
-- ON _þytr_ "howling" \
W: \
-- MHG _duz_ "sound, noise" \
REF: [EDPG]\: 555

### <a id="þuttōn">\*þutt/dōn- [wk.] "to bump" (v)</a>
N: \
-- Far. _tota_ "to dash" \
W: \
-- OE _þodettan_ "to hit, bump" \
-- Du. dial _dodderen_ "to bump" \
-- G _dotzen_, _dutzen_ "to bounce, bump" \
REF: [EDPG]\: 555

### <a id="þwahan">\*þwahan- [st.] "to wash" (v)</a>
E: \
-- Go. _þwahan_ \
N: \
-- ON _þvá_ \
-- Far. _tváa_ [st./wk.] \
-- Elfd. _twå_ [irr.] \
W: \
-- OE _þwēan_ \
-- OS _thwahan_ \
-- MDu. _dwaen_ \
-- OHG _dwahan_, MHG _dwahen_, _twahen_, _zwahen_ \
REF: [EDPG]\: 555

### <a id="þwahla">\*þwahla- [n.] "washing, bathing" (n)</a>
E: \
-- Go. _þwahl_ \
N: \
-- ON _þváll_ [m.] "soap" \
W: \
-- OE _þwēal_, _þwæhl_ [m./n.] "washing" \
-- OHG _dwahal_ "bath, baptism" \
REF: [EDPG]\: 555

### <a id="þwangi">\*þwangi- [m.] "latchet" (n)</a>
N: \
-- ON _þvengr_ "thong, latchet" \
-- Far. _tvongur_ "shoelace" \
W: \
-- OE _þwang_ [m./f.] "thong, strip of leather", E _thong_ \
REF: [EDPG]\: 555

### <a id="þwarōn">\*þwarōn- [f.] "stirrer" (n)</a>
N: \
-- ON _þvara_ "pot-stick, stirrer" \
-- Far. _tvøra_ "stirrer with four scoops" \
REF: [EDPG]\: 555

### <a id="þweran">\*þweran- [st.] "to swirl" (v)</a>
W: \
-- OE _þweran_ "to twirl, stir" \
-- OHG _dweran_ "to stir up", MHG _twern_ "to turn quickly, stir up" \
REF: [EDPG]\: 555

### <a id="þwerha">\*þwerha- "cross" (adj)</a>
E: \
-- Go. _þwairhs_ "angry, irate, quicktempered" \
N: \
-- ON _þverr_ "transverse; adverse, contrary; cross, obstinate" \
-- Icel. _þver_ "transverse, lying-across; headstrong, unyielding" \
-- Far. _tvørur_ "transverse; difficult; obstinate; strange" \
W: \
-- OE _þweorh_, _þwerh_ "crooked, cross; adverse, opposed" \
-- OFri. _thweres_ "cross" (adv) \
-- OS _thwerh_ "stupid", MLG _dwer_ "cross, lateral" \
-- OHG _dwerah_ "cross, lateral", MHG _dwerch_, _twerch_ "id.", G _quer_, obs. _zwerch_ "id." \
REF: [EDPG]\: 556

### <a id="þwīnan">\*þwīnan- [st.] "to abate, disappear" (v)</a>
N: \
-- OSw. _þvina_ [wk.] "to dwindle, pine", Sw. _tvina_ [wk.] "id." \
-- Da. _tvine_ [st./wk.] "to disappear" \
W: \
-- OE _þwīnan_ "to go down, lessen, abate" \
REF: [EDPG]\: 556

### <a id="þwinhan">\*þwinhan- [st.] "to force" (v)</a>
W: \
-- OFri. _thwinga_ \
-- OS _thwingan_ "to force, urge" \
-- Du. _dwingen_ "id." \
-- OHG _dwingan_ "to force, subdue, oppress", G _zwingen_ "to force" \
REF: [EDPG]\: 556

## U

### <a id="uba">\*uba "under; above" (prep)</a>
E: \
-- Go. _uf_ "under" \
N: \
-- ON _of_ "over; about" \
W: \
-- OHG _ob(a)_ "above, on, over", G obs. _ob_ "above; due to" \
REF: [EDPG]\: 556

### <a id="uber">\*uber- "above, over" (prep)</a>
E: \
-- Go. _ufar_ "over" \
N: \
-- ON _yfir_ "over, above, beyond, across" \
-- Far. _yvir_ "id." \
-- Elfd. _yvyr_ "id." \
W: \
-- OE _ofer_, _ofor_, E _over_ \
-- OS _oƀar_ "over, across" \
-- Du. _over_ "id." \
-- OHG _ubar_ "id.", G _über_ "id." \
REF: [EDPG]\: 557

### <a id="ubila">\*ubila- "evil, bad" (adj)</a>
E: \
-- Go. _ubils_ "evil, bad" \
W: \
-- OE _yfel_ "evil, ill" \
-- OS _uƀil_ "evil" \
-- OHG _ubil_ "evil, bad, wrong" \
REF: [EDPG]\: 557

### <a id="ufna">\*ufna- [m.] "oven" (n)</a>
E: \
-- Go. _auhns_ \
N: \
-- ON _ofn_ "oven, furnace" \
-- Icel. _ofn_ "oven, stove, furnace, kiln", _ónn_ "furnace, ash-hole under a kiln" \
-- Far. _ovnur_ "oven, stove" \
-- OSw. _ughn_, _oghn_, _ofn_, _omn_ "id." \
-- Elfd. _ungen_ "id." \
W: \
-- OE _ofen_, _ofn_ "oven, furnace", E _oven_ \
-- OFri. _oven_ "id." \
-- MDu. _oven_ "id.", Du. _oven_ [c.] "id." \
-- OHG _ofan_, _ovan_ "id.", G _Ofen_ "id." \
REF: [EDPG]\: 557

### <a id="uftō">\*uftō "often" (adv)</a>
E: \
-- Go. _ufta_ \
N: \
-- ON _oft_ \
-- Far. _ofta_ \
W: \
-- OE _oft_, E _often_ \
-- OS _ofto_ \
-- OHG _ofto_, G _oft_ \
REF: [EDPG]\: 558

### <a id="ufuman">\*ufuman- "upmost, highest" (comp)</a>
E: \
-- Go. _auhuma\*_ "higher", _auhumists_ "highest" (superl) \
W: \
-- OE _ufemest_ "highest" (superl) \
REF: [EDPG]\: 558

### <a id="uhjōn">\*uhjōn- [wk.] "to sound" (v)</a>
N: \
-- Icel. _ýja_ "to remind, drop a hint" \
REF: [EDPG]\: 558

### <a id="uhsan">\*uhsan- [m.] "ox" (n)</a>
E: \
-- Go. _auhsa_, dat./acc. _auhsau_, gen.pl. _auhsne_, acc.pl. _auhsnuns_ \
N: \
-- ON _uxi_ \
-- Far. _oksi_ "ox; castrated bull" \
-- Elfd. _ukse_ "id." \
W: \
-- OE _oxa_ "id.", E _ox_ \
-- OFri. _oxa_ "id." \
-- OS _ohso_ "id." \
-- Du. _os_ [c.] "id." \
-- OHG _ohso_ "id.", G _Ochse_ "id." \
REF: [EDPG]\: 558

### <a id="umbi">\*umbi- "around, about" (prep)</a>
N: \
-- ON _umb_, _um_ \
-- Far. _um_ \
-- Elfd. _um_ \
W: \
-- OE _ymbe_, _ymb_ \
-- OS _umbi_ \
-- Du. _om_ \
-- OHG _umbi_, G _um_ \
REF: [EDPG]\: 558

### <a id="unda">\*unda "until" (prep)</a>
E: \
-- Go. _und_ "unto, up to, until" \
N: \
-- ON _unz (<_ \*und es (conj)) \
-- Icel. _uns_ \
W: \
-- OFri. _und_ \
REF: [EDPG]\: 559

### <a id="under">\*under- "under; among" (adv/prep)</a>
E: \
-- Go. _undar_ "under, underneath" (prep) \
N: \
-- ON _undir_ "under" \
W: \
-- OFri. _under_ "under, underneath" \
-- OS _undar_ "under, between" \
-- Du. _onder_ "id." (prep) \
-- OHG _untar_ "below, between", G _unter_ "id." (prep) \
REF: [EDPG]\: 559

### <a id="undi">\*undi- [f.] "affection" (n)</a>
N: \
-- ON _ǫfund_ "grudge, envy, ill-will" \
W: \
-- OHG _abunt_, _abunst_ [m.] "envy" \
REF: [EDPG]\: 559

### <a id="unhta">\*unhta- "accustomed" (adj)</a>
E: \
-- Go. _bi-uhts_ \
REF: [EDPG]\: 559

### <a id="unhtwōn">\*unhtwōn- [f.] "last part of the night" (n)</a>
E: \
-- Go. _uhtwo_ "foredawn, dawn, daybreak" \
N: \
-- ON _ótta_ \
-- Far. _ótta_ \
W: \
-- OE _ūht(a)_ [m.] "the time just before daybreak" (< \*unht(w)a(n)-) \
-- Du. _ochtend_ [c.] "morning" (with -_d_ from _avond_ "evening" < \*ēbanþ- ~ \*ēbund-) \
-- OHG _ūhta_ "morning, dawn" \
REF: [EDPG]\: 559

### <a id="unkan">\*unka(n)- [m.] "snake" (n)</a>
W: \
-- OHG _unk_ "snake, lizard", MHG _unc_, _unke_ "snake", G Rhnl. _unke_ "snake; newt" \
REF: [EDPG]\: 560

### <a id="unnan">\*unnan- [pp.] "to grant" (v)</a>
N: \
-- ON _unna_ "to grant; to love" \
-- Far. poet. _unna_ [wk.] "to love, like" \
-- Elfd. _unna_ [wk.] \
W: \
-- OE _unnan_ \
-- OS _gi-unnan_ \
-- Du. _gunnen_ [wk.] \
-- OHG _unnan_, G _gönnen_ [wk.] \
REF: [EDPG]\: 560

### <a id="unsti">\*unsti- [m./f.] "storm" (n)</a>
W: \
-- OE _ȳst_ [m.] \
-- OHG _unst_ [f.] \
REF: [EDPG]\: 560

### <a id="unþi">\*unþi- ~ \*unþjō- [f.] "wave" (n)</a>
N: \
-- ON _unnr_, _uðr_ \
W: \
-- OE _ȳð_ \
-- OS _ūthia_ \
-- OHG _undea_ "flood, wave", MHG _ünde_ "id." \
REF: [EDPG]\: 560

### <a id="ūra">\*ūra- [n.] "(ferriferous) sand" (n)</a>
W: \
-- Du. _oer_ "ferriferous sand, bog iron" \
REF: [EDPG]\: 561

### <a id="urba">\*urba- [n.] "inheritance" (n)</a>
N: \
-- OSw. _orf_, _urf_ "piece of cattle, property, inheritance" \
W: \
-- OE _orf_ "cattle" \
REF: [EDPG]\: 561

### <a id="ūru">\*ūru- [m.] "aurochs" (n)</a>
N: \
-- ON _úrr_ \
W: \
-- OE _ūr_ "kind of ox, bison" \
-- OHG _ūro_, MHG _ūre_, _ūr_, G _Auer-ochs_ \
REF: [EDPG]\: 561

### <a id="urzan">\*urzan- [m.] "black grouse, capercaillie" (n)</a>
N: \
-- ON _orri_ "black grouse" \
-- Far. _orri_ "id." \
-- Elfd. _uorre_ "id." \
W: \
-- OE _wōr-hana_ "pheasant" \
-- Du. _woer-haan_ "male pheasant" \
-- OHG _orre-huon_ "capercaillie", MHG _or-han_, _ūr-han_ "id.", G _Auer-huhn_ [n.] "id." \
REF: [EDPG]\: 561

### <a id="ustrja">\*ustr(j)a- "energetic" (adj)</a>
N: \
-- Sw. _yster_ "lively" \
W: \
-- OHG _ustar_ "stingy" \
REF: [EDPG]\: 562

### <a id="ūt">\*ūt "out" (adv)</a>
E: \
-- Go. _ut_ \
N: \
-- ON _út_ \
-- Far. _út_ \
-- Elfd. _aut_ \
W: \
-- OE _ūt_, E _out_ \
-- OFri. _ūt_ \
-- OS _ūt_ \
-- Du. _uit_ (prep/adv) \
-- OHG _ūz_ (prep/adv) "out, outside, but", G _aus_ (prep/adv) "id." \
REF: [EDPG]\: 562

### <a id="utra">\*utra- [m.] "otter" (n)</a>
N: \
-- ON _otr_ \
-- Far. _otur_ \
-- Elfd. _uotter_ \
W: \
-- OE _oter_, _otter_, E _otter_ \
-- Du. _otter_ \
-- OHG _otter_, G _Otter_ \
REF: [EDPG]\: 562

### <a id="uwur">\*u(w)ur- [n.] "drizzle" (n)</a>
N: \
-- ON _úr_ "drizzling rain" \
-- Elfd. _aur-weðer_ "snowstorm" \
REF: [EDPG]\: 562

### <a id="uwwilōn">\*uwwilōn- [f.] "owl" (n)</a>
N: \
-- ON _ugla_ \
-- Far. _ugli_ [m.], _ugla_ \
-- Elfd. _ugel_ \
W: \
-- OE _ūle_, E _owl_ \
-- MDu. _ule_ [m./f.], Du. _uil_ [c.] \
-- OHG _ūwila_, _ūla_, G _Eule_ \
REF: [EDPG]\: 562

### <a id="uz">\*uz "out (of)" (prep)</a>
E: \
-- Go. _us_ \
N: \
-- ON _ór_, _úr_ \
-- Far. _úr_ \
-- Nw. _or_ \
-- Elfd. _yr_ \
REF: [EDPG]\: 563

### <a id="uzda">\*uzda- [m.] "tip, prickle" (n)</a>
N: \
-- ON _oddr_ "point of a weapon; spear; spur" \
-- Far. _oddur_ "point, tip, end" \
-- Elfd. _udd_ "point, edge, cusp" \
W: \
-- OE _ord_ "point (of a weapon); line of battle; beginning" \
-- OS _ord_ "point" \
-- Du. _oord_ [n.] "location" \
-- OHG _ort_ [m./n.] "point, end, prickle", G _Ort_ "place, spot" \
REF: [EDPG]\: 563

### <a id="uz">\*uz-ētan- [m.] "manger; feed" (n)</a>
E: \
-- Go. _uz-eta_ "crib, manger" \
W: \
-- ME _orte_ "(refuse of) animal feed", E _orts_ \
-- MLG _ort(e)_ "remainder of animal feed; undigested food" \
-- MDu. _or(a)ete_ "leftovers from animal feed", Du. obs. _oort_ [n.] "leftovers" \
REF: [EDPG]\: 563

### <a id="uznan">\*uznan- [wk.] "to become warm" (v)</a>
N: \
-- Far. _orna_ \
-- OSw. _orna_ \
-- Elfd. _uonna_ \
REF: [EDPG]\: 563

## W

### <a id="wabja">\*wabja- [n.] "web" (n)</a>
N: \
-- ON _vefr_ [m.] "web; woven cloth" \
-- Far. _vevur_ [m.] "web; texture, tissue" \
-- Elfd. _wev_ [m.] \
W: \
-- OE _webb_, E _web_ \
-- OFri. _web_, _wob_, _wub_ \
-- OS _webbi_ \
-- Du. _web_ \
-- OHG _weppi_, MHG _weppe_ \
REF: [EDPG]\: 563

### <a id="wabjan">\*wabjan- [wk.] "to wrap" (v)</a>
N: \
-- ON _vefja_ "to wrap, fold; to entangle" \
-- Far. _vevja_ "to twist, twin; to wrap, swaddle" \
-- Elfd. _weva_ "to wrap" \
REF: [EDPG]\: 564

### <a id="wada">\*wada- [m.] "(fishing-)line" (n)</a>
N: \
-- ON _vaðr_ "fishing-line; line for measuring" \
-- Far. _vað_ [n.] "fishing-line" \
REF: [EDPG]\: 564

### <a id="wadan">\*wadan- [st.] "to wade" (v)</a>
N: \
-- ON _vaða_ \
-- Far. _vaða_ "to wade; to wash up to" \
W: \
-- OE _wadan_ "to go, wade", E _to wade_ \
-- OFri. _wada_ \
-- MDu. _waden_ "to go, proceed, wade; to flow, melt; to sink into", Du. _waden_ [wk.] "to wade" \
-- OHG _watan_, G _waten_ \
REF: [EDPG]\: 564

### <a id="wadja">\*wadja- [n.] "pledge, surety" (n)</a>
E: \
-- Go. _wadi_ \
N: \
-- ON _veð_ \
-- Far. _veð_ \
W: \
-- OE _wed_ \
-- OFri. _wed_ \
-- MLG _wedde_ "pledge, pledge agreement, fine, agreement, law" \
-- MDu. _wedde_ [m./n.] "pledge; surety; dowry; stake(s)", Du. _wedde_ [c.] "salary" \
-- OHG _weti_, _wetti_ "pledge; agreement; penance", G _Wette_ [f.] "bet" \
REF: [EDPG]\: 564

### <a id="wagja">\*wagja- [m.] "wedge" (n)</a>
N: \
-- ON _veggr_ \
-- Elfd. _wegg_ \
W: \
-- OE _wecg_, E _wedge_ \
-- OS _weggi_ \
-- EDu. _wig_ [c.] \
-- OHG _wecki_, G _Weck_ "bread roll" \
REF: [EDPG]\: 565

### <a id="wagjan">\*wagjan- [wk.] "to shake, move" (v)</a>
E: \
-- Go. _wagjan_ \
W: \
-- OE _wecgan_ \
-- OHG _weggen_, _wegen_ "to shake, provoke" \
REF: [EDPG]\: 565

### <a id="wagna">\*wagna- [m.] "wagon" (n)</a>
N: \
-- ON _vagn_ \
-- Far. _vognur_ \
-- Elfd. _waungen_ \
W: \
-- OE _wægn_, _wægen_, E _wain_ \
-- OFri. _wein_ \
-- Du. _wagen_ [c.] \
-- OHG _wagan_, G _Wagen_ \
REF: [EDPG]\: 565

### <a id="wagnisan">\*wagnisan- [m.] "plowshare" (n)</a>
N: \
-- ON _vangsni_ \
-- Nw. _vagnse_, _vangsne_ \
W: \
-- OHG _waganso_, G Swi. _wäge(n)se_ \
REF: [EDPG]\: 565

### <a id="wahan">\*wahan- [st.] "to remark" (v)</a>
W: \
-- OS _gi-wegi_ "suggerat", MLG _ge-wagen_ "to notice" \
-- MDu. _ge-waghen_ "to make notice of, tell", Du. _ge-wagen_ "to make mention of" \
-- OHG _gi-wahan_ (pret. _gi-wuog_) "to memorate, mention" \
REF: [EDPG]\: 565

### <a id="wahsa">\*wahsa- [n.] "wax" (n)</a>
N: \
-- ON _vax_ \
-- Far. _vaks_ \
-- Elfd. _waks_ \
W: \
-- OE _weax_, E _wax_ \
-- OFri. _wax_ \
-- Du. _was_ [c.] \
-- OHG _wahs_, G _Wachs_ \
REF: [EDPG]\: 566

### <a id="wahsjan">\*wahs(j)an- [st.] "to grow" (v)</a>
E: \
-- Go. _wahsjan_ \
N: \
-- ON _vaxa_, _vexa_ \
-- Far. _vaksa_ \
-- Elfd. _weksa_ [wk.] \
W: \
-- OE _weahsan_, E _to wax_ \
-- OFri. _waxa_, _wexa_ \
-- OS _wahsan_ \
-- OHG _wahsan_, G _wachsen_ \
REF: [EDPG]\: 566

### <a id="wahstu">\*wahstu- [m.] "growth; stature" (n)</a>
E: \
-- Go. _wahstus_ \
N: \
-- ON _vǫxtr_ "growth; stature; yield" \
-- Far. _vøkstur_ "id." \
REF: [EDPG]\: 566

### <a id="wai">\*wai "woe" (intj)</a>
E: \
-- Go. _wai_ \
N: \
-- ON _vei_ \
W: \
-- OE _wā_, E _woe_ \
-- OS _wē_ \
-- Du. _wee_ \
-- OHG _wē_, G _Weh_ \
REF: [EDPG]\: 566

### <a id="waigō">\*waigō- [f.] "strength" (n)</a>
N: \
-- ON _veig_ "pith, strength" \
REF: [EDPG]\: 566

### <a id="waigra">\*waigra- "resistant" (adj)</a>
W: \
-- MDu. _we(i)ger_ \
-- OHG _weigar_ \
REF: [EDPG]\: 566

### <a id="waika">\*waika- "weak" (adj)</a>
N: \
-- ON _veikr_ "weak, sick" \
-- Far. _veikur_ \
W: \
-- OE _wāc_ "weak, miserable" \
-- OS _wēk_ "id." \
-- Du. _week_ "soft, weak" \
-- OHG _weich_ "id.", G _weich_ "id." \
REF: [EDPG]\: 567

### <a id="wainaga">\*wainaga- "miserable, poor" (adj)</a>
E: \
-- Go. _wainags_ \
W: \
-- Du. _weinig_ "few, little" \
-- OHG _wēnag_ "id.", G _wenig_ "few, little" \
REF: [EDPG]\: 567

### <a id="waisunda">\*waisunda/ō- [m./f.] "windpipe; gullet" (n)</a>
W: \
-- OE _wāsend_ [m.] "windpipe; gullet" \
-- OFri. _wāsanda_, _wāsenda_ [m.] "wind-pipe" \
-- OHG _weisunt_, _weisant_ [f.] "wind-pipe; vein" \
REF: [EDPG]\: 567

### <a id="waiza">\*waiza- [n.] "seaweed(?)" (n)</a>
W: \
-- OE _wār_ "seaweed; sand", ME _wōr_ "seashore, beach; seaweed" \
-- MDu. _wier_ "id.", _woer_ [f.] "seaweed", EDu. Zeal. _woer_ "id.", Du. _wier_ "id." \
REF: [EDPG]\: 567

### <a id="waizda">\*waizda- [n.] "woad" (n)</a>
W: \
-- OE _wād_, E _woad_ \
-- OFri. _wēde_ [f.] (?) \
-- OS _wēd_ [m.(?)] \
-- ODu. _wēd(e)_ [m.(?)], Du. _wede_ [c.] \
-- OHG _weit_ [m.], G _Weit_ [m.] \
REF: [EDPG]\: 567

### <a id="wajju">\*wajju- [m.] "wall" (n)</a>
E: \
-- Go. _waddjus_ \
N: \
-- ON _veggr_ \
-- Far. _veggur_ \
-- Elfd. _wegg_ \
REF: [EDPG]\: 568

### <a id="wakan">\*wakan- [st.] "to awaken" (v)</a>
E: \
-- Go. _wakan_ \
N: \
-- ON _vakinn_ ptc. "awake" \
W: \
-- OE _wacan_ "to be born" \
REF: [EDPG]\: 568

### <a id="wakjan">\*wakjan- [wk.] "to awaken" (v)</a>
E: \
-- Go. _us-wakjan_ \
N: \
-- ON _vekja_ \
-- Far. _vekja_ \
-- Elfd. _wettja_ \
W: \
-- OE _weccan_ \
-- OS _wekkian_ \
-- Du. _wekken_ \
-- OHG _wecken_, G _wecken_ \
REF: [EDPG]\: 568

### <a id="wakkōn">\*wakk/gōn- [wk.] "to move, shake" (v)</a>
N: \
-- ON _vaga_ "to move" \
-- Far. _vagga_ "to rock; to stagger" \
-- Nw. _vakka_ [st.] "to travel" \
W: \
-- OE _wagian_ "to oscillate, shake", ME _wag(g)en_ "id.", E _to wag_ \
-- MDu. _waggen_ [st.] "to shake, waver", Du. _waggelen_ "to stagger" \
-- OHG _wagōn_ "to move, shake; to dare", G _wagen_ "to dare" \
REF: [EDPG]\: 568

### <a id="wakra">\*wakra- "alert, awake" (adj)</a>
N: \
-- ON _vakr_ \
W: \
-- OE _wacor_ \
-- Du. _wakker_ \
-- OHG _wackar_, G _wacker_ \
REF: [EDPG]\: 568

### <a id="wakwa">\*wakwa- "moist" (adj)</a>
N: \
-- ON _vǫkr_ \
W: \
-- MDu. _wac_ \
REF: [EDPG]\: 569

### <a id="wala">\*wala- [n.] "the slain" (n)</a>
N: \
-- ON _valr_ [m.] \
W: \
-- OE _wæl_ "id.; battle field" \
-- OHG _wal_ "id." \
REF: [EDPG]\: 569

### <a id="waldan">\*waldan- [st.] "to rule, have authority over" (v)</a>
E: \
-- Go. _waldan_ \
N: \
-- ON _valda_ "to rule over; to cause" \
W: \
-- OE _wealdan_ "to have power over", E _to wield_ \
-- OFri. _walda_ "id." \
-- OHG _waltan_ "id." \
REF: [EDPG]\: 569

### <a id="waldō">\*waldō- [f.] "dyer's rocket" (n)</a>
W: \
-- ME _wēlde_, _wolde_, E _weld_ \
-- MDu. _wolde_, _woude_, Du. _wouw_ [c.] \
-- G _Wau_ [m.] \
REF: [EDPG]\: 569

### <a id="waljan">\*waljan- [wk.] "to choose" (v)</a>
E: \
-- Go. _waljan_ \
N: \
-- ON _velja_ \
-- Far. _velja_ \
-- Elfd. _wela_ \
W: \
-- OS _wellian_ \
-- OHG _wellan_, G _wählen_ \
REF: [EDPG]\: 570

### <a id="walkan">\*walkan- [st.] "to roll" (v)</a>
W: \
-- OE _wealcan_ "to roll, toss; to wind", ME _walken_ [st./wk.] "to roll, toss; to wander, proceed", E _to walk_ \
-- OHG _walkan_ "to full, felt" \
REF: [EDPG]\: 570

### <a id="walkōn">\*walk/gōn- [wk.] "to roll" (v)</a>
N: \
-- ON _valka_ "toss to and fro" \
-- Far. _válka_ "to make a mess" \
W: \
-- MLG _walgen_ "to struggle, wrestle"; imp. "to be disgusted" \
-- MDu. _walgen_ (imp.) "to be nauseated", Du. _walgen_ "to be disgusted" \
-- OHG _walgōn_ "to roll" \
REF: [EDPG]\: 570

### <a id="wallan">\*wallan- [st.] "to well up, boil, seethe" (v)</a>
W: \
-- OE _weallan_ "to well, bubble forth, spring out, flow" \
-- OFri. _walla_ "id." \
-- OS _wallan_ "id." \
-- OHG _wallan_ "id.", G _wallen_ [wk.] "id." \
REF: [EDPG]\: 571

### <a id="wallōn">\*wal(l)ōn- [wk.] "to roll" (v)</a>
W: \
-- OE _weallian_ "to wander, roam" \
-- MDu. _wal(l)en_ "to turn, roll, wander", EDu. _wallen_ "to turn" \
-- OHG _wal(l)ōn_ "to wander, roam" \
REF: [EDPG]\: 571

### <a id="walō">\*walō- [f.] "golden oriole" (n)</a>
W: \
-- ME _wōde-wāle_ "golden oriole, green woodpecker", E _wood-wall_, _hick-wall_ \
-- WFri. _giele-gou_ [c.] "golden oriole" (= "yellow-wall", i.e. with restored reduplication) \
-- MDu. _wede-wale_ "id.", Du. _wiele-waal_ [c.] "id." \
-- MHG _wite-wal_ "id." \
REF: [EDPG]\: 571

### <a id="walþu">\*walþu- [m.] "field, uncultivated area, wood" (n)</a>
N: \
-- ON _vǫllr_ "field" \
-- Far. _vøllur_ "greensward" \
-- Elfd. _wall_ "field" \
W: \
-- OE _weald_ "wood" \
-- OFri. _wald_ [m./n.] "id." \
-- OS _wald_ "id." \
-- Du. _woud_ [n.] "id." \
-- OHG _wald_ "id.", G _Wald_ "id." \
REF: [EDPG]\: 571

### <a id="walu">\*walu- [m.] "lump; staff" (n)</a>
E: \
-- Go. _walus_ \
N: \
-- ON _vǫlr_ "stick" \
-- Far. _vølur_ "lump, tumor, growth; steering pole" \
W: \
-- OFri. _walu-bera_ "pilgrim" \
REF: [EDPG]\: 572

### <a id="walwjan">\*walwjan- [wk.] "to roll" (v)</a>
E: \
-- Go. _-walwjan_ \
W: \
-- OE _wielwan_ \
REF: [EDPG]\: 572

### <a id="wambō">\*wambō- [f.] "womb, belly" (n)</a>
E: \
-- Go. _wamba_ \
N: \
-- ON _vǫmb_ \
-- Far. _vomb_ \
-- Elfd. _uomb_ \
W: \
-- OE _wamb_, E _womb_ \
-- OFri. _wamme_ \
-- ODu. _wamba_, MDu. _wamme_ \
-- OHG _wamba_, G _Wamme_, coll _Wampe_ [f.] "id." \
REF: [EDPG]\: 572

### <a id="wana-1">\*wana- "Jacking, missing, void" (adj)</a>
E: \
-- Go. _wans_ \
N: \
-- ON _vanr_ \
W: \
-- OE _wan_ \
-- OHG _wan_ \
REF: [EDPG]\: 572

### <a id="wana-2">\*wana- "accustomed" (adj)</a>
N: \
-- ON _vanr_ \
-- Far. _vanur_ \
-- Elfd. _wan_ \
REF: [EDPG]\: 573

### <a id="wanda">\*wanda- "turned, twisted" (adj)</a>
N: \
-- ON _vandr_ "difficult, bad" \
-- Elfd. _uond_ "bad" \
W: \
-- OS _wand_ "changeable" \
REF: [EDPG]\: 573

### <a id="wandjan">\*wandjan- [wk.] "to cause to turn" (v)</a>
E: \
-- Go. _ga-wandjan_ "to turn, convert" \
N: \
-- ON _venda_ "to turn; to change, convert" \
-- Far. _venda_ "id." \
-- Elfd. _wenda_ "to turn (around)" \
W: \
-- OE _wendan_ "to turn, cause to move, change direction" \
-- OFri. _wenda_ "id." \
-- OS _wendian_ "to turn" \
-- Du. _wenden_ "id." \
-- OHG _wenten_ "id.", G wenden "id." \
REF: [EDPG]\: 573

### <a id="wandu">\*wandu- [m.] "rod, cane" (n)</a>
E: \
-- Go. _wandus_ \
N: \
-- ON _vǫndr_ "switch, twig; stripe (in cloth)" \
-- Far. _vond_ [f.] "mast; thin strip of wood nailed over a split in a boat; structure; weave of cloth" \
-- Elfd. _uond_ "stick put on top of a hayrack" \
REF: [EDPG]\: 573

### <a id="wanga">\*wanga- [m.] "meadow" (n)</a>
E: \
-- Go. _waggs_ "meadow, park, paradise" \
N: \
-- ON _vangr_ "field" \
W: \
-- OE _wang_ "field; land" \
-- OS _wang_ "grove" \
-- OHG _holz-wanga_ [f.] "wooded area" \
REF: [EDPG]\: 573

### <a id="wantu">\*wantu- [m.] "mitten" (n)</a>
N: \
-- ON _vǫttr_ \
-- Far. _vøttur_ \
-- Sw. _vante_ [c.] \
-- Elfd. _uott_ \
W: \
-- MDu. _want_, _wante_, Du. _want_ [c.] \
REF: [EDPG]\: 573

### <a id="wara">\*wara- "aware" (adj)</a>
E: \
-- Go. _war\*_ "cautious" \
N: \
-- ON _varr_ \
-- Far. _varur_ \
-- Elfd. _war_ \
W: \
-- OE _wær_, E _aware_ \
-- OS _war_ "aware, careful" \
-- Du. _ge-waar_ "id." \
-- OHG _gi-war_ "id.", G _ge-wahr_ "id." \
REF: [EDPG]\: 574

### <a id="wardjan">\*wardjan- [wk.] "to cause to degenerate" (v)</a>
E: \
-- Go. _fra-wardjan_ "to corrupt, destroy" \
W: \
-- OE _ā-wierdan_ "to damage" \
-- OHG _far-werten_ "to damage, corrupt" \
REF: [EDPG]\: 574

### <a id="warha">\*warha- [n.] "rheum" (n)</a>
N: \
-- ON _var_ \
-- Nw. _var_ \
-- OSw. _var_ "mucus, rheum, pus" \
-- Elfd. _war_ "id." \
W: \
-- OHG _warah_ "pus, liquid manure" \
REF: [EDPG]\: 574

### <a id="warjan">\*warjan- [wk.] "to hinder, prevent" (v)</a>
E: \
-- Go. _warjan_ \
N: \
-- ON _verja_ "to defend, guard" \
-- Far. _verja_ "to defent, protect, cover" \
-- Elfd. _wera-s_ "to watch, guard" \
W: \
-- OE _werian_ "to hinder, check, restrain" \
-- OFri. _wera_ "to defend, resist" \
-- OS _werian_ "to p revent, hinder, protect" \
-- Du. _weren_ "to keep out" \
-- OHG _werren_, _weren_ "to defend, protect, hinder", G _wehren_ "to defend, fight" \
REF: [EDPG]\: 574

### <a id="warki">\*warki- [m.] "pain, ache" (n)</a>
N: \
-- ON _verkr_, gen. -_jar_ \
-- Far. _verkur_ "pain, hurt; swelling, blister" \
-- Elfd. _werk_ "id." \
REF: [EDPG]\: 574

### <a id="warma">\*warma- "warm" (adj)</a>
N: \
-- ON _varmr_ \
W: \
-- OE _wearm_, E _warm_ \
-- OFri. _warm_ \
-- OS _warm_ \
-- Du. _warm_ \
-- OHG _warm_, G _warm_ \
REF: [EDPG]\: 575

### <a id="warza">\*warza- [m.] "callus" (n)</a>
W: \
-- OE _wearr_ \
-- MDu. _war_, _weer_ "gnarl; callus", EDu. _weer_ "id." \
REF: [EDPG]\: 575

### <a id="wasra">\*wasra- ~ \*wazra- [n.] "spring" (n)</a>
N: \
-- ON _vár_ \
-- Far. _vár_ \
-- Elfd. _wår_ \
W: \
-- OFri. _wars_, _wērs_ [m.] \
REF: [EDPG]\: 575

### <a id="waskan">\*waskan- [st.] "to wash" (v)</a>
W: \
-- OE _wæscan_, E _to wash_ \
-- OFri. _waska_ \
-- OS _waskan_ \
-- Du. _wassen_ [st./wk.] \
-- OHG _wascan_, G waschen \
REF: [EDPG]\: 575

### <a id="watar">\*watar- ~ \*watan- [n.] "water" (n)</a>
E: \
-- Go. _wato_, gen. _watins_ \
N: \
-- ON _vatn_ \
-- Far. _vatn_ \
-- Elfd. _watten_ \
W: \
-- OE _wæter_, E _water_ \
-- OFri. _weter_ \
-- OS _watar_ \
-- Du. _water_ \
-- OHG _wazzar_, G _Wasser_ \
REF: [EDPG]\: 575

### <a id="waþwan">\*waþwan- [m.] "calf of the leg(?)" (n)</a>
N: \
-- ON _vǫðvi_ "muscle", _kvikva-vǫðvi_ "calf" \
-- Far. _vøddi_ "muscle; the thick of the thigh, buttock" \
W: \
-- OS _watho_ "calf" \
-- MDu. _waede_, _waey_ "id." \
-- OHG _wado_, _wada_ [f.] "id.", G _Wade_ [f.] "id." \
REF: [EDPG]\: 576

### <a id="wazjan">\*wazjan- [wk.] "to dress" (v)</a>
E: \
-- Go. _wasjan_ \
N: \
-- ON _verja_ \
W: \
-- OE _werian_, E _to wear_ \
-- OHG _werian_ \
REF: [EDPG]\: 576

### <a id="wēan">\*wēan- [st.] "to blow" (v)</a>
E: \
-- Go. _waian_ \
W: \
-- OE _wāwan_ \
-- OFri. _wāia_ \
-- Du. _waaien_ [st./wk.] \
-- OHG _wāen_, G _wehen_ \
REF: [EDPG]\: 576

### <a id="weban">\*weban- [st.] "to weave" (v)</a>
N: \
-- ON _vefa_ \
-- Far. _veva_ \
-- Elfd. _weva_ \
W: \
-- OE _wefan_, E _to weave_ \
-- Du. _weven_ [st./wk.] \
-- OHG _weban_, G _weben_ [st./wk.] \
REF: [EDPG]\: 576

### <a id="wēbna">\*wēbna- ~ \*wēpna- [n.] "weapon" (n)</a>
E: \
-- Go. _wepn\*_ \
N: \
-- ON _vápn_, _vákn_, _vámn_ \
-- Icel. _vopn_, obs. _vokn_ \
-- Far. _vápn_, _vákn_ "weapon; whaling lance" \
-- OSw. _vāpn_, _vākn_ "id." \
-- Elfd. _wapen_ "id." \
W: \
-- OE _wǣpn_, _wǣmn_ "id.", E _weapon_ \
-- OFri. _wēpen_ "id." \
-- OS _wāpan_ "id." \
-- Du. _wapen_ "id." \
-- OHG _wāfan_ "id.", G _Waffe_ [f.] "id." \
REF: [EDPG]\: 577

### <a id="wedan">\*wedan- [st.] "to conjoin" (v)</a>
E: \
-- Go. _ga-widan_ \
W: \
-- OHG _wetan_ "to join, yoke", MHG _weten_ "id." \
REF: [EDPG]\: 577

### <a id="wega">\*wega- [m.] "way, road" (n)</a>
E: \
-- Go. _wigs_ \
N: \
-- ON _vegr_ \
W: \
-- OE _weg_, E _way_ \
-- OFri. _wei_ \
-- OS _weg_ \
-- Du. _weg_ [c.] \
-- OHG _weg_, G _Weg_ \
REF: [EDPG]\: 577

### <a id="wegan">\*wegan- [st.] "to move, carry" (v)</a>
E: \
-- Go. _ga-wigan_ "to shake, move" \
N: \
-- ON _vega_ "to move, carry, lift, weigh" \
-- Far. _vega_ "to lift, raise; to weigh" \
W: \
-- OE _wegan_ "to move, carry, bring", E _to weigh_ \
-- OFri. _wega_ "to bring" \
-- OS _gi-wegan_ "to bring up", wegan "to weigh" \
-- Du. _wegen_ "to weigh" \
-- OHG _wegan_ "to move; to weigh; to consider", G _wegen_ "to weigh" \
REF: [EDPG]\: 577

### <a id="wehti-1">\*wehti- [f.] "thing" (n)</a>
E: \
-- Go. _waihts_ "thing, entity, matter" \
N: \
-- ON _véttr_ "being, thing" \
-- Far. _vættur_ "spirit" \
W: \
-- OE _wiht_ "being, demon, thing" \
-- OS _wiht_ [n.] "something", [m.pl.] "being, demon" \
-- Du. _wicht_ [n.] "girl" \
-- OHG _wiht_ [n.] "being, demon, thing", G _Wicht_ [m.] "midget" \
REF: [EDPG]\: 578

### <a id="wehti-2">\*wehti- [f.] "weight" (n)</a>
N: \
-- ON _vétt_ \
W: \
-- OE _wiht_, E _weight_ \
-- OFri. _wicht_ \
REF: [EDPG]\: 578

### <a id="weljan-1">\*weljan- [st.] "to want" (v)</a>
E: \
-- Go. _wiljan_ [wk.] \
N: \
-- ON _vilja_ [wk.] \
-- Far. _vilja_ [wk.] \
-- Elfd. _wila_ [wk.] \
W: \
-- OE _willan_ [wk.] \
-- OFri. _willa_ [wk.] \
-- Du. _willen_ [wk.] \
-- OHG _wellan_ [wk.], G _wollen_ [wk.] \
REF: [EDPG]\: 578

### <a id="weljan-2">\*weljan- [m.] "will, wish" (n)</a>
E: \
-- Go. _wilja_ \
N: \
-- ON _vili_ \
-- Far. _vilji_ \
-- Elfd. _wile_ \
W: \
-- OE _willa_, E _will_ \
-- OFri. _willa_ \
-- OS _willio_ \
-- Du. _wil_ [c.] \
-- OHG _willo_, _willio_, G _Wille_ \
REF: [EDPG]\: 578

### <a id="welþja">\*welþja- "wild, uncultivated, untamed" (adj)</a>
E: \
-- Go. _wilþeis_ \
N: \
-- ON _villr_ \
W: \
-- OE _wild_, E _wild_ \
-- OFri. _wilde_ \
-- OS _wild_ \
-- Du. _wild_ \
-- OHG _wildi_, G _wild_ \
REF: [EDPG]\: 579

### <a id="weman">\*weman- [st.] "to drill(?)" (v)</a>
W: \
-- MDu. _wemen_ [st.?] "to drill(?)" \
REF: [EDPG]\: 579

### <a id="weni">\*weni- [m.] "friend" (n)</a>
N: \
-- ON _vinr_ \
-- Far. _vinur_ \
-- Elfd. _wenn_ \
W: \
-- OE _wine_ \
-- OFri. _wine_ \
-- OS _wini_ \
-- OHG _wini_ "friend, beloved", MHG _wine_ "id." \
REF: [EDPG]\: 579

### <a id="wēni">\*wēni- [f.] "expectation" (n)</a>
E: \
-- Go. _wens_ "hope, expectation" \
N: \
-- ON _ván_ "hope, expectation, prospect" \
-- Far. _vón_ "id." \
-- Elfd. _uon_ (indecl) "id." \
W: \
-- OE _wēn_ "supposition, opinion, thought" \
-- OFri. _wēn_ [m.] "suspition" \
-- OS _wān_ [m.] "expectation" \
-- Du. _waan_ [c.] "delusion" \
-- OHG _wān_ [m.] "opinion, hope, fancy", MHG _wān_ [m.] "assumption, illusion, expectation", G _Wahn_ [m.] "illusion, delusion" \
REF: [EDPG]\: 579

### <a id="wera">\*wera- [n.] "fishing weir" (n)</a>
N: \
-- Icel. _ver_ "fishing station" \
W: \
-- OE _wer_ [m.] "weir", E _weir_ \
-- Du. _weer_ "fishing weir; strip of land between two canals" \
-- MHG _wer_ "dam" \
REF: [EDPG]\: 579

### <a id="werdu">\*werdu- [m.] "host" (n)</a>
E: \
-- Go. _wairdus_ \
W: \
-- OFri. _hūs-werda_ "landlord" \
-- OS _werd_, _wird_ "host, landlord" \
-- Du. _waard_ [c.] \
-- OHG _wirt_ "landlord, husband, host", G _Wirt_ "host" \
REF: [EDPG]\: 579

### <a id="wergan">\*wergan- [st.] "to strangle" (v)</a>
W: \
-- MHG _er-wergen_ \
REF: [EDPG]\: 580

### <a id="werila">\*werila/ō- [m./f.] "lip" (n)</a>
E: \
-- Go. _wairila\*_ [f.] \
W: \
-- OE _weleras_ [m.pl.] \
REF: [EDPG]\: 580

### <a id="werka-1">\*werka- [n.] "labor, work, act" (n)</a>
N: \
-- ON _verk_ \
-- Far. _verk_ \
-- Elfd. _werk_ \
W: \
-- OE _weorc_, E _work_ \
-- OFri. _werk_, WFri. _wurk_ \
-- OS _werk_ \
-- Du. _werk_ \
-- OHG _werah_, _werh_, _werc_, G _Werk_ \
REF: [EDPG]\: 580

### <a id="werka-2">\*werka- [n.] "rope" (n)</a>
W: \
-- WFri. _wurk_ "string of hemp, rope" \
-- MDu. _werc_ "id." \
-- OHG _werh_, _werih_ "id.", G _Werg_ "id." \
REF: [EDPG]\: 580

### <a id="werkan">\*werkan- [st.] "to bend, warp" (v)</a>
N: \
-- Sw. _vurken_ "pulled crooked" \
W: \
-- Du. _werken_ [wk.] "to warp (of e.g. wood)" \
REF: [EDPG]\: 580

### <a id="werōn">\*werōn- [f.] "stye, pustule" (n)</a>
N: \
-- Nw. _vere_ "larva of the gad-fly, hole in the skin resulting from gad-fly infestation" \
-- Sw. dial. _verna_ "id." \
-- Da. _verne_ [c.] "warble; warble fly larva" \
W: \
-- Du. _weern_ [c.] "stye" \
-- G _Werre_ "mole cricket" \
REF: [EDPG]\: 580

### <a id="werpan">\*werpan- [st.] "to warp, sling, throw" (v)</a>
E: \
-- Go. _wairpan_ "to throw" \
N: \
-- ON _verpa_ "to throw; to pile up; to lay eggs" \
-- Far. _verpa_ "to lay eggs" \
W: \
-- OE _weorpan_ "to cast, throw, fling", E _to warp_ \
-- OFri. _werpa_ "id." \
-- OS _werpan_ "id." \
-- Du. _werpen_ "id." \
-- OHG _werfan_ "id.", G _werfen_ "id." \
REF: [EDPG]\: 581

### <a id="werþa">\*werþa- "worth" (adj)</a>
E: \
-- Go. _wairþs_ \
N: \
-- ON _verðr_ \
-- Far. _verður_ \
-- Elfd. _werd_ \
W: \
-- OE _weorþ_, _worþ_, E _worth_ \
-- OFri. _werth_, _werd_, _wird_ \
REF: [EDPG]\: 581

### <a id="werþan">\*werþan- [st.] "to come about, happen, become" (v)</a>
E: \
-- Go. _wairþan_ \
N: \
-- ON _verða_ \
-- Far. _verða_ \
-- Nw. _verta_ \
-- Elfd. _werda_ \
W: \
-- OE _weorþan_ \
-- OFri. _wertha_ \
-- OS _werthan_ \
-- Du. _warden_ [irr.] \
-- OHG _werdan_, G _werden_ \
REF: [EDPG]\: 581

### <a id="wesan-1">\*wesan- [st.] "to be" (v)</a>
E: \
-- Go. _wisan_ \
N: \
-- ON _vesa_, _vera_ \
-- Far. _vera_ \
-- Elfd. _wårå_ \
W: \
-- OE _wesan_ \
-- OFri. _wesa_ \
-- OS _wesan_ \
-- Du. _wezen_ \
-- OHG _wesan_ \
REF: [EDPG]\: 582

### <a id="wesan-2">\*wesan- [st.] "to consume, feast" (v)</a>
E: \
-- Go. _waila wisan_ "to feast, enjoy", _fra-wisan_ "to consume" \
W: \
-- OE _wesan_ "to feast", ME _wes_ "tended livestock, pastured" \
-- OHG _fir-wesan_ [wk.] "id.", \
REF: [EDPG]\: 582

### <a id="westera">\*westera- "west, westwards" (adv)</a>
N: \
-- ON _vestr_ \
-- Far. _vestur_ \
-- Elfd. _wester_ \
W: \
-- OFri. _wester_ \
-- OS _westar_ \
-- OHG _westar_, MHG _wester_ \
REF: [EDPG]\: 582

### <a id="westi">\*westi- [f.] "being; staying" (n)</a>
E: \
-- Go. _wists_ "essence, nature" \
N: \
-- ON _vist_ "stay, abode; subsistence, food" \
W: \
-- OE _wist_ "being, subsistence, food" \
-- OS _wist_ [m.] "food" \
REF: [EDPG]\: 583

### <a id="wēta">\*wēta- "wet" (adj)</a>
N: \
-- ON _vátr_ \
-- Far. _vátur_ \
-- Elfd. _wåt_ \
W: \
-- OE _wǣt_, E _wet_ \
-- OFri. _wet_, WFri. _wiet_ \
REF: [EDPG]\: 583

### <a id="wetman">\*wetman- [m.] "dowry" (n)</a>
W: \
-- OE _weotuma_, _wetma_-, _wituma_ \
-- OFri. _wetma_ \
-- ODu. _withemo_ \
-- OHG _widomo_, _widemo_, G _Wittum_ \
REF: [EDPG]\: 583

### <a id="weþra">\*weþra- ~ \*wedra- [n.] "weather" (n)</a>
N: \
-- ON _veðr_ \
-- Far. _veður_ \
-- Elfd. _weðer_ \
W: \
-- OE _weder_, E _weather_ \
-- OFri. _weder_ \
-- OS _wedar_ \
-- Du. _weer_ \
-- OHG _wetar_, G _Wetter_ "id." \
REF: [EDPG]\: 583

### <a id="weþru">\*weþru- "(male) lamb; yearling" (adv)</a>
E: \
-- Go. _wiþrus_ [m.] "lamb, young ram" \
N: \
-- ON _veðr_ [m.] "ram" \
W: \
-- OE _weðer_ [m.] "id.", E _wether_ \
-- OS _withar_ [m.] "id." \
-- EDu. _weer_ [c.] "id." \
-- OHG _widar_ [m.] "id.", G _Widder_ [m.] "id." \
REF: [EDPG]\: 584

### <a id="wīa">\*wīa- [n.] "egg(?), brood(?)" (n)</a>
N: \
-- Icel. _ví_ "fly egg" \
-- Far. _víggj_ "id." \
-- Sw. dial. _vɨ_ "mating of birds; brood of birds" \
REF: [EDPG]\: 584

### <a id="wība">\*wība- [n.] "woman, wife" (n)</a>
N: \
-- ON _víf_ \
-- Far. poet. _vív_ \
W: \
-- OE _wīf_, E _wife_ \
-- OS _wīf_ \
-- Du. _wijf_ \
-- OHG _wīb_, G _Weib_ \
REF: [EDPG]\: 584

### <a id="wīda">\*wīda- "wide, distant" (adj)</a>
N: \
-- ON _víðr_ "wide, large, extensive" \
W: \
-- OE _wīd_ "wide", E _wide_ \
-- OFri. _wīd_ "wide, broad" \
-- Du. _wijd_ "wide" \
-- OHG _wīt_ "wide, spacious", G _weit_ "wide, distant" \
REF: [EDPG]\: 584

### <a id="widu">\*widu- [m. (n.?)] "tree; wood" (n)</a>
N: \
-- ON _viðr_, gen. _viðar_ [m.] \
-- Far. _viður_ [m.] \
-- Elfd. _wið_ [m.] \
W: \
-- OE _widu_, _wiodu_, _wudu_ [m.], E _wood_ \
-- OHG _witu_ [m./n.], MHG _wite_, _wit_ [m.] \
REF: [EDPG]\: 585

### <a id="widuwōn">\*widu(w)ōn- [f.] "widow" (n)</a>
E: \
-- Go. _widuwo_ \
W: \
-- OE _widewe_, _wudewe_, E _widow_ \
-- OFri. _widwe_ \
-- OS _widowa_ \
-- Du. _weduwe_ [c.] \
-- OHG _wituwa_, G _Witwe_ \
REF: [EDPG]\: 585

### <a id="wiggan">\*wiggan- [f.] "weevil(?)" (n)</a>
W: \
-- OE _wicga_, _ēar-wicga_ [m.] "beetle, earwig", E _earwig_ \
REF: [EDPG]\: 585

### <a id="wīha-1">\*wīha- "holy" (adj)</a>
E: \
-- Go. _weihs_ \
W: \
-- OHG _wīh_ \
REF: [EDPG]\: 585

### <a id="wīha-2">\*wīha- [m./n.] "sanctuary" (n)</a>
N: \
-- ON _vé_ [n.] "mansion; sanctuary" \
W: \
-- OE _wēoh_, _wīg_ [m.] "idol" \
-- OS _wīh_ [m.] "temple" \
REF: [EDPG]\: 586

### <a id="wīhan">\*wī̆han- [st.] "to fight" (v)</a>
E: \
-- Go. _weihan_ \
N: \
-- ON _vega_ \
-- Far. _vega_ "to fight, struggle; to fence; to chop" \
W: \
-- OE _wīgan_ "to fight, do battle" \
-- MDu. _wijgen_ [wk.] "id." \
-- OHG _wīgan_ "id.", MHG _wīgen_ "id." \
REF: [EDPG]\: 586

### <a id="wikkōn">\*wikkōn- [wk.] "to practice sorcery" (v)</a>
W: \
-- OE _wiccian_ \
-- WFri. _wikje_ "to tell the future, warn" \
-- MDu. _wicken_ \
-- MHG _wicken_ \
REF: [EDPG]\: 586

### <a id="wikōn">\*wikōn- [f.] "sequence, week" (n)</a>
E: \
-- Go. _wiko_ "order, sequence" \
N: \
-- ON _vika_ "week; sea-mile" \
-- Far. _vika_ "week" \
-- Elfd. _wikå_ "id." \
W: \
-- OE _wice_, _wucu_ "id.", E _week_ \
-- OFri. _wike_ "id." \
-- Du. _week_ [c.] "id." \
-- OHG _wecha_ "id.", G _Woche_ "id." \
REF: [EDPG]\: 586

### <a id="wīkwan">\*wīkwan- [st.] "to give way; to turn (away)" (v)</a>
N: \
-- ON _víkva_, _ýkva_, _víkja_ \
-- Far. _víkja_ \
-- Elfd. _waika_ \
W: \
-- OE _wīcan_ \
-- OFri. _wíka_ \
-- OS _wíkan_ \
-- Du. _wijken_ \
-- OHG _wīhhan_, G _weichen_ \
REF: [EDPG]\: 586

### <a id="wīman">\*wīman- [m.] "osier, wattle" (n)</a>
W: \
-- MLG _wīm(e)_ \
-- MDu. _wīme_ [f.] \
REF: [EDPG]\: 587

### <a id="winda">\*winda- [m.] "wind" (n)</a>
E: \
-- Go. _winds_ \
N: \
-- ON _vindr_ \
W: \
-- OE _wind_, E _wind_ \
-- OFri. _wind_, WFri. _wyn_ [c.] \
-- OS _wind_ \
-- Du. _wind_ [c.] \
-- OHG _wint_, G _Wind_ \
REF: [EDPG]\: 587

### <a id="windan">\*windan- [st.] "to wind" (v)</a>
E: \
-- Go. _bi-windan_ "to wind, wrap" \
N: \
-- ON _vinda_ "to wring, twist" \
-- Far. _vinda_ "to turn; to twist, wring" \
-- Elfd. _winda_ [wk.] "to hurl, sling" \
W: \
-- OS _windan_ "turn, move" \
-- Du. _winden_ [wk.] \
-- OHG _wintan_, G _winden_ \
REF: [EDPG]\: 587

### <a id="winnan">\*winnan- [st.] "to suffer" (v)</a>
E: \
-- Go. _winnan_ \
N: \
-- ON _vinna_ "to work, labor" \
-- Far. _vinna_ "to carry out, perform; to gain, win" \
-- Elfd. _winna_ "to reach; to win" \
W: \
-- OE _winnan_ "id.", E _to win_ \
-- OFri. _winna_ "to achieve, win" \
-- OS _winnan_ "to suffer; to gain" \
-- Du. _winnen_ "to gain; to win" \
-- OHG _winnan_ "to labor; to win", G _winnen_ "to win" \
REF: [EDPG]\: 587

### <a id="wintru">\*wintru- [m.] "winter; year" (n)</a>
E: \
-- Go. _wintrus_ \
N: \
-- ON _vetr_ \
-- Far. _vetur_ \
-- Elfd. _witter_ \
W: \
-- OE _winter_, E _winter_ \
-- OFri. _winter_ \
-- OS _wintar_ \
-- Du. _winter_ [c.] \
-- OHG _wintar_, G _Winter_ \
REF: [EDPG]\: 588

### <a id="wira">\*wira- [m.] "man" (n)</a>
E: \
-- Go. _wair_ \
N: \
-- ON _verr_ \
W: \
-- OE _wer_ \
-- OS _wer_ \
-- OHG _wer_ \
REF: [EDPG]\: 588

### <a id="wissa">\*wissa- "certain" (adj)</a>
E: \
-- Go. _un-wiss_ "uncertain" \
N: \
-- ON _viss_ \
W: \
-- OE _wiss_ \
-- OFri. _wis_ \
-- Du. _wis_, _on-gewis_ "uncertain" \
-- OHG _gi-wis_, G _ge-wiß_ \
REF: [EDPG]\: 588

### <a id="wisund">\*wisund- [m.] "wisent" (n)</a>
N: \
-- ON _vísundr_, gen. _vísundar_ (« WGm.?) \
W: \
-- OE _wesend_, _weosend_ \
-- MLG _wisent_, _wesent_ \
-- MDu. _wesent_ \
-- OHG _wisunt_, _wisant_, pl. _wisunti_, G _Wisund_ \
REF: [EDPG]\: 588

### <a id="wit">\*wit- "we (du.)" (pron)</a>
E: \
-- Go. _wit_, obl. _ugkis_ \
N: \
-- ON _vit_, obl. _okkr_ \
-- Far. _vit_, obl. _okkum_ \
-- Elfd. _wįð_, obl. _uoss_ \
W: \
-- OE _wit_, obl. _unc_ \
-- OS _wit_, obl. _unk_ \
REF: [EDPG]\: 589

### <a id="witan">\*witan- [pp.] "to know" (v)</a>
E: \
-- Go. _witan_ \
N: \
-- ON _vita_ \
-- Elfd. _witå_ \
W: \
-- OE _witan_ \
-- OFri. _wita_ \
-- OS _witan_ \
-- EDu. _weten_ \
-- OHG _wizzan_, G _wissen_ \
REF: [EDPG]\: 589

### <a id="wītan">\*wītan- [st.] "to see; to blame" (v)</a>
E: \
-- Go. _fra-weitan_ "to avenge" \
W: \
-- OE _ge-wītan_ "to see, look; to go" \
-- Du. _wijten_ "to blame" \
-- OHG _(far-)wīzan_ "to rebuke", G _ver-weisen_ "id." \
REF: [EDPG]\: 589

### <a id="witēn">\*witēn- [wk.] "to watch, heed" (v)</a>
E: \
-- Go. _witan_ \
N: \
-- ON _vitaðr_ ptc. "known" \
W: \
-- OE _witian_ \
-- OHG _wizzēn_ \
REF: [EDPG]\: 589

### <a id="witōþa">\*witōþa- [n.] "law" (n)</a>
E: \
-- Go. _witoþ_ \
W: \
-- Du. _wet_ [c.] \
-- OHG _wizzōd_ [m./n.] "law, order, divine will" \
REF: [EDPG]\: 590

### <a id="wittōn">\*witt/dōn- [wk.] "to tremble" (v)</a>
E: \
-- Go. _wiþon_ \
N: \
-- Icel. _viða_ "to waver, swing" \
-- Nw. _vitta_ "to wave, beckon" \
REF: [EDPG]\: 590

### <a id="wiþra">\*wiþra "toward, against" (prep)</a>
E: \
-- Go. _wiþra_ \
N: \
-- ON _við_, _viðr_ "against; along with; with, by" \
-- Far. _við_ "with, at, by" \
W: \
-- OE _wiðer_ "id." \
-- OFri. _wither_ "against" \
-- OS _withar_ "against, in return" \
-- MDu. _weder_ \
-- OHG _widar_ \
REF: [EDPG]\: 590

### <a id="wīwan">\*wī̆wan- [m.] "kite" (n)</a>
W: \
-- MDu. _wouwe_, Du. _wouw_ [c.] \
-- OHG _wī(w)o_, MHG _wī(w)e_, _wewe_, G _Weihe_, Cimb. _bibo_ \
REF: [EDPG]\: 590

### <a id="wīz">\*wīz "we" (pron)</a>
E: \
-- Go. _weis_, obl. _uns_, _unsis_ \
N: \
-- ON _vér_, obl. _ós_, _oss_, _øss_ \
W: \
-- OE _wē̆_, obl. _ūs_, acc. _ūsic_, E _we_, obl. _us_ \
-- OS _wī_, _we_, obl. _ūs_ \
-- Du. _wij_, _we_, obl. _ons_ \
-- OHG _wir_, obl. _uns_, acc. _unsih_, G _wir_, obl. _uns_ \
REF: [EDPG]\: 590

### <a id="wlaitōn">\*wlaitōn- [wk.] "to look" (v)</a>
E: \
-- Go. _wlaiton_ "to look around" \
N: \
-- ON _leita_ "to search, look for" \
-- Far. _leita_ "id." \
-- Elfd. _lieta_ "to look for, search; to inspect" \
W: \
-- OE _wlātian_ "to gaze, look" \
REF: [EDPG]\: 591

### <a id="wlakwa">\*wlakwa- "lukewarm, tepid" (adj)</a>
W: \
-- OE _wlacu_, _wlæc_, ME _wlak_ \
-- MLG _wlak_ \
-- MHG _vlac_ \
REF: [EDPG]\: 591

### <a id="wlanka">\*wlanka- "bold" (adj)</a>
W: \
-- OE _wlanc_, _wlonc_ "proud, bold", ME _wlonk_ "excellent, fine; haughty, arrogant" \
-- OS _wlank_ "proud, arrogant" \
REF: [EDPG]\: 591

### <a id="wlītan">\*wlītan- [st.] "to see" (v)</a>
N: \
-- ON _líta_ \
-- Far. _líta_ "to cast a glance (in a certain direction)" \
W: \
-- OE _wlītan_ "to look, gaze" \
REF: [EDPG]\: 591

### <a id="wlittōn">\*wlittōn- [wk.] "to strive" (v)</a>
W: \
-- MHG _litzen_ "to strive for, desire" \
REF: [EDPG]\: 591

### <a id="wlōma">\*wlōma- "turbulent(?)" (adj)</a>
W: \
-- MLG _wlōm_, _wloum_ "turbulent" \
-- MDu. _wloemich_ "confused, anxious" \
REF: [EDPG]\: 592

### <a id="wōda">\*wōda- "delirious" (adj)</a>
E: \
-- Go. _wods_ "possessed" \
N: \
-- ON _óðr_ "frantic, furious" \
W: \
-- OE _wōd_ "insane" \
-- Du. obs. _woed_ "frantic, wild, crazy" \
REF: [EDPG]\: 592

### <a id="wōgi">\*wōgi- "moving a lot" (adj)</a>
W: \
-- MDu. _woeg_ "fidgety, restive" \
REF: [EDPG]\: 592

### <a id="wōgjan">\*wōgjan- [wk.] "to (make) remember" (v)</a>
W: \
-- MHG _wüegen_ \
REF: [EDPG]\: 592

### <a id="wōkra">\*wōkra- [m.] "growth" (n)</a>
E: \
-- Go. _wokrs_ "interest, additional increment" \
N: \
-- ON _okr_ [n.] "usury" \
W: \
-- OE _wōcor_ [f.] "offspring, interest" \
-- OFri. _wōker_ [m./n.] "interest; usury" \
-- Du. _woeker_ [c.] "rampant growth; usury" \
-- OHG _wuohhar_ [m./n.] "fruit, harvest, gain", G _Wucher_ "usury" \
REF: [EDPG]\: 592

### <a id="wōla">\*wōla- [m.] "doom(?)" (n)</a>
W: \
-- OE _wōl_ [m./f.] "pest, plague" \
-- OS _wōl_ "pest, ruin" \
-- OHG _wuol_ "ruin, disaster" \
REF: [EDPG]\: 592

### <a id="wōnō">\*wōnō- [f.] "hole in the ice" (n)</a>
W: \
-- MDu. _woene_ \
-- MHG _wuone_, G _Wuhne_ \
REF: [EDPG]\: 592

### <a id="wōrja">\*wōrja- "intoxicated" (adj)</a>
N: \
-- ON _œrr_ "raging, furious" \
W: \
-- OE _wērig_ "tired", E _weary_ \
-- OFri. _wirch_ "id.", WFri. _wurch_ "id." \
-- OS _wōrig_ "worn out" \
REF: [EDPG]\: 593

### <a id="wōstu">\*wōstu- "desert, waste" (adj)</a>
W: \
-- OE _wēste_ \
-- OS _wōsti_ \
-- Du. _woest_ \
-- OHG _wuosti_, G _wüst_ \
REF: [EDPG]\: 593

### <a id="wōpja">\*wōpja- "sweet, pleasant" (adj)</a>
E: \
-- Go. _woþeis_ \
W: \
-- OE _wēðe_ \
-- OS _wōthi_ \
-- OHG _wuodi_ \
REF: [EDPG]\: 593

### <a id="wraiha">\*wraiha- "crooked" (adj)</a>
N: \
-- Sw. dial. _vrå_ "obstinate" \
W: \
-- ME _wrau_ "contrary, angered, cross" \
REF: [EDPG]\: 593

### <a id="wraikwa">\*wraikwa- "crooked" (adj)</a>
E: \
-- Go. _wraiqus\*_ \
W: \
-- OFri. _wrāk_ \
REF: [EDPG]\: 593

### <a id="wraitjan">\*wraitjan- [wk.] "to vex, excite" (v)</a>
N: \
-- ON _reita_ \
W: \
-- OHG _reizen_ "to vex, tease, tempt", G _reizen_ "id." \
REF: [EDPG]\: 594

### <a id="wraiþa">\*wraiþa- "twisted; angry" (adj)</a>
N: \
-- ON _reiðr_ "angry" \
-- Far. _reiður_ "id." \
W: \
-- OE _wrǣð_ "id." \
-- OS _wrēth_ "wretched, hostile" \
-- OHG _reid_, _reidi_ "curly, wavy" \
REF: [EDPG]\: 594

### <a id="wrakjan">\*wrakjan- [m.] "persecutor" (n)</a>
W: \
-- OE _wrecca_ "exile, stranger" \
-- OS _wrekkio_ "foreign warrior, stranger, exile" \
-- OHG _reck(e)o_ "fugitive, hero", G _Recke_ "champion" \
REF: [EDPG]\: 594

### <a id="wrandan">\*wrandan-(?) [m.] "wren" (n)</a>
N: \
-- ON _rindil-þvari_ \
-- Icel. _rindill_ \
-- Nw. dial. _rennil_ "wagtail" \
W: \
-- OE _wrenna_, _werna_, E _wren_ \
-- OS _wrendo_, _wrendilo_ \
-- OHG _wrentilo_ \
REF: [EDPG]\: 594

### <a id="wranga">\*wranga- "twisted, wrong" (adj)</a>
N: \
-- ON _rangr_ "wry, crooked; wrong" \
-- Far. _rangur_ "slanted, inside out, wrong" \
-- Elfd. _rwaungg_ "contrary, obstinate" \
W: \
-- OE _wrang_ [n.] "wrong", E _wrong_ \
-- MDu. _wranc_ "bitter, sharp, unfriendly, brave", Du. _wrang_ "bitter" \
REF: [EDPG]\: 594

### <a id="wrankjan">\*wrankjan- [wk.] "to twist, wring" (v)</a>
W: \
-- OE _wrencan_ \
-- MHG _renken_, G _renken_ \
REF: [EDPG]\: 594

### <a id="wratjōn">\*wratjōn- [f.] "madder" (n)</a>
W: \
-- OE _wrætte_ \
-- OHG _rezza_ \
REF: [EDPG]\: 595

### <a id="wratōn">\*wratōn- [wk.] "to roam" (v)</a>
E: \
-- Go. _wraton_ "to travel" \
N: \
-- ON _rata_ "to travel; to meet with, find (the way)" \
W: \
-- MHG _razzeln_ "to turn" \
REF: [EDPG]\: 595

### <a id="wrekan">\*wrekan- [st.] "to pursue" (v)</a>
E: \
-- Go. _wrikan_ "to persecute" \
N: \
-- ON _reka_ "to hunt, pursue" \
W: \
-- OE _wrecan_ "to drive; to punish", E _to wreak_ \
-- OFri. _wreka_ "to revenge" \
-- OS _wrekan_ "id." \
-- Du. _wreken_ "id." \
-- OHG _rehhan_ "id.", G _rächen_ [wk.] "id." \
REF: [EDPG]\: 595

### <a id="wrēkō">\*wrēkō- [f.] "persecution, revenge" (n)</a>
N: \
-- ON _rák_ \
W: \
-- OFri. _wrēke_ "revenge" \
-- OS _wrāka_ "id." \
-- Du. _wraak_ [c.] "id." \
-- OHG _rāhha_ "id.", G _Rache_ "id." \
REF: [EDPG]\: 595

### <a id="wreskwan">\*wreskwan- [st.] "to mature" (v)</a>
E: \
-- Go. _ga-wrisqan_ \
N: \
-- ON _roskinn_ "mature" \
-- Icel. _roskinn_ "middle-aged" \
REF: [EDPG]\: 595

### <a id="wrēþu">\*wrēþu- [m.] "herd (of swine)" (n)</a>
E: \
-- Go. _wriþus_ (for expected \*_wreþus_) \
N: \
-- ODa. _vrāth_ "swineherd", Da. _vråd_ [c.] "id." \
W: \
-- OE _wrǣð_ "herd; host" \
REF: [EDPG]\: 595

### <a id="wrīhan">\*wrī̆han- [m.] "instep" (n)</a>
W: \
-- MDu. _wrijghe_, _wrijch_, _wrijf_, Du. _wreef_ [c.] \
-- OHG _rīho_, G _Reihen_ \
REF: [EDPG]\: 596

### <a id="wrīnan">\*wrīnan- [st.] "to howl" (v)</a>
N: \
-- Nw. _vrina_ "to howl; to bare one's teeth" \
-- Elfd. _rwaina_ "to howl, wail" \
REF: [EDPG]\: 596

### <a id="wringan">\*wringan- [st.] "to wring, squeeze" (v)</a>
W: \
-- OE _wringan_ "to wring, twist", E _to wring_ \
-- OS _ūt-wringan_ "to squeeze out" \
-- Du. _wringen_ "to wring" \
-- OHG _ringan_ "to wring; to wrestle", G _ringen_ "id." \
REF: [EDPG]\: 596

### <a id="wrinþla">\*wrinþla-(?) [m.] "a certain plant" (n)</a>
N: \
-- Gutn. _rind_ "clubmoss" \
W: \
-- ME _wrīthel_ "wild lettuce(?)" \
REF: [EDPG]\: 596

### <a id="wrītan">\*wrītan- [st.] "to carve" (v)</a>
W: \
-- OE _wrītan_ "to write; to carve" \
-- OFri. _wrīta_ "to write", WFri. _write_ "to tear; to ache, hurt" \
-- OS _wrītan_ "to tear; to write" \
REF: [EDPG]\: 596

### <a id="writi">\*writi- [m.] "stroke" (n)</a>
E: \
-- Go. _writs_ \
W: \
-- MDu. _ret_ "crack, fissure, lacuna" \
-- OHG _riz_ "scratching, line, letter", G _Riß_ "crack, scratch" \
REF: [EDPG]\: 597

### <a id="writtōn">\*writtōn- [wk.] "to carve, write" (v)</a>
W: \
-- OHG _rizzōn_, _rezzōn_, G _ritzen_ \
REF: [EDPG]\: 597

### <a id="wrīþan">\*wrīþan- ~ \*wrītan- [st.] "to twist" (v)</a>
N: \
-- ON _ríða_ "to turn, ring, tie" \
-- OSw. _vrīþa_ "to ring" \
-- Elfd. _rwaiða_ "to turn, twist, ring" \
W: \
-- OE _wrīðen_ "to twist, ring", E _writhe_ "id." \
-- MDu. _writen_ "to twist, ring, wrap" \
-- OHG _rīdan_ "to wind, turn", MHG _rīden_ "to wind, wrap" \
REF: [EDPG]\: 597

### <a id="wrōt">\*wrōt- [f.] "root" (n)</a>
N: \
-- ON _rót_, pl. _rœtr_ \
-- Far. _rót_, pl. _røtur_ \
-- OSw. _rōt_ \
-- Elfd. _ruot_, pl. _ryöter_ \
REF: [EDPG]\: 597

### <a id="wrungōn">\*wrungōn- [f.] "snare" (n)</a>
E: \
-- Go. _wruggo_ "snare, noose" \
REF: [EDPG]\: 597

### <a id="wulan">\*wulan- [st.] "to well up, boil, seethe" (v)</a>
E: \
-- Go. _wulan_ "to boil, be hot" \
REF: [EDPG]\: 597

### <a id="wulfa">\*wulfa- [m.] "wolf" (n)</a>
E: \
-- Go. _wulfs_ \
N: \
-- ON _ulfr_ \
-- Far. _úlvur_ \
-- Elfd. _ulv_ \
W: \
-- OE _wulf_, E _wolf_ \
-- OS _wulf_, _wolf_ \
-- Du. _wolf_ [c.] \
-- OHG _wolf_, G _Wolf_ \
REF: [EDPG]\: 598

### <a id="wulgī">\*wulgī- ~ \*wulbjō- [f.] "she-wolf" (n)</a>
N: \
-- ON _ylgr_, _ylgja_ \
-- Elfd. _ylva_ \
W: \
-- OHG _wulpa_, MHG _wülpe_ \
REF: [EDPG]\: 598

### <a id="wulhja">\*wulhja- [m.] "haze" (n)</a>
N: \
-- ON _ylr_ "warmth" \
-- Nw. _øl_ "(mild) warmth; steam" \
REF: [EDPG]\: 598

### <a id="wulkōn">\*wulk/gōn- [wk.] "to be nauseated" (v)</a>
N: \
-- Icel. _ólga_ "to foam, swell, seethe" (also cf. _ólgra_ "to ferment, boil") \
-- Far. _ulka_ "to vomit, be sick" \
-- Nw. _ulka_ "to be nauseated, feel sick" \
W: \
-- G _wulgern_ "to be nauseated" \
REF: [EDPG]\: 598

### <a id="wullō">\*wullō- [f.] "wool" (n)</a>
E: \
-- Go. _wulla_ \
N: \
-- ON _ull_ \
-- Far. _ull_ \
W: \
-- OE _wull_, E _wool_ \
-- OFri. _wolle_ \
-- Du. _wol_ [c.] \
-- OHG _wolla_, G _Wolle_ \
REF: [EDPG]\: 598

### <a id="wulþu">\*wulþu- [m.] "glory" (n)</a>
E: \
-- Go. _wulþus_ \
REF: [EDPG]\: 599

### <a id="wunan">\*wuna(n)- "accustomed" (adj)</a>
W: \
-- OE _ge-wun_ \
-- OS _gi-wono_, _gi-wuno_ \
-- Du. _ge-woon_ \
-- OHG _gi-won_, MHG _ge-won_, _ge-wone_ \
REF: [EDPG]\: 599

### <a id="wunda">\*wunda- "wounded" (adj)</a>
E: \
-- Go. _wunds_ \
W: \
-- OE _wund_ \
-- OS _wund_ \
-- MDu. _wont_ \
-- OHG _wunt_, MHG _wunde_ \
REF: [EDPG]\: 599

### <a id="wunēn">\*wunēn- [wk.] "to be used to" (v)</a>
E: \
-- Go. _un-wunands_ "discontent, unhappy" \
N: \
-- ON _una_ "to be content, to enjoy; to dwell, abide" \
-- Elfd. _uonå_ "to rest, stay" \
W: \
-- OE _wunian_ "to dwell, remain" \
-- OFri. _wona_, _wuna_ "id." \
-- OS _wonon_, _wunon_ "id." \
-- Du. _wonen_ "id." \
-- OHG _wonēn_ "to dwell, be used to", G _wohnen_ "to dwell, live" \
REF: [EDPG]\: 599

### <a id="wunni">\*wunni- [f.] "suffering" (n)</a>
E: \
-- Go. _wunns_ \
REF: [EDPG]\: 599

### <a id="wunskjan">\*wunskjan- [wk.] "to wish" (v)</a>
N: \
-- ON _œskja_ \
W: \
-- OE _wȳscan_, E _to wish_ \
-- MDu. _wunscen_, _wenscen_, Du. _wensen_ \
-- OHG _wunscan_, G _wünschen_ \
REF: [EDPG]\: 599

### <a id="wurda">\*wurda- [n.] "word" (n)</a>
E: \
-- Go. _waurd_ \
N: \
-- ON _orð_ \
W: \
-- OE _word_, E _word_ \
-- OFri. _word_ \
-- OS _word_ \
-- Du. _woord_ \
-- OHG _wort_, G _Wort_ \
REF: [EDPG]\: 600

### <a id="wurdi">\*wurdi- [f.] "fate, chance" (n)</a>
W: \
-- OE _wyrd_ \
-- OS _wurđ_ \
-- OHG _wurt_ \
REF: [EDPG]\: 600

### <a id="wurgjan">\*wurgjan- [wk.] "to strangle" (v)</a>
W: \
-- OE _wyrigan_, _wirgan_ \
-- OFri. _wergia_, WFri. _wjirgje_ \
-- Du. _wurgen_ \
-- OHG _wurgen_, G _würgen_ \
REF: [EDPG]\: 600

### <a id="wurkjan">\*wurkjan- [wk.] "to work, produce, effect" (v)</a>
E: \
-- Go. _waurkjan_ \
N: \
-- ON _yrkja_ "to work; to write verses" \
-- Far. _yrkja_ "to compose, write" \
W: \
-- OE _wyrcan_ "to work" \
-- OHG _wurchen_ "id.", MHG _wurken_, _würken_ "id." \
REF: [EDPG]\: 600

### <a id="wurmi">\*wurmi- [m.] "worm" (n)</a>
E: \
-- Go. _waurms_ "snake" \
N: \
-- ON _ormr_ "id." \
-- Far. _ormur_ "id." \
-- Elfd. _uorm_ "id." \
W: \
-- OE _wyrm_ "snake, worm" \
-- OFri. _wirm_, WFri. _wjirm_ [c.] \
-- OS _wurm_ \
-- Du. _worm_, _wurm_ [c.] \
-- OHG _wurm_ "worm, snake", G _Wurm_ "id." \
REF: [EDPG]\: 600

### <a id="wurti">\*wurti- [f.] "(medicinal) root; herb" (n)</a>
E: \
-- Go. _waurts_ "root" \
N: \
-- ON _urt_ "id." \
W: \
-- OE _wyrt_ "plant, herb" \
-- OS _wurt_ "id." \
-- MDu. _wort_ "id." \
-- OHG _wurz_ "root, herb, spice" \
REF: [EDPG]\: 601

### <a id="wurþi">\*wurþi- [f.] "mound, elevation" (n)</a>
N: \
-- ON _urð_ "heap of stones fallen from a hill" \
-- Far. _urð_ "scree; collection of stones or boulders" \
-- Elfd. _urd_ "stony slope" \
W: \
-- OFri. _wurth_ "raised homestead" \
-- OS _wurth_ "mound, elevated homestead" \
REF: [EDPG]\: 601
